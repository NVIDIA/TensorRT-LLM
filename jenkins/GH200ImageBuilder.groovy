...
def buildImage(action, type)
{
    ...
    docker.image(DOCKER_IMAGE).inside('-v /var/run/docker.sock:/var/run/docker.sock --privileged') {
        stage ("Install packages") {
            ...
            withCredentials([usernamePassword(credentialsId: "svc_tensorrt_gitlab_read_api_token", usernameVariable: 'USERNAME', passwordVariable: 'PASSWORD')]) {
                sh "docker login gitlab-master.nvidia.com:5005 -u ${USERNAME} -p ${PASSWORD}"
            }
        }
        try {
            ...
            stage ("Docker logout") {
                ...
                sh "docker logout gitlab-master.nvidia.com:5005"
            }
