    ...
    stage ("Setup Environment")
    {
        ...
        timeout(time: 45, unit: 'MINUTES') {
            ...
            if (stageName.contains("-Ray-")) {
                ...
                // These are prebuilt against one torch ABI, and upstream publishes
                // no build for every base image we run on, so on a mismatch the
                // extension loads with an undefined c10 symbol. Leaving a broken
                // install behind is worse than having none: transformers gates its
                // causal_conv1d import on a package-metadata probe, which a broken
                // install still passes, so the import blows up and takes the whole
                // module -- and every test collected from it -- down with it.
                // Uninstalled, the gate says no and the model falls back to its
                // Python path.
