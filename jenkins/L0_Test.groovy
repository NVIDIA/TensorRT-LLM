...
def rerunFailedTests(stageName, llmSrc, extraInternalEnv, pytestTestTimeout) {
    ...
    // Merge original results xml file with rerun results xml files for junit
    sh """
        python3 ${llmSrc}/tests/integration/defs/test_rerun.py \
        merge_junit_xmls \
        ${WORKSPACE}/${stageName}/results.xml \
    ...
    // Merge only rerun results xml files and convert to html
    sh """
        python3 ${llmSrc}/tests/integration/defs/test_rerun.py \
        merge_junit_xmls \
    ...
    echo "Test rerun report: https://urm.nvidia.com/artifactory/${UPLOAD_PATH}/${stageName}/rerun_results.xml"
}
