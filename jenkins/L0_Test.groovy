...
// Removed runIsolatedTests() - pytest-forked now handles ISOLATION tests automatically
// Removed processShardTestList() - pytest's --splits and --group parameters handle sharding directly
...
def generateRerunReport(stageName, llmSrc) {
    ...
    // Check if rerun directory has rerun_results_*.xml files
    def hasReruns = sh(script: "find ${rerunBaseDir} -name 'rerun_results_*.xml' 2>/dev/null | head -1 | grep -q . && echo 'true' || echo 'false'", returnStdout: true).trim() == 'true'
    ...
    // Add rerun results
    for (times in [1, 2]) {
        def rerunFile = "${rerunBaseDir}/rerun_results_${times}.xml"
...
def runLLMTestlistOnPlatformImpl(pipeline, platform, testList, config=VANILLA_CONFIG, perfMode=false, stageName="Undefined", splitId=1, splits=1, skipInstallWheel=false, cpver="cp312")
{
    ...
    stage ("[${stageName}] Run Pytest")
    {
        ...
        def pytestCommand = getPytestBaseCommandLine(
            ...
            coverageConfigFile,
            "",  // pytestUtil
            ["--test-list=${testDBList}", "--splitting-algorithm least_duration", "--splits ${splits}", "--group ${splitId}"],  // extraArgs
            containerPortStart,
            containerPortNum
        )
