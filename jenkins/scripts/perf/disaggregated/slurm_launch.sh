...
resourcePathNode=$7
pytestCommand=$8
coverageConfigFile=$9
container=${10}
mounts=${11}
scriptRunNode=${12}
scriptInstallNode=${13}
configYamlFile=${14}
testListPathNode=${15}
num_ctx_servers=${16}
num_gen_servers=${17}
gpus_per_node=${18}
gpus_per_ctx_server=${19}
gpus_per_gen_server=${20}
nodes_per_ctx_server=${21}
nodes_per_gen_server=${22}
total_nodes=${23}
total_gpus=${24}
...
if ! srun "${srunArgs[@]}" --kill-on-bad-exit=1 -N \$SLURM_NNODES --ntasks-per-node=1 --ntasks=\$SLURM_NNODES $scriptInstallNode &> $jobWorkspace/install.log; then
