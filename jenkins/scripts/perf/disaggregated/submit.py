...
def main():
    ...
    parser.add_argument(
        "--stage-name", required=True, help="Stage name (optional, local mode only)"
    )

    # Optional arguments for CI mode
    parser.add_argument(
        "--script-prefix", default="", help="Launch script prefix content (optional, CI mode only)"
    )
    parser.add_argument(
        "--srun-prologue",
        default="",
        help="Launch script srun prologue content (optional, CI mode only)",
    )
    parser.add_argument(
        "--srun-args",
        default="",
        help="Srun args as single string in launch script (optional, CI mode only)",
    )
    ...
    script_prefix_lines = []
    script_prologue_lines = []
    srun_args_lines = []
    if args.run_ci:
        script_prefix_lines = args.script_prefix.split("\n")
        script_prologue_lines = args.srun_prologue.split("\n")
        srun_args_lines = args.srun_args.split("\n")
    else:
    ...
    remove_whitespace_lines(script_prologue_lines)
    script_prologue = "\n".join(script_prologue_lines)
