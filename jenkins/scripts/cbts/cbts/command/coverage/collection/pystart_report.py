...
from cbts.coverage.collection.compact_db import SCHEMA_VERSION, canonicalize_path, merge_databases
...
def merge_to_sqlite(pattern: str, out_path: str) -> tuple[sqlite3.Connection, int]:
...
def _collect_defs(node: ast.AST, prefix: str, rel: str, funcs: set[tuple[str, str]]) -> None:
...
def enumerate_defs(source_root: str) -> tuple[set[str], set[tuple[str, str]]]:
...
def write_json(con: sqlite3.Connection, out_path: str) -> None:
...
@click.command("pystart-report", context_settings={"help_option_names": ["-h", "--help"]})
@click.option(
    "--glob",
    "glob_pattern",
...
@click.option("--out-sqlite", default=None, help="indexed compact touch DB for the selector")
@click.option(
    "--out-dir",
    default=None,
    help="directory for the split HTML report (index + per-file pages)",
)
@click.option("--out-json", default=None, help="optional full per-test -> [file::qualname] map")
@click.option(
    "--source-root",
    default=None,
    help="product tree (…/tensorrt_llm); when given, compute file/function coverage rate",
)
def main(glob_pattern, out_sqlite, out_dir, out_json, source_root):
    """Merge compact CBTS coverage databases into a compact touch DB and reports."""
    # Merge into a SQLite DB (bounded memory): --out-sqlite directly, else a unique per-invocation temp file.
    keep = out_sqlite is not None
    if keep:
        db_path = out_sqlite
    else:
        fd, db_path = tempfile.mkstemp(prefix="cbts_merge_", suffix=".sqlite")
        os.close(fd)
    con, n_data_files = merge_to_sqlite(glob_pattern, db_path)
    ...
    rate_line = ""
    if source_root:
        coverable_files, all_funcs = enumerate_defs(source_root)
        touched_files = {
    ...
    if out_json:
        write_json(con, out_json)
        print(f"wrote {out_json}")
    if out_dir:
        write_html_tree(out_dir, con, rate_line, n_tests, n_data_files)
