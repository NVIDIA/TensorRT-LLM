...
schema, producing the one DB consumed by `cbts.command main`.
...
`{path, meta}` — the two paths `cbts.command main` needs.
"""
...
from cbts.coverage.collection.compact_db import merge_databases
...
def prepare(dest_dir: str, pr_head: Optional[str]) -> Optional[dict]:
    ...
    `meta` is the selection JSON on disk, which `cbts.command main --coverage-db-meta` reads.
...
@click.command("artifact", context_settings={"help_option_names": ["-h", "--help"]})
@click.option(
    "--print-selection",
    "print_selection",
    is_flag=True,
...
@click.option("--build", type=int, default=None, help="pin a build number (skip auto-resolve)")
@click.option(
    "--prepare",
    "prepare_dir",
    metavar="DIR",
...
def main(print_selection, build, prepare_dir, pr_head):
    """Resolve which post-merge CBTS touch DBs to use."""
    if not print_selection and not prepare_dir:
        raise click.UsageError("one of --print-selection / --prepare is required")

    if prepare_dir:
        if build is not None or print_selection:
            raise click.UsageError("--prepare cannot be combined with --build or --print-selection")
        ready = prepare(prepare_dir, pr_head)
        if ready is None:
            sys.exit(1)
    ...
    if build is not None:
        urls = tarball_urls(build)
        if not all(_exists(url) for url in urls):
            sys.exit(1)
        commit = build_commit(build)
        best = {
            "url": urls[0],
            "urls": urls,
            "build": build,
