...
def _run(
    ...
    #
    # Reach `open_search_db` which is not on default path.
    # Immediately cleanup to not affect other imports.
    # jenkins/scripts/cbts/cbts/command/report_decision.py -> jenkins/scripts
    _jenkins_scripts = str(Path(__file__).resolve().parents[3])
    sys.path.insert(0, _jenkins_scripts)
    try:
        from open_search_db import CBTS_PROJECT_NAME, OpenSearchDB
    finally:
        sys.path.remove(_jenkins_scripts)
