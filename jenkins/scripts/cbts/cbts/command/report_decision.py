...
[--decision <decision JSON from `cbts.command main`>] [--pr-number <n>] [--repo-root <dir>].
...
from cbts.rules.base import format_reason
...
@click.command("report-decision", context_settings={"help_option_names": ["-h", "--help"]})
@click.option(
    "--status", required=True, help="pre_merge / post_merge / fallback / deferred / disabled."
)
@click.option("--reason", default="", help="Reason text (deferred only).")
@click.option("--decision", default=None, help="Path to `cbts.command main`'s decision JSON.")
@click.option("--pr-number", default="", help="PR / MR number for s_pr_number.")
@click.option("--repo-root", default=".", help="Repo root (for the case-rate counts).")
@click.option(
    "--multi-gpu-required",
    is_flag=True,
    default=False,
...
def main(
    ...
    repo_root,
    multi_gpu_required,
    multi_gpu_label_gate_open,
):
    """Post a CBTS decision to OpenSearch (CI-health monitoring).

    Never blocks CI: any failure past option parsing is logged and swallowed, exiting 0.
    """
...
def _run(
    ...
    # Lazy import, including the sys.path fix-up: any failure surfaces here and is caught
    # by the caller above, so telemetry never blocks CI. open_search_db imports cleanly
    # even without requests; postToOpenSearchDB falls back to urllib on pods (e.g. the
    # Setup Environment pod) that don't ship requests.
    #
    # `open_search_db` is a sibling of `cbts/` under jenkins/scripts, not part of the
    # package, so it needs its own directory on sys.path -- the one deliberate exception
    # to "cbts.* is the only importable thing on PYTHONPATH". Deferred to here (not module
    # level) so merely importing this module never mutates sys.path for whatever process
    # imported it (e.g. a test collecting cbts.command).
    # jenkins/scripts/cbts/cbts/command/report_decision.py -> jenkins/scripts
    sys.path.insert(0, str(Path(__file__).resolve().parents[3]))
    from open_search_db import CBTS_PROJECT_NAME, OpenSearchDB

    decision = json.loads(Path(decision_path).read_text()) if decision_path else {}
    multi_gpu_scheduled = _multi_gpu_scheduled(
        status,
        multi_gpu_required,
        multi_gpu_label_gate_open,
    ...
    doc = build_document(
        ...
        multi_gpu_label_gate_open=multi_gpu_label_gate_open,
    )
