...
- Every instrumented process saves a changed snapshot every `CBTS_PERIODIC_SAVE_SECONDS` (default 5s); unchanged snapshots are not rewritten. For immediate-capture MPI workers, this cadence begins only after the product import chain settles (bounded by `CBTS_WORKER_ACTIVATE_MAX_SECONDS`) so SQLite I/O does not delay worker bootstrap. Before result collection the pipeline creates `CBTS_STOP_FILE` (`<stage output dir>/cbts_stop`), which suppresses later saves, then waits for the directory mtime to settle so an in-flight save drains.
