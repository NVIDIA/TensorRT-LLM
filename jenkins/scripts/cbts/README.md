...
| Layer | Where | Action |
|---|---|---|
| **2. Stage** | `L0_Test.groovy::launchTestJobs` | Set `parallelJobsFiltered` to affected stages plus PackageSanityCheck (force-kept). Perf stages are excluded. Empty affectedSet → PackageSanityCheck only (trigger-mode mismatch). |
