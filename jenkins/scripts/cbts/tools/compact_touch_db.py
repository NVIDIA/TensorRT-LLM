...
def build_compact_database(source_path, output_path):
...
def _count(connection, sql):
    return connection.execute(sql).fetchone()[0]


def _allocated_bytes(connection):
...
def _run_queries(connection, sql, parameters):
    row_count = 0
...
def _benchmark_pair(source, compact, source_sql, compact_sql, parameters, repeats):
...
def benchmark_queries(source_path, compact_path, sample_size=100, repeats=5):
...
def _verify_equivalence(source, compact):
...
def evaluate(source_path, compact_path, build_seconds):
    source_path = Path(source_path).resolve()
...
def main():
    ...
    parser.add_argument("source", help="merged cbts_touchmap.sqlite")
    parser.add_argument("output", help="new compact SQLite path; must not already exist")
    parser.add_argument("--report-json", help="optional path for the JSON result")
    parser.add_argument(
        "--reuse-output",
        action="store_true",
    ...
    if args.reuse_output:
        if not Path(args.output).is_file():
            parser.error(f"compact output does not exist: {args.output}")
    else:
        build_compact_database(args.source, args.output)
    result = evaluate(args.source, args.output, time.perf_counter() - started)
    if args.benchmark:
        result["query_benchmark"] = benchmark_queries(args.source, args.output)
