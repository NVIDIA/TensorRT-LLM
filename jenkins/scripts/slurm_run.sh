...
if [ "${SLURM_JOB_NUM_NODES:-1}" -eq 1 ]; then
    # Unset MPI/SLURM variables (except SLURM_PROCID)
    for prefix in OMPI_ PMIX_ PMI_ SLURM_ MPI_ UCX_ I_MPI_ HYDRA_ KMP_ MPICH_ MV2_ CRAY_; do
        for v in $(env | grep "^$prefix" | cut -d"=" -f 1 || true); do
