...
class MagicFinder(PathFinder):
    ...
    @classmethod
    @contextlib.contextmanager
    def _recursive_guard(cls, fullname: str):
        """Claims ``fullname`` for the duration of the block.

        Yields True when the caller now owns the name and should go on to
        resolve it, and False when a resolution of that same name is already in
        flight on this thread, which is the recursion this guards against.
        Keyed per name so that resolving one module may still import another.
        """
        active_set = getattr(cls._resolving, "active_set", None)
        if active_set is None:
            active_set = set()
            cls._resolving.active_set = active_set
        if fullname in active_set:
            requester, _ = _requesting_source_file()
            if requester is None:
                requester = "<unknown>"
            cls._record(
                fullname,
                requester,
                "recursive-guard-fired",
            )
            yield False
        else:
            ...
            try:
                ...
                active_set.remove(fullname)

    @classmethod
    def _find_spec(cls, fullname, target=None):
    ...
    def find_spec(
        ...
        with cls._recursive_guard(fullname) as claimed:
            if claimed:
                return cls._find_spec(fullname, target)
            return None
