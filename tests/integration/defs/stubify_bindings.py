...
Used only by ``scripts/check_test_list.py`` so ``pytest --co`` can import product
...
Add ``_EXPLICIT`` entries only when Check Test List fails because a
callable or concrete type is required at import time. Dummy return
values (``-1``) are enough for collection. Prefer removing eager
imports in tests/conftest over growing this table.
...
def install_bindings_stub() -> StubModule:
    """Install stub modules for every ``_STUB_ROOTS`` entry into ``sys.modules``.

    Always wins over a compiled wheel already on ``sys.path``: Check Test List
    must collect against this checkout's Python, not against installed binaries
    that may expose a different API.
    """
