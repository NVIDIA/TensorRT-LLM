...
def run_disaggregated_test(example_dir,
    ...
    try:
        ...
        terminate(*ctx_workers, *gen_workers, disagg_server)
        # Debug escape hatch: keep the cluster work dir (worker/server
        # logs) for post-mortem when the env var is set. Combine with
        # TMPDIR pointing at persistent storage — work_dir comes from
        # tempfile.mkdtemp().
        if not os.environ.get("TLLM_TEST_KEEP_DISAGG_WORKDIR"):
            shutil.rmtree(work_dir, ignore_errors=True)
