    ...
    for suite in suite_list:
        ...
        for status, testcase in all_test_cases:
            test_rerun_sig = '[RERUN]' if testcase.attrib.get(
                'isrerun', '') == 'true' else ''
            test_name = f"{testcase.attrib.get('name', 'N/A')} - {testcase.attrib.get('classname', 'N/A')}"
            ...
            if status == "failure":
                ...
                details = f"""
                    <details class="test-details">
                        <summary>{test_name}<span class="test-rerun-sig">{test_rerun_sig}</span><span class="test-time">{test_time}</span></summary>
                        <pre>{''.join(f'<span>{line}</span>' for line in error.get('message', '').splitlines(True))}</pre>
                        <pre>{''.join(f'<span>{line}</span>' for line in (error.text or '').splitlines(True))}</pre>
                        <pre>{''.join(f'<span>{line}</span>' for line in (system_out.text or '').splitlines(True))}</pre>
                        <pre>{''.join(f'<span>{line}</span>' for line in (system_err.text or '').splitlines(True))}</pre>
                    </details>
                """
...
def generate_rerun_report(output_filename, input_filenames):
    ...
    for input_filename in input_filenames:
        ...
        if "rerun" not in input_filename:
            new_filename = "failed_" + input_filename
