...
# LMBenchmark lives at repo root (cloned alongside TRT-LLM)
...
def _run_lmbenchmark(
    ...
    # Grace window of 90s (was 30s) above the scripted duration. At high QPS
    # the LMBenchmark script needs extra time to drain its in-flight request
    # queue when the server is compute-bound; a too-tight deadline kills the
    # benchmark before its summary is written even though the server is fine.
    ...
    try:
        while proc.poll() is None:
            ...
            if now > deadline:
                return _kill(
                    f"LMBenchmark exceeded deadline ({duration + 90}s); server may have stalled"
                )
...
def _parse_csv_metrics(csv_path):
    ...
    result = {"num_requests": len(rows), "nan_count": nan_count}
    if ttfts:
        ttfts.sort()
...
def skip_if_model_missing():
    """Skip tests if the model weights are not available."""
    if not os.path.exists(MODEL_PATH):
        pytest.skip(f"Model not found: {MODEL_PATH}")


@pytest.fixture(scope="module")
def ensure_lmbenchmark(skip_if_model_missing):
    """Clone LMBenchmark if it is not already present."""
    if not os.path.exists(LMBENCHMARK_SCRIPT):
        subprocess.check_call(["git", "clone", "--depth", "1", LMBENCHMARK_REPO, LMBENCHMARK_DIR])
    if not os.path.exists(LMBENCHMARK_SCRIPT):
        pytest.skip(f"LMBenchmark script not found after clone: {LMBENCHMARK_SCRIPT}")
        ...
        with open(server_log, "w") as log_f:
            with popen(cmd, stderr=log_f, stdout=log_f, env=env) as proc:
                ...
                for qps in qps_values:
                    ...
                    print_info(f"Warmup before QPS={qps}...")
                    _run_and_assert_stage(
                        f"Warmup before QPS={qps}",
                        port,
                        str(tmp_path / f"warmup_1u_qps{qps}.csv"),
                        server_log,
                        qps=2,
                        num_users=1,
                        num_rounds=2,
