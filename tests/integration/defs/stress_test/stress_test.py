...
def launch_disaggregated_serving(disaggregated_server_config: Dict[str, Any],
    ...
    for i, port in enumerate(gen_ports):
        ...
        gen_server_args = [
            ...
            "pytorch", "--port",
            str(port), "--log_level", "verbose", "--extra_llm_api_options",
            gen_config_path, f"--tp_size={gen_tp}", f"--pp_size={gen_pp}"
        ]
...
@pytest.mark.parametrize("test_mode", ["stress-test-with-accuracy"],
                         ids=lambda x: x)
@pytest.mark.parametrize("stress_time_timeout", [(4000, 7200)],
                         ids=lambda x: f"stress_time_{x[0]}s_timeout_{x[1]}s")
@pytest.mark.parametrize(
    "model_config", [
...
def test_disaggregated_stress_test(model_config, test_mode,
    ...
    ctx_server_config = {
        ...
        "kv_cache_config": {
            ...
            "enable_block_reuse":
            False,  # Disable for stress test to avoid reuse tree race conditions
        },
    ...
    cuda_graph_config = {
        ...
        True,
        "batch_sizes":
        [1, 2, 4, 8, 16, 32, 64, 128, 256, 384, 512, 640, 768, 896, 1024],
    }
    ...
    is_healthy, _ = check_server_health(f"http://localhost:8000",
