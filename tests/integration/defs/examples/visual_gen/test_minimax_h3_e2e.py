...
"""Real-checkpoint quality gate for MiniMax-H3.

These tests never download MiniMax-H3; they use the copy already provisioned in
the shared CI model store.
...
The real-checkpoint cases resolve the weights from the shared CI model store
(``<llm_models_root>/MiniMax-H3``); ``MINIMAX_H3_CHECKPOINT`` overrides that for
a local run. A missing checkpoint fails rather than skips, so a registered test
can never report coverage it did not actually provide.

The tensor golden for the quality gate is not in the model store yet, so
``test_minimax_h3_t2va_diffusers_lpips_and_audio_golden`` still needs
``MINIMAX_H3_DIFFUSERS_GOLDEN`` and is not in ``l0_b200.yml``; register it once
that artifact is provisioned.
