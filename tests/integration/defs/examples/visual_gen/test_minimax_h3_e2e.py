...
Because no CI job sets ``MINIMAX_H3_E2E_AUTHORIZED``, the real-checkpoint cases
below are deliberately absent from ``l0_b200.yml``: listing a test that always
skips reports coverage that never runs. They are run manually::

    MINIMAX_H3_E2E_AUTHORIZED=1 \
    MINIMAX_H3_CHECKPOINT=/path/to/approved/MiniMax-H3 \
    pytest tests/integration/defs/examples/visual_gen/test_minimax_h3_e2e.py

Re-register them once the authorization is settled and they have been shown to
pass in CI -- in that order, so they are never enabled unvalidated.
