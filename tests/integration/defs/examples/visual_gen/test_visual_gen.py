...
def _test_ltx2_cuda_graph_trtllm_backend():
    ...
    scratch_space = conftest.llm_models_root()
    model_path = os.path.join(scratch_space, "LTX-2")
    text_encoder_path = _ltx2_lpips_text_encoder_path()
    ...
    try:
        ...
        assert output.video.numel() > 0
        assert output.audio.numel() > 0
...
@pytest.mark.skipif(not torch.cuda.is_available(), reason="CUDA not available")
def test_ltx2_cuda_graph_trtllm_backend():
    _test_ltx2_cuda_graph_trtllm_backend()
