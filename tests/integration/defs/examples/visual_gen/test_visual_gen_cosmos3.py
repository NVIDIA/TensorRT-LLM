...
def _run_cosmos3_lpips_pipeline(
    ...
    ``negative_prompt`` and ``max_sequence_length`` default to ``None``, which
    leaves the pipeline's own defaults in force. Callers whose golden predates
    the audio-output feature pass the pinned pre-audio values explicitly.
    """
