...
@filter_pytest_cases(
    ...
    # max throughput
    "enable_overlap_scheduler-enable_cuda_graph-enable_dp-nextn0-ep8-pp1-tp8-deepseek_r1-con32-bs32-enable_nvcc",
...
@pytest.mark.parametrize("concurrency", [1, 32, 1024], ids=lambda x: f"con{x}")
@pytest.mark.parametrize("model_name", ["DeepSeek-R1", "DeepSeek-V3"],
                         ids=["deepseek_r1", "deepseek_v3"])
@pytest.mark.parametrize("tp_size", [8], ids=["tp8"])
@pytest.mark.parametrize("pp_size", [1], ids=["pp1"])
