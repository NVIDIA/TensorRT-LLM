...
class RemoteOpenAIServer:
    ...
    def get_async_client(self, **kwargs):
        return openai.AsyncOpenAI(
            base_url=self.url_for("v1"), api_key=self.DUMMY_API_KEY, **kwargs)
...
@pytest.mark.asyncio
async def test_invalid_max_tokens(async_client: openai.AsyncOpenAI, model_name: str):
...
@pytest.mark.asyncio
async def test_invalid_temperature(async_client: openai.AsyncOpenAI, model_name: str):
    ...
    assert "temperature" in str(exc_info.value).lower() or "invalid" in str(exc_info.value).lower()


@pytest.mark.asyncio
async def test_invalid_top_p(async_client: openai.AsyncOpenAI, model_name: str):
    """Test that server rejects invalid top_p values."""
    print_info(f"Testing invalid top_p parameter: -0.1")
    ...
    assert "top_p" in str(exc_info.value).lower() or "invalid" in str(exc_info.value).lower()


@pytest.mark.asyncio
async def test_empty_messages_array(async_client: openai.AsyncOpenAI, model_name: str):
    ...
    assert "message" in str(exc_info.value).lower() or "empty" in str(exc_info.value).lower()


@pytest.mark.asyncio
async def test_missing_message_role(async_client: openai.AsyncOpenAI, model_name: str):
...
@pytest.mark.asyncio
async def test_invalid_token_ids(async_client: openai.AsyncOpenAI, model_name: str):
...
@pytest.mark.asyncio
async def test_extremely_large_token_id(async_client: openai.AsyncOpenAI, model_name: str):
...
@pytest.mark.asyncio
async def test_server_stability_under_invalid_requests(server: RemoteOpenAIServer, model_name: str):
    """
    ...
    print_info(f"Sent {error_count} invalid requests, all rejected as expected.")
...
@pytest.mark.asyncio
async def test_concurrent_invalid_requests(server: RemoteOpenAIServer, model_name: str):
    """
    ...
    print_info(f"All {len(results)} concurrent invalid requests rejected properly")
    
    # Verify server still works
    response = await async_client.chat.completions.create(
        model=model_name,
        messages=[{"role": "user", "content": "Final check"}],
        max_tokens=5
    )
    assert response is not None
...
@pytest.mark.asyncio
async def test_mixed_valid_invalid_requests(server: RemoteOpenAIServer, model_name: str):
    """
    ...
    async def send_request(request_id: int) -> dict:
        ...
        result = {"id": request_id, "success": False, "expected_error": False, "unexpected_error": False}
    ...
    print_info(f"Results: {successful} successful, {expected_errors} expected errors, {unexpected_errors} unexpected errors")
...
@pytest.mark.asyncio
async def test_health_check_during_errors(server: RemoteOpenAIServer, model_name: str):
    """
...
@pytest.mark.asyncio
async def test_request_exceeds_context_length(async_client: openai.AsyncOpenAI, model_name: str):
    ...
    try:
        ...
        assert "length" in str(e).lower() or "token" in str(e).lower() or "context" in str(e).lower()
...
def test_missing_content_type_header(server: RemoteOpenAIServer, model_name: str):
    ...
    assert response.status_code in [200, 400, 415]  # Success, Bad Request, or Unsupported Media Type


@pytest.mark.asyncio
async def test_extremely_large_batch(async_client: openai.AsyncOpenAI, model_name: str):
    ...
    try:
        ...
        assert "batch" in str(e).lower() or "too many" in str(e).lower() or "limit" in str(e).lower()
