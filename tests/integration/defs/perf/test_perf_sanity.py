...
def test_e2e(output_dir, perf_sanity_test_case):
    ...
    # Identify whether this pytest run is a step in a disagg 4-srun test, so
    # BENCHMARK can dump sibling logs and workers can drop a done-marker.
    is_disagg = config.runtime == "multi_node_disagg_server"
    disagg_serving_type = None
    if is_disagg:
        disagg_serving_type = config.server_configs[0][2].disagg_serving_type
    ...
    try:
        ...
        if is_disagg and disagg_serving_type != "BENCHMARK":
            print_info(
                f"Disagg serving type is {disagg_serving_type}, skipping perf result parsing and upload."
            )
            return
        ...
        if is_disagg:
            if disagg_serving_type == "BENCHMARK":
                # BENCHMARK: wait for sibling workers to finish, then dump their
                # logs into our stdout so CI (which only shows BENCHMARK's
                # output) has everything for debugging.
                marker_paths: List[str] = []
                log_paths: List[str] = []
                for server_idx in range(len(commands.server_cmds)):
                    marker_paths.extend(
