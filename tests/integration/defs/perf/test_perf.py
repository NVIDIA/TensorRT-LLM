...
class MultiMetricPerfTest(AbstractPerfScriptTestClass):
    ...
    def get_trtllm_build_command(self, engine_dir, checkpoint_dir) -> list:
        ...
        if self._config.max_batch_size != 2048:
            build_cmd.append(f"--max_batch_size={self._config.max_batch_size}")
        if self._config.max_num_tokens != 8192:
    ...
    def get_trtllm_bench_build_command(self, engine_dir) -> list:
        ...
        if self._config.max_batch_size != 2048:
            build_cmd.append(f"--max_batch_size={self._config.max_batch_size}")
        if self._config.max_num_tokens != 8192:
            build_cmd.append(f"--max_seq_len={self._config.max_num_tokens}")
    ...
    def get_model_yaml_config(self, model_label: str) -> dict:
        """
        ...
        model_configs = {
            'deepseek_r1-bench-pytorch-float16-maxbs:1-input_output_len:1000,2000-quant:fp8-reqs:10-ep:4-gpus:8':
            {
                'pytorch_backend_config': {
                    'enable_overlap_scheduler': True,
                    'use_cuda_graph': True,
            ...
            'deepseek_r1_nvfp4-bench-pytorch-float16-maxbs:1-input_output_len:1000,2000-quant:nvfp4-reqs:10-ep:4-tp:8-gpus:8':
            {
                'pytorch_backend_config': {
                    'enable_overlap_scheduler': True,
                    'use_cuda_graph': True,
    ...
    def get_trtllm_bench_command(self, engine_dir):
        ...
        if self._config.max_num_tokens != 8192:
            benchmark_cmd += [f"--max_num_tokens={self._config.max_num_tokens}"]
        ...
        if self._config.backend == "pytorch":
            import yaml
            config = self.get_model_yaml_config(self._config.to_string())
