...
class LogParser(object):
    ...
    def _extract_log(self, pattern: str, metric_names: List[str], log_content: str):
        ...
        for match in compiled.finditer(log_content):
            ...
            try:
                ...
                results.append(item)
                print(f"Successfully extracted: E2EL={values[0]}, TTFT={values[1]}, concurrency={concurrency}")
            except (ValueError, IndexError) as e:
                print(f"Error processing match: {e}")
                continue
        return results
    
    def parse(self, model_name: str, timestamps: Optional[Dict[str, str]] = None, 
        ...
        if len(raw_results) == 0:
            print(f"   ⚠️  No metrics extracted from log file")
            return {"status": False, "df": None}
    ...
    def _convert_to_perf_result_format(self, raw_results: List[dict], model_name: str,
        ...
        if timestamps:
            start_time = timestamps.get('start_timestamp', datetime.now().strftime('%Y-%m-%d %H:%M:%S'))
        ...
        expected_columns = [
            "network_name", "network_hash", "sm_clk", "mem_clk", "gpu_idx",
            "perf_case_name", "test_name", "original_test_name", "perf_metric",
            "total_time__sec", "start_timestamp", "end_timestamp", "state",
            "command", "threshold", "absolute_threshold", "metric_type"
        ]
