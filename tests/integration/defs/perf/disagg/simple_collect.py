...
def collect_system_info():
    ...
    # NVIDIA information
    try:
        result = subprocess.run(['nvidia-smi', '--query-gpu=driver_version,name,pci.device_id', '--format=csv,noheader,nounits'], 
        ...
        if lines and lines[0]:
            ...
            try:
                pci_device_id_int = int(pci_device_id, 16) if pci_device_id != 'unknown' else 'unknown'
            except:
                pci_device_id_int = 'unknown'
            
            data['gpu_properties'] = str(OrderedDict([
                ('device_product_name', gpu_name),
    ...
    # OS information
    data['os_properties'] = str(OrderedDict([
    ...
    data['cpu_properties'] = str(OrderedDict([
...
class SessionPropertiesWriter:
    ...
    def write_csv(self, data):
        ...
        fieldnames = ['username', 'start_timestamp', 'hostname', 'ip', 
                      'nvidia_driver_version', 'nvidia_device_count',
                      'os_properties', 'cpu_properties', 'gpu_properties',
                      'trt_change_id', 'trt_branch', 'commit_timestamp',
                      'cuda_version', 'cublas_version', 'cudnn_version', 'end_timestamp']
...
def main():
    ...
    print(f"  - session_properties.csv")
    print(f"  - gpu.txt")
    print(f"  - cpu.txt")
    print(f"  - driver.txt")
    print(f"  - trtllm_version.txt")
