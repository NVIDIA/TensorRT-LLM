...
class SlurmRunCommandBuilder:
    ...
    def build_script_command(self, cmd_type: str) -> List[str]:
        ...
        if cmd_type == SESSION_COLLECT_CMD_TYPE:
            ...
            #     return [
            #         "bash", "-c",
            #         f"cd {work_dir} && python3 {work_dir}/benchmark_collect.py --model-dir {model_dir} --output {output_path}"
            #     ]
...
class JobManager:
    ...
    @staticmethod
    def check_result(job_id: str, test_config, 
    ...
    def check_job_status(job_id: str) -> str:
        ...
        try:
            ...
            sacct_output = check_output(["sacct", "-j", job_id, "--noheader", "--format=State", "-X"], timeout=30)
    ...
    def wait_for_completion(job_id: str, timeout: int = 3600) -> bool:
        ...
        while time.time() - start_time < timeout:
            ...
            if status in ["COMPLETED", "FAILED", "CANCELLED", "TIMEOUT", "NODE_FAIL", "OUT_OF_MEMORY", "ERROR", "CANCELLED+"] or \
                ("error" in status.lower() and status != "ERROR"):
                if status == "COMPLETED":
    ...
    def get_log_and_yaml_files(directory: str) -> List[str]:
        ...
        files = [
            ...
            if f.is_file() and f.suffix in ['.log', '.yaml', '.yml']
        ]
