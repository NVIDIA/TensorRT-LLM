...
from common import EnvManager, SESSION_COLLECT_CMD_TYPE
...
class TestCaseTracker:
    ...
    def start_test_case(self, test_name: str):
        ...
        print(f"🚀 Test case started: {test_name} at {self.start_time.strftime('%Y-%m-%d %H:%M:%S')}")
    
    def end_test_case(self):
        """Record test case end time"""
        self.end_time = datetime.now()
        if self.test_name:
            print(f"✅ Test case ended: {self.test_name} at {self.end_time.strftime('%Y-%m-%d %H:%M:%S')}")
...
class SessionTracker:
    ...
    def end_and_collect(self):
        ...
        run_result = run_job(SESSION_COLLECT_CMD_TYPE, job_name)
        
        if run_result["status"]:
            # update timestamps in CSV
            self._update_csv_timestamps()
            print(f"📊 Session properties collected successfully")
        else:
