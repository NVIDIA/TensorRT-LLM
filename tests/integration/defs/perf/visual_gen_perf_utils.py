...
def get_visual_gen_match_keys(server_recipe_id: str = "") -> list[str]:
    """Return the match keys used for baseline/regression lookup."""
    if server_recipe_id:
        return COARSE_GRAINED_MATCH_KEYS.copy()
    return FINE_GRAINED_MATCH_KEYS.copy()
...
def build_visual_gen_db_entry(
    *,
    ...
    entry = {
        ...
        "s_server_recipe_id": str(server_recipe_id),
        ...
        "b_enable_parallel_vae": bool(
            _get_nested_value(server_config, "parallel.enable_parallel_vae", True)
        ),
        ...
        "s_test_case_name": f"{case_name}-{client_name}",
    }
