...
def tiny_llama_lora_base_model_root():
    ...
    Kept separate from `tiny_llama_model_root` because the LoRA adapter
    returned by `tiny_llama_lora_model_root` is fine-tuned specifically
    for TinyLlama-1.1B-Chat-v1.0 and has no Qwen3-0.6B equivalent.
    """
    models_root = llm_models_root()
    assert models_root, "Did you set LLM_MODELS_ROOT?"
