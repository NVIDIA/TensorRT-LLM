...
``build_engines.py`` / ``trtllm-build``). Instead of a prebuilt engine, the
model is served from the ``llmapi`` Triton model
(``triton_backend/all_models/llmapi/tensorrt_llm``): a ``model.yaml`` points at
a HuggingFace checkpoint that the PyTorch backend loads directly — no
checkpoint conversion or engine-build step.

Note: ``tensorrt_llm`` here is the Triton *model/backend directory name*, not
the (removed) TensorRT engine backend.
