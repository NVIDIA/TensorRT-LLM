...
Covers per-tensor FP8 qdq (Mixtral-8x7B) base weights, with several adapters of
varying rank applied in one batch. The adapters are fabricated on disk in the
per-expert key layout the TRT-LLM loader expects, so the tests do not depend on
a real PEFT export.
...
def test_mixtral_moe_routed_expert_fp8_multi_lora_varying_ranks(moe_lora_mode: str) -> None:
    ...
    It runs both routed-expert LoRA input schemas (which share the grouped-GEMM
    LoRA core) so the FP8 dequant is validated under each, selected by
    moe_lora_mode:
