...
class TestGLM53FlashFP8(LlmapiAccuracyTestHarness):
    """GLM-5.3-Flash (glm5_next): hybrid KDA + sparse-MLA layers.

    Only the Python NIXL transceiver moves the recurrent state and the indexer
    side cache between workers, so the C++ transceiver is not an option here.
    """
    MODEL_NAME = "zai-org/GLM-5.3-Flash"
    MODEL_PATH = f"{llm_models_root()}/GLM-5.3-Flash"

    @pytest.mark.skip_less_device(8)
    @parametrize_with_ids("mtp", [False, True])
    def test_fp8_nixl(self, mtp):
        # Import the selected agent, including the automatic Python fallback
        # when C++ bindings are unavailable. A Python-package-only check
        # would incorrectly skip builds that use the C++ agent.
        pytest.importorskip(
            "tensorrt_llm._torch.disaggregation.nixl.agent",
            reason="selected NIXL transfer agent is unavailable",
        ...
        # The checkpoint's single MTP layer chained for three drafts; both
        # workers carry it so the context worker hands over a replay-ready
        # KDA state and the generation worker verifies on the fused kernel.
        speculative_config = {
            "decoding_type": "MTP",
            "max_draft_len": 3,
        } if mtp else None
        ...
        with launch_disaggregated_llm(disaggregated_server_config,
            ...
            # fill in the quantization so the reference lookup matches the
            # registered FP8_BLOCK_SCALES entry.
            llm.args.quant_config.quant_algo = "FP8_BLOCK_SCALES"
            if mtp:
