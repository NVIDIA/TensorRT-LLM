...
"""Accuracy test for the KVCacheManagerV2 rebalance hook.
...
def _inject_pool_ratio_mismatch(llm: LLM, *, skew: float = 2.0) -> None:
    """Force the V2 auto-tuner to do real pool-resize work on the next
    rebalance call.
...
def _vswa_kv_cache_config(*, enable_rebalance: bool) -> KvCacheConfig:
    """V2 manager + explicit VSWA pattern that yields multiple pool
    groups for Gemma-3-1B (5 sliding-window layers : 1 full-attention
    layer).
    """
...
def _generate_tokens(*, model_path: str, disable_overlap: bool,
    ...
    with LLM(model_path,
             ...
             kv_cache_config=_vswa_kv_cache_config(
                 enable_rebalance=enable_rebalance)) as llm:
        if enable_rebalance:
...
class TestKvPoolRebalanceAccuracy:
    """Token-exact greedy-decode equivalence between rebalance=off and
    rebalance=on with a forced mid-generation adjust()."""

    MODEL_PATH = f"{llm_models_root()}/gemma/gemma-3-1b-it/"

    @pytest.mark.parametrize("disable_overlap", [True, False],
                             ids=["no_overlap", "overlap"])
    def test_rebalance_matches_baseline(self, disable_overlap, monkeypatch):
        ...
        baseline = _generate_tokens(model_path=self.MODEL_PATH,
                                    disable_overlap=disable_overlap,
                                    enable_rebalance=False)

        treated = _generate_tokens(model_path=self.MODEL_PATH,
                                   disable_overlap=disable_overlap,
