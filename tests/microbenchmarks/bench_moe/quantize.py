...
def _make_moe_test_utils_stub() -> types.ModuleType:
    stub = types.ModuleType("_torch.moe.moe_test_utils")
...
def _load_get_test_quant_params():
    ...
    dependency_name = "_torch.moe.moe_test_utils"
    ...
    try:
        quantize_utils = importlib.import_module("_torch.moe.quantize_utils")
    finally:
        if inserted_stub:
