...
def _time_moe_forward_eager(
    ...
    *,
    ...
    When ``nsys=True`` the timed iters loop is bracketed with
    ``cudaProfilerStart``/``Stop`` (AFTER warmup, so warmup is excluded) and each
    forward is wrapped with the ``bench_moe.measured`` NVTX range so an external
    ``nsys -c cudaProfilerApi`` run captures only the pure forward. ``nsys``
    forces ``collect_kernels`` off (CUPTI/Kineto conflicts with nsys).
    """
    if nsys:
