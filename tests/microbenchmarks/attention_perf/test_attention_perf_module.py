...
import pytest
import torch
from attention_perf_harness import AttnCase, collect_signals
...
# --------------------------------------------------------------------------- #
# golden store — JSON keyed by case_id -> arch -> value. CSV log for raw runs.
...
def _golden_for(case_id: str, arch: str):
    return _load_golden().get(case_id, {}).get(arch)
...
def _bootstrap_or_skip(case_id, arch, observed):
    golden = _golden_for(case_id, arch)
    if golden is None:
        pytest.skip(
            f"[bootstrap] no golden for {case_id} on {arch}. observed={observed!r}. "
...
def _bootstrap_continuous_msg(case_id, arch, median, cv, n):
...
def test_attn_ctx_dispatch():
    ...
    golden = _bootstrap_or_skip(_CTX.case_id, sig.arch, observed)
...
def test_attn_decode_dispatch():
    ...
    golden = _bootstrap_or_skip(_DEC.case_id, sig.arch, sig.launch_count)
    _record(sig, golden, verdict="checked")
...
def _run_continuous(case, what: str):
    ...
    golden_entry = _golden_for(case.case_id, sig.arch)
    if golden_entry is None:
        _record(sig, None, verdict="bootstrap", observed_cv=round(cv, 4))
        _bootstrap_continuous_msg(case.case_id, sig.arch, observed, cv, len(sig.raw_times_ms))
