...
Decode configs (seq_len_q != seq_len_kv) and e4m3 are skipped by the torch-op
backend; route those through --backend fmha-exe to use the legacy fmha.exe
harness as cross-validation.
"""
...
from tests.microbenchmarks.skip_softmax import bench_skip_softmax_fmha_exe  # noqa: E402
...
DEFAULT_FMHA_EXE = bench_skip_softmax_fmha_exe.DEFAULT_FMHA_EXE
PASS1_COLUMNS = [
    "config",
    "dtype",
    "batch",
...
def _load_configs(
    ...
    if threshold_sweep is None and batch_size is None:
        return cfgs
...
def pass1_sparsity(cfgs: list[FmhaConfig]) -> list[dict[str, object]]:
...
def pass2_speedup(cfgs: list[FmhaConfig]) -> list[dict[str, object]]:
    rows: list[dict[str, object]] = []
    if not cfgs:
        return rows
...
def main() -> int:
    ...
    cfgs = _load_configs(args.config, threshold_sweep, args.batch_size)
    if args.backend == "fmha-exe":
        return _run_fmha_exe_backend(args, cfgs)
