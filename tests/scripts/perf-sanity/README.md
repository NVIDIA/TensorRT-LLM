...
**Usage**:
```bash
python parse_benchmark_results.py <folder_name> [output_csv]
```

**Arguments**:
- `folder_name`: Folder containing benchmark log files (required)
- `output_csv`: Output CSV filename (optional, default: folder_name.csv)

**Output**:
- CSV file with benchmark results (default: `<folder_name>.csv` in parent directory)


**Examples**:
```bash
# Specify custom CSV filename
python parse_benchmark_results.py benchmark.run.12345.2024-01-15-10:30:00.abc123 my_results.csv
