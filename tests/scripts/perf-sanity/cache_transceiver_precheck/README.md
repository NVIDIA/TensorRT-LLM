...
| Same transceiver config | `CacheTransceiverConfig(**yaml["worker_config"][role]["cache_transceiver_config"])` — the YAML block is passed through verbatim. The E2E recipes omit `max_tokens_in_buffer`; DEFAULT/auto and explicit backend/runtime settings are preserved, along with timeouts and other configured options. |
| Same KV cache manager version + transceiver runtime | The launch generator forwards the real test's `LLM_MODELS_ROOT` into every precheck process. Explicit per-side `kv_cache_config.use_kv_cache_manager_v2` wins; absent means "auto" and requires a registered model class, then resolves via `get_preferred_kv_cache_manager_version()`. For configurations using `auto`, resolution uses the same llm_utils code as serving, including model preferences and fallback checks; explicit runtime selections remain unchanged. An unresolved manager-version `auto` setting fails with INIT_ERROR instead of silently assuming V1. V2 requires the Python transceiver (the C++ one only supports V1); a V2+CPP combination also fails fast. |
...
csv/ctx_<i>/<uuid>_<rank>.csv       # Python KVSendTask perf (throughput_mbs)
csv/gen_<i>/<uuid>_<rank>.csv       # Python KVRecvTask perf
csv/gen_<i>/<uuid>_<rank>_gen_transfer_summary.csv  # request transfer time and size
```
