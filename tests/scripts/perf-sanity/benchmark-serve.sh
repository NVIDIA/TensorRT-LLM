...
# Usage: sbatch benchmark-serve.sh [IMAGE] [bench_dir] [output_dir] [skip_pattern] [select_pattern]
#   IMAGE: Docker image to use (default: tensorrt-llm-staging/release:main-x86_64)
#   bench_dir: Directory containing config file and benchmark scripts (default: current directory)
#   skip_pattern: Pattern to skip test cases (default: none)
#   select_pattern: Pattern to select test cases (default: all)
    ...
    docker run --rm --ipc=host --ulimit memlock=-1 --ulimit stack=67108864 \
        ...
        bash -c "
            ...
            if [[ -f '${bench_dir}/parse_benchmark_results.py' ]]; then
                python3 ${bench_dir}/parse_benchmark_results.py ${output_folder} ${output_folder}.csv
