...
try:
    from tensorrt_llm._torch.cute_dsl_kernels.blackwell.top_k.gvr_topk_decode_cluster import (
        GvrTopKClusterKernel,
    )
except (ModuleNotFoundError, ImportError):
    sys.path.insert(0, str(Path(__file__).parents[4] / "tensorrt_llm/_torch/cute_dsl_kernels"))
    from blackwell.top_k.gvr_topk_decode_cluster import (
        GvrTopKClusterKernel,  # type: ignore[no-redef]
    )
...
def _tie_aware_correct(
    ...
    ``GvrTopKClusterKernel.gvr_topk_kernel``):
