...
def import_allowed_perf_config():
    if trt_environment:
        ...
        if ALLOWED_CONFIGS_CACHE is None:
            sys.path.append(
                (os.path.join(get_llm_root(),
                              "tests/llm-test-defs/turtle/defs/perf")))
...
class MultiMetricPerfTest(AbstractPerfScriptTestClass):
    ...
    def set_runtime_configs(self, llm_root, working_dir,
        ...
        if self._config.runtime == "bench":
            ...
            build_script = os.path.join(
                llm_root, "tests/llm-test-defs/turtle/defs/perf/build.py")
    ...
    def run_metrics(self, llm_venv, gpu_clock_lock, trt_session_data_writer,
                    output_dir):
        """
...
def run_perf_test(turtle_case_name, trt_performance_cache_fpath,
    ...
    test_runner = MultiMetricPerfTest(turtle_case_name)
