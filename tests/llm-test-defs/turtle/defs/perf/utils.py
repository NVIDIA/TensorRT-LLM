...
from trt_test.misc import check_output, print_error, print_info, temp_wd
...
from trt_test.test_list_parser import parse_test_list
    ...
    def run_cmd(self, cmd_idx: int, venv) -> str:
        ...
        if cmd_idx == 0:
            ...
            if self.is_python:
                ...
                # turtle itself manipulates os.environ['CUDA_VISIBLE_DEVICES'] to 0 even though user does not set it
                # this will cause the subprocess can only see 1 GPU, see https://nvbugs/4369573
                print(
...
class AbstractPerfScriptTestClass(abc.ABC):
    ...
    def run_ex(self,
               full_test_name: str,
               venv: Optional[VirtualenvRunner],
            ...
            if cmd_idx not in outputs:
                ...
                with io.StringIO() as buf:
                    ...
                    with contextlib.redirect_stdout(
                            buf), self._gpu_clock_lock, temp_wd(
    ...
    def _write_result(self, full_test_name: str,
                      trt_session_data_writer: SessionDataWriter,
                      output_dir: str, outputs: Dict[int, str],
                      original_test_name: str, cmd_idx: int) -> None:
        """
        ...
        test_description_dict = {
            ...
            "sm_clk": gpu_clocks.get("sm_clk", None),
            "mem_clk": gpu_clocks.get("mem_clk", None),
            "gpu_idx": gpu_idx
        }
...
def generate_test_nodes(session, config, items, valid_prefixes: List[str],
    ...
    substr_list = config._trt_config["substring_filter"]
    filter_files = config._trt_config["filter_file"]
    mako_opts = config._trt_config["mako_opts"] if config._trt_config[
        "mako_opts"] else {}
    test_prefix = config._trt_config["test_prefix"] if config._trt_config[
        "test_prefix"] else ""

    # Read all the test names.
    all_tests = []
    for test_list in filter_files:
        try:
