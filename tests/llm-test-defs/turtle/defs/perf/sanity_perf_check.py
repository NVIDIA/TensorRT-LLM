...
# csv.field_size_limit(sys.maxsize) # On Windows machine, this causes error "OverflowError: Python int too large to convert to C long"
...
class SanityPerfCheck():
    ...
    USEFUL_METRICS = [
        "original_test_name", "turtle_case_name", "metric_type", "perf_metric",
    ...
    def _parse_result(self, csv_path):
        ...
        with open(csv_path) as csv_file:
            ...
            for row in parsed_csv_file:
                ...
                result[row['metric_type']][row['turtle_case_name']] = float(
                    row['perf_metric'])
        return result
