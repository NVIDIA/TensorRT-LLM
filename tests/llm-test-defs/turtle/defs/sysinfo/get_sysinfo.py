...
try:
    chip_name_mapping_file = os.path.join(cur_dir, r"gpu_chip_mapping.json")
    with open(chip_name_mapping_file, "r") as fp:
        chip_name_mapping = json.load(fp)
except Exception:
    pass

# Load arch name mapping
try:
    arch_mapping_file = os.path.join(cur_dir, r"architecture_mapping.json")
    with open(arch_mapping_file, "r") as fp:
        arch_mapping = json.load(fp)
except Exception:
    pass
...
def try_set(write_dict, key, callable_func):
    try:
        ...
        print_warning("Unable to set mako variable {}: {}".format(key, e))
    return write_dict
...
def construct_gpu_properties(mako_opts, device_index=0):
    ...
    try:
        ...
        # Chip name and architecture name
        if pci_device_id is not None and chip_name_mapping:
            ...
            if entity in chip_name_mapping:
                ...
                # architecture name
                architecture = arch_mapping.get(chip[:2].lower(), "Unknown")
            else:
