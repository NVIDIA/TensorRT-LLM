...
from .test_list_parser import (TestCorrectionMode, apply_waives,
                               ...
                               modify_by_test_list)
from .trt_test_alternative import (altered_env, call, check_output, exists,
                                   is_windows, is_wsl, makedirs, print_info,
                                   print_warning, wsl_to_win_path)
...
@pytest.fixture(scope="session")
def llm_root(trt_config, gitlab_token):
    return get_llm_root(trt_config, gitlab_token)
...
# Set this env to true to let turtle run on GPU that is not in kSUPPORTED_GPU_LIST.
SKIP_GPU_CHECK = os.environ.get("SKIP_GPU_CHECK", False)

if USE_TURTLE:

    @pytest.fixture(scope="session")
    def llm_venv(trt_config, versions_from_infer_device,
                 trt_py_base_venv_factory, llm_root):
        """
        ...
        if SKIP_GPU_CHECK:
            ...
            for target in kSUPPORTED_GPU_LIST:
                target_gpu = [gpu for gpu in gpu_list if target in gpu]
                if len(target_gpu) > 0:
                    break

            assert len(target_gpu), f"supported GPU not found"

        device_set = set()

        for gpu in target_gpu:
            for i in range(9):
                if f"GPU {i}:" in gpu:
                    device_set.add(i)
                    break

        visible_devices = ",".join([str(i) for i in list(device_set)])

        print_info(f"Setting CUDA_VISIBLE_DEVICES to {visible_devices}.")

        if not trt_environment:
            with altered_env(CUDA_VISIBLE_DEVICES=visible_devices):
                venv = trt_py_base_venv_factory("python3")
                return venv

        nccl_root_dir = trt_environment.nccl_root()
        nccl_library_path = os.path.join(nccl_root_dir, "lib")
        trt_config["new_ld_library_path"] = os.pathsep.join(
            [nccl_library_path, trt_config["new_ld_library_path"]])

        hf_home = tempfile.mkdtemp()
        hf_home_src = Path(llm_models_root()) / "hugging-face-cache"
        shutil.copytree(hf_home_src, str(hf_home), dirs_exist_ok=True)

        with altered_env(CUDA_VISIBLE_DEVICES=visible_devices,
                         HF_HOME=str(hf_home)):
            venv = trt_py_base_venv_factory("python3")
            trt_environment.install_tensorrt_llm_to_venv(
                trt_config, venv, llm_root,
                versions_from_infer_device["cuda_version"])
...
def qcache_dir(llm_venv, llm_root):
    ...
    example_root = os.path.join(llm_root, "examples", "quantization")
...
def star_attention_input_root(llm_root):
    ...
    star_attention_input_root = os.path.join(llm_root, "tests", "_torch",
                                             "multi_gpu")
...
def test_case(request, llm_root):
    "get test case"
    test_cases_file = "tests/llm-test-defs/turtle/defs/test_cases.yml"
    input_file_dir = "tests/llm-test-defs/turtle/test_input_files"
...
def deterministic_test_root(llm_root, llm_venv):
    ...
    deterministic_root = os.path.join(
        llm_root, "tests/llm-test-defs/turtle/defs/deterministic")
...
def disaggregated_test_root(llm_root, llm_venv):
    ...
    disaggregated_root = os.path.join(
        llm_root, "tests/llm-test-defs/turtle/defs/disaggregated")
