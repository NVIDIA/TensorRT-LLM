...
sys.path.append(
    os.path.join(os.path.dirname(__file__), '../../../../examples/apps'))
...
def test_benchmark_sanity(llm_root, llm_venv, model_name, engine_dir):
    ...
    '''
    build_script_root = os.path.join(llm_root,
...
def test_llmapi_exit(llm_venv):
    llm_exit_script = Path(
        os.path.dirname(__file__)) / "../../../llmapi/run_llm_exit.py"
...
def test_llmapi_chat_example(llm_root, llm_venv):
    ...
    test_root = Path(os.path.join(llm_root, "tests", "llmapi", "apps"))
    llm_venv.run_cmd([
        "-m", "pip", "install", "-r",
...
def test_llmapi_quickstart_atexit(llm_root, engine_dir, llm_venv):
    script_path = Path(
        llm_root
    ) / "tests/llm-test-defs/turtle/defs/examples/run_llm_quickstart_atexit.py"
...
def test_llmapi_quant_llama_70b(llm_root, engine_dir, llm_venv):
    ...
    script_path = Path(
        llm_root
    ) / "tests/llm-test-defs/turtle/defs/examples/run_llm_fp8_quant_llama_70b.py"
