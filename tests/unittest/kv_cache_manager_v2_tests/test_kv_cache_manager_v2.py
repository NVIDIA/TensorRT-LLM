...
class TestNoBatching(TestKVCacheManagerV2):
    ...
    def test_resize_past_block_aligned_buffer(self) -> None:
        """Regression test: resize beyond block-aligned external buffer.

        Simulates the resource_manager.py code path where:
        1. resize_context aligns capacity to block boundary
        2. try_allocate_generation adds +1 token, requiring one more block
        3. The memoryview buffer (set by set_base_page_index_buf) must be
           large enough to hold max_blocks_per_seq + 1 blocks.

        Before the fix, max_blocks_per_seq = ceil(max_seq_len / tpb) was
        too small, causing: assert len(indices) >= new_num_blocks
        The fix adds +1 to max_blocks_per_seq in resource_manager.py.
        """
        tokens_per_block = 32
        ...
        req0 = self.new_request(0, None, prompt_len, decode_len)

        # Buffer sized to max_blocks_per_seq + 1 (the fixed calculation).
        # Without the +1, resize to block_aligned + 1 would fail with:
        #   assert len(indices) >= new_num_blocks  (_kv_cache.py:506)
        ...
        with TemporaryCudaStream([]) as s:
            ...
            # Simulate resize_context: grow to block-aligned capacity
            block_aligned = (max_num_blocks - 1) * tokens_per_block
            success = kv_cache.resize(block_aligned)
            assert success
            assert kv_cache.capacity == block_aligned

            # Simulate try_allocate_generation: +1 token past block boundary.
            # This requires (max_num_blocks - 1) + 1 = max_num_blocks blocks.
            # The +1 in buffer size ensures the memoryview is large enough.
            success = kv_cache.resize(block_aligned + 1)
            assert success, (
                "resize to block_aligned + 1 should succeed; "
                "buffer must accommodate max_blocks_per_seq + 1 blocks"
            )
