...
class TestKVCachePlacement(unittest.TestCase):
    """Tests for KVCacheManagerV2 request locality domain placement."""
    ...
    def test_prepare_context_localized_creation_uses_assigned_placement(self) -> None:
        ...
        stub._request_locality_domain_id = KVCacheManagerV2._request_locality_domain_id.__get__(
            stub
        )
        ...
        self.assertEqual(req.py_locality_domain_id, 1)
    ...
    def _make_create_kv_cache_stub(self, num_locality_domains: int) -> SimpleNamespace:
        from unittest.mock import MagicMock

        kv_cache = MagicMock()
        stub = SimpleNamespace(
            num_locality_domains=num_locality_domains,
            kv_cache_map={},
    ...
    def test_create_kv_cache_requires_locality_domain_id_when_localized(self) -> None:
        ...
        with self.assertRaises(AssertionError):
            KVCacheManagerV2._create_kv_cache(stub, 7, None, [1, 2, 3], locality_domain_id=None)
    ...
    def test_create_kv_cache_rejects_locality_domain_id_when_non_localized(self) -> None:
        ...
        with self.assertRaises(AssertionError):
            KVCacheManagerV2._create_kv_cache(stub, 7, None, [1, 2, 3], locality_domain_id=0)
    ...
    def test_add_dummy_requests_uses_request_id_placement(self) -> None:
        ...
        stub._create_kv_cache = MagicMock(return_value=kv_cache)
        stub.pick_locality_domain = KVCacheManagerV2.pick_locality_domain.__get__(stub)
        stub._request_locality_domain_id = KVCacheManagerV2._request_locality_domain_id.__get__(
            stub
        )
        ...
        self.assertEqual(requests[0].py_locality_domain_id, 1)
...
class TestLocalizedInvalidValueCheck(unittest.TestCase):
    def test_invalid_value_check_skips_localization_gap(self) -> None:
        ...
        self.assertEqual(
            seen_page_starts,
            [0, 0, 8, 8],
            "Localized invalid-value checks must only scan mapped per-locality domain page ranges",
        )
