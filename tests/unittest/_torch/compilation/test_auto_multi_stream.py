...
def _capture(graph: Graph, dest: Node, src: Node, layer: int) -> Node:
...
def _decoder_stack(
    n_layers: int, capture_layers: tuple[int, ...]
) -> tuple[GraphModule, Node, list[Node]]:
    """Build a chain of layers with selected outputs copied into ``dest``."""
    graph = Graph()
...
def _index_of(nodes: Iterable[T], predicate: Callable[[T], bool]) -> int:
...
def test_graph_exit_depends_on_unreturned_inplace_side_effect() -> None:
    """Keep unreturned mutations and their cross-stream waits before graph exit."""
    graph = Graph()
    dest = graph.placeholder("dest")
...
def test_every_capture_precedes_output(
    n_layers: int, capture_layers: tuple[int, ...], max_num_streams: int
) -> None:
    """Keep every hidden-state capture ahead of return in the emitted Python."""
