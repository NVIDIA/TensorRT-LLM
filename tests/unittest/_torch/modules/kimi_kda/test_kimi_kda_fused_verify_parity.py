...
@torch.no_grad()
def _make_runtime(seed, aux_stream=None, *, num_heads=H, use_full_rank_gate=True):
...
def test_fused_vs_sequential_two_rounds(num_heads, use_full_rank_gate):
    ...
    B = 4
    T = M + 1
    rt_seq = _make_runtime(seed=1, num_heads=num_heads, use_full_rank_gate=use_full_rank_gate)
