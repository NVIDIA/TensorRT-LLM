...
@pytest.mark.parametrize(
    "lens,h0_scale",
    [([1150], None), ([1150], 1.0), (_LONG_MIXED_LENS, None),
     (_LONG_MIXED_LENS, 1.0)],
    ids=["single_long", "single_long_state", "mixed_long", "mixed_long_state"],
)
@torch.no_grad()
def test_long_sequence_parity(dispatch_pair, gate_params, lens, h0_scale,
                              g_scale):
    optimized, reference = dispatch_pair
    n_seqs, total_t = len(lens), sum(lens)
    cu = torch.tensor(
        [0] + torch.cumsum(torch.tensor(lens), 0).tolist(), dtype=torch.long,
        device="cuda")
    q, k, v, g, beta, h0 = _make_inputs(1, total_t, h0_scale,
                                        seed=9000 + len(lens))
    g = (g.float() * g_scale).to(g.dtype)
    if h0 is not None:
        h0 = h0[:1].expand(n_seqs, -1, -1, -1).contiguous() * torch.linspace(
            0.5, 1.5, n_seqs, device="cuda").view(n_seqs, 1, 1, 1)
    out_opt, state_opt = _run(optimized, gate_params, q, k, v, g, beta, h0, cu)
    out_ref, state_ref = _run(reference, gate_params, q, k, v, g, beta, h0, cu)
...
def _nt(lens):
    return sum((l + 63) // 64 for l in lens)


@pytest.mark.parametrize("victim,poison", _POISON_STATE_CASES,
                         ids=["v8191_1", "v8000_150_42", "v1186_30"])
@torch.no_grad()
def test_final_state_parity_poisoned_scratch(dispatch_pair, gate_params,
                                             victim, poison):
    optimized, reference = dispatch_pair
    assert sum(victim) == sum(poison) and _nt(victim) == _nt(poison) and \
        len(victim) == len(poison), "pairs must share the buffer-cache key"
    total_t = sum(victim)

    # Poison pass: same buffer-cache key, huge magnitudes.
    q, k, v, g, beta, _ = _make_inputs(1, total_t, None, seed=31)
    cu_p = torch.tensor(
        [0] + torch.cumsum(torch.tensor(poison), 0).tolist(),
        dtype=torch.long, device="cuda")
    _run(optimized, gate_params, q * 100, k * 100, v * 1000, g, beta * 10,
         None, cu_p)
    ...
    h0 = h0[:1].expand(n_seqs, -1, -1, -1).contiguous() * torch.linspace(
        0.5, 1.5, n_seqs, device="cuda").view(n_seqs, 1, 1, 1)
    cu_v = torch.tensor(
        [0] + torch.cumsum(torch.tensor(victim), 0).tolist(),
        dtype=torch.long, device="cuda")
    out_opt, state_opt = _run(optimized, gate_params, q, k, v, g, beta, h0,
                              cu_v)
    out_ref, state_ref = _run(reference, gate_params, q, k, v, g, beta, h0,
                              cu_v)
...
def test_long_sequence_parity_tp_headcount(dispatch_pair, heads):
    ...
    h0 = rnd(len(lens), h, k, k, dtype=torch.float32) * torch.linspace(
        0.5, 1.5, k, device="cuda").view(1, 1, k, 1)

    def run(dispatch):
        return dispatch.prefill_chunk_kda(
            q=q.clone(), k=key.clone(), v=v.clone(), g=g.clone(),
            beta=beta.clone(), A_log=a_log, dt_bias=dt_bias,
            scale=k**-0.5, initial_state=h0.clone(), safe_gate=True,
            lower_bound=LOWER_BOUND, cu_seqlens=cu.clone())
...
def test_chunked_continuation_parity(dispatch_pair, gate_params):
    ...
    out1, state1 = _run(
        optimized, gate_params,
        q[:, sl1], k[:, sl1], v[:, sl1], g[:, sl1], beta[:, sl1], None, cu1)
    out2, state2 = _run(
        optimized, gate_params,
        q[:, sl2], k[:, sl2], v[:, sl2], g[:, sl2], beta[:, sl2], state1, cu2)
