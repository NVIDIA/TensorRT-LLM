...
def test_fused_moe_w4afp8(dtype, weight_loading_mode):
    ...
    with torch.device(f"cuda:{mapping.rank}"):
        ...
        for expert_id in range(NUM_EXPERTS):
            ...
            w1_pre_quant_scale = torch.rand(HIDDEN_SIZE, dtype=dtype, device="cuda") * 0.1 + 0.95
            w2_pre_quant_scale = (
                torch.rand(INTERMEDIATE_SIZE, dtype=dtype, device="cuda") * 0.1 + 0.95
            )
            w3_pre_quant_scale = torch.rand(HIDDEN_SIZE, dtype=dtype, device="cuda") * 0.1 + 0.95
        ...
        def ref():
            ...
            for e_idx in range(NUM_EXPERTS):
                ...
                # fc2
                fc2 = process_layer(fc1, w2, s2, p2, pre_quant_scale=a2, weight_scale_2=q2)
...
@pytest.mark.parametrize(
    ...
    [
        # smVersion
        pytest.param("TRTLLM", marks=[skip_blackwell_geforce, skip_pre_blackwell]),
        pytest.param("CUTLASS", marks=[skip_pre_hopper, skip_blackwell, skip_blackwell_geforce]),
    ],
)
    ...
    with torch.device(f"cuda:{mapping.rank}"):
        ...
        for expert_id in range(NUM_EXPERTS):
            w1_weight = torch.randint(
                0, 256, (INTERMEDIATE_SIZE, HIDDEN_SIZE // 2), dtype=torch.uint8, device="cuda"
            )
            w2_weight = torch.randint(
                0, 256, (HIDDEN_SIZE, INTERMEDIATE_SIZE // 2), dtype=torch.uint8, device="cuda"
            )
        ...
        # for FIXED_TACTIC_INDEX in [15, 16, 17, 18, 19, 20]:
        #     with AutoTuner.get().replay(tactics_list[FIXED_TACTIC_INDEX]), torch.inference_mode():
        #         output = fused_moe.forward(x, router_logits)
        #         i = 0
        #         for ctx_idx, (runner, tactic_value) in enumerate(tactics_list[FIXED_TACTIC_INDEX]):
        #             custom_op = all_tactics._captured_contexts[ctx_idx]['custom_op']
        #             gemm_idx = 1 if "gemm1" in custom_op else 2
        #             desc = cpp_runner.get_tactic_desc(gemm_idx, tactic_value)
        #             print(f"{i}  Op: {custom_op}, Runner: {type(runner).__name__}, Tactic: {tactic_value}")
        #             print(f"    -> {desc}", end='')
        #             i+=1
        ...
        for tactic in all_tactics:
            with AutoTuner.get().replay(tactic), torch.inference_mode():
                output = fused_moe.forward(x, router_logits)
                # i = 0
                # for ctx_idx, (runner, tactic_value) in enumerate(tactic):
                #     custom_op = all_tactics._captured_contexts[ctx_idx]['custom_op']
                #     gemm_idx = 1 if "gemm1" in custom_op else 2
                #     desc = cpp_runner.get_tactic_desc(gemm_idx, tactic_value)
                #     print(f"{i}  Op: {custom_op}, Runner: {type(runner).__name__}, Tactic: {tactic_value}")
...
def test_fused_moe_triton_mxfp4(
    experts, hidden_size, intermediate_size, fp8_activation, bias, dynamic_quant
):
    if fp8_activation:
    ...
    with torch.device(f"cuda:{mapping.rank}"):
        ...
        w1_weight = torch.randn((NUM_EXPERTS, INTERMEDIATE_SIZE, HIDDEN_SIZE), dtype=dtype).cuda()
        w2_weight = torch.randn((NUM_EXPERTS, HIDDEN_SIZE, INTERMEDIATE_SIZE), dtype=dtype).cuda()
        w3_weight = torch.randn((NUM_EXPERTS, INTERMEDIATE_SIZE, HIDDEN_SIZE), dtype=dtype).cuda()
