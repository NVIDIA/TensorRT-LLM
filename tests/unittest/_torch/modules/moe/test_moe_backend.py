...
def generate_element_wise_test_params() -> List:
    ...
    for activation_type in [ActivationType.Silu, ActivationType.Relu2]:
        for (
            ...
            [None, QuantAlgo.NVFP4, QuantAlgo.W8A16, QuantAlgo.W4A16],
        ):
            if skip_reason:
                continue
            ...
            if backend_type == MoeBackendType.TRTLLM and quant_algo is None:
                continue
            # Per-channel weight-only (INT8 and INT4) non-gated MoE is CUTLASS-path only.
            if (
                quant_algo in (QuantAlgo.W8A16, QuantAlgo.W4A16)
                and backend_type != MoeBackendType.CUTLASS
            ):
                continue
