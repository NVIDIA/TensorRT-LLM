...
races as required by the WideEP FT implementation plan §9.
"""
...
def test_initial_state_all_active(ep_size: int) -> None:
    h = EPGroupHealth(ep_size)
    assert h.ep_size == ep_size
    assert len(h) == ep_size
    assert h.get_active_count() == ep_size
    assert h.get_failed_ranks() == frozenset()
    assert h.all_active() is True
    assert h.generation == 0
    assert h.get_mask() == (1 << ep_size) - 1
...
def test_init_rejects_non_positive_ep_size(bad_size: int) -> None:
...
@pytest.mark.parametrize(
    "bad_rank",
    [-1, 8, 9, 100],
    ids=["neg_one", "at_ep_size", "above_ep_size", "way_above"],
)
...
def test_get_failed_ranks_returns_snapshot() -> None:
    ...
    # Snapshot is a frozenset — cannot be mutated by the caller.
...
def _expected_words(ep_size: int, num_words: int, fail_ranks: list[int]) -> tuple[int, ...]:
    """Reference computation: bit i set iff i < ep_size and i not in fail_ranks,
...
@pytest.mark.parametrize(
    ...
    [
        ...
        (72, 2, [70]),
        # extra words past ep_size are zero
        (8, 4, []),
        # ep_size > 128 (beyond default kernel ABI), one failure per word
        (256, 4, [10, 70, 130, 200]),
    ],
    ids=[
...
def test_get_mask_words_layout(ep_size: int, num_words: int, fail_ranks: list[int]) -> None:
    ...
    assert words == _expected_words(ep_size, num_words, fail_ranks)
...
@pytest.mark.parametrize(
    ...
    [
        ...
        (256, 2),  # ep_size > 128 does not fit in the default 2 words
...
def test_get_mask_words_rejects_too_few_words(ep_size: int, num_words: int) -> None:
    h = EPGroupHealth(ep_size)
    with pytest.raises(ValueError):
...
def test_concurrent_mark_failed_distinct_ranks() -> None:
    ...
    barrier = threading.Barrier(ep_size)
    ...
    threads = [threading.Thread(target=worker, args=(r,)) for r in range(ep_size)]
    for t in threads:
        t.start()
    for t in threads:
        t.join()
    ...
    assert h.get_failed_ranks() == frozenset(range(ep_size))
    assert h.get_mask() == 0
    assert h.generation == ep_size
...
def test_concurrent_mixed_fail_and_reactivate() -> None:
    ...
    all ranks active, with generation == 2 * ep_size. The reactivate thread
    ...
    barrier = threading.Barrier(ep_size * 2)
    fail_done = [threading.Event() for _ in range(ep_size)]
    ...
    fail_threads = [threading.Thread(target=fail, args=(r,)) for r in range(ep_size)]
    react_threads = [threading.Thread(target=reactivate, args=(r,)) for r in range(ep_size)]
...
def test_len_returns_ep_size_not_active_count() -> None:
    """__len__ is the EP group size; failures must not change it."""
    h = EPGroupHealth(8)
    assert len(h) == 8
    h.mark_failed(0)
    h.mark_failed(1)
    ...
    assert len(h) == 8  # ep_size does not
