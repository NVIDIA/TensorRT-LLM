...
def _import_mamba_kernels_full():
    ...
    from tensorrt_llm._torch.modules.mamba.checkpointing_state_update_slim import (
                ...
                for act_dtype in act_dtypes:
                    for sr_mode in sr_modes_list:
                        for mode in modes_list:
                            # write_checkpoint is per-slot from PNAT in both
                            # persistent modes; fix to True for the compile
                            # signature (the kernel always handles both).
                            for write_ckpt in [True]:
                                for rect in rect_list:
                                    # persistent_main / persistent_dynamic
                                    # both consume slot_perm and benefit from
                                    # write-first clustering when there's a
                                    # mix scenario.
                                    can_sort = args.mix_csv is not None
                                    effective_sort_list = (
...
def _bench_config(
    ...
    # Mix scenario: bench pre-bakes both a per-iter PNAT samples tensor
    # and (for persistent_main) a per-iter n_writes samples tensor;
    # grouped graph capture copies window rows into kernel-input tensors
    # before each in-graph L2 flush, so the timed kernels read PNAT cold.
        ...
        for (
            ...
            def _run_incr(
                ...
                if args.variant == "checkpointing":
                    ...
                    # by the search driver before the drop still parse —
                    # they always set REVN=0.
                    if state_scales_work is not None:
        ...
        for mtp_len in mtp_lengths:
            ...
            for state_dtype in state_dtypes:
                for act_dtype in act_dtypes:
                    for sr_mode in sr_modes_list:
                        for mode in modes_list:
                            # Persistent modes ignore write_checkpoint
                            # (per-slot from PNAT); fix to True.
                            for write_ckpt in [True]:
                                for rect in rect_list:
                                    # Sort/reverse only meaningful when a
                                    # mix scenario is present (the actual
                                    # sort experiment).  Both persistent
                                    # modes consume slot_perm.
...
def _parse_args() -> argparse.Namespace:
    ...
    parser.add_argument(
        ...
        "tail) and the persistent kernels read pid_b through that perm — "
        "clusters early-outs at one end of the grid.  Pure-batch cells "
        "skip sort=1 (no mix to sort).",
    )
    parser.add_argument(
        "--reverse-nowrite",
        type=str,
        ...
        "nowrite-side persistent kernel walks the perm in reverse so the "
        "two halves front-load real work from both ends.  reverse=1 with "
        "sort=0 is skipped (no perm to reverse).",
    )
    parser.add_argument(
        "--hardcode-sort",
        type=str,
