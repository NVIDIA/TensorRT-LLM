...
def _gdn_reference_work_items(state_indices, history_len):
...
class TestMamba2Metadata:
    ...
    @pytest.mark.parametrize("num_decodes", [1, 16, 17, 40, 255, 256, 257])
    def test_gdn_replay_work_items_match_reference(self, num_decodes):
        """The GDN builder preserves decode order and snapshots history."""
        ...
        torch.testing.assert_close(
            work_items, _gdn_reference_work_items(state_indices, history_len)
        )
        torch.testing.assert_close(n_writes.cpu(), torch.zeros(1, dtype=torch.int32))

    def test_prepare_gdn_replay_work_items_uses_decode_slice(self):
        ...
        manager = _GdnReplayCacheManager(history_len)
        ...
        expected_items = _gdn_reference_work_items(state_indices[num_contexts:], history_len)
    ...
    def test_prepare_gdn_replay_work_items_cuda_graph_replay(self):
        ...
        history_len.copy_((torch.arange(num_slots, dtype=torch.int32, device="cuda") * 7) % 21)
        graph.replay()
        torch.cuda.synchronize()

        expected_items = _gdn_reference_work_items(updated_state_indices, history_len)
        torch.testing.assert_close(metadata.replay_work_items[:num_decodes], expected_items)
        torch.testing.assert_close(
            metadata.replay_n_writes.cpu(), torch.zeros(1, dtype=torch.int32)
        )
