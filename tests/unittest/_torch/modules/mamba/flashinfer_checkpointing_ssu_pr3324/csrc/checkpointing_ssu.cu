...
void checkpointing_ssu(TensorView state, // (state_cache_size, nheads, dim, dstate)
    ...
    Optional<TensorView> cu_seqlens)
{                                             // (batch+1,) int32, varlen mode
    ...
    if (is_varlen)
    {
        ...
        CHECK_DIM(1, cs);
        FLASHINFER_CHECK(
            cs.size(0) >= 2, "cu_seqlens must have shape (batch+1,) with batch >= 1, got size(0)=", cs.size(0));
        FLASHINFER_CHECK(cs.dtype().code == kDLInt && cs.dtype().bits == 32, "cu_seqlens must be int32");
        CHECK_CONTIGUOUS(cs);
        batch = cs.size(0) - 1;
    ...
    FLASHINFER_CHECK(max_window == MAX_WINDOW, "old_x.size(1)=", max_window, " must equal JIT MAX_WINDOW=", MAX_WINDOW);
    FLASHINFER_CHECK(npredicted <= max_window, "npredicted=", npredicted, " must be <= max_window=", max_window);
    ...
    CHECK_DIM(4, state);
    {
        ...
        FLASHINFER_CHECK(
            s[2] == sz[3], "state dim 2 (dim) must be contiguous with dim 3, got stride ", s[2], " expected ", sz[3]);
        FLASHINFER_CHECK(s[1] == sz[2] * sz[3], "state dim 1 (nheads) must be contiguous with dim 2, got stride ", s[1],
            " expected ", sz[2] * sz[3]);
    }
    ...
    else
    {
        ...
        FLASHINFER_CHECK(x.size(1) == npredicted, "x.size(1)=", x.size(1), " must equal npredicted=", npredicted);
    }
    ...
    if (is_varlen)
    {
        ...
        FLASHINFER_CHECK(
            dt.size(1) == total_tokens, "varlen: dt.size(1)=", dt.size(1), " must equal x.size(1)=", total_tokens);
    }
    else
    {
        FLASHINFER_CHECK(dt.size(0) == batch, "dt.size(0)=", dt.size(0), " must equal batch=", batch);
        FLASHINFER_CHECK(dt.size(1) == npredicted, "dt.size(1)=", dt.size(1), " must equal npredicted=", npredicted);
    }
    ...
    if (is_varlen)
    {
        ...
        FLASHINFER_CHECK(
            B.size(1) == total_tokens, "varlen: B.size(1)=", B.size(1), " must equal x.size(1)=", total_tokens);
    }
    else
    {
        FLASHINFER_CHECK(B.size(0) == batch, "B.size(0)=", B.size(0), " must equal batch=", batch);
        FLASHINFER_CHECK(B.size(1) == npredicted, "B.size(1)=", B.size(1), " must equal npredicted=", npredicted);
    }
    FLASHINFER_CHECK(B.size(3) == dstate, "B.size(3)=", B.size(3), " must equal dstate=", dstate);
    CHECK_LAST_DIM_CONTIGUOUS(B);
    FLASHINFER_CHECK(B.stride(2) == dstate, "B.stride(2)=", B.stride(2), " must equal dstate=", dstate,
        " ((ngroups, dstate) must be contiguous)");
    FLASHINFER_CHECK(nheads % ngroups == 0, "nheads=", nheads, " must be divisible by ngroups=", ngroups);
    ...
    if (is_varlen)
    {
        ...
        FLASHINFER_CHECK(
            C.size(1) == total_tokens, "varlen: C.size(1)=", C.size(1), " must equal x.size(1)=", total_tokens);
    }
    else
    {
        FLASHINFER_CHECK(C.size(0) == batch, "C.size(0)=", C.size(0), " must equal batch=", batch);
        FLASHINFER_CHECK(C.size(1) == npredicted, "C.size(1)=", C.size(1), " must equal npredicted=", npredicted);
    }
    ...
    FLASHINFER_CHECK(C.stride(2) == dstate, "C.stride(2)=", C.stride(2), " must equal dstate=", dstate,
        " ((ngroups, dstate) must be contiguous)");
    ...
    {
        FLASHINFER_CHECK(output.size(0) == batch, "output.size(0)=", output.size(0), " must equal batch=", batch);
        FLASHINFER_CHECK(
            output.size(1) == npredicted, "output.size(1)=", output.size(1), " must equal npredicted=", npredicted);
    }
    FLASHINFER_CHECK(output.size(2) == nheads, "output.size(2)=", output.size(2), " must equal nheads=", nheads);
    FLASHINFER_CHECK(output.size(3) == dim, "output.size(3)=", output.size(3), " must equal dim=", dim);
    CHECK_LAST_DIM_CONTIGUOUS(output);
    FLASHINFER_CHECK(output.stride(2) == dim, "output.stride(2)=", output.stride(2), " must equal dim=", dim,
        " ((nheads, dim) must be contiguous)");
    ...
    FLASHINFER_CHECK(
        old_x.size(1) == max_window, "old_x.size(1)=", old_x.size(1), " must equal max_window=", max_window);
    FLASHINFER_CHECK(old_x.size(2) == nheads, "old_x.size(2)=", old_x.size(2), " must equal nheads=", nheads);
    FLASHINFER_CHECK(old_x.size(3) == dim, "old_x.size(3)=", old_x.size(3), " must equal dim=", dim);
    CHECK_LAST_DIM_CONTIGUOUS(old_x);
    FLASHINFER_CHECK(old_x.stride(2) == dim, "old_x.stride(2)=", old_x.stride(2), " must equal dim=", dim,
        " ((nheads, dim) must be contiguous)");
    ...
    FLASHINFER_CHECK(old_B.size(0) == state_cache_size, "old_B.size(0)=", old_B.size(0),
        " must equal state_cache_size=", state_cache_size);
    FLASHINFER_CHECK(old_B.size(1) == 2, "old_B.size(1) must be 2 (double-buffered), got ", old_B.size(1));
    FLASHINFER_CHECK(
        old_B.size(2) == max_window, "old_B.size(2)=", old_B.size(2), " must equal max_window=", max_window);
    FLASHINFER_CHECK(old_B.size(3) == ngroups, "old_B.size(3)=", old_B.size(3), " must equal ngroups=", ngroups);
    FLASHINFER_CHECK(old_B.size(4) == dstate, "old_B.size(4)=", old_B.size(4), " must equal dstate=", dstate);
    CHECK_LAST_DIM_CONTIGUOUS(old_B);
    FLASHINFER_CHECK(old_B.stride(3) == dstate, "old_B.stride(3)=", old_B.stride(3), " must equal dstate=", dstate,
        " ((ngroups, dstate) must be contiguous)");
    ...
    FLASHINFER_CHECK(old_dt.size(2) == nheads, "old_dt.size(2)=", old_dt.size(2), " must equal nheads=", nheads);
    FLASHINFER_CHECK(
        old_dt.size(3) == max_window, "old_dt.size(3)=", old_dt.size(3), " must equal max_window=", max_window);
    ...
    FLASHINFER_CHECK(old_cumAdt.size(0) == state_cache_size, "old_cumAdt.size(0)=", old_cumAdt.size(0),
        " must equal state_cache_size=", state_cache_size);
    FLASHINFER_CHECK(old_cumAdt.size(1) == 2, "old_cumAdt.size(1) must be 2, got ", old_cumAdt.size(1));
    FLASHINFER_CHECK(
        old_cumAdt.size(2) == nheads, "old_cumAdt.size(2)=", old_cumAdt.size(2), " must equal nheads=", nheads);
    ...
    FLASHINFER_CHECK(cache_buf_idx.size(0) == state_cache_size, "cache_buf_idx.size(0)=", cache_buf_idx.size(0),
        " must equal state_cache_size=", state_cache_size);
    ...
    FLASHINFER_CHECK(prev_num_accepted.size(0) == state_cache_size,
        "prev_num_accepted.size(0)=", prev_num_accepted.size(0), " must equal state_cache_size=", state_cache_size);
    ...
    if (dt_bias.has_value())
    {
        ...
        CHECK_CUDA(db);
        CHECK_DIM(2, db);
        FLASHINFER_CHECK(db.size(0) == nheads, "dt_bias.size(0)=", db.size(0), " must equal nheads=", nheads);
        FLASHINFER_CHECK(db.size(1) == dim, "dt_bias.size(1)=", db.size(1), " must equal dim=", dim);
        FLASHINFER_CHECK(db.stride(0) == 1, "dt_bias.stride(0) must be 1 (tie_hdim), got ", db.stride(0));
        FLASHINFER_CHECK(db.stride(1) == 0, "dt_bias.stride(1) must be 0 (tie_hdim), got ", db.stride(1));
    }
    ...
    if (z.has_value())
    {
        ...
        if (is_varlen)
        {
            ...
            FLASHINFER_CHECK(
                zv.size(1) == total_tokens, "varlen: z.size(1)=", zv.size(1), " must equal x.size(1)=", total_tokens);
        }
        else
        {
            FLASHINFER_CHECK(zv.size(0) == batch, "z.size(0)=", zv.size(0), " must equal batch=", batch);
            FLASHINFER_CHECK(zv.size(1) == npredicted, "z.size(1)=", zv.size(1), " must equal npredicted=", npredicted);
        }
    ...
    if (state_batch_indices.has_value())
    {
        ...
        CHECK_DIM(1, sbi);
        FLASHINFER_CHECK(
            sbi.size(0) == batch, "state_batch_indices.size(0)=", sbi.size(0), " must equal batch=", batch);
    ...
    if (state_scale.has_value())
    {
        ...
        FLASHINFER_CHECK(ss.size(0) == state_cache_size, "state_scale.size(0)=", ss.size(0),
            " must equal state_cache_size=", state_cache_size);
        FLASHINFER_CHECK(ss.size(1) == nheads, "state_scale.size(1)=", ss.size(1), " must equal nheads=", nheads);
        FLASHINFER_CHECK(ss.size(2) == dim, "state_scale.size(2)=", ss.size(2), " must equal dim=", dim);
        FLASHINFER_CHECK(ss.stride(2) == 1, "state_scale.stride(2) must be 1, got ", ss.stride(2));
        FLASHINFER_CHECK(ss.stride(1) == dim, "state_scale.stride(1)=", ss.stride(1), " must equal dim=", dim,
            " ((nheads, dim) must be contiguous)");
    }
    ...
    // Cache scalar tensors have fixed dtypes hardcoded in the kernel.
    {
        ...
        FLASHINFER_CHECK(old_dt.dtype().code == kDLFloat && old_dt.dtype().bits == 32, "old_dt must be float32");
        FLASHINFER_CHECK(
            old_cumAdt.dtype().code == kDLFloat && old_cumAdt.dtype().bits == 32, "old_cumAdt must be float32");
        // Index tensors used by the kernel as int32 scalars.
        FLASHINFER_CHECK(
            cache_buf_idx.dtype().code == kDLInt && cache_buf_idx.dtype().bits == 32, "cache_buf_idx must be int32");
        FLASHINFER_CHECK(prev_num_accepted.dtype().code == kDLInt && prev_num_accepted.dtype().bits == 32,
    ...
    FLASHINFER_CHECK(
        d_split == 1 || d_split == 2, "d_split=", d_split, " must be one of {1, 2} (d_split=4 is deferred to v12.x)");
    ...
    if (D.has_value())
        p.D = const_cast<void*>(D.value().data_ptr());
    if (z.has_value())
    {
    ...
    if (rand_seed.has_value())
    {
        ...
        p.rand_seed = static_cast<int64_t const*>(rs.data_ptr());
    }
    ...
    launchCheckpointingSsu<input_t, dt_t, weight_t, matrixA_t, state_t, stateIndex_t, state_scale_t>(p, stream);
}
