...
void checkpointing_ssu(TensorView state, // (cache, nheads, dim, dstate)
    TensorView x,                        // 4D (batch, T, nheads, dim) or 4D (1, total_tokens, nheads, dim) under varlen
    TensorView dt,     // (batch, T, nheads, dim) tie_hdim / (1, total_tokens, nheads, dim) under varlen
...
TVM_FFI_DLL_EXPORT_TYPED_FUNC(checkpointing_ssu, flashinfer::mamba::checkpointing::checkpointing_ssu);
