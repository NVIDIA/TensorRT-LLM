...
{
    __nv_bfloat16 val;
    float const fa = __bfloat162float(a);
    float const fb = __bfloat162float(b);
...
FLASHINFER_INLINE __nv_bfloat162 __floats2bfloat162_rn(float const a, float const b)
{
    __nv_bfloat162 val;
...
struct vec_cast
{
    ...
    FLASHINFER_INLINE static void cast(dst_t* dst, src_t const* src)
    {
#pragma unroll
...
{
    template <size_t vec_size>
    FLASHINFER_INLINE static void cast(__nv_fp8_e4m3* dst, float const* src)
    {
...
{
    template <size_t vec_size>
    FLASHINFER_INLINE static void cast(__nv_fp8_e5m2* dst, float const* src)
    {
        if constexpr (vec_size == 1)
        {
...
{
    template <size_t vec_size>
    FLASHINFER_INLINE static void cast(float* dst, half const* src)
    {
        if constexpr (vec_size == 1)
        {
            dst[0] = (float) src[0];
...
{
    template <size_t vec_size>
    FLASHINFER_INLINE static void cast(half* dst, float const* src)
    {
        if constexpr (vec_size == 1)
        {
...
__device__ void fast_dequant_f8f16x4(uint32_t* input, uint2* output)
{
    ...
    else
    {
        ...
        if constexpr (std::is_same_v<fp16_dtype, half>)
        {
            ...
            *(half2*) &(output->x) = __hmul2(*reinterpret_cast<half2 const*>(&Out1), bias_reg);
            *(half2*) &(output->y) = __hmul2(*reinterpret_cast<half2 const*>(&Out2), bias_reg);
        }
        else
        {
            ...
            *(nv_bfloat162*) &(output->x) = __hmul2(*reinterpret_cast<nv_bfloat162 const*>(&Out1), bias_reg);
            *(nv_bfloat162*) &(output->y) = __hmul2(*reinterpret_cast<nv_bfloat162 const*>(&Out2), bias_reg);
        }
    }
}
...
{
    template <size_t vec_size>
    FLASHINFER_INLINE static void cast(nv_bfloat16* dst, __nv_fp8_e4m3 const* src)
    {
        if constexpr (vec_size == 1)
        {
...
{
    template <size_t vec_size>
    FLASHINFER_INLINE static void cast(nv_bfloat16* dst, __nv_fp8_e5m2 const* src)
    {
        if constexpr (vec_size == 1)
        {
            dst[0] = nv_bfloat16(src[0]);
...
{
    template <size_t vec_size>
    FLASHINFER_INLINE static void cast(__nv_fp8_e4m3* dst, half const* src)
    {
...
{
    template <size_t vec_size>
    FLASHINFER_INLINE static void cast(__nv_fp8_e5m2* dst, half const* src)
    {
#ifdef FLASHINFER_HARDWARE_FP8_CONVERSION_ENABLED
        if constexpr (vec_size == 1)
        {
...
{
    template <size_t vec_size>
    FLASHINFER_INLINE static void cast(half* dst, __nv_fp8_e4m3 const* src)
    {
#ifdef FLASHINFER_HARDWARE_FP8_CONVERSION_ENABLED
        if constexpr (vec_size == 1)
        {
...
{
    template <size_t vec_size>
    FLASHINFER_INLINE static void cast(half* dst, __nv_fp8_e5m2 const* src)
    {
#ifdef FLASHINFER_HARDWARE_FP8_CONVERSION_ENABLED
        if constexpr (vec_size == 1)
        {
...
{
    template <size_t vec_size>
    FLASHINFER_INLINE static void cast(half* dst, __nv_fp4x2_e2m1 const* src)
    {
...
#pragma unroll
        for (size_t i = 0; i < vec_size / 2; ++i)
        {
            ...
            uint32_t b = reinterpret_cast<uint8_t const*>(src)[i * 2];
            asm volatile(
                "{\n"
                ".reg .b8 fp4_byte;\n"
...
#pragma unroll
        for (size_t i = 0; i < vec_size / 2; ++i)
        {
            uint8_t b = reinterpret_cast<uint8_t const*>(src)[i * 2];
...
{
    template <size_t vec_size>
    FLASHINFER_INLINE static void cast(nv_bfloat16* dst, __nv_fp4x2_e2m1 const* src)
    {
        static_assert(vec_size % 2 == 0, "vec_size must be even for fp4x2 dequantization");
#if (defined __CUDA_ARCH__) && (__CUDA_ARCH__ >= 1000)
#pragma unroll
...
{
    template <size_t vec_size>
    FLASHINFER_INLINE static void cast(float* dst, nv_bfloat16 const* src)
    {
        if constexpr (vec_size == 1)
        {
            dst[0] = (float) src[0];
...
{
    template <size_t vec_size>
    FLASHINFER_INLINE static void cast(nv_bfloat16* dst, float const* src)
    {
        if constexpr (vec_size == 1)
        {
            dst[0] = nv_bfloat16(src[0]);
...
struct vec_t
{
    ...
    FLASHINFER_INLINE float_t const& operator[](size_t i) const;
    FLASHINFER_INLINE void fill(float_t val);
    FLASHINFER_INLINE void load(float_t const* ptr);
    ...
    template <typename T>
    FLASHINFER_INLINE void cast_from(vec_t<T, vec_size> const& src);
    template <typename T>
    FLASHINFER_INLINE void cast_load(T const* ptr);
    template <typename T>
    FLASHINFER_INLINE void cast_store(T* ptr) const;
    FLASHINFER_INLINE static void memcpy(float_t* dst, float_t const* src);
    FLASHINFER_INLINE float_t* ptr();
};

template <typename src_float_t, typename tgt_float_t, size_t vec_size>
FLASHINFER_INLINE void cast_from_impl(vec_t<tgt_float_t, vec_size>& dst, vec_t<src_float_t, vec_size> const& src)
{
...
template <typename src_float_t, typename tgt_float_t, size_t vec_size>
FLASHINFER_INLINE void cast_load_impl(vec_t<tgt_float_t, vec_size>& dst, src_float_t const* src_ptr)
{
...
template <typename src_float_t, typename tgt_float_t, size_t vec_size>
FLASHINFER_INLINE void cast_store_impl(tgt_float_t* dst_ptr, vec_t<src_float_t, vec_size> const& src)
{
    if constexpr (std::is_same_v<src_float_t, tgt_float_t>)
    {
        src.store(dst_ptr);
...
struct vec_t<__nv_fp8_e4m3, 1>
{
    ...
    FLASHINFER_INLINE __nv_fp8_e4m3 const& operator[](size_t i) const
    {
        return ((__nv_fp8_e4m3 const*) (&data))[i];
    }
    ...
    FLASHINFER_INLINE void load(__nv_fp8_e4m3 const* ptr);
    FLASHINFER_INLINE void store(__nv_fp8_e4m3* ptr) const;

    template <typename T>
    FLASHINFER_INLINE void cast_from(vec_t<T, 1> const& src)
    {
    ...
    FLASHINFER_INLINE static void memcpy(__nv_fp8_e4m3* dst, __nv_fp8_e4m3 const* src);
};
...
FLASHINFER_INLINE void vec_t<__nv_fp8_e4m3, 1>::load(__nv_fp8_e4m3 const* ptr)
{
    data = *ptr;
}
...
FLASHINFER_INLINE void vec_t<__nv_fp8_e4m3, 1>::memcpy(__nv_fp8_e4m3* dst, __nv_fp8_e4m3 const* src)
{
    *dst = *src;
}
...
struct vec_t<__nv_fp8_e4m3, 2>
{
    ...
    template <typename T>
    FLASHINFER_INLINE void cast_from(vec_t<T, 2> const& src)
    {
        cast_from_impl(*this, src);
    }
...
FLASHINFER_INLINE void vec_t<__nv_fp8_e4m3, 2>::load(__nv_fp8_e4m3 const* ptr)
{
...
FLASHINFER_INLINE void vec_t<__nv_fp8_e4m3, 2>::memcpy(__nv_fp8_e4m3* dst, __nv_fp8_e4m3 const* src)
{
...
struct vec_t<__nv_fp8_e4m3, 4>
{
    ...
    template <typename T>
    FLASHINFER_INLINE void cast_from(vec_t<T, 4> const& src)
    {
        cast_from_impl(*this, src);
    }
...
FLASHINFER_INLINE void vec_t<__nv_fp8_e4m3, 4>::load(__nv_fp8_e4m3 const* ptr)
{
...
FLASHINFER_INLINE void vec_t<__nv_fp8_e4m3, 4>::memcpy(__nv_fp8_e4m3* dst, __nv_fp8_e4m3 const* src)
{
...
struct vec_t<__nv_fp8_e4m3, 8>
{
    ...
    template <typename T>
    FLASHINFER_INLINE void cast_from(vec_t<T, 8> const& src)
    {
        cast_from_impl(*this, src);
    }
...
FLASHINFER_INLINE void vec_t<__nv_fp8_e4m3, 8>::load(__nv_fp8_e4m3 const* ptr)
{
    data = *((uint2*) ptr);
}
...
FLASHINFER_INLINE void vec_t<__nv_fp8_e4m3, 8>::memcpy(__nv_fp8_e4m3* dst, __nv_fp8_e4m3 const* src)
{
    *((uint2*) dst) = *((uint2*) src);
}
...
#pragma unroll
        for (size_t i = 0; i < vec_size / 16; ++i)
        {
            ((__nv_fp8x4_e4m3*) (&(data[i].x)))->__x = (__nv_fp8x4_storage_t(val.__x) << 24)
                | (__nv_fp8x4_storage_t(val.__x) << 16) | (__nv_fp8x4_storage_t(val.__x) << 8)
...
struct vec_t<__nv_fp8_e5m2, 1>
{
    ...
    FLASHINFER_INLINE __nv_fp8_e5m2 const& operator[](size_t i) const
    {
        return ((__nv_fp8_e5m2 const*) (&data))[i];
    }
    ...
    FLASHINFER_INLINE void load(__nv_fp8_e5m2 const* ptr);
    ...
    FLASHINFER_INLINE static void memcpy(__nv_fp8_e5m2* dst, __nv_fp8_e5m2 const* src);
};
...
FLASHINFER_INLINE void vec_t<__nv_fp8_e5m2, 1>::load(__nv_fp8_e5m2 const* ptr)
{
    data = *ptr;
}
...
FLASHINFER_INLINE void vec_t<__nv_fp8_e5m2, 1>::memcpy(__nv_fp8_e5m2* dst, __nv_fp8_e5m2 const* src)
{
    *dst = *src;
}
...
FLASHINFER_INLINE void vec_t<__nv_fp8_e5m2, 2>::load(__nv_fp8_e5m2 const* ptr)
{
...
FLASHINFER_INLINE void vec_t<__nv_fp8_e5m2, 2>::memcpy(__nv_fp8_e5m2* dst, __nv_fp8_e5m2 const* src)
{
...
FLASHINFER_INLINE void vec_t<__nv_fp8_e5m2, 4>::load(__nv_fp8_e5m2 const* ptr)
{
...
FLASHINFER_INLINE void vec_t<__nv_fp8_e5m2, 4>::memcpy(__nv_fp8_e5m2* dst, __nv_fp8_e5m2 const* src)
{
...
FLASHINFER_INLINE void vec_t<__nv_fp8_e5m2, 8>::load(__nv_fp8_e5m2 const* ptr)
{
    data = *((uint2*) ptr);
}
...
FLASHINFER_INLINE void vec_t<__nv_fp8_e5m2, 8>::memcpy(__nv_fp8_e5m2* dst, __nv_fp8_e5m2 const* src)
{
    *((uint2*) dst) = *((uint2*) src);
}
...
struct vec_t<__nv_fp4_e2m1, 2>
{
    ...
    FLASHINFER_INLINE __nv_fp4_e2m1* ptr()
    {
        return reinterpret_cast<__nv_fp4_e2m1*>(&data);
    }
    ...
    FLASHINFER_INLINE void load(__nv_fp4_e2m1 const* ptr)
    {
        data = *((uint8_t*) ptr);
    }
    ...
    FLASHINFER_INLINE static void memcpy(__nv_fp4_e2m1* dst, __nv_fp4_e2m1 const* src)
    {
...
struct vec_t<__nv_fp4_e2m1, 16>
{
    ...
    template <typename T>
    FLASHINFER_INLINE void cast_from(vec_t<T, 16> const& src)
    {
        cast_from_impl(*this, src);
    }
...
struct vec_t<half, 1>
{
    ...
    FLASHINFER_INLINE half const& operator[](size_t i) const
    {
        return ((half const*) (&data))[i];
    }
    ...
    FLASHINFER_INLINE void load(half const* ptr);
    ...
    FLASHINFER_INLINE static void memcpy(half* dst, half const* src);
};
...
FLASHINFER_INLINE void vec_t<half, 1>::load(half const* ptr)
{
    data = *ptr;
}
...
FLASHINFER_INLINE void vec_t<half, 1>::memcpy(half* dst, half const* src)
{
    *dst = *src;
}
...
FLASHINFER_INLINE void vec_t<half, 2>::load(half const* ptr)
{
    data = *((half2*) ptr);
}
...
FLASHINFER_INLINE void vec_t<half, 2>::memcpy(half* dst, half const* src)
{
    *((half2*) dst) = *((half2*) src);
}
...
FLASHINFER_INLINE void vec_t<half, 4>::load(half const* ptr)
{
    data = *((uint2*) ptr);
}
...
FLASHINFER_INLINE void vec_t<half, 4>::memcpy(half* dst, half const* src)
{
    *((uint2*) dst) = *((uint2*) src);
}
...
struct vec_t<nv_bfloat16, 1>
{
    ...
    FLASHINFER_INLINE nv_bfloat16 const& operator[](size_t i) const
    {
        return ((nv_bfloat16 const*) (&data))[i];
    }
    ...
    FLASHINFER_INLINE void load(nv_bfloat16 const* ptr);
    ...
    FLASHINFER_INLINE static void memcpy(nv_bfloat16* dst, nv_bfloat16 const* src);
};
...
FLASHINFER_INLINE void vec_t<nv_bfloat16, 1>::load(nv_bfloat16 const* ptr)
{
    data = *ptr;
}
...
FLASHINFER_INLINE void vec_t<nv_bfloat16, 1>::memcpy(nv_bfloat16* dst, nv_bfloat16 const* src)
{
    *dst = *src;
}
...
FLASHINFER_INLINE void vec_t<nv_bfloat16, 2>::load(nv_bfloat16 const* ptr)
{
...
FLASHINFER_INLINE void vec_t<nv_bfloat16, 2>::memcpy(nv_bfloat16* dst, nv_bfloat16 const* src)
{
...
FLASHINFER_INLINE void vec_t<nv_bfloat16, 4>::load(nv_bfloat16 const* ptr)
{
    data = *((uint2*) ptr);
}
...
FLASHINFER_INLINE void vec_t<nv_bfloat16, 4>::memcpy(nv_bfloat16* dst, nv_bfloat16 const* src)
{
    *((uint2*) dst) = *((uint2*) src);
}
...
struct vec_t<uint8_t, 1>
{
    ...
    FLASHINFER_INLINE uint8_t const& operator[](size_t i) const
    {
        return ((uint8_t const*) (&data))[i];
    }

    FLASHINFER_INLINE uint8_t* ptr()
    {
        return reinterpret_cast<uint8_t*>(&data);
    }

    FLASHINFER_INLINE void fill(uint8_t val);
    FLASHINFER_INLINE void load(uint8_t const* ptr);
    ...
    FLASHINFER_INLINE static void memcpy(uint8_t* dst, uint8_t const* src);
};

FLASHINFER_INLINE void vec_t<uint8_t, 1>::fill(uint8_t val)
{
    data = val;
}

FLASHINFER_INLINE void vec_t<uint8_t, 1>::load(uint8_t const* ptr)
{
    data = *ptr;
}
...
FLASHINFER_INLINE void vec_t<uint8_t, 1>::memcpy(uint8_t* dst, uint8_t const* src)
{
    *dst = *src;
}
...
FLASHINFER_INLINE void vec_t<uint8_t, 2>::load(uint8_t const* ptr)
{
    data = *((uint16_t*) ptr);
}
...
FLASHINFER_INLINE void vec_t<uint8_t, 2>::memcpy(uint8_t* dst, uint8_t const* src)
{
    *((uint16_t*) dst) = *((uint16_t*) src);
}
...
FLASHINFER_INLINE void vec_t<uint8_t, 4>::load(uint8_t const* ptr)
{
    data = *((uint32_t*) ptr);
}
...
FLASHINFER_INLINE void vec_t<uint8_t, 4>::memcpy(uint8_t* dst, uint8_t const* src)
{
    *((uint32_t*) dst) = *((uint32_t*) src);
}
...
FLASHINFER_INLINE void vec_t<uint8_t, 8>::load(uint8_t const* ptr)
{
    data = *((uint2*) ptr);
}
...
FLASHINFER_INLINE void vec_t<uint8_t, 8>::memcpy(uint8_t* dst, uint8_t const* src)
{
    *((uint2*) dst) = *((uint2*) src);
}
...
struct vec_t<float, 1>
{
    ...
    FLASHINFER_INLINE float const& operator[](size_t i) const
    {
        return ((float const*) (&data))[i];
    }
    ...
    FLASHINFER_INLINE void load(float const* ptr);
    ...
    FLASHINFER_INLINE static void memcpy(float* dst, float const* src);
};
...
FLASHINFER_INLINE void vec_t<float, 1>::load(float const* ptr)
{
    data = *ptr;
}
...
FLASHINFER_INLINE void vec_t<float, 1>::memcpy(float* dst, float const* src)
{
    *dst = *src;
}
...
FLASHINFER_INLINE void vec_t<float, 2>::load(float const* ptr)
{
    data = *((float2*) ptr);
}
...
FLASHINFER_INLINE void vec_t<float, 2>::memcpy(float* dst, float const* src)
{
