...
template <typename input_t, typename state_t_, int NPREDICTED_, int MAX_WINDOW_, int D_PER_CTA, int DSTATE>
struct CheckpointingSsuStorage8bit
{
    ...
    static constexpr int NPREDICTED_SWIZZLE_R = next_multiple_of<SmemSwizzle<input_t>::ATOM_ROWS>(NPREDICTED);
...
__device__ __forceinline__ uint8_t state_byte_of(state_t v)
{
    ...
    {
        static_assert(std::is_same_v<state_t, __nv_fp8_e4m3>, "8-bit state_t must be int8_t or __nv_fp8_e4m3");
...
__forceinline__ __device__ auto convert_layout_acc_Aregs_sm80(Layout acc_layout)
{
    ...
    using X = Underscore;
    static_assert(decltype(size<0, 0>(acc_layout))::value == 2, "C-frag inner mode must be (col_pair=2, row_pair=2)");
    static_assert(decltype(size<0, 1>(acc_layout))::value == 2, "C-frag inner mode must be (col_pair=2, row_pair=2)");
    static_assert(decltype(rank(acc_layout))::value == 3, "C-frag must be rank-3 ((C0,C1), MMA_M, MMA_N)");
    ...
    return make_layout(make_layout(get<0, 0>(l), get<0, 1>(l), get<2, 0>(l)), // ((col_pair, row_pair, k_half))
        get<1>(l),                                                            // MMA_M
...
template <typename input_t, typename state_t, int DIM, int D_PER_CTA, int DSTATE, typename SmemT, typename FragYDxT>
__device__ __forceinline__ void replay_state_mma_8bit_chain(SmemT& smem, CheckpointingSsuParams const& params, int warp,
    int lane, int prev_k, int d_tile, int64_t cache_slot, int head, bool must_checkpoint, FragYDxT& frag_y_DxT,
    float (&encode_scale_per_row_out)[2], float (&total_scale_out)[2])
{
    ...
    static_assert(sizeof(state_t) == 1, "replay_state_mma_8bit_chain is for 1-byte state_t (int8/fp8) only");
    static_assert(D_PER_CTA == 64, "replay_state_mma_8bit_chain requires D_PER_CTA == 64 (M-shard, per-warp M=16).");
    ...
    using MmaAtomReplayType
        = std::conditional_t<MAX_WINDOW_PAD_MMA_K == MMA_prop::K_BIG, MMA_prop::AtomK16, MMA_prop::AtomK8>;
    using LdsmA = std::conditional_t<MAX_WINDOW_PAD_MMA_K == MMA_prop::K_BIG, SM75_U16x8_LDSM_T, SM75_U16x4_LDSM_T>;
    using LdsmB = std::conditional_t<MAX_WINDOW_PAD_MMA_K == MMA_prop::K_BIG, SM75_U16x4_LDSM_T, SM75_U16x2_LDSM_T>;
    ...
    auto tiled_mma_replay = make_tiled_mma(MMA_Atom<MMA_Traits<MmaAtomReplayType>>{}, Layout<Shape<_4, _1>>{});
    ...
    auto tiled_mma_chain = make_tiled_mma(MMA_Atom<MMA_Traits<MMA_prop::AtomK16>>{}, Layout<Shape<_4, _1>>{});
    ...
    int64_t const state_scale_base
        = cache_slot * params.state_scale_stride_seq + (int64_t) head * DIM + (int64_t) d_tile * D_PER_CTA;
    ...
    auto layout_A_full = make_swizzled_layout_rc_transpose<input_t, MAX_WINDOW_PAD_MMA_K, D_SMEM_COLS>();
    Tensor smem_A_full
        = make_tensor(make_smem_ptr(reinterpret_cast<MMA_prop::operand_t const*>(smem.old_x)), layout_A_full);
    Tensor smem_A
        = local_tile(smem_A_full, make_shape(Int<D_PER_CTA>{}, Int<MAX_WINDOW_PAD_MMA_K>{}), make_coord(_0{}, _0{}));
    ...
    Tensor frag_A_replay = thr_mma_replay.partition_fragment_A(
        make_tensor((MMA_prop::operand_t*) 0x0, make_shape(Int<D_PER_CTA>{}, Int<MAX_WINDOW_PAD_MMA_K>{})));
    ...
    Tensor smem_B_full
        = make_tensor(make_smem_ptr(reinterpret_cast<MMA_prop::operand_t const*>(smem.old_B)), layout_B_replay);
    ...
    auto layout_C_swz = make_aliased_swizzled_layout_rc<input_t, NPREDICTED_PAD_MMA_M, DSTATE, SmemT::NPREDICTED>();
    Tensor smem_C = make_tensor(make_smem_ptr(reinterpret_cast<MMA_prop::operand_t const*>(smem.C)), layout_C_swz);
    auto s2r_B_chain = make_tiled_copy_B(Copy_Atom<SM75_U32x2_LDSM_N, MMA_prop::operand_t>{}, tiled_mma_chain);
...
#pragma unroll
    for (int kpair = 0; kpair < NUM_K_PAIRS; ++kpair)
    {
        ...
        Tensor a_kpair = thr_mma_chain.partition_fragment_A(
            make_tensor((MMA_prop::operand_t*) 0x0, make_shape(Int<D_PER_CTA>{}, Int<MMA_prop::K_BIG>{})));
        static_assert(decltype(size(a_kpair))::value == 8, "a_kpair must hold 1 m16n8k16 K-atom of A = 8 bf16/thread");
...
#pragma unroll
        for (int local_n = 0; local_n < 2; ++local_n)
        {
            ...
            static_assert(
                decltype(size(frag_h))::value == FRAG_SIZE, "FRAG_SIZE must match the partitioned C-fragment size");
            ...
            Tensor smem_B_n = local_tile(
                smem_B_full, make_tile(Int<N_PER_PASS>{}, Int<MAX_WINDOW_PAD_MMA_K>{}), make_coord(n, _0{}));
            auto smem_B_s2r_n = s2r_thr_B_replay.partition_S(smem_B_n);
            Tensor frag_B_replay = thr_mma_replay.partition_fragment_B(
                make_tensor((MMA_prop::operand_t*) 0x0, make_shape(Int<N_PER_PASS>{}, Int<MAX_WINDOW_PAD_MMA_K>{})));
...
#pragma unroll
        ...
        Tensor smem_C_k = local_tile(
            smem_C, make_tile(Int<NPREDICTED_PAD_MMA_M>{}, Int<MMA_prop::K_BIG>{}), make_coord(_0{}, kpair));
        auto smem_C_k_s2r = s2r_thr_B_chain.partition_S(smem_C_k);
        Tensor b_kpair = thr_mma_chain.partition_fragment_B(
            make_tensor((MMA_prop::operand_t*) 0x0, make_shape(Int<NPREDICTED_PAD_MMA_M>{}, Int<MMA_prop::K_BIG>{})));
...
#pragma unroll
    ...
    {
        per_thread_amax[i]
            = fmaxf(per_thread_amax[i], __shfl_xor_sync(constants::MASK_ALL_LANES, per_thread_amax[i], 1));
        per_thread_amax[i]
            = fmaxf(per_thread_amax[i], __shfl_xor_sync(constants::MASK_ALL_LANES, per_thread_amax[i], 2));
    }
...
template <typename input_t, typename state_t, int DIM, int D_PER_CTA, int DSTATE, int PHILOX_ROUNDS, typename SmemT>
__device__ __forceinline__ void encode_state_replay_8bit(SmemT& smem, CheckpointingSsuParams const& params, int warp,
    int lane, int prev_k, int d_tile, int64_t cache_slot, int head, float const (&encode_scale_per_row)[2],
    float const (&total_scale)[2], int64_t rand_seed, int64_t state_ptr_offset)
{
    ...
    static_assert(sizeof(state_t) == 1, "encode_state_replay_8bit is for 1-byte state_t (int8/fp8) only");
    static_assert(D_PER_CTA == 64, "encode_state_replay_8bit requires D_PER_CTA == 64 (M-shard, per-warp M=16).");
...
#pragma unroll
    for (int n = 0; n < NUM_N_PASSES; ++n)
    {
        ...
        Tensor smem_B_n
            = local_tile(smem_B_full, make_tile(Int<N_PER_PASS>{}, Int<MAX_WINDOW_PAD_MMA_K>{}), make_coord(n, _0{}));
        auto smem_B_s2r_n = s2r_thr_B_replay.partition_S(smem_B_n);
        Tensor frag_B_replay = thr_mma_replay.partition_fragment_B(
            make_tensor((MMA_prop::operand_t*) 0x0, make_shape(Int<N_PER_PASS>{}, Int<MAX_WINDOW_PAD_MMA_K>{})));
        auto frag_B_replay_view = s2r_thr_B_replay.retile_D(frag_B_replay);
        ...
        // caller's __syncthreads before the cooperative store_state call.
        {
            ...
            if constexpr (PHILOX_ROUNDS > 0)
            {
                ...
                if (rand_pos == 0)
                {
                    int64_t const philox_off = state_ptr_offset + (int64_t) row_lo * DSTATE + (frag_col_base + n_base);
                    conversion::philox_randint4x<PHILOX_ROUNDS>(
                        rand_seed, philox_off, rand_idx[0], rand_idx[1], rand_idx[2], rand_idx[3]);
                }
                ...
                if constexpr (std::is_same_v<state_t, int8_t>)
                {
                    packed = conversion::cvt_rs_sat_s8x4_f32(
                        frag_h(0) * e0, frag_h(1) * e0, frag_h(2) * e1, frag_h(3) * e1, rand_idx[rand_pos]);
                }
                else
                {
                    static_assert(
                        std::is_same_v<state_t, __nv_fp8_e4m3>, "8-bit SR supports state_t in {int8_t, __nv_fp8_e4m3}");
                    packed = conversion::cvt_rs_e4m3x4_f32(
                        frag_h(0) * e0, frag_h(1) * e0, frag_h(2) * e1, frag_h(3) * e1, rand_idx[rand_pos]);
                }
            ...
            else
            {
                ...
                Pair<state_t> q_lo;
                q_lo.raw
                    = static_cast<uint16_t>(state_byte_of(q0_lo)) | (static_cast<uint16_t>(state_byte_of(q1_lo)) << 8);
                *reinterpret_cast<Pair<state_t>*>(&state_base[off_lo]) = q_lo;
                ...
                Pair<state_t> q_hi;
                q_hi.raw
                    = static_cast<uint16_t>(state_byte_of(q0_hi)) | (static_cast<uint16_t>(state_byte_of(q1_hi)) << 8);
                *reinterpret_cast<Pair<state_t>*>(&state_base[off_lo + 1024]) = q_hi;
            }
        }
    }
...
template <typename input_t, int NPREDICTED, int DIM, int D_PER_CTA, int DSTATE, int NUM_WARPS, typename SmemT,
    typename FragYDxT>
__device__ __forceinline__ void compute_output_8bit(SmemT& smem, CheckpointingSsuParams const& params, int warp,
    int lane, int d_tile, int64_t out_seq_base, int head, int64_t cache_slot, float D_val, int seq_len,
    FragYDxT& frag_y_DxT)
{
    using namespace cute;
    ...
    auto layout_x_trans = make_swizzled_layout_rc_transpose<input_t, NPREDICTED_PAD_MMA_M, D_SMEM_COLS>();
    Tensor smem_x_trans
        = make_tensor(make_smem_ptr(reinterpret_cast<MMA_prop::operand_t const*>(smem.x)), layout_x_trans);
    Tensor smem_x_trans_tile
        = local_tile(smem_x_trans, make_shape(Int<D_PER_CTA>{}, Int<NPREDICTED_PAD_MMA_M>{}), make_coord(_0{}, _0{}));
    ...
    auto layout_z = make_aliased_swizzled_layout_rc<input_t, NPREDICTED_PAD_MMA_M, D_SMEM_COLS, SmemT::NPREDICTED>();

    // CB_scaled (T, T_pad) within (NPREDICTED_PAD_MMA_M, CB_ROW_STRIDE).
    auto layout_cb = make_swizzled_layout_rc<input_t, NPREDICTED_PAD_MMA_M, NPREDICTED_PAD_MMA_M, CB_ROW_STRIDE>();
    Tensor smem_CB
        = make_tensor(make_smem_ptr(reinterpret_cast<MMA_prop::operand_t const*>(smem.CB_scaled)), layout_cb);
...
#pragma unroll
    ...
    auto s2r_A_x = make_tiled_copy_A(Copy_Atom<SM75_U16x8_LDSM_T, MMA_prop::operand_t>{}, tiled_mma_chain);
    auto s2r_thr_A_x = s2r_A_x.get_slice(tid);
    auto smem_x_s2r = s2r_thr_A_x.partition_S(smem_x_trans_tile);
    Tensor frag_A_x = thr_mma_chain.partition_fragment_A(
        make_tensor((MMA_prop::operand_t*) 0x0, make_shape(Int<D_PER_CTA>{}, Int<NPREDICTED_PAD_MMA_M>{})));
    ...
    auto s2r_B_CB = make_tiled_copy_B(Copy_Atom<SM75_U32x2_LDSM_N, MMA_prop::operand_t>{}, tiled_mma_chain);
    ...
    Tensor frag_B_CB = thr_mma_chain.partition_fragment_B(
        make_tensor((MMA_prop::operand_t*) 0x0, make_shape(Int<NPREDICTED_PAD_MMA_M>{}, Int<NPREDICTED_PAD_MMA_M>{})));
...
#pragma unroll
    for (int i = 0; i < size(frag_y_DxT); ++i)
    {
        ...
        if (t < seq_len)
        {
            ...
            smem_out_base[t * kSmemRowStride + d] = pack_float2<input_t>(make_float2(frag_y_DxT(i), 0.f))[Int<0>{}];
        }
    }
    ...
    if (stg_t < seq_len)
    {
        ...
        using Vec = uint4;
        *reinterpret_cast<Vec*>(&output_ptr[gmem_off]) = *reinterpret_cast<Vec const*>(&smem_out_base[smem_off]);
    }
}
...
template <typename input_t, typename state_t, int DIM, int D_PER_CTA, int DSTATE, typename SmemT, typename TiledMma,
    typename ThrMma, typename... FragY>
__device__ __forceinline__ void add_init_out_8bit(
    SmemT const& smem, int warp, int lane, TiledMma const& tiled_mma, ThrMma const& thr_mma, int tid, FragY&... frag_y)
{
    using namespace cute;
    ...
    Tensor smem_C_ktiled
        = local_tile(smem_C, make_tile(Int<NPREDICTED_PAD_MMA_M>{}, Int<K_TILE>{}), make_coord(_0{}, _));
...
#pragma unroll
    for (int k = 0; k < NUM_K_TILES; ++k)
    {
        ...
        for (int n = 0; n < NUM_N_TILES; ++n)
        {
            ...
            Pair<MMA_prop::operand_t> const b_lo
                = pack_float2<MMA_prop::operand_t>(make_float2(toFloat(p_lo[Int<0>{}]), toFloat(p_lo[Int<1>{}])));
            Pair<MMA_prop::operand_t> const b_hi
                = pack_float2<MMA_prop::operand_t>(make_float2(toFloat(p_hi[Int<0>{}]), toFloat(p_hi[Int<1>{}])));
...
template <typename input_t, typename state_t, int NPREDICTED, int MAX_WINDOW, int DIM, int D_PER_CTA, int DSTATE,
    int NUM_WARPS, typename SmemT>
__device__ __forceinline__ void compute_no_write_output_8bit(SmemT& smem, CheckpointingSsuParams const& params,
    int warp, int lane, int prev_k, int d_tile, int64_t out_seq_base, int head, int64_t cache_slot, float D_val,
    int seq_len)
{
    using namespace cute;
    ...
    auto tiled_mma = make_tiled_mma(MMA_Atom<MMA_Traits<MMA_prop::AtomK16>>{}, Layout<Shape<_1, _4>>{});
    auto thr_mma = tiled_mma.get_slice(tid);

    // ── TiledMMA for matmul-4-old: K = MAX_WINDOW_PAD_MMA_K ∈ {8, 16} → atom dispatch ──
    using MmaAtomOld = std::conditional_t<MAX_WINDOW_PAD_MMA_K == MMA_prop::K_BIG, MMA_prop::AtomK16, MMA_prop::AtomK8>;
    using LdsmAOld = std::conditional_t<MAX_WINDOW_PAD_MMA_K == MMA_prop::K_BIG, SM75_U32x4_LDSM_N, SM75_U32x2_LDSM_N>;
    using LdsmBOld = std::conditional_t<MAX_WINDOW_PAD_MMA_K == MMA_prop::K_BIG, SM75_U16x4_LDSM_T, SM75_U16x2_LDSM_T>;
    ...
    Tensor smem_x = make_tensor(make_smem_ptr(reinterpret_cast<MMA_prop::operand_t const*>(smem.x)), layout_x_swz);
    auto layout_x_trans_swz = make_swizzled_layout_rc_transpose<input_t, NPREDICTED_PAD_MMA_M, D_SMEM_COLS>();
    Tensor smem_x_trans
        = make_tensor(make_smem_ptr(reinterpret_cast<MMA_prop::operand_t const*>(smem.x)), layout_x_trans_swz);

    auto layout_old_x_trans_swz = make_swizzled_layout_rc_transpose<input_t, MAX_WINDOW_PAD_MMA_K, D_SMEM_COLS>();
    Tensor smem_old_x_trans
        = make_tensor(make_smem_ptr(reinterpret_cast<MMA_prop::operand_t const*>(smem.old_x)), layout_old_x_trans_swz);

    auto layout_z_swz = make_aliased_swizzled_layout_rc<input_t, NPREDICTED_PAD_MMA_M, D_SMEM_COLS, NPREDICTED>();
    Tensor smem_z = make_tensor(make_smem_ptr(reinterpret_cast<input_t const*>(smem.z)), layout_z_swz);

    // ── S2R copies (matmul-4-new) ──
    auto s2r_A = make_tiled_copy_A(Copy_Atom<SM75_U32x4_LDSM_N, MMA_prop::operand_t>{}, tiled_mma);
    auto s2r_thr_A = s2r_A.get_slice(tid);
    auto s2r_B_trans = make_tiled_copy_B(Copy_Atom<SM75_U16x2_LDSM_T, MMA_prop::operand_t>{}, tiled_mma);
    ...
    auto s2r_B_old_trans = make_tiled_copy_B(Copy_Atom<LdsmBOld, MMA_prop::operand_t>{}, tiled_mma_old);
    auto s2r_thr_B_old_trans = s2r_B_old_trans.get_slice(tid);

    // ── Load CB_scaled A operand (cols [0, NPREDICTED_PAD_MMA_M)) ──
    auto layout_cb_swz = make_swizzled_layout_rc<input_t, NPREDICTED_PAD_MMA_M, NPREDICTED_PAD_MMA_M, CB_ROW_STRIDE>();
    Tensor smem_CB
        = make_tensor(make_smem_ptr(reinterpret_cast<MMA_prop::operand_t const*>(smem.CB_scaled)), layout_cb_swz);
    ...
    Tensor smem_CB_full
        = make_tensor(make_smem_ptr(reinterpret_cast<MMA_prop::operand_t const*>(smem.CB_scaled)), layout_cb_full);
    Tensor smem_CB_old = local_tile(smem_CB_full, make_tile(Int<NPREDICTED_PAD_MMA_M>{}, Int<MAX_WINDOW_PAD_MMA_K>{}),
    ...
    Tensor decay_bcast = make_tensor(make_smem_ptr(smem.cumAdt),
    ...
    static_assert(NUM_N_TILES == 2, "compute_no_write_output_8bit assumes NUM_N_TILES == 2 (D_PER_CTA=64, N_TILE=32)");
...
#pragma unroll
        ...
        add_cb_old_x<input_t, MMA_prop::operand_t, N_TILE, MAX_WINDOW_PAD_MMA_K>(frag_y, frag_CB_old_A,
            smem_old_x_trans, s2r_B_old_trans, s2r_thr_B_old_trans, thr_mma_old, tiled_mma_old, n);
        ...
        auto gOut_tile = make_tensor(make_gmem_ptr(output_ptr + out_base + n * N_TILE),
            make_layout(
                make_shape(Int<NPREDICTED_PAD_MMA_M>{}, Int<N_TILE>{}), make_stride(params.out_stride_token, _1{})));
...
#pragma unroll
    ...
    add_init_out_8bit<input_t, state_t, DIM, D_PER_CTA, DSTATE>(
        smem, warp, lane, tiled_mma, thr_mma, tid, frag_y_0, frag_y_1);
    epilogue(frag_y_0, 0);
    epilogue(frag_y_1, 1);
}
...
template <typename input_t, typename state_t, int NPREDICTED, int MAX_WINDOW, int DIM, int D_PER_CTA, int DSTATE,
    int NUM_WARPS, typename SmemT>
__device__ __forceinline__ void ssu_nocheckpoint_8bit(SmemT& smem, CheckpointingSsuParams const& params, int warp,
    int lane, int prev_k, int d_tile, int64_t out_seq_base, int head, int64_t cache_slot, float D_val, int seq_len)
{
    __syncthreads();
    compute_no_write_output_8bit<input_t, state_t, NPREDICTED, MAX_WINDOW, DIM, D_PER_CTA, DSTATE, NUM_WARPS>(
...
template <typename input_t, typename state_t, int NPREDICTED, int DIM, int D_PER_CTA, int DSTATE, int NUM_WARPS,
    int PHILOX_ROUNDS, typename SmemT>
__device__ __forceinline__ void ssu_checkpoint_8bit(SmemT& smem, CheckpointingSsuParams const& params, int warp,
    int lane, int prev_k, int d_tile, int64_t out_seq_base, int head, int64_t cache_slot, float D_val, int seq_len)
{
    using namespace cute;
    int const tid = warp * warpSize + lane;
    ...
    auto tiled_mma_chain = make_tiled_mma(MMA_Atom<MMA_Traits<MMA_prop::AtomK16>>{}, Layout<Shape<_4, _1>>{});
    auto thr_mma_chain = tiled_mma_chain.get_slice(tid);
    auto id_DxT = make_identity_tensor(make_shape(Int<D_PER_CTA>{}, Int<SmemT::NPREDICTED_PAD_MMA_M>{}));
    ...
    float total_scale[2];
    replay_state_mma_8bit_chain<input_t, state_t, DIM, D_PER_CTA, DSTATE>(smem, params, warp, lane, prev_k, d_tile,
        cache_slot, head, /*must_checkpoint=*/true, frag_y_DxT, encode_scale_per_row, total_scale);
    ...
    int64_t const state_ptr_offset = cache_slot * params.state_stride_seq + (int64_t) head * DIM * DSTATE;
    ...
    encode_state_replay_8bit<input_t, state_t, DIM, D_PER_CTA, DSTATE, PHILOX_ROUNDS>(smem, params, warp, lane, prev_k,
        d_tile, cache_slot, head, encode_scale_per_row, total_scale, rand_seed, state_ptr_offset);
    ...
    store_state<state_t, DIM, D_PER_CTA, DSTATE, NUM_WARPS>(smem, params, warp, lane, d_tile, head, cache_slot);
...
template <typename input_t, typename dt_t, typename weight_t, typename matrixA_t, typename state_t,
    typename stateIndex_t, typename state_scale_t, int NPREDICTED, int MAX_WINDOW, int DIM, int DSTATE,
    int HEADS_PER_GROUP, int PHILOX_ROUNDS, int NUM_WARPS, bool VARLEN = false>
__global__ void checkpointing_ssu_kernel_8bit(CheckpointingSsuParams params)
{
    using namespace cute;
    static_assert(sizeof(state_t) == 1, "checkpointing_ssu_kernel_8bit requires 1-byte state_t (int8 or fp8 e4m3)");
    ...
    using SmemT = CheckpointingSsuStorage8bit<input_t, state_t, NPREDICTED, MAX_WINDOW, D_PER_CTA, DSTATE>;
    ...
    {
        load_pre_pdl_wait_data<input_t, dt_t, state_t, NPREDICTED, MAX_WINDOW, DIM, D_PER_CTA, DSTATE, NUM_WARPS>(smem,
            params, lane, warp, d_tile, head, group_idx, cache_slot, buf_read, A_val, dt_bias_val, dt_seq_base,
            z_seq_base, seq_len);
        gdc_wait();
    ...
    {
        load_data<input_t, dt_t, state_t, NPREDICTED, MAX_WINDOW, DIM, D_PER_CTA, DSTATE, NUM_WARPS>(smem, params, lane,
            warp, d_tile, head, group_idx, cache_slot, buf_read, A_val, dt_bias_val, outer, seq_len);
    }
    ...
    {
        compute_CB_old_2warp<input_t, NPREDICTED, MAX_WINDOW, DSTATE>(smem, warp, lane, prev_k, seq_len);
    }
    ...
    if (must_checkpoint)
    {
        ssu_checkpoint_8bit<input_t, state_t, NPREDICTED, DIM, D_PER_CTA, DSTATE, NUM_WARPS, PHILOX_ROUNDS>(
            smem, params, warp, lane, prev_k, d_tile, out_seq_base, head, cache_slot, D_val, seq_len);
    }
    else
    {
        ssu_nocheckpoint_8bit<input_t, state_t, NPREDICTED, MAX_WINDOW, DIM, D_PER_CTA, DSTATE, NUM_WARPS>(
            smem, params, warp, lane, prev_k, d_tile, out_seq_base, head, cache_slot, D_val, seq_len);
    }
    ...
    store_old_x<input_t, NPREDICTED, DIM, D_PER_CTA>(
        smem, params, warp, lane, d_tile, head, cache_slot, write_offset, seq_len);
    if (d_tile == 0 && warp == 0 && lane < seq_len)
    {
        auto* __restrict__ old_dt_w = reinterpret_cast<float*>(params.old_dt);
        int64_t const dt_w_base = cache_slot * params.old_dt_stride_seq + buf_write * params.old_dt_stride_dbuf
    ...
    if (d_tile == 0 && warp == 1 && lane < seq_len)
    {
        ...
        int64_t const ca_w_base = cache_slot * params.old_cumAdt_stride_seq + buf_write * params.old_cumAdt_stride_dbuf
