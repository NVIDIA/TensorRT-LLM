...
{
    if (warp < 12)
        return WarpRole::kCompute;
    if (warp < 15)
        return WarpRole::kTMALoad;
...
__device__ __forceinline__ void fma_f32x2(float2& d, float2 const& a, float2 const& b, float2 const& c)
{
#if defined(__CUDA_ARCH__) && __CUDA_ARCH__ >= 1000
...
__device__ __forceinline__ void convertAndStoreSRHorizontal(state_t& out0, state_t& out1, float s0, float s1,
    int64_t rand_seed, int state_ptr_offset, int dd, int col0, int e, uint32_t (&rand_ints)[4])
{
    using namespace conversion;
    if constexpr (PHILOX_ROUNDS > 0)
    {
        if (e % 4 == 0)
            philox_randint4x<PHILOX_ROUNDS>(rand_seed, state_ptr_offset + dd * DSTATE + col0 + e, rand_ints[0],
