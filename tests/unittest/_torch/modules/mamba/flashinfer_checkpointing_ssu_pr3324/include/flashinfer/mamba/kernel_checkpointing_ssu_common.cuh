...
struct Pair<__nv_fp8_e4m3>
{
    ...
    {
        static_assert(I == 0 || I == 1, "Pair index must be 0 or 1");
        __nv_fp8_storage_t const byte
            = (I == 0) ? static_cast<__nv_fp8_storage_t>(raw & 0xFFu) : static_cast<__nv_fp8_storage_t>(raw >> 8);
...
struct SmemSwizzle
{
    static_assert(Copy_prop::vec_bytes % sizeof(Elem) == 0, "element size must divide LDSM atom (16 bytes)");
...
__device__ __forceinline__ auto make_swizzled_layout_rc()
{
    ...
    static_assert(ROW_STRIDE % S::ATOM_COLS == 0, "ROW_STRIDE must be a multiple of the swizzle atom cols");
    ...
    if constexpr (ROW_STRIDE == COLS)
    {
        auto atom = composition(typename S::type{},
            make_layout(make_shape(Int<S::ATOM_ROWS>{}, Int<S::ATOM_COLS>{}), make_stride(Int<S::ATOM_COLS>{}, _1{})));
        return tile_to_shape(atom, make_shape(Int<ROWS>{}, Int<COLS>{}));
    }
    else
    {
        return composition(typename S::type{},
            make_layout(make_shape(Int<ROWS>{}, Int<COLS>{}), make_stride(Int<ROW_STRIDE>{}, _1{})));
    }
}
...
__device__ __forceinline__ auto make_aliased_swizzled_layout_rc()
{
    using namespace cute;
    using S = SmemSwizzle<Elem>;
    static_assert(LOGICAL_ROWS % S::ATOM_ROWS == 0, "LOGICAL_ROWS must be a multiple of the swizzle atom rows");
    ...
    static_assert(LOG_M_TILES % PHYS_M_TILES == 0, "LOGICAL_ROWS must be a multiple of PHYS_ROWS for clean alias");
    ...
    auto outer = make_layout(make_shape(make_shape(Int<PHYS_M_TILES>{}, Int<ALIAS>{}), Int<N_TILES>{}),
...
__device__ __forceinline__ void load_tile_async(input_t* __restrict__ smem_dst, input_t const* __restrict__ gmem_src,
    int gmem_row_stride, int lane, int valid_rows_rt = VALID_ROWS)
{
    using namespace cute;
    ...
    Tensor s_full = make_tensor(make_smem_ptr(smem_dst), make_swizzled_layout_rc<input_t, ROWS_PAD, SMEM_COLS>());
    Tensor g_full = make_tensor(make_gmem_ptr(gmem_src),
        make_layout(make_shape(Int<ROWS_PAD>{}, Int<SMEM_COLS>{}), make_stride(gmem_row_stride, Int<1>{})));

    constexpr int VAL_COLS_PER_THREAD = Copy_prop::vec_bytes / sizeof(input_t);
    static_assert(SMEM_COLS % VAL_COLS_PER_THREAD == 0, "SMEM_COLS must be divisible by VAL_COLS_PER_THREAD");
    ...
    auto g2s = make_tiled_copy(
        Copy_Atom<Copy_prop::AtomZFill, input_t>{}, ThrLayout{}, Layout<Shape<_1, Int<VAL_COLS_PER_THREAD>>>{});
...
__device__ __forceinline__ void load_state_per_warp(
    SmemT& smem, state_t const* __restrict__ state_ptr, int64_t state_base, int warp, int lane)
{
    using namespace cute;
    ...
    Tensor sState_full = make_tensor(
        make_smem_ptr(reinterpret_cast<state_t*>(smem.state)), make_swizzled_layout_rc<state_t, D_PER_CTA, DSTATE>());
    Tensor gState_full = make_tensor(make_gmem_ptr(state_ptr + state_base),
        make_layout(make_shape(Int<D_PER_CTA>{}, Int<DSTATE>{}), make_stride(Int<DSTATE>{}, Int<1>{})));

    Tensor sState = local_tile(sState_full, make_shape(Int<DIM_PER_WARP>{}, Int<DSTATE>{}), make_coord(warp, _0{}));
    Tensor gState = local_tile(gState_full, make_shape(Int<DIM_PER_WARP>{}, Int<DSTATE>{}), make_coord(warp, _0{}));

    constexpr int VAL_COLS = Copy_prop::vec_bytes / sizeof(state_t);
    auto g2s = make_tiled_copy(Copy_Atom<Copy_prop::Atom, state_t>{}, Layout<Shape<_4, _8>, Stride<_8, _1>>{},
...
template <typename state_t, int D_PER_CTA, int DSTATE, int NUM_WARPS, typename SmemT>
__device__ __forceinline__ void load_state_cta(
    SmemT& smem, state_t const* __restrict__ state_ptr, int64_t state_base, int tid)
{
    using namespace cute;
    static_assert(NUM_WARPS == 4, "Expected 4 warps");
    ...
    static_assert(D_PER_CTA % THR_ROWS == 0, "D_PER_CTA must be divisible by the thread layout's row count");
    auto g2s = make_tiled_copy(Copy_Atom<Copy_prop::Atom, state_t>{}, ThrLayout{}, Layout<Shape<_1, Int<VAL_COLS>>>{});
...
// zero in smem.
template <typename input_t, typename dt_t, typename state_t, int NPREDICTED, int MAX_WINDOW, int DIM, int D_PER_CTA,
    int DSTATE, int NUM_WARPS, typename SmemT>
__device__ __forceinline__ void load_pre_pdl_wait_data(SmemT& smem, CheckpointingSsuParams const& params, int lane,
    int warp, int d_tile, int head, int group_idx, int64_t cache_slot, int buf_read, float A_val, float dt_bias_val,
    int64_t dt_seq_base, int64_t z_seq_base, int seq_len)
{
    ...
    int64_t const oB_base
        = cache_slot * params.old_B_stride_seq + buf_read * params.old_B_stride_dbuf + group_idx * DSTATE;
    ...
    // full head's D, cooperative 128-thread load when D is sharded. ──
    {
        ...
        int64_t const state_base
            = cache_slot * params.state_stride_seq + (int64_t) head * DIM * DSTATE + (int64_t) d_tile_off * DSTATE;
        if constexpr (DIM == D_PER_CTA)
        {
            // D_SPLIT=1: per-warp partition (warp w loads contiguous DIM/4 D-rows).
            load_state_per_warp<state_t, D_PER_CTA, DSTATE, NUM_WARPS>(smem, state_ptr, state_base, warp, lane);
        }
        else
        {
    ...
    load_tile_async<OldBShape, MAX_WINDOW>(smem.old_B, old_B_ptr + oB_base, params.old_B_stride_token, lane);

    // ── old_x: redundant on all 4 warps (small, simpler than partitioning).
    // VALID_ROWS = MAX_WINDOW (cache rows). ──
    load_tile_async<OxShape, MAX_WINDOW>(smem.old_x, old_x_ptr + ox_base, params.old_x_stride_token, lane);
    ...
    if (warp == 3 && z_ptr)
    {
        ...
        load_tile_async<ZShape, NPREDICTED>(smem.z, z_ptr + z_base, params.z_stride_token, lane, seq_len);
    }
    ...
    if (lane < MAX_WINDOW)
    {
        int64_t const dt_rd_base = cache_slot * params.old_dt_stride_seq + buf_read * params.old_dt_stride_dbuf
            + head * params.old_dt_stride_head;
        smem.old_dt[lane] = old_dt_ptr[dt_rd_base + lane];

        int64_t const ca_rd_base = cache_slot * params.old_cumAdt_stride_seq + buf_read * params.old_cumAdt_stride_dbuf
...
__device__ __forceinline__ void load_post_pdl_wait_data(SmemT& smem, CheckpointingSsuParams const& params, int lane,
    int warp, int d_tile, int head, int group_idx, int64_t outer, int seq_len)
{
    constexpr int INPUT_PACK = 16 / sizeof(input_t); // 8 for bf16
    static_assert(DSTATE % INPUT_PACK == 0, "DSTATE must be divisible by input pack size");
    static_assert(D_PER_CTA % INPUT_PACK == 0, "D_PER_CTA must be divisible by input pack size");
...
template <typename input_t, typename dt_t, typename state_t, int NPREDICTED, int MAX_WINDOW, int DIM, int D_PER_CTA,
    int DSTATE, int NUM_WARPS, typename SmemT>
__device__ __forceinline__ void load_data(SmemT& smem, CheckpointingSsuParams const& params, int lane, int warp,
    int d_tile, int head, int group_idx, int64_t cache_slot, int buf_read, float A_val, float dt_bias_val,
...
__device__ __forceinline__ void compute_CB_scaled_2warp(SmemT& smem, int warp, int lane, int seq_len)
{
    using namespace cute;
    ...
    static_assert(
        N_HALF % MMA_prop::N == 0, "compute_CB_scaled_2warp: NPREDICTED_PAD_MMA_M / 2 must be a multiple of MMA::N");
...
#pragma unroll
    ...
    auto layout_C = make_aliased_swizzled_layout_rc<input_t, NPREDICTED_PAD_MMA_M, DSTATE, NPREDICTED>();
    ...
    auto tiled_mma = make_tiled_mma(MMA_Atom<MMA_Traits<MMA_prop::AtomK16>>{}, Layout<Shape<_1, _1>>{});
    ...
    Tensor smem_B_half = local_tile(smem_B, make_tile(Int<N_HALF>{}, Int<K_TILE>{}), make_coord(warp, _));
...
#pragma unroll
    ...
    Tensor smem_CB = make_tensor(make_smem_ptr(reinterpret_cast<MMA_prop::operand_t*>(smem.CB_scaled)), layout_cb_swz);
    // Tile into [NPREDICTED_PAD_MMA_M, N_HALF] halves; warp selects its half
    Tensor smem_CB_half
        = local_tile(smem_CB, make_tile(Int<NPREDICTED_PAD_MMA_M>{}, Int<N_HALF>{}), make_coord(_0{}, warp));
...
__device__ __forceinline__ void compute_CB_old_2warp(SmemT& smem, int warp, int lane, int prev_k, int seq_len)
{
    using namespace cute;
    ...
    static_assert(
        MAX_WINDOW_PAD_MMA_K % N_HALF == 0, "compute_CB_old_2warp: MAX_WINDOW_PAD_MMA_K must be a multiple of MMA::N");
    ...
    Tensor smem_C = make_tensor(make_smem_ptr(reinterpret_cast<input_t const*>(smem.C)), layout_C);
    Tensor smem_old_B = make_tensor(make_smem_ptr(reinterpret_cast<input_t const*>(smem.old_B)), layout_old_B);
    ...
    Tensor smem_old_B_half = local_tile(smem_old_B, make_tile(Int<N_HALF>{}, Int<K_TILE>{}), make_coord(sub_warp, _));
...
#pragma unroll
    ...
    Tensor smem_CB = make_tensor(make_smem_ptr(reinterpret_cast<MMA_prop::operand_t*>(smem.CB_scaled)), layout_cb_full);
    ...
    Tensor smem_CB_half = local_tile(
        smem_CB, make_tile(Int<NPREDICTED_PAD_MMA_M>{}, Int<N_HALF>{}), make_coord(_0{}, CB_OLD_TILE_BASE + sub_warp));
    Tensor smem_CB_part = thr_mma.partition_C(smem_CB_half);
...
#pragma unroll
    for (int i = 0; i < size(frag_acc); ++i)
    {
        ...
        {
            val = frag_acc(i) * __expf(smem.cumAdt[t] + total_old_cumAdt - smem.old_cumAdt[j]) * smem.old_dt[j];
        }
        else
        {
            val = 0.f;
...
__device__ __forceinline__ void precompute_dB_coeff(
    float coeff[DB_COEFFS_PER_LANE], SmemT const& smem, float total_cumAdt, int prev_k, int lane)
{
    static_assert(DB_COEFFS_PER_LANE == 2 || DB_COEFFS_PER_LANE == 4, "DB_COEFFS_PER_LANE must be 2 (k8) or 4 (k16)");
...
__device__ __forceinline__ void compute_dB_scaling(FragB& frag_B, float const coeff[DB_COEFFS_PER_LANE])
{
    using namespace cute;
...
__device__ __forceinline__ void apply_dA_coeff(
    FragA& frag_A, SmemT const& smem, float total_cumAdt, int prev_k, int lane)
{
    using namespace cute;
    constexpr int FRAG_A_SIZE = size(FragA{});
    static_assert((MAX_WINDOW_PAD_MMA_K == 16 && FRAG_A_SIZE == 8) || (MAX_WINDOW_PAD_MMA_K == 8 && FRAG_A_SIZE == 4),
    ...
    {
        float const c0 = (K_base < prev_k) ? __expf(total_cumAdt - smem.old_cumAdt[K_base]) * smem.old_dt[K_base] : 0.f;
        float const c1 = (K_base + 1 < prev_k)
            ? __expf(total_cumAdt - smem.old_cumAdt[K_base + 1]) * smem.old_dt[K_base + 1]
            : 0.f;
#pragma unroll
        for (int i = 0; i < 4; ++i)
        {
...
#pragma unroll
        for (int i = 0; i < size(src); i += 2)
        {
            *reinterpret_cast<Pair<dst_t>*>(&dst(i)) = pack_float2<dst_t>(make_float2(src(i), src(i + 1)));
        }
    }
}
...
template <typename input_t, typename MmaT, int N_TILE, int NPREDICTED_PAD_MMA_M, typename FragY, typename FragCB,
    typename SmemXTrans, typename S2RBTrans, typename S2RThrBTrans, typename ThrMma, typename TiledMma>
__device__ __forceinline__ void add_cb_x(FragY& frag_y, FragCB const& frag_CB, SmemXTrans const& smem_x_trans,
    S2RBTrans const& s2r_B_trans, S2RThrBTrans const& s2r_thr_B_trans, ThrMma const& thr_mma, TiledMma const& tiled_mma,
    int n)
{
    using namespace cute;
    Tensor smem_x_trans_ntile
        = local_tile(smem_x_trans, make_tile(Int<N_TILE>{}, Int<NPREDICTED_PAD_MMA_M>{}), make_coord(n, _0{}));
...
template <typename input_t, typename MmaT, int N_TILE, int MAX_WINDOW_PAD_MMA_K, typename FragY, typename FragCBOld,
    typename SmemOldXTrans, typename S2RBTransOld, typename S2RThrBTransOld, typename ThrMmaOld, typename TiledMmaOld>
__device__ __forceinline__ void add_cb_old_x(FragY& frag_y, FragCBOld const& frag_CB_old,
    SmemOldXTrans const& smem_old_x_trans, S2RBTransOld const& s2r_B_trans_old,
    S2RThrBTransOld const& s2r_thr_B_trans_old, ThrMmaOld const& thr_mma_old, TiledMmaOld const& tiled_mma_old, int n)
{
    using namespace cute;
    Tensor smem_old_x_ntile
        = local_tile(smem_old_x_trans, make_tile(Int<N_TILE>{}, Int<MAX_WINDOW_PAD_MMA_K>{}), make_coord(n, _0{}));
...
template <typename input_t, int NPREDICTED_PAD_MMA_M, int N_TILE, typename FragY, typename SmemX, typename ThrMma>
__device__ __forceinline__ void add_D_skip(
    FragY& frag_y, SmemX const& smem_x, ThrMma const& thr_mma, float D_val, int n)
{
    using namespace cute;
    if (D_val == 0.f)
        return;
    Tensor smem_x_tile = local_tile(smem_x, make_tile(Int<NPREDICTED_PAD_MMA_M>{}, Int<N_TILE>{}), make_coord(_0{}, n));
...
template <typename input_t, int NPREDICTED_PAD_MMA_M, int N_TILE, typename FragY, typename SmemZ, typename ThrMma>
__device__ __forceinline__ void compute_z_gating(
    FragY& frag_y, SmemZ const& smem_z, ThrMma const& thr_mma, void const* z_ptr, int n)
{
    using namespace cute;
    if (!z_ptr)
        return;
    Tensor smem_z_tile = local_tile(smem_z, make_tile(Int<NPREDICTED_PAD_MMA_M>{}, Int<N_TILE>{}), make_coord(_0{}, n));
...
// D_SPLIT=1).
template <int NumStages, int NumKTiles, typename ATypeIn, typename BTypeIn, typename MmaT, typename TiledMma,
    typename ThrMma, typename SmemAKtiled, typename SmemB, typename... FragY>
__device__ __forceinline__ void pipelined_kloop_gemm(TiledMma const& tiled_mma, ThrMma const& thr_mma, int tid,
    SmemAKtiled const& smem_A_ktiled, SmemB const& smem_B, FragY&... frag_y)
{
    using namespace cute;
    ...
    static_assert((std::is_same_v<FragY0, FragY> && ...), "all FragY parameters must be the same type");
...
#pragma unroll
    for (int k = 0; k < NumKTiles; ++k)
    {
        ...
        if (k_load < NumKTiles)
            load_one(k_load, slot_load);
        compute_one(slot_compute);
        if (k_load < NumKTiles)
            convert_one(slot_load);
    }
}
...
template <typename input_t, typename state_t, int D_PER_CTA, int DSTATE, typename SmemT, typename TiledMma,
    typename ThrMma, typename... FragY>
__device__ __forceinline__ void add_init_out(
    SmemT const& smem, TiledMma const& tiled_mma, ThrMma const& thr_mma, int tid, FragY&... frag_y)
{
    using namespace cute;
    constexpr int NPREDICTED_PAD_MMA_M = SmemT::NPREDICTED_PAD_MMA_M;
    constexpr int K_TILE = cute::tile_size<2>(TiledMma{});
...
__device__ __forceinline__ void store_state(
    SmemT& smem, CheckpointingSsuParams const& params, int warp, int lane, int d_tile, int head, int64_t cache_slot)
{
    using namespace cute;
    ...
    int64_t const state_base
        = cache_slot * params.state_stride_seq + (int64_t) head * DIM * DSTATE + (int64_t) d_tile * D_PER_CTA * DSTATE;
    ...
    constexpr int VAL_COLS = Copy_prop::vec_bytes / sizeof(state_t);
    auto s2g = make_tiled_copy(Copy_Atom<UniversalCopy<uint128_t>, state_t>{}, Layout<Shape<_16, _8>, Stride<_8, _1>>{},
        Layout<Shape<_1, Int<VAL_COLS>>>{});
...
__device__ __forceinline__ void store_old_x(SmemT& smem, CheckpointingSsuParams const& params, int warp, int lane,
    int d_tile, int head, int64_t cache_slot, int write_offset, int seq_len)
{
    using namespace cute;
    constexpr int NPREDICTED_PAD_MMA_M = SmemT::NPREDICTED_PAD_MMA_M;
    int const flat_tid = warp * warpSize + lane;
    ...
    int64_t const ox_w_base = cache_slot * params.old_x_stride_seq + (int64_t) write_offset * params.old_x_stride_token
        + head * DIM + (int64_t) d_tile * D_PER_CTA;
    ...
    auto s2g = make_tiled_copy(Copy_Atom<UniversalCopy<uint128_t>, input_t>{}, ThrLayoutX{}, Layout<Shape<_1, _8>>{});
...
__device__ __forceinline__ void store_old_B(SmemT& smem, CheckpointingSsuParams const& params, int warp, int lane,
    int head, int group_idx, int64_t cache_slot, int buf_write, int write_offset, int seq_len)
{
    using namespace cute;
    if (head % HEADS_PER_GROUP != 0)
        return;
    ...
    int64_t const oB_base = cache_slot * params.old_B_stride_seq + buf_write * params.old_B_stride_dbuf
    ...
    Tensor gB = make_tensor(make_gmem_ptr(old_B_w + oB_base),
        make_layout(
            make_shape(Int<NPREDICTED_PAD_MMA_N>{}, Int<DSTATE>{}), make_stride(params.old_B_stride_token, Int<1>{})));

    // 64 threads, (8, 8) × (1, 8) = atom-aligned per-tile (8, 64).
    auto s2g = make_tiled_copy(Copy_Atom<UniversalCopy<uint128_t>, input_t>{}, Layout<Shape<_8, _8>, Stride<_8, _1>>{},
        Layout<Shape<_1, _8>>{});
    auto thr_s2g = s2g.get_slice(flat_tid);
