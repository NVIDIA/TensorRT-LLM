...
template <typename input_t, typename state_t, int NPREDICTED_, int MAX_WINDOW_, int D_PER_CTA, int DSTATE>
...
__device__ __forceinline__ uint32_t stochastic_round_pair_with_philox_refresh(
    float a, float b, int pair_idx, int64_t rand_seed, int64_t philox_off, uint32_t (&rand_idx)[4])
{
...
__device__ __forceinline__ void exchange_ntile_state_store_global(state_t* __restrict__ state_w_base, int np, int lane,
    uint32_t const (&my_packed)[2][PAIRS_PER_PASS], IdPart const& id_part)
{
    using namespace cute;
    static_assert(
        sizeof(state_t) == 2, "exchange_ntile_state_store_global requires 2-byte state_t for STG.64 alignment");
...
__device__ __forceinline__ void replay_state_mma(SmemT& smem, CheckpointingSsuParams const& params, int warp, int lane,
    int prev_k, int d_tile, int64_t state_ptr_offset, state_t* state_w_base, int64_t rand_seed, bool must_checkpoint)
{
    using namespace cute;
    ...
    static_assert(DSTATE % N_PER_PASS == 0, "DSTATE must be divisible by 4 * MMA_prop::N for _1x4 warp layout");
    ...
    Tensor smem_B_full = make_tensor(make_smem_ptr(reinterpret_cast<MMA_prop::operand_t const*>(smem.old_B)), layout_B);
            ...
            for (int i = 0; i < size(frag_h); i += 2)
            {
                ...
                *reinterpret_cast<pair_t*>(&state_base[off]) = q;
                ...
                if constexpr (kPhiloxF16)
                {
                    ...
                    int64_t const philox_off = state_ptr_offset + (int64_t) (d_tile * D_PER_CTA + row) * DSTATE + col;
...
__device__ __forceinline__ void compute_and_store_output(SmemT& smem, CheckpointingSsuParams const& params, int warp,
    int lane, int d_tile, int64_t out_seq_base, int head, int64_t cache_slot, float D_val, bool must_checkpoint,
    int seq_len)
{
    using namespace cute;
    ...
    static_assert(
        NUM_N_TILES == 1 || NUM_N_TILES == 2, "Output epilogue supports NUM_N_TILES = D_PER_CTA / N_TILE in {1, 2}");
...
#pragma unroll
    ...
    if constexpr (NUM_N_TILES == 2)
    {
        ...
        add_init_out<input_t, state_t, D_PER_CTA, DSTATE>(smem, tiled_mma, thr_mma, tid, frag_y_0, frag_y_1);
        ...
        if constexpr (!kSkipSmemToGmemState)
        {
            if (must_checkpoint)
            {
                store_state<state_t, DIM, D_PER_CTA, DSTATE, NUM_WARPS>(
                    smem, params, warp, lane, d_tile, head, cache_slot);
            }
        }
        epilogue(frag_y_0, 0);
        epilogue(frag_y_1, 1);
...
__device__ __forceinline__ void compute_no_write_output(SmemT& smem, CheckpointingSsuParams const& params, int warp,
    int lane, int prev_k, int d_tile, int64_t out_seq_base, int head, int64_t cache_slot, float D_val, int seq_len)
{
    ...
    constexpr int NUM_N_TILES = D_PER_CTA / N_TILE;
    static_assert(NUM_N_TILES == 1 || NUM_N_TILES == 2, "compute_no_write_output supports NUM_N_TILES in {1, 2}");
...
template <typename input_t, typename state_t, int NPREDICTED, int DIM, int D_PER_CTA, int DSTATE, int NUM_WARPS,
    int PHILOX_ROUNDS, typename SmemT>
__device__ __forceinline__ void ssu_checkpoint(SmemT& smem, CheckpointingSsuParams const& params, int warp, int lane,
    int prev_k, int d_tile, int64_t out_seq_base, int head, int64_t cache_slot, float D_val, int seq_len)
{
    ...
    state_t* const state_w_base = reinterpret_cast<state_t*>(params.state) + cache_slot * params.state_stride_seq
        + (int64_t) head * DIM * DSTATE + (int64_t) d_tile * D_PER_CTA * DSTATE;
    replay_state_mma<input_t, state_t, DIM, D_PER_CTA, DSTATE, PHILOX_ROUNDS>(smem, params, warp, lane, prev_k, d_tile,
        state_ptr_offset, state_w_base, rand_seed,
        /*must_checkpoint=*/true);

    __syncthreads();

    compute_and_store_output<input_t, state_t, NPREDICTED, DIM, D_PER_CTA, DSTATE, NUM_WARPS, PHILOX_ROUNDS>(
        smem, params, warp, lane, d_tile, out_seq_base, head, cache_slot, D_val, /*must_checkpoint=*/true, seq_len);
}

template <typename input_t, typename state_t, int NPREDICTED, int MAX_WINDOW, int DIM, int D_PER_CTA, int DSTATE,
    int NUM_WARPS, typename SmemT>
__device__ __forceinline__ void ssu_nocheckpoint(SmemT& smem, CheckpointingSsuParams const& params, int warp, int lane,
    int prev_k, int d_tile, int64_t out_seq_base, int head, int64_t cache_slot, float D_val, int seq_len)
{
    ...
    compute_no_write_output<input_t, state_t, NPREDICTED, MAX_WINDOW, DIM, D_PER_CTA, DSTATE, NUM_WARPS>(
...
template <typename input_t, typename dt_t, typename weight_t, typename matrixA_t, typename state_t,
    ...
    int HEADS_PER_GROUP, int PHILOX_ROUNDS, int NUM_WARPS, int D_SPLIT = 1, bool VARLEN = false>
__global__ void checkpointing_ssu_kernel(CheckpointingSsuParams params)
{
    ...
    static_assert(NPREDICTED <= MAX_WINDOW, "NPREDICTED must be <= MAX_WINDOW (new tokens must fit in cache)");
    static_assert(
        MAX_WINDOW <= MMA_prop::K_BIG, "MAX_WINDOW must be <= MMA::K_BIG=16 (single replay K-tile assumption)");
    ...
    using SmemT = CheckpointingSsuStorage<input_t, state_t, NPREDICTED, MAX_WINDOW, D_PER_CTA, DSTATE>;
