...
inline __device__ float toFloat(int8_t val)
{
    return static_cast<float>(val);
}

inline __device__ float toFloat(int16_t val)
{
    return static_cast<float>(val);
}
...
inline __device__ float2 toFloat2(float2 packed)
{
    return packed;
}

inline __device__ float2 toFloat2(__half2 packed)
{
...
inline __device__ float2 toFloat2(float const* ptr)
{
    return {ptr[0], ptr[1]};
}
...
inline __device__ float2 toFloat2(int8_t const* ptr)
{
    return {toFloat(ptr[0]), toFloat(ptr[1])};
}

inline __device__ float2 toFloat2(int16_t const* ptr)
{
    return {toFloat(ptr[0]), toFloat(ptr[1])};
}
...
__device__ __forceinline__ void philox_randint4x(
    int64_t seed, int64_t offset, uint32_t& r0, uint32_t& r1, uint32_t& r2, uint32_t& r3)
{
...
__device__ __forceinline__ uint16_t cvt_rs_f16_sw(float x, uint32_t rand13)
{
    ...
    { // 127 + 15 = 142 → overflow to Inf
        f16_bits = 0x7C00u;
    }
    else if (f32_exp < 113)
    { // 127 - 14 = 113 → underflow to zero
        f16_bits = 0;
    }
    else
    {
...
inline __device__ void convertSRAndStore(__half* output, float input, int64_t seed, uint32_t offset)
{
...
__device__ __forceinline__ uint32_t cvt_rs_e4m3x4_f32(float a, float b, float c, float d, uint32_t rbits)
{
#if defined(__CUDA_ARCH__) && __CUDA_ARCH__ >= 1000 && defined(__CUDA_ARCH_FEAT_SM100_ALL)
    uint32_t packed;
    ...
    return static_cast<uint32_t>(pa) | (static_cast<uint32_t>(pb) << 8) | (static_cast<uint32_t>(pc) << 16)
        | (static_cast<uint32_t>(pd) << 24);
#endif
}
...
{
    float const rand01 = static_cast<float>(rand_bits & 0x00FFFFFFu) * (1.0f / static_cast<float>(1 << 24));
...
// cvt_rs_sat_s8 path.
__device__ __forceinline__ uint32_t cvt_rs_sat_s8x4_f32(float a, float b, float c, float d, uint32_t rbits)
{
