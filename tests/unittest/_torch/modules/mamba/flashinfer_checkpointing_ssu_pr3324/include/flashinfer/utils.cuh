...
#define FLASHINFER_CUDA_CHECK(func)                                                                                    \
    ...
    {                                                                                                                  \
        cudaError_t e = (func);                                                                                        \
        FLASHINFER_CHECK(e == cudaSuccess, "CUDA Error: ", cudaGetErrorString(e), " (", int(e), ") at ", __FILE__,     \
            ":", __LINE__, " in ", STR(func));                                                                         \
    } while (0)

#define FLASHINFER_CHECK_ALIGNMENT(ptr, size_bytes)                                                                    \
    FLASHINFER_CHECK(reinterpret_cast<uintptr_t>(ptr) % (size_bytes) == 0, #ptr, " must be aligned to ", (size_bytes), \
        " bytes, got address ", (uintptr_t) (ptr))
 ...
 */
__forceinline__ __device__ short ld_na_global_s16(short const* addr)
{
    short val;
 ...
 */
__forceinline__ __device__ int ld_na_global_v1(int const* addr)
{
    int val;
 ...
 */
__forceinline__ __device__ int2 ld_na_global_v2(int2 const* addr)
{
    int2 val;
 ...
 */
template <typename T>
__forceinline__ __device__ void prefetch_L2(T const* addr)
{
...
__device__ __forceinline__ uint32_t dim2_offset(uint32_t const& dim_a, uint32_t const& idx_b, uint32_t const& idx_a)
{
    return idx_b * dim_a + idx_a;
}

__device__ __forceinline__ uint32_t dim3_offset(
    uint32_t const& dim_b, uint32_t const& dim_a, uint32_t const& idx_c, uint32_t const& idx_b, uint32_t const& idx_a)
{
    return (idx_c * dim_b + idx_b) * dim_a + idx_a;
}

__device__ __forceinline__ uint32_t dim4_offset(uint32_t const& dim_c, uint32_t const& dim_b, uint32_t const& dim_a,
    uint32_t const& idx_d, uint32_t const& idx_c, uint32_t const& idx_b, uint32_t const& idx_a)
{
