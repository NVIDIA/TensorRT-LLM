...
def _bench_config(
    ...
    if mix_samples_cpu is not None and mode != "monolithic":
        ...
        # Computed for ALL mix scenarios so the JSON output (--json-detailed)
        # can pair each iter's span_us with its mix composition for downstream
        # analysis (group iters by # writes → per-bucket median → analytic
        # expectation under the steady-state PNAT distribution).
        n_writes_per_iter_all = ((src + mtp_len) > max_window).sum(axis=1).astype(np.int32)
        ...
        for (
            ...
            if done_keys:
                ...
                _resume_key = _build_json_key(
                    args.variant, batch, mtp_len, prev_k_for_print,
