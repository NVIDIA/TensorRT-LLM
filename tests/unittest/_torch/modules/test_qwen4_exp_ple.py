...
"""PLE n-gram short-convolution unit tests for Qwen3.8-Flash-Next.
...
Independence (risk R3): ``_RefPLE`` re-derives the prime head-vocab sizes with
...
class _RefPLE:
    ...
    def _grouped_norm(self, x_flat: torch.Tensor, weight: torch.Tensor) -> torch.Tensor:
        ...
        # back to the module dtype (matches Qwen4ExpPLEGroupedNorm exactly).
...
def _run_parity(dtype: torch.dtype, tol_max: float):
    ...
    ma, mea, cos = _metrics(out, ref_out)
    print(f"[PLE parity dtype={dtype} prefill] max_abs={ma:.3e} mean_abs={mea:.3e} cos={cos:.6f}")
    ...
    ma1, mea1, cos1 = _metrics(out1, ref_out1)
    print(f"[PLE parity dtype={dtype} decode] max_abs={ma1:.3e} mean_abs={mea1:.3e} cos={cos1:.6f}")
    ...
    ma2, mea2, cos2 = _metrics(out2, ref_out2)
    print(
        f"[PLE parity dtype={dtype} decode2] max_abs={ma2:.3e} mean_abs={mea2:.3e} cos={cos2:.6f}"
    )
