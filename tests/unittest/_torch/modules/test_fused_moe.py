...
def test_fused_moe_wfp4a16(dtype):
    ...
    for expert_id in range(NUM_EXPERTS):
        w1_weight = torch.randint(0,
                                  256, (INTERMEDIATE_SIZE, HIDDEN_SIZE // 2),
                                  dtype=torch.uint8,
                                  device='cuda')
        w2_weight = torch.randint(0,
                                  256, (HIDDEN_SIZE, INTERMEDIATE_SIZE // 2),
                                  dtype=torch.uint8,
                                  device='cuda')
        ...
        w1_input = torch.ones(1, dtype=torch.float32).cuda()
        w2_input = w1_input
        w3_input = w1_input
    ...
    def ref():
        results = torch.zeros_like(x)
        selected_experts, final_scales = routing_method.apply(router_logits)
        unpacker = torch.ops.trtllm.Hopper_mxfp4_dequantize
        for e_idx in range(NUM_EXPERTS):
            mask = selected_experts == e_idx
            activated_tokens = mask.sum(1).bool()
            act = x[activated_tokens, :]
