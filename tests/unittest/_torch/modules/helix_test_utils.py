...
def create_helix_spec_gen_metadata(
    ...
    *,
    ...
    ``helix_is_inactive_rank`` has length ``batch * num_query_tokens``. The owned
    token count per sequence (used for kv_lens) is derived from those flags in
    ``prepare()``. The linear-chain causal mask is generated by
    ...
    # position and inactive-flag buffers, per request for total_input_len).
