...
import pytest
from nemotron35_vl_lora_utils import (
    assert_adapter_keys_parse,
    create_nemotron35_lora_adapter,
...
def test_every_adapter_key_reaches_a_module(mini_checkpoint, tmp_path, key_prefix):
    ...
    """
    adapter = create_nemotron35_lora_adapter(
...
def test_adapters_differing_only_by_seed_have_different_weights(mini_checkpoint, tmp_path):
    ...
    a = create_nemotron35_lora_adapter(str(tmp_path / "a"), mini_checkpoint, seed=0)
    b = create_nemotron35_lora_adapter(str(tmp_path / "b"), mini_checkpoint, seed=1)
...
def test_vl_wrapper_exposes_lora_config():
    ...
    config = NemotronH_Nano_VL_V2.lora_config("")
...
def test_vl_wrapper_hf_mapping_matches_the_adapter_builder():
    ...
    """
    from nemotron35_vl_lora_utils import NEMOTRON_H_HF_MODULES

    from tensorrt_llm._torch.models.modeling_nemotron_nano import NemotronH_Nano_VL_V2

    mapping = NemotronH_Nano_VL_V2.lora_config("").trtllm_modules_to_hf_modules
...
def test_vl_wrapper_lora_request_fails_loudly_without_a_bundled_adapter(mini_checkpoint):
    ...
    with pytest.raises(FileNotFoundError, match="No bundled LoRA adapter"):
        NemotronH_Nano_VL_V2.lora_request(1, "image", mini_checkpoint)
