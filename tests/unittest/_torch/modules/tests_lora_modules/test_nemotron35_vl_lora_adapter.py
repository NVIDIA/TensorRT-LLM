...
def test_layer_plan_accepts_equivalent_declarations() -> None:
    assert layer_plan(
        {
...
def test_layer_plan_rejects_conflicting_declarations(pattern: str) -> None:
    """Reject different layer types or counts before generating adapter weights."""
    with pytest.raises(ValueError, match="declare different layer plans"):
        layer_plan(
            {
...
def test_every_adapter_key_reaches_a_module(mini_checkpoint, tmp_path, key_prefix):
    """Synthetic decoder keys with either prefix parse without component filtering."""
...
def test_vision_qkv_keys_are_rejected_by_decoder_mapping(mini_checkpoint, tmp_path):
    """Synthetic vision qkv keys are outside the selected decoder module mapping."""
...
def test_vl_wrapper_exposes_lora_config():
    """The wrapper exposes decoder targets without requiring a bundled adapter."""
