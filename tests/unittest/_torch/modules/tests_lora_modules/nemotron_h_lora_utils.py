...
"""Synthetic LoRA adapters for the Nemotron-H checkpoints, text-only and VL.

No Nemotron-H LoRA adapter is published, so LoRA validation fabricates its own.
One builder serves the whole family because it reads dimensions and the layer
plan out of the checkpoint's own ``config.json``, and the family spells both
more than one way:

* dimensions sit at the top level on the text-only checkpoints and under
  ``llm_config`` on the VL ones;
* the layer plan is either the ``layers_block_type`` list (``mamba`` / ``moe``
  / ``attention``) or the ``hybrid_override_pattern`` string (``M`` / ``E`` /
  ``*``);
* weights carry an optional ``language_model.`` key prefix, which is what an
  adapter trained against the VL wrapper would have.

Every one of those layouts must parse. :func:`assert_adapter_keys_parse` is the
cheap CPU check that they do, because
...
# Three vocabularies name the same 88-layer plan, and a checkpoint in this
# family may use any of them:
#
