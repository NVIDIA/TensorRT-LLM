...
def calculate_ref_result_gen(
        fused_q: torch.Tensor, q_pe: torch.Tensor, compressed_kv: torch.Tensor,
        k_pe: torch.Tensor, latent_cache: torch.Tensor,
        rope_cos_sin: torch.Tensor, softmax_stats: torch.Tensor, num_heads: int,
    ...
    rope_cos_sin shape: (max_position_embeddings, 2, qk_rope_head_dim)
    softmax_stats shape: (num_tokens, num_heads, 2)
    """
    num_requests = len(sequence_lengths)
    ...
    for i in range(num_requests):
        ...
        cos, sin = rope_cos_sin[:sequence_lengths[i]].chunk(2, dim=-2)
        # cache_pe_seq = latent_cache_seq[..., -qk_rope_head_dim:]
        # cache_pe_seq = ((cache_pe_seq * cos) +
        #             (rotate_half(cache_pe_seq) * sin)).to(dtype=cache_pe_seq.dtype)
        # latent_cache_seq[..., -qk_rope_head_dim:] = cache_pe_seq
    ...
    for i in range(num_requests):
        ...
        attn_weights = torch.matmul(q, k.transpose(1, 2))
        scale = 1. / (q_scaling *
                      math.sqrt(qk_nope_head_dim + qk_rope_head_dim))
        print(attn_weights.shape)
        print(attn_weights[0, 0])
        print(attn_weights[0, 0] * scale)
        ...
        print(attn_weights[0, 0])
        softmax_stats[..., 0] = attn_weights.amax(dim=-1).t().to(torch.float32)
        exp = (attn_weights.to(torch.float32) -
               softmax_stats[..., :1].transpose(0, 1)).exp()
        softmax_stats[..., 1] = exp.sum(dim=-1).t()
        # Apply softmax to get attention probabilities
        attn_weights = torch.nn.functional.softmax(attn_weights,
                                                   dim=-1,
                                                   dtype=torch.float32).to(
        ...
        torch.testing.assert_close(
            attn_weights,
            (exp / softmax_stats[..., 1:2].transpose(0, 1)).to(q.dtype))
        print(attn_weights[0, 0])
...
max_context_sequence_length = 1024
min_num_contexts = 1
max_num_contexts = 1
...
def _run_test_for_backend(backend_name, num_heads, num_kv_heads, num_layers,
    ...
    for layer_idx in range(num_layers):
        ...
        latent_cache0 = torch.load("latent_cache_0.pt", map_location=device)
        compressed_kv0, k_pe0 = latent_cache0.split(
            (kv_lora_rank, qk_rope_head_dim), dim=-1)
        gen_compressed_kv_list = [compressed_kv0]
        gen_k_pe_list = [k_pe0]
        fused_q = torch.load("q_0.pt", map_location=device)
        q_pe = torch.load("q_pe_0.pt", map_location=device)
        gen_fused_q_list = [fused_q]
        gen_q_pe_list = [q_pe]
    ...
    rope_config = RopeConfig(
        ...
        num_attention_heads=rope_config.num_attention_heads,
        rope_scaling=None,
        max_position_embeddings=rope_config.max_position_embeddings,
        rope_theta=rope_config.rope_theta,
        qk_rope_head_dim=rope_config.qk_rope_head_dim,
    ...
    for step in range(num_generation_steps + 1):
        ...
        for layer_idx in range(num_layers):
            ...
            if step == 0:
                ...
                # for tok in range(context_sequence_lengths[0]):
                #     block, t = divmod(tok, kv_cache_tokens_per_block)
                #     latent_cache_ref_all_list[layer_idx][tok, :].copy_(kv_cache_[block, 0, t, 0, :])
                ...
                latent_cache = torch.cat([compressed_kv, k_pe], dim=-1)
                kv_cache_ = kv_cache_.view(
                    -1, kv_lora_rank +
                    qk_rope_head_dim)[:context_sequence_lengths[0]]
                torch.testing.assert_close(
                ...
                torch.testing.assert_close(
                    ...
                    gen_layers[layer_idx].wrapper.rotary_cos_sin[0].view(
                        -1, qk_rope_head_dim, 2).transpose(-2, -1),
                    atol=5e-4,
                    rtol=1e-5)
                softmax_stats = torch.empty(
                    [generation_seq_len_q, num_heads, 2],
                    dtype=torch.float32,
                    device=device,
                )
                ...
                torch.testing.assert_close(latent_cache[:, :kv_lora_rank],
                                           latent_cache_ref[-1:, :kv_lora_rank])
                # torch.testing.assert_close(latent_cache[:, kv_lora_rank:], latent_cache_ref[-1:, kv_lora_rank:])
                print("First few elements of results:")
                print(result[0, :10])
                print(softmax_stats[0, 0])
                print(ref_result[0, :10])
                print(softmax_stats_ref[0, 0])
                latent_cache_ref_all_list[layer_idx] = latent_cache_ref
            # Compare results
