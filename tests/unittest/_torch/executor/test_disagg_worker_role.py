...
class TestArgsField:
    ...
    def test_survives_model_defaults(self):
        """`apply_model_defaults_to_llm_args` rebuilds the args from
        `model_dump()`, which omits `exclude=True` fields. Copying the rebuilt
        values back used to reset the role to None for any model that declares
        defaults, so its context workers captured generation graphs again with
        ...
        apply_model_defaults_to_llm_args(args, {"max_beam_width": 1})
        assert args.disagg_worker_role is DisaggWorkerRole.CONTEXT

    def test_alias_passes_the_pytorch_backend_validator(self):
        """`_TorchLLM` re-checks kwargs against its own allowlist, separate
        from the generic one in `BaseLLM.__init__`. Missing the alias there let
        every disaggregated worker die at startup with "arguments are not
        supported by the PyTorch backend", while the unit tests stayed green
        because they never reached that validator."""
        from tensorrt_llm.llmapi.llm import _TorchLLM

        # Drive the real validator rather than assert on its allowlist, so this
        # keeps testing the thing that broke even if the allowlist is rebuilt.
        _TorchLLM._validate_args_for_torch_backend(
            None, {"_disagg_worker_role": DisaggWorkerRole.CONTEXT}
        )

        with pytest.raises(ValueError, match="not supported by the PyTorch"):
            _TorchLLM._validate_args_for_torch_backend(None, {"_no_such_launcher_arg": 1})
