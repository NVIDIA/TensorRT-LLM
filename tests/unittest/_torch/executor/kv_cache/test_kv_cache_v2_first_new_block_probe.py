...
class StubRequest:
    ...
    # The C++ binding returns a zero-copy int32 numpy view, not a list. The two
    # carry the same ids but not the same semantics: an array of more than one
    # element has no truth value, so a boolean test on the marshalled tokens
    # raises here and passes silently on the list.
...
@pytest.fixture(params=["cpp_view", "python_list"])
def token_source(request, monkeypatch):
    """Run the test against both shapes ``_reuse_token_source`` can return.

    Which shape is live is a property of the installed backend, not of the test:
    ``_cpp_introspection`` is None exactly when the pure-Python core is active,
    and the C++ core is what CI runs. Pinning it here is what makes the array
    shape reachable from a CPU-only run.
    """
    monkeypatch.setattr(
        kvcm_v2,
        "_cpp_introspection",
        object() if request.param == "cpp_view" else None,
    )
    return request.param


def prepared_tokens_and_scope(mgr, req):
    """Tokens and reuse scope that ``prepare_context_cache`` hands to the impl."""
    mgr.impl.create_kv_cache.reset_mock()
    # A settled claim short-circuits the match, so drop any cache a previous
    # call in the same test left behind.
    mgr.kv_cache_map.pop(req.py_request_id, None)
    mgr.prepare_context_cache(req)
...
class TestTokenParity:
    ...
    @pytest.mark.parametrize("reuse_match_backoff", [0, 2])
    def test_tokens_match_prepare_context(self, return_perf_metrics, reuse_match_backoff):
        # Both settings of return_perf_metrics, because it selects which manager
        # attributes _create_kv_cache reads on the way to the impl. Both settings
        # of the backoff, because it is the one input that lengthens the match
        # evidence: a probe that marshals tokens itself instead of going through
        # _context_reuse_tokens agrees at 0 and diverges at 2.
        mgr = make_stub_manager(reuse_match_backoff=reuse_match_backoff)
    ...
    def test_last_token_is_excluded(self):
        """The match drops the final token (it cannot be recovered)."""
...
def test_marshalled_tokens_are_never_boolean_tested(monkeypatch):
    """The probe must test the token sequence by length, not by truthiness.

    ``_context_reuse_tokens`` hands back the request's zero-copy int32 view when
    the C++ core is installed, and ``bool()`` on an array of more than one
    element raises. A truthiness guard therefore does not degrade to a missed
    deferral: it takes down the executor event loop on the first context request
    of every run, which is every run, since block reuse and prefix-aware
    scheduling are both on by default.
    """
    monkeypatch.setattr(kvcm_v2, "_cpp_introspection", object())
    mgr = make_stub_manager()
    req = make_request(range(20))
    with pytest.raises(ValueError):
        bool(mgr._context_reuse_tokens(req))
    assert mgr.probe_first_new_block_key(req) is not None
