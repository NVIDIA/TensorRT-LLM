...
def _quota_for_resident_sequences(num_sequences):
    ...
    A typical request costs one 96-byte state slot plus its attention pages:
    avg_seq_len is unset, so typical capacity is max_seq_len / 2 = 64 tokens =
    2 blocks of 16 B/token * 32 tokens. One dummy slot and one attention page
    are reserved regardless of residency.
    """
    state_bytes = 64 + 32
    attention_block_bytes = 16 * 32
    per_sequence = state_bytes + 2 * attention_block_bytes
