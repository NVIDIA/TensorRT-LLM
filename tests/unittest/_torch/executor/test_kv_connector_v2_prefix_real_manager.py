...
def test_attention_sinks_are_never_masked(vswa_manager):
    ...
    sliding, full = _sliding_and_full(vswa_manager)
    tokens_per_block = vswa_manager.tokens_per_block
    num_sink_tokens = 2 * tokens_per_block
...
def test_a_sliding_single_group_reports_sentinels_in_the_flat_list():
    """The flat list is still reported, and it is no longer V1's list.

    V1 drops the blocks the window has passed; here they are held in place as
    ``BAD_PAGE_INDEX`` so the ordinals stay aligned. A connector carried over
    from V1 has to filter, which is what `valid_page_slots` is for.
    """
    torch.cuda.init()
