...
def _make_creator(tokens_per_block, dummy_reqs, enable_attention_dp, tp_size,
                  batch_size=1, model_max_seq_len=1,
    ...
    c._mapping = Mock(enable_attention_dp=enable_attention_dp,
                      tp_size=tp_size, cp_config={})

    c._llm_args = Mock(disable_overlap_scheduler=True)

    c._model_engine = Mock(batch_size=batch_size,
...
def _no_gpu():
    ...
    huge = 100 * (1 << 30)
    with patch('torch.cuda.mem_get_info', return_value=(huge, huge)), \
...
def test_adp_reduces_blocks_to_per_rank_share():
    ...
    baseline = _make_creator(tpb, [_make_mock_request(n_in)],
                             enable_attention_dp=False, tp_size=1)
    adp = _make_creator(tpb,
                        [_make_mock_request(n_in) for _ in range(tp)],
                        enable_attention_dp=True, tp_size=tp)

    assert adp._get_token_num_for_estimation() \
        == baseline._get_token_num_for_estimation()
...
def test_without_adp_all_blocks_counted():
    ...
    c = _make_creator(tpb,
                      [_make_mock_request(n_in) for _ in range(n_reqs)],
                      enable_attention_dp=False, tp_size=1)
...
def test_adp_various_tp_sizes(tp_size):
    ...
    c = _make_creator(tpb,
                      ...
                      enable_attention_dp=True, tp_size=tp_size)
