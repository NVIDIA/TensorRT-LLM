    ...
    with torch.no_grad():
        if qkv_mode == QKVMode.FUSE_QKV:
            ...
            if ref_attn.qkv_proj.bias is not None:
                ...
                tp_attn.qkv_proj.bias.data.copy_(torch.cat([
...
def _manual_attention_with_norm(x, qkv_weight, qkv_bias, out_weight, out_bias,
                                norm_q_w, norm_k_w, eps, num_heads, head_dim):
...
def _run_tp_with_params(rank, world_size, batch, seq, hidden_size, num_heads,
                        qkv_mode=QKVMode.FUSE_QKV, qk_norm=False):
    ...
    attn_ref = Attention(
        hidden_size, num_heads, qkv_mode=qkv_mode, qk_norm=qk_norm,
        qk_norm_mode="full", config=config_ref,
    ).to(device)
    ...
    attn_tp = Attention(
        hidden_size, num_heads, qkv_mode=qkv_mode, qk_norm=qk_norm,
        qk_norm_mode="full", config=config_tp,
    ).to(device)
    ...
    if qkv_mode == QKVMode.FUSE_QKV:
        ...
        if qk_norm:
            ref_out = _manual_attention_with_norm(
                x, qkv_w, qkv_b, out_w, out_b,
                attn_ref.norm_q.weight.data, attn_ref.norm_k.weight.data,
                attn_ref.norm_q.variance_epsilon, num_heads, head_dim,
            )
        else:
            ref_out = _manual_attention(x, qkv_w, qkv_b, out_w, out_b, num_heads, head_dim)
    else:
        ref_out = _manual_attention_separate(
            x, *_wb(attn_ref.to_q), *_wb(attn_ref.to_k), *_wb(attn_ref.to_v),
            out_w, out_b, num_heads, head_dim,
        )
...
def _logic_tp_ulysses_combined(rank, world_size):
    ...
    attn_combined = Attention(hidden_size, num_heads, qk_norm=False, config=config_combined).to(device)
    ...
    x_shard = x_full[:, ulysses_rank * seq_per_rank : (ulysses_rank + 1) * seq_per_rank].contiguous()
