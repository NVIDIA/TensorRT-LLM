...
class TestRunDiffusionWorkerDeviceAssignment:
    ...
    def _call_worker(
        self, rank, world_size, local_rank, mock_set_device, init_process_group_error=None
    ):
        ...
        with (
            ...
            patch(
                ...
                side_effect=init_process_group_error,
            ),
            ...
            if init_process_group_error is None:
                run_worker()
            else:
                with pytest.raises(RuntimeError, match=str(init_process_group_error)):
                    run_worker()
    ...
    def test_releases_affinity_when_worker_initialization_fails(self):
        self._call_worker(
            rank=0,
            world_size=1,
            local_rank=0,
            mock_set_device=MagicMock(),
            init_process_group_error=RuntimeError("distributed setup failed"),
        )
