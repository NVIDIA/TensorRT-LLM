...
# TransformerEngine availability (the TE backend imports cleanly without it, but
# TEAttention.__init__ raises ImportError, so probe the package directly).
try:
...
def _run_attn2d_te_case(
    ...
    """Attention2DAttention with the FP8 TE inner backend vs full-sequence SDPA.

    Single stream: batch=1, i.e. one CFG stream evaluated on the mesh, which is
    what the NVL72 recipe runs per CFG replica.

    The head count mirrors the x72 recipe on Wan 2.2 T2V-A14B: 40 attention heads
    sharded by ulysses_size=4 leaves 10 heads on each context-parallel rank.

    FP8 tolerances are looser than the BF16 FA4 cases above: the TE backend
    quantizes Q/K/V, so the comparison is on cosine similarity and relative
    Frobenius error rather than elementwise assert_close.
    """
    if not _te_available:
        pytest.skip("transformer_engine not available")

    seq_full = seq_per_rank * world_size
    batch = 1  # single stream
    # Use whatever device the launcher already selected: _init_worker calls
    # set_device(rank % device_count), and a multi-node launcher sets the node-local
    # rank. Deriving it from the global rank instead breaks as soon as the ranks per
    # node differ from the GPUs per node -- tensors land on one device while the NCCL
    # communicator is bound to another, which surfaces as an illegal memory access.
    device = (
    ...
    inner = TEAttention(num_heads=num_heads, head_dim=head_dim)
    try:
        attn = Attention2DAttention(inner, row_pg, col_pg)
    except ImportError:
        pytest.skip("flash_attn_combine JIT kernels not available")
    ...
    torch.manual_seed(42)
    q_full = torch.randn(
        batch, seq_full, num_heads, head_dim, device=device, dtype=torch.bfloat16
    )
    k_full = torch.randn(
        batch, seq_full, num_heads, head_dim, device=device, dtype=torch.bfloat16
    )
    ...
    out_f = output.reshape(-1).float()
    exp_f = expected_shard.reshape(-1).float()
    cos_sim = F.cosine_similarity(out_f, exp_f, dim=0).item()
    rel_err = (torch.linalg.vector_norm(out_f - exp_f) / torch.linalg.vector_norm(exp_f)).item()

    assert cos_sim > 0.99, (
        f"Rank {rank}: {row_size}x{col_size} TE Attention2D cosine similarity "
        f"{cos_sim:.4f} < 0.99 vs full-sequence attention"
    )
    # Measured on GB300: TEAttention on its own (no Attention2D) lands at 0.077-0.080
    # relative error against fp32 SDPA for B=1 H=10 D=128 across S=128..1024, and the
    # 2x2 and 3x3 meshes reproduce the same 0.077-0.079. FP8 quantization sets the
    # floor here, not the merge, so the bound only has to catch a broken combine.
    assert rel_err < 0.12, (
        f"Rank {rank}: {row_size}x{col_size} TE Attention2D relative error "
        f"{rel_err:.4f} > 0.12 vs full-sequence attention"
    )


def _logic_attn2d_te_2x2(rank, world_size):
    """TE inner backend on a 2x2 mesh (4 GPUs, single node)."""
    _run_attn2d_te_case(rank, world_size, row_size=2, col_size=2)


def _logic_attn2d_te_x72_3x3(rank, world_size):
    """TE inner backend on the 3x3 mesh of the 72-GPU `cfg2 x ulysses4 x A2D 3x3` recipe."""
    _run_attn2d_te_case(rank, world_size, row_size=3, col_size=3)


class TestAttn2DAttentionTE:
    """Attention2DAttention using the FP8 TransformerEngine backend as inner backend.

    These cover the path TEAttention.forward_with_lse exists for: the partial
    output and LSE it returns are merged by flash_attn_combine across the row
    group, so an error in either one shows up as a mismatch against
    full-sequence attention.
    """

    def test_attn2d_te_2x2_mesh(self):
        """Single stream on a 2x2 mesh: runs on one 4-GPU node."""
        run_test_in_distributed(world_size=4, test_fn=_logic_attn2d_te_2x2, use_cuda=True)

    def test_attn2d_te_x72_single_stream(self):
        """Single stream on the 3x3 context-parallel mesh of the 72-GPU recipe."""
        run_test_in_distributed(world_size=9, test_fn=_logic_attn2d_te_x72_3x3, use_cuda=True)
