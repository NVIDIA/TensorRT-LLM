...
def _logic_flux1_tp_vs_single_gpu_with_config(rank, world_size, config_dict):
    ...
    with torch.no_grad():
        ...
        tp_output = tp_model(
            hidden_states=hidden_states,
            attn_metadata=flux_attn_metadata(tp_model, hidden_states, encoder_hidden_states),
            encoder_hidden_states=encoder_hidden_states,
            pooled_projections=pooled_projections,
            timestep=timestep,
            img_ids=img_ids,
...
def _logic_flux2_tp_ulysses_vs_single_gpu(rank, world_size):
    ...
    with torch.no_grad():
        ...
        combined_output = combined_model(
            hidden_states=hidden_states,
            attn_metadata=flux_attn_metadata(combined_model, hidden_states, encoder_hidden_states),
            encoder_hidden_states=encoder_hidden_states,
            timestep=timestep,
            img_ids=img_ids,
            txt_ids=txt_ids,
