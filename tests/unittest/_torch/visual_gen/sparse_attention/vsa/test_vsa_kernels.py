...
"""VSA kernels against references.

The predictor kernels (tile + cube mean, row sort, coarse/fine blend) are
checked on CUDA (Triton path) and CPU (PyTorch fallback) over cube layouts with
ragged fills and head layouts whose row width is not a power of two. The CuTe
DSL block-sparse fine kernel is checked against dense and masked SDPA
references; the TRTLLM fine stage runs the PrimTS block-sparse FMHA covered by
...
_REQUIRES_CUDA = pytest.mark.skipif(not torch.cuda.is_available(), reason="kernel test needs CUDA")
...
# --------------------------------------------------------------------------- CuTe DSL fine kernel
def _cute_fine_kernel(q: torch.Tensor):
    """Return the CuTe block-sparse fine kernel or skip when this device cannot run it."""
    ...
    return block_sparse_attn_from_indices_cute
...
def test_cute_kernel_matches_dense_at_full_topk() -> None:
    ...
    out_kernel, _lse = kernel(q, k, v, q2k_idx, q2k_num, variable_block_sizes)
...
def test_cute_kernel_50pct_sparsity_quality_vs_dense() -> None:
    ...
    out_sparse, _lse = kernel(q, k, v, q2k_idx, q2k_num, variable_block_sizes)
