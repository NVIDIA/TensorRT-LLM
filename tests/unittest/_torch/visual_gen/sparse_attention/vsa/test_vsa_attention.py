...
"""Behavior tests for the VisualGen VSA backends.

Both the CuTeDSL and the TRTLLM backend share the VisualGen-owned predictor and
post-processing; this module covers dispatch, the predictor envelope, metadata
caching and the CUDA Graph contract. Numerical parity of the two backends with
dense attention lives in ``test_vsa_parity.py``, and the kernels are covered by
``test_vsa_kernels.py``.
"""
