...
# Quantization dtype -> quant_attention_config accepted by AttentionConfig.
QUANT_CONFIGS = {
    None: None,
...
# Cosine similarity against an FP32 reference. The quantized dtypes are bounded by
# the FP8 P*V GEMM that cuDNN performs internally on both quantized paths.
MIN_COSINE = {None: 0.9999, "fp8": 0.995, "mxfp8": 0.995}


def _require_cudnn(quant_dtype: str | None) -> None:
    if not torch.cuda.is_available():
        pytest.skip("cuDNN attention backend requires CUDA.")
    if quant_dtype is not None and torch.cuda.get_device_capability()[0] < 10:
...
def test_cudnn_attention(quant_dtype, shape, is_causal):
    """Output and LSE match an FP32 SDPA reference for every quantization mode and mask."""
    ...
    assert attention.quant_dtype == quant_dtype
    ...
    assert cosine > MIN_COSINE[quant_dtype], (
        f"quant_dtype={quant_dtype!r}: cosine similarity {cosine} too low"
    )
    # LSE is computed in FP32 by cuDNN in all modes; only the quantized logits differ.
    torch.testing.assert_close(
        lse, ref_lse, atol=0.5 if quant_dtype is not None else 1e-2, rtol=0.0
    )
...
def test_mxfp8_scale_factor_layout_roundtrip(seq_len, head_dim):
    ...
    Guards the two scale-factor layouts the MXFP8 mode relies on: Q/K block along
...
def test_cudnn_backend_wires_validated_quant_dtype():
    ...
    attention = create_attention(
        ...
        attention_config=AttentionConfig(
            backend="CUDNN", quant_attention_config=QUANT_CONFIGS["mxfp8"]
        ),
    )
    assert isinstance(attention, CuDNNAttention)
    assert attention.quant_dtype == "mxfp8"
