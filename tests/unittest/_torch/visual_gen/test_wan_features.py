...
Tests what the correctness-vs-HF pipeline tests do NOT cover:
  ...
  - CFG parallelism (requires 2 GPUs)
...
# ============================================================================
# CFG parallel worker — module-level for mp.spawn pickling
# ============================================================================
...
def _cfg_worker(rank, world_size, checkpoint_path, inputs, return_dict):
    ...
    try:
        ...
        assert cfg["enabled"] and cfg["cfg_size"] == world_size, (
            f"Rank {rank}: CFG parallel not configured correctly"
        )
...
class TestWanPipelineFeatures:
    ...
    def test_fp8_e2e_accuracy(
        ...
        """FP8/FP8_BLOCK_SCALES full-transformer output close to BF16 (cos_sim > 0.95)."""
        ...
        assert cos_sim > 0.95, f"cos_sim too low: {cos_sim:.6f}"


# ============================================================================
# Tests: multi-GPU CFG parallel
# ============================================================================
...
class TestWanCFGParallel:
    """CFG parallelism: two GPUs each handle one prompt branch (positive / negative)."""

    def test_cfg_parallel_single_step(self):
        """CFG parallel config is correct and produces valid output for one denoising step."""
        if not os.path.exists(WAN21_1_3B_PATH):
            pytest.skip(
                f"Checkpoint not found: {WAN21_1_3B_PATH} (set DIFFUSION_MODEL_PATH_WAN21_1_3B)"
            )
        ...
        assert not torch.isnan(output).any(), "CFG parallel output contains NaN"
        assert not torch.isinf(output).any(), "CFG parallel output contains Inf"
        print(
