...
def _load_eval_module():
    ...
    script_path = repo_root / "examples" / "visual_gen" / "qwen_image_bench_eval.py"
    spec = importlib.util.spec_from_file_location("qwen_image_bench_eval",
                                                  script_path)
...
def test_build_user_prompt_contains_dimension_and_image_marker():
    ...
    user_prompt = module.build_user_prompt("A cute cat playing piano",
                                           "Alignment")
...
"""
    ...
    fixed_scores, dimension_score = module.parse_dimension_output(
        output, "Quality")
