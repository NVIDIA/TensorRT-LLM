...
def test_sfc_finalize_all_zeros():
    """All-zero raw scales must produce finite, zero-valued carrier and scale metadata (no NaN/Inf)."""
    raw = torch.zeros(128, 8, dtype=torch.float32)
    max_raw = torch.amax(raw)
    is_positive = max_raw > 0.0
    safe_max_raw = torch.where(
        is_positive, max_raw, torch.tensor(1.0, dtype=raw.dtype)
    )
    s = torch.where(is_positive, FP8_E4M3_MAX / safe_max_raw, torch.zeros_like(max_raw))
    reciprocal_scale = max_raw / 448.0
    assert s == 0.0
    assert reciprocal_scale == 0.0
    assert torch.isfinite(s)
    assert torch.isfinite(reciprocal_scale)

    if torch.cuda.is_available() and hasattr(torch.ops.trtllm, "nvfp4_sfc_finalize"):
        raw_cuda = raw.cuda()
        sf, s_cuda, max_raw_cuda = torch.ops.trtllm.nvfp4_sfc_finalize(raw_cuda)
        assert s_cuda == 0.0
        assert max_raw_cuda == 0.0
        assert torch.isfinite(s_cuda)
        assert (sf == 0).all()
