...
#
# Tests for the LTX-2 fused DiT AdaLN kernel family (cpp/tensorrt_llm/kernels/fusedDiTNormKernel.cu)
...
def test_resid_rmsnorm_shift_scale_dual_bf16(hidden_dim, batch_size, tokens_per_batch):
    ...
    x_impl = x.clone()
    o1, o2 = torch.ops.trtllm.fused_dit_resid_rmsnorm_shift_scale_dual(
...
def test_resid_rmsnorm_shift_scale_dual_fp4_quant(hidden_dim, batch_size, tokens_per_batch):
    ...
    x_bf16 = x.clone()
    o1_bf16, o2_bf16 = torch.ops.trtllm.fused_dit_resid_rmsnorm_shift_scale_dual(
    ...
    x_fp4 = x.clone()
    o1_fp4, o1_sf, o2_fp4, o2_sf = torch.ops.trtllm.fused_dit_resid_rmsnorm_shift_scale_dual_quant(
        x_fp4,
        attn2,
        s1_table,
        s1_ts,
...
def test_gate_resid_rmsnorm_shift_scale_fp4_quant(hidden_dim, batch_size, tokens_per_batch):
    ...
    x_bf16 = x.clone()
    out_bf16 = torch.ops.trtllm.fused_dit_gate_resid_rmsnorm_shift_scale(
...
def test_gate_resid_rmsnorm_bf16(hidden_dim, batch_size, tokens_per_batch):
    ...
    x_impl = x.clone()
    out = torch.ops.trtllm.fused_dit_gate_resid_rmsnorm(x_impl, attn, gate_table, gate_ts, eps)
...
def test_gate_resid_rmsnorm_quant_rejects_unsupported_dim():
    ...
    with pytest.raises(RuntimeError):
        torch.ops.trtllm.fused_dit_gate_resid_rmsnorm_quant(
...
def test_gate_resid_bf16(hidden_dim, batch_size, tokens_per_batch):
    ...
    x_impl = x.clone()
    out = torch.ops.trtllm.fused_dit_gate_resid(x_impl, attn, gate_table, gate_ts)
...
def test_gate_resid_rejects_non_contiguous():
    ...
    with pytest.raises((RuntimeError, AssertionError)):
        torch.ops.trtllm.fused_dit_gate_resid(x_nc, attn[:, ::2], gate_table[::2], gate_ts[:, ::2])
