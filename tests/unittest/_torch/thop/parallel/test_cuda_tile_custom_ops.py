...
pytestmark = pytest.mark.skipif(
    not IS_CUDA_TILE_AVAILABLE,
    reason="CUDA tile is not available"
)
...
def prepare_testcase_environment(tmp_path):
    ...
    # Set cuda.tile IR and binary dump directories.
...
def reference_rms_norm(
    ...
    hidden_states = F.rms_norm(
        ...
        weight=weight_to_apply.to(torch.float32),
        eps=eps
    )
...
def test_cuda_tile_rms_norm(M, N, use_gemma, static_persistent, gather, dtype):
    ...
    from tensorrt_llm._torch.custom_ops.cuda_tile_custom_ops import cuda_tile_rms_norm
    ...
    cuda_output = cuda_tile_rms_norm(
        x=x,
        weight=weight,
        eps=eps,
...
def test_cuda_tile_rms_norm_fuse_residual(M, N, use_gemma, static_persistent, gather, dtype):
    ...
    from tensorrt_llm._torch.custom_ops.cuda_tile_custom_ops import cuda_tile_rms_norm_fuse_residual_
