...
if not torch.cuda.is_available():
    pytest.skip("CUDA is required for indexer_topk tests",
...
def _make_logits(num_rows: int, num_columns: int,
                 seed: int) -> torch.Tensor:
    generator = torch.Generator(device="cuda").manual_seed(seed)
    return torch.randn(num_rows,
                       num_columns,
                       dtype=torch.float32,
                       device="cuda",
                       generator=generator)
...
def _row_kv_lens(kv_lens: list[int], verify_lens: list[int]) -> torch.Tensor:
    ...
    for kv_len, verify_len in zip(kv_lens, verify_lens):
        rows.extend(kv_len - verify_len + offset + 1
                    for offset in range(verify_len))
    return torch.tensor(rows, dtype=torch.int32, device="cuda")


def _run(logits: torch.Tensor,
         seq_lens: list[int],
         next_n: int,
         row_kv_lens: torch.Tensor = None,
         compress_ratio: int = 1) -> torch.Tensor:
    num_rows = logits.shape[0]
    indices = torch.empty(num_rows,
                          INDEX_TOPK,
                          dtype=torch.int32,
                          device="cuda")
...
def test_genuinely_ragged_matches_per_request_reference():
    ...
    ragged = _run(logits,
                  kv_lens,
                  next_n=1,
                  row_kv_lens=_row_kv_lens(kv_lens, verify_lens))
...
def test_divisible_ragged_batch_is_not_silently_accepted():
    ...
    assert not torch.equal(ragged.sort(dim=-1).values,
                           accidental.sort(dim=-1).values), (
...
def test_row_kv_lens_length_is_validated():
    ...
    with pytest.raises(RuntimeError, match="one entry per logits row"):
        _run(logits, [1024, 2048], next_n=1,
             row_kv_lens=torch.tensor([1024] * 4,
                                      dtype=torch.int32,
                                      device="cuda"))
...
def test_cuda_graph_capture_replays_with_new_lens():
    ...
    indices = torch.empty(total_rows,
                          INDEX_TOPK,
                          dtype=torch.int32,
                          device="cuda")
    ...
    eager = _run(logits,
                 kv_lens,
                 next_n=1,
                 row_kv_lens=_row_kv_lens(kv_lens, new_verify_lens))
