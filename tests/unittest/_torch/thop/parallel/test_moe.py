...
class TestMoeFp4:
    ...
    def run_moe_fp4_test(self, num_tokens: int, hidden_size: int,
        ...
        if use_topk_as_input:
            if routing_method_type != RoutingMethodType.DeepSeekV3 or num_tokens != 150:
                pytest.skip(
                    "use_topk_as_input is tested only with routing_method_type=DeepSeekV3 and num_tokens=150"
                )
...
def test_moe_fp8_per_tensor_scale(num_tokens, expert_info, hidden_size,
    ...
    if use_topk_as_input:
        if num_tokens != 4:
            pytest.skip("use_topk_as_input is tested only with num_tokens=4")
    #
    # Data Generation
    #
...
def test_moe_mxe2m1_weights(num_tokens, hidden_size, intermediate_size,
    ...
    if use_topk_as_input:
        if dtype_activation != "mxfp8" or top_k != 4 or act_type_str != "SwiGlu" or not use_autotune or num_tokens != 256:
            pytest.skip(
                "use_topk_as_input is tested only with mxfp8, topk=4, SwiGlu, autotune, and num_tokens=256"
            )
