...
if not torch.cuda.is_available():
    pytest.skip("CUDA is required for compressor tests",
...
def _register_ops() -> None:
    ...
    for candidate in (
            os.path.join(repo, "cpp", "build", "tensorrt_llm", "thop",
                         "libth_common.so"),
        ...
        if os.path.exists(candidate):
            ...
            return
    pytest.skip("neither tensorrt_llm nor libth_common.so is importable",
                allow_module_level=True)
...
class _CompressorCase:
    ...
    def __init__(self, kv_lens: List[int], new_tokens: List[int],
        ...
        max_blocks = max(
            (kv_len + PAGE_SIZE - 1) // PAGE_SIZE for kv_len in kv_lens) + 1
        num_blocks = self.batch_size * max_blocks
        self.block_table = (torch.arange(
            num_blocks, dtype=torch.int32).reshape(self.batch_size,
                                                   max_blocks).to(device))

        self.paged_kv = torch.empty([num_blocks, PAGE_SIZE, STATE_DIM],
                                    dtype=dtype).uniform_(
                                        -1, 1, generator=generator).to(device)
        self.paged_score = torch.empty([num_blocks, PAGE_SIZE, STATE_DIM],
                                       dtype=dtype).uniform_(
                                           -1, 1,
        ...
        self.cu_seq_lens = torch.tensor(cu_seq_lens,
                                        dtype=torch.int32,
                                        device=device)
        ...
        self.cu_kv_comp = torch.tensor(cu_kv_comp,
                                       dtype=torch.int32,
                                       device=device)
    ...
    def run(self, next_n: int,
            new_tokens_per_seq: Optional[torch.Tensor]) -> torch.Tensor:
        ...
        output = torch.full([max(self.total_outputs, 1), HEAD_DIM],
                            float("nan"),
                            dtype=self.dtype,
                            device=self.device)
...
def test_ragged_new_tokens_matches_per_request_uniform(dtype: torch.dtype):
    ...
    ragged_out = batched.run(next_n, torch.tensor(new_tokens,
                                                  dtype=torch.int32,
                                                  device=device))
    assert not torch.isnan(ragged_out).any(), (
        "ragged call left output rows unwritten")
    ...
    for index, (kv_len, count) in enumerate(zip(kv_lens, new_tokens)):
        ...
        single.paged_kv = batched.paged_kv[block_start:block_start +
                                           num_blocks].clone()
        single.paged_score = batched.paged_score[block_start:block_start +
                                                 num_blocks].clone()
