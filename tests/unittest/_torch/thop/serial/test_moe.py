...
# TODO: migrate the use_fine_grained=True variants to
# tests/unittest/_torch/modules/moe/test_moe_backend.py once that backend
# abstraction can autotune the fine-grained cubins.
@pytest.mark.skipif(
...
# TODO(fine_grained-sync): Migrate use_fine_grained=True variants to
# tests/unittest/_torch/modules/moe/test_moe_backend.py once the backend
# abstraction handles autotune for FineGrained cubins without segfaulting.
# For now keep this enabled on SM107 to get FineGrained coverage for the
# NVFP4 producer/consumer cubins.
@pytest.mark.skipif(
    getSMVersion() < 100 or getSMVersion() >= 110,
...
# Calls the runner ops directly rather than going through create_moe_backend, so the
# fine-grained cubins are covered without the backend autotune path.
# TODO: migrate the use_fine_grained=True variant to
# tests/unittest/_torch/modules/moe/test_moe_backend.py once that path is stable.
@pytest.mark.skipif(
