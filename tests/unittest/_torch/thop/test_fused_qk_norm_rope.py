...
def torch_ref_rms_norm_rope(qkv, position_ids, num_heads_q, num_heads_k, num_heads_v, head_dim, eps=1e-5, base=10000.0):
    """
    ...
    rotary_emb = RotaryEmbedding(rope_params=rope_params, head_dim=head_dim, is_neox=True).to(qkv.device)
    ...
    [q_rope, k_rope] = rotary_emb(position_ids.unsqueeze(0), [q_normalized.unsqueeze(0), k_normalized.unsqueeze(0)])
...
# Set lower log level to reduce test output noise
tensorrt_llm.logger.set_level('error')


# Test parameters
head_dims = [64, 128, 256]
num_heads_groups = [(1, 1, 1)]  # (Q heads, K heads, V heads)
num_tokens_list = [1, 8, 16, 128, 1024]
...
def test_fused_qk_norm_rope(head_dim, num_heads_group, num_tokens, dtype):
    ...
    torch.testing.assert_close(
        ...
        rtol=1e-2,
        atol=atol,
        msg=f"Failed with head_dim={head_dim}, num_heads=({num_heads_q},{num_heads_k},{num_heads_v}), num_tokens={num_tokens}, dtype={dtype}"
    )
