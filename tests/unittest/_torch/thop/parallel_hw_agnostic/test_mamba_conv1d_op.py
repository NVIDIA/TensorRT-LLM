    ...
    def test_mamba_conv1d(
        ...
        if max_seq_len == 131072:
            ...
            if total_gpu_mem <= 33 * 1024**3:
                pytest.skip("The long sequence test needs at least 33GB memory, skipping")
        ...
        if remove_padding and req_type == "context":
            last_token_ids = torch.randint(1, seq_len + 1, (batch_size,), dtype=torch.int32)
            last_token_ids[0] = seq_len
            host_context_lengths = last_token_ids.detach().clone().cpu()
            last_token_ids_input = torch.cumsum(last_token_ids, dim=0, dtype=torch.int32).to(device)
        else:
            last_token_ids = torch.ones((batch_size,), dtype=torch.int32, device=device) * seq_len
            host_context_lengths = last_token_ids.detach().clone().cpu()
        ...
        host_request_types = torch.tensor(
            [0 if req_type == "context" else 1] * batch_size, dtype=torch.int32
        )
        ...
        if with_stride:
            base_shape = [x_input.shape[i] for i in range(len(x_input.shape) - 1)]
            pad_pre_shape = base_shape + [pre_stride]
            pad_post_shape = base_shape + [post_stride]
            pad_pre = torch.randn(pad_pre_shape, device=device, dtype=torch_dtype)
            pad_post = torch.randn(pad_post_shape, device=device, dtype=torch_dtype)
            x_input = torch.cat([pad_pre, x_input, pad_post], dim=-1).contiguous()
