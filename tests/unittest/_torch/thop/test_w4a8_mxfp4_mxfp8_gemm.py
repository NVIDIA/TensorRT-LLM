...
class TestFunctional(unittest.TestCase):
    ...
    def test_w4a8_mxfp4_mxfp8_gemm_torch(self, m: int, n: int, k: int):
        ...
        mat_b = torch.ones([n, k // 2], dtype=torch.float32).to(
            torch.uint8).fill_(34).cuda().contiguous()
        mat_b_ref = torch.ones([n, k], dtype=torch.float32).cuda().contiguous()

        a_block_sf = torch.ones([m, k // 32],
                                dtype=torch.float32).cuda().contiguous()
        a_block_sf = a_block_sf.to(torch.uint8).fill_(127).contiguous()

        b_block_sf = torch.ones([n, k // 32],
                                dtype=torch.float32).cuda().contiguous()
        b_block_sf = b_block_sf.to(torch.uint8).fill_(127).contiguous()
