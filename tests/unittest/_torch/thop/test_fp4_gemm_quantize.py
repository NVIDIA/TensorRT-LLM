...
class TestFunctional(unittest.TestCase):
    ...
    def test_fp4_fp8_gemm_trtllmgen(self, m, n, k):
        a = torch.randn([m, k], dtype=torch.float32)
        b = torch.randn([n, k], dtype=torch.float32)
        b_fp8, b_global_sf = torch.ops.tensorrt_llm.quantize_e4m3_per_tensor(b.cuda())
        ...
        a_pt = e2m1_and_ufp8_scale_to_float_tensor_v2(
            a_fp4, a_sf, 1.0 / a_global_sf, sf_vec_size, 1, False
        )
        ...
        a_sf_shuffled = fp4_utils.shuffle_matrix_sf_a(
            a_sf.reshape((m, -1)),
            epilogue_tile_m,
            sf_vec_size
        )
