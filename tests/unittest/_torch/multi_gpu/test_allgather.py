...
def test_allgather_pg_op(seq_len, hidden_size, var_len):
    ...
    world_size = 2
    if var_len:
        test_tensor_list = [torch.randn((seq_len * (i+1), hidden_size), dtype=dtype) for i in range(world_size)]
    ...
    try:
        ...
        for rank in range(world_size):
            remotePGTests.append(AllgatherPGTest.options(runtime_env=runtime_env).remote(
                    rank, world_size
                )
        )

        if var_len:
            results = ray.get([remotePGTest.run_allgather_pg_op.remote(test_tensor, expected_result, sizes) for remotePGTest, test_tensor in zip(remotePGTests,test_tensor_list)])
        else:
            results = ray.get([remotePGTest.run_allgather_pg_op.remote(test_tensor, expected_result, sizes) for remotePGTest in remotePGTests])
