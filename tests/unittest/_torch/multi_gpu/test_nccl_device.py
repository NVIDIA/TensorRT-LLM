...
def run_single_rank_ar_rms_norm(tensor_parallel_size, a, b, c, gamma):
    ...
    try:
        ...
        n_chunk_size = b.size(0) // tensor_parallel_size
        ...
        ar = AllReduce(mapping=mapping, strategy=AllReduceStrategy.NCCL_SYMMETRIC)
...
def test_user_buffers_ar_rms_norm(mnk, mpi_pool_executor):
    ...
    assert k % tensor_parallel_size == 0, f"k dimension {k} must be divisible by tensor_parallel_size {tensor_parallel_size}"
    assert n % tensor_parallel_size == 0, f"n dimension {n} must be divisible by tensor_parallel_size {tensor_parallel_size}"
