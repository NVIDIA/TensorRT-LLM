...
def test_cuda_actual_sampler_saved_q_and_graph_replay(skip) -> None:
    ...
    sampling_mode = AdvancedSamplingMode.NO_TOPK_NO_TOPP if skip else meta.advanced_sampling_mode
    expected = spec.SpecWorkerBase._compute_probs_from_logits(
        sampling_mode,
        logits,
...
def test_cuda_actual_rs_preserves_target_distribution(skip) -> None:
    ...
    p = spec.SpecWorkerBase._compute_probs_from_logits(
        meta.advanced_sampling_mode,
        target_logits,
        meta.temperatures,
        meta.top_ks,
        meta.top_ps,
...
def _run_adp_draft_transitions(skip: bool, rejection: bool) -> int:
    ...
    for flags in ((True, True), (True, False), (False, True), (False, False), (True, True)):
        ...
        if rejection and not all(flags):
            sampling_mode = (
                AdvancedSamplingMode.NO_TOPK_NO_TOPP if skip else meta.advanced_sampling_mode
            )
            expected_q = spec.SpecWorkerBase._compute_probs_from_logits(
                sampling_mode,
                logits,
                meta.request_temperatures,
                meta.request_top_ks,
