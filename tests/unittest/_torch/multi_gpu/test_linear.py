...
@pytest.mark.skipif(torch.cuda.device_count() < 2,
                    reason='needs 2 GPUs to run this test')
@pytest.mark.parametrize("seq_len", [512, 1024], ids=lambda x: f"seqlen:{x}")
@pytest.mark.parametrize("output_size", [256, 512], ids=lambda x: f"output:{x}")
@pytest.mark.parametrize("hidden_size", [256, 512], ids=lambda x: f"hidden:{x}")
