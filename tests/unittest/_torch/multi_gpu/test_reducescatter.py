...
class ReducescatterPGTest:
    ...
    def run_reducescatter_pg_op(self, test_tensor, expected_result, sizes):
        ...
        mapping = Mapping(world_size=self.world_size,
            gpus_per_node=self.world_size, tp_size=self.world_size, rank=self.rank)
        ...
        rtol, atol = 0.05, 0.15
        torch.testing.assert_close(result_tensor, expected_result, rtol=rtol, atol=atol)
...
@pytest.mark.parametrize("hidden_size", [128, 1024], ids=lambda x: f"hidden:{x}")
...
def test_reducescatter_pg_op(seq_len, hidden_size, var_len):
    ...
    if var_len:
        ...
        for i in range(world_size):
            expected_result_list.append(test_tensor[offset:offset+seq_len * (i+1)] * world_size)
            offset += seq_len * (i+1)
        sizes = [seq_len * (i+1) for i in range(world_size)]
    else:
        test_tensor = torch.randn((seq_len*world_size, hidden_size), dtype=dtype)
        expected_result_list = [test_tensor[i*seq_len:(i+1)*seq_len] * world_size for i in range(world_size)]
        sizes = None
    ray_init_args = {
    ...
    try:
        ...
        for rank in range(world_size):
            remotePGTests.append(ReducescatterPGTest.options(runtime_env=runtime_env).remote(
                    rank, world_size
                )
        )

        if var_len:
            results = ray.get([remotePGTest.run_reducescatter_pg_op.remote(test_tensor, expected_result, sizes) for remotePGTest, expected_result in zip(remotePGTests,expected_result_list)])
        else:
            results = ray.get([remotePGTest.run_reducescatter_pg_op.remote(test_tensor, expected_result, sizes) for remotePGTest, expected_result in zip(remotePGTests,expected_result_list)])
