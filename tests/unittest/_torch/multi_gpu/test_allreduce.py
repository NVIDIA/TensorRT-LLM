...
def run_mxfp8_op(
    ...
    def mxfp8_quantize(x, swizzled):
        # Nested to keep "ops" out of this function's co_names; see
        # run_nvfp4_linear_sf_op for why that matters to cloudpickle.
    ...
    for strategy in (AllReduceStrategy.ONESHOT, AllReduceStrategy.TWOSHOT):
        ...
        assert_same_bytes(residual_out, ref_residual, f"{strategy} residual_out")
...
@pytest.mark.parametrize("seq_len", [4, 128, 130],
                         ids=lambda x: f"seqlen:{x}")
@pytest.mark.parametrize("hidden_size", [128, 6144],
                         ids=lambda x: f"hidden:{x}")
@pytest.mark.parametrize("mpi_pool_executor", [2], indirect=True)
def test_allreduce_fusion_mxfp8(seq_len, hidden_size, mpi_pool_executor):
    ...
    results = mpi_pool_executor.map(
        run_single_rank,
        *zip(*[(tensor_parallel_size, run_mxfp8_op, x, residual, [],
                hidden_size, dtype,
                AllReduceFusionOp.RESIDUAL_RMS_NORM_OUT_QUANT_MXFP8)] *
             tensor_parallel_size),
    )
    for r in results:
        assert r is True
