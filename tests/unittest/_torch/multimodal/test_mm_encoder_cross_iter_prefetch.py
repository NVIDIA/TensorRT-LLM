...
@requires_cuda
def test_cross_iter_prefetch_does_not_synchronize_main_stream(mocker):
    ...
    device_synchronize = mocker.spy(torch.cuda, "synchronize")
    stream_synchronize = mocker.spy(torch.cuda.Stream, "synchronize")
    event_synchronize = mocker.spy(torch.cuda.Event, "synchronize")

    n = maybe_prefetch_mm_encoder_for_next_iter(model, [req], max_prefetch_ahead=2)

    assert n == 1
    device_synchronize.assert_not_called()
    stream_synchronize.assert_not_called()
    event_synchronize.assert_not_called()
    # Drain the queued aux-stream work before teardown.
    req.py_mm_encoder_event.synchronize()
