...
def test_async_padding_preserves_connector_plan() -> None:
    ...
    # Stand in for KVCacheManager.prepare_resources, which publishes the batch
    # to the connector unless the caller opts out.
    def prepare_resources(scheduled_batch, *, publish_connector_output=True):
        if publish_connector_output:
            manager.set_scheduler_output(SchedulerOutput())

    cache.prepare_resources.side_effect = prepare_resources
    ...
    cache.prepare_resources.assert_called_once_with(batch, publish_connector_output=False)
...
def test_adp_v2_query_commit_and_callbacks(forbid_connector_collectives: None) -> None:
    ...
    manager.cancel_load(request, 4, 8)
    manager.scheduler.cancel_load.assert_called_once_with(request, 4, 8)
    manager.commit_new_matched_tokens(request, 4, False)
    assert manager.scheduler_output_manager.external_loads[17] == 4
    manager.update_state_after_alloc(request, [2, 4])
    manager.scheduler.update_state_after_alloc.assert_called_once_with(request, [2, 4])
    assert manager.request_finished(request, [2, 4])
    manager.scheduler.request_finished.assert_called_once_with(request, [2, 4])
