...
- Insufficient-KV fail-fast during the benchmark fill phase
"""
...
class TestFailFastDuringBenchmarkFill:
    ...
    def test_fitting_init_requests_are_prepared_for_transfer(self) -> None:
        """Every scheduler-fitting INIT request is prepared for KV transfer."""
        fitting = [_make_active_request(in_init=True), _make_active_request(in_init=True)]
        ex = self._make_executor(fill_phase_active=True, fitting_init_requests=fitting)
        ex._check_disagg_transfer_progress_when_idle = Mock()

        result, _ = ex._prepare_and_schedule_batch()

        assert result is not None
        ex._prepare_disagg_gen_init.assert_called_once_with(fitting)
