"""Functional tests for col_sharded_global_rmsnorm custom op.

This module tests that the col_sharded_global_rmsnorm op produces correct
...
def _run_col_sharded_global_rmsnorm_test(
    rank: int,
    world_size: int,
    batch_size: int = 2,
    seq_len: int = 8,
    ...
    """Test that col_sharded_global_rmsnorm matches non-sharded global RMSNorm.
    ...
    4. Calls col_sharded_global_rmsnorm on local shard
    ...
    # Call col_sharded_global_rmsnorm
    local_output = torch.ops.auto_deploy.col_sharded_global_rmsnorm(
        local_input, local_weight, eps, world_size
    )
    ...
    torch.testing.assert_close(
        ...
        atol=1e-2,
        rtol=1e-2,
        msg=f"col_sharded_global_rmsnorm result doesn't match reference (rank={rank})",
    )


@pytest.mark.parametrize("device_count", get_device_counts(num_gpu_list=[2]))
def test_col_sharded_global_rmsnorm_functional(device_count):
    """Functional test: verify col_sharded_global_rmsnorm produces correct numerical results."""
    spawn_multiprocess_job(job=_run_col_sharded_global_rmsnorm_test, size=device_count)


@pytest.mark.parametrize("device_count", get_device_counts(num_gpu_list=[2]))
@pytest.mark.parametrize("hidden_size", [64, 128, 256])
def test_col_sharded_global_rmsnorm_different_hidden_sizes(device_count, hidden_size):
    """Test col_sharded_global_rmsnorm with different hidden sizes."""
    spawn_multiprocess_job(
        job=partial(_run_col_sharded_global_rmsnorm_test, hidden_size=hidden_size),
        size=device_count,
    )
...
def test_col_sharded_global_rmsnorm_different_dtypes(device_count, dtype):
    """Test col_sharded_global_rmsnorm with different dtypes."""
    spawn_multiprocess_job(
        job=partial(_run_col_sharded_global_rmsnorm_test, dtype=dtype),
        size=device_count,
    )
