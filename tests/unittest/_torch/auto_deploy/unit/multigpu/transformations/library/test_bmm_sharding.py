...
from tensorrt_llm._torch.auto_deploy.transformations.transform import torch_export_to_gm
from tensorrt_llm._torch.auto_deploy.transformations.library.sharding import bmm_shard
...
class BMM(nn.Module):
    def __init__(self, num_experts, num_features):
        ...
        self.gate_up_proj = nn.Parameter(torch.randn(self.num_experts, self.num_features, 2 * self.num_features))
        self.down_proj = nn.Parameter(torch.randn((self.num_experts, self.num_features, self.num_features)))
...
def test_sharding(device_count: int):
    dist_common.spawn_multiprocess_job(
        job=partial(_run_job),
        size=device_count,
    )
