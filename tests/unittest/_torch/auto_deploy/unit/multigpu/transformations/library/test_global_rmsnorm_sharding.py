...
1. Full hidden norm (weight shape = [num_heads * head_dim]):
   - Detected as global QK norm → replaced with col_sharded_global_rmsnorm
...
class SimpleAttentionWithQKNorm(nn.Module):
    ...
    Args:
        ...
        use_full_hidden_norm: If True, use full hidden norm (like MiniMax)
            ...
            - Should be transformed to col_sharded_global_rmsnorm
...
class TestGlobalRMSNormShardingTransform:
    ...
    def test_full_hidden_norm_transformed_to_global(self):
        """Test that full hidden norm RMSNorm ops are replaced with col_sharded_global_rmsnorm.

        When weight shape = [hidden_size] (matches q_proj output dim):
        - torch_rmsnorm should be replaced with col_sharded_global_rmsnorm
        ...
        before_global = count_ops(gm, torch.ops.auto_deploy.col_sharded_global_rmsnorm.default)
        assert before_rmsnorm == 2, f"Expected 2 torch_rmsnorm before, got {before_rmsnorm}"
        assert before_global == 0, (
            f"Expected 0 col_sharded_global_rmsnorm before, got {before_global}"
        )
        ...
        after_global = count_ops(
            gm_transformed, torch.ops.auto_deploy.col_sharded_global_rmsnorm.default
        )

        # The QK norms (weight matching q/k output dims) should be transformed
        assert after_global == 2, (
            f"Expected 2 col_sharded_global_rmsnorm after transform, got {after_global}. "
    ...
    def test_per_head_norm_not_transformed(self):
        ...
        - No col_sharded_global_rmsnorm should be added
        """
        ...
        assert after_global == 0, (
            f"Expected 0 col_sharded_global_rmsnorm for per-head norm, got {after_global}"
        )
        ...
        print(
            f"After transform: {after_rmsnorm} torch_rmsnorm, {after_global} col_sharded_global_rmsnorm"
        )
