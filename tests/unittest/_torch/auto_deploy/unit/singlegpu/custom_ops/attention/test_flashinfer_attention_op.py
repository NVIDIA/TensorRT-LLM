...
def test_flashinfer_attention_op_context(seq_length, n_heads, batch_size, dtype, device):
    ...
    batch_info_host = torch.tensor(
        [BATCH_SIZE, BATCH_SIZE * SEQ_LEN, 0, 0, 0, 0], dtype=torch.int32, device=device
    )
...
def test_flashinfer_attention_context_and_generate(
    ...
    # [num_prefill, num_prefill_tokens, num_extend, num_extend_tokens, num_decode, num_decode_tokens]
    batch_info_host = torch.tensor(
        [BATCH_SIZE, BATCH_SIZE * PREFILL_SEQ_LEN, 0, 0, 0, 0], dtype=torch.int32, device=device
    )
...
def test_flashinfer_attention_with_paged_kvcache(seq_lengths, n_heads, dtype, device):
    ...
    # [num_prefill, num_prefill_tokens, num_extend, num_extend_tokens, num_decode, num_decode_tokens]
    batch_info_host = torch.tensor(
        [BATCH_SIZE, SEQ_LEN, 0, 0, 0, 0], dtype=torch.int32, device=device
    )
    flashinfer_output = torch.ops.auto_deploy.flashinfer_attention_mha_with_cache(
        # Q, K, V
        q,
