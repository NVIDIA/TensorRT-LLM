...
def test_decode_only(gdr_env):
    ...
    q_flat = q_norm.view(batch, num_heads, -1)
    k_flat = k_norm.view(batch, num_heads, -1)
    v_flat = v.view(batch, num_heads, -1)
    g_flat = g.view(batch, num_heads)
    beta_flat = beta.view(batch, num_heads)
