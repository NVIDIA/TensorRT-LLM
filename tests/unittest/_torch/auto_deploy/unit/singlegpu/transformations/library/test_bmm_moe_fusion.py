...
def test_bmm_moe_fusion_with_reference(dtype):
    ...
    # Check for view nodes that might be batched_input
    view_nodes = [n for n in gm_original.graph.nodes if is_op(n, torch.ops.aten.view)]
    print(f"\nFound {len(view_nodes)} view nodes:")
    for i, view_node in enumerate(view_nodes[:5]):  # Print first 5
        print(f"  View {i}: {view_node.name}")
        if view_node.args and isinstance(view_node.args[0], Node):
            print(f"    args[0]: {view_node.args[0].name} (op: {view_node.args[0].op})")
            if is_op(view_node.args[0], torch.ops.aten.mul):
                print("    -> Found mul before view!")
                mul_node = view_node.args[0]
                if mul_node.args:
                    for j, arg in enumerate(mul_node.args[:2]):
                        if isinstance(arg, Node):
                            print(f"      mul.args[{j}]: {arg.name} (op: {arg.op})")
                            if is_op(arg, torch.ops.aten.repeat):
                                print("        -> Found repeat!")
                            elif is_op(arg, torch.ops.aten.reshape):
                                print("        -> Found reshape (routing weights)!")
                                # Trace back from reshape to find topk
                                reshape_node = arg
                                print("        Tracing routing path from reshape...")
                                # Check if reshape has all_input_nodes attribute
                                if hasattr(reshape_node, "all_input_nodes"):
                                    print(
                                        f"          reshape.all_input_nodes: "
                                        f"{[n.name for n in reshape_node.all_input_nodes]}"
                                    )
                                else:
                                    print(
                                        "          reshape does NOT have all_input_nodes attribute!"
                                    )
                                # Also check scatter's args to see if topk is there
                                if reshape_node.args and isinstance(reshape_node.args[0], Node):
                                    transpose_check = reshape_node.args[0]
                                    if (
                                        is_op(transpose_check, torch.ops.aten.transpose)
                                        and transpose_check.args
                                    ):
                                        sigmoid_check = (
                                            transpose_check.args[0]
                                            if isinstance(transpose_check.args[0], Node)
                                            else None
                                        )
                                        if (
                                            sigmoid_check
                                            and is_op(sigmoid_check, torch.ops.aten.sigmoid)
                                            and sigmoid_check.args
                                        ):
                                            scatter_check = (
                                                sigmoid_check.args[0]
                                                if isinstance(sigmoid_check.args[0], Node)
                                                else None
                                            )
                                            if scatter_check and is_op(
                                                scatter_check, torch.ops.aten.scatter
                                            ):
                                                print(
                                                    f"          Found scatter node: {scatter_check.name}"
                                                )
                                                scatter_args_str = [
                                                    str(a) if not isinstance(a, Node) else a.name
                                                    for a in (
                                                        scatter_check.args[:5]
                                                        if scatter_check.args
                                                        else []
                                                    )
                                                ]
                                                print(f"          scatter.args: {scatter_args_str}")
                                                # Check if topk is in scatter's args
                                                if (
                                                    scatter_check.args
                                                    and len(scatter_check.args) > 3
                                                ):
                                                    src_arg = (
                                                        scatter_check.args[3]
                                                        if len(scatter_check.args) > 3
                                                        else None
                                                    )
                                                    if isinstance(src_arg, Node) and is_op(
                                                        src_arg, torch.ops.aten.topk
                                                    ):
                                                        print(
                                                            "            -> Found topk in scatter.args[3]!"
                                                        )
                                                    elif (
                                                        isinstance(src_arg, Node)
                                                        and src_arg.op == "call_function"
                                                        and hasattr(src_arg.target, "__name__")
                                                        and src_arg.target.__name__ == "getitem"
                                                    ):
                                                        getitem_check = src_arg
                                                        if (
                                                            getitem_check.args
                                                            and isinstance(
                                                                getitem_check.args[0], Node
                                                            )
                                                            and is_op(
                                                                getitem_check.args[0],
                                                                torch.ops.aten.topk,
                                                            )
                                                        ):
                                                            print(
                                                                "            -> Found topk via getitem "
                                                                "in scatter.args[3]!"
                                                            )
                                # Try to manually trace back
                                if reshape_node.args and isinstance(reshape_node.args[0], Node):
                                    reshape_arg0 = reshape_node.args[0]
                                    print(
                                        f"          reshape.args[0]: {reshape_arg0.name} "
                                        f"(op: {reshape_arg0.op})"
                                    )
                                    if is_op(reshape_node.args[0], torch.ops.aten.transpose):
                                        transpose_node = reshape_node.args[0]
                                        print("            -> Found transpose!")
                                        if transpose_node.args and isinstance(
                                            transpose_node.args[0], Node
                                        ):
                                            transpose_arg0 = transpose_node.args[0]
                                            print(
                                                f"            transpose.args[0]: {transpose_arg0.name} "
                                                f"(op: {transpose_arg0.op})"
                                            )
                                            if is_op(
                                                transpose_node.args[0], torch.ops.aten.sigmoid
                                            ):
                                                sigmoid_node = transpose_node.args[0]
                                                print("                -> Found sigmoid!")
                                                if sigmoid_node.args and isinstance(
                                                    sigmoid_node.args[0], Node
                                                ):
                                                    sigmoid_arg0 = sigmoid_node.args[0]
                                                    print(
                                                        f"                sigmoid.args[0]: "
                                                        f"{sigmoid_arg0.name} (op: {sigmoid_arg0.op})"
                                                    )
                                                    if is_op(
                                                        sigmoid_node.args[0], torch.ops.aten.scatter
                                                    ):
                                                        scatter_node = sigmoid_node.args[0]
                                                        print(
                                                            "                    -> Found scatter!"
                                                        )
                                                        if scatter_node.args and isinstance(
                                                            scatter_node.args[0], Node
                                                        ):
                                                            scatter_arg0 = scatter_node.args[0]
                                                            print(
                                                                f"                    scatter.args[0]: "
                                                                f"{scatter_arg0.name} (op: {scatter_arg0.op})"
                                                            )
                                                            if is_op(
                                                                scatter_node.args[0],
                                                                torch.ops.aten.topk,
                                                                ...
                                                                scatter_node.args[0].op
                                                                == "call_function"
                                                                and hasattr(
                                                                    scatter_node.args[0].target,
                                                                    "__name__",
                                                                )
                                                                and scatter_node.args[
                                                                    0
                                                                ].target.__name__
                                                                == "getitem"
                                                            ):
                                                                getitem_node = scatter_node.args[0]
                                                                print(
                                                                    "                        -> Found getitem, "
                                                                    "checking its input..."
                                                                )
                                                                if getitem_node.args and isinstance(
                                                                    getitem_node.args[0], Node
                                                                ):
                                                                    getitem_arg0 = (
                                                                        getitem_node.args[0]
                                                                    )
                                                                    print(
                                                                        f"                        getitem.args[0]: "
                                                                        f"{getitem_arg0.name} (op: {getitem_arg0.op})"
                                                                    )
                                                                    if is_op(
                                                                        getitem_node.args[0],
                                                                        torch.ops.aten.topk,
