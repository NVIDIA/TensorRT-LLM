...
from tensorrt_llm._torch.auto_deploy.custom_ops.triton_attention import (
...
from tensorrt_llm._torch.auto_deploy.transformations.library.sdpa import (
    insert_unfused_mha_with_kv_cache,
)
...
class GQAWithSdpa(GQA):
    ...
    def forward(self, x: torch.Tensor, position_ids: Optional[torch.Tensor] = None) -> torch.Tensor:
        ...
        attn_output = torch.ops.sdpa.attention(
...
def test_sdpa_with_kv_cache(dtype, attention_op, gqa_config):
    ...
    gm_transformed = insert_unfused_mha_with_kv_cache(
