...
def unsupported_reason(backend: str, case) -> Optional[str]:
    ...
    if sparse_config is not None:
        ...
        if backend == "TRTLLM" and algorithm == "deepseek_v4":
            # Selected sparse MLA uses trtllm-gen kernels that only ship for
            # Blackwell (sm_100+). On Hopper, MLA generation falls back to the
            # dense FlashMLA kernel, which has no sparse path.
            if sm < 100:
                return f"TRTLLM DeepSeek V4 requires sm>=100/Blackwell (have sm{sm})"
