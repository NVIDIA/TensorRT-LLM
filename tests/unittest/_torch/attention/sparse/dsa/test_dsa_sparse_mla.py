...
def _apply_yarn_rope(x: torch.Tensor, cos: torch.Tensor, sin: torch.Tensor) -> torch.Tensor:
    dtype = x.dtype
    x = x.unflatten(-1, [-1, 2]).transpose(-2, -1).flatten(start_dim=-2)
    x = ((x * cos) + (rotate_half(x) * sin)).to(dtype)
    return x.unflatten(-1, [2, -1]).transpose(-2, -1).flatten(start_dim=-2)


def _rope_qk_for_vanilla(
    ...
    """RoPE-pre-apply the yarn positional embedding to the Vanilla golden's q_pe/k_pe.

    Vanilla applies no positional embedding itself while the kernel does via ``rope_append``;
    the query rope dims (in ``fused_q``) and the k_pe tail (in ``latent_cache``) are rotated at
    absolute positions ``[past, past + seq_len)`` per request.
    """
    fused_q = fused_q.clone()
    latent_cache = latent_cache.clone()
    fused_head_dim = latent_cache.shape[-1]
    off = 0
    for seq_len, past in zip(seq_lens, past_lens):
        cos, sin = rope_cos_sin[past : past + seq_len].chunk(2, dim=-2)
        q = fused_q[off : off + seq_len].view(seq_len, num_heads, fused_head_dim)
        q[..., -qk_rope_head_dim:] = _apply_yarn_rope(q[..., -qk_rope_head_dim:], cos, sin)
        fused_q[off : off + seq_len] = q.view(seq_len, -1)
        kv = latent_cache[off : off + seq_len]
        kv[..., -qk_rope_head_dim:] = _apply_yarn_rope(
            kv[..., -qk_rope_head_dim:].unsqueeze(-2), cos, sin
        ).squeeze(-2)
        off += seq_len
...
def _run_test_for_backend(
    ...
    rope_cos_sin = _yarn_rope_cos_sin(rope_config, device) if is_vanilla else None
        ...
        for step in range(num_generation_steps + 1):
            ...
            for layer_idx in range(num_layers):
                ...
                if step == 0:
                    ...
                    if is_vanilla:
                        # Vanilla has no indexer and applies no RoPE; feed the selection directly
                        # and pre-apply RoPE at context positions 0..len.
                        ctx_fused_q, ctx_latent = _rope_qk_for_vanilla(
                            fused_q,
                            latent_cache,
                            rope_cos_sin,
                            context_sequence_lengths,
                    ...
                    if is_vanilla:
                        # Pre-apply RoPE at absolute positions [cached_len, cached_len+seq_q).
                        seq_q = [generation_seq_len_q] * len(context_sequence_lengths)
                        backend_fused_q, backend_latent_cache = _rope_qk_for_vanilla(
                            fused_q,
                            latent_cache,
                            rope_cos_sin,
                            seq_q,
