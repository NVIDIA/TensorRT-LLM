    ...
    class MockMetadata(DSAtrtllmAttentionMetadata):
        def __init__(self):
            ...
            self.topk_indices_buffer = torch.zeros(
                (num_tokens, self.sparse_mla_topk), device="cuda", dtype=torch.int32
            )

            if self.num_contexts > 0 and self.enable_indexer_skip:
                self.skip_indexer_for_ctx_reqs = (
                    kv_lens[: self.num_contexts].max().item() <= self.sparse_mla_topk
                )
            else:
            ...
            if self.num_generations > 0 and self.enable_indexer_skip:
                ...
                self.skip_indexer_for_gen_reqs = (
                    kv_lens[self.num_contexts : self.num_seqs].max().item() <= self.sparse_mla_topk
                )
            else:
...
def test_indexer_k_cache_scatter_custom_op():
    ...
    torch.ops.trtllm.indexer_k_cache_scatter_op(
        k_fp8_bytes, k_scale_bytes, k_cache_cuda, flat_indices_fp8, flat_indices_scale
    )
    ...
    for tok_id in range(total_tokens):
        ...
        if window_size >= index_topk:
            ...
            assert num_valid == index_topk, (
                f"[Custom Topk] Token {tok_id}: window={window_size}, "
                f"but only {num_valid}/{index_topk} valid indices"
            )
