...
def test_cutedsl_mqa_logits_output_buffer_persistent():
    ...
    cache_manager, sparse_attn_config = create_dsa_cache_manager(
        ...
        max_seq_len=kv_len, num_layers=1, index_topk=index_topk)
    create_indexer(sparse_attn_config, layer_idx=0)

    request_ids = list(range(batch_size))
    kv_lens = torch.full((batch_size,), kv_len, dtype=torch.int32)
    cache_manager.add_dummy_requests(
        request_ids=request_ids, token_nums=kv_lens.tolist(),
        is_gen=False, prepare_resource=True)

    metadata = _create_mock_metadata(
        request_ids, batch_size, num_contexts=0, num_generations=batch_size,
        seq_lens=torch.full((batch_size,), next_n, dtype=torch.int32),
        kv_lens=kv_lens.clone(), num_cached_tokens=[kv_len - next_n] * batch_size,
        cache_manager=cache_manager, num_ctx_tokens=0,
        num_tokens=batch_size * next_n, max_draft_tokens=next_n - 1,
        index_topk=index_topk, use_cute_dsl_paged_mqa_logits=True)
    Indexer.prepare(metadata)
