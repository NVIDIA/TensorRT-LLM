...
- :class:`BaseKVCacheCompressionManager` ABC contract: the 8 lifecycle hooks
  ...
  once registered).
- :class:`SparseAttentionManager` / :class:`KVCacheStorageManager` subclasses.
- The resource-manager API -> semantic-hook translation, gated on PyExecutor's
  ...
  ``update_resources`` fires ``on_context_end`` for each request in
...
class TestBaseABC:
    ...
    def test_8_hooks_default_noop(self, fake_kv_cache_manager):
        ...
        assert m.on_context_end(MagicMock(), meta) is None
    ...
    def test_implements_introspection(self, fake_kv_cache_manager):
        ...
        assert m.implements("on_context_end") is True
...
class TestSubclasses:
    def test_sparse_manager(self):
        assert issubclass(SparseAttentionManager, BaseKVCacheCompressionManager)
        assert SparseAttentionManager.supports_kv_cache_reuse is False
        assert SparseAttentionManager.physically_evicts_kv is False

    def test_storage_manager(self):
        assert issubclass(KVCacheStorageManager, BaseKVCacheCompressionManager)
...
class TestResourceManagerAPI:
    ...
    def test_update_fires_context_end_on_last_chunk(self, fake_kv_cache_manager):
        ...
        assert "s:on_context_end" in rec
...
class TestFactories:
    ...
    def test_sparse_factory_none_for_legacy_algorithm(self, fake_kv_cache_manager):
        cfg = MagicMock()
        cfg.algorithm = "rocket"  # legacy memory-layer method, not behavior-layer
...
class TestCanonicalImports:
    def test_manager_names_importable(self):
        ...
        assert hasattr(sparse, "SparseAttentionManager")
        assert hasattr(sparse, "KVCacheStorageManager")
