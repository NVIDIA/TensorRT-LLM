...
# of the CUDA fixes in PR #17550; see gvr_topk_decode.py Phase-3 repair)
# ============================================================================
# Three inputs used to defeat the threshold search and produce a silently
# wrong per-row top-K:
#
#   1. hostile / degenerate hint — the search starts from a bracket of
#      HINTED values with invented counts; when both ends sit on the same
#      side of the true K-th value the old retry-shrink only repaired the
#      overflow side, and an all-identical hint used to emit row[0:K]
#      verbatim (the head of the row, not a top-K).
#   2. ReLU-sparse rows — a handful of positive scores plus an exact-0.0
#      plateau wider than kC: no threshold admits a count in [K, kC], and
#      the R0-miss retry's fail-soft used to stamp done=1 on an
#      undershooting threshold, shipping (K - count) trailing -1 slots.
#      The V3.2 MTP draft layer produces exactly these rows on
#      low-temporal-locality (e.g. random-token) prompts.
#   3. the same shapes under MTP (next_n > 1) row geometry.
#
# All are hint/shape-quality driven, so they deliberately violate the
# "pre_idx[..., 0] is the argmax" speed contract: after the repair the hint
# may only ever affect speed, never the answer. TRTLLM_GVR_TIERS_DISABLE
# forces the in-tree kernel (the tiers have their own guards and are
# covered by test_cute_dsl_gvr_topk_tiers.py).
...
def test_cute_dsl_gvr_topk_decode_hostile_hint(top_k, hint, _intree_only):
    ...
    ``uniform`` (every slot = the same index) collapses Phase 1's min/max
    bracket to a point, which used to short-circuit the kernel into
    emitting row[0:top_k].
    """
    N, cr = 65536, 4
...
def test_cute_dsl_gvr_topk_decode_relu_sparse_plateau(n_pos, _intree_only):
    """ReLU-sparse row: n_pos positives + an exact-0.0 plateau wider than kC.

    The K-th value is 0.0 with more ties than the candidate buffer holds and
    the count profile jumps from n_pos straight past kC — the shipped
    R0-miss fail-soft stamped done=1 on the undershooting side and returned
    (top_k - n_pos) trailing -1 slots. The repair must return top_k distinct
    valid indices (all positives + an arbitrary subset of the tie class).
    """
