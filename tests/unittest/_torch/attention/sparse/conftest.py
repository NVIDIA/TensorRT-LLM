...
"""Shared checkers for the top-K decode test modules in this directory.

The tie-aware checker lives here (single definition) and is handed to tests
...
def _tie_aware_check_impl(
    ...
    ``ref_vals_cache``: optional id-keyed memo dict for the reference top-K
    values. Pass it ONLY when (logits, seq_lens) are pinned alive for the
    process lifetime by the caller (id-keying on transient tensors would
    alias after GC); the caller owns that guarantee.
    """
    ...
    # per-row N_eff.
    ref_key = None
    if ref_vals_cache is not None:
        ref_key = (id(logits), id(seq_lens), top_k, next_n, compress_ratio)
    if ref_key is not None and ref_key in ref_vals_cache:
        ref_vals = ref_vals_cache[ref_key]
    else:
        ...
        if ref_key is not None:
            ref_vals_cache[ref_key] = ref_vals
...
@pytest.fixture
def tie_aware_check():
    """The shared tie-aware top-K correctness checker (plain function)."""
    return _tie_aware_check_impl
