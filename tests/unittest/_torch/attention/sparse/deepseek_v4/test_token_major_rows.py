...
def _expand(meta, *, num_contexts, row_req_idx, num_seqs, block_offsets):
    ...
    meta.row_req_idx_host[:ngt].copy_(
        torch.tensor(row_req_idx, dtype=torch.long))
    meta.row_kv_lens_host[:ngt].fill_(7)
    meta.row_kv_correction_host[:ngt].zero_()
    meta.prompt_lens_cpu[:num_seqs].fill_(1)
    meta._prepare_token_major_gen_rows(
        torch.full((num_seqs, ), 7, dtype=torch.int32))
...
def test_row_block_table_matches_op_layout(num_pools):
    ...
    meta = _metadata(num_pools=num_pools, max_blocks=max_blocks,
                     max_seqs=max_seqs)

    src = torch.arange(num_pools * max_seqs * 2 * max_blocks,
                       dtype=torch.int32,
                       device="cuda").reshape(num_pools, max_seqs, 2,
                                              max_blocks)
...
def test_row_indices_stay_in_bounds_of_the_sequence_axis():
    ...
    assert int(idx.max()) < src.shape[1], (
        f"row->request map reaches {int(idx.max())} but the sequence axis is "
        f"{src.shape[1]} long")
...
def test_context_rows_precede_generation_rows():
    ...
    src = torch.arange(1 * 6 * 2 * 8, dtype=torch.int32,
                       device="cuda").reshape(1, 6, 2, 8)
    nc = 2
    rows = _expand(meta,
                   num_contexts=nc,
                   row_req_idx=[0, 1, 1],
