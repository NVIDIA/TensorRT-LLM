...
def test_nvfp4_mla_kv_cache_gather(global_dequant_scale_value):
    ...
    nibble_pairs = torch.arange(packed_head_dim, dtype=torch.uint8, device="cuda")
    packed_row = ((nibble_pairs * 2 + 1) << 4) | (nibble_pairs * 2)
    data_pool = packed_row.repeat(num_pool_tokens, 1).contiguous()
