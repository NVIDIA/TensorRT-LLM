...
from tensorrt_llm._torch.attention_backend.sparse.dsa import DSABackendForwardArgs
...
def _build_sparse_topk_indices(
    ...
    Rows alternate between ``singleton`` (one token repeated across all top-k
    slots, giving an analytic value oracle) and ``random``, except when the
    visible prefix already fits within top-k, which selects densely.
    ``singleton_oracle_mask`` marks the singleton rows for the caller.

    Only token-level selection is implemented; ``indices_block_size`` must be 1.
    """

    # TODO: Remove the check if we support block/page/compress-entry unit sparse.
    if indices_block_size != 1:
        raise NotImplementedError(
            f"Only token-level selection generation is implemented "
            f"(indices_block_size=1), got {indices_block_size}"
        )
...
def generate_sparse_mla_inputs(case: BackendCase, seed: int = 0) -> Dict:
    ...
    topk_indices, singleton_oracle_mask = _build_sparse_topk_indices(
        case,
        selection_gen,
        indices_block_size=sparse_params.indices_block_size,
    )
    ...
    *,
    ...
    try:
        ...
        for attention_input_type, token_slice in phases:
            ...
            # Selection correctness is validated separately (test_sparse_mla_forward.py);
            # this harness only checks the attention math given a selection, so it
            # injects the harness-built top-k directly on Vanilla and mocks the
            # production indexer to return the same selection on other backends.
            ...
            if backend == "VANILLA":
                ...
                attn.indexer.forward_from_projected = Mock(return_value=topk_indices)
