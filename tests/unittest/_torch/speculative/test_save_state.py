...
def test_multi_save_state():
    ...
    with tempfile.TemporaryDirectory() as temp_dir:
        ...
        assert saved_data["hidden_state_features"].shape == (
            len(tok_ids), 2048 * len(layers_to_capture))
        ...
        # with torch.no_grad():
        #     tok_ids = torch.tensor([tok_ids], dtype=torch.long, device="cuda")
        #     hf_hidden_states = hf_model(tok_ids, output_hidden_states=True).hidden_states
        # # compare the hidden states of saved and HF version
        # concat_hidden_states = torch.cat([
        #     hf_hidden_states[i+1][0] for i in list(layers_to_capture)], dim=1).cpu()
        # assert torch.allclose(hidden_states, concat_hidden_states, atol=1e-2)
...
def test_save_state(layers_to_capture):
    ...
    with tempfile.TemporaryDirectory() as temp_dir:
        ...
        if layers_to_capture is None:
            assert saved_data["hidden_state_features"].shape == (len(tok_ids),
                                                                 2048 * 3)
            assert saved_data["hidden_state"].shape == (len(tok_ids), 2048)
            assert saved_data["input_ids"].tolist() == tok_ids
        else:
