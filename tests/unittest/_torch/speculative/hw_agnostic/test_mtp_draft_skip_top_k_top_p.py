...
def _worker() -> SimpleNamespace:
    ...
    for name in (
        "_sample_from_logits",
        "_sample_from_logits_with_probs",
        "_compute_probs_from_logits",
        "_zero_padding_rows",
        "_restore_greedy_draft_rows",
    ):
        setattr(worker, name, getattr(spec.SpecWorkerBase, name))
    return worker
...
def _metadata(batch=2, vocab=8, steps=2, device="cpu", skip=True, mode="full") -> SimpleNamespace:
    ...
    return SimpleNamespace(
        ...
        request_min_ps=torch.zeros(batch, device=device),
...
def test_step_callsite_filters_and_exact_q_storage(monkeypatch, mode, skip) -> None:
    ...
    def sampler(sampling_mode, logits, temperatures, top_k, top_p, min_p, *, seed, offset):
        top_k, top_p = spec.flashinfer_sampling.resolve_advanced_sampling_filters(
    ...
    worker = _worker()
    worker._sample_from_logits_with_probs = sampler
    tokens = spec.SpecWorkerBase.advanced_sample_draft(
        worker, torch.zeros(2, 8), meta, 2, draft_step=1
    )
    expected_mode = AdvancedSamplingMode.NO_TOPK_NO_TOPP if skip else meta.advanced_sampling_mode
    assert seen["mode"] is expected_mode
...
def test_token_only_branch_filters_without_proposal_storage(
    ...
    worker = _worker()
    worker._sample_from_logits = sampler
    tokens = spec.SpecWorkerBase.advanced_sample_draft(
        worker, torch.zeros(2, 8), meta, 2, draft_step=step
    )
    expected_mode = AdvancedSamplingMode.NO_TOPK_NO_TOPP if skip else mode
    assert seen["mode"] is expected_mode
...
@pytest.mark.cpu_only
def test_temperature_only_draft_keeps_greedy_tie_deterministic() -> None:
    from tensorrt_llm._torch.pyexecutor.sampler.ops.vanilla import GREEDY_TEMPERATURE_THRESHOLD

    logits = torch.tensor([[4.0, 4.0, 1.0], [1.0, 2.0, 3.0]])
    temperatures = torch.tensor([GREEDY_TEMPERATURE_THRESHOLD / 10, 0.8])
    sampled_tokens = torch.tensor([1, 2], dtype=torch.int32)
    sampled_probs = torch.tensor([[0.5, 0.5, 0.0], [0.1, 0.2, 0.7]])
    tokens, probs = spec.SpecWorkerBase._restore_greedy_draft_rows(
        logits, temperatures, sampled_tokens, sampled_probs
    )
    assert torch.equal(tokens, torch.tensor([0, 2], dtype=torch.int32))
    assert probs is not None
    torch.testing.assert_close(probs[0], torch.tensor([1.0, 0.0, 0.0]))
    torch.testing.assert_close(probs[1], sampled_probs[1])
...
def test_target_filter_and_proposal_pass_through(monkeypatch, skip, mode) -> None:
    ...
    meta.min_ps = meta.request_min_ps.repeat_interleave(2)
    seen = {}
    proposal = torch.full((2, 1, 8), 0.125)

    def probs(sampling_mode, logits, temperatures, top_k, top_p, min_p):
        top_k, top_p = spec.flashinfer_sampling.resolve_advanced_sampling_filters(
            sampling_mode, top_k, top_p
        )
        seen["target_mode"] = sampling_mode
    ...
    worker = _worker()
    worker._compute_probs_from_logits = probs
    spec.SpecWorkerBase._sample_and_accept_draft_tokens_rejection(
        worker, torch.zeros(4, 8), torch.zeros(2, 1, dtype=torch.int32), proposal, 0, 2, meta
    )
    assert seen["target_mode"] is mode
...
def test_context_target_sampling_unchanged(
    ...
    meta.min_ps = meta.request_min_ps
    meta.seeds = meta.request_seeds
    meta.offsets = meta.request_offsets
    seen = {}

    def sample(sampling_mode, logits, temperatures, top_k, top_p, min_p, *, seed, offset):
        top_k, top_p = spec.flashinfer_sampling.resolve_advanced_sampling_filters(
            sampling_mode, top_k, top_p
        )
        seen.update(
    ...
    worker = _worker()
    worker._sample_from_logits = sample
    spec.SpecWorkerBase._sample_tokens_for_batch(worker, torch.zeros(2, 8), meta, 2, 2)
...
def test_vanilla_mtp_draft_logits_layout(
    ...
    if (skip or rejection) and lm_head_tp and not custom_head:
        with pytest.raises(RuntimeError, match="forward_local_full_vocab"):
            MTPWorker._compute_draft_logits(
                worker,
    ...
    if (skip or rejection) and lm_head_tp:
        assert calls == ["custom_preprocessing", "local_projection"]
        assert torch.equal(logits, (local_states + 1) @ weight)
    else:
...
@pytest.mark.cpu_only
def test_mtp_eagle_local_head_requires_model_hook() -> None:
    from tensorrt_llm._torch.speculative.eagle3 import Eagle3OneModelWorker

    with pytest.raises(RuntimeError, match="forward_local_full_vocab"):
        Eagle3OneModelWorker._forward_local_mtp_head(
            SimpleNamespace(),
            torch.zeros(1, 4),
            SimpleNamespace(lm_head=lambda states: states),
            None,
        )
...
def test_vanilla_mtp_greedy_stochastic_transitions(skip: bool, rejection: bool, rank: int) -> None:
    ...
    def local_full_vocab(states, head, metadata, return_context_logits):
        assert metadata is None and return_context_logits
        return head(states)

    shared_head.forward_local_full_vocab = local_full_vocab
