...
def test_even_spacing_matches_real_k3_drafter():
    ...
    assert GEN.even_target_layer_ids(K3_NUM_TARGET_LAYERS,
                                     6) == K3_TARGET_LAYER_IDS


def test_tensor_plan_matches_real_dspark_schema():
    """Plan must match the dummy-dspark0724 safetensors header exactly."""
    plan = GEN.drafter_tensor_plan(K3_HIDDEN, GEN.K3_DRAFTER,
...
def test_k3_drafter_config_is_dspark():
    cfg = GEN.drafter_config(K3_HIDDEN, K3_VOCAB, K3_NUM_TARGET_LAYERS,
                             K3_TARGET_LAYER_IDS, K3_MASK_TOKEN_ID,
                             GEN.K3_DRAFTER)
    ...
    assert dflash_cfg["use_swa"] is True and \
        dflash_cfg["swa_window_size"] == 1024
...
def test_drafter_config_drives_generic_dflash_path():
    cfg = GEN.drafter_config(K27_HIDDEN, 163840, 61, K27_TARGET_LAYER_IDS,
                             163838, GEN.K27_DRAFTER)
...
def test_generator_tiny_roundtrip(tmp_path):
    ...
    subprocess.run(
        [sys.executable, GEN.__file__, "--tiny", "--out",
         str(out)],
        check=True,
    )
    ...
    plan = GEN.drafter_tensor_plan(
        cfg["hidden_size"], GEN.TINY, len(dflash_cfg["target_layer_ids"]),
        vocab=cfg["vocab_size"], markov_rank=GEN.TINY["markov_rank"],
        use_confidence_head=True)
    ...
    assert all(0 <= t < cfg["num_target_layers"]
               for t in dflash_cfg["target_layer_ids"])
...
def test_generator_real_config_mode(tmp_path):
    ...
    out = tmp_path / "synth"
    subprocess.run(
        [sys.executable, GEN.__file__, "--config",
         str(cfg_path), "--out",
         str(out)],
        check=True,
    )
    ...
    plan = GEN.drafter_tensor_plan(
        ...
        vocab=real_cfg["vocab_size"], markov_rank=16,
        use_confidence_head=True)
    assert {"markov_w1.weight", "markov_w2.weight",
            "confidence_proj.weight", "confidence_proj.bias"} <= set(plan)
    with safe_open(out / "model.safetensors", "pt") as f:
...
@pytest.mark.skipif(not torch.cuda.is_available(),
                    reason="DFlashSpecMetadata allocates capture buffer on cuda")
def test_dflash_spec_metadata_capture_prefix_sum_convention():
    ...
    h1 = torch.randn(max_tokens, hidden_size, dtype=torch.bfloat16,
                     device="cuda")
