...
def test_verify_length_comes_from_the_padded_token_total():
    ...
    got = sorted((s.batch_size, s.verify_len, s.step_time_ms)
                 for s in samples_from_iter_log(
                     [log], max_draft_len=5, padded_batch_sizes=LADDER))
...
def test_the_padded_width_is_what_gets_recorded():
    ...
    Filing it under 244 would put the same shape in as many cells as the
    roster had sizes, and alpha(bs) would be fitted against a row count no
    step ever ran.
    """
...
def test_the_ladder_is_what_removes_the_ambiguity():
    """768 tokens over 128 rows is L=5 at 128, L=3 at 192, or L=2 at 256.

    Without the ladder all three are admissible and the step is dropped; with
    it, padding rounds 128 up to 128 and the answer is L=5. Nearly every
    low-occupancy step is in this position, which is why the ladder matters:
    the first version of this collector kept 312 of 5682 production steps.
    """
    ambiguous = resolve_padded_shape(num_rows=128, num_generation_tokens=768,
                                     max_draft_len=5, max_batch_size=256)
    assert ambiguous is None
    resolved = resolve_padded_shape(num_rows=128, num_generation_tokens=768,
                                    max_draft_len=5, max_batch_size=256,
                                    padded_batch_sizes=LADDER)
    assert resolved == (128, 5)


def test_a_step_whose_padding_declined_is_skipped():
    """_get_padded_batch can bail; then the width the ladder implies is wrong.

    Nothing in the log says padding declined, so the only honest move is to
    drop a step whose token total the padded width cannot explain.
    """
...
def test_steps_beyond_the_ladder_are_skipped_not_mislabelled():
    """Filing a step under a shape it did not run is the mislabelling that
    once shipped eight fictional cells with near-zero residuals."""
    log = _write([
        _line(2, 244, 79.2, 768),       # padded to 256 -> kept
        _line(3, 256, 300.0, 2048),     # L = 7, past the block
        _line(4, 300, 400.0, 1800),     # more rows than the ladder holds
    ])
    got = [(s.batch_size, s.verify_len) for s in
           samples_from_iter_log([log], max_draft_len=5,
                                 padded_batch_sizes=LADDER)]
    assert got == [(256, 2)]
...
def test_only_the_requested_rank_is_read():
    ...
    log = _write([
        ...
        _line(2, 256, 79.9, 768, rank="3"),
    ])
    assert len(samples_from_iter_log([log], max_draft_len=5)) == 1


def test_outlier_steps_are_bounded_out():
    log = _write([
        _line(2, 256, 79.2, 768),
        _line(3, 256, 5000.0, 768),
    ])
...
def test_samples_expose_the_token_total_the_table_is_indexed_by():
    log = _write([_line(1, 1, 100.0, 6), _line(2, 256, 79.2, 768)])
    sample = samples_from_iter_log([log], max_draft_len=5)[0]
    assert sample.total_verify_tokens == 768
