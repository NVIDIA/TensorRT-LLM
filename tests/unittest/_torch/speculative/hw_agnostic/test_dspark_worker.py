...
"""Unit tests for the DSpark worker + metadata plumbing (GPU).
...
draft + accept is covered by the LLM-API test (``test_dspark.py``).
"""
...
def test_worker_max_draft_len():
    assert _make_worker().max_draft_len == 5
...
def test_worker_lazy_init_allocates_batch_to_slot():
    """The CUDA-graph slot mirror is allocated alongside the windows."""
    worker = _make_worker()
