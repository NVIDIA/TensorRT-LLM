...
from tensorrt_llm._torch.models.modeling_speculative import (
    DFlashForCausalLM, dspark_layer_window_size, dspark_markov_chain_logits,
...
def test_markov_chain_matches_deepspec_reference():
    ...
    ref_tokens, ref_logits = _RefVanillaMarkov(w1, w2).sample_block_tokens(
        base, anchor)
...
def test_markov_chain_sharded_matches_full_vocab():
    ...
    reassembled = torch.cat(
        [torch.stack(rank_outputs[r], dim=1) for r in range(tp)], dim=-1)
...
def _tiny_weights(seed=7):
    ...
    nh, nkv, hd = (TINY["num_attention_heads"], TINY["num_key_value_heads"],
                   TINY["head_dim"])
    w = {
...
def _build_drafter(dspark: bool, weights):
    ...
    model_config = ModelConfig(pretrained_config=_tiny_config(dspark),
                               attn_backend="TRTLLM")
    ...
    if not dspark:
        weights = {
            ...
            if not k.startswith(("markov_", "confidence_"))
        }
...
def _oracle_block_decode(weights, captured, noise_embed, use_swa):
    ...
    ctx_feat = _rms(captured.float() @ w["fc.weight"].T,
    ...
    for i in range(TINY["num_hidden_layers"]):
        ...
        k_ctx = (ctx_feat @ w[p + "self_attn.k_proj.weight"].T).view(
            ctx, nkv, hd)
        k_noise = (h @ w[p + "self_attn.k_proj.weight"].T).view(blk, nkv, hd)
        v_ctx = (ctx_feat @ w[p + "self_attn.v_proj.weight"].T).view(
            ctx, nkv, hd)
        ...
        if use_swa:
            ...
            scores = scores.masked_fill(dist.unsqueeze(0) > SWA_WINDOW - 1,
                                        float("-inf"))
...
def test_dspark_drafter_loads_head_weights_and_parses_config():
    ...
    assert drafter._dspark_layer_windows == [(SWA_WINDOW - 1, SWA_WINDOW - 1)
                                             ] * 2
    assert drafter.has_markov_head
    torch.testing.assert_close(drafter.markov_w1.cpu(),
                               weights["markov_w1.weight"])
    torch.testing.assert_close(drafter.markov_w2.cpu(),
                               weights["markov_w2.weight"])
    # Confidence weights loaded for MR B, but never consumed here.
    torch.testing.assert_close(drafter.confidence_proj_weight.cpu(),
                               weights["confidence_proj.weight"])
...
def _run_block_decode(drafter, weights, captured, noise_embed):
    dev = "cuda"
    blk = TINY["block_size"]
    proj = drafter.project_target_hidden(
        captured.to(dev, torch.bfloat16))
    ...
    pool_k = torch.zeros(1, L, CTX_LEN + blk, nkv, hd, dtype=torch.bfloat16,
                         device=dev)
...
def test_dspark_block_decode_matches_reference_oracle():
    ...
    captured = torch.randn(CTX_LEN,
                           TINY["hidden_size"] * NUM_CAPTURE,
                           generator=g) * 0.5
    noise_embed = torch.randn(TINY["block_size"],
                              TINY["hidden_size"],
                              generator=g) * 0.5
...
def test_plain_dflash_block_decode_matches_full_attention_oracle():
    ...
    noise_embed = torch.randn(TINY["block_size"],
                              ...
                              generator=g) * 0.5
    out = _run_block_decode(drafter, weights, captured, noise_embed)
    oracle = _oracle_block_decode(weights, captured.to(torch.bfloat16),
                                  noise_embed.to(torch.bfloat16), False)
