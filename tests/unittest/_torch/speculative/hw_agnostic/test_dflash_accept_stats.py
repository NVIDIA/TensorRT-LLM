...
try:
    ...
    _path = os.path.join(
        os.path.dirname(__file__), *([os.pardir] * 5), "tensorrt_llm",
        "_torch", "speculative", "accept_stats.py")
    _spec = importlib.util.spec_from_file_location("accept_stats",
...
def test_recorder_accumulation_and_flush(tmp_path):
    rec = accept_stats.DFlashAcceptStatsRecorder(str(tmp_path),
                                                 max_draft_len=3,
                                                 rank=0,
                                                 flush_every=1000,
                                                 num_conf_bins=4)
...
def test_merge_snapshots_and_load(tmp_path):
    r0 = accept_stats.DFlashAcceptStatsRecorder(str(tmp_path), 2, rank=0,
                                                num_conf_bins=4)
    r1 = accept_stats.DFlashAcceptStatsRecorder(str(tmp_path), 2, rank=1,
                                                num_conf_bins=4)
...
def test_recorder_excludes_dummy_request_id_zero(tmp_path):
    ...
    assert sum(sum(r) for r in
               snap["confidence_calibration"]["attempts"]) == 3
...
def test_confidence_provider_none_collects_nothing(tmp_path):
    ...
    assert sum(sum(r) for r in
               snap["confidence_calibration"]["attempts"]) == 0
...
def test_mocked_calibration_end_to_end(tmp_path):
    ...
    def mock_provider(draft_model, gen_hidden, first_prev_tokens,
                      gen_draft_tokens):
        provider_calls.append(draft_model)
        return scripted_rows[len(provider_calls) - 1]

    rec = accept_stats.DFlashAcceptStatsRecorder(
        str(tmp_path), max_draft_len=K, rank=0, flush_every=1000,
        num_conf_bins=NBINS, confidence_provider=mock_provider)
