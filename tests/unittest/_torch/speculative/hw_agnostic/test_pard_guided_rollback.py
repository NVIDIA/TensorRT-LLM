...
"""GPU unit tests: PARD speculation under guided decoding.
...
tail in ``execute_draft_batch(draft_step=0)``; PARD predicts the whole draft
block in a single forward and never enters that loop, so ``PARDWorker`` has to
roll the matchers back itself through ``rollback_rejected_batch``, exactly as
``DFlashWorker`` and ``SAWorker`` do (see ``test_dflash_guided_rollback.py``
and ``test_sa_guided_rollback.py``, the latter also pins the decoder-level
rollback semantics with a real xgrammar matcher). Without it the next
verification masks from a grammar state that still contains rejected draft
tokens: grammar-forbidden tokens get through, and once a rejected draft token
terminated the matcher, masking stops entirely.
"""
...
pytestmark = pytest.mark.skipif(
    ...
    reason="the PARD worker runs on CUDA tensors",
)
...
def test_pard_worker_rolls_back_after_verification(monkeypatch):
    worker = PARDWorker(PARDDecodingConfig(max_draft_len=K), Mapping())
    ...
    for stub in (
        "_prepare_attn_metadata_for_pard",
...
def test_pard_worker_skips_the_rollback_when_not_drafting(monkeypatch):
