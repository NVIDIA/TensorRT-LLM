...
def test_rejection_reason_names_the_feature(name, kwargs, expected):
    ...
    # min_p is the one rule the deploy config can waive; the rest hold either way.
    fused = one_model_sampling_rejection_reason(SamplingParams(**kwargs), fused_sampling=True)
    assert fused == (None if name == "min_p" else expected)
