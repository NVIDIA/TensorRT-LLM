...
from tensorrt_llm._torch.speculative.dspark_heads import (
...
def test_rnn_state_carries_across_positions():
    # The RNN head must depend on the prefix: zeroing joint_proj (state path)
    # should change later-position logits vs a non-zero head.
    torch.manual_seed(2)
    head = RNNHead(vocab_size=VOCAB, markov_rank=RANK, hidden_size=HID).eval()
    base = torch.zeros(1, BLK, VOCAB)
    first = torch.zeros(1, dtype=torch.long)
    hid = torch.randn(1, BLK, HID)
    with torch.no_grad():
        _, logits = head.sample_block_tokens(
            base, first_prev_token_ids=first, hidden_states=hid, temperature=0.0
        )
    # Non-trivial bias is produced at every position.
