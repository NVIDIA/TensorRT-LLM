...
"""Unit tests for the DSpark draft I/O (capture + proposal) stages."""

import torch

from tensorrt_llm._torch.speculative.dspark_draft import (
    DSparkCapture,
    build_draft_input_ids,
    dspark_propose,
)
from tensorrt_llm._torch.speculative.dspark_heads import DSparkConfidenceHead, build_markov_head

VOCAB, HID, RANK, B, BLK, NCAP = 257, 32, 16, 2, 5, 3
NOISE_ID = 199


def test_capture_projection_shape():
    cap = DSparkCapture(hidden_size=HID, num_capture_layers=NCAP).eval()
    captured = torch.randn(B, BLK, HID * NCAP)
    out = cap(captured)
    assert out.shape == (B, BLK, HID)
