...
def test_a_grouped_scheduler_and_a_single_group_cache_are_accepted():
    ...
    The single-group arm is the compatibility promise -- every connector
    written against the V1 manager defines only the flat forms, and must keep
    starting up.
    """
...
def test_a_flat_scheduler_under_a_sliding_window_warns_but_runs(monkeypatch):
    ...
    connector is not refused -- refusing would stop a connector that runs on
    the V1 manager today. What it does not get on V1 is a `-1` where the window
    has passed a block, so the difference is logged instead.
    """
