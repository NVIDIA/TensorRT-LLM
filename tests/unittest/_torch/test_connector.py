...
def test_connector_manager_query_is_side_effect_free(mpi_pool_executor):
    ...
    A cache that allocates per context chunk can be offered more than the
    pages the scheduler reserved. It therefore commits the amount it
...
def test_connector_manager_commits_only_what_is_honoured(mpi_pool_executor):
    ...
    ownership of the whole request at `request_finished`.
    """
...
def test_scheduler_output_resets_a_destroyed_allocation():
    ...
    allocation lives and wrong the moment one is destroyed. A destructive
    recompute pause does exactly that and the request replays -- reachable only
    where the scheduler can pause a live request. Left stale, the replay lands in
...
class _GroupedOnlyScheduler(KvCacheConnectorScheduler):
    """A connector written against the per-layer-group API, as VSWA requires."""
    ...
    def request_finished_by_layer_group(self, request,
        ...
        self.finished.append(
            [list(group) for group in cache_block_ids_by_layer_group])
        return False

    def update_state_after_alloc_by_layer_group(self, request,
                                                block_ids_by_layer_group):
        self.allocs.append([list(group) for group in block_ids_by_layer_group])


def test_a_released_request_reaches_the_per_layer_group_callbacks():
    """Empty per-group lists route by length, so a released request still lands.

    A grouped-only connector has no flat method of its own, only the stand-in
    that raises. Routing a released request there kills the executor loop on
    the leader and strands every other rank in the broadcast that was waiting
    on its answer.
    """
    scheduler = _GroupedOnlyScheduler()
...
def test_an_empty_group_list_reaches_the_flat_callbacks():
    """No per-group view of the cache means the flat list is the description.

    A caller that cannot report per layer group leaves the grouped argument
    unset. Dispatching that to the per-layer-group form would reach the ABC
    default's "0 layer groups" refusal instead of the flat callback the
...
def test_a_per_layer_group_connector_needs_no_flat_stubs():
    ...
    # The flat forms are still reachable, and say which form this connector
    # implements rather than reporting nothing.
...
def _grouped_scheduler(*implemented):
    """A scheduler class overriding exactly the named per-layer-group forms."""

    body = {
        "build_connector_meta": lambda self, scheduler_output: None,
        "get_num_new_matched_tokens": lambda self, request, num_computed:
        (0, False),
        "request_finished": lambda self, request, cache_block_ids: False,
        "update_state_after_alloc": lambda self, request, block_ids: None,
    }
    body.update({
        name: lambda self, request, by_group: None
        for name in implemented
    })
    return type("PartialScheduler", (KvCacheConnectorScheduler, ), body)


@pytest.mark.parametrize("implemented", [
    (),
    ("request_finished_by_layer_group", ),
    ("update_state_after_alloc_by_layer_group", ),
])
def test_a_partial_grouped_scheduler_is_rejected_at_bring_up(implemented):
    """Mixing the two forms must fail before the model is loaded.

    The two pairs clear their abstract flags independently, so a connector
    implementing one grouped form and leaving the other flat constructs
    cleanly. Left to the base defaults it would then raise on the first
    scheduled request -- after model load, which is what this check buys back.
    """
    scheduler = _grouped_scheduler(*implemented)(llm_args=None)
    manager = KvCacheConnectorManager(MagicMock(), scheduler=scheduler)

    with pytest.raises(NotImplementedError, match="by_layer_group"):
        manager.reject_flat_only_scheduler(2)


def test_a_grouped_scheduler_and_a_single_group_cache_are_accepted():
    """The two ways to pass: both forms implemented, or only one layer group.

    The single-group arm is the compatibility promise -- every existing
    connector defines only the flat forms, and must keep starting up.
    """
    both = _grouped_scheduler("request_finished_by_layer_group",
                              "update_state_after_alloc_by_layer_group")
    KvCacheConnectorManager(
        MagicMock(),
        scheduler=both(llm_args=None)).reject_flat_only_scheduler(2)

    flat_only = _grouped_scheduler()
    KvCacheConnectorManager(
        MagicMock(),
        scheduler=flat_only(llm_args=None)).reject_flat_only_scheduler(1)
...
def test_a_flat_scheduler_under_a_sliding_window_warns_but_runs(monkeypatch):
    """The compatibility promise and the hazard, together.

    One window across every layer is a single layer group, so a flat-only
    connector is not refused -- refusing would stop connectors that work
    today. What such a connector does not otherwise get is a `-1` where the
    window has passed a block, so the difference is logged instead.
    """
    warnings = _record_warnings(monkeypatch)
    flat_only = _grouped_scheduler()
    manager = KvCacheConnectorManager(MagicMock(),
                                      scheduler=flat_only(llm_args=None))

    manager.reject_flat_only_scheduler(1)
    manager.warn_flat_scheduler_under_swa(4096)

    assert len(warnings) == 1
    assert "valid_page_slots" in warnings[0]


def test_nothing_is_said_where_nothing_changed(monkeypatch):
    """A window is not enough on its own, and neither is a flat scheduler."""
    warnings = _record_warnings(monkeypatch)

    flat_only = _grouped_scheduler()
    KvCacheConnectorManager(
        MagicMock(),
        scheduler=flat_only(llm_args=None)).warn_flat_scheduler_under_swa(None)
