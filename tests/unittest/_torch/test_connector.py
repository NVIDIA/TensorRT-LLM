...
def test_connector_manager_get_finished_allgather(
...
@pytest.mark.parametrize("mpi_pool_executor", [2], indirect=True)
def test_connector_manager_num_matched_tokens(
        mpi_pool_executor: MPIPoolExecutor) -> None:
...
@pytest.mark.parametrize("mpi_pool_executor", [2], indirect=True)
def test_connector_manager_take_scheduled_requests(
        mpi_pool_executor: MPIPoolExecutor) -> None:
...
def test_scheduler_output_num_scheduled_tokens_with_mtp() -> None:
...
def test_scheduler_output_block_hashes_read_through() -> None:
