...
"""The audit that keeps pinned versions and dates out of the staircase tree.

Out of tree, staircase was a separate repo against a pinned `tensorrt_llm`, so
...
def test_no_file_pins_a_version_or_a_date():
    ...
    assert not offences, "staircase files must not pin a version or a date:\n" + "\n".join(offences)
