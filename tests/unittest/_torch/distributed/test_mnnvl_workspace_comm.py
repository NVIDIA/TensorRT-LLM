...
class _FakeProcessGroup:
    """Records the backend-level collectives the workspace helpers issue.

    Under Ray the helpers bypass the public c10d operators (MetaInitMode intercepts the tensors
    they create) and call ``pg._get_backend(cpu)`` directly.
    """

    def __init__(self, peer_flags=()):
        self.backend_devices = []
        self.allreduces = []
        self.waits = 0
        self._peer_flags = list(peer_flags)

    def _get_backend(self, device):
        self.backend_devices.append(device)
        pg = self
        ...
        class _Backend:
            ...
            def allreduce(self, tensors, opts):
                (flag, ) = tensors
                pg.allreduces.append((flag.clone(), opts.reduceOp))
                # Fold in the peers' contributions to the MIN reduction.
                for peer in pg._peer_flags:
                    flag.clamp_(max=peer)
                return _Work()
...
def test_all_succeeded_uses_process_group_backend_under_ray(
        ray_mode, monkeypatch):
    import torch

    pg = _FakeProcessGroup(peer_flags=[1, 1])

    def _no_public_collective(*args, **kwargs):
        pytest.fail(
            "public c10d collectives must not be used (MetaInitMode intercepts them)"
        )

    monkeypatch.setattr(ops.torch.distributed, "all_reduce",
                        _no_public_collective)
    monkeypatch.setattr(ops.torch.distributed, "barrier", _no_public_collective)

    assert ops._mnnvl_workspace_all_succeeded(pg, True) is True
    assert pg.backend_devices == [torch.device("cpu")]
    assert pg.waits == 1
    ((flag, reduce_op), ) = pg.allreduces
    assert flag.tolist() == [
        1
    ] and flag.dtype == torch.int32 and flag.device.type == "cpu"
    assert reduce_op == torch.distributed.ReduceOp.MIN


def test_all_succeeded_reports_a_failing_peer_under_ray(ray_mode):
    assert ops._mnnvl_workspace_all_succeeded(
        _FakeProcessGroup(peer_flags=[1, 0]), True) is False
    # A local failure is reported even when every peer succeeded.
    assert ops._mnnvl_workspace_all_succeeded(
        _FakeProcessGroup(peer_flags=[1, 1]), False) is False


def test_all_succeeded_uses_mpi_comm_under_mpi(mpi_mode):
    ...
    class _FakeMpiComm:
        ...
        def allreduce(self, value):
            ...
            return value + sum(self._peer_flags)

        def Get_size(self):
            return 1 + len(self._peer_flags)

    comm = _FakeMpiComm([1, 1, 1])
    assert ops._mnnvl_workspace_all_succeeded(comm, True) is True
    assert comm.values == [1]
    assert ops._mnnvl_workspace_all_succeeded(_FakeMpiComm([1, 0, 1]),
                                              True) is False
    assert ops._mnnvl_workspace_all_succeeded(_FakeMpiComm([1, 1, 1]),
                                              False) is False
...
def mnnvl_capable_hardware(monkeypatch):
    ...
    monkeypatch.setattr(mnnvl_utils.MnnvlMemory, "supports_mnnvl",
...
def test_explicit_request_enables_mnnvl_on_a_single_node(
        mnnvl_capable_hardware):
    import torch

    mapping = _FakeMapping()
    mapping.multi_node = False
    assert ops.MNNVLAllReduce.is_mnnvl(mapping,
                                       torch.bfloat16,
...
def test_explicit_request_still_respects_hardware(mnnvl_capable_hardware,
                                                  monkeypatch):
    ...
    mapping = _FakeMapping()
    mapping.multi_node = True
    assert not ops.MNNVLAllReduce.is_mnnvl(
        mapping, torch.int8, explicitly_requested=True)
