...
def make_conv_data(B, H, K=128, V=128, M=2, W=4, lower_bound=-5.0,
                   scale=None, seed=2025):
    ...
    cs_q = torch.randn(B, S, dim, dtype=torch.float32,
                       device=device).transpose(1, 2)
    cs_k = torch.randn(B, S, dim, dtype=torch.float32,
                       device=device).transpose(1, 2)
    cs_v = torch.randn(B, S, dim, dtype=torch.float32,
                       device=device).transpose(1, 2)
    initial_state_kfirst = torch.randn(B, H, K, V, dtype=torch.float32,
                                       device=device)
    initial_state_cute = initial_state_kfirst.permute(0, 1, 3, 2).contiguous()
    cu_seqlens = torch.arange(0, B * T + 1, T, dtype=torch.int32,
                              device=device)
    ...
    return {
        "x_q": x_q, "x_k": x_k, "x_v": x_v,
        "w_q": w_q, "w_k": w_k, "w_v": w_v,
        "g": g, "beta": beta, "A_log": A_log, "dt_bias": dt_bias,
        "cs_q": cs_q, "cs_k": cs_k, "cs_v": cs_v,
        "initial_state_kfirst": initial_state_kfirst,
        "initial_state_cute": initial_state_cute,
        "qkg_cache": qkg_cache, "v_cache": v_cache, "beta_cache": beta_cache,
        "cu_seqlens": cu_seqlens, "ssm_state_indices": ssm_state_indices,
        "num_accepted_tokens": num_accepted_tokens,
        "B": B, "H": H, "K": K, "V": V, "M": M, "W": W,
        "T": T, "T_total": T_total,
...
def cpu_reference(data):
    ...
    B, H, K, V, M, W = (data["B"], data["H"], data["K"], data["V"],
                        data["M"], data["W"])
    T = data["T"]
        ...
        for h in range(H):
            ...
            for i_t in range(T_loop):
                if i_t < commit_len:
                    ...
                    hist_q = torch.cat([hist_q[:, 1:], xq_raw.unsqueeze(-1)],
                                       dim=1)
                    hist_k = torch.cat([hist_k[:, 1:], xk_raw.unsqueeze(-1)],
                                       dim=1)
                    hist_v = torch.cat([hist_v[:, 1:], xv_raw.unsqueeze(-1)],
                                       dim=1)
                else:
                    token = bos + i_t
                    ...
                    cq = (torch.cat([hist_q, xq_raw.unsqueeze(-1)], dim=-1) *
                          w_q[hk:hk + K]).sum(dim=-1)
                    ck = (torch.cat([hist_k, xk_raw.unsqueeze(-1)], dim=-1) *
                          w_k[hk:hk + K]).sum(dim=-1)
                    cv = (torch.cat([hist_v, xv_raw.unsqueeze(-1)], dim=-1) *
                          w_v[hv:hv + V]).sum(dim=-1)
                    q_t = F.normalize(cq / (1.0 + torch.exp(-cq)), p=2,
                                      dim=-1) * scale
                    k_t = F.normalize(ck / (1.0 + torch.exp(-ck)), p=2,
                                      dim=-1)
                    v_t = cv / (1.0 + torch.exp(-cv))
                    gr = g[0, token, h] + dt_bias[hk:hk + K]
                    gk_t = lower_bound * torch.sigmoid(gr *
                                                       torch.exp(A_log[h]))
...
def cute_run(data, zero_accepted_hint=False):
    ...
    for name in ("cs_q", "cs_k", "cs_v"):
        src = data[name]
        dst = torch.empty(src.shape[0], src.shape[2], src.shape[1],
                          dtype=src.dtype,
                          device=src.device).transpose(1, 2)
        dst.copy_(src)
        cs[name] = dst
...
def _fla_sequential_reference(data, num_accepted):
    ...
    for n in range(B):
        ...
        for i_t in range(a + 1 + M):
            ...
            if replay:
                # Replayed tokens use the cached post-processed q/k/g/v/beta
                # exactly as the kernel does (delta rule applied directly).
                q_t = data["qkg_cache"][n, i_t, 0].float().view(H, K)
...
def _assert_close(name, a, b, atol, rtol=0.0):
    ...
    assert ok, (f"{name}: max_abs={diff.max().item():.3e} "
...
def test_round1_zero_accepted(B, H):
    ...
    fla_out, fla_committed = _fla_sequential_reference(
        data, data["num_accepted_tokens"].cpu())

    new_rows = torch.cat(
        [torch.arange(n * T, n * T + 1 + M) for n in range(B)]).cuda()

    # Kernel vs the fp32 golden (tight: fp32 accumulation).
    _assert_close("out(cute vs cpu)", cute_out["out"][0, new_rows],
                  cpu["out"][0, new_rows], atol=2e-2)
    _assert_close("state(cute vs cpu)", cute_out["recurrent_state"],
                  cpu["recurrent_state"], atol=1e-4)
    for name in ("qkg_cache", "v_cache", "beta_cache", "cs_q", "cs_k",
                 "cs_v"):
        _assert_close(f"{name}(cute vs cpu)", cute_out[name], cpu[name],
                      atol=2e-2)

    # Kernel vs the FLA sequential path (the in-tree _forward_verify math).
    _assert_close("out(cute vs fla)", cute_out["out"][0, new_rows].float(),
                  fla_out[0, new_rows], atol=5e-2, rtol=5e-2)
    _assert_close("state(cute vs fla)", cute_out["state_v_first"],
                  fla_committed, atol=5e-3, rtol=5e-3)


@pytest.mark.parametrize("B,H", [(128, 2), (32, 12), (32, 6)],
                         ids=lambda v: str(v))
def test_round2_replay(B, H):
    ...
    data2["initial_state_cute"] = (
        r1["recurrent_state"].cuda().permute(0, 1, 3, 2).contiguous())
    ...
    rows = torch.cat([
        torch.arange(n * T + int(accept[n]),
                     n * T + int(accept[n]) + 1 + M) for n in range(B)
    ]).cuda()

    _assert_close("out2(cute vs cpu)", cute2["out"][0, rows],
                  cpu2["out"][0, rows], atol=2e-2)
    _assert_close("state2(cute vs cpu)", cute2["recurrent_state"],
                  cpu2["recurrent_state"], atol=1e-4)
    _assert_close("out2(cute vs fla)", cute2["out"][0, rows].float(),
                  fla_out2[0, rows], atol=5e-2, rtol=5e-2)
    _assert_close("state2(cute vs fla)", cute2["state_v_first"],
                  fla_committed2, atol=5e-3, rtol=5e-3)
...
def test_zero_accepted_hint_variant(B, H):
    ...
    for name in ("out", "recurrent_state", "qkg_cache", "v_cache",
                 "beta_cache", "cs_q", "cs_k", "cs_v"):
        _assert_close(f"{name}(fast vs general)", fast[name], general[name],
                      atol=1e-5)
