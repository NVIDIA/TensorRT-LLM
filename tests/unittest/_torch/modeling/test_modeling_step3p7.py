...
# checkpoint is a modelopt export: the decoder body lives under
# ``model.language_model.layers.*`` and vision keys under
# ``model.vision_model.*``. It also ships the 3 plain-path MTP layers (45..47),
# but under the bare ``model.layers.*`` namespace (the loader flattens the
# ``model.language_model.*`` body to ``model.layers.*`` so both land together).
...
class TestStep3p7Checkpoint(unittest.TestCase):
    ...
    MTP layers; the NVFP4 modelopt export stores the decoder body under
    ``model.language_model.layers.*`` and the 3 plain-path MTP layers (45..47)
    under the bare ``model.layers.*`` namespace. The checkpoints are expected
    under ``LLM_MODELS_ROOT`` (as in CI); a missing checkpoint surfaces as a test
    ...
    def test_config_and_weight_accounting(self, name, checkpoint_dir):
        ...
        - All consumed text-path keys can be enumerated; only vision, plain-path
          MTP (layers 45..47), and the vision projector are ignored. The NVFP4
          export nests the decoder body under ``model.language_model.*`` and
          vision keys under ``model.vision_model.*``, while its MTP layers use the
          bare ``model.layers.45..47`` namespace.
        """
        import re
        ...
        #    sliding elsewhere. The raw layer_types array is 48 entries long
        #    because it also covers the 3 MTP layers (45..47); all three
        #    checkpoints now ship those layers.
        ...
        #    nests the decoder body under ``model.language_model.*`` and the
        #    vision tower under ``model.vision_model.*``, while its 3 MTP layers
        #    use the bare ``model.layers.45..47`` namespace. FP8/BF16 use bare
        #    ``model.layers.*`` / ``vision_model.*`` prefixes for everything.
        if is_nvfp4:
            ...
            # MTP layers (45..47) live under the bare ``model.layers.*`` prefix.
            ...
            # Body and MTP share the ``model.layers.*`` namespace; the MTP layers
            # are the >= 45 indices.
            mtp_layer_re = text_layer_re
        ...
        # All three checkpoints now ship 3 plain-path MTP layers (45..47).
    ...
    def test_per_layer_attention_geometry_helpers(self, name, checkpoint_dir):
        ...
        differs. All three checkpoints carry 48 ``layer_types`` entries (the 3
        MTP layers 45..47 included), and layer 45 resolves to sliding-attention
        geometry.
        """
        from tensorrt_llm._torch.model_config import ModelConfig
        from tensorrt_llm._torch.models.modeling_step3p7 import (
