...
def test_mistral3_vlm_sanity(mistral_small_3_1_24b_config, quant_algo):
...
@torch.no_grad()
def test_mistral3_vlm_allclose_to_hf(mistral_small_3_1_24b_config, backend, use_cuda_graph):
    ...
    with kv_cache_manager_context(kv_cache_manager):
        ...
        with torch.inference_mode():
            attn_metadata.prepare()
            # This is where it fails for VANILLA and FLASHINFER.
            logits = mistral.forward(
                input_ids=input_ids, position_ids=position_ids, attn_metadata=attn_metadata
            )
