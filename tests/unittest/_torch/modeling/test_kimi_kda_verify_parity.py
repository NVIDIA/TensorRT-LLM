...
class _LayerCache:
    ...
    def __init__(self, slots, dim3, w, h, v, k, t_max, device):
        self.conv = torch.zeros(slots, dim3, w, dtype=torch.bfloat16,
                                device=device)
        self.temporal = torch.zeros(slots, h, v, k, dtype=torch.float32,
                                    device=device)
...
def test_kda_verify_matches_sequential_decode(batch, t_steps):
    ...
    cache = _LayerCache(slots, 3 * dim, w, h, lin["head_dim"],
                        lin["head_dim"], t_steps, device)
    slot_indices = torch.arange(2, 2 + batch, device=device,
                                dtype=torch.long)
    ...
    x = torch.randn(batch, t_steps, cfg.hidden_size, dtype=torch.bfloat16,
                    device=device) * 0.1
    ...
    for t in range(t_steps):
        out = runtime._forward_decode(x[:, t], ref_conv, ref_ssm,
                                      slot_indices)
        ref_outs.append(out)
    ...
    out_verify = runtime._forward_verify(
        x.reshape(batch * t_steps, cfg.hidden_size), t_steps, cache,
        cache.conv, cache.temporal, slot_indices)
    ...
    for t in range(t_steps):
        torch.testing.assert_close(out_verify[:, t], ref_outs[t],
                                   rtol=2e-2, atol=2e-2)
        torch.testing.assert_close(
            cache.intermediate_conv_window[:batch, t],
            ref_conv_steps[t], rtol=2e-2, atol=2e-2)
        torch.testing.assert_close(
            cache.intermediate_ssm[:batch, t],
            ref_ssm_steps[t], rtol=2e-2, atol=2e-2)
