...
class TestLoraMLPPivotVsTRT(unittest.TestCase):
    ...
    def _create_lora_params(self):
        ...
        lora_weight_ins_down = [
            tmp.transpose(1, 0).contiguous() for tmp in lora_weight_ins_down
        ]
        lora_weight_outs_down = [
        ...
        lora_params_pivot = {
            ...
            0: {
                LoraModuleType.MLP_H_TO_4H: {
                    ...
                    'weight_pointers':
                    torch.tensor([[
                        lora_params['lora_weight_outs_up'][0].data_ptr(),
                        lora_params['lora_weight_ins_up'][0].data_ptr()
                    ]]),
                    'is_dora':
                    False,
                    'weight_tensors': [
                        lora_params['lora_weight_outs_up'][0],
                        lora_params['lora_weight_ins_up'][0]
                    ]
                },
                LoraModuleType.MLP_4H_TO_H: {
                    ...
                    'weight_pointers':
                    torch.tensor([[
                        lora_params['lora_weight_outs_down'][0].data_ptr(),
                        lora_params['lora_weight_ins_down'][0].data_ptr()
                    ]]),
                    'is_dora':
                    False,
                    'weight_tensors': [
                        lora_params['lora_weight_outs_down'][0],
                        lora_params['lora_weight_ins_down'][0]
                    ]
                }
            }
        }
