...
def _wrapper_after_provider_flip(comm: Optional[Communication]) -> ConfigurableMoE:
    ...
    nothing else of the wrapper is needed to exercise the contract.
    """
...
@pytest.mark.parametrize(
    "comm, backend_provider",
    [
        (None, True),
        (AllGatherReduceScatter.__new__(AllGatherReduceScatter), True),
        (NVLinkTwoSided.__new__(NVLinkTwoSided), False),
    ],
    ids=[
        "flip_without_comm",
...
def test_configurable_moe_resync_op_provider_updates_mirror(
    comm: Optional[Communication], backend_provider: bool
) -> None:
    """Every other combination just brings the mirror in line with the backend."""
    moe = _wrapper_after_provider_flip(comm)
