...
def _make_kimi_k3_moe_weights(config):
    ...
    return {
        ...
        "routed_norm": (
            1.0 + randn(config.routed_expert_hidden_size, scale=0.02).to(torch.float32)
        ).to(torch.bfloat16),
...
def _kimi_k3_moe_reference(hidden_states, weights, config):
    ...
    for expert_id in range(config.num_experts):
        ...
        expert_output = F.linear(situ(gate_up), weights["experts"][f"{expert_id}.w2.weight"])
...
def _make_runtime_aux_stream_dict():
    """Real CUDA streams for a live ``KimiK3MoERuntime``.

    ``_make_aux_stream_dict`` above hands out plain ``object()`` sentinels,
    which is enough for the sharding assertions but not for a forward pass.
    The key set mirrors ``KimiLinearModel.aux_stream_dict``.
    """
    ...
    try:
        ...
        with torch.device(f"cuda:{rank}"):
            runtime = modeling_kimi_linear.KimiK3MoERuntime(
                ...
                config,
                layer_idx=0,
                aux_stream_dict=_make_runtime_aux_stream_dict(),
            )
            weights = _make_kimi_k3_moe_weights(config)
            _load_kimi_k3_moe_weights(runtime, weights)

            input_generator = torch.Generator().manual_seed(9000 + rank if attention_dp else 9000)
            hidden_states = (
                torch.randn(
                    16,
                    config.hidden_size,
