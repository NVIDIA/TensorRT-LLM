...
def _worker_nccl_ep_internal_fp8_dispatch(config: CommTestConfig) -> dict:
    ...
    expected = _make_nccl_ep_internal_fp8_hidden_states(sender_rank, config).float()
    # LL rank-major groups received rows by source rank but does not promise
    # the source token order within each group.  The first block scale is
    # unique for every test token, so compare the dequantized payload after
    # ordering both sides by that scale.
    received_scales = recv_sf[sender_rank, :num_tokens, :, 0]
    received_rows = dequantized[sender_rank, :num_tokens].index_select(
        0, torch.argsort(received_scales[:, 0])
    )
    expected_first_block_scales = (
        512.0
        + sender_rank * 1536.0
        + torch.arange(num_tokens, dtype=torch.float32, device="cuda") * 128.0
    ) / 448.0
    expected_rows = expected.index_select(0, torch.argsort(expected_first_block_scales))
    payload_error = (received_rows - expected_rows).abs()
    valid_rows = (
    ...
    return {
        ...
        "payload_matches": bool(torch.allclose(received_rows, expected_rows, rtol=1e-3, atol=1e-3)),
