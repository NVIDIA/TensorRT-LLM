...
def test_base_mapper_normalizes_moe_sources_without_inventing_preload_membership(
    ...
    assert plan.demands[0].group_id.startswith("module:")
...
def test_base_mapper_uses_opaque_order_for_serial_loading_override(monkeypatch) -> None:
    monkeypatch.setenv("TRT_LLM_DISABLE_LOAD_WEIGHTS_IN_PARALLEL", "true")
    catalog = _catalog_with_names("layer.q_proj.weight", "layer.special.weight")
