...
for export compatibility. Mistral uses GQA with SwiGLU MLP and RMSNorm.
"""
...
@torch.no_grad()
def test_mistral3_rmsnorm_equivalence(B, S, dtype):
    ...
    custom_norm = Mistral3RMSNorm(config.hidden_size, eps=config.rms_norm_eps)
...
def test_mistral3_attention_equivalence(B, S, dtype):
    ...
    custom_attn = Mistral3Attention(config, layer_idx=0)
    ...
    custom_rotary = Mistral3RotaryEmbedding(config)
...
def test_mistral3_decoder_layer_equivalence(B, S, dtype):
    ...
    custom_layer = Mistral3DecoderLayer(config, layer_idx=0)
...
@torch.no_grad()
def test_mistral3_full_model_equivalence(B, S, dtype, device):
    ...
    custom_model = Mistral3TextForCausalLM(config)
...
@torch.no_grad()
def test_mistral3_model_can_be_exported():
    """Test that the custom model can be exported with torch_export_to_gm."""
    device = "cuda"
    dtype = torch.bfloat16
    config = _create_small_config()

    model = Mistral3TextForCausalLM(config)
...
def test_mistral3_config_registration():
...
def test_mistral3_state_dict_keys():
