...
class ComplexEagerAttentionModel(torch.nn.Module):
    """A model that implements a complex eager attention pattern.

    Similar to the one in the user's graph, this includes additional to_dtype
    operations and different transpose patterns.
    """
...
class CounterExampleModel(torch.nn.Module):
    """A model with similar operations that shouldn't match the repeat_kv pattern.

    Uses unsqueeze -> expand -> reshape with different dimensions than repeat_kv.
    """
