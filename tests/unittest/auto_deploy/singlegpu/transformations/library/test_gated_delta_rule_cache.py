...
class GatedDeltaRuleModel(nn.Module):
    ...
    def forward(
        ...
        q = self.q_proj(x).view(bsz, s, self.num_k_heads, self.key_dim)
        k = self.k_proj(x).view(bsz, s, self.num_k_heads, self.key_dim)
        v = self.v_proj(x).view(bsz, s, self.num_v_heads, self.value_dim)
        a = self.a_proj(x)  # [B, S, HV]
        b = self.b_proj(x)  # [B, S, HV]
...
def test_gated_delta_rule_with_cache(num_k_heads, num_v_heads):
