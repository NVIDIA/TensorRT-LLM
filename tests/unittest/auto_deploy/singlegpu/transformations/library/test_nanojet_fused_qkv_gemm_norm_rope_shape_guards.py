...
HIDDEN, HEAD_DIM, NUM_HEADS, NUM_KV_HEADS, EPS = 256, 64, 8, 4, 1e-6


def _build(kv_rows_match: bool = True, table_width: int = HEAD_DIM):
    ...
    def projection(weight_attr):
        return graph.call_function(
            torch.ops.auto_deploy.trtllm_quant_fp8_linear.default,
            args=(hidden, weight_attr, None, attrs["input_scale"], attrs["weight_scale"]),
        )
    ...
    key = normed(projection(attrs["key_weight"]), attrs["key_norm"])
...
def test_harness_reaches_the_shape_guards():
    ...
    result = _reason()
    assert result not in ("kv-row-mismatch", "rope-table-width"), result
    assert not isinstance(result, str) or "norm-over-projection" not in result, result
