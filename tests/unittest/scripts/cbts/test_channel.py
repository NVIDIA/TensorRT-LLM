...
def test_flush_runs_before_the_ack_for_a_context_switch(server):
    """A superseded test's coverage must be durable before its ACK tells the producer so."""
    calls = []
    flushed = threading.Event()

    def on_context(nodeid):
        calls.append(("context", nodeid))
    ...
    subscriber = ContextSubscriber.subscribe(
        server.address, on_context=on_context, on_stop=lambda: None, on_flush=on_flush
    )
    assert subscriber is not None
    _wait_for_subscribers(server, 1)
    assert flushed.wait(5.0)  # the initial context delivered during subscribe()
    calls.clear()
    flushed.clear()
    try:
        assert server.announce("suite.py::test_flush", ack_timeout=5.0) is True
        assert flushed.wait(5.0)
        assert calls == [("context", "suite.py::test_flush"), ("flush",)]
    finally:
        subscriber.close()
...
def test_a_crash_mid_test_is_tainted(server):
    """Unlike a clean unsubscribe, an EOF with no LEAVE before any STOP is a real loss.

    A subscriber that leaves through ``ContextSubscriber.close()`` sends ``LEAVE`` first,
    which is how ``test_a_clean_session_records_no_taint`` gets away with no taint. A raw
    socket standing in for a crashed process never sends it, and never holds a ticket
    either, so its death can't be confirmed: the loss must surface with the same doubt as
    a wedged-but-possibly-alive consumer.
    """
    victim = socket.socket(socket.AF_UNIX, socket.SOCK_STREAM)
    victim.connect(server.address)
    victim.sendall(b"I victim-proc\n")
    _wait_for_subscribers(server, 1)
    server.announce("suite.py::test_crash", ack_timeout=0)
    time.sleep(0.1)
    victim.close()
    _wait_for_subscribers(server, 0)
    assert server.taints == [
        ("victim-proc", "suite.py::test_crash", TAINT_ATTRIBUTION, TAINT_UNACKNOWLEDGED),
        ("victim-proc", "suite.py::test_crash", TAINT_INCOMPLETE, TAINT_STOPPED_RECORDING),
    ]


def test_a_crash_does_not_taint_tests_announced_afterward(server):
    """A subscriber confirmed dead by its ticket can't miss or misattribute anything.

    Unlike a wedged consumer that might still be alive (`test_losing_a_subscriber_taints_
    every_later_test_too`) or a crashed one whose death can't be confirmed
    (`test_a_long_lived_subscribers_crash_taints_every_later_test_too`), this subscriber's
    ticket lock is released before its connection drops -- exactly what the kernel does on
    a real process exit -- so the producer can tell for a fact that it cannot possibly
    record anything for a test it was never even told about: only the one it crashed
    during is in doubt, not every test announced afterward.
    """
    recorder = Recorder()
    subscriber = ContextSubscriber.subscribe(
        server.address,
        ...
        identity="victim-proc",
    )
    assert subscriber is not None
    _wait_for_subscribers(server, 1)
    assert server.announce("suite.py::test_crash", ack_timeout=5.0) is True
    # Simulate the kernel releasing the ticket on process exit, then the connection
    # dropping -- exactly what a real crash looks like from the producer's side. shutdown()
    # before close() so the server sees a clean EOF regardless of the reader thread's own
    # in-flight recv().
    subscriber._ticket_fh.close()
    subscriber._conn.shutdown(socket.SHUT_RDWR)
    subscriber._conn.close()
    _wait_for_subscribers(server, 0)
    server.announce("suite.py::test_after", ack_timeout=0)
    assert server.taints == [
        ("victim-proc", "suite.py::test_crash", TAINT_INCOMPLETE, TAINT_SUBSCRIBER_DEAD),
    ]


def test_a_long_lived_subscribers_crash_taints_every_later_test_too(server):
    """An unconfirmed death is treated with the same suspicion as a wedged-but-alive one.

    A subscriber that disconnects without its ticket having been released first cannot be
    told apart from one that is still alive and running, so losing it for good must be
    treated with the same suspicion as `test_losing_a_subscriber_taints_every_later_test_
    too`'s wedged (but still possibly alive) consumer: it might still be misattributing
    every later test's work to the stale context it's stuck on. A real `ContextSubscriber`
    is used here (not a raw socket) so it actually acks the announcement instead of forcing
    `announce` to wait out its ack timeout, which would eject it through a different path
    ...
    _wait_for_subscribers(server, 1)
    assert server.announce("suite.py::test_crash", ack_timeout=5.0) is True
    # A crash: unlike the previous test, the ticket is never released. shutdown() before
    # close() so the server sees a clean EOF regardless of the reader thread's own
    # in-flight recv().
    subscriber._conn.shutdown(socket.SHUT_RDWR)
    subscriber._conn.close()
    _wait_for_subscribers(server, 0)
    server.announce("suite.py::test_after", ack_timeout=0)
    assert server.taints == [
        ("victim-proc", "suite.py::test_after", TAINT_INCOMPLETE, TAINT_STOPPED_RECORDING),
        ("victim-proc", "suite.py::test_crash", TAINT_ATTRIBUTION, TAINT_UNACKNOWLEDGED),
        ("victim-proc", "suite.py::test_crash", TAINT_INCOMPLETE, TAINT_STOPPED_RECORDING),
    ]
