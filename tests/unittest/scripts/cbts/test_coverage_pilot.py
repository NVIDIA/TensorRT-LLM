...
def pilot_module() -> ModuleType:
    return importlib.import_module("cbts.command.coverage.pilot")
...
def test_main_reads_bot_trigger_payload(
    ...
    # `main` is now a click.Command; call the undecorated callback directly so this
    # stays a plain function call (click's own arg-parsing/exit machinery is not
    # under test here) — equivalent to invoking with no --pr-api-url.
    pilot_module.main.callback(pr_api_url=None)
