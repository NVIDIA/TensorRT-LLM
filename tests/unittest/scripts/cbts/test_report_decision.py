...
# The module under test imports these by name, so the fakes below have to be
# registered under the same names to intercept them.
_BLOCKS = "cbts.blocks"
_OPEN_SEARCH_DB = "open_search_db"


@pytest.fixture()
def report_module():
    """The module under test, imported as part of the cbts package.

    Loading it by file path would run its standalone bootstrap, which puts the
    package's parent directory on sys.path for the rest of the session.
    """
    return importlib.import_module("cbts.command.report_decision")
...
def fake_blocks(monkeypatch):
    ...
    fake_module = types.ModuleType(_BLOCKS)
    ...
    monkeypatch.setitem(sys.modules, _BLOCKS, fake_module)
    return stages
...
def test_build_document_filters_unscheduled_stages_and_persists_valid_rate(
    report_module, cbts_applied, coverage_pilot_eligible
):
    decision = _decision()
...
def test_main_posts_case_skip_rate_to_opensearch(
    report_module, monkeypatch, tmp_path, cbts_applied, coverage_pilot_eligible
):
    posted_documents = []
    ...
    fake_module = types.ModuleType(_OPEN_SEARCH_DB)
    fake_module.CBTS_PROJECT_NAME = "cbts-test-project"
    fake_module.OpenSearchDB = FakeOpenSearchDB
    monkeypatch.setitem(sys.modules, _OPEN_SEARCH_DB, fake_module)
    ...
    # `main` is now a click.Command; call the undecorated callback directly, which
    # still runs the never-fail try/except wrapping `_run` — equivalent to invoking
    # with the given flags.
    report_module.main.callback(
        status="pre_merge",
        reason="",
        ...
        repo_root=".",
        multi_gpu_required=True,
        multi_gpu_label_gate_open=True,
        cbts_applied=cbts_applied,
        coverage_pilot_eligible=coverage_pilot_eligible,
    )
    ...
    assert posted_documents[0]["b_cbts_applied"] is cbts_applied
    assert posted_documents[0]["b_coverage_pilot_eligible"] is coverage_pilot_eligible
