...
from cbts.command import main as cbts_main
...
class CoverageArtifactTest(unittest.TestCase):
    def test_prepare_rejects_build_and_print_selection(self) -> None:
        from click.testing import CliRunner

        runner = CliRunner()
        for extra in (["--build", "1"], ["--print-selection"]):
            result = runner.invoke(artifact.main, ["--prepare", "out", *extra])
            self.assertNotEqual(result.exit_code, 0, result.output)
            self.assertIn("--prepare cannot be combined", result.output)
