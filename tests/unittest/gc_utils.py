...
def assert_resource_freed(object_creation_func, *args, **kwargs):
    ...
    try:
        ...
        # Background threads are only released by shutdown(); dropping the last
        # reference does not run it in time. Looked up on the type so no bound
        # method keeps the instance alive.
