...
def _make_config(ctx_url, gen_url, public_port):
    ...
    return DisaggServerConfig(
        ...
        ctx_router_config=RouterConfig(type="round_robin", server_role=ServerRole.CONTEXT),
        gen_router_config=RouterConfig(type="conversation", server_role=ServerRole.GENERATION),
    )
...
class _CoordinatorThread:
    ...
    def __init__(self, config):
        ...
        self._coordinator = DisaggCoordinatorService(
            config, client_factory=lambda router, role, mr=1: _ReadinessClient(router)
        )
        self._impl = _UvicornThread(CoordinatorServer(self._coordinator).app, self.port)
...
def test_disagg_completion_e2e_through_coordinator():
    with (
        ...
        with _CoordinatorThread(config) as coord:
            assert asyncio.run(_wait_healthy(coord.url)), "coordinator never became healthy"

            disagg = OpenAIDisaggServer(config=config, coordinator_url=coord.url)
            with _UvicornThread(disagg.app, public_port) as server:
                base = f"http://127.0.0.1:{server.port}"
                assert asyncio.run(_wait_healthy(base)), "disagg server never became healthy"

                async def drive():
                    async with aiohttp.ClientSession() as sess:
                        payload = {"model": "m", "prompt": "hello", "max_tokens": 8}
...
def test_disagg_completion_e2e_web_concurrency_4():
    ...
    with (
        ...
        with tempfile.TemporaryDirectory() as td:
            ...
            for k in list(env):
                if k.startswith(
                    ("SLURM_", "PMIX_", "PMI_", "OMPI_", "UCX_", "I_MPI_", "HYDRA_", "MPI_")
                ):
                    env.pop(k)
            ...
            try:
                ...
                async def _wait_all():
                    ...
                    assert await _wait_healthy(coord, 120.0), "coordinator never became healthy"
                    logger.info("coordinator healthy; waiting for fleet health...")
                    assert await _wait_healthy(base, 120.0), "disagg fleet never became healthy"
