...
def test_fanin_bounce_safe_gate() -> None:
    ...
    # Replicated shares are supported for pure-TP fan-in, while PP fan-in
    # must still fall back; single-writer overlap stays safe,
    ...
    assert safe(ov(1, 1, ranks=(0, 1)), ri([24], pt(MapperKind.REPLICATED)), None) is True
    assert safe(ov(1, 1, ranks=(0,)), ri([24], pt(MapperKind.REPLICATED)), None) is True
    assert (
        safe(
            ov(1, 2, ranks=(0, 1)),
            ri([24, 24], pt(MapperKind.NHD)),
...
class TestFanInReserve:
    def test_block_bytes_include_distinct_physical_pools_once(self) -> None:
        ...
        assert btr.block_bytes_per_group(page_table) == ([125], [0])
    ...
    @pytest.mark.parametrize("limit,accepted", [(99, False), (100, True), (None, True)])
    def test_sender_checks_receiver_capacity_before_gather(
        self, monkeypatch: pytest.MonkeyPatch, limit: int | None, accepted: bool
    ) -> None:
        t = _make_transport(monkeypatch, block_bytes_per_group=[100])
        gathered = []
        monkeypatch.setattr(t, "_gather_blocking", lambda *args: gathered.append(args))
        write_meta = SimpleNamespace(sizes=np.array([60, 40]), bounce_dst_bytes=limit)

        result = t._reserve_and_gather(write_meta, timeout=0)

        assert (result is not None) is accepted
        assert t._send_alloc.reserved_sizes == ([100] if accepted else [])
        assert len(gathered) == int(accepted)
...
class TestHybridK3Bounce:
    def test_block_bytes_per_group_keeps_mamba_placeholder(self):
        ...
        assert btr.block_bytes_per_group(_k3_page_table()) == ([_K3_MLA_BLOCK_BYTES, None], [0, 0])
    ...
    def test_reserve_engages_on_k3_mixed_layout(self, monkeypatch):
        ...
        t = _make_transport(
            monkeypatch,
            block_bytes_per_group=btr.block_bytes_per_group(_k3_page_table())[0],
            min_bytes=2 * _MIB,
        )
