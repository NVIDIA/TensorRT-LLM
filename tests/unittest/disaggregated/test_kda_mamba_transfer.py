...
def _create_kda_managers(tp: int, enable_attention_dp: bool = False,
...
def test_kda_layer_group_descriptors(enable_attention_dp):
    ...
    try:
        ...
        assert sorted(mlg.mamba_layer_offsets.keys()) == [
...
def _generate_ground_truth(num_requests: int, seed: int = 20260722):
    ...
    for _ in range(num_requests):
        ...
        for i, is_kda in enumerate(_KDA_MASK):
            ...
            layers[i] = {
                "conv": torch.rand(
                    3 * KDA_NUM_HEADS * KDA_HEAD_DIM, KDA_W, generator=gen
                ).to(CONV_DTYPE),
                "ssm": torch.rand(
                    KDA_NUM_HEADS, KDA_HEAD_DIM, KDA_HEAD_DIM, generator=gen,
                    dtype=SSM_DTYPE,
                ),
            }
