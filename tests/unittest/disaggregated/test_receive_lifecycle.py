...
class TestAdmissionAndPublication:
    ...
    def test_authenticated_no_access_proof_settles_published_bounce_writer(self) -> None:
        registry = RecvTransferRegistry()
        _prepare(registry)
        _publish(registry, 3)

        terminal = registry.record_result(
            KEY,
            3,
            WriterResult.FAILED,
        ...
        assert terminal.accepted and not terminal.conflict
        assert terminal.logical_state is LogicalState.FAILED
        assert terminal.writers_terminal
        assert LifecycleAction.RELEASE_BOUNCE in terminal.actions
        writers = {writer.rank: writer for writer in registry.context_snapshot(KEY).writers}
        assert writers[3].exposure is ExposureState.PUBLISHED
        assert writers[3].mode is WriterMode.NO_REMOTE_ACCESS
        assert _settle_bounce(registry).physical_state is PhysicalState.DRAINED
