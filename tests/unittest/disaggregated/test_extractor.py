...
def test_v2_mamba_layer_group_extracts_sparse_recurrent_side_state():
    ...
    side_view = layer_group.pool_views[side_view_idx]
    side_pool = pool_group.pools[side_view.pool_idx]
    assert side_view.mapper_kind == MapperKind.REPLICATED
    assert side_view.buffer_entries.tolist() == [(1, 0, 8 * side_storage.element_size())]
