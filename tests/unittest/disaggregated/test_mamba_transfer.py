...
def _make_mamba_page_table(*, base_address: int, side_bytes: int = 8):
    ...
    layer_group = page.MambaLayerGroup(
        pool_group_idx=0,
        local_layers=local_layers,
        pool_views=pool_views,
...
def test_mamba_policy_transfers_replicated_recurrent_side_state():
    ...
    self_group = self_page_table.layer_groups[0]
    peer_group = peer_page_table.layer_groups[0]
    ...
    mapped = mapper.map(src_region, dst_region)

    assert mapped.src.memory.ptrs.tolist() == [1000 + 512 + 32]
    assert mapped.dst.memory.ptrs.tolist() == [10000 + 512 + 2 * 32]
