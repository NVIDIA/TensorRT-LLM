...
def test_mamba_receiver_payload_bytes_matched_tp():
    ...
    pool_views = [
        ...
        PoolView(
            pool_idx=2,
            buffer_entries=np.array([(1, 0, side_slot_bytes)], dtype=BUFFER_ENTRY_DTYPE),
    ...
    # Standard state covers both layers; the side role exists only on layer 2.
    assert got == 2 * (conv_slot_bytes + ssm_slot_bytes) + side_slot_bytes


def _make_rank_info(*, tp_size: int, tp_rank: int = 0) -> RankInfo:
    return RankInfo(
        instance_name="test",
        instance_rank=tp_rank,
        tp_size=tp_size,
        tp_rank=tp_rank,
        pp_size=1,
        pp_rank=0,
...
def _make_side_state_page_table(*, side_bytes: int = 8, include_side: bool = True):
    from tensorrt_llm._torch.disaggregation.resource.page import (
        BUFFER_ENTRY_DTYPE,
        MAMBA_CONV_ROLE,
        MAMBA_SSM_ROLE,
    ...
    pools = [
        PhysicalPool(base_address=1000, slot_bytes=16, num_slots=4),
        PhysicalPool(base_address=2000, slot_bytes=16, num_slots=4),
    ]
    views = [
        PoolView(
            pool_idx=0,
            buffer_entries=np.array([(0, 0, 16), (1, 16, 16)], dtype=BUFFER_ENTRY_DTYPE),
            pool_role=MAMBA_CONV_ROLE,
            mapper_kind=MapperKind.SECTIONED,
            bytes_per_layer=16,
        ),
    ...
    if include_side:
        pools.append(PhysicalPool(base_address=3000, slot_bytes=side_bytes, num_slots=4))
        views.append(
            PoolView(
                pool_idx=2,
                buffer_entries=np.array([(1, 0, side_bytes)], dtype=BUFFER_ENTRY_DTYPE),
                pool_role=frozenset({"ple_ngram_context"}),
                mapper_kind=MapperKind.REPLICATED,
...
def test_mamba_policy_maps_replicated_state_without_tp_sharding():
    mapper = peer.MambaPolicy(_make_rank_info(tp_size=2)).build_mapper(
        peer_ri=_make_rank_info(tp_size=1),
        ...
        peer_bytes_per_layer=8,
    )
    mapped = mapper.map(
        SpecRegion(memory=MemRegionGroup(ptrs=np.array([1000]), bytes_per_region=8)),
        SpecRegion(memory=MemRegionGroup(ptrs=np.array([2000]), bytes_per_region=8)),
    )

    assert mapped.src.memory.ptrs.tolist() == [1000]
    assert mapped.dst.memory.ptrs.tolist() == [2000]
    assert mapped.src.memory.bytes_per_region == 8
    assert mapped.dst.memory.bytes_per_region == 8

    with pytest.raises(ValueError, match="Replicated Mamba state size differs"):
        peer.MambaPolicy(_make_rank_info(tp_size=2)).build_mapper(
            peer_ri=_make_rank_info(tp_size=1),
            mapper_kind=peer.MapperKind.REPLICATED,
            self_layer_offsets=np.array([0], dtype=np.int64),
            peer_layer_offsets=np.array([0], dtype=np.int64),
...
def test_mamba_policy_rejects_incompatible_replicated_side_state():
    ...
    self_page_table = _make_side_state_page_table()

    with pytest.raises(ValueError, match="bytes per layer differs"):
        peer.MambaPolicy.validate_peer_compatible(
            rank_info,
            rank_info,
            self_page_table,
            _make_side_state_page_table(side_bytes=16),
        )

    with pytest.raises(ValueError, match="differs for overlapping layers"):
        peer.MambaPolicy.validate_peer_compatible(
            rank_info,
            rank_info,
            self_page_table,
