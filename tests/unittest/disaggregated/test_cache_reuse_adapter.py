...
class TestAdapterPerLayerGroup:
    ...
    def test_swa_passthrough_above_stale(self):
        # SWA layer: scalar already exceeds stale_end*tpb, so no clamp.
    ...
    def test_swa_scalar_below_stale_clamped_up(self):
        # scalar=8 is below stale_end*tpb=16; adapter clamps up to stale_end.
...
class TestSwaTrim:
    ...
    def test_window_offset_skip_subtracts_stale(self):
        ...
        out = _swa_trim(
            [10, 11, 12], prompt_len=32, tpb=8, window_size=24, scalar_cached_tokens=16
        )
    ...
    def test_gen_side_reuse_inside_stale_no_skip(self):
        # Adapter clamps the reuse hit up to stale_end*tpb; cache_skip remains 0.
