...
def test_recv_req_info_round_trips_beam_tails():
    info = RecvReqInfo(
        sender_req_id=1,
        instance_name="decode",
        instance_rank=0,
        block_ids_per_layer_groups=[_positional(3, o1=[5, 6])],
        unique_rid=7,
        beam_tails_per_layer_groups=[np.array([20, 21], dtype=np.int64)],
    )
    back = RecvReqInfo.from_bytes(info.to_bytes())
    assert np.array_equal(back.block_ids_per_layer_groups[0], [-1, 5, 6])
    assert np.array_equal(back.beam_tails_per_layer_groups[0], [20, 21])
    plain = RecvReqInfo.from_bytes(_make_projection_req_info().to_bytes())
    assert plain.beam_tails_per_layer_groups is None
...
def _build_prefill_chunk_tokens_for(
    ...
    The mocked ``_create_kv_slice`` hands back a positional table over the whole
...
def test_build_prefill_chunk_rounds_unaligned_non_final_end_down():
    ...
    assert np.array_equal(kv_slice.block_ids_per_layer_groups[0], _positional(8, o0=[0]))
...
def test_window_covering_the_whole_prompt_streams_like_full_attention(window_tokens):
    ...
    assert kv_slice.is_last_slice is False
    assert np.array_equal(kv_slice.block_ids_per_layer_groups[0], _positional(8, o0=np.arange(4)))
...
def test_unaligned_reuse_prefix_still_extends_first_chunk_to_block_zero():
    ...
    assert np.array_equal(kv_slice.block_ids_per_layer_groups[0], _positional(8, o0=np.arange(3)))
...
def test_first_chunk_covers_ctx_prefix_reuse():
    ...
    assert np.array_equal(kv_slice.block_ids_per_layer_groups[0], _positional(8, o0=np.arange(6)))
    assert kv_slice.is_last_slice is False
