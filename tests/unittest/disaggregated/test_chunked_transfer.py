...
def _build_prefill_chunk_for(
    ...
    req.prepopulated_prompt_len = prepopulated_blocks * _REUSE_TPB
    req.py_last_context_chunk = (
        chunk_start_block * _REUSE_TPB,
        chunk_end_block * _REUSE_TPB,
    )
...
def test_first_chunk_covers_ctx_prefix_reuse():
    ...
    assert kv_slice.token_range == TokenRange(start=0, end=6 * _REUSE_TPB)
...
def test_only_the_first_chunk_extends_to_block_zero(
    ...
    assert kv_slice.token_range == TokenRange(
        start=expected_start_block * _REUSE_TPB, end=chunk_end_block * _REUSE_TPB
    )
    assert np.array_equal(
...
def test_single_chunk_with_reuse_degenerates_to_monolithic_slice():
    ...
    token_range.start == 0 with is_last_slice makes _build_kv_write_meta take its
    non-chunked branch, so the write is addressed exactly as an unpipelined one.
    """
    kv_slice = _build_prefill_chunk_for(
        prepopulated_blocks=3,
        chunk_start_block=3,
    ...
    assert kv_slice.token_range == TokenRange(start=0, end=_REUSE_TOTAL_BLOCKS * _REUSE_TPB)
    assert np.array_equal(
        kv_slice.block_ids_per_layer_groups[0],
