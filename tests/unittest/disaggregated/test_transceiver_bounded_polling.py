...
from tensorrt_llm._torch.disaggregation.handoff import HandoffEventState
...
def test_gen_transfer_idle_fast_path_still_retries_terminal_receipts() -> None:
    transceiver = _make_transceiver({})
    transceiver._ever_had_recv_session = False
    ...
    assert transceiver.check_gen_transfer_status(at_least_request_num=0) == ([], [], [])
    assert transceiver._transfer_worker.receipt_retry_count == 1
...
def test_v1_sync_receive_waits_for_peer_cleanup_before_handoff_commit(
    ...
    params = SimpleNamespace(
        ...
        logical_request_id=rid,
        prefill_artifact_id=str(uuid4()),
        artifact_version=1,
        handoff_attempt_uuid=str(uuid4()),
        consumer_grant_id=str(uuid4()),
        transfer_session_id=str(uuid4()),
        schedule_style=object(),
    )
    request = _RetirementRequest(rid)
    request.py_disaggregated_params = params
    request.py_disagg_transfer_protocol_identity = transfer_protocol_identity_from_params(params)
    request.py_client_id = 7
    ...
    session.kv_cache_size_bytes = 64
    ...
    assert session.closed
    assert set(transceiver._get_generation_retirements()) == {rid}
    assert transceiver.take_handoff_lifecycle_events() == []
...
def test_v1_sync_receive_rejects_missing_exact_identity_before_admission() -> None:
    rid = 64
    request = _RetirementRequest(rid)
    request.py_disagg_transfer_protocol_identity = None
    transceiver = object.__new__(KvCacheTransceiverV2)
    transceiver._lifecycle_contract = TransceiverCapabilities(
        protocol_version=int(ProtocolVersion.GENERATION_SAFE),
    )
    ...
    with pytest.raises(RuntimeError, match="requires exact transfer identity"):
        transceiver.request_and_receive_sync(request)
