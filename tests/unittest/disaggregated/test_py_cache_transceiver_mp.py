...
CHUNKED_MP_TEST_CONFIGS = [
    (1, 1, 1, 1, False, False, False, "v2_mp_tp1_pp1_chunked"),
    (1, 2, 1, 1, False, False, False, "v2_mp_tp1_pp2_to_tp1_pp1_chunked"),
]
...
@pytest.mark.parametrize(
    "ctx_tp,ctx_pp,gen_tp,gen_pp,ctx_enable_dp,gen_enable_dp,is_mla",
    [(c[0], c[1], c[2], c[3], c[4], c[5], c[6]) for c in CHUNKED_MP_TEST_CONFIGS],
    ids=[c[7] for c in CHUNKED_MP_TEST_CONFIGS],
)
@pytest.mark.parametrize("workflow", ["ctx_first"])
def test_v2_transceiver_mp_chunked(
    ctx_tp, ctx_pp, gen_tp, gen_pp, ctx_enable_dp, gen_enable_dp, is_mla, workflow
):
    """Test PyNativeCacheTransceiver with sender-side chunking enabled."""
    try:
        mp.set_start_method("spawn", force=True)
    except RuntimeError:
        pass

    # Uses chunk_size_blocks to enable sender-side chunking.
    # The underlying run_v2_transceiver_mp currently doesn't plumb
    # chunk_size_blocks through CacheTransceiverConfig to workers,
    # so this test verifies that the config field is accepted and
    # the transceiver initializes correctly.  Full e2e data
    # verification with chunking is covered in test_kv_transfer.py
    # (test_transfer_worker_v2_chunked).
    run_v2_transceiver_mp(
        ctx_tp=ctx_tp,
        ctx_pp=ctx_pp,
        gen_tp=gen_tp,
