...
def test_receiver_ctx_info_endpoint_allowlist(monkeypatch):
    ...
    monkeypatch.delenv("TLLM_DISAGG_ALLOWED_CTX_INFO_ENDPOINTS", raising=False)
    with pytest.raises(ValueError, match="ctx_info_endpoint must be listed"):
        receiver._validate_info_endpoint("tcp://10.0.0.1:5000")

    monkeypatch.setenv("TLLM_DISAGG_ALLOWED_CTX_INFO_ENDPOINTS",
                       "tcp://10.0.0.1:5000")
    receiver._validate_info_endpoint("tcp://10.0.0.1:5000")
