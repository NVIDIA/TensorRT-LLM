...
class TestSwaScratchReuseAutoResolution:
    ...
    @pytest.mark.parametrize("attn_backend", ["FLASHINFER", "flashinfer"])
    def test_auto_enables_on_v2_with_capable_backend(self, attn_backend):
        # Lowercase is a working config: get_attention_backend() upper-cases
        # the name, so the resolver has to as well or it silently declines.
    ...
    def test_auto_declines_trtllm_over_the_custom_mask_path(self):
        """TRTLLM can address a scratch page -- except for one context path.

        A bidirectional multimodal mask is served by TritonCustomMaskFmha,
        which refuses scratch reuse, and no other FMHA library accepts a
        CUSTOM mask -- so selection raises rather than falling back. Gemma3 and
        Gemma4 default to this backend and take that path on any image input,
        which is exactly the configuration 'auto' must not break.
        """
        llm_args = self._args(use_kv_cache_manager_v2=True,
                              attn_backend="TRTLLM")
    ...
    def test_auto_disables_on_v1(self):
        # Scratch reuse is a V2-only feature. Backend is one 'auto' accepts, so
        # a False here can only be the manager version.
    ...
    def test_auto_disables_when_manager_version_unresolved(self):
        ...
        llm_args = self._args(attn_backend="FLASHINFER")
    ...
    def test_explicit_value_is_untouched(self, user_setting):
        # TRTLLM on purpose: 'auto' declines it, an explicit True does not.
        # A text-only TRTLLM deployment can still opt in.
    ...
    def test_explicit_true_records_that_the_user_asked(self):
        """The V1 warning needs to tell a request from a resolved default."""
        explicit = self._args(enable_swa_scratch_reuse=True,
                              use_kv_cache_manager_v2=True,
                              attn_backend="FLASHINFER")
        resolved = self._args(use_kv_cache_manager_v2=True,
                              attn_backend="FLASHINFER")
        _resolve_swa_scratch_reuse_auto(resolved)

        # Same field value; only one of them was asked for.
        assert explicit.kv_cache_config.enable_swa_scratch_reuse is True
        assert resolved.kv_cache_config.enable_swa_scratch_reuse is True
        assert explicit.kv_cache_config._swa_scratch_reuse_was_explicit is True
        assert (resolved.kv_cache_config._swa_scratch_reuse_was_explicit
                is False)
