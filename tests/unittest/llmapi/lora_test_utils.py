...
def create_mock_nemo_lora_checkpoint(
    ...
    with tempfile.TemporaryDirectory() as temp_dir_str:
        ...
        # NOTE:
        # test_llm_pytorch.py::test_gqa_nemo_lora asserts that the LoRA adapter
        # changes the generated text relative to a no-LoRA baseline (difference-based
        # assertion — no output strings are hardcoded). Both seed=42 and
        # WEIGHTS_COEFFICIENT affect whether the adapter is large enough to shift
        # the output, so changes to either may cause that test to fail.
