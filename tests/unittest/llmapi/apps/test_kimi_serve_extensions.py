...
from tensorrt_llm.serve.openai_protocol import (
    ChatCompletionRequest, ChatCompletionToolsParam, StreamOptions,
    _response_format_to_guided_decoding_params)
from tensorrt_llm.serve.openai_server import (_apply_kimi_chat_extensions,
                                              _dynamic_tool_dicts,
                                              _enforce_kimi_param_policy,
                                              _validate_kimi_dynamic_tools)
from tensorrt_llm.serve.tool_parser.kimi_k3_tool_parser import (
    KimiK3ToolParser, _escape_attr, _parse_attrs, _unescape_attr)
...
def dynamic_system_msg(tools, **extra) -> dict:
    return {"role": "system", "content": "", "tools": tools, **extra}


class TestToolChoiceValidation:

    def test_required_with_tools_accepted(self):
    ...
    def test_required_with_empty_tools_rejected(self):
        with pytest.raises(ValidationError, match="tools.*must be set"):
            make_request(tools=[], tool_choice="required")

    def test_required_with_dynamic_only_tools_accepted(self):
        req = make_request(
            messages=[dynamic_system_msg([WEATHER_TOOL]), USER_MSG],
            tool_choice="required")
        assert req.tool_choice == "required"

    def test_named_with_dynamic_only_tools_rejected(self):
        with pytest.raises(ValidationError, match="tools.*must be set"):
            make_request(
                messages=[dynamic_system_msg([WEATHER_TOOL]), USER_MSG],
                tool_choice={
    ...
    def test_auto_without_tools_accepted(self):
    ...
    def test_dynamic_only_tools_default_tool_choice_to_auto(self):
        req = make_request(
            messages=[dynamic_system_msg([WEATHER_TOOL]), USER_MSG])
        assert req.tool_choice == "auto"

    def test_no_tools_defaults_to_none(self):
...
class TestMessageToolsCarrierValidation:
    ...
    validation strips unknown keys from non-system messages, so a serving-side
    check would never see them."""

    def test_tools_in_user_message_rejected(self):
    ...
    def test_tools_in_assistant_message_rejected(self):
        with pytest.raises(ValidationError, match="only allowed on system"):
            make_request(messages=[
                USER_MSG,
                {
    ...
    def test_null_tools_key_ignored_everywhere(self):
    ...
    def test_system_tools_key_survives_validation(self):
        req = make_request(
            messages=[dynamic_system_msg([WEATHER_TOOL]), USER_MSG])
        assert _dynamic_tool_dicts(req.messages) == [WEATHER_TOOL]

    def test_system_tools_with_content_survives_validation(self):
        ...
        req = make_request(messages=[
            dynamic_system_msg([WEATHER_TOOL], content="not empty"), USER_MSG
        ])
        assert _dynamic_tool_dicts(req.messages) == [WEATHER_TOOL]
...
class TestKimiDynamicToolsValidation:
    ...
    def test_valid_dynamic_tool_passes(self):
        self.check([dynamic_system_msg([WEATHER_TOOL]), USER_MSG])

    def test_absent_content_passes(self):
        self.check([{
            "role": "system",
            "tools": [WEATHER_TOOL]
        }, USER_MSG])

    def test_strict_false_passes(self):
        tool = json.loads(json.dumps(WEATHER_TOOL))
        tool["function"]["strict"] = False
        self.check([dynamic_system_msg([tool]), USER_MSG])

    def test_nonempty_content_rejected(self):
        with pytest.raises(ValueError, match="empty content"):
            self.check(
                [dynamic_system_msg([WEATHER_TOOL], content="x"), USER_MSG])

    def test_tools_not_array_rejected(self):
    ...
    def test_tool_item_not_object_rejected(self):
    ...
    def test_duplicate_within_message_rejected(self):
        with pytest.raises(ValueError, match="Duplicate tool name"):
            self.check(
                [dynamic_system_msg([WEATHER_TOOL, WEATHER_TOOL]), USER_MSG])

    def test_duplicate_across_messages_rejected(self):
        with pytest.raises(ValueError, match="Duplicate tool name"):
            self.check([
    ...
    def test_duplicate_against_request_tools_rejected(self):
        with pytest.raises(ValueError, match="Duplicate tool name"):
            self.check([dynamic_system_msg([WEATHER_TOOL]), USER_MSG],
                       tools=[WEATHER_TOOL],
                       tool_choice="auto")
...
class TestKimiExtensionMapping:
    ...
    def test_reasoning_effort_applies_when_thinking_effort_absent(self):
        req = self.apply(thinking={
            "type": "enabled",
            "keep": "all"
        },
                         reasoning_effort="max")
        assert self.kwargs_of(req)["thinking_effort"] == "max"

    def test_reasoning_effort_none_disables_thinking_when_alone(self):
    ...
    def test_reasoning_effort_none_does_not_override_explicit_thinking(self):
        req = self.apply(thinking={
            "type": "enabled",
            "keep": "all"
        },
    ...
    def test_no_effort_derived_for_explicitly_disabled_thinking(self):
        req = self.apply(thinking={"type": "disabled"},
                         reasoning_effort="high")
        assert self.kwargs_of(req)["thinking"] is False
        assert "thinking_effort" not in self.kwargs_of(req)

    def test_medium_effort_has_no_k3_equivalent(self):
        req = self.apply(reasoning_effort="medium")
        assert "thinking_effort" not in self.kwargs_of(req)
        assert "thinking" not in self.kwargs_of(req)

    def test_default_reasoning_effort_not_mapped(self):
        req = self.apply()
        assert req.chat_template_kwargs is None

    def test_client_chat_template_kwargs_win(self):
        req = self.apply(chat_template_kwargs={"thinking": True},
                         reasoning_effort="none")
        assert self.kwargs_of(req)["thinking"] is True

    def test_stream_options_defaulted_for_streaming(self):
    ...
    @pytest.mark.parametrize("wrapper, msg", [
        ({
            "schema": {
                "type": "object"
            }
    ...
    def test_response_format_json_schema_wrapper_validation(
            self, wrapper, msg):
        with pytest.raises(ValueError, match=msg):
            self.apply(response_format={
                "type": "json_schema",
                "json_schema": wrapper
            })
...
class TestKimiParamPolicy:
    ...
    def test_policy_env_off_switch(self, monkeypatch):
...
class TestKimiResponseFormatGuidedDecoding:
    ...
    def test_thinking_mode_builds_triggered_tags_on_response_channel(self):
    ...
    def test_non_thinking_mode_returns_raw_grammar(self):
        from tensorrt_llm.serve.openai_protocol import ResponseFormat
        params = _response_format_to_guided_decoding_params(
            ResponseFormat(type="json_object"),
            reasoning_parser="kimi_k3",
...
class TestKimiK3StrictGrammar:
    ...
    def build(self, monkeypatch, tools, gate="1"):
    ...
    def test_format_shape(self, monkeypatch):
        ...
        assert elements[2]["json_schema"] == WEATHER_TOOL["function"][
            "parameters"]
    ...
    def test_tool_name_attribute_escaping_round_trip(self, monkeypatch):
        name = 'we"ird&name'
        tool = {
            "type": "function",
            "function": {
    ...
    def test_grammar_compiles_with_xgrammar(self, monkeypatch):
