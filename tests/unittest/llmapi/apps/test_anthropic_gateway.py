...
def test_backend_refresh_takes_the_late_url_and_end_time():
    ...
    Both fields below arrive after the first read and were previously latched.
    `end_time` is absent until the job resolves its wall clock, and
...
# Restored after trimming. Both guard a default we deliberately changed or a
# failure that takes the serving backend down, which is a higher bar than the
# rest of the deleted set cleared.
...
def test_keyless_request_is_rejected_unless_anonymous_is_allowlisted():
    ...
    # With no backend reachable the request cannot be proxied, but it now gets
