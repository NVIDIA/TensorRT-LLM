...
from tensorrt_llm._torch.visual_gen.executor import (
    DiffusionResponse,
    _output_media_to_handles,
    _restore_output_media,
)
...
class TestExecutorSharedTensorIPC(unittest.TestCase):
    ...
    def test_restore_ignores_ready_dict_output(self):
        """READY responses carry a plain dict output, not a PipelineOutput."""
        resp = DiffusionResponse(request_id=-1, output={"status": "READY"})
        _restore_output_media(resp)  # must not raise
