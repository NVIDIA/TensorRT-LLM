...
def main():
    ...
    runner = ModelRunnerCpp.from_dir(
        engine_dir='models/llama1b_engine/',
        max_input_len=512,
        rank=0,
    )

    # Load tokenizer for encoding the input text
    tokenizer = AutoTokenizer.from_pretrained('models/llama1b')
    ...
    print(f"Input tokens [{input_ids[0].shape}]: {str(input_ids)}")
    ...
    print("Output tokens: {str(output_ids)}")
    print(output_ids)
