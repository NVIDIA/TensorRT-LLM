...
# _deps is not included in VENDOR_MARKERS because it has special handling
...
class PatternMatcher:
        ...
        try:
            ...
            for dep in data.get("dependencies", []):
                ...
                self.known_names.update(
                    [display.lower(), f"_deps/{name}-src".lower(), f"{name}-src".lower()]
                )
    ...
    def has_unknown_vendor(self, file_path: str) -> bool:
        """Check if file is in unknown vendor directory."""
        ...
        for marker in VENDOR_MARKERS:
            ...
            while (idx := path_lower.find(marker, idx)) != -1:
                ...
                component = file_path[start : end if end != -1 else None]
                if component and component.lower() not in self.known_names:
                    return True
