...
def parse_arguments():
    ...
    parser.add_argument('--hidden_act', type=str, default='gelu', help='Set to swiglu to use GLU in MoEs')
    parser.add_argument(
        '--moe_num_experts',
        default=0,
        type=int,
...
def convert_hf_dbrx(model_params: dict,
    ...
    for l in layers_range:
        ...
        if int8_kv_cache:
            qkv_weight = get_weight(model_params, f'{prefix}{norm_prefix}.attn.Wqkv', dtype)
            ...
            int8_weights = generate_int8(qkv_weight,
                                         act_range.get(f'{prefix}{norm_prefix}.attn.Wqkv'),
                                         is_qkv=True,
        ...
        # input layer_norm
        input_ln_weight = get_weight(model_params, f'{prefix}{norm_prefix}.norm_1', dtype)
        weights[f'{tllm_prex}.input_layernorm.weight'] = input_ln_weight

        # post layer_norm
        post_ln_weight = get_weight(model_params, f'{prefix}{norm_prefix}.norm_2', dtype)
        weights[f'{tllm_prex}.post_layernorm.weight'] = post_ln_weight

        if moe_config and moe_config.has_moe():
            # experts mlp w1
            experts_weights_1 = get_weight(model_params, f'{prefix}.ffn.experts.mlp.w1', dtype)
            experts_weights_1 = torch.reshape(experts_weights_1, (-1, mlp_hidden_size, num_hidden))
            if moe_config.tp_mode == MoeConfig.ParallelismMode.TENSOR_PARALLEL:
                ...
                experts_weights_1_proj_w = split_matrix(experts_weights_1,
                                        ...
                                        dim=0)
            # experts mlp v1
            experts_weights_3 = get_weight(model_params, f'{prefix}.ffn.experts.mlp.v1', dtype)
            experts_weights_3 = torch.reshape(experts_weights_3, (-1, mlp_hidden_size, num_hidden))
            if moe_config.tp_mode == MoeConfig.ParallelismMode.TENSOR_PARALLEL:
                ...
                experts_weights_3_proj_w = split_matrix(experts_weights_3,
                                            ...
                                            mapping.tp_rank,
                                            dim=0)
            experts_weights_1_proj_w = torch.concat([experts_weights_3_proj_w, experts_weights_1_proj_w], dim=-2)
            weights.update(
                get_tllm_linear_weight(experts_weights_1_proj_w, f'{tllm_prex}.mlp.experts_weight_1', None,
                                    use_weight_only, plugin_weight_only_quant_type, postfix="", quant_scale_name=tllm_prex +
                                       '.mlp.experts_scale_1'))

            # experts mlp w2
            experts_weights_2 = get_weight(model_params, f'{prefix}.ffn.experts.mlp.w2', dtype)
            experts_weights_2 = torch.reshape(experts_weights_2, (-1, mlp_hidden_size, num_hidden)).transpose(1,2)
            if moe_config.tp_mode == MoeConfig.ParallelismMode.TENSOR_PARALLEL:
                ...
                experts_weights_2_proj_w = split_matrix(experts_weights_2,
                                            ...
                                            dim=0)
            weights.update(
                    get_tllm_linear_weight(experts_weights_2_proj_w, f'{tllm_prex}.mlp.experts_weight_2', None,
                                        use_weight_only, plugin_weight_only_quant_type, postfix="", quant_scale_name=tllm_prex +
                                        '.mlp.experts_scale_2'))

            # router mlp
            router_weights = get_weight(model_params, f'{prefix}.ffn.router.layer', dtype)
            weights.update(
                    get_tllm_linear_weight(router_weights, f'{tllm_prex}.mlp.router.', None,
                                        False, plugin_weight_only_quant_type))
...
if __name__ == '__main__':
    ...
    if args.model_dir is not None:
        ...
        args.moe_num_experts = getattr(hf_config.ffn_config, "moe_num_experts", 0)
