...
if [ -z "${CONTAINER_IMAGE:-}" ]; then
    ...
    if [ "$MACHINE" == "x86_64" ]; then
        DOCKER_IMAGE=$(source ../../jenkins/current_image_tags.properties && echo $LLM_DOCKER_IMAGE)
    elif [ "$MACHINE" == "aarch64" ]; then
        DOCKER_IMAGE=$(source ../../jenkins/current_image_tags.properties && echo $LLM_SBSA_DOCKER_IMAGE)
    else
