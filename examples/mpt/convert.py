...
def write_zero_bias(weight_name: str, saved_path: str, bias_shape: List[int],
                    data_type: torch.dtype) -> None:
    """Write zeros for bias.

    MPT model might not have bias while FT expects bias for LayerNorm.

    Args:
        weight_name (str): Name of the weight tensor.
        saved_path (str): Directory path for storing the weight and zero bias.
    ...
    save_val(bias, saved_path, bias_name)
...
def split_and_save_weight(i, saved_dir, factor, key, vals, storage_type,
                          act_range, config):
    ...
    no_bias = config.get('no_bias', False)
    ...
    if "input_layernorm.weight" in key or "input_layernorm.bias" in key or \
        ...
        "post_attention_layernorm.bias" in key or "mlp.dense_4h_to_h.bias" in key:

        # shared weights, only need to convert the weights of rank 0
        if i == 0:
            save_val(vals[0], saved_dir, key)
            if 'weight' in key and no_bias:
                write_zero_bias(key, saved_dir, vals[0].shape[-1], storage_type)
    ...
    elif "attention.query_key_value.weight" in key or "attention.query_key_value.bias" in key:
        val = vals[0]
        ...
        split_vals = np.split(vals[0], factor, axis=0)
        save_split(split_vals, saved_dir, key, i, factor)
    else:
