...
def convert_weight_to_ft_each(out_dir: str, tensor_parallelism: int,
    ...
    if tensor_name.find('input_layernorm.weight') != -1 or tensor_name.find('input_layernorm.bias') != -1 or \
        ...
        if 'kv_n_heads' in config['attn_config']:
            ...
            assert data.shape == (
                config['d_model'] + 2 * config['attn_config']['kv_n_heads'] * head_dim,
                config['d_model']
            ...
            w_q, w_k, w_v = np.split(
                data, 
                [config['d_model'], config['d_model'] + (config['attn_config']['kv_n_heads'] * head_dim)], 
                axis=-1
            )
...
def convert_mpt_to_ft(model_name_or_path: str,
    ...
    try:
        ...
        if 'kv_n_heads' in hf_config['attn_config']:
            config['gpt']['n_kv_head'] = str(hf_config['attn_config']['kv_n_heads'])
