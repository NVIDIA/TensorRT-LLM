...
def plot_pareto(results: List[Dict], output_dir: str) -> Optional[str]:
    ...
    ax.set_title("DSV3.2 FP4 128k/8k TRTLLM Pareto, GB300 NVL72, Gen-Only SOL, #Gen GPUs Per Instance<=32", fontsize=10)
    ax.legend(loc="best")
    ...
    ax.set_xlabel("Output Throughput per User (tok/s)", fontsize=12)
    ax.set_ylabel("Output Throughput per GPU (tok/s/gpu)", fontsize=12)
    ax.set_title("DSV3.2 FP4 128k/8k TRTLLM Pareto, GB300 NVL72, Gen-Only SOL, #Gen GPUs Per Instance<=32 (Denoised)", fontsize=10)
    ax.legend(loc="best")
    ax.grid(True, alpha=0.3)
