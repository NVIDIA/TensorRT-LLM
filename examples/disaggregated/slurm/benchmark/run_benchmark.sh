...
# Copy initial worker logs if they exist
if [ -f "${log_path}/output_workers.log" ]; then
    cp ${log_path}/output_workers.log ${log_path}/workers_start.log
else
    echo "No initial worker logs found at ${log_path}/output_workers.log"
    touch ${log_path}/workers_start.log
fi
