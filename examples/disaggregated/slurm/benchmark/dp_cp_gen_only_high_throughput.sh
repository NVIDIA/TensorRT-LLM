...
# High Throughput Benchmark script for DP x CP sweep in gen-only mode
# This script sweeps TP (AttnDP) and CP combinations for high throughput scenarios.
#
# Constraints:
#   - Global Batch Size: 32, 64, 128, 256, 512, 1024
#   - num_gen_gpus: 4, 8, 16, 32
...
CONFIGS_DIR="${WORK_DIR}/saved_configs/${TIMESTAMP}_dp_cp_high_throughput"
...
# =============================================================================
# 2 GPU COMBINATIONS - TP x CP = 2, DEP mode
# =============================================================================
GEN2_COMBINATIONS=(
    # dep_2: TP=2, CP=1
...
# =============================================================================
# 4 GPU COMBINATIONS - TP x CP = 4, DEP mode
# =============================================================================
GEN4_COMBINATIONS=(
    # dep_4: TP=2, CP=2
    ...
    "4,128,131072,8192,1,2,2,4,1"
    # dep_4: TP=4, CP=1
    "4,8,131072,8192,1,4,1,4,1"
    "4,16,131072,8192,1,4,1,4,1"
    "4,32,131072,8192,1,4,1,4,1"
    "4,64,131072,8192,1,4,1,4,1"
)
...
GEN8_COMBINATIONS=(
    ...
    "8,512,131072,8192,1,4,2,8,1"
    # dep_8: TP=8, CP=1
    "8,16,131072,8192,1,8,1,8,1"
    "8,32,131072,8192,1,8,1,8,1"
    "8,64,131072,8192,1,8,1,8,1"
    "8,128,131072,8192,1,8,1,8,1"
    "8,256,131072,8192,1,8,1,8,1"
)
...
GEN16_COMBINATIONS=(
    ...
    "16,1024,131072,8192,1,4,4,16,1"
    # dep_16: TP=8, CP=2
    ...
    "16,1024,131072,8192,1,8,2,16,1"
    # dep_16: TP=16, CP=1
    "16,16,131072,8192,1,16,1,16,1"
    "16,32,131072,8192,1,16,1,16,1"
    "16,64,131072,8192,1,16,1,16,1"
    "16,128,131072,8192,1,16,1,16,1"
    "16,256,131072,8192,1,16,1,16,1"
    "16,512,131072,8192,1,16,1,16,1"
)
...
GEN32_COMBINATIONS=(
    ...
    # dep_32: TP=32, CP=1
    "32,32,131072,8192,1,32,1,32,1"
    "32,64,131072,8192,1,32,1,32,1"
    "32,128,131072,8192,1,32,1,32,1"
    "32,256,131072,8192,1,32,1,32,1"
    "32,512,131072,8192,1,32,1,32,1"
    "32,1024,131072,8192,1,32,1,32,1"
)

# =============================================================================
# CP > TP COMBINATIONS - Cases where context parallelism exceeds tensor parallelism
# These are separated as they may require special handling (e.g., CUDA graph min_batch_size)
# =============================================================================
CP_GT_TP_COMBINATIONS=(
    # dep_8: TP=2, CP=4
    "8,32,131072,8192,1,2,4,8,1"
    "8,64,131072,8192,1,2,4,8,1"
    "8,128,131072,8192,1,2,4,8,1"
    "8,256,131072,8192,1,2,4,8,1"
    "8,512,131072,8192,1,2,4,8,1"
    "8,1024,131072,8192,1,2,4,8,1"
    # dep_16: TP=2, CP=8
    "16,32,131072,8192,1,2,8,16,1"
    "16,64,131072,8192,1,2,8,16,1"
    "16,128,131072,8192,1,2,8,16,1"
    "16,256,131072,8192,1,2,8,16,1"
    "16,512,131072,8192,1,2,8,16,1"
    "16,1024,131072,8192,1,2,8,16,1"
    # dep_32: TP=2, CP=16
    "32,32,131072,8192,1,2,16,32,1"
    "32,64,131072,8192,1,2,16,32,1"
    "32,128,131072,8192,1,2,16,32,1"
    "32,256,131072,8192,1,2,16,32,1"
    "32,512,131072,8192,1,2,16,32,1"
    "32,1024,131072,8192,1,2,16,32,1"
    # dep_32: TP=4, CP=8
    "32,32,131072,8192,1,4,8,32,1"
    "32,64,131072,8192,1,4,8,32,1"
    "32,128,131072,8192,1,4,8,32,1"
    "32,256,131072,8192,1,4,8,32,1"
    "32,512,131072,8192,1,4,8,32,1"
    "32,1024,131072,8192,1,4,8,32,1"
)
...
usage() {
    echo "Usage: $0 [gen2|gen4|gen8|gen16|gen32|cp_gt_tp|all|all_with_cp_gt_tp]"
    echo ""
    echo "High Throughput DP x CP sweep for gen-only mode (DEP only, AttnDP=true)"
    echo "Constraints: num_gpus in {2, 4, 8, 16, 32}, PP=1"
    echo ""
    echo "Options:"
    echo "  gen2             Run 2 GPU combinations (CP <= TP)"
    echo "  gen4             Run 4 GPU combinations (CP <= TP)"
    echo "  gen8             Run 8 GPU combinations (CP <= TP)"
    echo "  gen16            Run 16 GPU combinations (CP <= TP)"
    echo "  gen32            Run 32 GPU combinations (CP <= TP)"
    echo "  cp_gt_tp         Run CP > TP combinations only (requires special CUDA graph handling)"
    echo "  all              Run all combinations where CP <= TP (default)"
    echo "  all_with_cp_gt_tp Run all combinations including CP > TP cases"
    echo ""
    echo "Naming convention:"
    echo "  dep_N  - DEP mode with N GPUs (AttnDP=true)"
    echo ""
    echo "Note: CP > TP cases are separated because they require min_batch_size >= CP"
    echo "      for CUDA graphs (auto-handled by TorchLlmArgs with Helix CP)."
    echo ""
    exit 1
}
...
case "$MODE" in
    ...
    gen32)
        ...
        ;;
    cp_gt_tp)
        COMBINATIONS=("${CP_GT_TP_COMBINATIONS[@]}")
        MODE_DESC="CP > TP combinations"
        ;;
    all)
        COMBINATIONS=("${GEN2_COMBINATIONS[@]}" "${GEN4_COMBINATIONS[@]}" "${GEN8_COMBINATIONS[@]}" "${GEN16_COMBINATIONS[@]}" "${GEN32_COMBINATIONS[@]}")
        MODE_DESC="All DP/CP High Throughput (CP <= TP)"
        ;;
    all_with_cp_gt_tp)
        COMBINATIONS=("${GEN2_COMBINATIONS[@]}" "${GEN4_COMBINATIONS[@]}" "${GEN8_COMBINATIONS[@]}" "${GEN16_COMBINATIONS[@]}" "${GEN32_COMBINATIONS[@]}" "${CP_GT_TP_COMBINATIONS[@]}")
        MODE_DESC="All DP/CP High Throughput (including CP > TP)"
        ;;
    -h|--help)
        usage
        ;;
...
echo "Constraints: num_gpus in {2, 4, 8, 16, 32}, PP=1, DEP only (AttnDP=true)"
echo "Mode: gen_only (high throughput focus)"
if [[ "$MODE" == "cp_gt_tp" || "$MODE" == "all_with_cp_gt_tp" ]]; then
    echo "Note: CP > TP cases included - CUDA graph min_batch_size will be set to CP"
fi
echo "============================================"
