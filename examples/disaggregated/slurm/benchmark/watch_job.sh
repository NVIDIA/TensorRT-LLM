...
# process's donated memory, so a single host here means the pool is prefill-only
# and the memory donors are either absent or not being allocated into.
placement() {
...
while [ ${SECONDS} -lt ${deadline} ]; do
    ...
    # The point of the donors: report as soon as a second host appears, since
    # that is the first moment prefill-written KV is provably on decode DRAM.
    ...
    # A dead batch script leaves the tree untouched; report it rather than
    # polling a corpse until the deadline.
