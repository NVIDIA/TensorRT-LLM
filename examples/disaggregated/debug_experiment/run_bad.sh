...
CUDA_VISIBLE_DEVICES=0 trtllm-serve "$MODEL_PATH" \
    ...
    --config "$SCRIPT_DIR/ctx_config_bad.yaml" 2>&1  | tee "$LOG_DIR/log_ctx_bad" &
...
CUDA_VISIBLE_DEVICES=1 trtllm-serve "$MODEL_PATH" \
    ...
    --served_model_name "$MODEL_NAME" \
    --custom_module_dirs "$CUSTOM_MODULE_DIR" \
    --config "$SCRIPT_DIR/gen_config_bad.yaml" 2>&1  | tee "$LOG_DIR/log_gen_bad" &

sleep 80
echo "Starting disaggregated server"
# Start disaggregated server
trtllm-serve disaggregated -c "$SCRIPT_DIR/disagg_config.yaml" 2>&1 | tee "$LOG_DIR/log_disagg_bad" &
...
curl http://localhost:8000/v1/completions \
    ...
    -d '{
        ...
        "temperature": 0,
        "detokenize": false
    }' -w "\n" 2>&1 | tee "$LOG_DIR/first_output_bad.json"
...
curl http://localhost:8000/v1/completions \
    ...
    -d '{
        ...
        "temperature": 0,
        "detokenize": false
    }' -w "\n" 2>&1 | tee "$LOG_DIR/second_output_bad.json"
