...
This document shows how to build and run `DeepSeek-v3` model in TensorRT-LLM.


***Current branch is a preview version only for supporting DeepSeek-V3 in bf16 precision, which will be part of the TRT-LLM official release in 0.18+ version.***
...
First, please download DeepSeek-V3 weights from HF https://huggingface.co/deepseek-ai/DeepSeek-V3.
...
The DeepSeek-V3 model requires least 32x80G GPU memory, model contains 660B parameters roughly 1.3TB memory (with BF16 precision).
...
***Caution: prefer using BF16 over FP16 for DeepSeek-V3 since model original training precision is FP8 and we found direct convert FP8 -> FP16 may cause unknown accuracy issues.***

## Usage

The TensorRT-LLM DeepSeek-V3 example code locates at [examples/deepseek_v3](./). It takes PyTorch weights as input, and builds corresponding TensorRT engines. The number of TensorRT engines depends on the number of GPUs used to run inference.
...
```bash
mpirun --allow-run-as-root -np 32 python ../summarize.py --engine_dir ./trtllm_engines/deepseek_v3/bf16/tp32-sel4096-isl2048-bs4 \
                       --hf_model_dir ./DeepSeek-V3 \
                       --data_type bfloat16 \
                       --batch_size 1 \
                       --test_trt_llm \
                       --test_hf
...
# Run MMLU evaluation
mpirun --allow-run-as-root -n 32 python ../mmlu.py --engine_dir ./trtllm_engines/deepseek_v3/bf16/tp32-sel4096-isl2048-bs4 \
                  --hf_model_dir ./DeepSeek-V3 \
                  --data_type bfloat16 \
                  --batch_size 1 \
                  --test_trt_llm \
