...
if [[ -z "$LLAMA_PATH" ]] || [[ ! -d "$LLAMA_PATH" ]] || [[ ! $(ls -A $LLAMA_PATH) ]]; then
    echo "Please export llama huggingface checkpoint dir to envvar LLAMA_PATH"
    exit 1
fi

echo "Use llama checkpoint $LLAMA_PATH"
...
case $QFORMAT in
    fp8|int8_sq)
        ;;
    *)
        echo "Usage: \`llama2_example.sh <quantization_format>\`" >&2
        echo "Unknown quantization-format argument: Expected one of: [fp8, int8_sq]" >&2
        exit 1
esac

if [ -z "$MODEL_PATH" ] || [ ! -d "$MODEL_PATH" ]; then
    echo "$MODEL_PATH is not a valid directory"
    exit 1
fi
