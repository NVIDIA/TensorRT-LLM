...
r"""Benchmark NCCL user-buffer registration for VisualGen collectives.
...
def _stats(times: list[float]) -> dict[str, float | int]:
    return {
...
def run_micro_benchmark(args) -> dict[str, dict[str, float | int]]:
...
def run_pipeline_benchmark(args) -> dict[str, dict[str, float | int]]:
...
def _print_summary(results: dict[str, dict[str, float | int]], tier: str) -> None:
    print(f"\n{'=' * 70}")
...
def main() -> None:
    ...
    parser.add_argument(
        "--out",
        type=str,
        default="bench_nccl_ub_results.json",
