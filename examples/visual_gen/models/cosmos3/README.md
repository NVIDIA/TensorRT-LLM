...
- **V2V** — video-conditioned video (`prompts/v2v.json`). Condition on a reference video via `--video_path` (a local frame directory, `.mp4`/`.avi` file, or single image; passing it without `--action_mode` selects V2V). Only the first (or last, per `condition_video_keep`) `max(condition_frame_indexes_vision) * 4 + 1` input frames condition the output (5 by default); `.mp4`/`.avi` decode requires `pip install av`.
