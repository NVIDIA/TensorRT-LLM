...
"""Evaluate a generated image with Qwen-Image-Bench/Q-Judger on TRTLLM.
...
def fix_score_json(score_json: dict[str, Any] | None,
                   level1_dim: str) -> dict[str, Any] | None:
    if not score_json:
        return score_json
...
def compute_dimension_score(score_json: dict[str, Any]) -> dict[str, Any]:
    ...
    for level2_name, level3_dict in score_json.items():
        ...
        level3_mapped = []
        for level3_name, score_obj in level3_dict.items():
            raw = score_obj.get("score") if isinstance(score_obj,
                                                       dict) else score_obj
...
def aggregate_total_score(dim_results: dict[str, dict[str, Any]]) -> float | None:
    return mean_non_none([
        result["level1_score"] for result in dim_results.values()
        if result is not None and result.get("level1_score") is not None
    ])


def parse_dimension_output(output_text: str,
                           level1_dim: str) -> tuple[dict[str, Any] | None,
                                                     dict[str, Any] | None]:
...
def evaluate(args: argparse.Namespace) -> dict[str, Any]:
    ...
    with LLM(
        ...
        for level1_dim in args.dimensions:
            ...
            output_text = llm.generate([trtllm_input],
