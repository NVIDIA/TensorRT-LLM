...
from tensorrt_llm._torch.auto_deploy.model_config_loader import resolve_registry_yaml_extra
...
class ExperimentConfig(DynamicYamlMixInForSettings, BaseSettings):
    ...
    def process_extra_cli_args(cls, data: Dict) -> Dict:
        ...
        if data.get("use_registry", False):
            ...
            # User-facing configs live next to this script, not next to the installed package.
            user_configs_dir = Path(__file__).resolve().parent / "model_registry" / "configs"
            registry_yaml_extra = resolve_registry_yaml_extra(
                model_name, config_id, required=True, user_dirs=[user_configs_dir]
            )
