...
@torch.no_grad()
def smooth_qwen2_model(model, scales, alpha, qwen2_qkv_para, qwen2_smoother):
    ...
    for name, module in model.named_modules():
        ...
        qwen2_qkv_para[layer_name_qkv] = weight.transpose(0, 1)

        # =================================================================
        layer_name = name + ".self_attn.o_proj"
        smoother = smooth_gemm(module.self_attn.o_proj.weight,
                               scales[layer_name]["x"], None, None, alpha)
        qwen2_smoother[layer_name] = smoother.float()
...
def main():
    ...
    config = {
        ...
        if hf_config is not None else "Qwen2ForCausalLM",
        'dtype': args.dtype,
    ...
    if args.model_dir is not None:
        ...
        if args.smoothquant is not None or args.int8_kv_cache:
            ...
            if args.smoothquant is not None:
                smooth_qwen2_model(model, act_range, args.smoothquant,
                                   qwen2_qkv_para, qwen2_smoother)
    convert_args = {
        'hf_model': model,
        'act_range': act_range,
    ...
    def covert_and_save(rank, convert_args):
        ...
        if args.use_weight_only and args.weight_only_precision == 'int4_gptq':
            weights = load_from_gptq_qwen(args.ammo_quant_ckpt_path,
                                           args.n_layer,
                                           args.vocab_size,
                                           mapping,
                                           dtype=args.dtype)
