...
To quickly run EXAONE-4.0 models, you can use [examples/pytorch/quickstart_advanced.py](../../../pytorch/quickstart_advanced.py):

```bash
python ../../../pytorch/quickstart_advanced.py --model_dir hf_models/$MODEL_NAME --disable_kv_cache_reuse
```
...
You can either do pre-quantized models in HF model hub, or can generate quantized model by yourself and then run models with above command:
