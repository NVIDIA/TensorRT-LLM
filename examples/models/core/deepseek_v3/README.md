...
TensorRT-LLM supports W(INT)4-A(FP)8 for MoE models on __Hopper__. Activations and weights are quantized at per-tensor and per-group (1x128) granularity respectively.

Checkpoints for DeepSeek-R1 W4A8 could be generated through PTQ from the original DeepSeek-R1 weights. The following recipes are required for generating a quantized checkpoint:
 - Calibrated FP8 per-tensor scales, could be generated by [ModelOpt](https://github.com/NVIDIA/TensorRT-Model-Optimizer), also provided [separately](https://github.com/Barry-Delaney/TensorRT-LLM/blob/user/barry/dev/convert/input_scales.safetensors)
 - DeepSeek-R1 original weights
 - Conversion scripts ([convert.sh](https://github.com/Barry-Delaney/TensorRT-LLM/blob/user/barry/dev/convert/convert.sh) with directories changed properly, and [convert.py](https://github.com/Barry-Delaney/TensorRT-LLM/blob/user/barry/dev/convert/convert.py))

After preparation, W4A8 checkpoint can be generated by calling `convert.sh`. The converted checkpoint could be used as `<YOUR_MODEL_DIR>` and consumed by other commands.
