...
Checkpoint for DeepSeek-R1 W4A8 is available at [DeepSeek-R1-W4AFP8](https://huggingface.co/Barrrrry/DeepSeek-R1-W4AFP8). The MoE activation scales are calibrated through [ModelOpt](https://github.com/NVIDIA/TensorRT-Model-Optimizer), and is available on [HuggingFace](https://huggingface.co/Barrrrry/DeepSeek-R1-W4AFP8/blob/main/act_scales.safetensors) or through the following commands:
...
Then the full checkpoint could be generated by the following commands:
```bash
#!/bin/bash
...
ACT_SCALES=/workspace/input_scales.safetensors
