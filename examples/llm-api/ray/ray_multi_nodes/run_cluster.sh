...
# To only start ray cluster across nodes:
#       >> bash -e launch_ray.sh
#
# To run TRT-LLM commands:
#       >> bash -e run_cluster.sh -- python simple_ray_single_node.py

set -euo pipefail

: "${CONTAINER:=/lustre/fsw/portfolios/coreai/users/erinh/images/trtllm_ray_dev.sqfs}"
: "${MOUNTS:=/lustre/fsw/portfolios/coreai/users/erinh:/lustre/fsw/portfolios/coreai/users/erinh}"
