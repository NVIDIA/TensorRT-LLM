"""
RL Integration Test with Placement Group Support for AsyncLLM

This script demonstrates how to use TensorRT-LLM AsyncLLM with Ray Placement Groups
for resource management in RLHF training scenarios.

Usage:
    python rl_integration_test_async_pg.py --model_dir /path/to/model --tensor_parallel_size 2

Features:
    - Automatic Ray Placement Group creation and management
    - Direct placement_where configuration
    - Compatible with FSDP weight updates
...
class trtllm_interface:
    def __init__(self, model_dir, tensor_parallel_size, placement_group=None, bundle_indices=None):
        self.world_size = dist.get_world_size()
        self.rank = dist.get_rank()
        self.device = torch.device(f"cuda:{self.rank}")
        self.model_dir = model_dir
    ...
    async def init_trtllm(self):
        self.llm = await self.load_trtllm_model(
            self.model_dir,
    ...
    async def load_trtllm_model(self, model_dir, tensor_parallel_size, placement_group=None, bundle_indices=None):
        if self.rank == 0:
            print(f"Loading TensorRT-LLM model: {model_dir}, tensor_parallel_size: {tensor_parallel_size}")
            print(f"placement_group: {placement_group}, bundle_indices: {bundle_indices}")
            ...
            try:
                # Build placement_where if placement_group and bundle_indices are provided
                placement_where = None

                if placement_group is not None and bundle_indices is not None:
                    # Simple case: one placement group with specified bundle indices
                    placement_where = [(placement_group, bundle_indices)]
                    print(f"placement_where: {placement_where}")
...
async def async_worker(rank, world_size, model_dir, tensor_parallel_size, use_fsdp, placement_group=None, bundle_indices=None):
    ...
    trtllm = trtllm_interface(model_dir, tensor_parallel_size, placement_group, bundle_indices)
...
def worker(rank, world_size, master_port, model_dir, tensor_parallel_size, use_fsdp, placement_group=None, bundle_indices=None):
    ...
    try:
        ...
        loop.run_until_complete(
            async_worker(rank, world_size, model_dir, tensor_parallel_size, use_fsdp, placement_group, bundle_indices)
        )
    finally:
...
def main():
    ...
    # Initialize Ray and create placement group
    import ray
    from ray.util.placement_group import placement_group as create_placement_group

    # Initialize Ray
    ray.init(ignore_reinit_error=True)
    print(f"Ray initialized. Available resources: {ray.cluster_resources()}")

    # Create placement group with bundles for all GPUs
    # Each bundle gets 1.0 GPU (or adjust as needed for sharing)
    n_gpus = 8
    bundles = [{"GPU": 1.0, "CPU": 1} for _ in range(n_gpus)]

    placement_group = create_placement_group(
        bundles=bundles,
        strategy="STRICT_PACK",  # Keep all GPUs on same node
        name=f"trtllm_pg_{n_gpus}gpus"
    )

    # Wait for placement group to be ready
    ray.get(placement_group.ready())
    print(f"Placement group created with {n_gpus} bundles")

    # Use all bundles
    bundle_indices = list(range(args.tensor_parallel_size))
    print(f"Using bundle indices: {bundle_indices}")

    world_size = args.tensor_parallel_size
    master_port = get_free_port()

    # Spawn workers with placement group and bundle indices
    mp.spawn(
        worker,
        args=(world_size, master_port, args.model_dir, args.tensor_parallel_size,
              args.use_fsdp, placement_group, bundle_indices),
        nprocs=world_size,
        join=True
    )
