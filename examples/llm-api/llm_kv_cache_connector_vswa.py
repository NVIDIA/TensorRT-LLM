...
### :title KV Cache Connector under VSWA
### :order 6
...
The cache key covers one block's tokens, as in the flat example. That assumes a
block's tokens determine its KV, which two prompts sharing a block but not the
prefix before it break: the second reads back KV computed under the first one's
prefix. A production connector chains the prefix into the key.
...
class VswaKvCacheConnectorLeader(KvCacheConnectorScheduler):
    def __init__(self, llm_args: TorchLlmArgs):
        ...
        # group count before the first request, and the configured window list
        # is all it has that early. That list is not what the cache groups on:
        # two configured windows at or above `max_seq_len` describe one group,
        # not two. Count effective windows instead.
        windows = self._llm_args.kv_cache_config.max_attention_window or [None]
        max_seq_len = self._llm_args.max_seq_len
        self.num_layer_groups = len(
            {self._effective_window(window, max_seq_len) for window in windows}
        )
    ...
    @staticmethod
    def _effective_window(window: Optional[int], max_seq_len: Optional[int]) -> Optional[int]:
        """The window a layer group is formed on, not the one that was configured.

        Mirrors `_resolve_v2_max_attention_window_vec`: clamp to `max_seq_len`,
        then report full attention (`None`) for a window that reaches it.
        `max_seq_len` is `None` when it was inferred from the model rather than
        configured, and the count then falls back to the configured values --
        `build_connector_meta` rejects the request if that disagrees with the
        cache.
        """
        ...
        if max_seq_len is None:
            return int(window)
        return None if int(window) >= int(max_seq_len) else int(window)
    ...
    def build_connector_meta(self, scheduler_output: SchedulerOutput):
        ...
        # the pages allocated so far; with chunked prefill the later chunks'
        # pages arrive under `cached_requests`, whose entries this loop never
        # reads. Blocks whose page is not yet allocated drop out of
        # `valid_by_group` below and are never revisited, so those chunks go
        # unsaved.
...
def build_llm(
    ...
    return LLM(
        ...
        kv_cache_config=KvCacheConfig(
            ...
            # VSWA needs the layout-describing registration path. "auto" is
            # enough for a model that declares the preference itself, which
            # Gemma-3 does.
