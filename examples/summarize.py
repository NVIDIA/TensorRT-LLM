...
def main(args):
    ...
    def eval_trt_llm(datapoint,
        ...
        if args.prompt_lookup_config is not None and \
            len(ast.literal_eval(args.prompt_lookup_config)) == 4:
            # Prompt-Lookup speculative decoding V1 workflow
    ...
    if test_trt_llm:
        ...
        if args.prompt_lookup_config is not None:
            ...
            prompt_lookup_config = ast.literal_eval(args.prompt_lookup_config)
            args.max_output_len = output_len
            max_input_len = prompt_lookup_config[
                0] + args.max_input_length + output_len
            if len(prompt_lookup_config) == 4:  # V1 workflow
                runner_kwargs.update(
                    is_orchestrator_mode=True,
                    device_ids=prompt_lookup_config[-1],
