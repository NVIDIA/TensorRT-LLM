...
from transformers import Rwkv5Model, Rwkv5ForCausalLM, Rwkv5Config
...
def build_rank_engine(builder: Builder,
    ...
    if args.model_dir is not None:
        logger.info(f'Loading HF RWKV ... from {args.model_dir}')
        tik = time.time()
        hf_rwkv = Rwkv5ForCausalLM.from_pretrained(args.model_dir,
                                                    torch_dtype="auto")
        tok = time.time()
        ...
        # TODO(Rinne): deal with bin model
        raise NotImplementedError
    ...
    with net_guard(network):
        # Prepare
        network.set_named_parameters(tensorrt_llm_rwkv.named_parameters())

        # Forward
        inputs = tensorrt_llm_rwkv.prepare_inputs(args.max_batch_size,
                                                   args.max_input_len,
                                                   args.max_output_len, True,
                                                   args.max_beam_width)
        tensorrt_llm_rwkv(*inputs)
        if args.enable_debug_output:
            # mark intermediate nodes' outputs
            for k, v in tensorrt_llm_rwkv.named_network_outputs():
                v = v.trt_tensor
                v.name = k
