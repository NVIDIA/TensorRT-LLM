...
def generate_int8(weights, act_range, is_qkv=False, multi_query_mode=False):
    ...
    if is_qkv and not multi_query_mode:
        # for ChatGLM2/3-6B, we pretend K/V has the same numbers of heads as q to reuse code of gpt
        def expandQKV(t):
            q = t[..., :4096]
            k = t[..., 4096:4096 + 256]
            v = t[..., 4096 + 256:]
            ...
            if len(t.shape) == 1:
                k = lib.tile(k, [4096 // 256])
                v = lib.tile(v, [4096 // 256])
            elif len(t.shape) == 2:
                k = lib.tile(k, [1, 4096 // 256])
                v = lib.tile(v, [1, 4096 // 256])
            else:
                print(f"Dim {len(t.shape)} is not supported")
            return lib.concatenate([q, k, v], axis=-1)

        weights = expandQKV(weights).astype(np.float32)
        act_range['x'] = act_range['x'].to(torch.float32)
        act_range['w'] = expandQKV(act_range['w']).to(torch.float32)
        act_range['y'] = expandQKV(act_range['y']).to(torch.float32)
    ...
    scale_y_orig_quant_t = np.array(127. / act_range["y"].max().item()).astype(
        np.float32)
    scale_y_quant_orig_t = 1 / scale_y_orig_quant_t
    ...
    # for CharGLM2/3-6B, restore weight back, now we try to skip this
    def squeezeQKV(t):
        q = t[..., :4096]
        k = t[..., 4096:4096 + 256]
    ...
    if is_qkv and not multi_query_mode:
        return {
            "weight.int8": to_i8( \
                weights.reshape(4096,3,4096) * scale_w_orig_quant_t.reshape(1,3,1) \
                    ),
            "weight.int8.col": to_i8( \
                weights.reshape(4096,3,4096) * scale_w_orig_quant_c.reshape(1,3,4096) \
                ),
...
def split_and_save_weight(
    ...
    if "input_layernorm.weight" in key or "input_layernorm.bias" in key or \
        ...
        "post_attention_layernorm.weight" in key or "post_attention_layernorm.bias" in key or \
        "mlp.dense_4h_to_h.bias" in key or \
        ...
        head_size = (val.shape[-1] - local_dim) // 4
        val = val.reshape(hidden_dim, local_dim + 4 * head_size)
