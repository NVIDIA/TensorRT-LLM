...
def drafter_tensor_plan(hidden, cfg, num_capture, vocab=None,
                        markov_rank=None, use_confidence_head=False):
...
def drafter_config(hidden, vocab, num_target_layers, target_layer_ids,
                   mask_token_id, cfg):
    dflash_cfg = {
    ...
    return {
        ...
        "layer_types": (["sliding_attention"] if is_dspark else
...
def target_dims_from_ckpt(ckpt_dir):
    ...
    return (text["hidden_size"], text["vocab_size"],
...
def drafter_cfg_from_real_config(path):
    ...
    if missing:
        raise KeyError(f"real drafter config is missing {missing}; "
    ...
    if "target_layer_ids" not in dflash_cfg:
        raise KeyError("real drafter config has no "
    ...
    dims = dict(
        ...
        head_dim=real.get(
            "head_dim",
            real["hidden_size"] // real["num_attention_heads"]),
    ...
    if not any("Laguna" in a for a in archs) and \
            real.get("model_type") not in ("qwen3", "llama"):
        print(f"WARNING: architectures={archs} model_type="
...
def main():
    ...
    mode.add_argument("--ckpt-dir",
                      help="real K3 target checkpoint dir (reads config.json)")
    ...
    mode.add_argument("--tiny",
                      action="store_true",
                      help="minimal dims for unit tests; no checkpoint access")
    ap.add_argument("--out", required=True, help="output drafter dir")
    ap.add_argument("--target-layer-ids",
                    type=int,
                    nargs="+",
                    default=None,
                    help="override capture layers (default: even spacing)")
    ap.add_argument("--mask-token-id",
                    type=int,
                    default=None,
    ...
    if args.config:
        ...
        target_layer_ids = (args.target_layer_ids
                            or real_cfg["dflash_config"]["target_layer_ids"])
        mask_token_id = (args.mask_token_id if args.mask_token_id is not None
                         else real_cfg["dflash_config"].get(
        ...
        if args.tiny:
            ...
            hidden, vocab, num_target_layers = target_dims_from_ckpt(
                args.ckpt_dir)
            cfg = dict(K3_DRAFTER)
            default_k = 6
        target_layer_ids = (args.target_layer_ids
        ...
        mask_token_id = (args.mask_token_id
                         if args.mask_token_id is not None else vocab - 2)

    torch.manual_seed(args.seed)
    plan = drafter_tensor_plan(
        hidden, cfg, len(target_layer_ids), vocab=vocab,
        markov_rank=cfg.get("markov_rank"),
        use_confidence_head=cfg.get("use_confidence_head", False))
    tensors = {
        k: (torch.randn(s, dtype=torch.float32) * 0.02).to(torch.bfloat16)
    ...
    if real_cfg is not None:
        ...
        out_cfg = drafter_config(hidden, vocab, num_target_layers,
                                 target_layer_ids, mask_token_id, cfg)
    ...
    print(f"wrote {len(tensors)} tensors ({total/1e6:.1f}M params) to "
