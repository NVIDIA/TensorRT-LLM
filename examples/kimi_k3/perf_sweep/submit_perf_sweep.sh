#!/bin/bash
# Submit the standard Kimi K3 17-point serving sweep (tuned 2026-07-26 recipes):
#   tep16     c 1 2 4 8 16         4 nodes  1:30
#   tep8      c 1 2 4 8 16         2 nodes  1:30  (--segment=2)
...
#
# Usage: submit_sweep.sh --model PATH --image PATH [--repo PATH] \
...
submit() {
    ...
    if (( DRY_RUN )); then
        ...
        echo "$name: $(sbatch "${opts[@]}" "$SBATCH_SCRIPT" "${args[@]}")"
    fi
}
