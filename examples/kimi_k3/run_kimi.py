...
def main() -> int:
    ...
    if not ok:
        return 1
    # Private-snapshot path (opt-in): the self-contained flow needs no
    # external kernel collection, but SA spec-dec still requires the
    # snapshot's fused MoE (in-tree cubins lack the bs<=8 shapes). When
    # KIMI_K3_OPT_WORK_DIR is set, validate it and materialize the
    # snapshot's missing flashinfer/_build_meta.py from version.txt.
    # HACK HACK: workaround for the (already hacky) PYTHONPATH-shadowing
    # flashinfer import — without _build_meta.py the snapshot reports
    # "0.0.0+unknown" and spec-worker construction fails the
    # flashinfer>=0.6.4 gate. Remove once the snapshot ships version
    # metadata or in-tree cubin coverage retires it for spec dec.
    opt_work = os.environ.get("KIMI_K3_OPT_WORK_DIR")
    if opt_work:
        snapshot_root = Path(opt_work).expanduser() / "trtllmgen_MOE"
        snapshot_data = snapshot_root / "flashinfer/data/csrc"
        if not snapshot_data.exists():
            print(f"error: {snapshot_data} missing — set up the flashinfer "
                  "snapshot per trtllmgen_MOE/SNAPSHOT_SETUP.md (3rdparty "
                  "clones + flashinfer/data symlinks)", file=sys.stderr)
            return 1
        build_meta = snapshot_root / "flashinfer/_build_meta.py"
        version_txt = snapshot_root / "version.txt"
        if not build_meta.exists() and version_txt.exists():
            snap_version = version_txt.read_text().strip()
            try:
                build_meta.write_text(
                    "# Generated by run_kimi.py from version.txt (the source\n"
                    "# snapshot skips the build step that would create this).\n"
                    f'__version__ = "{snap_version}"\n'
                    '__git_version__ = "unknown"\n')
                print(f"  generated {build_meta} (version {snap_version})")
            except OSError as e:
                print(f"warning: cannot generate {build_meta} ({e}); spec-dec "
                      "runs against this snapshot will fail the flashinfer "
                      "version gate", file=sys.stderr)
