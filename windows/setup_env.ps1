...
if (-not $skipCUDA){
    ...
    if (-not (Get-Package -Name $cudaVer -EA Ignore)) {
        ...
        } elseif ($cudaVersion -eq "12.6.2"){
            $cudaUri = 'https://developer.download.nvidia.com/compute/cuda/12.6.2/local_installers/cuda_12.6.2_560.94_windows.exe'
        } else {
...
if(-not $skipCUDNN){
    ...
    if(-not(Test-Path -Path $CUDA_PATH)){
        ...
        $ProgressPreference = 'SilentlyContinue'
        Invoke-WebRequest -Uri '://developer.download.nvidia.com/compute/cudnn/redist/cudnn/windows-x86_64/cudnn-windows-x86_64-9.5.0.50_cuda12-archive.zip' -OutFile $env:LOCALAPPDATA\CUDNN\cudnn.zip
        ...
        Copy-Item -Path "$env:LOCALAPPDATA\CUDNN\cudnn_unzip\cudnn-windows-x86_64-9.5.0.50_cuda12-archive\bin\*" -Destination $binPath
        Copy-Item -Path "$env:LOCALAPPDATA\CUDNN\cudnn_unzip\cudnn-windows-x86_64-9.5.0.50_cuda12-archive\include\*" -Destination $includePath
        Copy-Item -Path "$env:LOCALAPPDATA\CUDNN\cudnn_unzip\cudnn-windows-x86_64-9.5.0.50_cuda12-archive\lib\x64\*" -Destination $libPath
...
if (-not ($skipTRT)) {
    ...
    if (-not (Test-Path -Path $TRT_BASE)) {
        ...
        Invoke-WebRequest -Uri 'https://developer.nvidia.com/downloads/compute/machine-learning/tensorrt/10.6.0/zip/TensorRT-10.6.0.26.Windows.win10.cuda-12.6.zip' -OutFile .\TensorRT\trt.zip
        Expand-Archive -Path .\TensorRT\trt.zip -DestinationPath .\TensorRT\
        Remove-Item -Path .\TensorRT\trt.zip -Force
        $trtPath = Join-Path $TRT_BASE TensorRT-10.6.0.26
