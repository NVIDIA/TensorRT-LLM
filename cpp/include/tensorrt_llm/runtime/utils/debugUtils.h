...
template <typename T>
TRTLLM_API bool tensorHasInvalid(ITensor const& tensor, BufferManager const& manager, std::string const& infoStr);

TRTLLM_API bool tensorHasInvalid(
    size_t M, size_t K, nvinfer1::DataType type, void const* data, cudaStream_t stream, std::string const& infoStr);

TRTLLM_API int stallStream(
