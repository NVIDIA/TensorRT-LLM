...
public:
    ...
    //! Reduce one int32 across the group with MIN / MAX and return the result on every rank. The
    //! allocation converges per-rank success flags and state through these so that every rank
    //! takes the same branch after a phase that may fail on only some of them.
    [[nodiscard]] virtual int32_t allreduceMin(int32_t value) = 0;
    [[nodiscard]] virtual int32_t allreduceMax(int32_t value) = 0;
...
public:
    ...
    //! \param comm Group communicator; owned by this object if \p comm owns it.
    explicit MpiMcastGroupComm(tensorrt_llm::mpi::MpiComm comm);

    //! The wrapped communicator, e.g. to duplicate it.
    [[nodiscard]] tensorrt_llm::mpi::MpiComm const& getMpiComm() const
    {
        return mComm;
    }
