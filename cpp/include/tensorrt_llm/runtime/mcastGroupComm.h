...
public:
    ...
    //! \param takeOwnership Duplicate the communicator and free it on destruction. Needed when this
    //! object outlives the caller's handle, as it does across a checkpoint restore.
    explicit MpiMcastGroupComm(int64_t mpiCommFortranHandle, bool takeOwnership = false);
