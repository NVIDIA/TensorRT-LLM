...
public:
    ...
    // DeepGEMM
    template <typename LayoutIndexType>
    static void run(__nv_bfloat16* gmem_d, float* scales_b, LayoutIndexType* grouped_layout, uint32_t shape_n,
        CUtensorMap const& tma_a_desc,        // weight
        CUtensorMap const& tma_b_desc,        // act
        CUtensorMap const& tma_scales_b_desc, // act scales
        CUtensorMap const& tma_d_desc,        // out
        cudaStream_t stream, int num_sms, uint32_t smem_size)
    {
        using SchedulerType = typename SchedulerSelectorSwapAB<kGemmType, SHAPE_M, SHAPE_K, BLOCK_M, BLOCK_N, BLOCK_K,
            kNumGroups, kNumTMAMulticast>::type;
        using InputType = typename SchedulerType::Input;
        // NOTES: we must use 4 warps to do TMA, because `setmaxnreg.aligned` requires 4 warps
        constexpr uint32_t kNumTMAThreads = 128;
        constexpr uint32_t kNumMathThreadsPerGroup = 128;
        auto kernel = fp8_gemm_kernel_swapAB<SHAPE_M, SHAPE_K, BLOCK_M, BLOCK_N, BLOCK_K, kNumGroups, kNumStages,
            kNumTMAThreads, kNumMathThreadsPerGroup, kNumTMAMulticast, SchedulerType>;
        DG_HOST_ASSERT(
            cudaFuncSetAttribute(kernel, cudaFuncAttributeMaxDynamicSharedMemorySize, smem_size) == cudaSuccess);
        ...
        // Launch
        auto status = cudaLaunchKernelEx(
            &config, kernel, gmem_d, scales_b, input, tma_a_desc, tma_b_desc, tma_scales_b_desc, tma_d_desc);
        DG_HOST_ASSERT(status == cudaSuccess);
    }
    ...
    static void run(__nv_bfloat16* gmem_d, float* scales_b, LayoutIndexType* problem_n_offsets,
        ...
        CUtensorMap const& tma_a_desc,        // weight
        CUtensorMap const& tma_b_desc,        // act
        CUtensorMap const& tma_scales_a_desc, // act scales
        CUtensorMap const& tma_d_desc,        // out
        cudaStream_t stream, int num_sms, uint32_t smem_size)
    {
        using SchedulerType = typename SchedulerSelectorSwapAB<kGemmType, SHAPE_M, SHAPE_K, BLOCK_M, BLOCK_N, BLOCK_K,
    ...
    // Batched Strided GEMM
    static void run(__nv_bfloat16* gmem_d, float* scales_a, uint32_t shape_n, CUtensorMap const& tma_a_desc,
        CUtensorMap const& tma_b_desc, CUtensorMap const& tma_scales_b_desc, CUtensorMap const& tma_d_desc,
        uint64_t ld_a, uint64_t stride_a, uint64_t ld_b, uint64_t stride_b, uint64_t ld_d, uint64_t stride_d,
        cudaStream_t stream, int num_sms, uint32_t smem_size)
    {
        using SchedulerType = typename SchedulerSelectorSwapAB<kGemmType, SHAPE_M, SHAPE_K, BLOCK_M, BLOCK_N, BLOCK_K,
        ...
        InputType input{shape_n, ld_a, stride_a, ld_b, stride_b, ld_d, stride_d};
        // Launch
        auto status = cudaLaunchKernelEx(
            &config, kernel, gmem_d, scales_a, input, tma_a_desc, tma_b_desc, tma_scales_b_desc, tma_d_desc);
        DG_HOST_ASSERT(status == cudaSuccess);
    }
};
