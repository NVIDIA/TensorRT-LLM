...
TRTLLM_API void invokeQuantizeMatrix(T_OUT* output, T_S const* input_qua_amax_ptr, T_IN const* input, int64_t numel,
...
template <typename T_OUT, typename T_S, typename T_IN>
TRTLLM_API void invokeComputeScalesAndQuantizeMatrix(T_OUT* output, T_S* quant_ptr, const T_IN* weights,
    const int64_t numel, const int64_t lda, QuantizeMode quantize_mode, cudaStream_t stream);
