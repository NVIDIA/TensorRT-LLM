...
// nvbug-6627795: the capacity window now ends at kGENERATION_TO_COMPLETE so a retiring request stops
// consuming a slot in mMaxNumRequests. Its LoRA adapter, however, is still resident on device --
// LoraCache::markTaskDone runs as part of the teardown the overlap scheduler has deferred, and
// claimPagesWithEvict only evicts tasks in mDoneTasks. So the adapter must still be charged, or a
// request carrying a different adapter is admitted against a budget that only looks free and then dies
// in ensureBatch with LoraCacheFullException.
//
// Both directions are asserted. Under-charging is the bug; over-charging would be a new one, so the
// same-adapter case must still be admitted for free.
TEST_F(CapacitySchedulerTest, RetiringRequestStillChargesItsLoraAdapter)
{
    SizeType32 constexpr kvCacheTokensPerBlock = 10;
    // Deliberately generous: PEFT pages must be the only binding constraint, otherwise a KV shortfall
    // could make this test pass for the wrong reason.
    SizeType32 constexpr kvCacheMaxNumTokens = 2000;
    SizeType32 constexpr kvCacheMaxNumTokensPerSeq = 90;
    SizeType32 constexpr maxNumRequests = 8;
    // 20 pages per distinct adapter against 30 on device: one adapter fits, two never can.
    SizeType32 constexpr pagesPerTask = 20;
    SizeType32 constexpr maxDevicePages = 30;
    uint64_t constexpr retiringTaskId = 1234;
    uint64_t constexpr otherTaskId = 5678;

    auto const capacitySchedulerPolicies
        = std::vector<CapacitySchedulerPolicy>{CapacitySchedulerPolicy::kGUARANTEED_NO_EVICT,
            CapacitySchedulerPolicy::kMAX_UTILIZATION, CapacitySchedulerPolicy::kSTATIC_BATCH};

    for (auto capacitySchedulerPolicy : capacitySchedulerPolicies)
    {
        // true  -> the incoming request reuses the retiring adapter, so it needs no new pages.
        // false -> it brings a second adapter, which cannot fit alongside the resident one.
        for (bool reuseRetiringAdapter : {false, true})
        {
            auto kvCacheManager = getKvCacheManager(
            ...
            std::shared_ptr<BasePeftCacheManager> peftCacheManager
                = std::make_shared<MockPeftCacheManager>(pagesPerTask, maxDevicePages, maxDevicePages);
            // The window this fix installs: kGENERATION_TO_COMPLETE is outside it.
            auto capacityScheduler = CapacityScheduler(maxNumRequests, capacitySchedulerPolicy,
                static_cast<bool>(kvCacheManager), /*twoStepsLookAhead=*/false, LlmRequestState::kCONTEXT_INIT,
                LlmRequestState::kGENERATION_TO_COMPLETE);

            int32_t constexpr maxNewTokens = 50;
            int32_t constexpr promptLen = 10;

            RequestList activeRequests;
            // Final token produced, teardown deferred: not schedulable, but its adapter is still loaded.
            activeRequests.push_back(createRequest(promptLen, maxNewTokens, 0, retiringTaskId,
                tensorrt_llm::executor::Request::kDefaultPriority, LlmRequestState::kGENERATION_TO_COMPLETE));
            activeRequests.push_back(
                createRequest(promptLen, maxNewTokens, 1, reuseRetiringAdapter ? retiringTaskId : otherTaskId));
            ...
            // The retiring request is outside the window, so it must never be scheduled either way.
            for (auto const& req : scheduledRequests)
            {
                EXPECT_NE(req->mRequestId, 0u)
                    << "retiring request was scheduled, policy " << static_cast<int32_t>(capacitySchedulerPolicy);
            }

            bool incomingScheduled = false;
            for (auto const& req : scheduledRequests)
            {
                incomingScheduled |= (req->mRequestId == 1u);
            }

            if (reuseRetiringAdapter)
            {
                EXPECT_TRUE(incomingScheduled)
                    << "a request reusing the resident adapter needs no new pages and must still be "
                       "admitted; policy "
                    << static_cast<int32_t>(capacitySchedulerPolicy);
            }
            else
            {
                EXPECT_FALSE(incomingScheduled)
                    << "a second adapter cannot fit beside the retiring request's resident one; "
                       "admitting it would fail in ensureBatch. policy "
                    << static_cast<int32_t>(capacitySchedulerPolicy);
            }
        }
    }
