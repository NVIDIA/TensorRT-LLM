...
TEST_F(KVCacheManagerTest, KVCacheManagerMaxAttentionWindowWithReuseTest)
{
    ...
    EXPECT_THAT(seq0.getCacheBlockIds(onlyWindowSize).at(beamIdx),
        ::testing::ElementsAreArray({KVCacheBlock::kInvalidBlockId, 1, 2, 3, 4}));
    ...
    // One block is offloaded to secondary pool due to out-of-window.
    EXPECT_EQ(blockManager.getNumFreeBlocks(kSecondaryLevel), blocksInSecondaryPool - 1);
...
TEST_F(KVCacheManagerTest, KVCacheManagerVariableWindowAttentionWithReuseTest)
{
              ...
              std::initializer_list<int> expectedBlocksMin, std::initializer_list<int> expectedBlocksMax)
    {
        ...
        auto numValidBlocksMin = std::count_if(expectedBlocksMin.begin(), expectedBlocksMin.end(),
            [](int id) { return id != KVCacheBlock::kInvalidBlockId; });
    ...
    kvCacheManager.addToken(requestId);
    auto const numValidBlocks = assertBlocks(seq0, {KVCacheBlock::kInvalidBlockId, 1, 2}, {0, 1, 2});
    auto numAllocatedPrimaryBlocks = blockManager.getNumAllocatedBlocks() - blocksInSecondaryPoolPerWindow * numWindows;
    EXPECT_EQ(numAllocatedPrimaryBlocks, numValidBlocks);
    EXPECT_EQ(blockManager.getNumFreeBlocks(), allBlocksInPrimaryPools - numValidBlocks);

    EXPECT_NO_THROW(kvCacheManager.removeSequence(requestId, llmRequest));
    numAllocatedPrimaryBlocks = blockManager.getNumAllocatedBlocks() - blocksInSecondaryPoolPerWindow * numWindows;
    EXPECT_EQ(numAllocatedPrimaryBlocks, 0);
    EXPECT_EQ(blockManager.getNumFreeBlocks(kPrimaryLevel), blocksInPrimaryPoolPerWindow * numWindows);
    // One block is offloaded to secondary pool due to out-of-window.
    EXPECT_EQ(blockManager.getNumFreeBlocks(kSecondaryLevel), blocksInSecondaryPoolPerWindow * numWindows - 1);
...
TEST_F(KVCacheManagerTest, KVCacheManagerEventStream)
{
    ...
    // Offload 2 block, onboard 1 block, remove 2 secondary block.
