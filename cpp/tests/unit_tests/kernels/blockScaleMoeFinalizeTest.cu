...
TEST(BlockScaleMoeFinalizeTest, Bf16SinglePassMatchesReferenceAndGuardsDispatch)
{
    ...
    auto const weights = manager.copyFrom(
        hostWeights, ITensor::makeShape({static_cast<int64_t>(hostWeights.size())}), MemoryType::kGPU);
    auto const map
        = manager.copyFrom(hostMap, ITensor::makeShape({static_cast<int64_t>(hostMap.size())}), MemoryType::kGPU);
    auto const totalRows = manager.copyFrom(std::vector<int32_t>{kNumRows}, ITensor::makeShape({1}), MemoryType::kGPU);
    auto const output
        = manager.gpu(ITensor::makeShape({static_cast<int64_t>(kNumTokens) * kHiddenDim}), DataType::kBF16);
    stream->synchronize();
    ...
    // One thread per 128-bit vector of a row.
    ...
    // A row that is not a whole number of warps still gets one thread per
    // vector; the last warp runs partly idle and that is cheaper than a second
    // pass. 2688 is Nemotron-3-Nano and 3.5-Lightning.
    auto partialWarp = data;
    partialWarp.hiddenDim = 2688;
    partialWarp.hiddenDimPadded = 2688;
    auto const partialWarpNode = captureFinalizeNode(partialWarp, stream->get());
    EXPECT_EQ(partialWarpNode.blockDim.x, 2688U * 16U / 128U);
    ...
    auto const runAndCompare = [&](moe::dev::finalize::Data const& runData)
    {
        // Poison the output so a kernel that writes nothing cannot pass.
        manager.setMem(*output, 0x7F);
        moe::dev::finalize::run(runData, stream->get());
    ...
    // FC2 can write a padded hidden dim, so input rows are hiddenDimPadded
    // apart while the output stays unpadded. The pad holds a value large enough
    // to wreck any sum that reads it.
    constexpr int32_t kHiddenDimPadded = 2816;
    std::vector<__nv_bfloat16> hostPadded(static_cast<int64_t>(kNumRows) * kHiddenDimPadded, __float2bfloat16(1e30F));
    for (int32_t row = 0; row < kNumRows; ++row)
    {
        std::copy_n(hostInput.begin() + static_cast<int64_t>(row) * kHiddenDim, kHiddenDim,
            hostPadded.begin() + static_cast<int64_t>(row) * kHiddenDimPadded);
    }
    auto const paddedInput
        = manager.copyFrom(hostPadded, ITensor::makeShape({static_cast<int64_t>(hostPadded.size())}), MemoryType::kGPU);
    stream->synchronize();

    auto padded = data;
    padded.inPtr = bufferCast<__nv_bfloat16>(*paddedInput);
    padded.hiddenDimPadded = kHiddenDimPadded;
    auto const paddedNode = captureFinalizeNode(padded, stream->get());
    EXPECT_EQ(paddedNode.blockDim.x, kVecPerRow);
    runAndCompare(padded);
}
