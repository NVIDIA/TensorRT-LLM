...
TYPED_TEST(LargeMixtureOfExpertsTest, RunProfiler)
{
    ...
    auto test_func = [this](GemmProfilerBackend::GemmToProfile gemm_to_profile)
    {
        ...
        backend.init(this->mMoERunner, gemm_to_profile,
            ...
            /*use_fp8_block_scaling=*/false, MOEParallelismConfig{});

        auto ws_size = backend.getWorkspaceSize(128, /*need_weights=*/true);
    ...
    for (int64_t num_tokens : {1, 128})
    {
        ...
        for (int ep : {1, 4, 8})
        {
            backend.init(this->mMoERunner, GemmProfilerBackend::GemmToProfile::GEMM_1, nvinfer1::DataType::kHALF,
                ...
                mUseLora, /*min_latency_mode=*/false, /*use_fp8_block_scaling=*/false,
            ...
            auto workspaces = backend.getProfilerWorkspaces(
                num_tokens, getSMVersion() >= 90 && getSMVersion() < 120, /*need_weights=*/true);
