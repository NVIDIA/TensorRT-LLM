...
// Test CUDAVirtualMemory materialize and release memory correctly
TEST_F(VirtualMemoryTest, TestBasic)
{
    ...
    CUDAVirtualMemory::CreatorPtr creator
    ...
    ASSERT_EQ(vm.status(), CUDAVirtualMemory::RELEASED);

    auto memoryBegin = getCurrentProcessMemoryInfo();
    vm.materialize();
    ASSERT_EQ(vm.status(), CUDAVirtualMemory::MATERIALIZED);
...
TEST_F(VirtualMemoryTest, TestOrder)
{
    ...
    class OrderTrackingCreator : public CUDAVirtualMemory::Creator
    {
    public:
        int& mCallOrder;
        int createOrder = -1;
    ...
    class OrderTrackingConfigurator : public CUDAVirtualMemory::Configurator
    {
    public:
        int& mCallOrder;
        std::string name;
...
// Test CUDAVirtualMemory behaves correctly when exceptions were thrown
TEST_F(VirtualMemoryTest, TestException)
{
    // Dummy Creator that can be configured to throw on create() or release()
    class DummyCreator : public CUDAVirtualMemory::Creator
    {
    public:
    ...
    class DummyConfigurator : public CUDAVirtualMemory::Configurator
    {
    public:
    ...
    // Test 1: Exception in creator->create()
    {
        ...
        EXPECT_EQ(vm.status(), CUDAVirtualMemory::RELEASED);
    }
    ...
    // Test 2: Exception in first configurator setup()
    {
        ...
        EXPECT_EQ(vm.status(), CUDAVirtualMemory::ERRORED);
    ...
    // Test 4: Exception in configurator teardown() during release()
    {
        ...
        vm.materialize();
        EXPECT_EQ(vm.status(), CUDAVirtualMemory::MATERIALIZED);
...
{
    // Test default constructed CUDAVirtualMemory
    {
        CUDAVirtualMemory defaultVm;

        // Should be invalid
        EXPECT_FALSE(defaultVm);
        EXPECT_EQ(defaultVm.status(), CUDAVirtualMemory::INVALID);
    }
    ...
    // Test move semantic
    {
        ...
        // Create original CUDAVirtualMemory
        CUDAVirtualMemory::CreatorPtr creator
        ...
        CUDAVirtualMemory original(std::move(creator), std::move(configurators));
        original.materialize();
        EXPECT_EQ(original.status(), CUDAVirtualMemory::MATERIALIZED);

        // Test move constructor
        CUDAVirtualMemory moved = std::move(original);
        EXPECT_FALSE(original); // Original should be invalid after move
        EXPECT_TRUE(moved);     // Moved-to object should be valid
        EXPECT_EQ(moved.status(), CUDAVirtualMemory::MATERIALIZED);

        // Test move assignment
        CUDAVirtualMemory assigned;
        ...
        EXPECT_EQ(assigned.status(), CUDAVirtualMemory::MATERIALIZED);
...
TEST_F(VirtualMemoryTest, TestDestructor)
{
    ...
    // Test destructor calls release automatically for materialized memory
    {
        ...
        alignas(CUDAVirtualMemory) std::byte storage[sizeof(CUDAVirtualMemory)];
        CUDAVirtualMemory* vm = new (storage) CUDAVirtualMemory(std::move(creator), std::move(configurators));
    ...
    // Test destructor behavior with ERRORED state
    {
        ...
        class ThrowingConfigurator : public CUDAVirtualMemory::Configurator
        {
        public:
            bool& setupCalledRef;
            bool& teardownCalledRef;
...
TEST_F(VirtualMemoryTest, TestEdgeCases)
{
    ...
    {
        auto creator = std::make_unique<DummyCreator>();
        CUDAVirtualMemory vm(std::move(creator), {});
...
TEST_F(VirtualMemoryManagerTest, TestAddException)
{
    ...
    CUDAVirtualMemory::Configurators successConfigurators; // Empty configurators should work
...
TEST_F(VirtualMemoryManagerTest, TestMaterializeException)
{
    ...
    class TestMatEx_DummyCreator : public CUDAVirtualMemory::Creator
    {
    public:
        CreatorState& state;
    ...
    CUDAVirtualMemory vm1(std::move(creator1), {});
    CUDAVirtualMemory vm2(std::move(creator2), {});
    CUDAVirtualMemory vm3(std::move(creator3), {});
...
TEST_F(VirtualMemoryManagerTest, TestReleaseException)
{
    ...
    CUDAVirtualMemory::Configurators configurators1;
    configurators1.push_back(std::move(config1));

    CUDAVirtualMemory::Configurators configurators2;
    configurators2.push_back(std::move(config2));

    CUDAVirtualMemory::Configurators configurators3;
    configurators3.push_back(std::move(config3));

    CUDAVirtualMemory::Configurators configurators4;
...
TEST_F(VirtualMemoryManagerTest, TestCudaVirtualAddressAllocator)
{
    ...
    auto buffer = std::make_unique<VirtualAddressDeviceBuffer>(
        size, nvinfer1::DataType::kINT8, CudaVirtualAddressAllocator{config});
