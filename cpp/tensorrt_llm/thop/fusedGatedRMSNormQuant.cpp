    ...
    at::Tensor const& weight, int64_t group_size, double eps, std::optional<at::Tensor> const& sf_scale)
{
    ...
    // Pad M to a multiple of 32 to avoid out-of-bounds writes in the swizzled SF layout.
    // The swizzled layout uses 128x4 tiles; vectorized stores may assume M is padded.
    ...
    at::Tensor y_fp4_padded = at::detail::empty_cuda({M_padded, N / 8}, torch::kInt32, x.device(), std::nullopt);
    at::Tensor y_fp4 = (M_padded == M) ? y_fp4_padded : y_fp4_padded.narrow(0, 0, M);

    // sf_out: scale factors in swizzled layout
    // NOTE: allocate using M_padded to avoid OOB writes for the swizzled SF layout when M is not padded.
    ...
    at::Tensor sf_out_padded = at::detail::empty_cuda({sfSizePadded}, SF_DTYPE, x.device(), std::nullopt);
