        ...
        bool enable_dsv4_epilogue_fusion) const override
    {
        ...
        if (op.isMLAEnabled())
        {
            ...
            if (enable_dsv4_epilogue_fusion)
            {
                ...
                TORCH_CHECK(output.size(2) == heads_per_group * op.mMLAParams.v_head_dim,
                    "DSv4 fused epilogue output K dimension mismatch.");
                TORCH_CHECK(output_sf_tensor.size(1) == heads_per_group * (op.mMLAParams.v_head_dim / 128),
                    "DSv4 fused epilogue output_sf K dimension mismatch.");
