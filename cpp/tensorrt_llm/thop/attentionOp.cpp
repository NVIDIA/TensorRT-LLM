...
void attention_inplace(torch::Tensor q, torch::optional<torch::Tensor> k, torch::optional<torch::Tensor> v,
    ...
    torch::Tensor total_kv_lens, torch::Tensor context_lengths, torch::Tensor host_context_lengths,
    ...
    c10::List<bool> spec_decoding_bool_params, c10::ArrayRef<std::optional<torch::Tensor>> spec_decoding_tensor_params)
{
    ...
    auto const ctx_total_kv_len = total_kv_lens.index({0}).item<int32_t>();
    auto const gen_total_kv_len = total_kv_lens.index({1}).item<int32_t>();
    ...
    if ((num_contexts > 0) && (attn_input_type != AttentionInputType::GenerationOnly))
    {
        ...
        runner->run(*op,
            ...
            mrope_position_deltas, softmax_stats_tensor, spec_decoding_tensor_params);
    }
