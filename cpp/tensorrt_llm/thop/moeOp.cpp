...
private:
        ...
        cudaStream_t stream, int experts_per_token)
    {
        ...
        // plugin leaves grouped_gemm.enabled false and uses its own cuBLAS path.
        {
            ...
            dp.run = &moeLoraGroupedGemmRunImpl;
            populateLoraGroupedGemmModule(mFc1DeviceBuf, dp.fc1);
            populateLoraGroupedGemmModule(mFc2DeviceBuf, dp.fc2);
            if (has_gated)
            {
                populateLoraGroupedGemmModule(mGatedDeviceBuf, dp.gated);
            }
