...
public:
        ...
        torch::Tensor input, torch::optional<torch::Tensor> workspace, torch::TensorList reduce_fusion_inputs) noexcept
    {
        ...
        if (runtimeStrategy == AllReduceStrategyType::UB)
        {
            ...
            auto& ub_allocator = tensorrt_llm::runtime::ub::UserBuffersManager::get_instance();
            auto ub_buffer0 = ub_allocator.search_buffer(input.data_ptr());
            TLLM_CHECK(!ub_buffer0.invalid());

            auto ub_comm = ub_allocator.comm();
            ...
            if (mOp == AllReduceFusionOp::RESIDUAL_RMS_NORM_QUANT_FP8)
            {
                ...
                std::tie(finalOutput, ub_buffer1) = torch_ext::create_ub_tensor(input.sizes(), torch::kFloat8_e4m3fn);
            ...
            else if (mOp == AllReduceFusionOp::RESIDUAL_RMS_NORM_QUANT_NVFP4)
            {
                ...
                std::tie(finalOutput, ub_buffer1) = torch_ext::create_ub_tensor(output_shape, torch::kByte);
                std::tie(scaleOutput, ub_buffer2) = torch_ext::create_ub_tensor({scale_size}, torch::kByte);
            ...
            else if (mOp == AllReduceFusionOp::RESIDUAL_RMS_NORM)
            {
                ...
                tensorrt_llm::runtime::ub::UBBuffer ub_buffer1;
                std::tie(finalOutput, ub_buffer1) = torch_ext::create_ub_tensor(input.sizes(), input.scalar_type());
