...
public:
    ...
    std::vector<torch::Tensor> run(torch::Tensor input, torch::optional<torch::Tensor> residual,
        torch::optional<torch::Tensor> norm_weight, torch::optional<torch::Tensor> scale,
        torch::optional<torch::Tensor> bias, torch::optional<torch::Tensor> workspace) noexcept
    {
        size_t size = input.numel();

        // If strategy is set to UB, UB must be used as UB impl output is special and cannot be used
        // by others.
        AllReduceStrategyType runtimeStrategy = getRuntimeStrategy(size);
...
private:
    std::vector<torch::Tensor> runUBAllReduce(torch::Tensor input, torch::optional<torch::Tensor> residual,
        torch::optional<torch::Tensor> norm_weight, torch::optional<torch::Tensor> scale,
        torch::optional<torch::Tensor> bias) noexcept
    {
        ...
        if (mOp == AllReduceFusionOp::RESIDUAL_RMS_NORM)
        {
            ...
            auto [finalOutput, ub_buffer1] = torch_ext::create_userbuffers_tensor(input.sizes(), input.scalar_type());
        ...
        else if (mOp == AllReduceFusionOp::RESIDUAL_RMS_NORM_QUANT_FP8)
        {
            ...
            auto [finalOutput, ub_buffer1] = torch_ext::create_userbuffers_tensor(input.sizes(), torch::kFloat8_e4m3fn);
            tensorrt_llm::kernels::ub::allreduce2_userbuff_inplace_rmsnorm_quant_launcher(ub_buffer0.handle, 0,
                ...
                static_cast<float*>(scale.value().data_ptr()), residual.value().data_ptr(), output.data_ptr(), mType,
                ub_comm, stream);
        ...
        else if (mOp == AllReduceFusionOp::RESIDUAL_RMS_NORM_QUANT_NVFP4)
        {
            ...
            auto [finalOutput, ub_buffer1] = torch_ext::create_userbuffers_tensor(output_shape, torch::kByte);
            auto [scaleOutput, ub_buffer2] = torch_ext::create_userbuffers_tensor({scale_size}, torch::kByte);
            ...
            tensorrt_llm::kernels::ub::allreduce2_userbuff_inplace_rmsnorm_quant_fp4_launcher(ub_buffer0.handle, 0,
                ...
                mEps, static_cast<float*>(scale.value().data_ptr()), residual.value().data_ptr(), output.data_ptr(),
    ...
    std::vector<torch::Tensor> runNCCLAllReduce(torch::Tensor input, torch::optional<torch::Tensor> residual,
        torch::optional<torch::Tensor> norm_weight, torch::optional<torch::Tensor> scale,
        torch::optional<torch::Tensor> bias) noexcept
    {
        auto stream = at::cuda::getCurrentCUDAStream(input.get_device());
    ...
    std::vector<torch::Tensor> runFusionAllReduce(torch::Tensor input, torch::optional<torch::Tensor> residual,
        torch::optional<torch::Tensor> norm_weight, torch::optional<torch::Tensor> scale,
        torch::optional<torch::Tensor> bias, torch::optional<torch::Tensor> workspace) noexcept
    {
        ...
        allreduce_fusion_params.nranks = tpSize;
        allreduce_fusion_params.rank = rank;
    ...
    AllReduceStrategyType getRuntimeStrategy(size_t size) noexcept
    {
