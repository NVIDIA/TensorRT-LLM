...
void attention(torch::Tensor q, std::optional<torch::Tensor> k, std::optional<torch::Tensor> v, torch::Tensor& output,
    ...
    bool sage_attn_smooth_k = false, int64_t num_contexts = 0, int64_t num_ctx_tokens = 0,
    bool trtllm_gen_jit_warmup = false, std::optional<int64_t> aux_kv_cache_pool_ptr = std::nullopt,
    bool const is_cross = false, std::optional<torch::Tensor> cross_kv = std::nullopt,
    ...
    double skip_correction_threshold = 0.0, std::optional<bool> uses_spcompress = std::nullopt);
