...
void attention(torch::Tensor q, std::optional<torch::Tensor> k, std::optional<torch::Tensor> v, torch::Tensor& output,
    ...
    std::optional<torch::Tensor> variable_window_token_starts = std::nullopt,
    std::optional<torch::Tensor> variable_window_token_ends = std::nullopt, double skip_correction_threshold = 0.0);
