...
void attention(torch::Tensor q, std::optional<torch::Tensor> k, std::optional<torch::Tensor> v, torch::Tensor& output,
    ...
    std::optional<torch::Tensor> compact_pseudokv_key = std::nullopt,
    std::optional<torch::Tensor> compact_pseudokv_value = std::nullopt,
    std::optional<torch::Tensor> compact_pseudokv_positions = std::nullopt,
    std::optional<torch::Tensor> compact_pseudokv_causal_mask = std::nullopt,
    std::optional<int64_t> compact_pseudokv_source_seq_len = std::nullopt, bool const is_cross = false,
