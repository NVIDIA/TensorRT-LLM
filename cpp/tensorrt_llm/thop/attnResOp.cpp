    ...
    at::Tensor rms_weight, at::Tensor output_rms_weight, double rms_eps, double output_rms_eps)
{
    ...
    check_attn_res_contract(N, T, B, H);
    TORCH_CHECK(
        T == 1 && B == 1 && H == 7168, "attn_res_rmsnorm_fwd: only production decode shape T=B=1, H=7168 is supported");
    TORCH_CHECK((N >= 1 && N <= 9) || N == 12, "attn_res_rmsnorm_fwd: supported N values are [1, 9] and 12");
    ...
    double rms_eps, double output_rms_eps)
{
    ...
    c10::cuda::CUDAGuard device_guard(layer_residual.device());
    check_attn_res_contract(N, T, B, H);
    TORCH_CHECK(T == 1 && B == 1 && H == 7168,
        "attn_res_add_rmsnorm_fwd: only production decode shape T=B=1, H=7168 is supported");
    TORCH_CHECK((N >= 1 && N <= 9) || N == 12, "attn_res_add_rmsnorm_fwd: supported N values are [1, 9] and 12");
