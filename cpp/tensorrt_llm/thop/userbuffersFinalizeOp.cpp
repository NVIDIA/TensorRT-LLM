...
torch::Tensor userbuffers_allreduce_finalize(torch::Tensor input)
{
    ...
    tensorrt_llm::kernels::ub::allgather2_userbuff_residual_launcher(
        ub_buffer1.handle, 0, size, hidden_size, input.data_ptr(), dtype, ub_comm, stream);
...
{
    m.def("userbuffers_allreduce_finalize(Tensor input) -> Tensor");
}
