...
torch::Tensor userbuffers_allreduce_finalize(torch::Tensor input, bool force_applying_finalize)
{
    ...
    auto& allocator = tensorrt_llm::runtime::ub::UserBuffersManager::get_instance();
    tensorrt_llm::runtime::ub::UBBuffer ub_buffer;
    torch::Tensor output;
    std::tie(output, ub_buffer) = torch_ext::create_ub_tensor(input.sizes(), input.scalar_type());
    ...
    tensorrt_llm::kernels::ub::allgather2_userbuff_residual_launcher(ub_buffer.handle, 0, size, hidden_size,
        input.data_ptr(), dtype, allocator.comm(), stream, force_applying_finalize);
    return output;
#else
    return input;
