...
at::Tensor run_fp8_block_scale_moe(at::Tensor const& routing_logits, at::Tensor const& routing_bias,
    ...
    int64_t const num_experts, int64_t const top_k, int64_t const num_fused_shared_expert, int64_t const n_group,
    ...
    int64_t const routing_method_type, MoeRunnerType& moe_runner, int64_t moeConfigIndex)
{
    ...
    int64_t const num_total_experts = num_experts + num_fused_shared_expert;
    int64_t const total_experts_per_token = top_k + num_fused_shared_expert;
    int64_t const num_total_local_experts = local_num_experts + num_fused_shared_expert;
    ...
    args.top_k = top_k;
    args.num_fused_shared_expert = num_fused_shared_expert;
    ...
    at::Tensor expert_weights = at::detail::empty_cuda(
        {args.num_tokens, total_experts_per_token}, routing_bias.scalar_type(), routing_logits.device(), std::nullopt);
    ...
    TORCH_CHECK(gemm1_weights_scale.sizes()[0] == num_total_local_experts, "gemm1_weights_scale has incorrect shape.");
    ...
    TORCH_CHECK(gemm2_weights_scale.sizes()[0] == num_total_local_experts, "gemm2_weights_scale has incorrect shape.");
...
public:
    ...
    [[nodiscard]] at::Tensor run(at::Tensor const& routing_logits, at::Tensor const& routing_bias,
        at::Tensor const& hidden_states, at::Tensor const& hidden_states_scale, at::Tensor const& gemm1_weights,
        at::Tensor const& gemm1_weights_scale, at::Tensor const& gemm2_weights, at::Tensor const& gemm2_weights_scale,
        int64_t num_experts, int64_t top_k, int64_t num_fused_shared_expert, int64_t n_group, int64_t topk_group,
        ...
        int64_t routing_method_type, int64_t moeConfigIndex)
    {
        ...
        return run_fp8_block_scale_moe(routing_logits, routing_bias, hidden_states, hidden_states_scale, gemm1_weights,
            gemm1_weights_scale, gemm2_weights, gemm2_weights_scale, num_experts, top_k, num_fused_shared_expert,
