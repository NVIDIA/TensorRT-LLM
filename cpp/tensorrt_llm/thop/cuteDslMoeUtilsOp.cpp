...
std::vector<torch::Tensor> moe_topk_sort_impl(torch::optional<torch::Tensor> const& routing_logits,
    ...
    RoutingMethodType const routing_method_type, bool const return_token_map = false)
{
    ...
    {
        // All native routing methods emit BF16 weights, independently of the bias dtype.
    ...
    auto const route_map_size = return_token_map
    ...
    auto permuted_idx_to_expanded_idx
        = torch::empty({route_map_size}, torch::dtype(torch::kInt32).device(torch::kCUDA));
    ...
    auto const dtypeRoutingLogits = routing_logits.has_value()
        ...
        : btg::Dtype::Bfloat16;
    auto const dtypeRoutingBias = routing_bias_dtype == at::ScalarType::Float ? btg::Dtype::Fp32 : btg::Dtype::Bfloat16;
    routing_runner.run(routing_logits_ptr, routing_bias_ptr, num_tokens, num_experts, top_k,
        ...
        return_token_map ? nullptr : permuted_idx_to_expanded_idx.data_ptr<int>(),
        return_token_map ? permuted_idx_to_expanded_idx.data_ptr<int>() : nullptr, token_final_scales_ptr,
        token_selected_experts_ptr, num_tokens_per_expert.data_ptr<int>(), tile_idx_to_expert_idx.data_ptr<int>(),
        ...
        false /* use_deep_seek_fp8 */, routing_method_type, stream, dtypeRoutingLogits, dtypeRoutingBias);
...
std::vector<torch::Tensor> moe_topk_sort(torch::Tensor const& routing_logits,
    ...
    std::optional<int64_t> const n_group, std::optional<int64_t> const topk_group, int64_t const local_expert_offset,
    int64_t const local_num_experts, std::optional<double> const routed_scaling_factor, int64_t const tile_tokens_dim,
    int64_t const routing_method_type, bool const return_token_map)
{
    ...
    return moe_topk_sort_impl(routing_logits, routing_bias, std::nullopt, std::nullopt, num_experts, top_k, n_group,
        ...
        static_cast<RoutingMethodType>(routing_method_type), return_token_map);
}
    ...
    bool const return_token_map)
{
    ...
    return moe_topk_sort_impl(std::nullopt, std::nullopt, token_selected_experts, token_final_scales, num_experts,
        ...
        RoutingMethodType::DeepSeekV3, return_token_map);
}
...
TORCH_LIBRARY_FRAGMENT(trtllm, m)
{
    m.def(
        ...
        "tile_tokens_dim, int routing_method_type, bool return_token_map=False) -> Tensor[]");
    m.def(
        "moe_sort(Tensor token_selected_experts, Tensor token_final_scales, int num_experts, int top_k, "
        "int local_expert_offset, int local_num_experts, int tile_tokens_dim, bool return_token_map=False) -> "
        "Tensor[]");
    m.def(
