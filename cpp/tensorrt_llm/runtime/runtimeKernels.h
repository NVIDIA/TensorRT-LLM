...
template <typename T>
TRTLLM_API void invokeFill(IBuffer& buffer, T value, CudaStream const& stream);

TRTLLM_API void invokeFillBatch(
    IBuffer& buffer, IBuffer const& indices, std::size_t stride, IBuffer const& values, CudaStream const& stream);

TRTLLM_API void invokeGatherBatch(IBuffer& buffer, IBuffer const& values, IBuffer const& slotIndices,
    std::size_t slotStride, CudaStream const& stream);

TRTLLM_API void invokeCopyBatch(IBuffer const& srcBuffer, IBuffer& dstBuffer, IBuffer const& srcOffsets,
    IBuffer const& dstOffsets, IBuffer const& sizes, std::size_t maxStride, CudaStream const& stream);

TRTLLM_API void scatterTensor(ITensor& output, ITensor const& input, SizeType32 beamWidth, CudaStream const& stream);

TRTLLM_API void tileTensor(ITensor& output, ITensor const& input, SizeType32 beamWidth, CudaStream const& stream);

TRTLLM_API void mergeLogitsFragments(BufferManager const& bufferManager, ITensor& output,
...
TRTLLM_API void invokeUpdateKVBlockArrayDraftTokenLocation(ITensor const& seqAcceptedDraftTokenOffsets,
