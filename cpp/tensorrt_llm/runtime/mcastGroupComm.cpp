...
MPI_Comm resolveComm(int64_t mpiCommFortranHandle, bool takeOwnership)
{
    MPI_Comm borrowed = MPI_Comm_f2c(mpiCommFortranHandle);
    if (!takeOwnership)
    {
        return borrowed;
    }
    MPI_Comm owned{MPI_COMM_NULL};
    TLLM_MPI_CHECK(MPI_Comm_dup(borrowed, &owned));
    return owned;
}
#endif
} // namespace

MpiMcastGroupComm::MpiMcastGroupComm(int64_t mpiCommFortranHandle, bool takeOwnership)
#if ENABLE_MULTI_DEVICE
    : mComm(resolveComm(mpiCommFortranHandle, takeOwnership), takeOwnership)
#else
    : mComm(nullptr, false)
#endif
{
