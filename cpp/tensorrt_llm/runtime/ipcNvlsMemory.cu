...
private:
    static CUmemAllocationHandleType getMemHandleType()
    {
        ...
        TLLM_LOG_TRACE("fabric status: healthmask=%u state=%u status=%u clique=%u", device_id, fabric_info.healthMask,
            fabric_info.state, fabric_info.status, fabric_info.cliqueId);
