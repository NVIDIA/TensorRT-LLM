...
bool ipcNvlsSupported()
{
    ...
    static bool const supported = []() -> bool
    {
        ...
        try
        {
            ...
            {
                TLLM_LOG_WARNING(
                    "NVLS multicast is advertised by the GPU but the fabric/IMEX plane is not "
                    "provisioned on this node; treating NVLS as unsupported. Provision nvidia-imex "
                    "(and expose /dev/nvidia-caps-imex-channels to the container) to enable NVLS.");
                return false;
            }
        }
