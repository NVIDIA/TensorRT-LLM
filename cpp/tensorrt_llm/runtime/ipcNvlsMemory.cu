...
public:
    ...
    [[nodiscard]] int32_t allreduceMin(int32_t value) override
    {
        int32_t result{0};
        MPI_Allreduce(&value, &result, 1, MPI_INT32_T, MPI_MIN, mComm);
        return result;
    }

    [[nodiscard]] int32_t allreduceMax(int32_t value) override
    {
        int32_t result{0};
        MPI_Allreduce(&value, &result, 1, MPI_INT32_T, MPI_MAX, mComm);
        return result;
    }
