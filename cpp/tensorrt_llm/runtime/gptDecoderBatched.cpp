    ...
    SamplingConfig const& samplingConfig, ModelConfig const& modelConfig)
{
    ...
    auto outputIdsTileView = ITensor::view(outputIds, ITensor::makeShape({beamWidth, mMaxSequenceLength}));
    kernels::invokeFill(*outputIdsTileView, endId, *stream);
    ...
    if (beamWidth > 1)
    {
        kernels::tileTensorInplace(*outputIdsTileView, beamWidth, *stream);
    }
