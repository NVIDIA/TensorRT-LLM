    ...
    , mCacheType{cacheType}
{
    ...
    {
        TLLM_CHECK_WITH_INFO(
            windowSize <= maxSequenceLength, "window size should not be greater than max sequence length");
    ...
    for (auto const& [windowSize, manager] : mWindowBlockManagers)
    {
        ...
        auto const maxTokensPerSeq = maxSequenceLength;
...
void WindowBlockManager::adjustBlocksIfNeeded(GenerationRequest& sequence, bool isEnableBlockReuse)
{
    ...
    {
        TLLM_CHECK_DEBUG(mIsSWA); // Blocks only go out-of-window in SWA
...
void WindowBlockManager::releaseBlocks(GenerationRequest& sequence, OptionalRef<LlmRequest const> llmRequest)
{
    ...
    for (auto it = allocatedBlocks.begin() + sequence.getNumFrontBlocksRemoved(); it != allocatedBlocks.end(); ++it)
    {
        auto& block = *it;
        if (block->hasRefs())
        {
    ...
    SizeType32 sinkTokenLength, SizeType32 blockCapacity, SizeType32 beamWidth, SizeType32 tokensPerBlock)
{
    ...
    auto const usableLeftoverCapacity
        = leftoverBlockCapacity > kSWAExtraBlock ? leftoverBlockCapacity - kSWAExtraBlock : 0;
    return std::min(outputLength + usableLeftoverCapacity * tokensPerBlock, inputLength + outputLength);
}
} // namespace tensorrt_llm::batch_manager::kv_cache_manager
