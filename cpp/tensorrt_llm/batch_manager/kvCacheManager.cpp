    ...
    GenerationRequest& sequence, SizeType32 inputLength, SizeType32 numContextBlocks, LlmRequest& llmRequest)
{
    ...
    result.numSharedContextBlocks = beamWidth > 1 ? numContextBlocks - 1 : numContextBlocks;
    auto searchRoot = mCachedBlocksRoot;
    ...
    for (int bi = 0; bi < result.numSharedContextBlocks; ++bi)
    {
            ...
            && result.totalMatchedTokens + numMatched <= sequence.getCurrentPrepopulatedPromptLen())
        {
            ...
            // Priority update event (same as loadOrAllocateBlocks lines 1279-1286)
    ...
    GenerationRequest& sequence, LlmRequest& llmRequest, ClaimResult& claimResult)
{
    ...
    for (auto& claimed : claimResult.claimedBlocks)
    {
        ...
        {
            // Partial match needing copy: allocate new block, copy, use new block
    ...
    auto const prepopulatedPromptLen = claimResult.totalMatchedTokens;
    ...
    TLLM_LOG_DEBUG("%s::onboardAndAllocateBlocks: Request %lu, prepopulatedPromptLen %d, numConnectorMatchedTokens %d",
        mLogPrefix.c_str(), llmRequest.mRequestId, prepopulatedPromptLen, numConnectorMatchedTokens);
