...
void WindowBlockManager::detachFrontBlock(GenerationRequest& sequence, bool const isEnableBlockReuse)
{
    ...
    for (auto beamIdx = 0; beamIdx < beamWidth; ++beamIdx)
    {
        ...
        {
            if (isEnableBlockReuse && mEvictionPolicy->getNumFreeBlocks(kSecondaryLevel) > 0)
            {
                // If we have a free secondary block, then offload it to
                // secondary and release this primary block for others to use.
                offloadBlock(outOfWindowBlock);
            }
            else
            {
                // Otherwise, simply release the block.
