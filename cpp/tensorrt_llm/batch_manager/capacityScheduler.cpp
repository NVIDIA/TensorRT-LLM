        ...
        for (auto const& requests : {pendingDisGenInitRequests, pendingRequests})
        {
            for (auto const& req : requests)
            {
                ...
                if (isEncoderInit)
                {
                    ...
                    {
                        TLLM_LOG_WARNING(
                            "Encoder-init request %lu scheduled without a cross_kv_cache_manager; skipping.",
                            req->mRequestId);
                        continue;
                    }
    ...
    std::unordered_set<uint64_t>& seenTaskIds, std::optional<kv_cache_manager::PrefixReuseSummary> const& cachedSummary)
{
    if (scheduledRequests.size() < static_cast<std::size_t>(maxNumRequests))
    {
        ...
        if (req->isEncoderInitState())
        {
            ...
            {
                TLLM_LOG_WARNING(
                    "Encoder-init request %lu scheduled without a cross_kv_cache_manager; skipping.", req->mRequestId);
                return false;
            }
