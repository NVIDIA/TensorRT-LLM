    ...
    SizeType32 maxSequenceLength, SizeType32 beamWidth, OptionalRef<MedusaBuffers const> medusaBuffers) const
{
    ...
    auto batchSlotsView = copySequenceLengths(
        contextRequests, inputBuffers, *decoderState.getSequenceLengths(), beamWidth, bufferManager, runtimeStream);

    auto [lookaheadPrompt, lookaheadAlgoConfigs]
        = createDecoderRequests(contextRequests, inputBuffers.inputsIds, decodingConfig, decoderState, bufferManager,
