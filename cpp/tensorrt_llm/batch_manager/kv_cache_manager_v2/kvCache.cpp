...
void KvCache::_appendBeams(BeamIndex oldBeamWidth, BeamIndex newBeamWidth)
{
    ...
    {
        TLLM_CHECK_DEBUG(sourcePage && sourcePage->cacheLevel == kGpuLevel);
        PoolGroupIndex const pgIdx = storage.getPoolGroupIndex(lc);
        copySlotData(storage, kGpuLevel, kGpuLevel, pgIdx, slot.slotId(), sourcePage->slotId(), cudaStream());

        KVCacheIterationStatsDelta iterationStats;
        iterationStats.iterIntraDeviceCopyBlocks = 1;
        for (size_t const size : storage.slotSize(pgIdx))
            iterationStats.iterIntraDeviceCopyBytes += static_cast<int64_t>(size);
        _recordDirectIterationStats(lc, iterationStats);

        auto page = makeShared<UncommittedPage>(*this, ordinal, lc, kGpuLevel, beamIdx);
        page->setSlot(slot);
