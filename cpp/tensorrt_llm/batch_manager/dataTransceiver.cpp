...
public:
    ...
    bool cancelRequest(LlmRequest const& llmRequest)
    {
        bool isCancelled = false;
        {
            ...
            // Active mode must settle a queued request immediately so every local rank can reach a terminal state
            // even when the peer never sends RequestInfo. Keep a tombstone so a late peer still receives ready=false.
            // Legacy mode preserves its existing promise lifetime and settles only after the peer handshake.
