...
NoEvictScheduledBlocksManager::NoEvictScheduledBlocksManager(BaseKVCacheManager const& kvCacheManager)
    : mKvCacheManager(kvCacheManager)
    , mAvailableBlocks(mKvCacheManager.getBlockManager().getNumFreeBlocksPerWindowSize())
{
}

void NoEvictScheduledBlocksManager::decrementReservedBlocks(LlmRequest const& req)
{
...
bool NoEvictScheduledBlocksManager::enoughAvailableBlocks(LlmRequest const& req)
{
...
MaxUtilizationScheduledBlocksManager::MaxUtilizationScheduledBlocksManager(
    BaseKVCacheManager const& kvCacheManager, bool twoStepsLookAhead)
    : mKvCacheManager(kvCacheManager)
...
std::optional<std::map<SizeType32, SizeType32>>
MaxUtilizationScheduledBlocksManager::prepareNewNumberOfBlocksIfWeEndUpScheduling(LlmRequest const& req)
{
    ...
    for (auto const& [windowSize, numScheduled] : mNumScheduledBlocks)
    {
        ...
        TLLM_LOG_DEBUG("MaxUtilizationScheduler: request ID %lu required blocks %i for %i window size", req.mRequestId,
            required, windowSize);
...
void MaxUtilizationScheduledBlocksManager::updateScheduledBlocks(
    std::map<SizeType32, SizeType32> const& numBlocksIfScheduled)
{
