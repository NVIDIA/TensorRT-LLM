    ...
    runtime::GptDecoderBatched const& decoder, bool returnLogProbs, runtime::CudaEvent const& decoderFinishEvent) const
{
    ...
    copyBufferManager.copy(*decoder.getDecoderState().getAllNewTokens(), *decoderBuffers.newOutputTokensHost);
    copyBufferManager.copy(
        *decoder.getDecoderState().getJointDecodingOutput().lengths, *decoderBuffers.sequenceLengthsHost);
    ...
    if (returnLogProbs)
    {
        copyBufferManager.copy(*decoder.getDecoderState().getCumLogProbs(), *decoderBuffers.cumLogProbsHost);
        copyBufferManager.copy(*decoder.getDecoderState().getLogProbs(), *decoderBuffers.logProbsHost);
    }
