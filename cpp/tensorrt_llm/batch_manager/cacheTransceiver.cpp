    ...
    std::optional<executor::CacheTransceiverConfig> cacheTransceiverConfig)
{
    TLLM_CHECK_WITH_INFO(!common::getEnvDisaggEnableInflightCancel(),
        "TRTLLM_DISAGG_ENABLE_INFLIGHT_CANCEL=1 is unsupported by the legacy TensorRT executor; unset it or set it "
        "to 0 to continue without coordinated in-flight cancellation.");
    ...
    , mRnnStateManager{rnnStateManager}
{
    ...
    auto const backendType = mCacheTransceiverConfig.has_value()
        ? mCacheTransceiverConfig->getBackendType().value_or(BackendType::DEFAULT)
        : BackendType::DEFAULT;
    bool const inflightCancelCapable = backendType == BackendType::NIXL && common::getEnvNixlBackend() == "UCX"
        && mCacheTransceiverConfig->getMaxTokensInBuffer().value_or(0) > 0
    ...
    TLLM_CHECK_WITH_INFO(backendType != BackendType::DEFAULT, " CacheTransceiverConfig::BackendType is not set.");
    ...
    if (backendType == BackendType::UCX)
    {
    ...
    else if (backendType == BackendType::NIXL)
    {
        auto rnnState
    ...
    else if (backendType == BackendType::MOONCAKE)
    {
        auto rnnState
            = mCacheState->hasRnnConfig() ? std::make_optional(mCacheState->getRnnCacheState()) : std::nullopt;
        mManager = std::make_unique<tensorrt_llm::executor::kv_cache::AgentConnectionManager>(
...
CacheTransceiver::~CacheTransceiver()
{
    ...
    // Deregister transfer memory and destroy plugin-backed connection objects
    // before their buffer owners or dynamic library are destroyed.
    mManager.reset();
...
void CacheTransceiver::requestAndReceiveSync(std::shared_ptr<LlmRequest> llmRequest)
{
    ...
    // The default-off worker reports a cleanly rejected ready handshake by
    // setting the request state and resolving its future. Preserve that error
    // instead of converting the failed transfer into a false success.
    ...
    std::optional<int> const& atLeastRequestNum, bool const markComplete)
{
    ...
    for (auto const requestId : outcome.quiescedRequestIds)
    {
