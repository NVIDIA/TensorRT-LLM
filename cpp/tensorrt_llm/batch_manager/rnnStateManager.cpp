...
std::vector<RnnStateManager::SizeType32> RnnStateManager::getPpLayers(
    SizeType32 numLayers, WorldConfig const& worldConfig, std::optional<std::vector<bool>> const& layerMask)
{
    auto const ppSize = worldConfig.getPipelineParallelism();
    auto const ppRank = worldConfig.getPipelineParallelRank();

    std::vector<SizeType32> ppLayers;

    if (ppSize == 1)
    {
        SizeType32 totalLayers = numLayers;
        if (layerMask.has_value())
        {
            totalLayers = static_cast<SizeType32>(layerMask->size());
        }
        for (SizeType32 i = 0; i < totalLayers; ++i)
        {
            if (!layerMask.has_value() || (*layerMask)[i])
            {
                ppLayers.push_back(i);
            }
        }
    }
    else
    {
        ...
        SizeType32 layersPerRank = totalLayers / ppSize;
        SizeType32 remainder = totalLayers % ppSize;

        SizeType32 startLayer = ppRank * layersPerRank + std::min(ppRank, remainder);
        SizeType32 endLayer = startLayer + layersPerRank + (ppRank < remainder ? 1 : 0);

        for (SizeType32 i = startLayer; i < endLayer; ++i)
        {
            if (!layerMask.has_value() || (*layerMask)[i])
            {
                ppLayers.push_back(i);
...
RnnStateManager::RnnStateManager(SizeType32 dState, SizeType32 dConv, SizeType32 numHeads, SizeType32 nGroups,
    SizeType32 headDim, SizeType32 numLayers, SizeType32 maxBatchSize, WorldConfig const& worldConfig, int64_t stream,
    nvinfer1::DataType dtype, nvinfer1::DataType ssmCacheDtype, std::optional<std::vector<bool>> const& layerMask)
    ...
    , mBufferManager{std::make_shared<CudaStream>(reinterpret_cast<cudaStream_t>(stream))}
{
    ...
    auto const ppLayers = getPpLayers(numLayers, worldConfig, layerMask);
