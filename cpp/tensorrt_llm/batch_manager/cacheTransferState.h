...
struct TransferTopologyOutcome
{
    ...
    // Local ownership settlement is an intersection: every rank has reached a
    // local terminal state. This does not prove remote-memory quiescence.
    std::unordered_set<TransferRequestId> allRanksTerminalRequestIds;
    ...
    std::vector<std::vector<std::uint64_t>> const& rankPayloads)
{
    ...
    for (auto const& [requestId, counts] : countsByRequest)
    {
        ...
        {
            outcome.allRanksTerminalRequestIds.insert(requestId);
