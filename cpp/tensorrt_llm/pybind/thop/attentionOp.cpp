...
void initBindings(pybind11::module_& m)
{
    m.def(
        "attention_inplace",
        [](torch::Tensor q, std::optional<torch::Tensor> k, std::optional<torch::Tensor> v, torch::Tensor output,
            std::optional<torch::Tensor> output_sf, std::optional<torch::ScalarType> out_dtype,
            std::optional<torch::Tensor> workspace_, torch::Tensor sequence_length,
            ...
            std::optional<torch::Tensor> kv_cache_block_offsets,
            ...
            std::optional<torch::Tensor> q_pe, std::optional<torch::Tensor> block_ids_per_seq, bool is_fused_qkv,
            bool update_kv_cache, int64_t predicted_tokens_per_seq, int64_t layer_idx, int64_t num_heads,
            int64_t num_kv_heads, int64_t head_size, std::optional<int64_t> tokens_per_block, int64_t max_num_requests,
            ...
            std::optional<int64_t> v_head_dim, std::optional<torch::Tensor> mrope_rotary_cos_sin,
            std::optional<torch::Tensor> mrope_position_deltas, std::optional<torch::Tensor> mla_context_paged_kv,
            ...
            std::vector<std::optional<torch::Tensor>> spec_decoding_tensor_params)
        {
            ...
            torch_ext::attention_inplace(q, k, v, output, output_sf, out_dtype, workspace_, sequence_length,
                ...
                rotary_inv_freq, rotary_cos_sin, latent_cache, q_pe, block_ids_per_seq, is_fused_qkv, update_kv_cache,
                predicted_tokens_per_seq, layer_idx, num_heads, num_kv_heads, head_size, tokens_per_block,
                max_num_requests, max_context_length, attention_window_size, sink_token_length, beam_width, mask_type,
                quant_mode, q_scaling, position_embedding_type, rotary_embedding_dim, rotary_embedding_base,
                rotary_embedding_scale_type, rotary_embedding_scales, rotary_embedding_max_position_info,
                use_paged_context_fmha, attention_input_type, is_mla_enable, q_lora_rank, kv_lora_rank,
                qk_nope_head_dim, qk_rope_head_dim, v_head_dim, mrope_rotary_cos_sin, mrope_position_deltas,
                mla_context_paged_kv, mla_context_kv_cache_block_offsets, attention_chunk_size, softmax_stats_tensor,
                cpp_spec_decoding_bool_list, spec_decoding_tensor_params);
        },
        ...
        py::arg("block_ids_per_seq") = std::nullopt, py::arg("is_fused_qkv"), py::arg("update_kv_cache"),
