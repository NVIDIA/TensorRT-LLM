...
struct HostFuncUserData
{
    ...
    HostFuncUserData(nb::callable func, nb::tuple args, nb::dict kwargs)
...
uintptr_t launchHostFunc(uintptr_t streamPtr, nb::callable pyHostFunc, nb::args pyArgs, nb::kwargs pyKwargs)
{
    auto const stream = reinterpret_cast<cudaStream_t>(streamPtr);

    auto hostFuncUserData = std::make_unique<HostFuncUserData>(pyHostFunc, nb::tuple(pyArgs), nb::dict(pyKwargs));
