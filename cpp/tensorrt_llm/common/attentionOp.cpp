    ...
    EnqueueGenerationParams<T> const& generationsParams, bool forConfigurePlugin)
{
    ...
    if (xqaParams.kv_cache_data_type == DATA_TYPE_INT8
        || (xqaParams.kv_cache_data_type == DATA_TYPE_E4M3 && (mSM < kSM_90 || mSM == kSM_120 || mSM == kSM_121)))
    {
...
int AttentionOp::initialize() noexcept
{
    ...
    if (mFP8ContextFMHA)
    {
        ...
        TLLM_CHECK_WITH_INFO(
            mSM == 89 || mSM == 90 || mSM == 100 || mSM == 120 || mSM == 121, "FP8 FMHA can only be enabled on sm_89, sm_90, sm_100, sm_120 or sm_121.");
    }
    ...
    TLLM_CHECK_WITH_INFO(!mFuseFp4Quant || (mSM >= 100 && mSM < 120), "fuse_fp4_quant only supports SM100 devices.");
