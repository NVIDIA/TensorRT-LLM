...
template <typename T, typename KVCacheBuffer>
int AttentionOp::mlaPreContext(mlaParams<T>& params, EnqueueContextParams<T> const& context_params, cudaStream_t stream)
{
...
#define MLA_FUNC_DEFINE(T, KVCacheBuffer)                                                                              \
    template int AttentionOp::mlaPreContext<T, KVCacheBuffer>(                                                         \
        mlaParams<T> & params, EnqueueContextParams<T> const& context_params, cudaStream_t stream);                    \
    template int AttentionOp::mlaGeneration<T, KVCacheBuffer>(                                                         \
        mlaParams<T> & params, EnqueueGenerationParams<T> const& generation_params, cudaStream_t stream);
...
int AttentionOp::initialize() noexcept
{
    ...
    if (mEnableXQA)
    {
        ...
        memset(&fixedParams, 0, sizeof(fixedParams));
