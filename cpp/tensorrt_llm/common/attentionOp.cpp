    ...
    int32_t max_attention_window_size, int32_t max_num_tokens) const noexcept
{
    ...
    if (mIsMLAEnabled)
    {
        ...
        // FP8 MLA buffers
        // mla_bmm1_scale_size
        workspaces[3] = mFP8GenerationMLA ? sizeof(float) * 2 : 0;
        // mla_bmm2_scale_size
        workspaces[4] = mFP8GenerationMLA ? sizeof(float) : 0;
        // quant q buffer
        workspaces[5] = mFP8GenerationMLA
            ? max_num_tokens * mNumHeads * (mMLAParams.kv_lora_rank + mMLAParams.qk_rope_head_dim)
            : 0;
        ...
        workspaces[6] = size * 256 * mMultiProcessorCount * mMLAParams.kv_lora_rank;
        // The partialSum size.
...
int AttentionOp::initialize() noexcept
{
    ...
    if (mFP8GenerationMLA)
    {
        ...
        TLLM_CHECK_WITH_INFO(mSM == 100, "FP8 Generation MLA cannot be enabled except on Blackwell Arch.");
    }
    ...
    if (mEnableContextFMHA)
    {
        ...
        if (mPagedKVCache && mPagedContextFMHA)
        {
            ...
            if (mKVCacheQuantMode.hasFp8KvCache() && !mIsMLAEnabled)
            {
