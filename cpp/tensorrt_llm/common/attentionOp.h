...
public:
    ...
    // every other path (incl. sparse MLA, which reads K/V straight from the paged cache) returns 0. This is the
    // single source of truth shared by getWorkspaceSizeForContext (runtime sizing) and the KV-cache estimator
    // (which reserves headroom for it before sizing the KV pool), so the two can never drift.
