    ...
    cudaStream_t stream)
{
    ...
    if (mEnableContextFMHA)
    {
        if (enableRingAttn)
        {
            ...
            cudaMemcpyAsync(ring_kv_buf_1_, (void*) attention_input + ring_q_buf_size, ring_kv_buf_size,
                cudaMemcpyDeviceToDevice, stream);
            cudaMemcpyAsync((void*) ring_kv_buf_1_ + ring_kv_buf_size, cu_seqlens, sizeof(int) * (batch_size + 1),
                cudaMemcpyDeviceToDevice, stream);
            // init softmax_stats
            ...
            for (size_t iter = 0; iter < mCpGroup.size(); ++iter)
            {
                ...
                fmhaParams.cuKvSeqLenPtr = reinterpret_cast<int*>((void*) ring_send_kv_buf_ + ring_kv_buf_size);
