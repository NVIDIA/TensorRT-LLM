...
class WanAttentionPlugin : public BasePlugin
{
public:
    WanAttentionPlugin() = delete;

    WanAttentionPlugin(int num_heads, int head_size, float q_scaling,
        tensorrt_llm::kernels::ContextFMHAType context_fmha_type, nvinfer1::DataType type);

    WanAttentionPlugin(void const* data, size_t length);

    ~WanAttentionPlugin() override = default;
...
class WanAttentionPluginCreator : public BaseCreator
{
public:
    WanAttentionPluginCreator();
