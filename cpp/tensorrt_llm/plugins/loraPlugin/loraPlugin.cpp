    ...
    const void* const* inputs, void* const* outputs, void* workspace, cudaStream_t stream) noexcept
{
    ...
    // outputs
    //     output [-1, mNumLoraModules, N] (view as 2D)
    ...
    for (int loraModuleIdx = 0; loraModuleIdx < mNumLoraModules; loraModuleIdx++)
    {
        ...
        {
            deviceFill((half*) output, size, (half) 0.0f, stream);
        }
        else if (mType == nvinfer1::DataType::kFLOAT)
        {
            deviceFill((float*) output, size, 0.0f, stream);
        }
#ifdef ENABLE_BF16
        else if (mType == nvinfer1::DataType::kBF16)
        {
            deviceFill((__nv_bfloat16*) output, size, (__nv_bfloat16) 0.0f, stream);
        }
#endif
    }
