 ...
 */
 ...
 __global__ void insertUnfinishedPathKernelOptimized(BeamHypotheses bh)
 {
     ...
     int const indexDstStart = bh.numBeamsCBA[bid];
     int const srcBeam = bid * nBM + tid;
     int const dstBeam = bid * nBM * 2 + tid + indexDstStart;
 
     // 预先计算step值
     int const step = bh.sequenceLengths[srcBeam] - 1;
     if (step < 0) return; // 安全检查
 
     // 1. 复制最终token
     int const srcFinalId = srcBeam * nMSL + step;
     int const dstFinalId = dstBeam * nMSL + step;
     bh.outputIdsCBA[dstFinalId] = bh.outputIdsUnfinish[srcFinalId];
 
     // 2. 重构完整序列路径
     int parentChain[2048]; // 假设最大序列长度，需要根据实际情况调整
     int parentId = bh.parentIdsUnfinish[srcFinalId];
     
     // 构建父节点链
     for (int j = step - 1; j >= 0; --j) {
         parentChain[j] = parentId;
         int const index = bid * nBM * nMSL + parentId * nMSL + j;
         parentId = bh.parentIdsUnfinish[index];
     }
 
     // 3. 批量复制所有tokens
     for (int j = 0; j < step; j++) {
         int const srcIndex = bid * nBM * nMSL + parentChain[j] * nMSL + j;
         bh.outputIdsCBA[dstBeam * nMSL + j] = bh.outputIdsUnfinish[srcIndex];
     }
 
     // 4. 如果需要，复制logprobs
     if (bOutputLogProbs) {
         bh.logProbsCBA[dstFinalId] = bh.logProbsTiled[step * nMBS * nBM + srcBeam];
         for (int j = 0; j < step; j++) {
             int const logProbIdx = j * nMBS * nBM + bid * nBM + parentChain[j];
             bh.logProbsCBA[dstBeam * nMSL + j] = bh.logProbsTiled[logProbIdx];
         }
     }
     ...
     // 6. 原子更新计数器
     if (tid == 0) {
         atomicAdd(&bh.numBeamsCBA[bid], nBM);
     }
 }
 
 void invokeInsertUnfinishedPath(BeamHypotheses& bh, cudaStream_t stream)
 {
     // 根据beam width选择合适的block size
     int blockSize = static_cast<int>(min(static_cast<unsigned int>(bh.nBeamWidth), 1024u));
     insertUnfinishedPathKernelOptimized<<<bh.nBatchSize, blockSize, 0, stream>>>(bh);
 }
 ...
 __global__ void finalizeKernelOptimized(BeamHypotheses bh)
 {
     int const bid = blockIdx.x;
     int const tid = threadIdx.x;
     size_t const nBM = bh.nBeamWidth;
     size_t const nMSL = bh.nMaxSeqLen;
     int const nCBA = bh.numBeamsCBA[bid];
     ...
     // 1. 加载scores到shared memory（合并访问）
     for (int i = tid; i < nCBA; i += blockDim.x)
     {
         smemScore[i] = bh.normedScoresCBA[bid * nBM * 2 + i];
     }
     // 填充无效值
     for (int i = nCBA + tid; i < nBM * 2; i += blockDim.x)
     {
         smemScore[i] = -FLT_MAX;
     }
     __syncthreads();
 
     // 2. 优化排序：使用bitonic sort或radix sort
     if (nCBA <= 32)
     {
         // 原有的warp-level sort保持不变
         int const warpid = tid / 32;
         int const laneid = tid % 32;
         RankNorm rankNorm{tid, tid < nCBA ? smemScore[tid] : -FLT_MAX};
         ...
         if (warpid == 0 && nCBA > 1)
         {
             rankNorm = swap(rankNorm, 0x01, bfe(laneid, 1) ^ bfe(laneid, 0));
         }
         if (warpid == 0 && nCBA > 2)
         {
             rankNorm = swap(rankNorm, 0x02, bfe(laneid, 2) ^ bfe(laneid, 1));
             rankNorm = swap(rankNorm, 0x01, bfe(laneid, 2) ^ bfe(laneid, 0));
         }
         if (warpid == 0 && nCBA > 4)
         {
             rankNorm = swap(rankNorm, 0x04, bfe(laneid, 3) ^ bfe(laneid, 2));
             rankNorm = swap(rankNorm, 0x02, bfe(laneid, 3) ^ bfe(laneid, 1));
             rankNorm = swap(rankNorm, 0x01, bfe(laneid, 3) ^ bfe(laneid, 0));
         }
         if (warpid == 0 && nCBA > 8)
         ...
         if (warpid == 0 && nCBA > 16)
         {
             rankNorm = swap(rankNorm, 0x10, bfe(laneid, 5) ^ bfe(laneid, 4));
             rankNorm = swap(rankNorm, 0x08, bfe(laneid, 5) ^ bfe(laneid, 3));
             rankNorm = swap(rankNorm, 0x04, bfe(laneid, 5) ^ bfe(laneid, 2));
             rankNorm = swap(rankNorm, 0x02, bfe(laneid, 5) ^ bfe(laneid, 1));
             rankNorm = swap(rankNorm, 0x01, bfe(laneid, 5) ^ bfe(laneid, 0));
     ...
     {
         // 使用并行的 bitonic sort（适合 GPU）
         // 为每个元素创建 (index, score) 对
         int* tempIdx = smemSL; // 复用 smemSL 空间
         ...
         // Bitonic sort - 只排序前 nBM 个最大值
         // 使用 top-k 选择算法更高效
         int const nPow2 = 1 << (32 - __clz(nCBA - 1)); // 向上取2的幂次
         
         for (int k = 2; k <= nPow2; k *= 2)
         {
             for (int j = k / 2; j > 0; j /= 2)
             {
                 for (int i = tid; i < nCBA; i += blockDim.x)
                 {
                     ...
                     if (ixj > i && ixj < nCBA)
                     {
                         ...
                         if ((smemScore[i] < smemScore[ixj]) == ascending)
                         {
                             // Swap scores
                             float tempScore = smemScore[i];
                             smemScore[i] = smemScore[ixj];
                             smemScore[ixj] = tempScore;
                             
                             // Swap indices
                             int tempIndex = tempIdx[i];
                             tempIdx[i] = tempIdx[ixj];
                             tempIdx[ixj] = tempIndex;
                         }
                     }
                 }
                 __syncthreads();
         ...
         if (tid < nBM)
         {
             smemRank[tid] = tempIdx[tid];
         }
     }
     __syncthreads();
 
     // 3. 预加载序列长度到 shared memory
     if (tid < nBM)
     {
         smemSL[tid] = bh.sequenceLengthsCBA[bid * nBM * 2 + smemRank[tid]];
         bh.sequenceLengths[bid * nBM + tid] = smemSL[tid];
     ...
     // 4. 优化数据移动 - 并行处理所有beams
     int const totalElements = nBM * nMSL;
     for (int globalIdx = tid; globalIdx < totalElements; globalIdx += blockDim.x)
     {
         int const beamIdx = globalIdx / nMSL;
         int const tokenIdx = globalIdx % nMSL;
         
         if (tokenIdx < smemSL[beamIdx])
         {
             int const dst = bid * nBM * nMSL + globalIdx;
             int const src = bid * nBM * 2 * nMSL + smemRank[beamIdx] * nMSL + tokenIdx;
             bh.outputIds[dst] = bh.outputIdsCBA[src];
         }
     }
     ...
     if (bh.logProbs != nullptr)
     {
         for (int globalIdx = tid; globalIdx < totalElements; globalIdx += blockDim.x)
         {
             ...
             if (tokenIdx >= inputLength && tokenIdx < smemSL[beamIdx])
             {
                 int const dst = bid * nBM * nMSL + globalIdx - inputLength;
                 int const src = bid * nBM * 2 * nMSL + smemRank[beamIdx] * nMSL + tokenIdx;
                 bh.logProbs[dst] = bh.logProbsCBA[src];
             }
         }
     }
 }
 ...
 void invokeFinalize(BeamHypotheses& bh, cudaStream_t stream)
 {
     ...
     int const nThread = min(1024, roundUp(nBM * 2, 32));
     // 增加 shared memory 以容纳临时索引数组
     size_t const nByteSharedMemory = sizeof(int) * nBM * 3 + sizeof(float) * nBM * 2;
     finalizeKernelOptimized<<<bh.nBatchSize, nThread, nByteSharedMemory, stream>>>(bh);
