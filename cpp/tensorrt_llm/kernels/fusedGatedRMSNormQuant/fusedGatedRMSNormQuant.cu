 ...
 *
 * Uses standard fp32_vec_to_e2m1() from quantization.cuh for the PTX conversion.
 */
...
#pragma unroll
    ...
    // Convert to e2m1 using standard library function (from quantization.cuh)
...
#pragma unroll
    for (int i = 0; i < 4; i++)
    {
        ...
        gatedVals[i * 2] = fast_gated_silu(xf2.x, zf2.x);
        gatedVals[i * 2 + 1] = fast_gated_silu(xf2.y, zf2.y);
...
#pragma unroll
    ...
    // Pre-compute rstd * weight to reduce multiplies in the loop
    // ================================================================
...
#pragma unroll
    for (int i = 0; i < 4; i++)
    {
        ...
        // Pre-multiply rstd * weight, then single multiply with gated value
        float rstd_w0 = rstd * wf2.x;
        float rstd_w1 = rstd * wf2.y;
        gatedVals[i * 2] *= rstd_w0;
        gatedVals[i * 2 + 1] *= rstd_w1;
    }
...
void invokeFusedGatedRMSNormQuant(FusedGatedRMSNormQuantParams<T> const& params, int multiProcessorCount)
{
    TLLM_CHECK_WITH_INFO(
        params.N % params.groupSize == 0, "N (%d) must be divisible by groupSize (%d)", params.N, params.groupSize);
    TLLM_CHECK_WITH_INFO(params.groupSize % ELTS_PER_THREAD == 0,
        "groupSize (%d) must be divisible by ELTS_PER_THREAD (%d)", params.groupSize, ELTS_PER_THREAD);
