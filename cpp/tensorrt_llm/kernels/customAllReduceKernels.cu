...
__inline__ __device__ void update_barrier_flag(
    uint32_t* barrier_flag_ptr, uint32_t* barrier_flag_counter_ptr, bool is_grid_synced = false)
{
    if (!is_grid_synced)
    {
        if (thread0())
...
#endif
    // grid.sync();
    update_barrier_flag(params.barrier_flag_ptr, params.barrier_flag_counter_ptr, false);
}
    ...
    AllReduceParams& params, cudaStream_t stream)
{
    ...
    {
        TLLM_CUDA_CHECK(
            cudaLaunchKernelEx(&kernelConfig, oneShotAllReduceKernel<T, RANKS_PER_NODE, PUSH_MODE>, params));
    }
    else
    {
        TLLM_CUDA_CHECK(
            cudaLaunchKernelEx(&kernelConfig, twoShotAllReduceKernel<T, RANKS_PER_NODE, PUSH_MODE>, params));
    }
}
