...
#pragma unroll
        for (int i = 0; i < VEC_SIZE / 4; i++) {
            ...
            thread_sum_gate = __ffma2_rn(make_float2(a_val.x, a_val.y), make_float2(b_val_gate.x, b_val_gate.y), thread_sum_gate);
            thread_sum_gate = __ffma2_rn(make_float2(a_val.z, a_val.w), make_float2(b_val_gate.z, b_val_gate.w), thread_sum_gate);
            thread_sum_linear = __ffma2_rn(make_float2(a_val.x, a_val.y), make_float2(b_val_linear.x, b_val_linear.y), thread_sum_linear);
            thread_sum_linear = __ffma2_rn(make_float2(a_val.z, a_val.w), make_float2(b_val_linear.z, b_val_linear.w), thread_sum_linear);
#else
...
#pragma unroll
        ...
        C[token_idx * hidden_out + row] = __nv_fp8_e4m3(
            silu(block_sum_gate * in_scale_val) * block_sum_linear * in_scale_val * out_scale_inv[0]);
    }
    ...
    void* __restrict__ C, void const* in_scale, void const* out_scale_inv, cudaStream_t stream)
{
    ...
    void* args[] = {(void*)&num_tokens, (void*)&hidden_out, (void*)&A, (void*)&B, (void*)&C, (void*)&in_scale, (void*)&out_scale_inv};
