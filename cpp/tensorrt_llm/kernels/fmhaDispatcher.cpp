    ...
    , mUseTllmGen(tensorrt_llm::common::getSMVersion() == 100)
{
    if (mUseTllmGen)
    {
        // TODO: support mixed data types.
        mTllmGenFMHARunner.reset(
            new TllmGenFmhaRunner(mFixedParams.dataType, mFixedParams.dataType, mFixedParams.dataType));
    }
    else
    {
...
void FmhaDispatcher::run(MHARunnerParams runnerParams)
{
    if (mUseTllmGen)
    {
        ...
        constexpr int kIdxScaleSoftmaxPtr = 0;
        constexpr int kIdxScaleSoftmaxLog2Ptr = 1;
        ...
        tllmRunnerParams.mSumOfSeqLensKv = runnerParams.totalQSeqLen;
