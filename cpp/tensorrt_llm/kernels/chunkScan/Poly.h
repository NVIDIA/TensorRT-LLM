...
public:
    ...
    FT_DEV_CEXPR static bool divisible(Cn<b_>)
    {
        ...
        constexpr auto iBigTerm = indivisibleBigTerm(cn<b_>, cn<sizeof...(Ts_)>);
        constexpr auto iBig = iBigTerm.abs + iBigTerm.inf * cexpr_abs(b_);

        if constexpr (bias_ % b_)
        {
            return dMin == 0 && iMin == 0 && bias_ >= 0 && iMax + bias_ % b_ < cexpr_abs(b_)
                || dMax == 0 && iMax == 0 && bias_ <= 0 && iMin + bias_ % b_ > -cexpr_abs(b_);
        }
        else if constexpr (!std::is_same_v<decltype(iBigTerm), Rn<true>>)
        {
            return dMin == 0 && iMin == 0 && bias_ >= 0
                && (iMax < cexpr_abs(b_) || iMax - iBig < iBigTerm.multiplier && b_ % iBigTerm.multiplier == 0)
                || dMax == 0 && iMax == 0 && bias_ <= 0
                && (iMin > -cexpr_abs(b_) || iMin + iBig > iBigTerm.multiplier && b_ % iBigTerm.multiplier == 0);
        }
...
private:
    ...
    FT_DEV_CEXPR static auto indivisibleBigTerm(Cn<b_>, Cn<i_>)
    {
        if constexpr (i_ == 0)
            return Rn<true>{false};
        else
        {
            constexpr auto prev = indivisibleBigTerm(cn<b_>, cn<i_ - 1>);
            constexpr auto curr = std::tuple_element_t<i_ - 1, Terms>{0};

            if constexpr (curr.multiplier % b_ == 0 || curr.size == 1)
                return prev;
            else if constexpr (std::is_same_v<decltype(prev), Rn<true>>)
                return curr;
            else if constexpr (curr.max - curr.min > prev.max - prev.min || curr.inf)
                return curr;
            else
                return prev;
        }
