...
__device__ inline void ldsm(typename MarlinType<scalar_t>::FragA& frag_a, void const* smem_ptr)
{
...
__device__ inline void scale(
    typename MarlinType<scalar_t>::FragB& frag_b, typename MarlinType<scalar_t>::FragS& frag_s, int i)
{
    using scalar_t2 = typename MarlinType<scalar_t>::scalar_t2;
    scalar_t2 s = MarlinType<scalar_t>::num2num2(reinterpret_cast<scalar_t*>(&frag_s)[i]);
    frag_b[0] = __hmul2(frag_b[0], s);
    frag_b[1] = __hmul2(frag_b[1], s);
}

template <typename scalar_t>
__device__ inline void scale_and_sub(typename MarlinType<scalar_t>::FragB& frag_b, scalar_t s, scalar_t zp)
{
    using scalar_t2 = typename MarlinType<scalar_t>::scalar_t2;
    scalar_t2 s2 = MarlinType<scalar_t>::num2num2(s);
    scalar_t2 zp2 = MarlinType<scalar_t>::num2num2(zp);
...
__device__ inline void sub_zp(
    typename MarlinType<scalar_t>::FragB& frag_b, typename MarlinType<scalar_t>::scalar_t2& frag_zp, int i)
{
    using scalar_t2 = typename MarlinType<scalar_t>::scalar_t2;
    scalar_t2 zp = MarlinType<scalar_t>::num2num2(reinterpret_cast<scalar_t*>(&frag_zp)[i]);
...
template <typename scalar_t>
__device__ inline void scale4(typename MarlinType<scalar_t>::FragB& frag_b,
    typename MarlinType<scalar_t>::FragS& frag_s_1, typename MarlinType<scalar_t>::FragS& frag_s_2,
    typename MarlinType<scalar_t>::FragS& frag_s_3, typename MarlinType<scalar_t>::FragS& frag_s_4, int i)
{
    using scalar_t2 = typename MarlinType<scalar_t>::scalar_t2;
...
template <typename scalar_t>
__device__ inline void scale_float(float* c, typename MarlinType<scalar_t>::FragS& s)
{
    scalar_t* s_ptr = reinterpret_cast<scalar_t*>(&s);
    c[0] = __fmul_rn(c[0], MarlinType<scalar_t>::num2float(s_ptr[0]));
    c[1] = __fmul_rn(c[1], MarlinType<scalar_t>::num2float(s_ptr[1]));
}
...
__global__ void Marlin(MARLIN_KERNEL_PARAMS)
{
    ...
    // NVFP4 kernel: BF16 activations only, no FP8/Turing arch guards needed.
    static_assert(std::is_same<scalar_t, nv_bfloat16>::value, "NVFP4 kernel only supports BF16 compute type");

    int num_tokens_past_padded = num_tokens_past_padded_ptr[0];
    constexpr int moe_block_size = m_block_size_8 ? 8 : (16 * thread_m_blocks);

    constexpr bool use_fp16_accum = false;
    using Dtype = MarlinType<scalar_t>;

    using scalar_t2 = typename MarlinType<scalar_t>::scalar_t2;
    using scalar_32bit_t = typename MarlinType<scalar_t>::scalar_32bit_t;

    using FragA = typename MarlinType<scalar_t>::FragA;
    using FragB = typename MarlinType<scalar_t>::FragB;
    using FragC = typename MarlinType<scalar_t>::FragC;
    using FragS = typename MarlinType<scalar_t>::FragS;
    using FragZP = typename MarlinType<scalar_t>::FragZP;

    extern __shared__ int4 sh[];
    // NVFP4: b_type=FP4_E2M1, s_type=FP8_E4M3, a_type=c_type=BF16
    constexpr bool is_a_8bit = false;        // BF16 activations are 16-bit
    constexpr bool has_zp = false;           // FP4 E2M1 has no zero-points
    constexpr bool is_int_type = false;      // FP4 E2M1 is not int type
    constexpr bool dequant_skip_flop = true; // FP4 E2M1 + FP8 E4M3 scales

    scalar_t2 global_scale;

    constexpr bool has_act_order = group_blocks == 0;

    constexpr int pack_factor = 8; // 32 / 4 bits for FP4 E2M1
    static_assert(thread_m_blocks == 1 || !m_block_size_8);
    int const group_size = (!has_act_order && group_blocks == -1) ? prob_k : prob_k / num_groups;
    int const scales_expert_stride = prob_n * prob_k / group_size / 16;
    int const zp_expert_stride = 0; // No zero-points for NVFP4
...
#pragma unroll
        ...
        if (threadIdx.x < moe_block_size)
        {
            ...
            if (mul_topk_weights)
            {
                ...
                scalar_t2 topk_weight_val = Dtype::num2num2(Dtype::float2num(topk_weights_ptr[idx]));
                if constexpr (true /* FP4 E2M1 + FP8 E4M3 */)
                {
    ...
    constexpr int b_thread_vecs = true /* 4-bit FP4 */ ? 1 : 2;
    ...
    int s_gl_stride = prob_n / 16;
    constexpr int s_sh_stride = 16 * thread_n_blocks / 16;
    ...
    int zp_gl_stride = 0;           // No zero-points for NVFP4
    constexpr int zp_sh_stride = 0; // No zero-points for NVFP4
    ...
    if constexpr (has_zp)
    {
        if constexpr (false /* no zp_float */)
        {
            if constexpr (group_blocks != -1)
            {
        ...
        if constexpr (!has_act_order)
        {
            ...
            else if constexpr (group_blocks != -1)
            {
                ...
                else if (group_blocks2 < b_sh_wr_iters || k % b_sh_wr_iters == 0)
                {
                    ...
                    if constexpr (false /* always FP4 E2M1 */)
                    {
...
#pragma unroll
    ...
    auto fetch_zp_to_registers = [&](int k, int full_pipe)
    {
        ...
        if constexpr (has_zp && !false /* no zp_float */)
        {
            int pipe = full_pipe % stages;
...
#pragma unroll
        ...
        else if constexpr (has_zp && false /* no zp_float */)
        {
            int pipe = full_pipe % stages;
    ...
    {
        if constexpr (true /* 16 != 4 */)
        {
            if constexpr (is_a_8bit && has_zp)
            {
                dequant_fp4<dequant_skip_flop>(q, frag_b_ptr, zp);
            }
            else
            {
                dequant_fp4<dequant_skip_flop>(q, frag_b_ptr);
            }
        }
    };
    ...
    auto matmul = [&](int k, int pipe)
    {
        ...
        if constexpr (!dequant_skip_flop && has_zp && false /* no zp_float */)
        {
            if (is_new_zp)
            {
        ...
        if constexpr (true /* FP4 E2M1 */)
        {
            int s_quant_0 = reinterpret_cast<int*>(frag_s[k2])[0];
            int s_quant_1 = reinterpret_cast<int*>(frag_s[k2])[1];

            dequant_fp8_scales(s_quant_0, reinterpret_cast<scalar_t2*>(&frag_s[k2]));
            dequant_fp8_scales(s_quant_1, reinterpret_cast<scalar_t2*>(&frag_s[k2]) + 2);
        }
...
#pragma unroll
        for (int j = 0; j < 4; j++)
        {
            ...
            else if constexpr (true /* 4-bit FP4 */)
            {
            ...
            if constexpr (dequant_skip_flop && has_zp && !false /* no zp_float */ && !is_a_8bit)
            {
            ...
            else if constexpr (!dequant_skip_flop && has_zp && !false /* no zp_float */ && group_blocks == -1
                && !is_a_8bit)
            {
...
#pragma unroll
        for (int j = 0; j < 2; j++)
        {
            ...
            if (is_a_8bit && true /* 4-bit FP4 */ && !has_zp)
            {
            ...
            else if (is_a_8bit && true /* 4-bit FP4 */ && has_zp)
            {
...
#pragma unroll
            for (int i = 0; i < thread_m_blocks; i++)
            {
                mma<scalar_t>(frag_a[k2][i], frag_b[0], (group_blocks == -1 ? frag_c : frag_c_tmp)[i][j][0]);
                mma<scalar_t>(frag_a[k2][i], frag_b[1], (group_blocks == -1 ? frag_c : frag_c_tmp)[i][j][1]);
            }
...
#pragma unroll
                    ...
                    {
                        float2 s_vals[2];
                        if constexpr (true /* FP8 E4M3, not E8M0 */)
                        {
                            static_assert(true /* BF16 is 16-bit */ || false /* FP8 is 8-bit */);
                            s_vals[0] = Dtype::num22float2(frag_s[k2][j * 2][0]);
                            s_vals[1] = Dtype::num22float2(frag_s[k2][j * 2 + 1][0]);
                        }
                        else
                        {
        ...
        for (int i = 0; i < (m_block_size_8 ? 2 : thread_m_blocks * 4); i++)
        {
            if (!first)
            {
                ...
                if constexpr (is_a_8bit)
                {
                    ...
                    c_red_f16 = reinterpret_cast<scalar_t*>(&tmp);
                }
                else
                {
...
#pragma unroll
                for (int j = 0; j < 2 * (is_a_8bit ? 2 : 4); j++)
                {
                    ...
                    reinterpret_cast<float*>(&frag_c)[(is_a_8bit ? 2 : 4) * 2 * 4 * (i / 4) + 4 * j + (i % 4) + delta]
                        += Dtype::num2float(c_red_f16[j]);
                }
            }
            if (!last)
            {
                scalar_t c_f16[is_a_8bit ? 4 : 8];
#pragma unroll
                for (int j = 0; j < 2 * (is_a_8bit ? 2 : 4); j++)
                {
                    int delta = 0;
                    ...
                    c_f16[j] = Dtype::float2num(reinterpret_cast<float*>(
    ...
    auto write_result = [&](bool last)
    {
        ...
        auto write = [&](int idx, float c0, float c1, FragS& s, FragS& b_bias)
        {
            ...
            if constexpr (!has_act_order && group_blocks == -1 && !is_a_8bit
                && true /* 4-bit FP4 */ && (has_zp && dequant_skip_flop || !has_zp))
            {
...
#pragma unroll
        for (int i = 0; i < stages - 1; i++)
        {
            ...
            if constexpr (has_zp && !false /* no zp_float */ && group_blocks == -1)
            {
                if (i == 0)
                {
...
#pragma unroll
                    for (int i = 3; i >= 0; i--)
                    {
                        frag_c_part_float[i] = Dtype::num2float(frag_c_part_half[i]);
                    }
                }
            }
...
#pragma unroll
            ...
            {
                if (false /* not 8-bit */ || (last || use_atomic_add) || is_a_8bit)
                {
                    if (s_sh_wr_pred)
                    {
                        cp_async4(&sh_s[s_sh_wr], &scales_ptr[s_gl_rd]);
            ...
            if constexpr (!has_act_order && group_blocks == -1 && (has_zp && dequant_skip_flop || !has_zp || is_a_8bit))
            {
                ...
                else if (false /* not 8-bit */ || (last || use_atomic_add))
                {
                    cp_async_wait<0>();
                    __syncthreads();
                    if (threadIdx.x / 32 < tb_n_warps)
...
#pragma unroll
                ...
                {
                    float2 aa[2];
                    aa[0] = Dtype::num22float2(frag_s[0][j * 2][0]);
                    aa[1] = Dtype::num22float2(frag_s[0][j * 2 + 1][0]);
...
#pragma unroll
                ...
                && false /* not 8-bit */ && (has_zp && dequant_skip_flop || !has_zp))
            {
                if (threadIdx.x / 32 < tb_n_warps)
                {
#pragma unroll
