    ...
    int max_shared_mem)
{
    ...
    using c_scalar_t = scalar_t;
    using c_scalar_t2 = scalar_t2;
    ...
    constexpr bool is_a_8bit = false; // BF16 is 16-bit
    constexpr bool has_zp = false;    // NVFP4 has no zero-points
    // For NVFP4: dequant places bits, scale applied separately
...

    {
        ...
        global_scale = MarlinType<scalar_t>::num2num2(*reinterpret_cast<c_scalar_t*>(&val));
    }
    ...
    constexpr int pack_factor = 8; // 32 / 4 (FP4)
    ...
    constexpr int b_thread_vecs = 1; // FP4: 1 vec per thread
    ...
    int s_gl_stride = prob_n / (16 /* NVFP4 scale stride */);
    constexpr int s_sh_stride = 16 * thread_n_blocks / (16 /* NVFP4 scale stride */);
...
#pragma unroll
    ...
    auto matmul = [&](int k, int pipe)
    {
        ...
        // NVFP4: dequant FP8 scales to BF16
        {
            int s_quant_0 = reinterpret_cast<int*>(frag_s[k2])[0];
            int s_quant_1 = reinterpret_cast<int*>(frag_s[k2])[1];

            dequant_fp8_scales(s_quant_0, reinterpret_cast<c_scalar_t2*>(&frag_s[k2]));
            dequant_fp8_scales(s_quant_1, reinterpret_cast<c_scalar_t2*>(&frag_s[k2]) + 2);
        }
...
#pragma unroll
        for (int j = 0; j < 4; j++)
        {
            ...
            // NVFP4 (FE2M1f): shift to extract two halves
            ...
            else if constexpr (!dequant_skip_flop && has_zp && group_blocks == -1 && !is_a_8bit)
            {
                ...
                scalar_t2 s2
                    = MarlinType<scalar_t>::nums2num2(reinterpret_cast<scalar_t*>(&frag_s[j / 2][j % 2 * 2 + 0])[idx],
...
#pragma unroll
    ...
    auto global_reduce_fp16 = [&](bool first = false, bool last = false)
    {
        ...
        if (threadIdx.x < active_threads)
        {
            ...
            {
                c_gl_wr = c_gl_stride * ((threadIdx.x % 32) / 4) * (is_a_8bit ? 2 : 1) + 4 * (threadIdx.x / 32)
                    + threadIdx.x % 4;
...
#pragma unroll
                for (int i = 0; i < (m_block_size_8 ? 2 : thread_m_blocks * 4); i++)
                {
                    ...
                    else if constexpr (is_a_8bit)
                    {
                        ...
                        cp_async2_ca_pred(&sh_red_int2[c_sh_wr + c_sh_wr_delta * i],
...
#pragma unroll
                        for (int j = 0; j < 2 * (is_a_8bit ? 2 : 4); j++)
                        {
                            ...
                            reinterpret_cast<float*>(
                                ...
                                += MarlinType<scalar_t>::num2float(c_red_f16[j]);
                        }
                    }
                    if (!last)
                    {
...
#pragma unroll
                        for (int j = 0; j < 2 * (is_a_8bit ? 2 : 4); j++)
                        {
                            ...
                            c_f16[j] = MarlinType<scalar_t>::float2num(reinterpret_cast<float*>(
                                &frag_c)[(is_a_8bit ? 2 : 4) * 2 * 4 * (i / 4) + 4 * j + (i % 4) + delta]);
                        }
                        if constexpr (m_block_size_8)
                        {
...
#pragma unroll
    ...
    auto write_result = [&](bool last)
    {
        ...
        {
            c_scalar_t2 res = MarlinType<scalar_t>::nums2num2(
                MarlinType<scalar_t>::float2num(c0), MarlinType<scalar_t>::float2num(c1));
                ...
                && (has_zp && dequant_skip_flop || !has_zp))
            {
                ...
                {
                    tmp_scale
                        = MarlinType<scalar_t>::num2num2(reinterpret_cast<scalar_t*>(&s[0])[(threadIdx.x % 8) / 4]);
                }
            ...
            if (has_bias && last)
            {
                ...
                if constexpr (m_block_size_8)
                {
                    tmp_bias = MarlinType<scalar_t>::num2num2(
...
#pragma unroll
                    for (int i = 3; i >= 0; i--)
                    {
                        frag_c_part_float[i] = MarlinType<scalar_t>::num2float(frag_c_part_half[i]);
                    }
                }
            }
...
#pragma unroll
                            for (int i = 0; i < 8; i++)
                            {
                                frag_s_half2[i] = MarlinType<scalar_t>::num2num2(
...
#pragma unroll
                ...
                {
                    float2 aa[2];
                    aa[0] = MarlinType<scalar_t>::num22float2(frag_s[0][j * 2][0]);
                    aa[1] = MarlinType<scalar_t>::num22float2(frag_s[0][j * 2 + 1][0]);
