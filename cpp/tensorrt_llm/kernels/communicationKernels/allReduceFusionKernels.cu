    ...
    __device__ __forceinline__ void operator()(float4 val, int token_id)
    {
        ...
        if constexpr (GetQuantType<Pattern> == QuantType::kFP4)
        {
            ...
            auto sf_out = cvt_quant_to_fp4_get_sf_out_offset<uint32_t, 2>(std::nullopt, token_id, m_access_id_in_token,
                std::nullopt, m_params.hidden_dim, reinterpret_cast<uint32_t*>(m_params.scale_out));
...
#define DISPATCH_PATTERN(DType, NRanks)                                                                                \
    ...
    else if (params.pattern == AllReduceFusionPattern::kARResidualRMSNormFP8QuantV2)                                   \
    {                                                                                                                  \
        DISPATCH_ACC_TYPE(DType, AllReduceFusionPattern::kARResidualRMSNormFP8QuantV2, NRanks);                        \
    }                                                                                                                  \
    else if (params.pattern == AllReduceFusionPattern::kARResidualRMSNormFP4QuantV2)                                   \
    {                                                                                                                  \
        if constexpr (!std::is_same_v<DType, float>)                                                                   \
        {                                                                                                              \
            DISPATCH_ACC_TYPE(DType, AllReduceFusionPattern::kARResidualRMSNormFP4QuantV2, NRanks);                    \
        }                                                                                                              \
        else                                                                                                           \
        {                                                                                                              \
            TLLM_CHECK_WITH_INFO(false,                                                                                \
                "allreduce_fusion_kernel: AllReduceFusionPattern=kARResidualRMSNormFP4QuantV2 can not work with "      \
                "DType=float!");                                                                                       \
        }                                                                                                              \
    }                                                                                                                  \
    else                                                                                                               \
