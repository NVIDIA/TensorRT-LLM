...
//! Adapt a uniform group of KVCacheManagerV2 layer pools to the batched
//! compaction kernels (double-buffered cp.async pipelined bf16 copies, one
//! CTA per layer/KV-head/request). Device pointer arrays allow one layered
//! launch. Within each request and head, source ordinals must increase
//! strictly and satisfy destinationBases[request] + move <= source[move];
//! the per-request bases let one launch cover a cohort with mixed
//! pinned-prompt lengths. sourceHeadStride is the head-row stride of
//! sparseKvIndices: the index buffers may be wider than one round's total
//! move count.
template <typename T>
