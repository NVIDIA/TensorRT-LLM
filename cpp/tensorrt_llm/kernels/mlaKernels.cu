...
__global__ void applyMLARopeAndAssignQKVKernelGeneration(T* qkv_output, T* q_pe, T const* fuse_buf,
    void* quant_qkv_output, KVCacheBuffer kv_cache, float2 const* cos_sin_cache, size_t head_num, int c_k,
    ...
    if (blockIdx.x == 0 && blockIdx.y == 0 && threadIdx.x == 0)
    {
        ...
        if (cache_type == KvCacheDataType::FP8)
        {
            ...
            if (bmm1_scale)
            {
                ...
                bmm1_scale[0] = dequant_scale_q_val * dequant_scale_kv_val * host_bmm1_scale * kLog2e;
            }
            if (bmm2_scale)
            {
             ...
             global_token_idx < seq_len_loop_end; global_token_idx += TOKENS_PER_BLOCK * gridDim.x)
        {
            ...
            if (valid_token)
            {
                if (head_idx == head_num)
                {
                    ...
                    {
                        auto kDst = reinterpret_cast<T*>(kv_cache.getVBlockPtr(batch_idx, token_kv_idx));
                        auto inBlockIdx = kv_cache.getKVLocalIdx(
                            token_kv_idx, 0, TOTAL_VEC_PER_HEAD, K_VECS_PER_HEAD + head_dim_vec_idx);
                        if (cache_type == KvCacheDataType::FP8)
                        {
        ...
        if (cache_type == KvCacheDataType::FP8)
        {
                 ...
                 global_token_idx += (block_dim / head_num) * gridDim.x * K_TOKENS_PER_BLOCK)
            {
                if (global_token_idx < total_s_len)
                {
                    ...
                    quantCopy<T, ELTS_PER_VEC>(reinterpret_cast<__nv_fp8_e4m3*>(quant_qkv_output) + load_idx,
                        qkv_output + load_idx, quant_scale_q_val);
                }
            }
        }
    }
