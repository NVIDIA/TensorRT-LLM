    ...
    int multiprocessor_count, cudaStream_t const& stream)
{
    ...
    if (!xqaUseInputKV)
    {
        ...
        QKVPreprocessingParams<T, KVCacheBuffer> preprocessingParms{static_cast<T*>(const_cast<void*>(xqaParams.qkv)),
            ...
            static_cast<T const*>(xqaParams.qkv_bias), xqaParams.logn_scaling_ptr, launchParams.tokens_info,
            ...
            xqaParams.spec_decoding_position_offsets, (float2 const*) nullptr, xqaParams.mrope_position_deltas,
            int(batch_beam_size), xqaParams.generation_input_length, xqaParams.max_past_kv_length,
            xqaParams.cyclic_attention_window_size, xqaParams.sink_token_length, xqaParams.total_num_input_tokens,
