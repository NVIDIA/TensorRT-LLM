    ...
    int32_t numKvHeads, int32_t numBlocks, int32_t totalQueries, int32_t initBlocks, int32_t localBlocks)
{
    ...
    // Wait on the index-score kernel before the first dependent load.
...
// Every variant is launched with PDL so it can start into the tail of the
// index-score kernel that produces `scores`; each waits internally before its
// first dependent load.
template <typename KernelT, typename... ArgsT>
void launchSelectorWithPdl(
    KernelT kernel, int32_t gridSize, int32_t blockSize, cudaStream_t stream, ArgsT&&... args)
{
