    ...
    cudaStream_t stream)
{
    ...
    else
    {
        ...
        bool using_fused_finalize = use_fused_finalize_ && gemm2_config_->sm_version >= 90 && !use_w4afp8 && !use_lora;
...
#undef GET_WS_PTR
    ...
    for (int64_t i = 0; i < NUM_ROUTING_SAMPLES; i++)
    {
        ...
        if (mSM >= 90)
        {
            ...
            bool using_fused_finalize = mInterface->use_fused_finalize_ && mSM >= 90 && !mMinLatencyMode && !use_w4afp8;
            if (using_fused_finalize)
            {
