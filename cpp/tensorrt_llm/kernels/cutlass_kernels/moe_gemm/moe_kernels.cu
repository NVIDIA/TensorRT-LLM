...
__device__ void setupFP4BlockScalingFactors(LayoutInfo& layout_info, int expert, int gemm_m, int gemm_n, int gemm_k,
    ...
    TmaWarpSpecializedGroupedGemmInput::ElementSF const* weight_block_scale, int64_t num_tokens)
{
    ...
    // We use `expanded_num_tokens` for a consistent stride
    layout_info.fp4_block_scaling_factors_A[expert] = fp4_act_flat + getOffsetFlatSFArray(expert, num_tokens, gemm_k);

    layout_info.fp4_block_scaling_factors_B[expert] = weight_block_scale + getOffsetFlatSFArray(expert, gemm_n, gemm_k);
}
    ...
    ScaleBiasType const* bias1, ScaleBiasType const* bias2, OutputType* gemm1_output, OutputType* gemm2_output)
{
    ...
    if (quant_params.fp4.fc1.weight_block_scale)
    {
        setupFP4BlockScalingFactors(layout_info1, expert, gemm_m, gemm1_n, gemm1_k, fp4_act_flat1,
            quant_params.fp4.fc1.weight_block_scale, num_tokens);
    }
    ...
    if (quant_params.fp4.fc2.weight_block_scale)
    {
        setupFP4BlockScalingFactors(layout_info2, expert, gemm_m, gemm2_n, gemm2_k, fp4_act_flat2,
            quant_params.fp4.fc2.weight_block_scale, num_tokens);
    }
    ...
    if (quant_params.fp4.fc1.weight_block_scale)
    {
        ...
        if (is_active_expert)
        {
            ...
            layout_info1.fp4_block_scaling_factors_B[expert]
                = quant_params.fp4.fc1.weight_block_scale + getOffsetFlatSFArray(local_expert, gemm1_n, gemm1_k);
        }
        else
        {
    ...
    if (quant_params.fp4.fc2.weight_block_scale)
    {
        ...
        if (is_active_expert)
        {
            layout_info2.fp4_block_scaling_factors_B[expert]
                = quant_params.fp4.fc2.weight_block_scale + getOffsetFlatSFArray(local_expert, gemm2_n, gemm2_k);
        }
        else
        {
...
#define ADD(name) ADD_NAME(name, name##_size)
    ...
    ADD_NAME(fp4_act_scale, fp4_act_scale_size);
    ...
    bool use_fp8_block_scaling, bool min_latency_mode, bool use_awq)
{
    ...
    alpha_scale_ptr_array_fc1_ = getWsPtr((float const*){}, "alpha_scale_ptr_array_fc1");
    alpha_scale_ptr_array_fc2_ = getWsPtr((float const*){}, "alpha_scale_ptr_array_fc2");
...
T const* CutlassMoeFCRunner<T, WeightType, OutputType, InputType, ScaleBiasType, Enable>::applyPrequantScale(
    void* smoothed_act, void const* permuted_data, void const* prequant_scales,
    int const* permuted_token_selected_experts, int64_t const* num_valid_tokens_ptr, int64_t const expanded_num_rows,
    int64_t const seq_len, bool const use_awq, cudaStream_t stream)
{
    T const* gemm_input;
    ...
    if (use_prequant_scale_kernel)
    {
        ...
        {
            tensorrt_llm::kernels::apply_per_expert_scale_kernel_launcher<UnfusedGemmOutputType, T>(
                reinterpret_cast<T*>(smoothed_act), reinterpret_cast<UnfusedGemmOutputType const*>(permuted_data),
                reinterpret_cast<UnfusedGemmOutputType const*>(prequant_scales), permuted_token_selected_experts,
                num_valid_tokens_ptr, expanded_num_rows, seq_len, stream);
        }
    ...
    if (using_tma_ws_gemm1)
    {
        ...
        if (fc2_fp4_act_flat)
        {
            check_cuda_error(cudaMemsetAsync(
                fc2_fp4_act_flat, 0x0, getOffsetFlatSFArray(num_experts_per_node, num_rows, inter_size), stream));
        }
    ...
    cudaStream_t stream)
{
    ...
    if (min_latency_mode)
    {
        ...
        auto gemm2_input = applyPrequantScale(smoothed_act_, fc1_result_, quant_params.groupwise.fc2.act_scales,
            nullptr, num_valid_tokens_ptr, expanded_num_rows, inter_size, use_awq, stream);
    ...
    else
    {
        ...
        auto gemm1_input = applyPrequantScale(smoothed_act_, permuted_data_, quant_params.groupwise.fc1.act_scales,
            permuted_token_selected_experts_, num_valid_tokens_ptr, expanded_num_rows, hidden_size, use_awq, stream);
        sync_check_cuda_error(stream);
        ...
        auto gemm2_input = applyPrequantScale(smoothed_act_, fc1_result_, quant_params.groupwise.fc2.act_scales,
            permuted_token_selected_experts_, num_valid_tokens_ptr, expanded_num_rows, inter_size, use_awq, stream);
        sync_check_cuda_error(stream);
    ...
    int maxM, bool is_tma_ws_input)
{
    ...
    size_t weights_size = mGemmToProfile == GemmToProfile::GEMM_1 ? weights_1 : weights_2;
    ...
    quant_2_size = is_fp4_w_quant ? getOffsetFlatSFArray(num_experts_per_node, inter_size, hidden_size)
            * sizeof(TmaWarpSpecializedGroupedGemmInput::ElementSF)
                                  : quant_2_size;
    ...
    size_t quant_5_size = is_fp4_w_quant ? getOffsetFlatSFArray(num_experts_per_node, hidden_size, inter_size)
            * sizeof(TmaWarpSpecializedGroupedGemmInput::ElementSF)
                                         : 0;
    ...
    size_t const fc1_fp4_act_scale_size = is_fp4_act_quant
        ? getOffsetFlatSFArray(num_experts_per_node, maxM, hidden_size)
            * sizeof(TmaWarpSpecializedGroupedGemmInput::ElementSF)
        : 0;
    size_t const fc2_fp4_act_scale_size = is_fp4_act_quant
        ? getOffsetFlatSFArray(num_experts_per_node, maxM, inter_size)
            * sizeof(TmaWarpSpecializedGroupedGemmInput::ElementSF)
        : 0;
...
void GemmProfilerBackend::prepareTmaWsInputs(int num_tokens, char* workspace_ptr_char, cudaStream_t stream)
{
    if (mSM < 90)
    {
        return;
        ...
        if (mSM >= 90)
        {
            ...
            if (mMinLatencyMode)
            {
                std::tie(gemm1_tma_ws_input, gemm2_tma_ws_input)
                    = mInterface->computeStridesTmaWarpSpecializedLowLatencyDispatch(gemm1_tma_ws_input,
                        ...
                        mExpertInterSize, mNumExpertsPerNode, input, input, weights, weights,
            ...
            else
            {
                std::tie(gemm1_tma_ws_input, gemm2_tma_ws_input) = mInterface->computeStridesTmaWarpSpecializedDispatch(
                    ...
                    input, weights, weights, mQuantParams.fp8.dequant_fc1, mQuantParams.fp8.dequant_fc2,
...
void GemmProfilerBackend::prepare(int num_tokens, char* workspace_ptr_char, cudaStream_t stream)
{
    ...
    prepareTmaWsInputs(num_tokens, workspace_ptr_char, stream);
}
