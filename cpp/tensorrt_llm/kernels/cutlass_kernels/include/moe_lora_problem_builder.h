...
struct MoeLoraGemmGroupArrays
{
    ...
    // scratch size in fp32 elements -- matches the layout
    // cuda_graph_split_k_grouped_gemm consumes.
    int64_t* splitk_offsets = nullptr;                        // [P + 1]
};

// Phase 6b.C.1: device-side problem-and-pointer builder for one MoE LoRA
// module. Consumes the per-permuted-row outputs of
// `launchMoeLoraPointerExpand` (6b.B) plus uniform input / workspace /
// output base addresses, and writes every device-resident input the
// cuda_graph_(split_k_)grouped_gemm wrappers need.
//
// Inputs:
//   ranks_dev:    int32 [P]    per-permuted-row LoRA rank
//   ptrs_dev:     int64 [P*2]  per-permuted-row (A_ptr+offset, B_ptr+offset)
//                              -- raw pointer bits already adjusted for the
//                              per-expert weight stride by the pointer-
//                              expand kernel.
//
// Base pointers (the per-token row offset is computed inside the kernel
// from `i * stride * dtype_bytes`):
//   input_base:        [P, in_hidden_size]
//   lowrank_workspace: [P, max_lora_rank]    -- shared between the in-GEMM
//                                              output and the out-GEMM input
//   output_base:       [P, out_hidden_size]
//
// Scalars:
//   in_hidden_size      K for in-GEMM, also lda_in[i] and ldb_in[i]
//   out_hidden_size     N for out-GEMM, also ldb_out[i] and ldd_out[i]
//   max_lora_rank       ldd_in[i] (workspace stride, fixed regardless of
//                       per-token rank so the GEMM lands at a known offset)
//   dtype_bytes         scalar size in bytes (2 for bf16/fp16; 4 for fp32)
//   splitk_slices       split-K factor used by the in-GEMM; drives the
//                       per-problem split-K scratch stride
//
// The current implementation uses a *worst-case* fixed split-K stride
// (`max_lora_rank * splitk_slices` per problem) so the offsets can be
// computed from `i` alone without a prefix-sum. Switching to an exact
// scan (matching the attention path's BlockScan) is a perf-only change
// landed in 6b.E if profiling motivates it.
