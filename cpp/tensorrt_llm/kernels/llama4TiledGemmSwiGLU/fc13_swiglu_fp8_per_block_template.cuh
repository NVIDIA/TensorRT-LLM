...
#pragma unroll
                for (int i = 0; i < VEC_SIZE / 4; i++) {
                    ...
                    thread_sum_gate[tile_token_idx][tile_out_idx] = ffma2(make_float2(a_val.x, a_val.y), make_float2(b_val_gate.x, b_val_gate.y), thread_sum_gate[tile_token_idx][tile_out_idx]);
                    thread_sum_gate[tile_token_idx][tile_out_idx] = ffma2(make_float2(a_val.z, a_val.w), make_float2(b_val_gate.z, b_val_gate.w), thread_sum_gate[tile_token_idx][tile_out_idx]);
                    thread_sum_linear[tile_token_idx][tile_out_idx] = ffma2(make_float2(a_val.x, a_val.y), make_float2(b_val_linear.x, b_val_linear.y), thread_sum_linear[tile_token_idx][tile_out_idx]);
                    thread_sum_linear[tile_token_idx][tile_out_idx] = ffma2(make_float2(a_val.z, a_val.w), make_float2(b_val_linear.z, b_val_linear.w), thread_sum_linear[tile_token_idx][tile_out_idx]);
                }
            }
        }
    }
...
#pragma unroll
        for (int tile_token_idx = 0; tile_token_idx < TILE_TOKEN; tile_token_idx++) {
            float warp_sum_gate = thread_sum_gate[tile_token_idx][tile_out_idx].x + thread_sum_gate[tile_token_idx][tile_out_idx].y;
            float warp_sum_linear = thread_sum_linear[tile_token_idx][tile_out_idx].x + thread_sum_linear[tile_token_idx][tile_out_idx].y;
#pragma unroll
...
#pragma unroll
                ...
                C[current_token * hidden_out + current_row] = __nv_fp8_e4m3(silu(block_sum_gate * in_scale_val) * block_sum_linear * in_scale_val * out_scale_inv_val);
            }
        }
    }
