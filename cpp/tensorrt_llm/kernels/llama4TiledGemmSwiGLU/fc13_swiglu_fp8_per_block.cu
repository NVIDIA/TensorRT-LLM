...
void launch_kernel(dim3 grid_dim, dim3 block_dim, cudaStream_t stream, void* kernel_func, void* args[], int num_args)
{
    ...
    int num_tokens, int hidden_in, int hidden_out, int tile_out, cudaStream_t stream)
{
    if (tile_out == 1)
    {
        dispatch_fc13_swiglu_fp8_hidden_in<TILE_TOKEN, 1>(
            A, B, C, in_scale, out_scale_inv, num_tokens, hidden_in, hidden_out, stream);
    }
    else if (tile_out == 2)
    {
        dispatch_fc13_swiglu_fp8_hidden_in<TILE_TOKEN, 2>(
            A, B, C, in_scale, out_scale_inv, num_tokens, hidden_in, hidden_out, stream);
    }
    else if (tile_out == 4)
    {
        dispatch_fc13_swiglu_fp8_hidden_in<TILE_TOKEN, 4>(
            A, B, C, in_scale, out_scale_inv, num_tokens, hidden_in, hidden_out, stream);
    }
    else
    {
    ...
    int num_tokens, int hidden_in, int hidden_out, int tile_token, int tile_out, cudaStream_t stream)
{
    if (tile_token == 1)
    {
        dispatch_fc13_swiglu_fp8_tile_out<1>(
            A, B, C, in_scale, out_scale_inv, num_tokens, hidden_in, hidden_out, tile_out, stream);
    }
    else if (tile_token == 2)
    {
        dispatch_fc13_swiglu_fp8_tile_out<2>(
            A, B, C, in_scale, out_scale_inv, num_tokens, hidden_in, hidden_out, tile_out, stream);
    }
    else if (tile_token == 3)
    {
        dispatch_fc13_swiglu_fp8_tile_out<3>(
            A, B, C, in_scale, out_scale_inv, num_tokens, hidden_in, hidden_out, tile_out, stream);
    }
    else if (tile_token == 4)
    {
        dispatch_fc13_swiglu_fp8_tile_out<4>(
            A, B, C, in_scale, out_scale_inv, num_tokens, hidden_in, hidden_out, tile_out, stream);
    }
    else if (tile_token == 8)
    {
        dispatch_fc13_swiglu_fp8_tile_out<8>(
            A, B, C, in_scale, out_scale_inv, num_tokens, hidden_in, hidden_out, tile_out, stream);
    }
    else
    {
...
void launch_fc13_swiglu_fp8(LaunchParams params, int tile_token, int tile_out)
{
    ...
    dispatch_fc13_swiglu_fp8_tile_token(
        A, B, C, in_scale, out_scale_inv, num_tokens, hidden_in, hidden_out, tile_token, tile_out, stream);
}
