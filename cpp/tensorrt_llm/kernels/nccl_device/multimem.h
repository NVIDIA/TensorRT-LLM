...
template<>
__device__ __forceinline__ HalfVector multimemLoadSum<half, HalfVector>(const half* addr) {
  const uintptr_t multimem_addr = reinterpret_cast<uintptr_t>(addr);
  HalfVector result;
  asm volatile("multimem.ld_reduce.global.add.v4.f16x2.acc::f32 {%0, %1, %2, %3}, [%4];" : "=r"(result.data.x), "=r"(result.data.y), "=r"(result.data.z), "=r"(result.data.w) : "l"(multimem_addr) : "memory");
  return result;
}

template<>
__device__ __forceinline__ BFloat16Vector multimemLoadSum<__nv_bfloat16, BFloat16Vector>(const __nv_bfloat16* addr) {
  const uintptr_t multimem_addr = reinterpret_cast<uintptr_t>(addr);
  BFloat16Vector result;
  asm volatile("multimem.ld_reduce.global.add.v4.bf16x2.acc::f32 {%0, %1, %2, %3}, [%4];" : "=r"(result.data.x), "=r"(result.data.y), "=r"(result.data.z), "=r"(result.data.w) : "l"(multimem_addr) : "memory");
  return result;
}
...
template<>
__device__ __forceinline__ FP8E5M2x4Vector multimemLoadSum<__nv_fp8_e5m2, FP8E5M2x4Vector>(const __nv_fp8_e5m2* addr) {
  const uintptr_t multimem_addr = reinterpret_cast<uintptr_t>(addr);
  FP8E5M2x4Vector result;
  asm volatile("multimem.ld_reduce.global.add.v4.e5m2x4.acc::f16 {%0, %1, %2, %3}, [%4];" : "=r"(result.data.x), "=r"(result.data.y), "=r"(result.data.z), "=r"(result.data.w) : "l"(multimem_addr) : "memory");
  return result;
}

template<>
__device__ __forceinline__ FP8E4M3x4Vector multimemLoadSum<__nv_fp8_e4m3, FP8E4M3x4Vector>(const __nv_fp8_e4m3* addr) {
  const uintptr_t multimem_addr = reinterpret_cast<uintptr_t>(addr);
  FP8E4M3x4Vector result;
  asm volatile("multimem.ld_reduce.global.add.v4.e4m3x4.acc::f16 {%0, %1, %2, %3}, [%4];" : "=r"(result.data.x), "=r"(result.data.y), "=r"(result.data.z), "=r"(result.data.w) : "l"(multimem_addr) : "memory");
  return result;
}
...
template<>
__device__ __forceinline__ void multimemStore<half, HalfVector>(half* addr, const HalfVector val) {
  const uintptr_t multimem_addr = reinterpret_cast<uintptr_t>(addr);
  asm volatile("multimem.st.global.v4.f16x2 [%0], {%1,%2,%3,%4};" : : "l"(multimem_addr), "r"(val.data.x), "r"(val.data.y), "r"(val.data.z), "r"(val.data.w) : "memory");
}

#if ARCH_HAS_MULTIMEM_ACC_F32
template<>
__device__ __forceinline__ void multimemStore<__nv_bfloat16, BFloat16Vector>(__nv_bfloat16* addr, const BFloat16Vector val) {
  const uintptr_t multimem_addr = reinterpret_cast<uintptr_t>(addr);
  asm volatile("multimem.st.global.v4.bf16x2 [%0], {%1,%2,%3,%4};" : : "l"(multimem_addr), "r"(val.data.x), "r"(val.data.y), "r"(val.data.z), "r"(val.data.w) : "memory");
}
#endif // ARCH_HAS_MULTIMEM_ACC_F32
...
template<>
__device__ __forceinline__ void multimemStore<__nv_fp8_e5m2, FP8E5M2x4Vector>(__nv_fp8_e5m2* addr, const FP8E5M2x4Vector val) {
  const uintptr_t multimem_addr = reinterpret_cast<uintptr_t>(addr);
  asm volatile("multimem.st.global.v4.e5m2x4 [%0], {%1,%2,%3,%4};" : : "l"(multimem_addr), "r"(val.data.x), "r"(val.data.y), "r"(val.data.z), "r"(val.data.w) : "memory");
}

template<>
__device__ __forceinline__ void multimemStore<__nv_fp8_e4m3, FP8E4M3x4Vector>(__nv_fp8_e4m3* addr, const FP8E4M3x4Vector val) {
  const uintptr_t multimem_addr = reinterpret_cast<uintptr_t>(addr);
  asm volatile("multimem.st.global.v4.e4m3x4 [%0], {%1,%2,%3,%4};" : : "l"(multimem_addr), "r"(val.data.x), "r"(val.data.y), "r"(val.data.z), "r"(val.data.w) : "memory");
}
#endif // ARCH_HAS_MULTIMEM_FP8
#endif // ARCH_HAS_MULTIMEM
