    ...
    int N, int T, int B, float rms_eps)
{
    ...
    constexpr int ACC_PER_THREAD =
        H == 7168 ? 28 : SLICES_PER_GROUP * VEC;
    constexpr int TMEM_V_COLS_PER_GROUP =
        SLICES_PER_GROUP * N_CHUNK * VEC;
    constexpr int TMEM_COLS_TOTAL =
        CONSUMER_GROUPS * TMEM_V_COLS_PER_GROUP;
    ...
    const uint32_t my_v_tmem = (comp_tid >= 0)
        ? (plan.tmem_base + group * TMEM_V_COLS_PER_GROUP)
        : 0;
            ...
            if constexpr (H == 7168) {
                if (si == SLICES_PER_GROUP - 1) {
                    ...
                    for (int j = 0; j < 4; j++) {
                        int h = h_base + j;
                        q_cache[si * VEC + j] =
                            __bfloat162float(rms_w[h]) *
                            __bfloat162float(res_w[h]);
                    }
                    continue;
                }
            }
    ...
    if (wid == 0) {
        if (elect_one_sync()) {
            ...
            for (int tb = blockIdx.x; tb < TB; tb += num_ctas) {
                for (int ci = 0; ci < num_chunks; ci++, gci++) {
                    ...
                    int pc = phase_of(gci);
                    cute::wait_barrier(
                        plan.bar_consumed[chunk_slot], pc ^ 1);
                    cute::set_barrier_transaction_bytes(
                        plan.bar_ready[chunk_slot],
                        an * H * (int)sizeof(bf16_t));
                    #pragma unroll
                    for (int n = 0; n < N_CHUNK; n++) {
                        ...
                        cp_async_bulk(
                            buf_ptr(slot), src, H * sizeof(bf16_t),
                            plan.bar_ready[chunk_slot]);
                    }
                }
            }
        ...
        for (int tb = blockIdx.x; tb < TB; tb += num_ctas) {
            ...
            for (int ci = 0; ci < num_chunks; ci++, gci++) {
                    ...
                    for (int si = 0; si < SLICES_PER_GROUP; si++) {
                            ...
                            if (si == SLICES_PER_GROUP - 1) {
                                ...
                                for (int n = 0; n < AN; n++) {
                                    ...
                                    int2 vp = *reinterpret_cast<const int2*>(
                                        buf_ptr(slot) + h_base);
                                    __nv_bfloat162* v2 =
                                        reinterpret_cast<__nv_bfloat162*>(&vp);
                                    float2 f[2] = {
                                        __bfloat1622float2(v2[0]),
                                        __bfloat1622float2(v2[1])};
                                    if constexpr (FULL_N12) {
                                        if (n == AN - 1 && lane == 0) {
                                            cute::arrive_barrier(
                                                plan.bar_consumed[chunk_slot]);
                                        }
                                    }
                                    tmem_st_32dp32bNx<4>(
                                        my_v_tmem +
                                    ...
                                    sq_local[n] =
                                        float2_fma(f[0], f[0], sq_local[n]);
                                    sq_local[n] =
                                        float2_fma(f[1], f[1], sq_local[n]);
                                    dot_local[n] = float2_fma(
                                        f[0], make_float2(qv[0], qv[1]),
                                        dot_local[n]);
                                    dot_local[n] = float2_fma(
                                        f[1], make_float2(qv[2], qv[3]),
                                        dot_local[n]);
                                }
                                continue;
                            }
                        ...
                        for (int n = 0; n < AN; n++) {
                            ...
                            float2 f[4] = {
                                __bfloat1622float2(v2[0]),
                                __bfloat1622float2(v2[1]),
                                __bfloat1622float2(v2[2]),
                                __bfloat1622float2(v2[3])};
                            tmem_st_32dp32bNx<VEC>(
                                my_v_tmem +
                                    (si * N_CHUNK + n) * VEC,
                                reinterpret_cast<float*>(f));
                            ...
                            dot_local[n] = float2_fma(
                                f[0], make_float2(qv[0], qv[1]), dot_local[n]);
                            dot_local[n] = float2_fma(
                                f[1], make_float2(qv[2], qv[3]), dot_local[n]);
                            dot_local[n] = float2_fma(
                                f[2], make_float2(qv[4], qv[5]), dot_local[n]);
                            dot_local[n] = float2_fma(
                                f[3], make_float2(qv[6], qv[7]), dot_local[n]);
                        }
                    }
                    if constexpr (!FULL_N12) {
                ...
                #pragma unroll
                for (int n = 0; n < N_CHUNK; n++) {
                    reduce_pair[n] = make_float2(
                        sq_local[n].x + sq_local[n].y,
                ...
                for (int offset = 16; offset > 0; offset >>= 1) {
                    ...
                    for (int n = 0; n < N_CHUNK; n++) {
                        uint64_t packed =
                            reinterpret_cast<uint64_t&>(reduce_pair[n]);
                        packed = __shfl_xor_sync(
                            0xffffffff, packed, offset);
                        float2 other = reinterpret_cast<float2&>(packed);
                        reduce_pair[n] =
                            float2_add(reduce_pair[n], other);
                    }
                }
                if constexpr (FULL_N12) {
                    cutlass::arch::fence_view_async_tmem_store();
                ...
                #pragma unroll
                for (int n = 0; n < N_CHUNK; n++) {
                    logit_n[n] = __shfl_sync(
                        0xffffffff, local_logit, n);
                }
                ...
                if constexpr (FULL_N12) {
                    ...
                    for (int n = 0; n < N_CHUNK; n++) {
                        w_n[n] = exp2f(
                            (logit_n[n] - m_new) * LOG2_E);
                    }
                    w_sum =
                        (w_n[0] + w_n[1]) + (w_n[2] + w_n[3]);
                } else {
                    #pragma unroll
                    for (int n = 0; n < N_CHUNK; n++) {
                        if (n < an) {
                            ...
                            if (si == SLICES_PER_GROUP - 1) {
                                ...
                                for (int n = 0; n < AN; n++) {
                                    tmem_ld_32dp32bNx<4>(
                                        my_v_tmem +
                                            (si * N_CHUNK + n) * VEC,
                                        reinterpret_cast<float*>(f_cache[n]));
                                }
                                #pragma unroll
                                for (int n = 0; n < AN; n++) {
                                    float2 wn =
                ...
                if constexpr (FULL_N12) {
                    pass_B_body(
                        std::integral_constant<int, N_CHUNK>{});
                } else if constexpr (NC == 4) {
                    switch (an) {
    ...
    if (wid == 0) {
        ...
        for (int tb0 = blockIdx.x * TB_TILE; tb0 < TB;
            ...
            for (int r = 0; r < TB_TILE; r++) {
                ...
                if (lane == 0) {
                    plan.ws_stats[comp_wid][0] =
                        make_float2(sq_local, dot_local);
                }
                cutlass::arch::NamedBarrier::sync(CONSUMER_THREADS, 0);
    ...
    float rms_eps)
{
    ...
    for (int item = 0; item < ITEMS; item++) {
        ...
        float q = __bfloat162float(res_w[h]) *
                  __bfloat162float(rms_w[h]);
        bf16_t item_v[N];
        #pragma unroll
        for (int n = 0; n < N; n++) {
            ...
            stats[n] = float2_fma(
                make_float2(v, v), make_float2(v, q), stats[n]);
        }
        if constexpr (N > 1) {
            #pragma unroll
    ...
    float rms_eps)
{
    ...
    float2* warp_stats = reinterpret_cast<float2*>(
        smem_raw + (size_t)N * K_PER_CTA * sizeof(float));
