    ...
    for (int l = 0; l < loop_on_s; l++)
    {
        ...
        if (s_ < s_len)
        {
            ...
            if (cur_softmax_sum == 0)
                cur_softmax_sum = 1.0;
            /*if (hid == 0 && sid == 0)
            printf("for bid %d, hid %d, sid %d, accu_ss %f, ss %f , cur_ss %f, accu_max %f, max %f, cur_max %f oa_scale
            1 %f, oa_scale 2 %f\n", bid, hid, sid, my_accu_ss, my_ss, cur_softmax_sum, accu_max[lm_start_offset],
            max[lm_start_offset], cur_max, scale1 * my_accu_ss, scale2 * my_ss);*/
        ...
        for (int ss = 0;; ss++)
        {
            ...
            {
                // float old_accu_output = (float)accu_output[output_start_offset + eid];
                ...
                /*if (rank == 0 && sid == 0 && hid == 0 && eid < 4)
                    printf("for the e id is %d, old_accu_output %f, accu_output %f, output %f, p1 is %f, p2 is %f\n",
                        eid, old_accu_output, (float)accu_output[output_start_offset + eid],
                   (float)output[output_start_offset + eid], old_accu_output * scaled_my_ss1,
                   (float)output[output_start_offset + eid] * scaled_my_ss2);*/
            }
            sid += warp_num;
            if (sid >= s_end)
                break;
