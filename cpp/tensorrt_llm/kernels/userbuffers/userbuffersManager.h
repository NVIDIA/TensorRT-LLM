...
public:
    ...
    std::pair<std::unique_ptr<void, UserBufferDeleter>, tensorrt_llm::runtime::ub::UBBuffer> allocate_userbuffers(
        int64_t buffer_size);
...
private:
    std::mutex mutex_;
    std::vector<std::pair<tensorrt_llm::runtime::ub::UBBuffer, bool>> buffers_;
