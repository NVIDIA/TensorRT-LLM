...
void AgentConnection::sendRequestAndBufferInfo(batch_manager::RequestInfo& requestInfo,
    ...
    std::atomic<bool> const* perRequestCancel, bool* advertisementMayHaveOccurred)
{
    ...
    if (advertisementMayHaveOccurred != nullptr)
    {
        // Once notification begins, an exception cannot prove that the peer did not receive the advertised buffers.
        *advertisementMayHaveOccurred = true;
    }
    mAgentConnectionManager->getAgent()->notifySyncMessage(mRemoteAgentName, ss.str());
