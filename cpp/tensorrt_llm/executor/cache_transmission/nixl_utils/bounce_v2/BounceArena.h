...
//   shared by BOTH roles. The Python scheduler (its buddy allocator) carves variable-size regions
//   out of it by byte offset — receiver grants (remote senders' RDMA-write targets) and the local
//   sender's gather staging both draw from the same arena. Each chunk requests only its packed
//   extent, so small requests use little space while requests larger than the arena stream through
//   chunk by chunk.
//
// Allocation
...
//
// Threading
//   base()/baseAddr()/at() are const lookups into an immutable buffer; safe from any thread
//   concurrently. WHICH region a role may touch is arbitrated by the Python scheduler (a region is
//   granted/acquired to exactly one user until it is freed).
// ============================================================================
class BounceArena
{
public:
