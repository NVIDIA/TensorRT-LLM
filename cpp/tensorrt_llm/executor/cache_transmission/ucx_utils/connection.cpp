...
UcxConnection::UcxConnection(ConnectionIdType connectionId, ConnectionIdType connectionIdInPeer,
    std::shared_ptr<ucxx::Endpoint> endpoint, UcxConnectionManager* manager, bool fromRequester)
    : mConnectionId(connectionId)
    , mConnectionIdInPeer(connectionIdInPeer)
    , mEndpoint(std::move(endpoint))
    , mManager(manager)
    , mFromRequester(fromRequester)
{
    // Connection IDs are exchanged via ZMQ during connection setup in UcxConnectionManager,
    // so no UCX tagSend/tagRecv negotiation is needed here.
