...
compute_dq_dk_dv_1xN_kv_inner_loop_(const Params &params, Prng &ph, int kv_offset) {
    ...
    const int bidkv = blockIdx.x + kv_offset;
    ...
    fmha::Softmax_statistics<Traits, Cta_tile_p> sum_array(
        params, params.softmax_sum_ptr, binfo, tidx);

    // Offsets to store the different tiles in shared memory.
    const int SMEM_OFFSET_dO = 0;
    const int SMEM_OFFSET_Q = SMEM_OFFSET_dO + Smem_tile_do_a::BYTES_PER_TILE;
    const int SMEM_OFFSET_K = SMEM_OFFSET_Q + Smem_tile_q_a ::BYTES_PER_TILE;
    const int SMEM_OFFSET_V = SMEM_OFFSET_K + Smem_tile_kt_b::BYTES_PER_TILE;
    const int SMEM_OFFSET_dQ = SMEM_OFFSET_V + Smem_tile_vt_b::BYTES_PER_TILE;
    const int SMEM_OFFSET_S = SMEM_OFFSET_dQ + Smem_tile_dq ::BYTES_PER_TILE;
    const int SMEM_OFFSET_dP = SMEM_OFFSET_S + Smem_tile_st ::BYTES_PER_TILE;
...
#pragma unroll
        ...
        if( params.has_dropout ) {
            ...
            unsigned long long philox_subsequence =
                step * (binfo.actual_seqlen / 16) + block_col_idx;
...
#pragma unroll
        ...
        fmha::Fragment_accumulator<Traits> acc_dq[Mma_tile_ds_dq::MMAS_M]
                                                 [Mma_tile_ds_dq::VALID_MMAS_N];
...
inline __device__ void compute_dq_dk_dv_1xN_kv_inner_loop(const Params &params,
                                                          int kv_iterations = 1) {
...
template<typename Kernel_traits, typename Params>
inline __device__ void convert_dq_to_16bits(const Params &params) {
    ...
    // The input pointer.
    const float *input_ptr = reinterpret_cast<const float *>(params.dq_acc_ptr);
    ...
    const int quad_lane_id = threadIdx.x % 8;
    const int lane_offset = index_cal(quad_lane_id);
    const int src_lane = lane + lane_offset;
    ...
    for( int ii = tidx * FEATURES_PER_THREAD; ii < features_per_token; ii += features_per_stg ) {
        ...
        float2 f2 = *reinterpret_cast<const float2 *>(input_ptr);
