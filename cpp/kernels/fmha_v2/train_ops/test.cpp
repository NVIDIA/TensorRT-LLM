...
std::array<at::Tensor, 3> fprop_seq(const at::Tensor &Q8,
                                    const at::Tensor &K8,
                                    const at::Tensor &V8t,
                                    const at::Tensor &amask,
                                    const float scale_q_k,
                                    const float d_scale_qkv,
                                    const at::Tensor &amax_exp_p,
                                    const MetaMap &recipe) {
    ...
    float q_scale_exp_p = recipe.at("expP").q_scale(
        amax_exp_p
            .item<float>());  //pow(2, (int) log2f(fmha::MAX_E4M3 / amax_exp_p.item<float>()));
    ...
    for( int step = 1; step < steps; step++ ) {
        ...
        auto Pc = matmul_nt(
            Q8, K8c, recipe.at("QKV").dtype, recipe.at("QKV").dtype, d_scale_q_k * scale_q_k);
        auto Mc = torch::maximum(M, std::get<0>(Pc.max(-1, true)));  // [b,h,s,1]
        Pc =
            Pc.where(amaskc == 1.f, torch::ones_like(Pc) * -std::numeric_limits<float>::infinity());
        auto Tmp0 = Pc - Mc;
        auto ExpP = torch::exp(Tmp0);
...
std::array<at::Tensor, 18> reference(at::Tensor QKV8,
                                     at::Tensor dO8_,
                                     at::Tensor &amask,
                                     const float scale_q_k,
                                     const float d_scale_qkv,
                                     const float d_scale_do,
                                     const MetaMap &recipe) {

    // [b,s,h,d] => [b,h,s,d]
    auto Q8 =
        QKV8.index({ Slice(), Slice(), 0, Slice(), Slice() }).permute({ 0, 2, 1, 3 }).contiguous();
    auto Q8t =
        QKV8.index({ Slice(), Slice(), 0, Slice(), Slice() }).permute({ 0, 2, 3, 1 }).contiguous();
    auto K8 =
        QKV8.index({ Slice(), Slice(), 1, Slice(), Slice() }).permute({ 0, 2, 1, 3 }).contiguous();
    auto K8t =
        QKV8.index({ Slice(), Slice(), 1, Slice(), Slice() }).permute({ 0, 2, 3, 1 }).contiguous();
    auto V8 =
        QKV8.index({ Slice(), Slice(), 2, Slice(), Slice() }).permute({ 0, 2, 1, 3 }).contiguous();
    auto V8t =
        QKV8.index({ Slice(), Slice(), 2, Slice(), Slice() }).permute({ 0, 2, 3, 1 }).contiguous();
    ...
    auto amax_s = S.max();
    float q_scale_exp_p = recipe.at("expP").q_scale(
        amax_exp_p.item<float>());                     // MAX_E4M3 / amax_exp_p.item<float>();
    float q_scale_s =
    ...
    float d_scale_exp_p_v = d_scale_qkv / q_scale_exp_p;
    auto O =
        matmul_nt(ExpP8, V8t, recipe.at("expP").dtype, recipe.at("QKV").dtype, d_scale_exp_p_v);
    O *= Zinv;
    ...
    auto dS =
        matmul_nt(dO8, V8, recipe.at("dO").dtype, recipe.at("QKV").dtype, d_scale_do_v) * amask;
    ...
    // dK = dP' x Q
    auto dK = matmul_nt(dP8.permute({ 0, 1, 3, 2 }).contiguous(),
                        Q8t,
                        recipe.at("dP").dtype,
                        recipe.at("QKV").dtype,
                        d_scale_dp_qkv);
    // dV = S' x dO
    auto dV = matmul_nt(S8.permute({ 0, 1, 3, 2 }).contiguous(),
                        dO8t,
                        recipe.at("S").dtype,
                        recipe.at("dO").dtype,
                        d_scale_s_do);
    ...
    auto amax_dqkv =
        torch::concat({ dQ.abs().max().view(1), dK.abs().max().view(1), dV.abs().max().view(1) })
            .max();
    float q_scale_dqkv =
...
int main(int argc, char **argv) {
    ...
    const int verbose = FLAGS_verbose;

    const bool all_e5m2 = FLAGS_all_e5m2 == 1;
    ...
    const int show_batch = std::min(FLAGS_show_batch, b - 1);
    ...
    FP8DgradMeta dgrad_meta(d_scale_qkv,
                            d_scale_s_ref,
                            d_scale_o_ref,
                            d_scale_do,
                            d_scale_dp_ref,
    ...
    typename dgrad::Launch_params::Params params_(
        ...
        FLAGS_p_dropout,
        FLAGS_scale_q_k,
        dgrad_meta.d_scale_qkv.data_ptr<float>(),
        dgrad_meta.d_scale_s.data_ptr<float>(),
        dgrad_meta.d_scale_o.data_ptr<float>(),
        dgrad_meta.d_scale_do.data_ptr<float>(),
        d_scale_dp.data_ptr<float>(),
        d_scale_dqkv.data_ptr<float>(),
        dgrad_meta.q_scale_s.data_ptr<float>(),
        dgrad_meta.q_scale_dp.data_ptr<float>(),
        dgrad_meta.q_scale_dqkv.data_ptr<float>(),
    ...
    FP8FpropMeta fprop_meta(d_scale_qkv, d_scale_s_ref, q_scale_s, q_scale_o, options);
    {
        ...
        typename fprop::Launch_params::Params params_(
            ...
            FLAGS_p_dropout,
            FLAGS_scale_q_k,
            fprop_meta.d_scale_qkv.data_ptr<float>(),
            d_scale_s.data_ptr<float>(),
            d_scale_o.data_ptr<float>(),
            fprop_meta.q_scale_s.data_ptr<float>(),
        ...
        cout << "O rel: " << ((O32 - O32ref).abs().sum() / O32ref.abs().sum()).item<float>()
             << endl;
    ...
    if( verbose > 1 ) {
        auto print = torch::from_blob(
            print_buf.data_ptr(), { 8 }, { 1 }, torch::Deleter(), options.dtype(torch::kFloat32));
        cout << print << endl;
    }
    return 0;
}
