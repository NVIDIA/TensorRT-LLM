...
at::Tensor convert_fp32_to_e4m3(const at::Tensor &src, float scale) {
...
at::Tensor convert_e4m3_to_fp32(const at::Tensor &src, float scale) {
...
at::Tensor convert_fp32_to_e5m2(const at::Tensor &src, float scale) {
    TORCH_CHECK(src.scalar_type() == torch::kFloat32);
    auto options = src.options();
    auto dst = torch::empty(src.sizes(), options.dtype(torch::kByte));
...
at::Tensor convert_e5m2_to_fp32(const at::Tensor &src, float scale) {
    TORCH_CHECK(src.scalar_type() == torch::kByte);
    auto options = src.options();
    auto dst = torch::empty(src.sizes(), options.dtype(torch::kFloat32));
...
at::Tensor convert_fp32_to_fp8(const at::Tensor &src, float scale, cudaDataType_t dst_type) {
...
at::Tensor convert_fp8_to_fp32(const at::Tensor &src, float scale, cudaDataType_t src_type) {
...
at::Tensor
seqlens2mask(const at::Tensor &cu_seqlens, const int s, const at::TensorOptions &options) {
...
at::Tensor draw_tensor(const at::IntArrayRef dims,
...
int check_results(const at::Tensor &out,
    ...
    return check_results(out_h.data_ptr<float>(),
                         ref_h.data_ptr<float>(),
                         m,
                         n,
                         out_h.stride(0),
                         epsilon,
