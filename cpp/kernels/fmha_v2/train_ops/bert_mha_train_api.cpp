...
// void run_fmha_v2_fp16_128_64_sm80(Launch_params<Fused_multihead_attention_fprop_params> &launch_params, const bool configure);
// void run_fmha_v2_fp16_256_64_sm80(Launch_params<Fused_multihead_attention_fprop_params> &launch_params, const bool configure);
// void run_fmha_v2_fp16_384_64_sm80(Launch_params<Fused_multihead_attention_fprop_params> &launch_params, const bool configure);
// void run_fmha_v2_fp16_512_64_sm80(Launch_params<Fused_multihead_attention_fprop_params> &launch_params, const bool configure);
...
mha_fwd(const at::Tensor &qkv,  // total x num_heads x 3 x head_size, total := \sum_{i=0}^{b} s_i
    ...
    Launch_params<Fused_multihead_attention_fprop_params> launch_params(
        props, stream, is_training, is_causal, is_nl);
    ...
    set_params(launch_params.params,
               ...
               nullptr,         // softmax_sum
               nullptr,         // dq_acc
               random_seeds.data_ptr(),
               p_dropout,
               is_sequences_interleaved,
               is_bf16,
               has_alibi,
               is_causal);
...
std::vector<at::Tensor> mha_bwd_noloop(
    ...
    const float p_dropout,         // probability to drop
    const int max_seq_len,         // max sequence length to choose the kernel
    const bool is_sequences_interleaved,  // layout sequence interleaved ([s, b]) or not
    ...
    fmha_run_noloop_reduce(dqkv.data_ptr(),
                           dkv.data_ptr(),
