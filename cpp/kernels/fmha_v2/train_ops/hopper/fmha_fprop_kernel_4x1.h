...
struct Fprop_4x1 {
    ...
    __device__ inline Fprop_4x1(const Fmha_fprop_params &params,
                                const int bidb,
                                const int bidh,
                                const int tidx,
                                char *smem)
        : params_(params), binfo_(params, bidb, bidh, tidx), tidx_(tidx),
          ...
          zi_ptr_(&reinterpret_cast<float *>(params.zi_ptr)[binfo_.hidx * params.s]), amax_s_(0.f),
          amax_o_(0.f), q_scale_o_(*params_.ptr_q_scale_o) {
    ...
    __device__ inline void operator()(uint64_t seed, uint64_t offset) {
        ...
        // Input tiles.
        uint32_t scale = reinterpret_cast<const uint32_t &>(q_scale_o_);
        ...
        static_assert(Cta_tile_p::M == 64);
        const int steps_m = (binfo_.actual_seqlen + Cta_tile_p::M - 1) / Cta_tile_p::M;
        const int steps_n = (binfo_.actual_seqlen + Cta_tile_p::N - 1) / Cta_tile_p::N;
        ...
        for( int ri = 0; ri < steps_m; ++ri ) {
            ...
            constexpr int ELTS_PER_TILE_PER_THREAD =
                Cta_tile_p::M * Cta_tile_p::N / Kernel_traits::THREADS;
            int ci = 0;
            Philox ph(
                seed, binfo_.tidx_global + (ri * steps_n + ci) * ELTS_PER_TILE_PER_THREAD, offset);
            compute_single_tile<true>(ctile_p, ctile_o, p_max, p_sum, tidx_, ri, ci, ph);

            for( int ci = 1; ci < steps_n; ++ci ) {
                Philox ph(seed,
                          binfo_.tidx_global + (ri * steps_n + ci) * ELTS_PER_TILE_PER_THREAD,
                          offset);
...
#pragma unroll
            for( int mi = 0; mi < Mma_tile_p::CORES_M; mi++ ) {
                ...
                const float scale = p_sum[mi] * d_scale_s_v_;
#pragma unroll
    ...
    __device__ inline void compute_single_tile(Compute_tile_p &ctile_p,
...
#pragma unroll
        ...
        const float scale_q_k = d_scale_q_k_ * params_.scale_bmm_q_k;
// Compute S.
#pragma unroll
        for( int mi = 0; mi < Mma_tile_p::CORES_M; mi++ ) {
#pragma unroll
...
#pragma unroll
        ...
        if( IS_TRAINING ) {
            // Dropout.
            const float scale_dropout = params_.rp_keep * q_scale_s_;
#pragma unroll
            for( int ni = 0; ni < Mma_tile_p::CORES_N; ni++ ) {

                const float4 tmp = uniform4(ph());
                const float rand[4] = { tmp.x, tmp.y, tmp.z, tmp.w };

#pragma unroll
                for( int mi = 0; mi < Mma_tile_p::CORES_M; mi++ ) {
                    // Dropout mask.
                    const float dmask0 = rand[mi * 2 + 0] <= params_.p_keep ? scale_dropout : 0.f;
                    const float dmask1 = rand[mi * 2 + 1] <= params_.p_keep ? scale_dropout : 0.f;
...
#pragma unroll
        ...
        uint32_t scale_ = reinterpret_cast<const uint32_t &>(one_f);
        size_t sbhs_tile_offset_bytes =
            ri * Cta_tile_p::M * params_.s_stride_in_bytes + ci * Cta_tile_p::N * 4;
        Gmem_tile_p gmem_p(static_cast<char *>(params_.p_ptr) + sbhs_tile_offset_bytes,
                           params_.s_stride_in_bytes,
                           params_.s * 4,
                           scale_,
                           tidx_);
        Gmem_tile_p gmem_s(static_cast<char *>(params_.s_ptr) + sbhs_tile_offset_bytes,
                           params_.s_stride_in_bytes,
                           params_.s * 4,
                           scale_,
                           tidx_);
    ...
    const fused_multihead_attention::Block_info_padded<Kernel_traits::THREADS> binfo_;
    const int tidx_;
