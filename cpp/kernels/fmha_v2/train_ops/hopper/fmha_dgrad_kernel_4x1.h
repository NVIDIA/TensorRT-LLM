...
struct Dgrad_4x1 {
    ...
    __device__ inline void serialize_acc(typename Acc::Data_type *ptr, const Acc &src) {
...
#pragma unroll
    ...
    template<typename Acc>
    __device__ inline void deserialize_acc(Acc &dst, const typename Acc::Data_type *ptr) {
...
#pragma unroll
    ...
    __device__ inline Dgrad_4x1(const Fmha_dgrad_params &params,
                                const int bidb,
                                const int bidh,
                                const int tidx,
        ...
        : params_(params), binfo_(params, bidb, bidh, tidx), tidx_(tidx),
          ...
          zi_ptr_(&reinterpret_cast<float *>(params.zi_ptr)[binfo_.hidx * params.s]), amax_dp_(0.f),
          amax_dqkv_(0.f), q_scale_dqkv_(*params.ptr_q_scale_dqkv),
          q_scale_s_(*params.ptr_q_scale_s), q_scale_dp_(*params.ptr_q_scale_dp) {
        ...
        dq_tmp_ =
            reinterpret_cast<float *>(params.dq_tmp_ptr) + binfo_.hidx * params_.s * 64 + tidx * 4;
    ...
    __device__ inline void operator()(uint64_t seed, uint64_t offset) {
        ...
        // Output tiles.
        uint32_t q_scale_dqkv = reinterpret_cast<const uint32_t &>(q_scale_dqkv_);
        Gmem_tile_dq gmem_dq8(
            params_.dqkv_ptr, params_.qkv_stride_in_bytes, binfo_, tidx_, q_scale_dqkv, 0, 0);
        Gmem_tile_dk gmem_dk8(
            params_.dqkv_ptr, params_.qkv_stride_in_bytes, binfo_, tidx_, q_scale_dqkv, 0, 1);
        Gmem_tile_dv gmem_dv8(
            params_.dqkv_ptr, params_.qkv_stride_in_bytes, binfo_, tidx_, q_scale_dqkv, 0, 2);
        ...
        static_assert(Cta_tile_p::M == 64);
        const int steps = (binfo_.actual_seqlen + Cta_tile_p::M - 1) / Cta_tile_p::M;
        ...
        for( int ci = 0; ci < steps; ++ci ) {
            ...
            for( int ri = 0; ri < steps; ++ri ) {
                ...
                Philox ph(seed,
                          binfo_.tidx_global + (ri * steps + ci) * ELTS_PER_TILE_PER_THREAD,
                          offset);
                bool first_col = ci == 0;
                ...
                if( first_col && last_col ) {
                    compute_single_tile<true, true>(ctile_dk,
                                                    ctile_dv,
                                                    ctile_p,
                                                    ctile_ds,
                                                    ctile_dq,
                    ...
                    compute_single_tile<true, false>(ctile_dk,
                                                     ctile_dv,
                                                     ctile_p,
                                                     ctile_ds,
                                                     ctile_dq,
                    ...
                    compute_single_tile<false, true>(ctile_dk,
                                                     ctile_dv,
                                                     ctile_p,
                                                     ctile_ds,
                                                     ctile_dq,
                    ...
                    compute_single_tile<false, false>(ctile_dk,
                                                      ctile_dv,
                                                      ctile_p,
                                                      ctile_ds,
                                                      ctile_dq,
...
#pragma unroll
    ...
    __device__ inline void compute_single_tile(Compute_tile_dk &ctile_dk,
                                               Compute_tile_dv &ctile_dv,
                                               Compute_tile_p &ctile_p,
                                               Compute_tile_ds &ctile_ds,
                                               Compute_tile_dq &ctile_dq,
                                               const uint32_t smem_nvvm_do_a,
                                               const uint32_t smem_nvvm_do_b,
                                               const uint32_t smem_nvvm_q_a,
                                               const uint32_t smem_nvvm_q_b,
                                               const int tidx,
                                               const int ri,
                                               const int ci,
                                               Gmem_tile_dq &gmem_dq8,
...
#pragma unroll
        ...
        // Recomputed masked S.
        Acc_p s_[1][1];
        const float scale_bmm_q_k = params_.scale_bmm_q_k * d_scale_q_k_;
#pragma unroll
        for( int ni = 0; ni < Mma_tile_p::CORES_N; ni++ ) {
#pragma unroll
            for( int mi = 0; mi < Mma_tile_p::CORES_M; mi++ ) {

                const float alpha = p_sum[mi];
                const float beta = p_max[mi];
        ...
        const float &p_keep = params_.p_keep;
        const float &rp_keep = params_.rp_keep;
...
#pragma unroll
            for( int mi = 0; mi < Mma_tile_p::CORES_M; mi++ ) {
                // Dropout mask.
                const float dmask0 = rand[mi * 2 + 0] <= p_keep ? rp_keep : 0.f;
                const float dmask1 = rand[mi * 2 + 1] <= p_keep ? rp_keep : 0.f;

                //const float dmask0 = keep[mi * 2 + 0]? rp_keep : 0.f;
                //const float dmask1 = keep[mi * 2 + 1]? rp_keep : 0.f;

                const float gamma = doo_sum[mi];
...
#pragma unroll
    ...
    __device__ inline void compute_single_diag_doo(float *diag_doo,
                                                   Compute_tile_t &ctile_t,
                                                   const int row_local,
                                                   const int qid) {
...
#pragma unroll
        for( int ni = 0; ni < Mma_tile_p::CORES_N; ni++ ) {
            ...
            if( c0 == row_local ) {
                diag_doo[row_local + 0] =
                    acc.elt(ni * Mma_tile_p::CORES_M * 2 + 0 * 2 + 0) * d_scale_do_o_;
            }
            if( c1 == row_local ) {
                diag_doo[row_local + 0] =
                    acc.elt(ni * Mma_tile_p::CORES_M * 2 + 0 * 2 + 1) * d_scale_do_o_;
            }
            if( c0 == row_local + 8 ) {
                diag_doo[row_local + 8] =
                    acc.elt(ni * Mma_tile_p::CORES_M * 2 + 1 * 2 + 0) * d_scale_do_o_;
            }
            if( c1 == row_local + 8 ) {
                diag_doo[row_local + 8] =
                    acc.elt(ni * Mma_tile_p::CORES_M * 2 + 1 * 2 + 1) * d_scale_do_o_;
            }
            c0 += 8;
            c1 += 8;
        }
