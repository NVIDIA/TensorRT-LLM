...
struct Gmem_tile_mma_sd {
    ...
    inline __device__
    Gmem_tile_mma_sd(void *ptr, const Params &params, const Block_info &block_info, int tidx)
    ...
    inline __device__ void store(const Type &data, int mi, int ni) {
...
struct Gmem_tile_mma_s : public Base {
    ...
    template<typename Params, typename Block_info>
    inline __device__ Gmem_tile_mma_s(const Params &params, const Block_info &block_info, int tidx)
...
#pragma unroll
    ...
    template<typename Mask>
    inline __device__ void store_sign(const float (&softmax)[2 * M][4 * N], const Mask &mask) {
#pragma unroll
        for( int mi = 0; mi < M; mi++ ) {
#pragma unroll
            for( int ni = 0; ni < N; ni++ ) {
    ...
    softmax_to_regs(const float (&softmax)[2 * M][4 * N], uint4 (&regs)[M][N], const Mask &mask) {
#pragma unroll
        for( int mi = 0; mi < M; mi++ ) {
#pragma unroll
            for( int ni = 0; ni < N; ni++ ) {
...
template<
    ...
    typename Cta_tile,
    // The base class.
    typename Base = fmha::v2::Gmem_tile_qkv<Traits,
                                            Cta_tile,
...
struct Gmem_tile_dout : public Base {
    ...
    // Ctor.
    template<typename Params, typename Block_info>
    inline __device__ Gmem_tile_dout(const Params &params, const Block_info &block_info, int tidx)
...
struct Gmem_tile_out : public Base {
    ...
    // Ctor.
    template<typename Params, typename Block_info>
    inline __device__ Gmem_tile_out(const Params &params, const Block_info &block_info, int tidx)
        : Base(params, 0, block_info, tidx) {
...
struct Gmem_softmax_sum {
    ...
    // The helper class.
    using Helper = fmha::v2::Gmem_tile_qkv<Traits,
                                           Cta_tile,
                                           Traits::BITS_PER_ELEMENT_A,
                                           Cta_tile::M,
                                           Cta_tile::K,
    ...
    inline __device__ Gmem_softmax_sum(const Params &params, const int tidx)
...
#pragma unroll
    ...
    // Store data to global memory.
    inline __device__ void store_row(const uint32_t (&data)[LDGS], const int row) {
...
#pragma unroll
    ...
    template<int N>
    inline __device__ void load_row(uint32_t (&data)[LDGS], const int row) {
        enum { BYTES_PER_LDG = ROWS_PER_LDG * BYTES_PER_ELEMENT };
#pragma unroll
    ...
    inline __device__ void move_to(const int steps) {
...
struct Gmem_tile_dq : public Base {
    ...
    // Ctor.
    template<typename Params, typename Block_info>
    inline __device__ Gmem_tile_dq(const Params &params, const Block_info &block_info, int tidx)
...
struct Gmem_tile_dq_red_base {
    ...
    inline __device__ Gmem_tile_dq_red_base(const Params &params, const Binfo_ &binfo, int tidx)
...
struct Gmem_tile_dq_red<fmha::Ampere_hmma_fp32_traits, Cta_tile_>
    ...
    // Ctor.
    template<typename Params, typename Binfo_>
    inline __device__ Gmem_tile_dq_red(const Params &params, const Binfo_ &binfo, int tidx)
