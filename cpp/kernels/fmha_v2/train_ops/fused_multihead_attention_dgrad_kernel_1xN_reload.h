...
#pragma unroll
    for( int mi = 0; mi < M; ++mi ) {
        sum[mi] = fmha::Allreduce<THREADS_PER_ROW>::run(
                      fmha::fma8_in_float<Data_type_>(do_[mi], o[mi]), sum_op) *
                  scale;
    }
...
template<typename Kernel_traits, typename Params>
inline __device__ void compute_dv_1xN(const Params &params, const int isteps) {
    ...
    const int STEPS = (binfo.actual_seqlen + Cta_tile_p::M - 1) / Cta_tile_p::M;
    ...
    for( int loop_id = 0; loop_id < isteps; loop_id++ ) {  // newloop : new loop
        ...
        const int new_loop = newloop * Cta_tile_p::N;
        ...
        fmha::Clear_accumulator<typename Traits::Accumulator_type, Cta_tile_dv::WARPS_K>::apply(
            acc_dv);
...
#pragma unroll
            ...
            typename fmha::Softmax_statistics<Traits, Cta_tile_p> lse_array(
                params, params.lse_ptr, binfo, tidx);
                        ...
                        for( int jj = 0; jj < 4; jj++ ) {
                            ...
                            const float d_s = drop ? 0.f
                                                   : softmax.elt_[2 * mi + ii][4 * ni + jj] *
                                                         params.rp_dropout;  //dP'
                            // 3. tmp = dS * S, dP' * P'
                            s_dmask = fabsf(s_dmask);  // P'
                            softmax.elt_[2 * mi + ii][4 * ni + jj] =
                                d_s * fabsf(s_dmask);  // dP' * P'
                        }
                    }
                }
            }
...
inline __device__ void compute_dq_dk_1xN(const Params &params, const int isteps) {
    ...
    for( int loop_id = 0; loop_id < isteps; loop_id++ ) {
        ...
        const int new_loop = newloop * Cta_tile_o::K;
        if( new_loop >= binfo.actual_seqlen )
            //break;
            return;
...
#define LOAD_K 0
        ...
        fmha::Clear_accumulator<typename Traits::Accumulator_type, Cta_tile_dk::WARPS_K>::apply(
            acc_dk);
