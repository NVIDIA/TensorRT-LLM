...
using Kernel_traits =
    fmha::Kernel_traits_v2<fmha::Ampere_hmma_fp32_traits, 128, 64, 16, 1, 4, 1, 0x28u>;
static_assert(!Kernel_traits::HEADS_INTERLEAVED);

extern "C" __global__ void
fmha_dgrad_v2_fp16_128_64_sm80_kernel(Fused_multihead_attention_fprop_params params) {
...
void run_fmha_dgrad_v2_fp16_128_64_sm80(const Fused_multihead_attention_fprop_params &params,
                                        cudaStream_t stream) {

    constexpr int smem_size_softmax =
        Kernel_traits::Cta_tile_p::M * Kernel_traits::Cta_tile_p::WARPS_N * sizeof(float);
    constexpr int smem_size_q = Kernel_traits::Smem_tile_q::BYTES_PER_TILE;
    constexpr int smem_size_v = Kernel_traits::Smem_tile_v::BYTES_PER_TILE;
    constexpr int smem_size_o = Kernel_traits::Smem_tile_o::BYTES_PER_TILE;

    using Smem_tile_s =
        Smem_tile_mma_transposed<fmha::Ampere_hmma_fp32_traits, Kernel_traits::Cta_tile_p>;
    ...
    if( smem_size >= 48 * 1024 ) {
        FMHA_CHECK_CUDA(cudaFuncSetAttribute(fmha_dgrad_v2_fp16_128_64_sm80_kernel,
                                             cudaFuncAttributeMaxDynamicSharedMemorySize,
                                             smem_size));
    }

    dim3 grid(params.h, params.b);
    fmha_dgrad_v2_fp16_128_64_sm80_kernel<<<grid, Kernel_traits::THREADS, smem_size, stream>>>(
        params);
}
