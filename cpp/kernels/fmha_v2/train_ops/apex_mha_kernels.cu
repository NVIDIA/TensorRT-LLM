...
fwd_cuda(const torch::Tensor &qkv, const torch::Tensor &pad_mask, float dropout_prob) {
    // sxbxhx3xd
    const int head_dim = qkv.size(4);
    const int heads = qkv.size(2);
    const int sequences = qkv.size(1);
    const int q_seq_len = qkv.size(0);
    ...
    const half *q_lin_results_ptr = static_cast<const half *>(qkv.data_ptr());
    const half *k_lin_results_ptr = static_cast<const half *>(qkv.data_ptr()) + head_dim;
    const half *v_lin_results_ptr = static_cast<const half *>(qkv.data_ptr()) + 2 * head_dim;
    ...
    softmax_success = dispatch_additive_masked_softmax_dropout<half, half, float>(
        dropout_results_ptr,
        dropout_mask.data_ptr<uint8_t>(),
        bmm1_results_ptr,
        static_cast<half *>(pad_mask.data_ptr()),
...
std::vector<torch::Tensor> bwd_cuda(int heads,
                                    ...
                                    float dropout_prob) {
    const int embed_dim = input_lin_results.size(-1) * heads;
    const int sequences = input_lin_results.size(1);
    const int q_seq_len = input_lin_results.size(0);
    const int k_seq_len = q_seq_len;
