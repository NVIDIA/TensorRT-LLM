            ...
            for( int hi = 0; hi < h; hi++ ) {
                for( int ti = 0; ti < 3; ti++ ) {
                    for( int di = 0; di < d; di++ ) {
                        size_t out_idx =
                            size_t(si) * b * h * 3 * d + bi * h * 3 * d + hi * 3 * d + ti * d + di;
                        dst[out_idx] = *src;
                        *src++;
                    }
                }
        ...
        for( int si = 0; si < s_; si++ ) {
            for( int hi = 0; hi < h; hi++ ) {
                for( int ti = 0; ti < t; ti++ ) {
                    for( int di = 0; di < d; di++ ) {
                        size_t in_idx =
                            size_t(si) * b * h * t * d + bi * h * t * d + hi * t * d + ti * d + di;
                        dst[0] = src[in_idx];
                        dst++;
                    }
                }
...
static inline void store_q_and_contiguous_kv_cache(void *q_d,                    // [B, S, H, D]
                                                   ...
                                                   const float *qkv_packed_src,  // [B, S, H, 3, D]
                                                   const int *cu_kv_seqlens,     // [B + 1]
                                                   const int *cu_q_seqlens,      // [B + 1]
                                                   size_t b,
                                                   size_t s,
                                                   size_t h_q,
                                                   size_t h_kv,
    ...
    // Handle Q.
    const int total_q_seqlen = cu_q_seqlens[b];
        ...
        for( size_t si = 0; si < q_length; si++ ) {
            for( size_t hi = 0; hi < h_q; hi++ ) {
                for( size_t di = 0; di < d; di++ ) {
                    size_t src_offset =
                        (cu_kv_seqlens[bi] + kv_length - q_length + si) * h_q * (2 * d + dv) +
                        hi * (2 * d + dv) + di;
                    size_t dst_offset = (cu_q_seqlens[bi] + si) * h_q * d + hi * d + di;
                    switch( dtype ) {
                    case DATA_TYPE_FP16:
                        reinterpret_cast<half *>(q_tmp)[dst_offset] =
                            half(qkv_packed_src[src_offset]);
                        break;
                    case DATA_TYPE_BF16:
    ...
    const size_t total_kv_tokens = cu_kv_seqlens[b];
    // The kv cache size in bytes.
    const size_t kv_size_in_bytes = get_size_in_bytes(total_kv_tokens * 2 * h_kv * d, dtype);
    // Batch size.
    for( size_t bi = 0; bi < b; bi++ ) {
        // The current cumulative sequence length offset.
        const int seqlen_offset = cu_kv_seqlens[bi];
        // The actual kv sequence length.
        const int actual_kv_seqlen = cu_kv_seqlens[bi + 1] - cu_kv_seqlens[bi];
        // [B, S, H, 3, D]
        const float *kv_packed_src = qkv_packed_src + seqlen_offset * h_q * 3 * d;
        // Head.
        for( size_t hi = 0; hi < h_kv; hi++ ) {
            ...
            for( size_t si = 0; si < actual_kv_seqlen; si++ ) {
                ...
                for( size_t di = 0; di < d; di++ ) {
                    ...
                    switch( dtype ) {
                    case DATA_TYPE_FP16:
                        reinterpret_cast<half *>(contiguous_kv_h)[dst_k_offset] =
                            half(kv_packed_src[src_k_offset]);
                        reinterpret_cast<half *>(contiguous_kv_h)[dst_v_offset] =
                            half(kv_packed_src[src_v_offset]);
                        break;
                    case DATA_TYPE_BF16:
...
static inline void
store_paged_kv_cache(void **paged_kv_cache_ptrs,   // [B, 2, M] with [H, S_per_B, Dh]
                     const float *qkv_packed_src,  // [B, S, H, 3, Dh]
                     const int *cu_kv_seqlens,     // [B + 1]
    ...
    for( size_t bi = 0; bi < b; bi++ ) {
        const int actual_kv_seqlen = cu_kv_seqlens[bi + 1] - cu_kv_seqlens[bi];
        const size_t num_blocks = (actual_kv_seqlen + tokens_per_block - 1) / tokens_per_block;
        const float *kv_packed_src = qkv_packed_src + cu_kv_seqlens[bi] * h_q * (2 * d + dv);
        for( size_t block_idx = 0; block_idx < num_blocks; block_idx++ ) {
            ...
            for( size_t hi = 0; hi < h_kv; hi++ ) {
                for( size_t si = 0; si < seq_bound; si++ ) {
                    auto copy_vector = [&](void *block, size_t vector_d, int qkv_offset) {
                        size_t src_offset =
                            (si + block_idx * tokens_per_block) * h_q * (2 * d + dv) +
            ...
            FMHA_CHECK_CUDA(
                cudaMemcpy(paged_kv_cache_ptrs[block_idx], k_tmp, sz_k, cudaMemcpyDefault));
            FMHA_CHECK_CUDA(cudaMemcpy(paged_kv_cache_ptrs[max_blocks_per_seq + block_idx],
                                       v_tmp,
                                       sz_v,
                                       cudaMemcpyDefault));
        }
...
template<typename T>
static inline void extract_and_transpose_chunked_output(void *dst_,
                                                        void *src_,
                                                        const std::vector<uint32_t> &seqlens,
                                                        const std::vector<uint32_t> &q_seqlens,
                                                        const int s,
                                                        const int q_seqlen,
                                                        const int b,
                                                        const int h,
...
static inline void
pack_flash_attention_mask(uint32_t *packed_mask_h,
...
struct RefBMM {
    ...
    RefBMM(cudaDataType_t type_A,
        ...
        FMHA_CHECK_CUBLAS(cublasLtMatmulDescSetAttribute(
            matmul_desc, CUBLASLT_MATMUL_DESC_TRANSA, &op_B, sizeof(op_B)));
        FMHA_CHECK_CUBLAS(cublasLtMatmulDescSetAttribute(
            matmul_desc, CUBLASLT_MATMUL_DESC_TRANSB, &op_A, sizeof(op_A)));
...
static int check_softmax_results(const float *out,
                                 const float *ref,
                                 size_t b,
                                 size_t s,
                                 size_t h,
    ...
    for( int b_ = 0; b_ < b; ++b_ ) {
        for( int s_ = 0; s_ < seqlens[b_]; ++s_ ) {
            for( int h_ = 0; h_ < h; ++h_ ) {
                ...
                if( sum_ref != 1.0f &&
                    fabsf(sum - sum_ref) / (fabsf(sum) + fabsf(sum_ref)) > 0.01 ) {
                    n_errors++;
                }
            }
        }
