...
__global__ void convert_int32_to_int8_kernel(void *dst, const void *src, size_t n, float scale) {
    ...
    for( size_t ii = blockIdx.x * blockDim.x + threadIdx.x; ii < n / 4; ii += step ) {
        ...
        // Load 4 integers.
        int4 tmp = reinterpret_cast<const int4 *>(src)[ii];
        ...
        // Compact.
        char4 out;
        out.x = reinterpret_cast<const int8_t &>(a);
        out.y = reinterpret_cast<const int8_t &>(b);
        out.z = reinterpret_cast<const int8_t &>(c);
        out.w = reinterpret_cast<const int8_t &>(d);

        // Store.
        reinterpret_cast<uint32_t *>(dst)[ii] = reinterpret_cast<const uint32_t &>(out);
    }
}
...
template<typename T>
__device__ inline typename fmha::Uint_from_size_in_bytes<sizeof(T) * 4>::Type
pack_float4(const float4 &f);

////////////////////////////////////////////////////////////////////////////////////////////////////

template<>
__device__ inline uint2 pack_float4<fmha::fp16_t>(const float4 &f) {
...
template<>
__device__ inline uint2 pack_float4<fmha::bf16_t>(const float4 &f) {
...
template<>
__device__ inline uint32_t pack_float4<fmha::e4m3_t>(const float4 &f) {
    return fmha::float4_to_e4m3x4(f.x, f.y, f.z, f.w);
}

////////////////////////////////////////////////////////////////////////////////////////////////////
template<>
__device__ inline uint32_t pack_float4<fmha::e5m2_t>(const float4 &f) {
...
template<typename T>
__global__ void convert_fp32_to_T_kernel(void *dst, const void *src, size_t n, float scale = 1.f) {
    ...
    for( size_t ii = blockIdx.x * blockDim.x + threadIdx.x; ii < n / 4; ii += step ) {
        ...
        // Load 4 floats.
        float4 tmp = reinterpret_cast<const float4 *>(src)[ii];
        // Scale.
        tmp.x *= scale;
        tmp.y *= scale;
        tmp.z *= scale;
        ...
        // Store.
        reinterpret_cast<Dst *>(dst)[ii] = reinterpret_cast<const Dst &>(out);
    }
}

template<typename T>
__global__ void convert_T_to_fp32_kernel(void *dst, const void *src, size_t n, float scale = 1.f) {
    ...
    for( size_t ii = blockIdx.x * blockDim.x + threadIdx.x; ii < n / 4; ii += step ) {
        ...
        // Load 4 floats.
        data.raw = reinterpret_cast<const Src *>(src)[ii];
        float4 out;
        // Scale.
        ...
        // Store.
        reinterpret_cast<float4 *>(dst)[ii] = reinterpret_cast<const float4 &>(out);
    }
}
...
void run_conversion_fp32_to_e4m3(void *dst, const void *src, size_t n, float scale_o) {
    convert_fp32_to_T_kernel<fmha::e4m3_t><<<512, 256>>>(dst, src, n, scale_o);
}

////////////////////////////////////////////////////////////////////////////////////////////////////

void run_conversion_e4m3_to_fp32(void *dst, const void *src, size_t n, float scale_o) {
...
void run_conversion_fp32_to_e5m2(void *dst, const void *src, size_t n, float scale_o) {
    convert_fp32_to_T_kernel<fmha::e5m2_t><<<512, 256>>>(dst, src, n, scale_o);
}

////////////////////////////////////////////////////////////////////////////////////////////////////

void run_conversion_e5m2_to_fp32(void *dst, const void *src, size_t n, float scale_o) {
