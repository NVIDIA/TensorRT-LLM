...
static inline __device__ float to_float(const uint16_t &src, float) {
...
static inline __device__ float to_float(const fmha::bf16_t &src, float) {
...
// Disable warning #177-D because this function has not been used elsewhere
#pragma nv_diag_suppress 177
static inline __device__ float to_float(const fmha::e4m3_t &src, float) {
    return float(src);
}
#pragma nv_diag_default 177
...
static inline __device__ float to_float(const float &src, float) {
    return src;
}

////////////////////////////////////////////////////////////////////////////////////////////////////

static inline __device__ float to_float(const int &src, float scale) {
    float dst;
...
static inline __device__ void from_float(uint16_t &dst, const float &src, float) {
    dst = fmha::float_to_half(src);
}

////////////////////////////////////////////////////////////////////////////////////////////////////

static inline __device__ void from_float(fmha::bf16_t &dst, const float &src, float) {
...
static inline __device__ void from_float(int8_t &dst, const float &src, float scale) {
    dst = float_to_int8_rn(src * scale);
}

////////////////////////////////////////////////////////////////////////////////////////////////////

static inline __device__ void from_float(fmha::e4m3_t &dst, const float &src, float scale) {
...
reduce(float (&data_fp32)[N][1], const int8_t (&mask)[N][1], int warps_n, float &sum_fp32) {
...
template<int N>
static inline __device__ void
reduce(float (&data_fp32)[N][2], const int8_t (&mask)[N][2], int warps_n, float &sum_fp32) {
// Apply the masks.
#pragma unroll
    for( int ii = 0; ii < N; ++ii ) {
        data_fp32[ii][0] = mask[ii][0] ? data_fp32[ii][0] : -HUGE_VALF;
...
template<int N>
static inline __device__ void
reduce(float (&data_fp32)[N][4], const int8_t (&mask)[N][4], int warps_n, float &sum_fp32) {
...
struct VecX {
    ...
    static __device__ inline void to_floatX(float (&dst)[X],
                                            const Type &src,
                                            const float scale,
                                            const float attn_logit_softcapping_scale) {
        Alias tmp;
        tmp.raw = src;
#pragma unroll
        for( int it = 0; it < X; it++ ) {
            ...
            if( attn_logit_softcapping_scale != 0.f ) {
                dst[it] = attn_logit_softcapping_scale *
                          fmha::__tanhf(dst[it] / attn_logit_softcapping_scale);
            }
        }
    }

    static __device__ inline void from_floatX(Type &dst, const float (&src)[X], const float scale) {
        Alias tmp;
#pragma unroll
        for( int it = 0; it < X; it++ ) {
...
inline __device__ float get_alibi_head_scaling_factor(const int head_id, const int num_heads) {
    // Round down to power of 2
    const int num_heads_pow2 = (1u << (31 - __clz(num_heads)));
    if( head_id < num_heads_pow2 ) {
        return exp2f((head_id + 1) * -8.0f / num_heads_pow2);
    } else {
        const float adjusted_head_id = 2 * (head_id - num_heads_pow2) + 1;
...
static __global__ void softmax_kernel(Softmax_params<Dst_type, Src_type> params) {
    ...
    const SrcX_type *src_ptr = reinterpret_cast<const SrcX_type *>(&params.src[src_offset]);
...
#pragma unroll
    ...
    // Load the masks.
    const MaskX_type *mask_ptr = reinterpret_cast<const MaskX_type *>(&params.mask[mask_offset]);
...
#pragma unroll
    for( int ii = 0; ii < VECs_PER_THREAD; ++ii ) {
        VecI::to_floatX(
            data_fp32[ii], data_src[ii], params.scale_bmm1, params.softcapping_scale_bmm1);
...
#pragma unroll
    ...
    if( params.has_alibi ) {
        const float alibi_factor = get_alibi_head_scaling_factor(hi, params.h);
#pragma unroll
        for( int ii = 0; ii < VECs_PER_THREAD; ii++ ) {
#pragma unroll
...
void run_softmax(void *dst,
    ...
    if( s_inner == 32 ) {
        ...
        softmax_kernel<Dst_type, Src_type, 192, WARPS_PER_CTA, 2>
            <<<grid, threads_per_cta>>>(params);
    } else if( s_inner == 256 ) {
        softmax_kernel<Dst_type, Src_type, 256, WARPS_PER_CTA><<<grid, threads_per_cta>>>(params);
    } else if( s_inner == 384 ) {
        softmax_kernel<Dst_type, Src_type, 384, WARPS_PER_CTA, 2>
            <<<grid, threads_per_cta>>>(params);
    } else if( s_inner == 512 ) {
