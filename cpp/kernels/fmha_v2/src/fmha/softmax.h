...
inline __device__ float get_alibi_head_scaling_factor(const int in_head_id,
                                                      const AlibiParams &params) {
    const int head_id = params.head_idx_offset + in_head_id;
    if( head_id < params.h_pow_2 ) {
        // 2^(head_id * -8 / h)
        return exp2f((head_id + 1) * 2 * params.alibi_neg4_div_h) * params.scale_after_alibi;
    } else {
        // 1,3,5... etc
        const float adjusted_head_id = 2 * (head_id - params.h_pow_2) + 1;
...
#pragma unroll
    ...
    apply_mask_alibi(const Mask &mask, int head_id, const AlibiParams &alibi_params) {
...
#pragma unroll
    ...
    inline __device__ void apply_mask(const uint32_t (&packed_mask)[MMAS_M]) {
...
#pragma unroll
    ...
    inline __device__ void apply_mask(const int actual_seqlen) {

        // The warp/lane decomposition.
        const int warp = threadIdx.x / Cta_tile::THREADS_PER_WARP;
        const int lane = threadIdx.x % Cta_tile::THREADS_PER_WARP;
...
#pragma unroll
            for( int ni = 0; ni < MMAS_N; ++ni ) {
                ...
                const int offset = ni * Mma_tile::N_PER_MMA_PER_CTA + warp_n * 16;
    ...
    inline __device__ void apply_exp(const float max) {
#pragma unroll
        for( int mi = 0; mi < MMAS_M * 2; ++mi ) {
#pragma unroll
            for( int ni = 0; ni < MMAS_N * 4; ++ni ) {
    ...
    // Apply the exp to all the elements.
    inline __device__ void apply_scale_exp(const float (&max)[MMAS_M * 2], float scale_bmm1) {
#pragma unroll
        for( int mi = 0; mi < MMAS_M * 2; ++mi ) {
#pragma unroll
            for( int ni = 0; ni < MMAS_N * 4; ++ni ) {
                elt_[mi][ni] =
                    apply_exp_<Kernel_traits::VERSION>(scale_bmm1 * elt_[mi][ni], max[mi]);
            }
        }
    }
...
#pragma unroll
        ...
        float2 tmp = reinterpret_cast<const float2 *>(smem_)[tidx_];
...
struct Softmax_hmma : public Softmax_base<Traits, Cta_tile, Kernel_traits> {
    ...
    // Ctor.
    template<typename Params>
    inline __device__ Softmax_hmma(const Params &params, void *smem, int bidb, int tidx)
...
#pragma unroll
            for( int ni = 0; ni < MMAS_N; ++ni ) {
                ...
                half2_to_float2(
                    this->elt_[2 * mi + 0][4 * ni + 0], this->elt_[2 * mi + 0][4 * ni + 1], acc_0);
                half2_to_float2(
                    this->elt_[2 * mi + 1][4 * ni + 0], this->elt_[2 * mi + 1][4 * ni + 1], acc_1);
                half2_to_float2(
                    this->elt_[2 * mi + 0][4 * ni + 2], this->elt_[2 * mi + 0][4 * ni + 3], acc_2);
                half2_to_float2(
                    this->elt_[2 * mi + 1][4 * ni + 2], this->elt_[2 * mi + 1][4 * ni + 3], acc_3);
                ...
                if constexpr( ENABLE_BMM1_SOFTCAPPING_SCALE ) {
                    this->elt_[2 * mi + 0][4 * ni + 0] =
                        params_softcapping_scale_bmm1_ *
                        __tanhf(this->elt_[2 * mi + 0][4 * ni + 0]);
                    this->elt_[2 * mi + 0][4 * ni + 1] =
                        params_softcapping_scale_bmm1_ *
                        __tanhf(this->elt_[2 * mi + 0][4 * ni + 1]);
                    this->elt_[2 * mi + 1][4 * ni + 0] =
                        params_softcapping_scale_bmm1_ *
                        __tanhf(this->elt_[2 * mi + 1][4 * ni + 0]);
                    this->elt_[2 * mi + 1][4 * ni + 1] =
                        params_softcapping_scale_bmm1_ *
                        __tanhf(this->elt_[2 * mi + 1][4 * ni + 1]);
                    this->elt_[2 * mi + 0][4 * ni + 2] =
                        params_softcapping_scale_bmm1_ *
                        __tanhf(this->elt_[2 * mi + 0][4 * ni + 2]);
                    this->elt_[2 * mi + 0][4 * ni + 3] =
                        params_softcapping_scale_bmm1_ *
                        __tanhf(this->elt_[2 * mi + 0][4 * ni + 3]);
                    this->elt_[2 * mi + 1][4 * ni + 2] =
                        params_softcapping_scale_bmm1_ *
                        __tanhf(this->elt_[2 * mi + 1][4 * ni + 2]);
                    this->elt_[2 * mi + 1][4 * ni + 3] =
                        params_softcapping_scale_bmm1_ *
                        __tanhf(this->elt_[2 * mi + 1][4 * ni + 3]);
                }
            }
        }
    }
    ...
    inline __device__ void apply_exp_with_mask(const float (&max)[MMAS_M * 2]) {
#pragma unroll
        for( int mi = 0; mi < MMAS_M * 2; ++mi ) {
...
#pragma unroll
    ...
    const float params_softcapping_scale_bmm1_;
};
...
struct Fragment_helper<fmha::Volta_imma_int8_int32_traits> {
    ...
    static inline __device__ void load(int32_t (&dst)[2][4], const Accumulator &src) {
        dst[0][0] = src.elt(0);
        dst[0][1] = src.elt(1);
        dst[0][2] = src.elt(2);
        dst[0][3] = src.elt(3);
    ...
    static inline __device__ void store(Accumulator &dst, const uint32_t (&src)[2][4]) {
        dst.reg(0) = src[0][0];
        dst.reg(1) = src[0][1];
        dst.reg(2) = src[0][2];
        dst.reg(3) = src[0][3];
...
struct Softmax_imma : public Softmax_base<Traits, Cta_tile, Kernel_traits> {
    ...
    // Ctor.
    template<typename Params>
    inline __device__ Softmax_imma(const Params &params, void *smem, int bidb, int tidx)
        : Base(params, smem, bidb, tidx), params_scale_bmm1_(params.scale_bmm1),
    ...
    inline __device__ void store(Gmem_tile &gmem_tile) {
        ...
        const float scale = reinterpret_cast<const float &>(params_scale_softmax_);
        Accumulator acc[MMAS_M][MMAS_N];
#pragma unroll
        for( int mi = 0; mi < MMAS_M; ++mi ) {
#pragma unroll
...
#pragma unroll
    ...
    inline __device__ void unpack(const Accumulator (&acc)[MMAS_M][MMAS_N]) {
        const float scale = reinterpret_cast<const float &>(params_scale_bmm1_);
#pragma unroll
        for( int mi = 0; mi < MMAS_M; ++mi ) {
#pragma unroll
            for( int ni = 0; ni < MMAS_N; ++ni ) {
...
#pragma unroll
                    for( int nj = 0; nj < 4; ++nj ) {
                        ...
                        float f = reinterpret_cast<const float &>(tmp[mj][nj]);
                        this->elt_[2 * mi + mj][4 * ni + nj] = (f - FP32_I2F_MAGIC_NUMBER) * scale;
#else
                        this->elt_[2 * mi + mj][4 * ni + nj] =
                            static_cast<float>(tmp[mj][nj]) * scale;
    ...
    inline __device__ void pack(Fragment_a_ (&dst)[K][M]) {
        ...
        const float scale = reinterpret_cast<const float &>(this->params_scale_softmax_);
...
#pragma unroll
        for( int mi = 0; mi < M; ++mi ) {
            ...
            if( K > 1 ) {
                dst[1][mi].reg(0) =
                    fmha::float4_to_8bitx4<Dst_type>(tmp_08, tmp_09, tmp_0a, tmp_0b);
                dst[1][mi].reg(1) =
                    fmha::float4_to_8bitx4<Dst_type>(tmp_28, tmp_29, tmp_2a, tmp_2b);
            }
            ...
            if( Mma_tile::MMAS_N == 6 ) {
                ...
                dst[1][mi].reg(2) =
                    fmha::float4_to_8bitx4<Dst_type>(tmp_0c, tmp_0d, tmp_0e, tmp_0f);
                dst[1][mi].reg(3) =
                    fmha::float4_to_8bitx4<Dst_type>(tmp_2c, tmp_2d, tmp_2e, tmp_2f);
                dst[2][mi].reg(0) =
                    fmha::float4_to_8bitx4<Dst_type>(tmp_10, tmp_11, tmp_12, tmp_13);
                dst[2][mi].reg(1) =
                    fmha::float4_to_8bitx4<Dst_type>(tmp_30, tmp_31, tmp_32, tmp_33);
                dst[2][mi].reg(2) =
                    fmha::float4_to_8bitx4<Dst_type>(tmp_14, tmp_15, tmp_16, tmp_17);
                dst[2][mi].reg(3) =
                    fmha::float4_to_8bitx4<Dst_type>(tmp_34, tmp_35, tmp_36, tmp_37);
                ...
                dst[1][mi].reg(2) =
                    fmha::float4_to_8bitx4<Dst_type>(tmp_0c, tmp_0d, tmp_0e, tmp_0f);
                dst[1][mi].reg(3) =
                    fmha::float4_to_8bitx4<Dst_type>(tmp_1c, tmp_1d, tmp_1e, tmp_1f);
    ...
    // The scaling factors.
    const uint32_t params_scale_bmm1_, params_scale_softmax_;
};
...
struct Softmax_qmma<fmha::Ada_qmma_e4m3_fp32_traits, Cta_tile, Kernel_traits>
    ...
    // Ctor.
    template<typename Params>
    inline __device__ Softmax_qmma(const Params &params, void *smem, int bidb, int tidx)
...
#pragma unroll
            for( int ki = 0; ki < K; ++ki ) {
                ...
                // Pack to 4 registers.
                dst[ki][mi].reg(0) =
                    fmha::float4_to_fp8x4<Traits::A_type>(tmp_00, tmp_01, tmp_02, tmp_03);
                dst[ki][mi].reg(1) =
                    fmha::float4_to_fp8x4<Traits::A_type>(tmp_10, tmp_11, tmp_12, tmp_13);
                dst[ki][mi].reg(2) =
                    fmha::float4_to_fp8x4<Traits::A_type>(tmp_04, tmp_05, tmp_06, tmp_07);
                dst[ki][mi].reg(3) =
                    fmha::float4_to_fp8x4<Traits::A_type>(tmp_14, tmp_15, tmp_16, tmp_17);
            }
        }
    }
...
#pragma unroll
            for( int ni = 0; ni < MMAS_N; ++ni ) {
                ...
                elt_ptr0[0] =
                    fmha::half2_to_float2(fmha::hmul2(acc[mi][ni].reg(0), params_scale_bmm1_));
                elt_ptr0[1] =
                    fmha::half2_to_float2(fmha::hmul2(acc[mi][ni].reg(2), params_scale_bmm1_));
                elt_ptr1[0] =
                    fmha::half2_to_float2(fmha::hmul2(acc[mi][ni].reg(1), params_scale_bmm1_));
                elt_ptr1[1] =
                    fmha::half2_to_float2(fmha::hmul2(acc[mi][ni].reg(3), params_scale_bmm1_));
            }
        }
    }
...
#pragma unroll
                    for( int ii = 0; ii < 8; ++ii ) {
                        ...
                        if( mask.is_valid(row, col) ) {
                            ...
                            //  it is sufficient just to use the column as the multiplier
                            elt_[mi][8 * ni + ii] =
                                elt_[mi][8 * ni + ii] * alibi_params.scale_after_alibi +
                                m * (col + alibi_params.sequence_pos_offset);
                        } else {
                            elt_[mi][8 * ni + ii] = -FLT_MAX;
                        }
...
#pragma unroll
    ...
    // Mask the elements that are outside the the sequence length.
    inline __device__ void apply_mask(const int seqlen) {
        ...
        const int offset = warp_n * 16 + (threadIdx.x & 0x08) / 2;
...
#pragma unroll
    ...
    //  as it is possible that all tokens are masked out.
    template<bool APPLY_MASK = false>
    inline __device__ void apply_exp_with_mask(const float (&max)[MMAS_M]) {
#pragma unroll
        for( int mi = 0; mi < MMAS_M; ++mi ) {
            float max_val = APPLY_MASK && max[mi] == -FLT_MAX ? 0.f : max[mi];
#pragma unroll
...
#pragma unroll
        ...
        float2 tmp[2];
        if( tidx_ < Cta_tile::M ) {
            tmp[0] = reinterpret_cast<const float2 *>(&smem_[0 * ELEMENTS / 2])[tidx_];
            tmp[1] = reinterpret_cast<const float2 *>(&smem_[1 * ELEMENTS / 2])[tidx_];
        }
...
#pragma unroll
        ...
        float2 tmp[4];
        if( tidx_ < Cta_tile::M ) {
            tmp[0] = reinterpret_cast<const float2 *>(&smem_[0 * ELEMENTS / 4])[tidx_];
            tmp[1] = reinterpret_cast<const float2 *>(&smem_[1 * ELEMENTS / 4])[tidx_];
            tmp[2] = reinterpret_cast<const float2 *>(&smem_[2 * ELEMENTS / 4])[tidx_];
            tmp[3] = reinterpret_cast<const float2 *>(&smem_[3 * ELEMENTS / 4])[tidx_];
        }
...
struct Softmax_fp32 : public Softmax_hmma<Traits, Cta_tile, Kernel_traits> {
    ...
    // Ctor.
    template<typename Params>
    inline __device__ Softmax_fp32(const Params &params, void *smem, int bidb, int tidx)
        : Base(params, smem, bidb, tidx), smem_sum_(static_cast<float *>(smem), tidx),
...
#pragma unroll
    ...
    reduce_(float (&frag)[2 * MMAS_M], Operator &op, Smem_tile_red &smem_red) {
#pragma unroll
...
#pragma unroll
    ...
    inline __device__ void unpack(const Accumulator (&acc)[MMAS_M][MMAS_N]) {
        ...
        const float scale = params_scale_q_ * params_scale_k_;
#pragma unroll
        for( int mi = 0; mi < MMAS_M; ++mi ) {
#pragma unroll
            for( int ni = 0; ni < MMAS_N; ++ni ) {
...
#pragma unroll
    ...
    template<typename Params>
    inline __device__ void
    move_to_first_block(const Params &params, int bidb, int bidh, int q_loop) {
        int scale_q_iter =
        ...
        params_scale_q_ *= reinterpret_cast<const float &>(params_scale_bmm1_);

        int scale_k_iter =
            bidb * params.h * params.sage.k.max_nblock + bidh * params.sage.k.max_nblock;
        params_scale_k_iter = reinterpret_cast<const float *>(params.sage.k.scales + scale_k_iter);
    ...
    const float *params_scale_k_iter;
};
...
struct Softmax_gmma_base<Traits_, Cta_tile_, Kernel_traits_, 1> {
    ...
    template<typename Params>
    inline __device__ Softmax_gmma_base(const Params &params, void *, const int, const int)
        : params_scale_bmm1_(params.scale_bmm1),
          params_softcapping_scale_bmm1_(params.softcapping_scale_bmm1) {
    }
...
#pragma unroll
                    for( int jj = 0; jj < TOTAL_ELTS_PER_THREAD; ++jj ) {
                        if( !mask.is_valid(mi, ni, ii, jj) ) {
                            this->elt_[ROWS_PER_THREAD * mi + ii][TOTAL_ELTS_PER_THREAD * ni + jj] =
                                -FLT_MAX;
                        }
                    }  // jj
                }      // ni
                        ...
                        for( int jj = 0; jj < TOTAL_ELTS_PER_THREAD; ++jj ) {
                            ...
                            if( mask.is_valid(row, col) ) {
                                ...
                                elt_[ROWS_PER_THREAD * mi + ii][TOTAL_ELTS_PER_THREAD * ni + jj] =
                                    elt_[ROWS_PER_THREAD * mi + ii]
                                        [TOTAL_ELTS_PER_THREAD * ni + jj] *
                                        alibi_params.scale_after_alibi +
                                    m * (col + alibi_params.sequence_pos_offset);
                            } else {
                                elt_[ROWS_PER_THREAD * mi + ii][TOTAL_ELTS_PER_THREAD * ni + jj] =
...
#pragma unroll
    ...
    // Apply the exp to all the elements.
    inline __device__ void apply_exp(const float (&max)[MMAS_M * ROWS_PER_THREAD]) {
#pragma unroll
        for( int mi = 0; mi < MMAS_M * ROWS_PER_THREAD; ++mi ) {
#pragma unroll
            for( int ni = 0; ni < MMAS_N * COLS_PER_THREAD * ELTS_PER_THREAD; ++ni ) {
                this->elt_[mi][ni] =
                    apply_exp_<Kernel_traits::VERSION>(this->elt_[mi][ni], max[mi]);
            }
        }
    }

    // Scale all the elements.
    inline __device__ void scale(const float (&sum)[MMAS_M * ROWS_PER_THREAD]) {
        // Precompute the inverse sum to normalize. Without -use_fast_math, it makes a huge deal.
...
struct Softmax_gmma_base<Traits, Cta_tile, Kernel_traits, 2>
    ...
    template<typename Params>
    inline __device__
    Softmax_gmma_base(const Params &params, void *smem, const int bidb, const int tidx)
        : Base(params, smem, bidb, tidx) {
...
struct Softmax<fmha::Hopper_hgmma_fp16_traits<GMMA_M, GMMA_N, GMMA_K, GMMA_A_RF, GMMA_B_RF>,
    ...
    : public Softmax_gmma_base<
          fmha::Hopper_hgmma_fp16_traits<GMMA_M, GMMA_N, GMMA_K, GMMA_A_RF, GMMA_B_RF>,
          Cta_tile_,
          Kernel_traits_,
          Cta_tile_::WARPS_N> {
...
#pragma unroll
                    for( int row_idx = 0; row_idx < ROWS_PER_THREAD; ++row_idx ) {
                        ...
                        uint32_t acc_0 =
                            fmha::hmul2(acc[mi][ni].reg(col_idx * ROWS_PER_THREAD + row_idx),
                                        this->params_scale_bmm1_);
                        // Element index.
                        int elt_row_idx = ROWS_PER_THREAD * mi + row_idx;
                        int elt_col_idx =
                            COLS_PER_THREAD * ELTS_PER_THREAD * ni + col_idx * ELTS_PER_THREAD;
                        // Extract the values as floats.
...
#pragma unroll
                    for( int row_idx = 0; row_idx < ROWS_PER_THREAD; ++row_idx ) {
                        // the order of the acc rf is we traverse vertically first
                        // then we traverse horizontally.
                        float tmp_00 = this->elt_[ROWS_PER_THREAD * mi + row_idx]
                                                 [COLS_PER_THREAD * ELTS_PER_THREAD * ni +
                                                  col_idx * ELTS_PER_THREAD + 0];
                        float tmp_01 = this->elt_[ROWS_PER_THREAD * mi + row_idx]
                                                 [COLS_PER_THREAD * ELTS_PER_THREAD * ni +
...
struct Softmax<fmha::Hopper_hgmma_fp32_traits<GMMA_M, GMMA_N, GMMA_K, GMMA_A_RF, GMMA_B_RF>,
               ...
               Kernel_traits_>
    : public Softmax_gmma_base<
          fmha::Hopper_hgmma_fp32_traits<GMMA_M, GMMA_N, GMMA_K, GMMA_A_RF, GMMA_B_RF>,
          Cta_tile_,
          Kernel_traits_,
          Cta_tile_::WARPS_N> {
    ...
    inline __device__ void unpack(const Accumulator (&acc)[MMAS_M][MMAS_N]) {
        ...
        const float &scale_f = reinterpret_cast<const float &>(this->params_scale_bmm1_);
#pragma unroll
        for( int mi = 0; mi < MMAS_M; ++mi ) {
#pragma unroll
            for( int ni = 0; ni < MMAS_N; ++ni ) {
...
#pragma unroll
                    for( int row_idx = 0; row_idx < ROWS_PER_THREAD; ++row_idx ) {
                        ...
                        float elt0 =
                            acc[mi][ni].elt(col_idx * 2 * ROWS_PER_THREAD + 2 * row_idx + 0) *
                            scale_f;
                        float elt1 =
                            acc[mi][ni].elt(col_idx * 2 * ROWS_PER_THREAD + 2 * row_idx + 1) *
                            scale_f;
...
struct Softmax<fmha::Hopper_hgmma_bf16_traits<GMMA_M, GMMA_N, GMMA_K, GMMA_A_RF, GMMA_B_RF>,
               ...
               Kernel_traits_>
    : public Softmax_gmma_base<
          fmha::Hopper_hgmma_bf16_traits<GMMA_M, GMMA_N, GMMA_K, GMMA_A_RF, GMMA_B_RF>,
          Cta_tile_,
          Kernel_traits_,
          Cta_tile_::WARPS_N> {
...
struct Softmax_gmma_32bit_8bit_base
    ...
    template<typename Params>
    inline __device__
    Softmax_gmma_32bit_8bit_base(const Params &params, void *smem, int bidb, int tidx)
        : Base(params, smem, bidb, tidx) {
    }
...
#pragma unroll
                for( int ii = 0; ii < COLS_PER_THREAD; ++ii ) {
                    float tmp_00 = acc[mi][ni].elt(ii * ROWS_PER_THREAD * ELTS_PER_THREAD +
                                                   0 * ELTS_PER_THREAD + 0) *
                                   scalef;
                    float tmp_01 = acc[mi][ni].elt(ii * ROWS_PER_THREAD * ELTS_PER_THREAD +
                                                   0 * ELTS_PER_THREAD + 1) *
                                   scalef;
                    float tmp_10 = acc[mi][ni].elt(ii * ROWS_PER_THREAD * ELTS_PER_THREAD +
                                                   1 * ELTS_PER_THREAD + 0) *
                                   scalef;
                    float tmp_11 = acc[mi][ni].elt(ii * ROWS_PER_THREAD * ELTS_PER_THREAD +
                                                   1 * ELTS_PER_THREAD + 1) *
                                   scalef;
...
#pragma unroll
    ...
    const uint32_t params_scale_softmax_;
};
...
struct Softmax_statistics {
    ...
    inline __device__
    Softmax_statistics(const Params &params, const void *ptr, const Binfo &binfo, int tidx)
        : ptr_(reinterpret_cast<const int8_t *>(ptr)), seqlen_(binfo.actual_seqlen) {
...
#pragma unroll
            for( int ii = 0; ii < 2; ++ii ) {
                ...
                float val = 0.0f;
                if( token < seqlen_ ) {
                    val = reinterpret_cast<const float *>(ptr_)[token];
                }
                lm_[2 * mi + ii] = val;
            }
        }
