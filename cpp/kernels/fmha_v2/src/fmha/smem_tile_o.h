...
struct Convert_from_fp16 {
    ...
    static inline __device__ uint32_t convert(const Accumulators &acc, int ii) {
...
struct Convert_from_fp32 {
    ...
    // Convert one pair of fp16 numbers.
    template<typename Accumulators>
    inline __device__ uint32_t convert(const Accumulators &acc, int ii) {
    ...
    static inline __device__ uint32_t convert(const Ampere_bf16_Accumulator &acc, int ii) {
...
struct Hmma_smem_tile_o {
    ...
    inline __device__ Hmma_smem_tile_o(void *smem, int tidx) {
        ...
        // The XOR params.
        const int XOR_MOD = 8 / ROW_PACKING;
...
#pragma unroll
            for( int jj = 0; jj < Cta_tile::WARPS_K; ++jj ) {
                ...
                int imm = ii * ROWS_PER_LDS * BYTES_PER_ROW * Cta_tile::WARPS_K +
                          jj * BYTES_PER_ROW_WITH_PACKING;
...
#pragma unroll
    ...
    inline __device__ void store_(const Accumulators (&acc)[M][N], int mi) {
...
struct Smem_tile_o<fmha::Turing_hmma_fp16_traits, Cta_tile>
    ...
    template<int M, int N>
    inline __device__ void store(const Accumulators_fp16 (&acc)[M][N], int mi) {
    ...
    template<int M, int N>
    inline __device__ void store(const Accumulators_fp32 (&acc)[M][N], int mi) {
...
struct Smem_tile_o<fmha::Ampere_hmma_bf16_bf16_traits, Cta_tile>
    ...
    template<int M, int N>
    inline __device__ void store(const Accumulators_bf16 (&acc)[M][N], int mi) {
        this->template store_<Convert_from_fp32>(acc, mi);
    }
};
...
struct Regs_to_rows {
    ...
    static inline __device__ void extract(const Acc &acc, uint4 &row0, uint4 &row1) {
...
struct Regs_to_rows<fmha::Ada_qmma_e4m3_fp16_traits> {
    ...
    template<typename Acc>
    static inline __device__ void extract(const Acc &acc, uint2 &row0, uint2 &row1) {
...
template<typename Acc>
inline __device__ void add4(uint4 &dst, const uint4 &src) {
    reinterpret_cast<Acc &>(dst.x) += reinterpret_cast<const Acc &>(src.x);
    reinterpret_cast<Acc &>(dst.y) += reinterpret_cast<const Acc &>(src.y);
    reinterpret_cast<Acc &>(dst.z) += reinterpret_cast<const Acc &>(src.z);
    reinterpret_cast<Acc &>(dst.w) += reinterpret_cast<const Acc &>(src.w);
}

template<typename Acc>
inline __device__ void add_vec(uint4 &dst, const uint4 &src) {
    add4<Acc>(dst, src);
}

template<>
inline __device__ void add_vec<uint16_t>(uint4 &dst, const uint4 &src) {
