...
class CircularBufferWithDataWriter : public CircularBufferWriter<DEPTH, CGA_SIZE> {
  ...
  public:
    ...
    push(int tid0, const T &wrdat, bool writeData = true, uint32_t transactioncnt = 0) {
    ...
    inline __device__ int
    push_with_sync(int tid0, const T &wrdat, bool writeData = true, uint32_t transactioncnt = 0) {
        int ptr = this->threadReserve();
