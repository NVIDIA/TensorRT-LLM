...
struct DMA {
    ...
    using Transposer =
        Transposer<typename Kernel_traits::Traits_o,
    ...
    struct Device {
        ...
        static inline __device__ std::pair<int, int>
        compute_kv_tile_idx(const bert::Fused_multihead_attention_params_v2 &params,
                            int q_step_offset,
                            int q_step_end,
                            int kv_steps) {
        ...
        run_packed_qkv(const bert::Fused_multihead_attention_params_v2 &params, Shared *shared) {
            ...
            while( tile_id_ < params.num_tiles ) {
                ...
                for( int q_step_idx = 0; q_step_idx < q_steps; q_step_idx += 2 ) {
                    load_q(bidh,
                           (q_step_idx + 0 + q_step_offset) * STEP_Q,
                           desc_q,
                           shared->smem_q[0],
                           cbw0);
                    load_q(bidh,
                           (q_step_idx + 1 + q_step_offset) * STEP_Q,
                           desc_q,
                           shared->smem_q[1],
                           cbw1);

                    // Q step bound is 2 tiles away at this moment because of 2x1 math warpgroup
                    const int q_step_end = (q_step_idx + q_step_offset + 2) * STEP_Q - 1;

                    // The kv tile idx range for this q step.
                    const auto [kv_idx_start, kv_idx_end] = compute_kv_tile_idx(
                    ...
                    for( int kv_step_idx = kv_idx_start; kv_step_idx < kv_idx_end; kv_step_idx++ ) {
                        int bar_id = load_kv(bidh / params.h_q_per_kv,
                                             kv_step_idx * STEP_KV,
                                             desc_k,
                                             desc_v,
                                             shared,
                        ...
                        if( q_step_idx == 0 && kv_step_idx == kv_idx_start ) {
                            ...
                            typename Shared::Head_info info{ q_steps,
                                                             ...
                                                             bidh,
                                                             bidb };
                            // NOTE(tizheng): The need for the sync after consumer bar wait is to avoid a deadlock hazard
        ...
        run_separate_q_and_kv(const bert::Fused_multihead_attention_params_v2 &params,
            ...
            while( tile_id_ < params.num_tiles ) {
                ...
                for( int q_step_idx = 0; q_step_idx < q_steps; q_step_idx += 2 ) {
                    ...
                    const int q_step_end = (q_step_idx + 2) * STEP_Q - 1 + q_tile_offset;
...
#pragma unroll
            for( int di = 0; di < Kernel_traits::D_GROUPS; ++di ) {
                const int32_t coords[3] = { di * Kernel_traits::D_PER_GROUP,
                                            bidh,
...
#define PREPARE_KV_BUFFER()                                                                        \
    ...
    named_barrier_wait(SYNC_BARRIER, NUM_THREADS_IN_DMA_GROUP);
        ...
        inline __device__ int load_kv(int bidh_kv,
...
#pragma unroll
            for( int di = 0; di < Kernel_traits::D_GROUPS; ++di ) {
                const int32_t k_coords[3] = { di * Kernel_traits::D_PER_GROUP,
                                              bidh_kv,
...
#pragma unroll
            for( int di = 0; di < Kernel_traits::DV_GROUPS; ++di ) {
                const int32_t v_coords[3] = { di * Kernel_traits::D_PER_GROUP,
                                              bidh_kv,
                                              sum_s_kv_ + kv_tile_start_offset };
                ...
                fmha::utmaldg<3, fmha::cudaTmaDescType::TILED, false>(
                    ...
                    __cvta_generic_to_shared(v_barrier_ptr),
                    v_coords,
                    elect_one_);
            }
        ...
        template<typename BufferWriter, typename BufferWriterScratch>
        inline __device__ int load_paged_kv(int bidh_kv,
                                            int kv_tile_start_offset,
...
#pragma unroll
    ...
    struct Host {
        ...
        void init_params(bert::Fused_multihead_attention_params_v2 &params,
            ...
            const auto layout = launch_params.attention_input_layout;
