...
struct DMA {
    ...
    using Circular_buffer_v_scratch_reader =
        typename Kernel_traits::Circular_buffer_v_scratch_reader;
    using Circular_buffer_v_scratch_writer =
        typename Kernel_traits::Circular_buffer_v_scratch_writer;
    ...
    using Transposer = Transposer<typename Kernel_traits::Traits_o,
    ...
    struct Device {
        ...
        run_packed_qkv(const bert::Fused_multihead_attention_params_v2 &params, Shared *shared) {
            // DMA.
            ...
            Circular_buffer_v_scratch_reader cbr_v_scratch =
                shared->tma_v_scratch_tracker.createReader();
            Circular_buffer_v_scratch_writer cbw_v_scratch =
                shared->tma_v_scratch_tracker.createWriter();
            ...
            while( tile_id_ < params.num_tiles ) {
                ...
                if( SCHEDULING_MODE == 0 ) {
                    ...
                    if( CAUSAL_MASK && !SLIDING_WINDOW_ATTENTION &&
                ...
                const cudaTmaDesc *desc_kv = &params.tma_desc_kv;
                int actual_seqlen;
                ...
                for( int q_step_idx = 0; q_step_idx < q_steps; q_step_idx += 2 ) {
                    ...
                    const int kv_idx_start = SLIDING_WINDOW_ATTENTION
                                                 ? (max(0,
                                                        (q_step_idx + q_step_offset) * STEP_Q -
                                                            params.sliding_window_size) /
                                                    STEP_KV)
                                                 : 0;

                    // Q step bound is 2 tiles away at this moment because of 2x1 math warpgroup
                    const int q_step_bound = (q_step_idx + q_step_offset + 2) * STEP_Q - 1;

                    // Early stop when causal mask is enabled.
                    const int kv_idx_end = SKIP_CAUSAL_MASK_TILES
                                               ? ((q_step_bound + STEP_KV - 1) / STEP_KV)
                                               : kv_steps;

                    for( int kv_step_idx = kv_idx_start; kv_step_idx < kv_idx_end; kv_step_idx++ ) {
                        int bar_id = load_kv(bidh,
                                             params.h,
                                             params.h_kv,
                                             kv_step_idx,
                                             desc_kv,
                        ...
                        if( q_step_idx == 0 && kv_step_idx == kv_idx_start ) {
                            ...
                            // then MMA warps have finished producer bar wait phase 0 and consumer bar arrive phase 1. At
                            // this time other DMA threads start consumer bar wait phase 0. It will never become ready.
                            // DMA warps then fail to continue to the next loop.
                            //
                            // It is the same consideration for the sync after tmaReserve in load_q and load_kv implementation
                            // below.
                            headinfo_tracker0
                                .template push_with_sync<SYNC_BARRIER, NUM_THREADS_IN_DMA_GROUP>(
                                    elect_one_, info);
                            headinfo_tracker1
                                .template push_with_sync<SYNC_BARRIER, NUM_THREADS_IN_DMA_GROUP>(
                                    elect_one_, info);
        ...
        inline __device__ int remap_kv_tile_idx(int kv_tile_idx,
                                                int num_kv_cache_tiles,
                                                int past_kv_length,
                ...
                if( past_kv_length > sliding_window_size ) {
                    ...
                    if( kv_tile_idx * STEP_KV >= past_kv_length ) {
                        remapped_kv_tile_idx =
                            num_kv_cache_tiles +
                            int((kv_tile_idx * STEP_KV - past_kv_length) / STEP_KV);
                    } else {
        ...
        inline __device__ void
        run_separate_q_and_kv(const bert::Fused_multihead_attention_params_v2 &params,
                              Shared *shared) {
            // DMA.
            int local_wid = (threadIdx.x / 32) % 4;
            int tiw = threadIdx.x % 32;
            ...
            while( tile_id_ < params.num_tiles ) {
                ...
                if( SCHEDULING_MODE == 0 ) {
                    ...
                    bidh = tmp / params.num_tiles_per_head;
                    local_q_tile_offset =
                        (tmp % params.num_tiles_per_head) * NUM_COMPUTE_GROUPS * STEP_Q;
                    q_steps = NUM_COMPUTE_GROUPS;
                ...
                const cudaTmaDesc *desc_q = &params.tma_desc_q;
                const int32_t *paged_block_offsets =
                    params.paged_kv_cache.mBlockOffsets +
                    bidb * 2 * params.paged_kv_cache.mMaxBlocksPerSeq;
                const cudaTmaDesc *desc_kv = &params.tma_desc_kv;
                ...
                for( int q_step_idx = 0; q_step_idx < q_steps; q_step_idx += 2 ) {
                    ...
                    const int kv_idx_start = SLIDING_WINDOW_ATTENTION
                                                 ? (max(0,
                                                        q_step_idx * STEP_Q + q_tile_offset -
                                                            params.sliding_window_size) /
                                                    STEP_KV)
                                                 : 0;

                    // Q step bound is 2 tiles away at this moment because of 2x1 math warpgroup
                    const int q_step_bound = (q_step_idx + 2) * STEP_Q - 1 + q_tile_offset;
                    ...
                    for( int kv_step_idx = kv_idx_start; kv_step_idx < kv_idx_end; kv_step_idx++ ) {
                        ...
                        int remapped_kv_step_idx =
                            remap_kv_tile_idx(kv_step_idx,
                                              params.sliding_window_size / STEP_KV,
                                              past_kv_length,
                                              params.sliding_window_size);
                        // The barrier id.
                        int bar_id;
                        ...
                        if constexpr( PAGED_KV_INPUT ) {
                            bar_id = load_paged_kv(bidh_kv,
                                                   remapped_kv_step_idx * STEP_KV,
                                                   num_valid_kv_blocks,
                                                   params.paged_kv_cache.mTokensPerBlockLog2,
                                                   params.blocks_per_tma_load,
                                                   params.blocks_per_tma_load_log2,
                                                   params.paged_kv_cache.mMaxBlocksPerSeq,
                                                   paged_block_offsets,
                                                   desc_kv,
                                                   shared,
                                                   cbw_k,
                            ...
                            bar_id = load_contiguous_kv(bidh,
                                                        params.h,
                                                        params.h_kv,
                                                        remapped_kv_step_idx,
                                                        desc_kv,
                                                        shared,
                                                        cbw_k,
                                                        cbw_v,
        ...
        inline __device__ void load_q(int bidh,
            ...
            const int32_t q_coord_dim1 = !HEADS_INTERLEAVED || multi_query_attention_ ? bidh : 0;
            const int32_t q_coord_dim2 = !HEADS_INTERLEAVED || multi_query_attention_ ? 0 : bidh;
#pragma unroll
            for( int di = 0; di < Kernel_traits::D_GROUPS; ++di ) {
                const int32_t coords[4] = { di * Kernel_traits::D_PER_GROUP,
                                            q_coord_dim1,
                                            q_coord_dim2,
                                            sum_s_q_ + q_step_idx * STEP_Q };
        ...
        template<typename BufferWriter, typename Smem_q>
        inline __device__ void load_separate_q(int bidh,
                                               int q_tile_start_offset,
                                               const cudaTmaDesc *desc_q,
                                               Smem_q &smem_q,
                                               BufferWriter &cbw) {
...
#pragma unroll
            for( int di = 0; di < Kernel_traits::D_GROUPS; ++di ) {
                const int32_t coords[4] = {
                    di * Kernel_traits::D_PER_GROUP, bidh, 0, sum_s_q_ + q_tile_start_offset
                };
                fmha::utmaldg<4, fmha::cudaTmaDescType::TILED, false>(
                    desc_q,
                    __cvta_generic_to_shared(
        ...
        inline __device__ void load_kv_impl(int bidh,
                                            int h,
                                            int h_kv,
                                            int kv_step_idx,
                                            const cudaTmaDesc *desc_kv,
                                            Shared *shared,
                                            BufferWriter &cbw_k,
                                            BufferWriter &cbw_v) {

            int k_barrier_id =
                cbw_k.tmaReserve(elect_one_, (TILE_SIZE_K)*Kernel_traits::ELEMENT_BYTES);

            int v_barrier_id =
                cbw_v.tmaReserve(elect_one_, (TILE_SIZE_V)*Kernel_traits::ELEMENT_BYTES);
...
#pragma unroll
            for( int di = 0; di < Kernel_traits::D_GROUPS; ++di ) {
                const int32_t k_coords[4] = { di * Kernel_traits::D_PER_GROUP,
                                              multi_query_attention_ ? h + bidh / (h / h_kv)
                                                                     : k_coord_dim1,
                                              multi_query_attention_ ? 0 : k_coord_dim2,
                                              sum_s_q_ + kv_step_idx * STEP_KV };
                ...
                const int32_t v_coords[4] = { di * Kernel_traits::D_PER_GROUP,
                                              multi_query_attention_ ? h + h_kv + bidh / (h / h_kv)
                                                                     : v_coord_dim1,
        ...
        template<typename BufferWriter>
        inline __device__ void load_contiguous_kv_impl(int bidh,
                                                       int h,
                                                       int h_kv,
                                                       int kv_step_idx,
                                                       const cudaTmaDesc *desc_kv,
...
#pragma unroll
        ...
        // Load k,v tiles from gmem to smem by TMA.
        template<typename BufferWriter>
        inline __device__ void load_paged_kv_impl(int bidh,
                                                  int kv_tile_start_offset,
                                                  int num_valid_kv_blocks,
                                                  int tokens_per_block_log2,
            ...
            // coordinates: d, s, h, 1
            const int tile_size_k_per_block = TILE_SIZE_K_PER_D_GROUP >> blocks_per_tma_load_log2;
            ...
            for( int bi = 0; bi < blocks_per_tma_load; ++bi ) {
                const int bounded_block_idx = min(num_valid_kv_blocks - 1, paged_kv_block_idx + bi);
...
#pragma unroll
        ...
        inline __device__ void transpose_v_tile(int v_scratch_barrier_id,
                                                Shared *shared,
                                                BufferWriter &cbw_v,
            ...
            uint32_t smem_v_src =
                __cvta_generic_to_shared(&shared->smem_v_scratch[v_scratch_barrier_id]);
...
#pragma unroll
                for( int dgroup_idx = 0; dgroup_idx < Kernel_traits::D_GROUPS; dgroup_idx++ ) {
                    ...
                    transposer.template transpose_<false>(smem_v_src + src_offset,
                                                          smem_v_dst + dst_offset);
                }
            }
        ...
        inline __device__ int load_kv_transpose_v_impl(int bidh,
                                                       int h,
                                                       int h_kv,
                                                       int kv_step_idx,
                                                       const cudaTmaDesc *desc_kv,
...
#pragma unroll
            for( int di = 0; di < Kernel_traits::D_GROUPS; ++di ) {
                ...
                fmha::utmaldg<4, fmha::cudaTmaDescType::TILED, false>(
                    desc_kv,
                    __cvta_generic_to_shared(
                        &shared->smem_v_scratch[v_scratch_barrier_id * TILE_SIZE_V +
                                                di * TILE_SIZE_V_PER_D_GROUP]),
                    __cvta_generic_to_shared(cbw_v_scratch.barrier_ptr(v_scratch_barrier_id)),
                    v_coords,
                    elect_one_);
            }
        ...
        load_contiguous_kv_transpose_v_impl(int bidh,
                                            int h,
                                            int h_kv,
                                            int kv_step_idx,
                                            const cudaTmaDesc *desc_kv,
...
#pragma unroll
        ...
        template<typename BufferWriter, typename BufferWriterScratch, typename BufferReaderScratch>
        inline __device__ int load_paged_kv_transpose_v_impl(int bidh,
                                                             int kv_tile_start_offset,
                                                             int num_valid_kv_blocks,
                                                             int tokens_per_block_log2,
                                                             int blocks_per_tma_load,
...
#pragma unroll
        ...
        // Load k,v tiles from gmem to smem by TMA.
        template<typename BufferWriter, typename BufferWriterScratch, typename BufferReaderScratch>
        inline __device__ int load_kv(int bidh,
                                      int h,
                                      int h_kv,
                                      int kv_step_idx,
                                      const cudaTmaDesc *desc_kv,
            ...
            if constexpr( DMA_GROUP_TRANSPOSE_V ) {
                int v_scratch_barrier_id = load_kv_transpose_v_impl(bidh,
                                                                    h,
                                                                    h_kv,
                                                                    kv_step_idx,
                                                                    desc_kv,
        ...
        // Load contiguous kv tiles [B, S, 2, H, D] from gmem to smem by TMA.
        template<typename BufferWriter, typename BufferWriterScratch, typename BufferReaderScratch>
        inline __device__ int load_contiguous_kv(int bidh,
                                                 int h,
                                                 int h_kv,
                                                 int kv_step_idx,
                                                 const cudaTmaDesc *desc_kv,
            ...
            if constexpr( DMA_GROUP_TRANSPOSE_V ) {
                int v_scratch_barrier_id = load_contiguous_kv_transpose_v_impl(bidh,
                                                                               h,
                                                                               h_kv,
                                                                               kv_step_idx,
                                                                               desc_kv,
        ...
        // Load paged k,v tiles from gmem to smem by TMA.
        template<typename BufferWriter, typename BufferWriterScratch, typename BufferReaderScratch>
        inline __device__ int load_paged_kv(int bidh,
                                            int kv_tile_start_offset,
                                            int num_valid_kv_blocks,
                                            int tokens_per_block_log2,
                                            int blocks_per_tma_load,
            ...
            if constexpr( DMA_GROUP_TRANSPOSE_V ) {
                int v_scratch_barrier_id = load_paged_kv_transpose_v_impl(bidh,
                                                                          kv_tile_start_offset,
                                                                          num_valid_kv_blocks,
                                                                          tokens_per_block_log2,
                                                                          blocks_per_tma_load,
                                                                          blocks_per_tma_load_log2,
        ...
        inline __device__ void get_next_tile_id(int local_wid,
                                                int tiw,
                                                uint32_t smem_tile_id,
        ...
        void init_params(bert::Fused_multihead_attention_params_v2 &params,
            ...
            if( launch_params.attention_input_layout == fmha::Attention_input_layout::PACKED_QKV ) {
                ...
                static constexpr fmha::cudaTmaDescFormat desc_format =
                    (Kernel_traits::ELEMENT_BYTES == 1) ? fmha::cudaTmaDescFormat::U8
                ...
                if( launch_params.attention_input_layout ==
                    fmha::Attention_input_layout::CONTIGUOUS_Q_KV ) {

                    // Total sequence length.
                    const int total_seqlen =
                        params.is_s_padded ? (params.b * params.s) : launch_params.total_kv_seqlen;
                    ...
                    contiguous_kv_tma_descriptor.set_tma_desctriptor(
                        reinterpret_cast<char *>(params.kv_ptr),
                        desc_format,
                        fmha::cudaTmaDescInterleave::INTERLEAVE_DISABLED,
                        swizzle_mode,
                        fmha::cudaTmaDescPromotion::PROMOTION_DISABLED,
                    ...
                    box_size_kv[2] = 1;
                    box_size_kv[1] =
                        std::min(params.paged_kv_cache.mTokensPerBlock, int32_t(STEP_KV));
                    box_size_kv[0] = Kernel_traits::D_PER_GROUP;

                    assert(int32_t(STEP_KV) % params.paged_kv_cache.mTokensPerBlock == 0 ||
                           params.paged_kv_cache.mTokensPerBlock % int32_t(STEP_KV) == 0);
                    params.blocks_per_tma_load =
                        std::max(1, int32_t(STEP_KV) / params.paged_kv_cache.mTokensPerBlock);
                    ...
                    paged_kv_tma_descriptor.set_tma_desctriptor(
                        reinterpret_cast<char *>(params.paged_kv_cache.mPoolPtr),
                        desc_format,
                        fmha::cudaTmaDescInterleave::INTERLEAVE_DISABLED,
                        swizzle_mode,
                        fmha::cudaTmaDescPromotion::PROMOTION_DISABLED,
