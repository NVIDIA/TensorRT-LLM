...
struct Gmem_tile_qkv {
    ...
    inline __device__ Gmem_tile_qkv(void *qkv_ptr,
                                    ...
                                    int cta_row_offset = 0,
                                    int cta_col_offset_in_bytes = 0)
        : params_qkv_stride_in_bytes_(qkv_stride_in_bytes), actual_seqlen_(binfo.actual_seqlen),
        ...
        if( num_kv_heads < num_heads ) {
            ...
            if( qkv_offset == 0 ) {         // Q tensor
                ...
                idx += (num_heads + num_kv_heads) * d * BYTES_PER_ELEMENT +
                       kv_head_id * VALID_BYTES_PER_ROW;
            }
...
struct Gmem_tile_q {
    ...
    template<typename Block_info>
    inline __device__ Gmem_tile_q(const bert::Fused_multihead_attention_params_v2 &params,
                                  int qkv_offset,
                                  const Block_info &binfo,
                                  int tidx,
                                  int cta_row_offset = 0,
    ...
    inline __device__ Gmem_tile_q(const bert::Fused_multihead_attention_params_v2 &params,
        ...
        : params_q_stride_in_bytes_(params.q_stride_in_bytes),
          actual_seqlen_(binfo.actual_q_seqlen), q_ptr_(reinterpret_cast<char *>(params.q_ptr)) {
...
struct Gmem_tile_contiguous_kv {
    ...
    template<typename Block_info>
    inline __device__
    Gmem_tile_contiguous_kv(const bert::Fused_multihead_attention_params_v2 &params,
                            int qkv_offset,  // q = 0, k = 1, v = 2.
                            const Block_info &binfo,
                            int tidx,
                            int cta_row_offset = 0,
                            int cta_col_offset_in_bytes = 0)
        : Gmem_tile_contiguous_kv(params.kv_ptr,
                                  params.kv_stride_in_bytes,
                                  params.h_kv,
                                  params.h_q_per_kv,
                                  qkv_offset,
                                  binfo,
    ...
    // Ctor.
    template<typename Block_info>
    inline __device__ Gmem_tile_contiguous_kv(void *kv_ptr,
                                              size_t kv_stride_in_bytes,
                                              int num_kv_heads,
                                              int head_group_size,
                                              int qkv_offset,
                                              const Block_info &binfo,
                                              ...
                                              int cta_row_offset = 0,
                                              int cta_col_offset_in_bytes = 0)
        : params_kv_stride_in_bytes_(kv_stride_in_bytes), actual_seqlen_(binfo.actual_kv_seqlen),
...
struct Gmem_tile_paged_kv {
    ...
    // Ctor.
    template<typename Block_info>
    inline __device__ Gmem_tile_paged_kv(const bert::Fused_multihead_attention_params_v2 &params,
                                         int qkv_offset,  // q = 0, k = 1, v = 2.
                                         const Block_info &binfo,
                                         int tidx,
                                         int cta_row_offset = 0,
        ...
        const int32_t paged_kv_block_offset =
        ...
        // The head offset.
        head_stride_in_bytes_ =
            (int64_t)(binfo.bidh / params.h_q_per_kv) * params.kv_stride_in_bytes;
    ...
    inline __device__ void load(Smem_tile &smem_tile) {
        ...
        const uint64_t head_col_in_bytes = head_stride_in_bytes_ + col_in_bytes_;
...
#pragma unroll
        for( int ii = 0; ii < LDGS; ++ii ) {
            ...
            const char *local_kv_ptr = reinterpret_cast<char *>(
                paged_kv_block_pool_ptr_ + params_kv_block_size_in_bytes_ *
                                               paged_kv_global_block_offsets_[paged_kv_block_idx]);
            ...
            ptrs[ii] = local_kv_ptr + head_col_in_bytes +
                       (int64_t)row_idx_in_block * token_stride_in_bytes_;
        }
...
struct Gmem_tile_q_kv {
    ...
    inline __device__ Gmem_tile_q_kv(const Params &params,
                                     ...
                                     int tidx,
                                     int cta_row_offset = 0)
        : params_stride_in_bytes_(params.stride_in_bytes), actual_seqlen_(binfo.actual_seqlen),
...
struct Gmem_tile_qkv_interleaved {
    ...
    inline __device__ Gmem_tile_qkv_interleaved(const Params &params,
        ...
        : actual_seqlen_(block_info.actual_seqlen - cta_row_offset),
          ...
          kv_ptr_(reinterpret_cast<const char *>(params.qkv_ptr)) {
