...
struct Store_accumulator<Traits, 16> {
    ...
    store(char *ptr, int64_t step_m, int64_t step_n, const Acc &acc, uint32_t scale) {
...
struct Store_accumulator<Traits, 32> {
    ...
    // Store.
    inline __device__ void
    store(char *ptr, int64_t step_m, int64_t step_n, const Accumulator &acc, uint32_t) {
        ...
        uint32_t acc_0 = reinterpret_cast<const uint32_t &>(tmp_0);
        uint32_t acc_1 = reinterpret_cast<const uint32_t &>(tmp_1);
        uint32_t acc_2 = reinterpret_cast<const uint32_t &>(tmp_2);
        uint32_t acc_3 = reinterpret_cast<const uint32_t &>(tmp_3);
        uint32_t acc_4 = reinterpret_cast<const uint32_t &>(tmp_4);
        uint32_t acc_5 = reinterpret_cast<const uint32_t &>(tmp_5);
        uint32_t acc_6 = reinterpret_cast<const uint32_t &>(tmp_6);
        uint32_t acc_7 = reinterpret_cast<const uint32_t &>(tmp_7);
...
struct Store_accumulator<Ampere_hmma_fp32_traits, 32> {
    ...
    // Store.
    inline __device__ void
    store(char *ptr, int64_t step_m, int64_t step_n, const Accumulator &acc, uint32_t scale) {

        const float scalef = reinterpret_cast<const float &>(scale);

        const float tmp_0 = acc.elt(0) * scalef;
        const float tmp_1 = acc.elt(1) * scalef;
        const float tmp_2 = acc.elt(2) * scalef;
        const float tmp_3 = acc.elt(3) * scalef;
        const float tmp_4 = acc.elt(4) * scalef;
        const float tmp_5 = acc.elt(5) * scalef;
        const float tmp_6 = acc.elt(6) * scalef;
        const float tmp_7 = acc.elt(7) * scalef;
...
struct Store_accumulator<fmha::Hopper_qgmma_fp8_fp32_traits<GMMA_M,
                                                            GMMA_N,
                                                            GMMA_K,
                                                            GMMA_A_RF,
                                                            GMMA_B_RF,
                         ...
                         32> {
    // The traits.
    using Traits = fmha::Hopper_qgmma_fp8_fp32_traits<GMMA_M,
                                                      GMMA_N,
                                                      GMMA_K,
                                                      GMMA_A_RF,
                                                      GMMA_B_RF,
...
#pragma unroll
            for( int row_idx = 0; row_idx < ROWS_PER_THREAD; ++row_idx ) {
                const float acc_0 = acc.elt((col_idx * ROWS_PER_THREAD + row_idx) * 2 + 0) * scalef;
                const float acc_1 = acc.elt((col_idx * ROWS_PER_THREAD + row_idx) * 2 + 1) * scalef;
                uint2 acc_;
                acc_.x = reinterpret_cast<const uint32_t &>(acc_0);
                acc_.y = reinterpret_cast<const uint32_t &>(acc_1);
                int64_t offset = (int64_t)row_idx * step_m + (int64_t)col_idx * step_n;
...
#pragma unroll
            for( int row_idx = 0; row_idx < ROWS_PER_THREAD; ++row_idx ) {
                const int32_t acc_0 = acc.elt((col_idx * ROWS_PER_THREAD + row_idx) * 2 + 0);
                const int32_t acc_1 = acc.elt((col_idx * ROWS_PER_THREAD + row_idx) * 2 + 1);
                uint2 acc_;
                acc_.x = reinterpret_cast<const uint32_t &>(acc_0);
                acc_.y = reinterpret_cast<const uint32_t &>(acc_1);
                int64_t offset = (int64_t)row_idx * step_m + (int64_t)col_idx * step_n;
...
static __device__ inline uint16_t pack_e4m3x2(const float x, const float y) {
...
static __device__ inline uint16_t pack_e5m2x2(const float x, const float y) {
#if( defined(__CUDA_ARCH__) && (__CUDA_ARCH__ >= 900) )
    uint16_t storage;
...
__device__ inline uint16_t pack_fp8x2(const float x, const float y);

template<>
__device__ inline uint16_t pack_fp8x2<fmha::e4m3_t>(const float x, const float y) {
    return pack_e4m3x2(x, y);
}

template<>
__device__ inline uint16_t pack_fp8x2<fmha::e5m2_t>(const float x, const float y) {
...
#pragma unroll
            for( int row_idx = 0; row_idx < ROWS_PER_THREAD; ++row_idx ) {
                const float acc_0 = acc.elt((col_idx * ROWS_PER_THREAD + row_idx) * 2 + 0);
                const float acc_1 = acc.elt((col_idx * ROWS_PER_THREAD + row_idx) * 2 + 1);
...
#pragma unroll
            for( int row_idx = 0; row_idx < ROWS_PER_THREAD; ++row_idx ) {
                const uint32_t acc_0 = acc.reg((col_idx * ROWS_PER_THREAD + row_idx) * 2 + 0);
                const uint32_t acc_1 = acc.reg((col_idx * ROWS_PER_THREAD + row_idx) * 2 + 1);
...
struct Gmem_tile_ps {
    ...
    // Ctor.
    inline __device__ Gmem_tile_ps(void *ptr,
                                   const int64_t params_stride_in_bytes,
                                   const uint32_t params_scale,
                                   int tidx,
                                   int cta_row_offset = 0)
        : params_stride_in_bytes_(params_stride_in_bytes), params_scale_(params_scale),
    ...
    inline __device__ void store(const Accumulators (&acc)[M][N]) {
        ...
        const int64_t step_m = 8 * params_stride_in_bytes_;
        const int64_t step_n = 8 * BYTES_PER_ELEMENT;
...
#pragma unroll
            for( int ni = 0; ni < N; ++ni ) {
                int64_t offset =
                    (int64_t)mi * Mma_tile::M_PER_MMA_PER_CTA * params_stride_in_bytes_ +
    ...
    const int64_t params_stride_in_bytes_;
    // The scale to apply before storing the element.
    const uint32_t params_scale_;
    // The pointer.
    char *ptr_;
};
...
struct Gmem_tile_p : public Gmem_tile_ps<Traits, Cta_tile, BITS_PER_ELEMENT> {
    ...
    // Ctor.
    inline __device__ Gmem_tile_p(void *ptr,
                                  const int64_t params_stride_in_bytes,
                                  const uint32_t params_scale,
                                  int tidx,
                                  int cta_row_offset = 0)
...
struct Gmem_tile_ps_hopper {
    ...
    // Ctor.
    inline __device__ Gmem_tile_ps_hopper(void *ptr,
                                          const int64_t params_stride_in_bytes,
    ...
    inline __device__ void store(const Accumulators (&acc)[M][N]) {
        ...
        const int64_t step_m = 8 * (this->params_stride_in_bytes_);
        const int64_t step_n = 8 * BYTES_PER_ELEMENT;
...
#pragma unroll
            for( int ni = 0; ni < N; ++ni ) {
                ...
                Store_accumulator<Traits, BITS_PER_ELEMENT> delegate;
                delegate.store(
                    this->ptr_ + offset, step_m, step_n, acc[mi][ni], this->params_scale_);
            }
        }
    }
...
struct Gmem_tile_s : public Gmem_tile_ps<Traits, Cta_tile, BITS_PER_ELEMENT> {
    ...
    // Ctor.
    inline __device__ Gmem_tile_s(void *ptr,
                                  const int64_t params_stride_in_bytes,
                                  const uint32_t params_scale,
                                  int tidx)
...
struct Gmem_tile_s<Ampere_hmma_fp32_traits, Cta_tile, BITS_PER_ELEMENT>
    ...
    inline __device__ Gmem_tile_s(void *ptr,
                                  ...
                                  int cta_row_offset = 0)
        : Base(ptr,
               params_stride_in_bytes,
               float_to_half2(reinterpret_cast<const float &>(params_scale)),
               tidx,
               cta_row_offset) {
    }
};
