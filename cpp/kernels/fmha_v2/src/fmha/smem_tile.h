...
struct Smem_tile_without_skews {
    ...
    using Smem_tile_ = Smem_tile_without_skews<Cta_tile,
                                               M_,
                                               N_,
                                               BITS_PER_ELEMENT_,
                                               BYTES_PER_STS_,
            ...
            for( int row = 0; row < ROWS; ++row ) {
                for( int col = 0; col < BYTES_PER_ROW; col += 4 ) {
                    if( threadIdx.x == 0 ) {
                        ...
                        printf("block=(x=%2d, y=%2d, z=%2d) (smem_=0x%08x, buffer=%2d, row=%2d, "
                               "byte=%4d)=0x%08x\n",
                               blockIdx.x,
                               blockIdx.y,
                               blockIdx.z,
...
struct Smem_tile_without_skews<Cta_tile,
    ...
    inline __device__ void store(const void *p_desc,
                                 ...
                                 bool mcast_enabled) {
        uint32_t smem = this->smem_write_offset_ + smem_offset;
        fmha::utmaldg<DIM, DESC_TYPE>(reinterpret_cast<const cudaTmaDesc *>(p_desc),
                                      smem,
                                      unsigned(this->smem_barrier_offset_),
                                      coord0,
                                      coord1,
...
struct Smem_tile_turing_row_a : public Smem_tile_without_skews<Cta_tile,
    ...
    inline __device__ Smem_tile_turing_row_a(void *smem, int tidx) : Base(smem, tidx) {
        ...
        if( Base::ROWS_PER_XOR_PATTERN == 8 ) {
            smem_read_row =
                (tidx & WARP_MASK_M) / WARP_DIV_M * Mma_tile::M_PER_MMA / 1 + (tidx & 0x0f);
...
struct Smem_tile_turing_col_b : public Smem_tile_without_skews<Cta_tile,
    ...
    inline __device__ Smem_tile_turing_col_b(void *smem, int tidx) : Base(smem, tidx) {
        ...
        if( Base::ROWS_PER_XOR_PATTERN == 8 ) {
            ...
            smem_read_row =
                (tidx & WARP_MASK_N) / WARP_DIV_N * Mma_tile::N_PER_MMA / 1 + (tidx & 0x0f);
            smem_read_col = (tidx & 0x07);
        } else if( Base::ROWS_PER_XOR_PATTERN == 4 ) {
            smem_read_row =
...
struct Smem_tile_ampere_row_b : public Smem_tile_without_skews<Cta_tile,
    ...
    inline __device__ Smem_tile_ampere_row_b(void *smem, int tidx) : Base(smem, tidx) {
        ...
        if( USE_LDSMT && Base::ROWS_PER_XOR_PATTERN == 8 ) {
            ...
            smem_read_row =
                (tidx & WARP_MASK_K) / WARP_DIV_K * Mma_tile::MMAS_K * 8 + (tidx & 0x03);
