...
struct Hmma_gmem_tile_o {
    ...
    inline __device__ Hmma_gmem_tile_o(const Params &params,
                                       const Block_info &binfo,
                                       int tidx,
                                       int cta_row_offset,
                                       int cta_col_offset_in_bytes = 0)
        : params_o_stride_in_bytes_(params.o_stride_in_bytes),
          actual_seqlen_(binfo.actual_q_seqlen), o_ptr_(reinterpret_cast<char *>(params.o_ptr)) {
        ...
        const int valid_bytes_per_row = params.dv * BYTES_PER_ELEMENT;
...
#pragma unroll
    ...
    store(const uint4 (&src)[STGS_PER_LOOP], const uint4 (&old)[STGS_PER_LOOP], int mi) {
...
struct Gmem_tile_o<fmha::Ampere_hmma_fp32_traits, Cta_tile, CTAS_PER_HEAD>
    ...
    // Load data from global memory.
    inline __device__ void load(const uint4 (&dst)[STGS_PER_LOOP], int mi) {
...
inline __device__ uint32_t quantize(const int4 val,
                                    const float scale,
                                    const bool params_enable_i2f_trick) {
    // Extract the floats and scale.
    float f0, f1, f2, f3;
#if defined(USE_I2F_EMULATION_TRICK)
    if( params_enable_i2f_trick ) {

        f0 = reinterpret_cast<const float &>(val.x) - FP32_I2F_MAGIC_NUMBER;
        f1 = reinterpret_cast<const float &>(val.y) - FP32_I2F_MAGIC_NUMBER;
        f2 = reinterpret_cast<const float &>(val.z) - FP32_I2F_MAGIC_NUMBER;
        f3 = reinterpret_cast<const float &>(val.w) - FP32_I2F_MAGIC_NUMBER;
...
struct Acc_packer<int32_t, int8_t, true> {
    ...
    static inline __device__ uint32_t run(const This *this_, const uint4 &src_regs) {
        const float &scale = reinterpret_cast<const float &>(this_->params_scale_bmm2_);
        // The accumulators are in int32_t.
        const int4 &val = reinterpret_cast<const int4 &>(src_regs);
...
struct Acc_packer<float, fmha::e4m3_t, true> {
    ...
    static inline __device__ uint32_t run(const This *this_, const uint4 &src_regs) {
        ...
        const float4 &val = reinterpret_cast<const float4 &>(src_regs);
    ...
    template<typename This>
    static inline __device__ uint16_t run(const This *this_, const uint2 &src_regs) {

        const float &scale = reinterpret_cast<const float &>(this_->params_scale_bmm2_);

        const float2 &val = reinterpret_cast<const float2 &>(src_regs);
...
struct Acc_packer<uint16_t, fmha::e4m3_t, true> {
    ...
    template<typename This>
    static inline __device__ uint2 run(const This *this_, const uint4 &src_regs) {
...
struct Acc_packer<float, uint16_t, true> {
    ...
    static inline __device__ uint2 run(const This *this_, const uint4 &src_regs) {
        ...
        uint2 dst =
            fmha::float4_to_half4(val.x * scale, val.y * scale, val.z * scale, val.w * scale);
        return dst;
    }
};
...
struct Acc_packer<float, nv_bfloat16, true> {
    ...
    static inline __device__ uint2 run(const This *this_, const uint4 &src_regs) {
        ...
        uint2 dst = fmha::float4_to_16bit_x4<bf16_t>(
...
struct Gmem_tile_o_8bit {
    ...
    // Ctor.
    template<typename Params, typename Block_info>
    inline __device__ Gmem_tile_o_8bit(const Params &params,
                                       const Block_info &block_info,
                                       int tidx,
                                       int cta_row_offset = 0,
                                       int cta_col_offset_in_bytes = 0)
        ...
        is_active_ =
            ALL_THREADS_ACTIVE || (row < ROWS_PER_LOOP && col_in_bytes_ < VALID_BYTES_PER_ROW);
...
#pragma unroll
        for( int ii = 0; ii < STGS_PER_LOOP; ++ii ) {
            ...
            const float *row_ptr = reinterpret_cast<const float *>(&src[ii]);
    ...
    #pragma unroll
            for( int ii = 0; ii < STGS_PER_LOOP; ++ii ) {
                const float4 &src_ii = reinterpret_cast<const float4 &>(src[ii]);
                const float4 &old_ii = reinterpret_cast<const float4 &>(old[ii]);
                ...
                tmp[ii].x = reinterpret_cast<const uint32_t &>(x);
                tmp[ii].y = reinterpret_cast<const uint32_t &>(y);
                tmp[ii].z = reinterpret_cast<const uint32_t &>(z);
                tmp[ii].w = reinterpret_cast<const uint32_t &>(w);
            }
        } else
#endif
        {
#pragma unroll
            for( int ii = 0; ii < STGS_PER_LOOP; ++ii ) {
                const int4 &src_ii = reinterpret_cast<const int4 &>(src[ii]);
                const int4 &old_ii = reinterpret_cast<const int4 &>(old[ii]);
...
#pragma unroll
    ...
    const uint32_t params_scale_bmm2_;
    // Do we enable the i2f trick?
    const bool params_enable_i2f_trick_;
    // The pointer.
    char *o_ptr_;
...
struct Gmem_tile_o_16bit {
    ...
    // Ctor.
    template<typename Params, typename Block_info>
    inline __device__ Gmem_tile_o_16bit(const Params &params,
                                        const Block_info &block_info,
                                        int tidx,
                                        int cta_row_offset = 0,
                                        int cta_col_offset_in_bytes = 0)
...
struct Gmem_tile_o_uint16 : public Gmem_tile_o_16bit<Traits, Cta_tile, uint16_t, CTAS_PER_HEAD> {
    ...
    // Ctor.
    template<typename Params, typename Block_info>
    inline __device__ Gmem_tile_o_uint16(const Params &params,
                                         const Block_info &block_info,
                                         int tidx,
                                         int cta_row_offset = 0,
                                         int cta_col_offset_in_bytes = 0)
...
struct Gmem_tile_o_bfloat16
    ...
    // Ctor.
    template<typename Params, typename Block_info>
    inline __device__ Gmem_tile_o_bfloat16(const Params &params,
                                           const Block_info &block_info,
                                           int tidx,
                                           int cta_row_offset = 0,
                                           int cta_col_offset_in_bytes = 0)
...
struct Imma_gmem_tile_o_interleaved {
    ...
    // Ctor.
    template<typename Params, typename Block_info>
    inline __device__ Imma_gmem_tile_o_interleaved(const Params &params,
                                                   const Block_info &block_info,
                                                   int tidx,
                                                   int cta_row_offset = 0)
        : params_o_stride_in_bytes_(params.o_stride_in_bytes),
          actual_seqlen_(block_info.actual_seqlen - cta_row_offset),
          params_scale_bmm2_(params.scale_bmm2), params_enable_i2f_trick_(params.enable_i2f_trick),
          o_ptr_(reinterpret_cast<char *>(params.o_ptr)), total_(params.o_stride_in_bytes) {
    ...
    inline __device__ void store(const uint4 (&src)[STGS_PER_LOOP], int mi) {
        ...
        // The scale.
        const float &scale = reinterpret_cast<const float &>(params_scale_bmm2_);
...
#pragma unroll
        for( int ii = 0; ii < STGS_PER_LOOP; ++ii ) {
            ...
            // The accumulators are in int32_t.
            const int4 &val = reinterpret_cast<const int4 &>(src[ii]);
    ...
    // The stride between rows for the QKV matrice.
    const int64_t params_o_stride_in_bytes_;
    // The scaling factor to convert to int8.
    const uint32_t params_scale_bmm2_;
    // Do we enable the i2f trick?
    const bool params_enable_i2f_trick_;
