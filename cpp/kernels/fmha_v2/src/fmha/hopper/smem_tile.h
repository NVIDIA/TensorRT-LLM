...
struct Smem_tile_hopper_gmma_row_a {
    ...
    using Gmma_descriptor = fmha::Gmma_descriptor_a<fmha::Gmma_descriptor_transpose::NOTRANS,
                                                    desc_mode,
                                                    Cta_tile,
...
#pragma unroll
    ...
    inline __device__ void store(const void *(&gmem_ptrs)[N], uint32_t (&preds)[M], uint64_t = 0) {
...
struct Smem_tile_hopper_gmma_col_b {
    ...
    // The SWIZZLE_128B descriptor
    using Gmma_descriptor = fmha::Gmma_descriptor_b<fmha::Gmma_descriptor_transpose::NOTRANS,
                                                    desc_mode,
                                                    Cta_tile,
...
struct Smem_tile_hopper_gmma_row_b {
    ...
    // The SWIZZLE_128B descriptor
    using Gmma_descriptor = fmha::Gmma_descriptor_b<fmha::Gmma_descriptor_transpose::TRANS,
                                                    desc_mode,
                                                    Cta_tile,
                                                    Traits::BITS_PER_ELEMENT_B,
                                                    Traits::GMMA_M,
...
template<typename Traits,
         ...
         int BUFFERS_PER_TILE_,
         fmha::Gmma_descriptor_mode desc_mode>
struct Smem_tile_hopper_b<Traits,
                          Cta_tile,
                          fmha::Col,
                          BUFFERS_PER_TILE_,
                          desc_mode,
...
struct Smem_tile_hopper_gmma_tma_row_a {
    ...
    inline __device__ Smem_tile_hopper_gmma_tma_row_a(char *smem, char *smem_barrier)
        : smem_(__nvvm_get_smem_pointer(smem)),
          smem_barrier_(__nvvm_get_smem_pointer(smem_barrier)), smem_write_offset_(0),
    ...
    inline __device__ void store(const cudaTmaDesc *p_desc,
                                 const int32_t (&coord)[DIM],
                                 uint16_t filter_offsets = 0,
...
struct Smem_tile_hopper_a : public fmha::Smem_tile_without_skews<
                                Cta_tile_,
                                Layout_::COL ? Cta_tile_::K : Cta_tile_::M,
                                Layout_::COL ? Cta_tile_::M : Cta_tile_::K,
                                Traits_::BITS_PER_ELEMENT_A,
                                BYTES_PER_STS_,
                                BUFFERS_PER_TILE_,
                                0,
                                ...
                                true,
                                USE_TMA,
                                128 * 8 / Traits_::BITS_PER_ELEMENT_A> {
    using Traits = Traits_;
    using Cta_tile = Cta_tile_;
    // The base class.
    using Base =
        fmha::Smem_tile_without_skews<Cta_tile,
                                      Layout_::COL ? Cta_tile::K : Cta_tile::M,
                                      Layout_::COL ? Cta_tile::M : Cta_tile::K,
                                      Traits::BITS_PER_ELEMENT_A,
                                      BYTES_PER_STS_,
                                      BUFFERS_PER_TILE_,
                                      0,
                                      ...
                                      true,
                                      USE_TMA,
                                      128 * 8 / Traits::BITS_PER_ELEMENT_A>;
    ...
    static constexpr int GMMA_GROUP_SMEM_DISTANCE =
        Layout::COL ? (Mma_tile::K_PER_GMMA_GROUP * NUM_COLS_PER_GMMA_GROUP_M *
                       Base::BYTES_PER_ROW * Cta_tile::WARP_GROUP_M)
...
struct Smem_tile_hopper_b
    ...
    using Base =
        fmha::Smem_tile_without_skews<Cta_tile,
                                      Layout_::COL ? Cta_tile::N : Cta_tile::K,
                                      Layout_::COL ? Cta_tile::K : Cta_tile::N,
                                      Traits::BITS_PER_ELEMENT_B,
                                      BYTES_PER_STS_,
                                      BUFFERS_PER_TILE_,
                                      0,
                                      ...
                                      true,
                                      USE_TMA,
                                      128 * 8 / Traits::BITS_PER_ELEMENT_B>;
...
#pragma unroll
            for( int ki = 0; ki < K; ki++ ) {  // 2
                fmha::ldsmt(tmp[nii][ki],
                            smem_read_loc_ + ki * ROWS_PER_LDSM_PER_CTA * BYTES_PER_ROW);
            }
...
#pragma unroll
            for( int ki = 0; ki < K; ki++ ) {
                fmha::swizzle_rows(
                    regs_[nii][ki].x, regs_[nii][ki].z, tmp[nii][ki].x, tmp[nii][ki].y);  //PRMT 0+1
                fmha::swizzle_rows(
                    regs_[nii][ki].y, regs_[nii][ki].w, tmp[nii][ki].z, tmp[nii][ki].w);  //PRMT 2+3
            }
        }
    }
...
struct Smem_tile_v_gmma {
    ...
    using Cta_tile_gmma_ = typename Traits::template Cta_tile<Cta_tile::M,
                                                              Cta_tile::N,
                                                              K_,
                                                              Cta_tile::WARP_GROUP_M,
                                                              Cta_tile::WARP_GROUP_N,
                                                              Cta_tile::WARP_GROUP_K>;
    ...
    using Base_ = fmha::Smem_tile_hopper_b<Traits,
                                           Cta_tile_gmma_,
                                           fmha::Col,
                                           BYTES_PER_STS,
                                           BUFFERS_PER_TILE * NUM_KGROUPS,
                                           GMMA_DESC_MODE_V,
                                           USE_TMA>;
    ...
    struct Base : public Base_ {
        ...
        using Store_delegate = Smem_tile_without_skews<Cta_tile,
                                                       Cta_tile::K,
                                                       Cta_tile::N,
                                                       8,
                                                       BYTES_PER_STS,
...
#pragma unroll
                for( int ki = 0; ki < K; ki++ ) {
                    fmha::swizzle_rows(
                        regs[ni][ki].x, regs[ni][ki].z, tmp[ni][ki].x, tmp[ni][ki].y);  //PRMT 0+1
                    fmha::swizzle_rows(
                        regs[ni][ki].y, regs[ni][ki].w, tmp[ni][ki].z, tmp[ni][ki].w);  //PRMT 2+3
                }
            }
            ...
            __syncthreads();  // LDSM.T done. We should now have a D x S tile in registers. SMEM can be written.
            constexpr int ROWS_PER_XOR_PATTERN =
                fmha::Rows_per_xor_pattern_ampere_b<Traits, S>::VALUE;
            static_assert(ROWS_PER_XOR_PATTERN == 8);
...
struct Smem_tile_v<
    ...
    using Base = typename fmha::
        Smem_tile_v_gmma<Traits, Cta_tile, BUFFERS_PER_TILE, desc_mode, USE_TMA>::Base;
