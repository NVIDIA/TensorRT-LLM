...
struct Arrive_wait {
  public:
    ...
    bar_arrive_set_transactioncnt(int id, int expected_copy_bytes, uint32_t pred) {
#if defined(__CUDA_ARCH__) && __CUDA_ARCH__ >= 900
    ...
    inline __device__ void bar_arrive_dsmem(const int &id) {
    ...
    // NOTE : Caller has to ensure that set_bar_base_dsmem has been called prior to using this
    // This is done as a compiler optimizations (since set barrier base is independent)
    inline __device__ void bar_arrive_dsmem(const int &id, const uint32_t &pred) {
#if CUDA_CP_ASYNC_MBARRIER_ARRIVE_ACTIVATED
        asm volatile("{\n\t"
                     " .reg .pred p;\n\t"
                     " .reg .s64 addr;\n\t"
    ...
    inline __device__ void set_bar_base_dsmem(const uint32_t &cta_id) {
