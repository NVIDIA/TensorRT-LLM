...
struct Gmem_tile_o_hopper_16bits {
    ...
    static_assert(Cta_tile::VALID_N % 8 == 0,
                  "The valid head dimension needs to be multiple of 8.");
    ...
    inline __device__ Gmem_tile_o_hopper_16bits(const Params &params,
        ...
        : params_o_stride_in_bytes_(params.o_stride_in_bytes),
          actual_seqlen_(block_info.actual_seqlen), o_ptr_(reinterpret_cast<char *>(params.o_ptr)) {
...
#pragma unroll
                for( int col_idx = 0; col_idx < Base::COLS_PER_THREAD; ++col_idx ) {
                    ...
                    float reg0 =
                        acc[0][mma_ni].elt(2 * (col_idx * Base::ROWS_PER_THREAD + row_idx));
                    float reg1 =
                        acc[0][mma_ni].elt(2 * (col_idx * Base::ROWS_PER_THREAD + row_idx) + 1);
                    uint32_t out = fmha::float2_to_half2(reg0, reg1);

                    int64_t offset =
                        (int64_t)row_idx * step_m +
...
struct Gmem_tile_o_gmma_32bit_8bit {
    ...
    // The number of valid columns (stored to GMEM) by each thread.
    enum {
        VALID_COLS_PER_THREAD_FOR_LAST_MMA = (VALID_N % Mma_tile::N_PER_MMA) == 0
                                                 ? COLS_PER_THREAD
                                                 : (VALID_N % Mma_tile::N_PER_MMA) / 8
    };
    ...
    inline __device__ Gmem_tile_o_gmma_32bit_8bit(void *o_ptr,
                                                  ...
                                                  int cta_row_offset = 0,
                                                  int mat_offset = 0,
                                                  uint32_t smem_base = 0,
                                                  const cudaTmaDesc *desc_o = nullptr,
                                                  int head_num = 0)
        : params_o_stride_in_bytes_(o_stride_in_bytes), actual_seqlen_(block_info.actual_seqlen),
          o_ptr_(reinterpret_cast<char *>(o_ptr)), params_scale_bmm2_(scale_bmm2),
          smem_base_(smem_base), desc_o_(desc_o) {
        ...
        if( USE_TMA_STORE ) {
            ...
            int row_offset_warp =
                cta_row_offset + warp_idx_within_warpgroup * (Mma_tile::M_PER_MMA / 4);
            ...
            smem_base_ += warp_idx_within_warpgroup * (Mma_tile::M_PER_MMA / 4) * Cta_tile::N *
                              BYTES_PER_ELEMENT +
    ...
    inline __device__ void store(const Accumulators (&acc)[M][N]) {
        ...
        if( USE_TMA_STORE ) {
            ...
            const int swizzled_row = (lane_ % 16);
            const int swizzled_col = (lane_ / 16);
...
#pragma unroll
                for( int ci = 0; ci < STSM_PER_MMA; ++ci ) {
                    const int col_bank = ((mma_ni)*STSM_PER_MMA + ci) * 2 + swizzled_col;
                    const int di = col_bank / N_BANKS_PER_GROUP;    // which N group it belongs to
                    stsm_addr[mma_ni][ci] =
                        smem_base_ + di * 16 * N_BYTES_PER_GROUP +  // group dimension
...
#pragma unroll
                for( int di = 0; di < N_GROUPS; ++di ) {
                    const int32_t coords[4] = { di * N_PER_GROUP, bidh_, 0, row_tma_ };
                ...
                static_assert((VALID_N % BYTES_PER_THREAD) == 0,
                              "VALID_N must be divided by 8 for STG.64");
                ...
                const int64_t step_m = (this->params_o_stride_in_bytes_);
                // Initial column index
                const int ci = lane_ % THREADS_PER_ROW;
                const int bank_idx = (ci * BYTES_PER_THREAD) / BYTES_PER_BANK;
                const int bank_offset = (ci * BYTES_PER_THREAD) % BYTES_PER_BANK;
...
#pragma unroll
                    for( int ri = lane_ / THREADS_PER_ROW; ri < row_size; ri += ROWS_PER_WARP ) {
                        ...
                        uint32_t smem_offset =
                            ...
                            (((ri / swizzle_row_divider) % max_swizzle_id) ^ bank_idx) *
                                BYTES_PER_BANK +
                            bank_offset;
                        uint2 buffer;
...
#pragma unroll
                    for( int ci = 0; ci < COLS_PER_THREAD / 2; ++ci ) {
                        ...
                        uint4 src;
                        src.x =
                            acc[0][mma_ni].reg(((2 * ci + 0) * ROWS_PER_THREAD + ri) * 2 + 0);  // 0
                        src.y =
                            acc[0][mma_ni].reg(((2 * ci + 1) * ROWS_PER_THREAD + ri) * 2 + 0);  // 4
                        src.z =
                            acc[0][mma_ni].reg(((2 * ci + 0) * ROWS_PER_THREAD + ri) * 2 + 1);  // 1
                        src.w =
                            acc[0][mma_ni].reg(((2 * ci + 1) * ROWS_PER_THREAD + ri) * 2 + 1);  // 5
                        ...
                        int64_t offset = (int64_t)ri * step_m +
                                         (int64_t)(ci + mma_ni * COLS_PER_THREAD / 2) * STEP_N;
...
#pragma unroll
                    for( int ci = 0;
                         ci < fmha::Div_up<VALID_COLS_PER_THREAD_FOR_LAST_MMA, 2>::VALUE;
                         ++ci ) {
                        // Assuming EVEN,EVEN,ODD,ODD column pattern due to packing of V.
                        uint2 src0, src1;
                        src0.x =
                        ...
                        int64_t offset =
                            (int64_t)ri * step_m + (int64_t)(col_idx)*BYTES_PER_ELEMENT;
...
struct Gmem_tile_o_qgmma_fp32_16bits {
    ...
    // Ctor.
    template<typename Params, typename Block_info, typename Shared>
    inline __device__ Gmem_tile_o_qgmma_fp32_16bits(const Params &params,
                                                    const Block_info &block_info,
                                                    Shared &&,
                                                    int tidx,
                                                    int cta_row_offset = 0)
    ...
    template<typename Accumulators, int M, int N>
    inline __device__ void store(const Accumulators (&acc)[M][N]) {
        const int64_t step_m = 8 * params_o_stride_in_bytes_;
        // we assume M = 1. some shortcuts.
        static_assert(M == 1);
