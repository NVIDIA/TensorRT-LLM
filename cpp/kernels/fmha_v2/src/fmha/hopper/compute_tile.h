...
#pragma unroll
        for( int mma_m_idx = 0; mma_m_idx < MMAS_M; ++mma_m_idx ) {
            gmma_desc_a_[mma_m_idx].set_smem_pointer(
                a_smem + mma_m_idx * Smem_tile_a::GMMA_GROUP_SMEM_DISTANCE);
...
#pragma unroll
        for( int mma_n_idx = 0; mma_n_idx < MMAS_N; ++mma_n_idx ) {
            gmma_desc_b_[mma_n_idx].set_smem_pointer(
                b_smem + mma_n_idx * Smem_tile_b::GMMA_GROUP_SMEM_DISTANCE);
...
#pragma unroll
    ...
    inline __device__ void increment_gmma_desc_b_group(int N = 1) {
        bool reset_buffer = RESET_CHECK && gmma_desc_b_[0].get_descriptor(0) >=
                                               gmma_desc_b_[0].get_max_descriptor_0();
...
#pragma unroll
    ...
    template<int K>
    inline __device__ void compute_incta_splitk(const Fragment (&frag_a)[K][1], const int warp_k) {
        ...
        if( Smem_tile_b::Gmma_descriptor::TRANS_MODE == Gmma_descriptor_transpose::NOTRANS ) {
            ...
            constexpr int ELTS_PER_KGROUP =
                Smem_tile_b::BYTES_PER_ROW / sizeof(typename Traits::B_type);
...
#pragma unroll
            ...
            constexpr int BYTES_PER_K_GROUP_NO_4LSB = Mma_tile::K_PER_WARP_GROUP *
                                                      Mma_tile::N_PER_WARP_GROUP *
                                                      sizeof(Traits::B_type) / 16;
