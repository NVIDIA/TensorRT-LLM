...
struct Smem_tile_v_turing_hmma
    : public Smem_tile_v_hmma<Traits, Cta_tile, Rows_per_xor_pattern_turing_b, BUFFERS_PER_TILE>::
          Base {
...
#pragma unroll
        for( int ni = 0; ni < Mma_tile::MMAS_N; ++ni ) {
            ...
            if( ni < Mma_tile::VALID_MMAS_N ) {
                ...
                int row =
                    (ki / 2) * 16 * Cta_tile::WARPS_K / ROW_PACKING + (ki % 2) * 8 / ROW_PACKING;
...
#pragma unroll
        for( int ni = 0; ni < Mma_tile::MMAS_N; ++ni ) {
            ...
            if( ni < Mma_tile::VALID_MMAS_N ) {
                ...
                if( lane < 16 ) {
                    fmha::lds(tmp16,
                              this->smem_ + this->smem_read_offset_ + row * Base::BYTES_PER_ROW);
                }
