...
void run_softmax_fp32(void *dst,
                      const void *src,
                      const void *mask,
                      void *softmax_sum_d,
                      void *cu_q_seqlens_d,
...
void run_conversion_fp32_to_e4m3(void *dst,
                                 const void *src,
                                 int s,
                                 int b,
                                 int h,
...
void run_sage_quant(unsigned int batch_size,
...
void ground_truth(RefBMM &bmm1,
                  ...
                  const float softcapping_scale_bmm1,
                  void *qkv_d,
                  void *vt_d,
                  void *mask_d,
                  void *p_d,
...
static inline void set_params(bert::Fused_multihead_attention_params_v2 &params,
    ...
    const bool enable_attn_logit_softcapping = softcapping_scale_bmm1 != 0.f;
    ...
    FMHA_CHECK_CUDA(cudaMemcpy(
        params.scale_bmm2_d, &params.scale_bmm2, sizeof(uint32_t), cudaMemcpyHostToDevice));
...
static inline void determine_launch_params(Launch_params &launch_params,
                                           ...
                                           const Attention_input_layout input_layout,
                                           const bool interleaved,
    ...
    launch_params.flash_attention = (data_type == DATA_TYPE_FP16 || data_type == DATA_TYPE_BF16 ||
    ...
    launch_params.use_granular_tiling = !force_non_granular_tiling &&
    ...
    if( launch_params.use_granular_tiling && (data_type == DATA_TYPE_E4M3 && sm == 80) ) {
        printf("Fallback to non-granular-tiling kernels as tiled e4m3 kernels"
...
int main(int argc, char **argv) {
    ...
    if( save_softmax == true ) {
        if( input_layout != Attention_input_layout::CONTIGUOUS_Q_KV ) {
            input_layout = Attention_input_layout::CONTIGUOUS_Q_KV;
            printf("Only '--contiguous-q-kv' layout supports '-save-softmax', switched to "
                   "contiguous-q-kv\n");
        }
        if( data_type == DATA_TYPE_E4M3 ) {
    ...
    const size_t kv_size = b * 2 * s * h_kv * d;
    // The size in bytes.
    ...
    for( size_t bi = 0; bi < b; bi++ ) {
        for( int kv_offset = 0; kv_offset < 2; kv_offset++ ) {
            ...
            for( size_t block_i = 0; block_i < max_blocks_per_seq; block_i++ ) {
                kv_cache_ptrs_h[ptr_index] = reinterpret_cast<void *>(
                    reinterpret_cast<char *>(kv_cache_pool_ptr) + abs_offset);
    ...
    FMHA_CHECK_CUDA(cudaMemcpy(kv_cache_block_offsets_d,
                               kv_cache_block_offsets_h,
                               num_total_blocks * sizeof(int32_t),
                               cudaMemcpyDefault));
    ...
    printf("v1=%d il=%d s_q=%lu, s=%lu b=%lu h=%lu/%lu d=%lu/%lu dtype=%s, output_dtype=%s, "
           "flash_attn=%s, "
           "warp_spec=%s, mask=%s, "
           ...
           launch_params.flash_attention
               ? (launch_params.use_granular_tiling ? "true_tiled" : "true")
               : "false",
           launch_params.warp_specialization ? "true" : "false",
           mask_type_to_string(attention_mask_type).c_str(),
           has_alibi ? "true" : "false",
           h_kv == 1 ? "mqa" : (h_kv == h ? "mha" : "gqa"),
           attention_input_layout_to_string(input_layout).c_str(),
           warps_m,
           warps_n);
        ...
        for( size_t sbi = 0; sbi < s * b; sbi++ ) {
            for( size_t hi = 0; hi < h; hi++ ) {
                for( size_t di = 0; di < d; di++ ) {
                    ...
                    size_t src_offset = sbi * h * 3 * d + hi_kv_scatter * 3 * d +
                                        di;  // [sbi, hi_kv_scatter, 0, di]
                    ...
                    qkv_h[dst_offset + 1 * d] =
                        qkv_h[src_offset +
                              1 * d];  // qkv[sbi, hi, 1, di] = qkv[sbi, hi_kv_scatter, 1, di]
                    qkv_h[dst_offset + 2 * d] =
                        qkv_h[src_offset +
                              2 * d];  // qkv[sbi, hi, 2, di] = qkv[sbi, hi_kv_scatter, 2, di]
                }
            }
        }
    }
    ...
    FMHA_CHECK_CUDA(cudaMemcpy(
        cu_seqlens_d, cu_seqlens.data(), sizeof(int) * cu_seqlens.size(), cudaMemcpyHostToDevice));
    FMHA_CHECK_CUDA(cudaMemcpy(cu_q_seqlens_d,
                               cu_q_seqlens.data(),
                               sizeof(int) * cu_q_seqlens.size(),
                               cudaMemcpyHostToDevice));
    FMHA_CHECK_CUDA(cudaMemcpy(cu_mask_rows_d,
                               cu_mask_rows.data(),
                               sizeof(int) * cu_mask_rows.size(),
                               cudaMemcpyHostToDevice));
    ...
    extract_and_transpose_input<float>(
        qkv_packed_h.data(), qkv_h, seqlens, s, b, h, d, dv, 3, false);
    if( interleaved ) {
    ...
    extract_and_transpose_input<float>(
        qkv_bsh3d_h.data(), qkv_h, seqlens, s, b, h, d, dv, 3, is_s_padded);
    if( interleaved ) {
    ...
    if( d == dv ) {
        ...
        for( size_t bi = 0; bi < b; bi++ ) {
            ...
            for( size_t si = 0; si < actual_s; si++ ) {
                for( size_t hi = 0; hi < h; hi++ ) {
                    for( size_t di = 0; di < d; di++ ) {
                        mqa_qkv_h[bi * s * (h + 2 * h_kv) * d + si * (h + 2 * h_kv) * d + hi * d +
                                  di] = qkv_bsh3d_h[bi * s * h * 3 * d + si * h * 3 * d +
                                                    hi * 3 * d + 0 * d + di];
                        if( hi < h_kv ) {
                            // E.g., h=8, h_kv=4
                            //     src kv id: 0, 0, 1, 1, 2, 2, 3, 3
    ...
    // Contiguous KV Cache.
    store_q_and_contiguous_kv_cache(q_d,
                                    contiguous_kv_h,
                                    contiguous_kv_d,
                                    reinterpret_cast<const float *>(qkv_packed_h.data()),
                                    reinterpret_cast<const int *>(cu_seqlens.data()),
                                    reinterpret_cast<const int *>(cu_q_seqlens.data()),
                                    b,
                                    s,
                                    h,
                                    h_kv,
    ...
    // Paged KV Cache.
    store_paged_kv_cache(kv_cache_ptrs_h,
                         reinterpret_cast<const float *>(qkv_packed_h.data()),
                         reinterpret_cast<const int *>(cu_seqlens.data()),
                         max_blocks_per_seq,
                         tokens_per_block,
        ...
        for( size_t bi = 0; bi < b; ++bi ) {
            ...
            for( size_t si = 0; si < s; ++si ) {  // s_kv
                ...
                if( !skip_checks ) {
                    ...
                    mask_h[so * b * s + bi * s + si] =
                        valid ? 1.f : 0.f;  // mask dims [s_q, b, s_kv]
                }
            }
        }
    }
    ...
    if( v1 ) {
        ...
        if( sm == 70 ) {
            pack_mask_sm70(
                packed_mask_h, mask_h, s, b, mmas_m, mmas_n, warps_m, warps_n, threads_per_cta);
        } else {
            pack_mask(
                packed_mask_h, mask_h, s, b, mmas_m, mmas_n, warps_m, warps_n, threads_per_cta);
        }
        ...
        pack_flash_attention_mask(packed_mask_h,
                                  mask_h,
                                  b,
                                  s,
                                  warps_m,
                                  ...
                                  threads_per_cta,
                                  mmas_n,
                                  fmha::FLASH_ATTEN_MASK_MMA_N / 8,
                                  mask_h_row_offsets.data(),
                                  cu_mask_rows.data());
    ...
    if( sage_block_size_q > 0 || sage_block_size_k > 0 || sage_block_size_v > 0 ) {
...
#if 1
        {
            ...
            FMHA_CHECK_CUDA(
                cudaMemcpy(quant_qkv, quant_qkv_h, qkv_packed_size, cudaMemcpyHostToDevice));
            free(quant_qkv_h);
            auto init_scales =
                [&](bert::Fused_multihead_attention_params_v2::SageAttention::Scales &x) {
                    std::vector<float> scales(x.max_nblock * h * b, const_scale);
                    FMHA_CHECK_CUDA(cudaMemcpy(x.scales,
                                               scales.data(),
...
#else
        {
            ...
            const int stride_qkv = params_v2.qkv_stride_in_bytes;
            run_sage_quant(b,
                           h,
                           d,
                           s,
                           ...
                           stride_qkv,
                           stride_qkv,
                           params_v2.cu_q_seqlens,
                           params_v2.cu_kv_seqlens,
                           sage_block_size_q,
                           sage_block_size_k,
                           sage_block_size_v,
                           quant_qkv,
                           quant_qkv + h * d,
                           quant_qkv + 2 * h * d,
                           params_v2.sage.q.scales,
                           params_v2.sage.k.scales,
                           params_v2.sage.v.scales);
        }
#endif
    ...
    if( !skip_checks ) {
        ...
        */
        ...
        FMHA_CHECK_CUDA(
            cuda_memcpy_d2h(softmax_sum_ref_h, softmax_stats_d, b * s * h, DATA_TYPE_FP32));
    }
    ...
    if( skip_checks ) {
        ...
        if( v1 ) {
            ...
            FMHA_CHECK_CUDA(
                cuda_memcpy_d2h(softmax_sum_h, softmax_stats_d, b * s * h, DATA_TYPE_FP32));
            ...
            if( chunked_attention ) {
                extract_and_transpose_chunked_output<float>(o_ref_trans_h.data(),
                                                            o_ref_h,
                                                            seqlens,
                                                            q_seqlens,
                                                            s,
                ...
                extract_and_transpose_output<float>(
                    o_ref_trans_h.data(), o_ref_h, seqlens, s, b, h, dv, is_s_padded);
            }

            if( verbose ) {
                printf("\nChecking .....: O = V * S\n");
            }
            status = check_results(o_h,
                                   o_ref_trans_h.data(),
                                   dv,
                                   is_s_padded ? s_q * b * h : cu_q_seqlens.back() * h,
                                   dv,
                                   epsilon,
                                   verbose,
                                   true);
            if( save_softmax ) {
                int errors = check_softmax_results(
                    softmax_sum_h, softmax_sum_ref_h, b, s, h, seqlens, cu_seqlens);
    ...
    total_flops =
        attention_mask_type == Attention_mask_type::CAUSAL ? total_flops / 2 : total_flops;
...
#if defined(DEBUG_HAS_PRINT_BUFFER)
    FMHA_CHECK_CUDA(cuda_memcpy_d2h(
        print_buffer.data(), params.print_ptr, print_buffer.size(), DATA_TYPE_FP32));
