...
void ground_truth(RefBMM &bmm1,
    ...
    for( int ii = 0; ii < runs; ++ii ) {
        ...
        // P = Q x K'
        bmm1(static_cast<char *>(q_d) + 0 * qkv_stride,
             static_cast<char *>(kv_d) + 0 * qkv_stride,
             p_d,
             &alpha,
             &beta,
             stream);
...
int main(int argc, char **argv) {
    ...
    // The padded sizes.
    const int s_kv_padded = std::pow(2, std::ceil(std::log(s_kv) / std::log(2)));
    const int d_padded = std::pow(2, std::ceil(std::log(d) / std::log(2)));
    ...
    random_init(
        "K", kv_h + 0 * d, d, s_kv_padded * b * h, 2 * d, use_1s_k, range_k, scale_k, verbose);
    random_init(
        "V", kv_h + 1 * d, d, s_kv_padded * b * h, 2 * d, use_1s_v, range_v, scale_v, verbose);
    ...
    const auto create_seqlen = [min_s, fix_s, b](int s,
    ...
    extract_and_transpose_input<float>(
        kv_packed_h.data(), kv_h, seqlens_kv, s_kv_padded, b, h, d, 2);
    if( interleaved ) {
    ...
    check_results(p_h,
                  p_ref_h,
                  s_kv_padded,
                  cu_seqlens_q.back() /*not needed: * b -- already counted */ * h,
                  s_kv_padded,
                  0.f,
                  true,
                  true);
    ...
    if( skip_checks ) {
        ...
        status = check_results(
            o_packed_h, o_ref_packed_h, d, cu_seqlens_q.back() * h, d, epsilon, verbose, true);
