...
inline __device__ void device_flash_attention_nl_tiled(const Params &params) {
    ...
    const int ctas_per_o_row = (Cta_tile_o::VALID_N + Cta_tile_o::N - 1) / Cta_tile_o::N;
    const int q_loop = blockIdx.x / ctas_per_o_row;
    // Each CTA works on a specific row partition of O.
    const int o_part = blockIdx.x % ctas_per_o_row;
    ...
    Gmem_tile_v gmem_v(
        params, 2, binfo, tidx, 0, o_part * Cta_tile_o::N * Gmem_tile_v::BYTES_PER_ELEMENT);
    ...
    Gmem_tile_o gmem_o(params,
                       binfo,
                       tidx,
                       q_loop * Gmem_tile_o::ROWS,
    ...
    const int kv_mask_loop_start = int(q_sequence_start / Cta_tile_p::N) * Cta_tile_p::N;

    // The start/end step of kv loops.
    // Do we need to mask out the tokens that is not in the sliding window.
    const bool mask_sliding_window = Kernel_traits::SLIDING_WINDOW_ATTENTION &&
    ...
    const int kv_loop_end = ((valid_seqlen + Cta_tile_p::N - 1) / Cta_tile_p::N) * Cta_tile_p::N;
    const int kv_loop_start =
        mask_sliding_window
            ? (max(0, q_sequence_start - params.sliding_window_size) / Cta_tile_p::N) *
                  Cta_tile_p::N
            : 0;
    const int sliding_window_mask_end =
        mask_sliding_window
            ? (max(0, q_sequence_start + Cta_tile_p::M - 1 - params.sliding_window_size) /
               Cta_tile_p::N) *
                  Cta_tile_p::N
            : 0;
    ...
    constexpr int BMM2_MAIN_MMAS_K_BOUND =
        Kernel_traits::TOTAL_BMM2_MMAS_K - BMM2_TAIL_MMAS_K_BOUND;
    ...
    for( int kv_loop = kv_loop_start; kv_loop < kv_loop_end; kv_loop += Cta_tile_p::N ) {
        ...
        const bool apply_sliding_window_mask =
...
#pragma unroll
        ...
        // BMM2 tail loop
        {
            if( kv_loop + Cta_tile_p::N < kv_loop_end ) {
                ...
                int col_steps =
                    fmha::div_up((int)Kernel_traits::VALID_D, (int)Kernel_traits::CTA_P_TILE_K);
