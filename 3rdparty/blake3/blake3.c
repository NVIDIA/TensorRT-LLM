...
char const* blake3_version(void)
{
...
INLINE size_t chunk_state_len(blake3_chunk_state const* self)
{
    return (BLAKE3_BLOCK_LEN * (size_t) self->blocks_compressed) + ((size_t) self->buf_len);
}

INLINE size_t chunk_state_fill_buf(blake3_chunk_state* self, uint8_t const* input, size_t input_len)
{
...
INLINE uint8_t chunk_state_maybe_start_flag(blake3_chunk_state const* self)
{
...
INLINE output_t make_output(const uint32_t input_cv[8], const uint8_t block[BLAKE3_BLOCK_LEN], uint8_t block_len,
...
// bytes.
INLINE void output_chaining_value(output_t const* self, uint8_t cv[32])
{
    uint32_t cv_words[8];
...
INLINE void output_root_bytes(output_t const* self, uint64_t seek, uint8_t* out, size_t out_len)
{
    if (out_len == 0)
    {
        return;
    ...
    {
        blake3_xof_many(
            self->input_cv, self->block, self->block_len, output_block_counter, self->flags | ROOT, out, out_len / 64);
    }
...
INLINE void chunk_state_update(blake3_chunk_state* self, uint8_t const* input, size_t input_len)
{
    if (self->buf_len > 0)
    {
...
INLINE output_t chunk_state_output(blake3_chunk_state const* self)
{
...
INLINE size_t compress_chunks_parallel(
    uint8_t const* input, size_t input_len, const uint32_t key[8], uint64_t chunk_counter, uint8_t flags, uint8_t* out)
{
    ...
    uint8_t const* chunks_array[MAX_SIMD_DEGREE];
    ...
    blake3_hash_many(chunks_array, chunks_array_len, BLAKE3_CHUNK_LEN / BLAKE3_BLOCK_LEN, key, chunk_counter, true,
        flags, CHUNK_START, CHUNK_END, out);
...
INLINE size_t compress_parents_parallel(uint8_t const* child_chaining_values, size_t num_chaining_values,
    const uint32_t key[8], uint8_t flags, uint8_t* out)
{
    ...
    uint8_t const* parents_array[MAX_SIMD_DEGREE_OR_2];
...
size_t blake3_compress_subtree_wide(uint8_t const* input, size_t input_len, const uint32_t key[8],
    uint64_t chunk_counter, uint8_t flags, uint8_t* out, bool use_tbb)
{
    ...
    uint8_t const* right_input = &input[left_input_len];
    ...
    blake3_compress_subtree_wide_join_tbb(key, flags, use_tbb,
        // left-hand side
        input, left_input_len, chunk_counter, cv_array, &left_n,
        // right-hand side
        right_input, right_input_len, right_chunk_counter, right_cvs, &right_n);
#else
    left_n = blake3_compress_subtree_wide(input, left_input_len, key, chunk_counter, flags, cv_array, use_tbb);
    right_n = blake3_compress_subtree_wide(
        right_input, right_input_len, key, right_chunk_counter, flags, right_cvs, use_tbb);
...
INLINE void compress_subtree_to_parent_node(uint8_t const* input, size_t input_len, const uint32_t key[8],
    uint64_t chunk_counter, uint8_t flags, uint8_t out[2 * BLAKE3_OUT_LEN], bool use_tbb)
{
#if defined(BLAKE3_TESTING)
...
void blake3_hasher_init_derive_key_raw(blake3_hasher* self, void const* context, size_t context_len)
{
...
void blake3_hasher_init_derive_key(blake3_hasher* self, char const* context)
{
...
INLINE void blake3_hasher_update_base(blake3_hasher* self, void const* input, size_t input_len, bool use_tbb)
{
    ...
    uint8_t const* input_bytes = (uint8_t const*) input;
    ...
    while (input_len > BLAKE3_CHUNK_LEN)
    {
        ...
        else
        {
            ...
            compress_subtree_to_parent_node(
                input_bytes, subtree_len, self->key, self->chunk.chunk_counter, self->chunk.flags, cv_pair, use_tbb);
...
void blake3_hasher_update(blake3_hasher* self, void const* input, size_t input_len)
{
    bool use_tbb = false;
...
#if defined(BLAKE3_USE_TBB)
void blake3_hasher_update_tbb(blake3_hasher* self, void const* input, size_t input_len)
{
    bool use_tbb = true;
    blake3_hasher_update_base(self, input, input_len, use_tbb);
}
...
void blake3_hasher_finalize(blake3_hasher const* self, uint8_t* out, size_t out_len)
{
    blake3_hasher_finalize_seek(self, 0, out, out_len);
}

void blake3_hasher_finalize_seek(blake3_hasher const* self, uint64_t seek, uint8_t* out, size_t out_len)
{
    // Explicitly checking for zero avoids causing UB by passing a null pointer
    // to memcpy. This comes up in practice with things like:
    //   std::vector<uint8_t> v;
