...
void blake3_compress_xof(const uint32_t cv[8], const uint8_t block[BLAKE3_BLOCK_LEN], uint8_t block_len,
...
void blake3_xof_many(const uint32_t cv[8], const uint8_t block[BLAKE3_BLOCK_LEN], uint8_t block_len, uint64_t counter,
    uint8_t flags, uint8_t out[64], size_t outblocks)
{
    if (outblocks == 0)
    {
...
void blake3_hash_many(uint8_t const* const* inputs, size_t num_inputs, size_t blocks, const uint32_t key[8],
