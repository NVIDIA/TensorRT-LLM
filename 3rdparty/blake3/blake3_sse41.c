...
void blake3_compress_xof_sse41(const uint32_t cv[8], const uint8_t block[BLAKE3_BLOCK_LEN], uint8_t block_len,
...
static void blake3_hash4_sse41(uint8_t const* const* inputs, size_t blocks, const uint32_t key[8], uint64_t counter,
...
INLINE void hash_one_sse41(uint8_t const* input, size_t blocks, const uint32_t key[8], uint64_t counter, uint8_t flags,
...
void blake3_hash_many_sse41(uint8_t const* const* inputs, size_t num_inputs, size_t blocks, const uint32_t key[8],
