...
{
    return _mm_loadu_si128((__m128i const*) src);
}
...
INLINE void compress_pre(__m128i rows[4], const uint32_t cv[8], const uint8_t block[BLAKE3_BLOCK_LEN],
...
void blake3_compress_xof_sse2(const uint32_t cv[8], const uint8_t block[BLAKE3_BLOCK_LEN], uint8_t block_len,
...
INLINE void transpose_msg_vecs(uint8_t const* const* inputs, size_t block_offset, __m128i out[16])
{
    ...
    {
        _mm_prefetch((void const*) &inputs[i][block_offset + 256], _MM_HINT_T0);
    }
...
static void blake3_hash4_sse2(uint8_t const* const* inputs, size_t blocks, const uint32_t key[8], uint64_t counter,
...
INLINE void hash_one_sse2(uint8_t const* input, size_t blocks, const uint32_t key[8], uint64_t counter, uint8_t flags,
...
void blake3_hash_many_sse2(uint8_t const* const* inputs, size_t num_inputs, size_t blocks, const uint32_t key[8],
    uint64_t counter, bool increment_counter, uint8_t flags, uint8_t flags_start, uint8_t flags_end, uint8_t* out)
{
    while (num_inputs >= DEGREE)
    {
        blake3_hash4_sse2(inputs, blocks, key, counter, increment_counter, flags, flags_start, flags_end, out);
    ...
    while (num_inputs > 0)
    {
        hash_one_sse2(inputs[0], blocks, key, counter, flags, flags_start, flags_end, out);
        if (increment_counter)
        {
            counter += 1;
        }
