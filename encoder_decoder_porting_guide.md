...
6. **Encoder-output storage.** The packed hidden states must persist across decode steps. Two options:
   ...
   - **Page into the cross-KV pool immediately** (matches the legacy **device-side** lifetime in §2.8 / §2.10) — let the decoder's first context step write projected K/V into the cross pool and drop the raw GPU hidden states once decoder context completes. Uses the `cross_kv_cache_manager` from 3.B.3.
...
- **KV cache reuse (decision: namespaced reuse, matching legacy).** Enc-dec requests cannot share self-KV blocks naïvely because decoder hidden states depend on `encoder_output` via the cross-attention sublayer — two requests with identical decoder prefixes but different encoder inputs must not collide. The port **commits to the namespaced-reuse option**: extend the self-KV reuse key so it combines `hash(encoder_unique_tokens) ++ hash(decoder_unique_tokens)` when `is_encoder_decoder`, rather than disabling reuse entirely. Concretely:
  - **Cross-KV pool:** enable reuse; key is `LlmRequest.get_encoder_unique_tokens()` only. Already consumed by `scheduler.py` L1307-L1329 (contribution accounting) and L1370-L1382 (`_beneficial_to_skip`). Flip `cross_kv_cache_manager.enable_block_reuse=True` at construction (§3.B.3).
...
| Knob | Value |
|------|-------|
| Model | `google/t5-base`, `facebook/bart-base`; add `google/flan-t5-large` for a second size class |
