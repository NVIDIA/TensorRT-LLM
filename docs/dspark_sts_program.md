...
| 载荷 | 全局 bs | notrim | **scheduled** | Δ |
|---|---|---|---|---|
| poetry | 512 | 12,351 | 13,568 | **+9.9%** |
| poetry | 1024 | 15,586 | 19,431 | **+24.7%** |
| arena | 512 | 15,362 | 16,400 | **+6.8%** |
| arena | 1024 | 20,887 | 22,124 | **+5.9%** |

**判决**:①四 cell 全正,最难的 poetry@1024 达 **+24.7%**,超过 SGLang blog 量级(H200 +10-20%);②master arc"为什么没有收益"终章——收益从未缺席,被我们自己的三个问题吃掉:#32 prep 同步链(每步 ~14ms)、#31 半批上限(cap=maxbs/2,兼致 2.7× 步时惩罚)、以及嵌入这两个伪影的旧成本表;③修复组合 = b87ea28bf3(双缓冲)+ maxbs=2× 目标并发(部署参数)+ 修复后重采表。
**方法论注记**:745396 首次 S 臂遇 nvlink peer 显存错误(warmup 期),同配置两节点随后均干净复跑 → 判硬件抖动,非确定性 bug;若复发再立案。
**余留**:#31 代码级根因(spec 请求 2× 槽记账,已有部署级规避)、GSM8K 精度例行回归、#19 连续预算(θ 单调后价值上升)、反馈 SGLang(2026.2.1 nsys、192 行图上限)。
...
## 【08-18 10:30】slo20 形态成本表重采(laliao 侧):**78% 步时不可裁** —— 拒裁的定量铁证,模型预测与 pin 阶梯实测误差 1-3pp

用户质疑"trim 率为何这么低",直指表失配风险(旧表采自 sweep 形态:TRTLLM moe/ADP off/maxbs64)。**在 slo20 形态原地重采 pin{2,3,4,5} 阶梯**(u128,各 3.5-5.3 万步),得 `slo20_shape_table_v1.json`:

| | sweep 形态旧表 | **slo20 形态新表** |
|---|---|---|
| fixed | 15.6ms | **35.3ms** |
| α(16/rank) | 1.8ms | 1.4ms |
| θ(96 token) | ~14.5ms(插值) | **10.5ms** |
| bs16 满窗步时构成 | — | 35.3+1.4+10.5=47.2ms,**不可裁部分 78%** |

**pin 阶梯实测(SLO 口径,同点四格)**:

| pin | SSE p50 tok/s | accept | steps | 模型预测 | 实测 |
|---|---|---|---|---|---|
| 5(满窗) | 91.6 | 3.61 | 34.7k | — | — |
| 4 | 88.6 | 3.41 | 35.9k | −4.3% | **−3.3%** |
| 3 | 83.5 | 3.06 | 38.6k | −11.3% | **−8.8%** |
| 2 | 63.6 | 2.25 | 53.4k | −32.8% | **−30.6%** |

- **成本模型在 30% 动态范围内预测实测误差 1-3pp**——θ/accept 双侧定价都对。裁剪在此形态每一档都是净损,**planner 拒裁(trim 0.23%)是唯一正确答案**;新表比旧表更不看好裁剪(fixed 翻倍),即拒裁结论被强化而非推翻。
- **根因对照**:poetry/agg 形态 54-56% 步时可裁 → 裁剪赚;slo20 形态**只有 22% 可裁**(ADP all-to-all + MEGAMOE_DEEPGEMM + 131k 窗稀疏注意力全是 per-step 固定成本)+ AA 高接受率 [0.98→0.89] → 裁剪两头亏。**"收益 = 可裁比例 × 接受曲线陡度"在两个形态上完成定量闭环。**
- 方法论再证:**表必须在目标形态原地重采**(同模型同硬件,仅 moe 后端/ADP/maxbs 不同,fixed 就差 2.3×)。
- **sched x 新表已验证(预测命中)**:trim **0.0016**(新表比旧表更保守:0.0023->0.0016,仅 40/29k 请求被裁)、accept 3.59、SSE p50 **91.5** vs notrim 91.6/91.7 —— 逐位打平,零回归。置信度调度在本形态的正确行为就是「看表拒裁」,特性本身无损。
...
## 【08-11 11:40】v4 实测表 + 混合步门控:agg t128 翻回打平/微正(当日收官)
