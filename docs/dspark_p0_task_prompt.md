...
| 项 | uniform | ragged | 倍数 |
|---|---|---|---|
| CUDA graph 张数 | 4.1 / 批次档 | 12.1 / 批次档 | ~3× |
| attention workspace | 68 MiB | 604 MiB | ~9× |

graph key 多出的 token-bucket 维度是 3× 的直接来源。`max_batch_size=512` 时
graph 私有池达 13.97 GiB 并 OOM——ragged 让显存悬崖来得更早。
