...
No existing approach satisfies all four. **Single-request benchmarks** dressed in agentic shapes — long input, short output — are convenient and reproducible, but neither realistic (they never exercise prefix-cache reuse) nor intuitive (their token-level numbers cannot answer what deployments ask: how many deep-research tasks does one GB200 finish per hour?). **Live agent systems** are the most realistic and do yield intuitive agent-level metrics, but they are neither convenient — every sandbox, browser, and search service must stay healthy under high concurrency — nor reproducible, because the agent loop amplifies generation nondeterminism until runs stop being comparable. **Trace-based** methods are convenient and reproducible by construction, and can be intuitive, so realism is the only property left to earn; existing traces, however, are usually too coarse for it: they simulate only part of the prefix structure — which is driven by the system prompt *and* the growing multi-turn conversation — drop tool-call latency, which inflates hit rates (a KV block that survives an instant tool return is often evicted across a 30-second one) and rules out agent-level metrics, and assume a single ReAct thread, while real agents fan out to subagents, think in parallel, and compact their context. Our methodology is therefore trace-based, built to close exactly that realism gap while keeping the other three properties.
