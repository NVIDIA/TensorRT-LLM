...
Benchmarking an inference system for agentic scenarios currently takes three broad forms, each with a real limitation. Extending **single-request benchmarks** with agentic-looking shapes — long inputs, short outputs — is simple and well established, but it leaves a wide gap to real agent inference: capabilities such as prefix-cache reuse are never exercised, and token-level numbers cannot answer the question deployments actually ask, e.g. how many medium-difficulty deep-research tasks one GB200 finishes per hour. Running **live agent systems** restores realism and does yield agent-level metrics, but it is hard to deploy — every sandbox, browser, and search service must stay healthy under high concurrency — and unstable, because the nondeterminism of autoregressive generation is amplified by the agent loop. **Trace-based** methods avoid both problems and are the right starting point, but existing traces are usually too coarse to be faithful: prefix matching is driven by both the system prompt and the growing multi-turn tool conversation, and simulating only one of them misstates reuse; tool-call latency is typically dropped, which inflates cache hit rates (a KV block that survives an instant tool return is often evicted across a 30-second one) and makes agent-level metrics impossible to compute; and the agent is assumed to be a single ReAct thread, while real paradigms fan out to subagents, think in parallel, and rewind or compact their context. Our methodology is trace-based, designed to close exactly those gaps.
...
### Relation to Other Agentic Benchmarks

Benchmarking practice here is still taking shape, and industry efforts are converging on the same trace-replay idea from different directions: [AA-AgentPerf](https://artificialanalysis.ai/methodology/agentperf) replays recorded coding sessions to report how many concurrent agents a deployment sustains under an SLO, and SemiAnalysis's [InferenceX AgentX](https://inferencex.semianalysis.com/datasets) replays coding traces with per-turn token counts and KV-block hashes to reproduce prefix reuse. Our framework differs in scope rather than in kind: it traces agent architectures beyond a coding ReAct thread — supervisor fan-out, parallel branch search, context compaction — replays their execution graph concurrently, and reports job-level rather than SLO-conditioned concurrency, which is what lets us ask configuration questions such as how batch size should follow an agent's branching structure.
