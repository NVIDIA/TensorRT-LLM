...
- **更长序列基准测试**：在 ISL=32K 和 ISL=64K 下评估，其中 Top-K 占比显著更大，预期端到端增益更为明显。
- **自适应预测源**：探索在时序相关性低时（如分布偏移后的第一个 token）使用静态 RoPE 候选集作为回退，并动态融合静态和时序预测。
- **多 CTA 启发式**：将启发式方法扩展到超长序列（N > 200K）的多 CTA 拆分方案，当前实现在此场景回退到 radix-select。
- **预填充阶段启发式**：研究启发式方法是否有益于预填充阶段，其中多个查询 token 共享同一键集，查询间预测可行。
- **跨模型泛化**：将时序相关引导的 Top-K 应用于其他使用 Top-K 选择的稀疏注意力方法，如 RocketKV 的生成阶段动态 Top-K。
- **FP8 indexer 分数支持**：随着 indexer MQA 向低精度输出发展，适配直方图和阈值逻辑以适应 FP8 或 BF16 分数分布。
- **与内核级稀疏性集成**：探索将启发式 Top-K 与 Skip Softmax Attention 结合，用于同时使用框架级和内核级稀疏注意力的模型，有望实现复合收益。

## 致谢

本工作建立在 TensorRT-LLM 中 DeepSeek-V3.2 稀疏注意力实现的基础之上。我们感谢负责 radix-select Top-K 内核、稀疏 MLA 内核和稀疏注意力框架的团队所做的基础性贡献。时序相关性洞察通过 SWE-Bench 评测的真实解码数据得到验证，算法在 NVIDIA Blackwell GPU 上通过系统化 profiling 迭代优化。

该启发式引导方法展示了一个更广泛的原则：**数据感知算法设计**——根据目标工作负载的统计特性定制 GPU 内核算法——能够带来超越分布无关方法的显著性能提升。随着 LLM 推理工作负载在更长上下文和更复杂的 Agentic 行为方面持续演进，我们预期该原则将在推理技术栈中获得更广泛的应用。
