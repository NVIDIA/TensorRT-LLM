...
- **多 batch 与变长序列极致优化**：当前内核已针对 batch=1 最小延迟场景进行了重点调优；多 batch 解码已具备功能支持但尚未极致优化。针对高并发 batch 和 MTP > 1 投机解码产生的变长序列场景——各请求序列长度分化——需要自适应内核参数配置（线程数、迭代预算、候选容量）以充分释放 GPU 吞吐潜力。
