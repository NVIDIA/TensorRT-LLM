...
N-Gram speculative decoding leverages the natural repetition in many LLM workloads. It uses a Suffix Automaton to efficiently find matching patterns in previously generated text and propose draft tokens based on what followed those patterns.
...
## Highlights
* **Fast & GPU-accelerated.** N‑Gram algorithm runs on GPU via a CUDA kernel, enabling CUDA graph capture for optimal performance.
...
> **Note**: NGram now uses a Suffix Automaton (SA) with a CUDA kernel for efficient GPU-accelerated pattern matching. This enables CUDA graph capture and batched operations across multiple requests. The figures below illustrate the conceptual key-value approach for understanding.

`NGramDecodingConfig` in TensorRT LLM:
```python
spec_config = NGramDecodingConfig(
    max_draft_len = v ,             # max length of draft tokens
    max_matching_ngram_size  = k ,  # max length for keys (use -1 for longest match mode)
)
```
* **Processing New Request** ‑ The Suffix Automaton is built from the input sequence, enabling efficient pattern matching.

    Conceptually, with *max_matching_ngram_size = 3, max_draft_len = 5, input_sequence_len=8*, Figure 1 illustrates the pattern-match relationships. The SA encodes these relationships in a compact state machine.
...
<p align="center"><sub><em>Figure 1. Request initial scan (TODO image needs to be updated)</em></sub></p>

* **Per‑token update** ‑ The Suffix Automaton is extended with each new token on GPU.

    Figure 2 illustrates how new patterns become available as tokens are generated. The batched CUDA kernel extends the SA and generates draft tokens in a single GPU operation.
...
* **Lookup** ‑ construct the last k tokens as the key and propose draft tokens as its value.
    ...
    The lookup runs on GPU via a batched CUDA kernel, processing multiple requests in parallel without CPU-GPU synchronization.
