...
The VisualGen worker readiness timeout covers the complete initialization path, not only operating-system process creation. This includes distributed initialization, pipeline construction, checkpoint loading, optional compilation and warmup, and the final cross-worker synchronization. The default is 3600 seconds (one hour), which is intentionally conservative for large checkpoints loaded from cold network storage.
