...
- **Ulysses Parallelism** (`ulysses_size: N`): Splits the sequence dimension across GPUs for longer sequences. Cosmos3 rejects an explicitly selected `CUTEDSL` backend when Ulysses is enabled because request prompt lengths may differ; use another supported backend.
...
- **Context Parallel (CP)** — Partitions the sequence into shards so that each rank computes partial attention. Requires an LSE-capable attention backend (`FA4` or `CUTEDSL`). Cosmos3 rejects `CUTEDSL` when Attention2D is enabled because request prompt lengths may differ. CP can be composed with Ulysses, giving a total sequence-parallel (SP) degree = `cp_size · ulysses_size`. The CP degree depends on the implementation below:
