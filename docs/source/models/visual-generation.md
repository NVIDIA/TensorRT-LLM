...
Setting `nccl_buffer_reg: true` in `parallel_config` enables NCCL CUMEM mode before VisualGen creates its process group. This makes NCCL allocate its internal scratch buffers through VMM for inter-GPU communication (Ulysses all-to-all, ring all-gather, VAE all-reduce). The PR also includes an internal `NCCLBufferRegistrar` helper for experiments with explicit per-tensor registration; the config flag does not automatically register pipeline tensors.

| Mechanism | When active | NCCL requirement |
|---|---|---|
| **CUMEM mode** (`NCCL_CUMEM_ENABLE=1`) | Always when `nccl_buffer_reg: true` | >= 2.21 |
| **Explicit `ncclCommRegister`** | Internal helper only; not wired into the VisualGen pipeline path | >= 2.19 |

**CUMEM mode** instructs NCCL to allocate its internal scratch buffers via `cuMemCreate` (VMM). `NCCL_CUMEM_ENABLE` exists in earlier NCCL releases, but VisualGen requires NCCL >= 2.21 as a conservative floor for this opt-in path. On NVLink-connected GPUs (e.g. H100, B200 NVL), this enables NCCL to fuse collectives with NVLS (NVLink SHARP), reducing SM usage and latency. On IB-connected multi-node setups, set `NCCL_PXN_DISABLE=1` alongside `NCCL_CUMEM_ENABLE=1`; PXN is incompatible with buffer registration.
...
parallel_config:
  ...
  nccl_buffer_reg: true    # enables NCCL_CUMEM_ENABLE before process-group init
```
