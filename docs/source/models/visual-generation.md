...
- **External launcher** — SPMD-style launches where running the same script on every rank is acceptable. `RANK`/`WORLD_SIZE` (torchrun) or `SLURM_PROCID` (plain `srun`) in the environment selects this mode. This mode relies on the launcher to terminate the remaining ranks when any rank exits; `torchrun` does this, and with Slurm you must pass `srun --kill-on-bad-exit=1`, or surviving ranks keep running and hold their GPU memory after a peer or the coordinator exits.
