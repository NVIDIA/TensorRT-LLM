...
[^15]: Inkling is an NVFP4 checkpoint (`thinkingmachines/Inkling-NVFP4`) with a RoPE-free hybrid-attention text decoder plus a BF16 hMLP vision tower and dMel audio tower; video is handled as multi-frame images. The vision and audio towers stay BF16 even though the text decoder and routed experts are NVFP4. Accuracy is gated on the text decoder (GSM8K and MMLU) and the vision path (MMMU); the audio and video paths are covered by module unit tests only, not an accuracy benchmark. A reasoning parser is registered (`--reasoning_parser inkling`) for Inkling's typed-content thinking/content blocks. Not supported in this release: MTP / speculative decoding (the checkpoint ships next-N draft weights; nothing builds or loads them), LoRA, function calling, constrained/guided decoding, EPD disaggregated serving, and multimodal-hash prefix caching. Expert parallelism is available through the standard `moe_expert_parallel_size`: the 256 routed experts go through the generic fused-MoE factory, so `Mapping` derives the split and the CUTLASS backend remaps the NVFP4 per-expert scales onto each rank's slice. That backend does not support a non-divisible split, so the value must divide 256; it is rejected at load otherwise. Attention data parallelism is available through `enable_attention_dp`, and is the layout to pair with expert parallelism: attention, the embedding, the LM head and the two dense MLP layers all replicate and run over each rank's own requests, leaving only the routed experts sharded. Two things to know before enabling it. First, it costs memory rather than saving it -- what used to be a 1/tp slice is now a full copy on every rank, measured at 145.98 GB -> 162.68 GB of weights per rank at TP=4, which on a 184 GiB device leaves little room for the KV cache; the two knobs that matter pull in OPPOSITE directions, since `max_num_tokens` / `max_batch_size` bound the KV-cache estimation forward (which OOMs before the pool is sized, so `free_gpu_memory_fraction` cannot help it) while `free_gpu_memory_fraction` is what keeps the pool itself from starving the scheduler. Second, it is not bit-identical to pure TP and cannot be: under TP the attention output is an all-reduced sum of four partial results, under attention DP it is computed whole on one rank, and float addition is not associative -- so a long chain-of-thought generation forks. Measured on 30 GSM8K items, attention DP with either expert TP or `moe_expert_parallel_size=2` reproduces the TP-only extracted answer and correct/incorrect flag on every item (accuracy 0.9667, zero score flips). On 30 MMMU items, paired against a TP run at identical runtime settings, per-item results do move -- 6 score flips, 4 of them in ADP's favour, a net +2 items or 0.76 sigma, which n=30 cannot distinguish from noise. The attention backend must stay at Inkling's default (`attn_backend='INKLING'`): the Triton decode kernel reads its per-step seq_lens and page table from `InklingAttentionMetadata`, which only that backend supplies, so an override is rejected at load. KV block reuse is unsupported and defaulted off (`kv_cache_config.enable_block_reuse=False`): each decoder layer carries four depthwise short convolutions whose `kernel_size - 1` token window is per-request state outside the KV cache, so a reused prefix would resume attention from the correct history while the convolutions restarted from zeros, corrupting the first few tokens of every reused chunk. Every Inkling all-reduce is pinned to the `ONESHOT` strategy after construction to avoid a captured symmetric-all-reduce defect under CUDA graph (a 12288 B one-token decode all-reduce goes non-finite and decode collapses to a repeated token).
