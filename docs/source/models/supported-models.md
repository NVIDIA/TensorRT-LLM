...
[^17]: Kimi K3 additionally supports DSpark speculative decoding. It has no MTP or EAGLE-3 head, and its DSpark checkpoints are not compatible with plain `DFlash`.
[^18]: NGram and Suffix Automaton (SA) speculative decoding are supported for all models in this matrix and are omitted from the individual model entries.
