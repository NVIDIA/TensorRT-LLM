...
[^16]: Kimi K3 supports suffix-automaton and DSpark speculative decoding. It has no MTP or EAGLE-3 head, and its DSpark checkpoints are not compatible with the plain `DFlash` mode represented by this matrix.
[^17]: Guided decoding for `Glm4MoeForCausalLM` is currently supported only with the `xgrammar` backend; `llguidance` is not working.
