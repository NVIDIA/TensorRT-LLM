...
[^15]: NVFP4 checkpoint with a RoPE-free hybrid-attention text decoder and BF16 vision and audio towers; video is handled as multi-frame images. Use `--reasoning_parser inkling` for its typed-content thinking blocks. Enforced at load: `moe_expert_parallel_size` must divide 256, and `attn_backend` must stay in the `TRTLLM` family (the framework default). MTP, LoRA, function calling, constrained/guided decoding, disaggregated serving, and multimodal-hash prefix caching are not supported in this release. KV cache reuse additionally needs `kv_cache_config.mamba_state_config.periodic_snapshot_interval` set to a positive number of tokens, which snapshots the short-conv window a prefix hit has to restore; it applies to text prompts only. CUDA graph works at the default `moe_expert_parallel_size=1` but is broken with expert parallelism, hence No.
