...
| `InklingForConditionalGeneration` [^15] | Yes       | No         | Yes                        | No                    | No              | No  | No               | No                | No     | Yes           | Untested         | No             | Yes                      | Yes                   | No              |
...
[^15]: Inkling is an NVFP4 checkpoint (`thinkingmachines/Inkling-NVFP4`, and the smaller `thinkingmachines/Inkling-Small-NVFP4`) with a RoPE-free hybrid-attention text decoder and BF16 vision and audio towers; video is handled as multi-frame images. Use `--reasoning_parser inkling` for its typed-content thinking blocks. Enforced at load: `moe_expert_parallel_size` must divide 256, and `attn_backend` must stay in the `TRTLLM` family (the framework default) -- the backend is selected from `sparse/registry.py`, which the `VANILLA` and `FLASHINFER` families do not consult. KV cache reuse, MTP, LoRA, function calling, constrained/guided decoding, disaggregated serving (both EPD and the regular kind), and multimodal-hash prefix caching are not supported in this release. The KV-cache ones are refused at load rather than silently producing wrong output: the four per-layer short convolutions hold a `kernel_size - 1` window as per-request state outside the KV cache, so a reused prefix has no window to restore and a transferred request arrives without one. CUDA graph is listed as No because it is not supported in every configuration: capture is known to be broken with expert parallelism enabled. It does work at the default `moe_expert_parallel_size=1`, so the column will move to Yes once the expert-parallel case is fixed.
...
| `InklingForConditionalGeneration`    | Yes               | No         | No              | Yes           | Untested         | No             | Yes                   | No                        | L + I + V + A [^15] |
