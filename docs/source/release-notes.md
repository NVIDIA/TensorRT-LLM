...
- For LLaMA family models with biases, converting HF checkpoints with `*.safetensors` files under FP16/BF16 will run into error, as the biases are ignored. The suggestion to workaround these is to enable the legacy loading function by setting [the condition](../../tensorrt_llm/models/llama/convert.py?ref_type=heads#L1318-1319) to True, and this should be fixed in the next version.
