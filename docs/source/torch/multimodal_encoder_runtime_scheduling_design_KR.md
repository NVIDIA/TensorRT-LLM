...
독립적으로 실행하지 않는다. A/B 실험을 위해 opt-in 환경 변수로 eager encoder 정책을 제공한다. 이 정책은
...
명시적인 encoder token budget은 strict user contract다. `None`은 compatibility policy를 선택하며,
resolve된 LLM 값에서 시작해 model의 가장 큰 유효 atomic item을 scheduling할 수 있도록 올릴 수 있다.
...
2. Configured/base/effective 값을 log하고 compatibility mode에서 값을 올릴 때 warning을 한 번 기록한다.
...
명시적 token budget에서는 그 budget보다 큰 item을 포함한 개별 request가 invalid다. Compatibility mode에서는
startup 시 선언된 model maximum보다 큰 item이 invalid다. Input processing은 보통 media resize 또는
sampling으로 model limit 안에 맞춰야 한다. Defensive runtime check는 해당 request만 실패시키며 metadata를
동적으로 resize하거나 server를 종료하지 않는다.
...
제공하지 않는 processor는 legacy 경로를 사용한다. Runtime kill switch는 없으며 scheduler gating을 위해
iteration마다 `model.modules()`를 scan하지 않는다.
...
4. `encoder_max_num_tokens`가 explicit이었는지 보존한다. Explicit 값은 strict하게 유지하고, unset이면
   필요할 때 effective fallback을 가장 큰 유효 atomic item 크기로 올린다.
...
2. `_can_schedule_full_request_mm_batch()`로 fitting request를 scan한다.
3. 모든 pending MM request가 pristine이고 fitting set 전체가 두 encoder budget에 들어오면 기존 scheduler
   result를 변경 없이 반환한다. 기존 full-request MM encoder 경로가 model forward 안에서 실행된다.
4. 그렇지 않으면 capacity-fitting list에서 atomic item을 선택하고, partial로 남을 MM request를 제외한 뒤
...
Diagnostic environment variable 하나는 유지한다.
...
과대 계산하지 않으면서 maximum context length와 maximum item/sequence pressure를 모두 확인한다. Strict
mode에서는 두 shape 모두 explicit budget 안에 있고, compatibility mode에서는 auto-raised effective budget을
사용한다.
...
현재 observability는 의도적으로 좁다. Startup log는 configured/base/effective/model atomic maximum과
strict/auto policy를 기록하고, compatibility mode에서 값을 올릴 때 warning을 남긴다. Eager scheduling
활성화도 기록한다. `TLLM_MM_ENCODER_FORWARD_LOG=1`은
...
2. Startup이 configured/base/effective token budget을 보존한다. Explicit configuration은 strict하고 unset 값은
   model atomic maximum을 위해 auto-raise될 수 있다. Effective 값은 encoder metadata setup과
   long-item/many-item profiling에 전달된다.
...
9. 실험적 eager scheduling이 LLM capacity 전에 기존 active set에서 선택하며, 기본 off인 environment
   variable로 gate된다.
