...
| --- | --- | --- | --- |
| 두 값 모두 설정 | `encoder_max_batch_size` | `encoder_max_num_tokens` | `max(base, model atomic maximum)` |
| item 값만 설정 | `encoder_max_batch_size` | `max_num_tokens` | `max(base, model atomic maximum)` |
...
가독성을 위해 `get_multimodal_encoder_progress()`는 이 slot과 기존 full-request embedding에서 request-level
...
Full-request fast path는 5단계에서 갈라진다. Selected-item map을 만들지 않고 기존 model forward가 전체 MM
