...
> 4. **슬롯 상태 헬퍼**: item별 슬롯 쓰기를
>    `fill_mm_encoder_output_into_request()` /
>    `finalize_multimodal_encoder_request()`로 중앙화 (single-writer 불변식).
...
- **Read**: `PyExecutor._attach_mm_encoder_cache_hits()`가 `_schedule()`에서 item 선택 전에 실행된다.
  Hit는 `fill_mm_encoder_output_into_request()`으로 request-local slot에 commit되며 encoder item/token
