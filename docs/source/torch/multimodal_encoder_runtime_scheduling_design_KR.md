...
`encoder_max_batch_size`와 `encoder_max_num_tokens`를 MM encoder forward에 제출되는 실제 작업량의
...
독립적으로 실행하지 않는다. A/B 실험을 위해 opt-in 환경 변수로 independent 정책을 제공한다. 이 정책은
이미 admit된 request가 현재 iteration의 LLM-capacity filtering보다 먼저 encoder progress를 만들게 하지만,
waiting request를 미리 encode하지는 않는다.
...
stream을 재사용하지 않고, 원본 MM item을 쪼개지 않으며, 별도의 encoder-only request queue를 만들지 않는다.

## 구현 다이어그램

아래 다이어그램은 engine 초기화부터 waiting admission, iteration별 scheduling, 선택된 item 실행,
request-local commit, 조건부 LLM forward까지 현재 코드 경로를 따른다. PP/stream 동작, native
...
- `encoder_max_batch_size`: request와 modality가 공유하는, 한 iteration의 encoder 실행에 schedule되는
  최대 atomic MM item 수
...
7. Encoder-independent admission을 기본값으로 만들지 않고 통제된 실험으로 제공한다.
...
- Native encoder-decoder scheduling 또는 그 encoder stream을 변경하는 것
...
| Encoder output embedding | encoder connector/merger 이후 출력되는 한 row | LLM embedding stream에 삽입되는 row |
...
`None`은 unlimited 또는 legacy behavior가 아니라, resolve된 LLM 값으로 fallback한다는 뜻이다.
...
현재 범위는 item을 나누지 않으므로, 유효한 atomic item 하나가 configured token budget보다 크면 effective
budget을 올린다. 구현은 다음을 보장한다.

1. Configured 값과 effective 값을 모두 보존한다.
2. 값을 올릴 때 warning을 한 번 기록한다.
3. Scheduling, profiling, attention metadata, workspace sizing에 effective 값을 일관되게 사용한다.
4. 기존 positive-value validation 외의 이유로 `encoder_max_batch_size`를 올리지 않는다.

개별 request item이 startup 시 선언된 model maximum을 넘으면 invalid request다. Input processing은 보통
media resize 또는 sampling으로 model limit 안에 맞춰야 한다. Defensive runtime check는 해당 request만
실패시키며 metadata를 동적으로 resize하거나 server를 종료하지 않는다.
...
`TLLM_MM_ENCODER_RUNTIME_SCHEDULING=0`은 A/B 비교를 위해 runtime feature를 끈다. 그 외에는 combined
capability가 true일 때만 `MultimodalScheduler`를 설치한다. Mixin 밖의 모델과 완전한 item metadata를
제공하지 않는 processor는 legacy 경로를 사용한다. Scheduler gating을 위해 iteration마다
`model.modules()`를 scan하지 않는다.
...
`active_requests`를 scan하고, `_can_schedule_full_request_mm_batch()`가 scheduling call마다
...
| CUDA stream | 기존 `execution_stream` | 전용 `encoder_stream` |
| Decoder timing | 같은 iteration 실행 가능 | 이후 iteration에서 decoder scheduling 재진입 |

`MultimodalScheduler`는 wrapped scheduler의 capacity/microbatch stage를 직접 호출하기 전에
`drop_decoder_context_requests_waiting_for_encoder_output()`을 호출한다. 이는 MM request를
encoder-decoder request로 만드는 것이 아니다. `SimpleScheduler.schedule_request()`가 실제 encoder-decoder
request에 적용했을 prefilter를 보존하기 위한 것이다. MM request에는 `py_encoder_output_ready_event`가 없으므로
이 helper는 MM에 no-op이다. MM 경로는 request를 `encoder_requests`에 넣지 않고, `forward_encoder()`를
호출하지 않으며, `encoder_stream`을 사용하지 않는다.
...
가독성을 위해 `get_multimodal_encoder_progress()`는 이 slot과 legacy full embedding에서 request-level
`MultimodalEncoderProgress.PENDING`, `PARTIAL`, `READY`를 계산한다. 별도로 저장하는 두 번째 state가 아니며
C++ `LlmRequestState`도 변경하지 않는다. MM request는 `CONTEXT_INIT`을 유지하므로 같은 iteration의
...
4. `configured_encoder_max_num_tokens`를 보존하고 필요하면 effective `encoder_max_num_tokens`를 가장 큰
   유효 atomic item 크기로 올린다.
5. Effective budget으로 `MultimodalEncoderMixin` attention metadata와 direct encoder profiling을 초기화한다.
6. Runtime scheduling이 켜져 있으면 MM side-stream prefetch를 거부하고 `MultimodalScheduler`를 설치한다.
   Independent mode는 attention DP와 disaggregated KV cache transceiver도 거부한다.

`TLLM_MM_ENCODER_RUNTIME_SCHEDULING`을 끄면 wrapper와 request-local item 초기화를 건너뛰고, A/B 비교를
위해 기존 full-request MM encoder 경로를 그대로 둔다.
...
5. **Scheduling:** `MultimodalScheduler`가 coupled 또는 independent 정책을 실행하고 optional
   `request_id -> item_indices` map을 반환한다. 기존 schedule serialization이 이 map을 전달한다.
...
In-budget legacy branch는 5단계에서 갈라진다. Selected-item map을 만들지 않고 기존 model forward가 전체 MM
payload를 encode한다. Item slot은 계속 `None`이지만 완성된 `multimodal_embedding`으로 readiness를 판단한다.
Admission gate는 ready request를 명시적으로 건너뛰므로 이후 chunked-prefill iteration에서 encoder cost를
ghost charge하지 않는다.
...
per-iteration scheduling:
    ...
    independent mode: capacity 전에 이미 active인 모든 request에서 선택
...
Cost가 `[600, 500, 400]`인 request 하나, `encoder_max_batch_size=2`,
...
1. Native encoder-decoder readiness prefilter를 보존한다.
2. Active request에 기존 LLM CapacityScheduler를 실행한다.
3. `_can_schedule_full_request_mm_batch()`로 fitting request를 scan한다.
4. 모든 pending MM request가 pristine이고 fitting set 전체가 두 encoder budget에 들어오면 기존 scheduler
   result를 변경 없이 반환한다. 기존 full-request MM encoder 경로가 model forward 안에서 실행된다.
5. 그렇지 않으면 capacity-fitting list에서 atomic item을 선택하고, partial로 남을 MM request를 제외한 뒤
   나머지 LLM-eligible request에 기존 MicroBatchScheduler를 실행한다.
...
```text
max_num_items = resolved_encoder_max_batch_size  # Scheduler 내부 이름
remaining_items = max_num_items
remaining_tokens = effective_encoder_token_budget
```

User-facing knob은 기존 `encoder_max_batch_size` 이름을 유지한다. `MultimodalScheduler` 내부에서는
`max_num_items`를 사용해 이 limit가 LLM request, beam, model-internal attention segment가 아니라 atomic item을
센다는 점을 명확히 한다.
...
batch `_can_queue()` 검사 전에 `_execute_scheduled_mm_encoder_items()`를 실행한다. 그 뒤 빈 LLM batch는
...
### 실험적 independent encoder admission

다음 process environment variable이 A/B 실험을 켠다.

```text
TLLM_MM_ENCODER_INDEPENDENT_SCHEDULING=1
```

기본값은 `0`이다. 초기 실험에서는 의도적으로 public `LlmArgs` field로 만들지 않는다.

두 diagnostic/rollout environment variable도 있다.

```text
TLLM_MM_ENCODER_RUNTIME_SCHEDULING=0  # item scheduling 비활성화; 기본값 1
TLLM_MM_ENCODER_FORWARD_LOG=1         # Qwen-style grid item/token count 기록
```

Independent mode는 LLM capacity filtering 전에 기존 `active_requests`에서 encoder item을 선택한다. 이미
...
Independent mode는 attention data parallelism 또는 disaggregated KV cache transceiver가 켜지면
`NotImplementedError`를 낸다. 기본 coupled 정책에는 이 명시적 거부가 없다. 모든 item-scheduled mode는
`TLLM_MM_SIDE_STREAM_MAX_AHEAD > 0`도 executor 초기화에서 거부한다. MM side-stream prefetch가
request-local item state와 통합되지 않았기 때문이다. 일반 executor overlap loop는 계속 켜져 있지만 MM
work는 아래 설명처럼 execution stream에서 synchronize된다.
...
legacy full-request MM 경로도 PP stage마다 encoder를 중복 실행한다. 따라서 item scheduling은 stage 0 전용
...
def prepare_multimodal_items(
    ...
    """CPU에서 item을 slice하고 output length와 modality를 보존한다."""

def encode_prepared_multimodal_items(
    self,
    prepared_items: Sequence[tuple[MultimodalParams, int, str]],
) -> list[torch.Tensor]:
    """Prepared item을 encode하고 입력 순서대로 item별 tensor를 반환한다."""
```
...
`prepare_multimodal_items()`를 호출하며, 준비된 microbatch만 전송한 다음
`encode_prepared_multimodal_items()`를 호출한다.
...
수 없는 향후 모델은 `encode_prepared_multimodal_items()`를 override하거나 더 엄격한 compatibility key를
...
`encoder_max_batch_size`는 원본 item을 세지만 `AttentionMetadata.max_num_requests`는 internal sequence,
...
`setup_attn_metadata()`는 다음 model hook을 통해 이 mapping을 얻는다.
...
이 module scan은 initialization-only이며 iteration별 scheduler gating과 무관하다. Qwen에서 `T`를
`effective_encoder_max_num_tokens`, `U = spatial_merge_unit`(spatial merge size의 제곱)이라고 하자.
Qwen2/2.5 full-attention temporal segment는 최소 `U`개의 physical token을 포함한다. Nonempty window도 양수 개의
merged cell로 구성되므로 최소 `U`개의 physical token을 포함한다. Qwen3 temporal segment도 같은 minimum을
가진다. 따라서 구현이 사용하는 정확한 static upper bound는 다음과 같다.
...
Pixtral은 scheduled image item 하나를 nonempty attention context 하나로 매핑한다. 각 context에는 최소 한 개의
physical token이 있으므로 두 budget 축을 모두 사용한다.

```text
Pixtral attention max_num_requests = max(1, min(encoder_max_num_items, T))
```

Mistral3는 `PixtralVisionModel`을 vision tower로 사용하므로 같은 mapping을 상속한다. 보통 item bound가 더
작지만 token 항도 유지해 token-to-context capacity contract를 보존한다. Runtime scheduling 대상인 Qwen과
...
가장 큰 atomic item을 위해 startup에서 올린 값을 포함한 effective token budget이 attention metadata setup과
direct encoder profiling에 전달된다.
...
Same-step consumption에는 이 순서가 필요하다. MM item execution은 native encoder-decoder의
`encoder_stream`이 아니라 기존 executor `execution_stream`을 쓴다.
...
두 wait는 selected MM work를 이전 current-stream work 뒤에 직렬화하고, 이후 LLM work를 MM completion 뒤에
직렬화한다. Correctness는 명시적이지만 MM request가 들어올 때 일반 overlap executor의 이득을 줄이는
synchronization bubble이 생길 수 있다. Overlap control flow 자체는 계속 켜져 있다. 별도 MM side-stream
prefetch는 item scheduling과 조용히 결합하지 않고 initialization에서 거부한다.
...
encoding을 끝낼 수 있는 independent mode에서는 특히 위험하다.

Independent mode를 supported default로 만들기 전에 다음 후속 작업이 필요하다.

1. Resident MM output embedding을 item/byte로 추적한다.
2. Item forward 뒤가 아니라 scheduling 전에 resident capacity를 reserve한다.
3. Resident budget이 고갈되면 independent selection을 멈춘다.
...
별도 cross-request cache는 scheduler identity를 바꾸지 않고 통합할 수 있다. Hit는 대응 request-local item
slot을 채우고 encoder item/compute-token budget을 소비하지 않는다. Cache insertion은 item result의 성공적
commit 뒤에만 한다. Cache를 켜기 전에 다음이 필요하다.

- Canonical item ordering을 hash/UUID와 검증
- Key에 model, preprocessing, dtype, 향후 encoder-adapter identity 포함
- Compute budget을 쓰지 않는 cached tensor도 resident memory에 accounting
- Cancel 시 eviction과 reference release 정의
- 현재 one-modality hashing 가정에 cache가 의존하지 않도록 보장
...
9. Default/independent admission에서 동일 encoder output과 final greedy token
...
- LLM-capacity rejection 뒤 independent selection
...
현재 observability는 의도적으로 좁다. Startup log는 configured token budget이 effective atomic-item budget으로
올라갈 때 warning을 남기고 independent scheduling 활성화를 기록한다. `TLLM_MM_ENCODER_FORWARD_LOG=1`은
encoder forward 시 Qwen-style grid 기반 item/token count를 기록한다. Qwen-like input 전용 진단이므로
Mistral input에는 0을 기록할 수 있다. Normal scheduler는 item마다 log하지 않는다.
...
- Independent mode를 중심으로 request-local ready byte
...
- Default coupled admission과 independent admission
...
independent encoder progress가 E2E latency 또는 throughput을 개선하는지가 핵심이다.
...
2. Startup이 model maximum atomic item cost를 검증하고 configured/effective token budget을 모두 보존하며,
   effective 값을 encoder metadata setup과 profiling에 전달한다.
...
4. `ScheduledRequests`와 `SerializableSchedulerOutput`이 기존 schedule distribution으로 selected item ID를
   전달한다.
5. 기본 wrapper가 LLM capacity를 먼저 실행하고, in-budget full-request fast path를 유지하며, overflow atomic
   item을 별도 item/token budget으로 pack하고, partial MM request를 LLM microbatch 전에 제외한다.
...
9. 실험적 independent scheduling이 LLM capacity 전에 기존 active set에서 선택하며, 기본 off인 environment
   variable로 gate된다.
10. 검증되지 않은 path를 조용히 조합하지 않도록 MM side-stream prefetch를 거부하고, independent mode에서는
...
3. MM side-stream prefetch 및 전용 overlap/concurrency design
...
이미 복잡한 executor에 별도 ownership/lifecycle domain을 추가하므로 deferred했다. 실험적 independent mode는
...
1. Independent mode에 어떤 resident-output byte limit과 eviction policy를 써야 하는가? GPU-to-host offload는
   현재 작업에서 계획하지 않는다.
2. MM prefetch와 overlap scheduler가 item ownership 및 CUDA stream을 어떻게 조정해야 하는가?
3. 향후 public configuration이 실험적 environment variable을 대체해야 하는가?
