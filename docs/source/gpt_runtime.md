...
LLaMA, for example. Encoder-decoder models, like T5, that will be added to
TensorRT-LLM in a future release, will require adjustments to those runtime
components.

## The Session

The main component of the C++ runtime is the session. For GPT-like
auto-regressive models, there currently are two classes that implement
the session:

 * [`GptSession`](../cpp/include/tensorrt_llm/runtime/gptSession.h) is for
   models compiled without the [in-flight batching](./in_flight_batching.md)
   feature,
 * [`GptSessionWithDecoderBatch`](../cpp/include/tensorrt_llm/runtime/gptSessionWithDecoderBatch.h)
   is for models compiled with the [in-flight batching](./in_flight_batching.md)
   feature.

Those two classes will be unified in a future release (or, at least, share a
common base class). The `GptSessionWithDecoderBatch` class, which is more
general, is a good proxy for what the future unification could look like.

### Creation

The constructors of those classes allow users to specify the model and the
 ...
 * `numKvHeads`, is the number of heads for K and V in the attention component.
   ...
   Multi-query Attention. Otherwise, it uses Group-query Attention, instead of
   multihead attention (MHA). See [GPT Attention](gpt_attention.md),
 ...
 * `usePagedKvCache`, indicates if the K/V cache uses paging.
   See [GPT Attention](gpt_attention.md),
...
***GptSessionWithDecoderBatch***

The `GptSessionWithDecoderBatch::setup` member function is slightly different.
It replaces the fourth argument with the `beamWidth` argument that controls the
beam width for the different requests. The `SamplingConfig` is removed and
passed as a `std::vector` to the `GptSessionWithDecoderBatch::generate` member
function (to support one sampling configuration per request).
