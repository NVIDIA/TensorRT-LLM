...
| 6b — GPU-side LoRA expansion (lift host-sync, enable graph capture) | 6b.A done; 6b.B done (kernel + unit tests, unwired); 6b.C.1 done (kernel + unit tests, unwired); 6b.C.2.a done (device scratch + LoraParams field, unconsumed); 6b.C.2.b / 6b.D / 6b.E in progress. | Replaces the `setupLoraWorkspace` / `LoraImpl::run` host-CPU branching with a device-side problem-builder kernel. Re-enables the skipped graph-capture parity test. |
...
- **6b.C — wire `cudaGraphGroupedGemm` (in progress).** Replace the `LoraImpl::run` host-side run-length encoding + pageable staging with `cudaGraphGroupedGemm` / `cudaGraphSplitKGroupedGemm` invocations that take the device pointers built in 6b.B. Split into two sub-checkpoints because the integration is sizeable:
  ...
  - **6b.C.2 — integration (split into 6b.C.2.a + 6b.C.2.b).** The integration is itself two slices so the diff in each stays reviewable:
    ...
    - **6b.C.2.b — runner integration (next).** Branch `setupLoraWorkspace` on `device_path.enabled` (call `launchMoeLoraPointerExpand` instead of the host loop) and replace the `LoraImpl::run` call sites in `loraFC1`/`loraFC2` with a `launchMoeLoraProblemBuilder` + `cudaGraphSplitKGroupedGemm` + `cudaGraphGroupedGemm` sequence. Add the eager-vs-host parity test (`pytest tests/unittest/_torch/lora/test_moe_lora_op.py` with the env var toggled).
