...
- **6b.C — wire `cudaGraphGroupedGemm` (planned).** Replace the `LoraImpl::run` host-side run-length encoding + pageable staging with `cudaGraphGroupedGemm` / `cudaGraphSplitKGroupedGemm` invocations that take the device pointers built in 6b.B. Behind an environment flag for a transitional release so we can A/B the new path; baseline parity is gated on `test_moe_lora_op` and the (still skipped) graph capture test.
