...
| 6b — GPU-side LoRA expansion (lift host-sync, enable graph capture) | 6b.A done; 6b.B done; 6b.C.1 done; 6b.C.2.a done; 6b.C.2.b code complete (awaiting GPU validation -- eager parity test added); 6b.D / 6b.E in progress. | Replaces the `setupLoraWorkspace` / `LoraImpl::run` host-CPU branching with a device-side problem-builder kernel. Re-enables the skipped graph-capture parity test. |
...
- **6b.D — lift the CUDA-graph rejection (planned).** Drop the `TORCH_CHECK(!isCapturing)` in `moeOp.cpp`, drop the corresponding skip on `test_moe_lora_slot_indexed_cuda_graph_replay_matches_eager`, and add an explicit `cudaStreamCaptureStatus` check + capture-aware logging. F7 is marked resolved in this file once the test goes green.
