...
#### Basic Wide-EP Configuration

```yaml
moe_config:
    backend: WIDEEP
    max_num_tokens: 9216
    load_balancer: moe_load_balancer.yaml  # optional
```

| Parameter | Description | Default | Notes |
|-----------|-------------|---------|-------|
| `backend` | MoE backend type | `CUTLASS` | Set to `WIDEEP` to enable wide EP |
| `max_num_tokens` | Maximum tokens processed per chunk | `None` | Controls memory usage for large EP sizes |
| `load_balancer` | Configuration for MoE load balancing | `None` | Path to EPLB configuration file |

#### Load Balancer Configuration

**Offline EPLB Configuration:**
```yaml
num_slots: 288
...
initial_global_assignments:
  ...
  # ... more layer assignments
```

**Online EPLB Configuration:**
```yaml
num_slots: 288
...
| `layer_updates_per_iter` | Layers updated per iteration | `0` | `0` = offline, `>0` = online |
| `initial_global_assignments` | Initial expert-to-slot mapping | `None` | Required for offline EPLB |

### Usage Examples

#### 1. Basic Wide-EP Setup

```bash
# Enable Wide-EP with basic configuration
cat > parallel_config.yaml <<EOF
...
#### 2. Wide-EP with Offline Load Balancer

```bash
# Step 1: Collect expert statistics
...
trtllm-bench --model deepseek-ai/DeepSeek-R1 \
    --tp 32 --ep 32 \
    --extra_llm_api_options parallel_config.yaml

# Step 2: Generate EPLB configuration
...
# Step 3: Run with EPLB
cat > parallel_config_eplb.yaml <<EOF
tensor_parallel_size: 36
moe_expert_parallel_size: 36
enable_attention_dp: true
moe_config:
...
#### 3. Wide-EP with Online Load Balancer

```bash
# Create online EPLB configuration
cat > moe_load_balancer.yaml <<EOF
num_slots: 288
layer_updates_per_iter: 2
EOF

# Run with online EPLB
cat > parallel_config_online.yaml <<EOF
tensor_parallel_size: 36
moe_expert_parallel_size: 36
enable_attention_dp: true
moe_config:
...
#### Memory Management
- **max_num_tokens**: Controls chunking for large EP sizes to prevent OOM
- **Expert slots**: More slots provide better load balancing but increase memory usage
- **Weight placement**: Weights can be stored in host memory and transferred via C2C

#### Communication Optimization
- **MNNVL**: Utilizes NVIDIA GB200 Multi-Node NVLink for efficient inter-GPU communication
- **Asynchronous updates**: Layer-wise updates minimize inference disruption
- **Stream overlap**: Lightweight kernels overlap with forward passes

#### Load Balancing Effectiveness
- **Offline EPLB**: Provides optimal balance based on historical data
- **Online EPLB**: Adapts to real-time traffic patterns
- **Imbalance ratio**: Target < 0.15 for good performance

### Troubleshooting

#### Transparent HugePages
If you encounter `madvise(MADV_HUGEPAGE) failed`:
```bash
...
#### Memory Issues
- Increase `max_num_tokens` if experiencing OOM errors
- Reduce `num_slots` if memory is constrained
- Monitor expert statistics to identify hot experts

#### Performance Issues
- Ensure MNNVL is properly configured for GB200 systems
- Verify load balancer configuration matches workload patterns
- Check communication patterns for bottlenecks

### Advanced Features

#### Disaggregated Serving
Wide-EP supports disaggregated serving architectures where different components run on separate nodes:
- Context servers for prompt processing
- Generation servers for token generation
- Expert servers for MoE computation

#### SLURM Integration
TensorRT-LLM provides SLURM scripts for easy deployment on HPC clusters:
```bash
# Use provided SLURM scripts
cd examples/wide_ep/slurm_scripts/
# Modify submit.sh with your cluster configuration
./submit.sh
```
