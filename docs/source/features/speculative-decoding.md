...
When the NGram algorithm is used, TRT-LLM uses a Suffix Automaton to efficiently find matching patterns in the generated sequence. The NGram matching procedure can be tuned with the following options:

* `max_draft_len`: Maximum draft candidate length.
* `max_matching_ngram_size`: Maximum prompt suffix length to match. Use a positive value (e.g., 2, 3, 4) for fixed-size matching, or -1 for longest match mode which finds the longest matching suffix.

```python
from tensorrt_llm.llmapi import NGramDecodingConfig

speculative_config = NGramDecodingConfig(
    max_draft_len=3, max_matching_ngram_size=4)
