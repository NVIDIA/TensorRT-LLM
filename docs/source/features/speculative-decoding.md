...
When the NGram algorithm is used, TRT-LLM keeps every request's token history (the prompt plus the tokens accepted so far) in a GPU pool. After each target model step, a kernel finds the longest earlier occurrence of the sequence's suffix in that history and proposes the tokens that followed it as the next drafts. For example, if the 3-gram ["The ", " future ", " is"] appeared earlier followed by [" bright", " because"], those two tokens are drafted. Drafting runs inside the target model forward, so NGram supports CUDA graphs and the overlap scheduler. The matching procedure can be tuned with the following options:

* `max_draft_len`: Maximum draft candidate length.
* `max_matching_ngram_size`: Maximum suffix length to match against the token history.
* `is_public_pool`: If true, the token histories of all in-flight requests are searched for a match (a request's own history is preferred). Otherwise, each request only searches its own history.
* `is_use_oldest`: If true, the earliest occurrence of the matched suffix is used to draft. Otherwise, the latest occurrence is used.
* `is_keep_all`: Accepted for backward compatibility and ignored; the full token history is always searched.
