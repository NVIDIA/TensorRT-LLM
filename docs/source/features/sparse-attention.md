...
- **Set `target_sparsity` ∈ [0, 1].** The runtime maps it to
  ...
  This checkpoint metadata is generated by
  [NVIDIA Model Optimizer](https://github.com/NVIDIA/Model-Optimizer). A
  checkpoint may contain multiple `config_groups` for different sparse
  algorithms, but at most one group may configure skip-softmax. Multiple
  groups whose `algorithm` is `"skip_softmax"` are invalid. Layer `ignore`
  patterns and calibration defaults come from that single skip-softmax group.
  ...
  - `prefill` / `decode` — per-phase coefficient dictionaries; each
    ...
    `target_sparsity`).
  - `target_sparsity` — optional checkpoint-provided target values. User
    config values still override them.
  - `ignore` — optional fnmatch layer patterns where the calibrated
    skip-softmax config should not apply.
