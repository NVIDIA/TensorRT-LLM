...
This checkpoint config is **optional**. It is only required when using `target_sparsity`, which is often more intuitive than directly choosing the kernel-facing `threshold_scale_factor`. But please note that `target_sparsity` only serves as a guidance, the actual **achieved** sparsity in the kernel would vary.
