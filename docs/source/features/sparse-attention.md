...
- **Set `target_sparsity` ∈ [0, 1].** The runtime maps it to
  ...
  checkpoint may contain multiple `config_groups`; skip-softmax scans the groups
  whose `algorithm` is `"skip_softmax"`. Layer `ignore` patterns are merged
  across matching groups, and calibration defaults come from the first matching
