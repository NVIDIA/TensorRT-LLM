...
1 are required. Variable sliding-window pools, internal host offloading, and
Mamba/hybrid state remain unsupported. Connector support for V2 layer-group
layouts and backend-specific integrations are separate extensions.
...
The filesystem example below provides an opted-in implementation.
