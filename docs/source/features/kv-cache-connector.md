...
* **`request_finished_by_layer_group(self, request: LlmRequest, cache_block_ids_by_layer_group: list[list[int]]) -> bool`**
  ...
  * **Which form to implement**: prefer the per-layer-group forms. They cover every model — with a single layer group the base implementation folds them back to the flat call — so one connector runs everywhere without a second code path. The flat forms run only where the cache has one layer group, which is every non-VSWA, non-hybrid model.

    Implement both per-layer-group methods or neither. On a multi-group model a connector that defines only the flat forms, or that mixes the two, is rejected during executor bring-up, naming the method it is missing. See [Running under VSWA](#running-under-vswa).
...
| Configuration | Rejected on | Mechanism |
|---|---|---|
| Speculative decoding | `KVCacheManagerV2` | Rejected draft tokens shrink a request's page list, and the freed slot goes to whichever request allocates next. The connector is never told the tail block moved. |
| A capacity scheduler policy other than `GUARANTEED_NO_EVICT` | `KVCacheManager` (V1) | A destroyed-and-replayed request comes back on different pages, and V1 keeps the connector's per-request block delta across the replay. `KVCacheManagerV2` drops the delta with the allocation, so it accepts any policy — and its scheduler runs `MAX_UTILIZATION` whatever is configured. |
| A host or disk cache tier | `KVCacheManagerV2` | Tier eviction reassigns the GPU slot. See [KV cache tiers are GPU-only under a connector](#kv-cache-tiers-are-gpu-only-under-a-connector). |

Speculative decoding is also untested with a connector on `KVCacheManager` (V1), where it is not
rejected — the same list shrink happens there. Treat the combination as unsupported on both managers.
...
A model whose layers all share one sliding window stays a single layer group, so the flat callbacks still apply and such a connector is not refused. What differs from V1 is that the callbacks cover the **live window only**. Blocks the window has passed report `-1` (`BAD_PAGE_INDEX`) in place — the list stays aligned to block ordinals, so entry `i` still describes tokens `[i * tokens_per_block, (i+1) * tokens_per_block)`, but the up-front blocks carry no page and are not available to load into or save from. V1 dropped those entries from the list instead, so a connector carried over from V1 has to add the `valid_page_slots` filter described in [Blocks with no page](#blocks-with-no-page); without it a `-1` resolves to the last page slot of the pool. A warning naming the window size is logged at start-up when a flat-only scheduler is attached to such a model.
