...
- **Counter-signals:** GPU-bound steps (host prep already hidden — nothing to win, and the extra in-flight state costs memory); beam search / speculative decoding / encoder models (mechanism auto-disables); debugging sessions that need strictly serialized step semantics.
