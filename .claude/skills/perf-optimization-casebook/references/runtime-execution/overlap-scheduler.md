...
- **Counter-signals:** GPU-bound steps (host prep already hidden — nothing to win, and the extra in-flight state costs memory); beam search / encoder models / the speculative-decoding modes that do not support it — N-gram, user-provided drafter and save-hidden-states (`SpeculativeDecodingMode.support_overlap_scheduler`); one-model MTP, MTP-Eagle, Eagle3, SA, PARD/DFlash/DSpark and Draft-Target all keep overlap; debugging sessions that need strictly serialized step semantics.
