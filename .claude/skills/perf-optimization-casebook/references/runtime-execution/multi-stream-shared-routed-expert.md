...
- **Prior art:** PR #3459. Files: `_torch/modules/multi_stream_utils.py` (`maybe_execute_in_parallel`), `modeling_deepseekv3.py` (`Deepseekv3MoE.forward`), `_torch/modules/attention.py` (MLA), `_torch/utils.py` (`AuxStreamType`, `EventType`). Owning specialist: **perf-torch-cuda-graph-specialist**.
