...
description: "Translates a HuggingFace model into a prefill-only AutoDeploy custom model using reference custom ops, validates with hierarchical equivalence tests. Use when the user wants to onboard, convert, or port a HuggingFace model to TensorRT-LLM AutoDeploy, or mentions custom model onboarding, AD custom model, prefill-only export, HuggingFace to AutoDeploy translation, or model equivalence testing."
license: Apache-2.0
metadata:
...
For porting an existing AutoDeploy custom model to explicit sharding hint ops, see [references/sharding-ir.md](references/sharding-ir.md). Covers the step-by-step porting procedure (copy → replace linears → insert all_reduce → register → YAML config), reference examples, layer-specific sharding patterns (MHA, MLP, MoE, MLA, SSM, GatedDeltaNet), common pitfalls, and validation checklist.
