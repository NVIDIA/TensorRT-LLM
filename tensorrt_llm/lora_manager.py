...
class LoraManager(object):
    ...
    def load_from_hf(
        ...
        def preprocess_lora_weights(lora_model, swap_gate_up_proj):
            # Swap weights of gate_up_proj
            for key, value in lora_model.items():
                if "gate_up_proj.lora_B.weight" in key and swap_gate_up_proj:
        ...
        def load_from_model_dir(uid, model_dir, hf_config):
            ...
            swap_gate_up_proj = getattr(model_config, "swap_gate_up_proj", True)
            lora_model = preprocess_lora_weights(lora_model, swap_gate_up_proj)
