...
def sign_internal_disagg_request(secret: str, request) -> str:
    return hmac.new(secret.encode("utf-8"), _canonical_disagg_payload(request),
...
def build_internal_disagg_auth_headers(secret: str, request) -> dict[str, str]:
    return {
        ...
        sign_internal_disagg_request(secret, request)
    }


def verify_internal_disagg_request(secret: str, request,
...
def validate_internal_disagg_request(secret: Optional[str], request,
                                     headers: Optional[Mapping[str, str]]) -> None:
    if not request_requires_internal_disagg_auth(request):
        return
    if not secret:
        raise ValueError(
            "ctx_info_endpoint requires authenticated disaggregated proxy request"
        )
