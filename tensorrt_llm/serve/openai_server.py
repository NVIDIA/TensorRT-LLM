...
class OpenAIServer:
    ...
    async def get_iteration_stats(self) -> JSONResponse:
        ...
        if not enable_iter_perf_stats:
            return stats
        iter_perf_stats_size = self.llm.pytorch_backend_config.iter_perf_stats_size
        if iter_perf_stats_size is not None:
            for stat in self.llm.get_stats_async(2).get_latest(iter_perf_stats_size):
                stats.append(stat)
        else:
