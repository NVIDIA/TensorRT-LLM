...
Extracted from ``router.py`` so both the surface routers there and the
centralized KV-cache router core (``kv_cache_router/router_core.py``) can share a
single implementation of block hashing without importing the whole router module.
"""
