...
class Qwen3CoderToolParser(BaseToolParser):
        ...
        while True:
            ...
            if self._function_name_sent:
                ...
                if self.tool_call_end_token in self._buf:
                    ...
                    current_streamed = self.streamed_args_for_tool[
                        self.current_tool_id]
                    if current_streamed:
                        ...
                        if open_braces > close_braces:
                            ...
                            self.streamed_args_for_tool[
                                self.current_tool_id] = (current_streamed + "}")
    ...
    def _parse_and_stream_parameters(self,
                                     text_to_parse: str) -> List[ToolCallItem]:
        """
        ...
        param_matches = list(
            re.finditer(r"<parameter=([^>]+)>(.*?)</parameter>", text_to_parse,
                        re.DOTALL))
        ...
        if new_params != self._current_parameters:
            previous_args_json = self.streamed_args_for_tool[
                self.current_tool_id]
            ...
            if not self._current_parameters:
                ...
                self.streamed_args_for_tool[
                    self.current_tool_id] = json_fragment

            else:
                # Additional parameters - add them incrementally
                new_keys = set(new_params.keys()) - set(
                    self._current_parameters.keys())
                if new_keys:
            ...
            self.prev_tool_call_arr[
                self.current_tool_id]["arguments"] = new_params
    ...
    def _extract(self, text: str,
                 tools: List[Tool]) -> Tuple[str, List[ToolCallItem]]:
