...
class OpenAIDisaggregatedService(OpenAIService):
    ...
    async def _send_disagg_request(
        ...
        ctx_response = None
        gen_req = request
        request_id = get_global_disagg_request_id(self._config.node_id)
        if need_ctx:
            ctx_req = self._get_ctx_request(request, request_id)
    ...
    def _get_ctx_request(
        self, request: UCompletionRequest, ctx_request_id: int
    ) -> UCompletionRequest:
        ctx_request = copy.deepcopy(request)
        ctx_request.disaggregated_params = DisaggregatedParams(
            request_type="context_only", ctx_request_id=ctx_request_id
        )
