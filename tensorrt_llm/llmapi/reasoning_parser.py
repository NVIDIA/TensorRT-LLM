...
class ReasoningParserFactory:
    ...
    def needs_raw_special_tokens(cls, reasoning_parser: str) -> bool:
        """Whether the registered parser must see special tokens in the
        decoded stream (see ``BaseReasoningParser.needs_raw_special_tokens``)."""
...
class KimiK3ReasoningParser(BaseReasoningParser):
    ...
    def _partial_suffix_len(text: str, markers: tuple[str, ...]) -> int:
        """Length of the longest text suffix that is a proper prefix of any
        marker. Markers may contain internal ``<`` (e.g.
        ``<|close|>tools<|sep|>``), so every suffix length up to
        ``len(marker) - 1`` must be checked, not just the one starting at
        the last ``<``."""
        best = 0
    ...
    def _step(self, content: list, reasoning: list) -> bool:
        """Consume as much of the buffer as possible; return False when more
        input is needed."""
        buf = self._buffer
        if self._state == "done":
            self._buffer = ""
        ...
        if self._state == "tools_pass":
            ...
            return True
        raise RuntimeError(
            "Unreachable state in `KimiK3ReasoningParser._step`")
