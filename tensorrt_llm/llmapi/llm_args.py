...
class TorchLlmArgs(BaseLlmArgs):
    ...
    disable_flashinfer_fused_add_rmsnorm_with_nvfp4_marlin: bool = Field(
        default=False,
        description=(
            "Disable the BF16 layer-boundary residual-add RMSNorm path from "
            "using FlashInfer fused_add_rmsnorm when NVFP4 Marlin GEMM is "
            "selected. Disabled by default (FlashInfer path is on); set true "
            "to retain the ATen fallback. This does not change Marlin GEMM "
            "selection or NVFP4 quantization."),
        status="beta")
