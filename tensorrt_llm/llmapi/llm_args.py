    ...
    def validate_speculative_config(self):
        if self.speculative_config:
            ...
            if self.speculative_config.use_rejection_sampling:
                # Supported paths: Eagle3 one-model, vanilla MTP, PARD, DFlash,
                # DraftTarget one-model. Classify by spec-dec mode (not config
                # class): MTP-Eagle variants share MTPDecodingConfig but route
                # through different workers and are unsupported.
                from tensorrt_llm._torch.speculative.interface import \
                    SpeculativeDecodingMode as TorchSpeculativeDecodingMode
                ...
                # SpecWorkerBase.maybe_gather_draft_logits_for_rejection).
