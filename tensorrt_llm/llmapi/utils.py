...
class BackendType(MultiValueEnum):
    ...
    @staticmethod
    def print_backend_info(backend: Optional[str]):
        """ Print the backend info. """
        backend = backend or BackendType.default_value()
        logger.info(f"Running with {BackendType(backend)} backend.")

    # TODO[Superjom]: Remove this method after v1.0.0 is released.
    @staticmethod
    def get_default_backend_with_warning(backend: Optional[str]) -> str:
