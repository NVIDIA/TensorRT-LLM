...
class ModelLoader:
    ...
    def _update_from_hf_quant_config(self) -> bool:
        ...
        if hf_quant_config is not None:
            ...
            parsed_quant_config, _ = ModelConfig.load_hf_quant_config(
                hf_quant_config, self.llm_args.moe_config.backend)
            if parsed_quant_config.quant_algo is None:
                raise NotImplementedError(
                    f"Unsupported quantization_config: {hf_quant_config}.")

            quant_config.quant_algo = parsed_quant_config.quant_algo
            quant_config.group_size = parsed_quant_config.group_size
            quant_config.exclude_modules = parsed_quant_config.exclude_modules
            return True
