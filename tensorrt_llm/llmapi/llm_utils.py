...
# yapf: disable
from .llm_args import (SWA_SCRATCH_AUTO_ATTN_BACKENDS, CalibConfig,
...
def _resolve_swa_scratch_reuse_auto(llm_args: 'TorchLlmArgs',
    ...
    a V2 KV cache manager and an attention backend that can address a scratch
    page on every path it serves -- which today means FlashInfer only, see
    ``SWA_SCRATCH_AUTO_ATTN_BACKENDS``. Whether the model has sliding-window
    layers at all is decided by the manager, which is the first place the
    per-layer window vector exists; without them the feature is inert rather
    than wrong.
    ...
    # attn_backend is a free-form str and get_attention_backend() upper-cases
    # it, so "flashinfer" is a working config that must not silently lose the
    # feature here.
    enable = (uses_v2 and llm_args.attn_backend.upper()
              in SWA_SCRATCH_AUTO_ATTN_BACKENDS)
