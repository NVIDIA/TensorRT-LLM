...
class RpcWorkerMixin:
    ...
    def init_postproc_workers(self):
        """Spawn local PostprocWorker processes and route their outputs back
        into the RPC response stream.
        ...
        self._postproc_input_queues = [
            IpcQueue(is_server=True, name=f"rpc_worker_postproc_input_{i}")
            for i in range(num)
        ]
        self._postproc_collector = IpcQueue(is_server=True,
        ...
        self.set_postproc_queues(self._postproc_input_queues,
                                 coexist_with_result_queue=True)
        ...
        self._postproc_collector_thread = threading.Thread(
            target=_collect_postproc_outputs,
            name="rpc_worker_postproc_collector",
            daemon=True)
