...
from .utils import ExecutorResponse, has_event_loop
    ...
    def _handle_response(self,
        ...
        if isinstance(response, PostprocWorker.Output):
            ...
            if self._background_error_handler and (
                    handler := self._background_error_handler()):
                handler()
        elif isinstance(response, ExecutorResponse):
            ...
            if self.sampling_params.use_beam_search:
                for beam_idx, _ in enumerate(tensors.output_token_ids):
                    self._handle_sequence(finish_reasons, tensors, beam_idx)
            else:
                self._handle_sequence(finish_reasons, tensors,
