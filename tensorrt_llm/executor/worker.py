...
from ..lora_manager import LoraManager, PeftConfig
...
class ExecutorBindingsWorker(GenerationExecutor):
    ...
    def __init__(
        ...
        if self.rank == 0 and isinstance(self.engine, tllm.Executor):
            ...
            self._runtime_model_config = _engine_config_to_model_config(
                engine_config)
            # TODO (dafrimi): not sure why we need this sincce its tllm.Executor
            if peft_config is None:
                self.peft_config = PeftConfig()
                self.peft_config.update_model_config(
                    self._runtime_model_config.hidden_size,
                    self._runtime_model_config.dtype)
        ...
        # FIXME (dafrimi) consider how to move into _create_engine()
        if getattr(executor_config, "backend",
                   "") == "pytorch" and peft_config is not None:
            self._lora_manager = LoraManager()
            self.peft_config = peft_config
            self._lora_manager.load_from_ckpt(
                peft_config.lora_manager_prefetch_dir_list,
                model_config=self.peft_config,
    ...
    def _load_lora_adapter(self, lora_request: LoRARequest):
        self._lora_manager.load_from_ckpt(
            ...
            if self._runtime_model_config else self.peft_config,
            runtime_mapping=None,
    ...
    def _enqueue_request(self, request: GenerationRequest) -> int:
        ...
        if self._lora_manager is not None and request.lora_request is not None and int(
                request.lora_request.adapter_id
        ) >= 0:  # TODO (dafrimi) check why do we need adapter_id >= 0, we might want to support -1 as baseline with no lora adapter
