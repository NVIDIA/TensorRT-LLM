...
class WorkerBase:
    ...
    This class is responsible for managing the states of the worker such as
    engines. It doesn't implement the actual main-loop of the worker, neither
    threads, IPCs or RPCs.
    """
    ...
    def __init__(
        ...
        if isinstance(engine, list):
            # For multi-GPU setups, subclasses should handle rank selection
            pass  # This will be handled by subclasses
        ...
        self.engine = self._create_engine(engine, executor_config, lora_config,
                                          garbage_collection_gen0_threshold)

        self._lora_manager: Optional[LoraManager] = None
        self._prompt_adapter_manager: Optional[PromptAdapterManager] = None
        self._runtime_model_config: Optional[ModelConfig] = None
        self._lora_model_config = None

        self._initialize_adapters(engine, executor_config, lora_config)

    def _create_engine(self, engine, executor_config, lora_config,
                       garbage_collection_gen0_threshold):
        """Create the engine - to be potentially overridden by subclasses for different device handling"""
        device_id = 0  # Default device, subclasses can override
    ...
    def _initialize_adapters(self, engine, executor_config, lora_config):
        """Initialize LoRA and prompt adapters"""
        # This should be implemented by subclasses that know about ranks

    def return_queue(self, client_id: int):
        """ Return the appropriate queue for the client. Base implementation returns the result queue directly. """
    ...
    def abort_request(self, client_id: int) -> None:
        ...
        if self.engine and self.engine.can_enqueue_requests():
    ...
    def _has_background_error(self) -> bool:
        # Base implementation - subclasses should override if they have error queues
        return False

    def _create_error_response(self, response: tllm.Response) -> ErrorResponse:
        # Base implementation - subclasses should override if they have error queues
        return ErrorResponse(response.client_id, "Background error occurred",
                             response.request_id)
    ...
    def _load_lora_adapter(self, lora_request: LoRARequest):
        ...
        model_config = self._runtime_model_config if self._runtime_model_config is not None else self._lora_model_config
        if model_config is None:
            raise RuntimeError(
                "Model config is not available for LoRA adapter loading")

        self._lora_manager.load_from_ckpt([lora_request.path],
    ...
    def _load_prompt_adapter(self,
        ...
        if self._prompt_adapter_manager is None:
            raise RuntimeError("Prompt adapter manager is not initialized")
        if self._runtime_model_config is None:
            raise RuntimeError(
                "Runtime model config is not available for prompt adapter loading"
            )
    ...
    def _enqueue_request(self, request: GenerationRequest) -> int:
        ...
        if request.prompt_adapter_request is not None:
            ...
            if self._prompt_adapter_manager is None:
                raise RuntimeError("Prompt adapter manager is not initialized")
            if self._runtime_model_config is None:
                raise RuntimeError("Runtime model config is not available")
        ...
        is_overlap_enabled = (self._is_pytorch_backend
                              and self._executor_config is not None
                              and hasattr(self._executor_config,
                                          'pytorch_backend_config')
                              and not self._executor_config.
                              pytorch_backend_config.disable_overlap_scheduler)
        if is_overlap_enabled:
            is_disaggregated = self.engine and hasattr(
                self.engine, 'kv_cache_transceiver'
        ...
        if self._executor_config is None:
            raise RuntimeError("Executor config is not available")
    ...
    def _basic_shutdown(self):
        """Basic shutdown logic without thread management"""
    ...
    def submit(self, request: GenerationRequest) -> GenerationResult:
        ...
        This is the base implementation - subclasses should override for specific behavior. """
        ...
        result = GenerationResult(
            ...
            executor=
            None,  # To be set by subclasses that implement GenerationExecutor
    ...
    def _get_next_client_id(self):
        ...
        raise NotImplementedError(
            "Subclasses must implement _get_next_client_id")

    def _get_logprob_params(self, request):
        """To be implemented by subclasses"""
        raise NotImplementedError(
            "Subclasses must implement _get_logprob_params")

    def _handle_background_error(self):
        """To be implemented by subclasses"""


class GenerationExecutorWorker(WorkerBase, GenerationExecutor):
    ...
    def __init__(
        ...
        # Handle multi-GPU engine selection
        if isinstance(engine, list):
            engine = engine[self.rank]

        # Initialize base class
        WorkerBase.__init__(self, engine, executor_config,
                            batched_logits_processor, postproc_worker_config,
                            is_llm_executor, lora_config,
                            garbage_collection_gen0_threshold)
    ...
    def _get_logprob_params(self, request):
        return GenerationExecutor._get_logprob_params(self, request)

    def _create_engine(self, engine, executor_config, lora_config,
                       garbage_collection_gen0_threshold):
        """Override to handle MPI-specific device and parallel configuration"""
        ...
        # Call parent implementation with MPI setup
        return super()._create_engine(engine, executor_config, lora_config,
                                      garbage_collection_gen0_threshold)

    def _initialize_adapters(self, engine, executor_config, lora_config):
        """Override to handle MPI-specific adapter initialization"""
    ...
    def _iteration_result_task(self, it_result_queue: IterationResultQueue,
        ...
        if queue is None:
            return True  # Early return if no queue available
        ...
        try:
            for results in engine_get_result_api():
                ...
                if self._is_llm_executor and result_singleton:
                    ...
                    while hasattr(queue, 'full') and queue.full():
                        if hasattr(queue, 'get'):
                            queue.get()
                        else:
                            break  # If queue doesn't have get method, break the loop
            ...
            if async_queues and hasattr(queue, 'loop'):
    ...
    def submit(self, request: GenerationRequest) -> GenerationResult:
        ...
        # Override parent submit to pass self as executor (since this class implements GenerationExecutor)
        client_id = request.id if request.id is not None else self._get_next_client_id(
        )
        if request.id is None:
            request.set_id(client_id)
        ...
        result = GenerationResult(
            ...
            background_error_handler=self._handle_background_error,
            executor=
            self,  # Pass self since GenerationExecutorWorker implements GenerationExecutor
            disaggregated_params=request.disaggregated_params,
            logprob_params=logprob_params)
