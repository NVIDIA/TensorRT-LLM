...
from .utils import BATCH_RESP_IN_AWAIT, RequestError
...
class ExecutorBindingsProxy(GenerationExecutor):
    ...
    def workers_main(
        ...
        Args:
            ...
            postproc_worker_config (Optional[PostprocWorkerConfig]): The config for postprocessing workers. Defaults to None.
        '''
        pid = os.getpid()
        cpus = os.sched_getaffinity(pid)
        if cpus:
            logger.warning(
                f"Found worker process {pid} was bound to {cpus}, this may harm performance.",
            )
