...
from .utils import (ErrorResponse, RequestError, WorkerCommIpcAddrs,
                    create_mpi_comm_session, get_multi_frontend_ipc_info,
...
class GenerationExecutorProxy(GenerationExecutor):
    ...
    def __init__(
        ...
        *,
        ...
        self._num_frontends = int(
            os.getenv("TLLM_SERVE_NUM_FRONTENDS", "1")
            or "1") if self._multi_frontend_info is not None else 1
...
class GenerationExecutorFrontendProxy(GenerationExecutorProxy):
    ...
    rpc_common.namespace_client_id). It never owns the engine: no MPI
