...
- ``"safetensors"``: writes a single file with named tensors
  (``image``/``video``/``audio``/``action``). Scalar metadata
  (``frame_rate``, ``audio_sample_rate``) is stored two ways: as a 0-d
  tensor under the same key (so ``safetensors.torch.load(bytes)`` returns
  it alongside the media tensors — consumers call ``.item()`` to unbox)
  and as a stringified value in the file header (preserved for callers
  using ``safe_open(...).metadata()``). No pickle on load.
