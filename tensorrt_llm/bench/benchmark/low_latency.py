...
class LatencyBenchmark:
    ...
    def start_benchmark(self) -> None:
        ...
        while len(self.requests) > 0:
            ...
            request_start_time = monotonic_ns()
            ...
            while not final:
                ...
                for resp in responses:
                    if time_on_first_token is None:
                        time_on_first_token = now

                    final = resp.result.is_final
                    tokens += resp.result.output_token_ids[0]
                    decoding_iters = resp.result.decoding_iter

            self.statistics.register_request_perf_item(
                NewRequestPerfItemTuple(
                    start_timestamp=request_start_time,
                    end_timestamp=now,
                    request_id=req_id,
                    num_input_tokens=len(request.input_token_ids),
                    response_is_final=final,
                    error=False,
                    tokens=tokens,
                    decoding_iteration=decoding_iters,
