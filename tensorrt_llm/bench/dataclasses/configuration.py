...
class RuntimeConfig(BaseModel):
    ...
    def get_llm_args(self) -> Dict:
        ...
        if self.backend == "pytorch":
            if llm_args.cuda_graph_config:
                # Use runtime max_batch_size as cuda_graph_config.max_batch_size
                # if both max_batch_size and batch_sizes are not set.
                batch_sizes_set = llm_args.cuda_graph_config.batch_sizes is not None
                max_batch_size_set = llm_args.cuda_graph_config.max_batch_size is not None
                if not batch_sizes_set and not max_batch_size_set:
                    llm_args.cuda_graph_config.max_batch_size = self.settings_config.max_batch_size
