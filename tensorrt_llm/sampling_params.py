...
class LogitsProcessor(ABC):
    ...
    def __call__(self, req_id: int, logits: torch.Tensor,
        ...
        """Logits processing callback. The TensorRT backend modifies logits in-place; PyTorch backend must return the modified logits for a single batch item.
...
class SamplingParams:
    ...
    Args:
        ...
        logits_processor (tensorrt_llm.sampling_params.LogitsProcessor or List[LogitsProcessor], optional): The logits postprocessor callback(s).
            If a list, each processor is applied in order during generation (supported in PyTorch backend only). Defaults to None.
