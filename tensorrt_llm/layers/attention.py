...
class RopeEmbeddingUtils:
    ...
    def rotate_every_two(tensor: Tensor) -> Tensor:
        ...
        x1 = expand_dims(x1, 4)
        x2 = expand_dims(x2, 4)
        minusOne = constant(-1 * np.ones([1], dtype=np.float32))
        x2 = x2 * minusOne
        x = concat([x2, x1], 4)
        return view(
            x, concat([shape(x, 0),
