...
from ..functional import conv1d, conv_transpose1d, conv2d, conv_transpose2d, conv3d, conv_transpose3d
...
class ConvTranspose1d(Module):
    ...
    def __init__(
        ...
        self.weight = Parameter(shape=(in_channels, out_channels // groups, kernel_size),
                                dtype=dtype)
        if bias:
            self.bias = Parameter(shape=(out_channels,), dtype=dtype)
        else:
    ...
    def _output_padding(self,
        ...
        if output_size is None:
            ...
            for d in range(num_spatial_dims):
                dim_size = (
                    (input.size(d + num_non_spatial_dims) - 1) * stride -
                    2 * padding +
                    (dilation if dilation is not None else 1) *
                    (kernel_size - 1) + 1)
                min_sizes.append(dim_size)
                max_sizes.append(min_sizes[d] + stride - 1)
    ...
    def forward(self, input, output_size=None):
        ...
        return conv_transpose1d(input, self.weight.value,
                                None if self.bias is None else self.bias.value,
...
class ConvTranspose3d(Module):
    ...
    def forward(self, input, output_size=None):
        ...
        return conv_transpose3d(input, self.weight.value,
                                None if self.bias is None else self.bias.value,
                                self.stride, self.padding, output_padding,
                                self.dilation, self.groups)
