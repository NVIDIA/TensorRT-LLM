...
class KVCacheManager:
    ...
    # Topology only -- a copy in flight does not change the answer. life_cycles
    # narrows, never widens: the match arrives pruned by every one there is.
    # ValueError on a key that is not 32 bytes, or a life cycle id out of range.
    def servable_chain(
        self, keys: Sequence[bytes], life_cycles: Sequence[LifeCycleId]
    ) -> tuple[CacheLevel, int] | None: ...
    # Holds against dropping, NOT against migration: a page evicted downward
    # hands its slot back, so an address from this holder can come to name other
    # data. ValueError on an empty chain, a bad key, or a block that is not whole.
    def create_kv_cache_from_keys(
        self, keys: Sequence[bytes], id: int | None = None
    ) -> _KVCache: ...
