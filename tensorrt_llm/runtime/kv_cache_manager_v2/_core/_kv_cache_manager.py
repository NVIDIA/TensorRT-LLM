...
class KVCacheManager:
    ...
    def create_kv_cache(
        ...
        """
        lora_task_id: match lora_task_id before matching any tokens. A ReuseScope is accepted
            for compatibility and is split into lora_task_id/cache_salt_id.
        ...
        cache_salt_id: when set, isolates this request's reuse from requests carrying a different
        salt (or no salt) by seeding a distinct radix-tree branch.
        """
        if isinstance(lora_task_id, ReuseScope):
            if cache_salt_id is not None and lora_task_id.salt != cache_salt_id:
                raise ValueError("cache_salt_id conflicts with ReuseScope.salt")
            cache_salt_id = lora_task_id.salt
            lora_task_id = lora_task_id.lora_id
        return _KVCache(
            self,
            lora_task_id,
            input_tokens,
    ...
    def _match_reuse(
        self, lora_task_id: int | ReuseScope | None, input_tokens: Sequence[TokenIdExt]
    ) -> ReuseMatch:
        cache_salt_id = None
        if isinstance(lora_task_id, ReuseScope):
            cache_salt_id = lora_task_id.salt
            lora_task_id = lora_task_id.lora_id
        tree_task_id = _KVCache._make_tree_task_id(lora_task_id, cache_salt_id)
        return self._radix_tree.match(tree_task_id, input_tokens, self.enable_partial_match)
    ...
    def probe_reuse(
        ...
        if input_tokens is None:
            input_tokens = ()
        return self._match_reuse(lora_task_id, input_tokens).num_tokens
