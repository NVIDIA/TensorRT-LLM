...
class KVCacheManager:
    ...
    def create_kv_cache(
        ...
        locality_domain_id: the locality domain to pin this KV cache to. None means non-localized.
        """
        if reuse_scope is None:
            reuse_scope = ReuseScope()
        assert type(reuse_scope) is ReuseScope
        if locality_domain_id is not None:
            reuse_scope = reuse_scope._replace(locality_domain_id=locality_domain_id)
        reuse_match = (
    ...
    def num_locality_domains(self) -> int:
        """Number of locality domains. Returns 1 for non-localized (standard) configurations."""
        gpu_storage = self._storage._levels[GPU_LEVEL].storage
        if isinstance(gpu_storage, GpuCacheLevelStorage):
            return gpu_storage.num_locality_domains
        return 1

    def get_per_locality_domain_free_slots(self) -> list[int]:
        """Free slot count per locality domain, summed across all pool groups.

        Returns a list of length num_locality_domains. For non-localized
        configurations (num_locality_domains == 1), returns total free slots
        across all pool groups as a single-element list.
        """
        n = self.num_locality_domains
        storage = self._storage
        num_pg = storage.num_pool_groups
        gpu_storage = storage._levels[GPU_LEVEL].storage
        result = [0] * n
        for uid in range(n):
            for pg in typed_range(num_pg):
                if n > 1:
                    result[uid] += gpu_storage.get_num_free_slots(pg, uid)
                else:
                    result[uid] += gpu_storage.get_num_free_slots(pg)
        return result
    ...
    def clamp_max_seq_len_for_mem(self, batch_size: int, token_num_upper_bound: int) -> int:
        ...
        remaining_slots = cast(
            ...
            [
                storage.num_slots(pg, locality_domain_id=0)
                if use_primary_locality_domain
                else storage.num_slots(pg)
