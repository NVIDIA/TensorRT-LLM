...
class _KVCache:
    ...
    def resize(self, capacity: int | None, history_length: int | None = None) -> bool:
        ...
        if new_num_blocks >= old_num_blocks:
            ...
            page_slots_to_publish = tuple(chain.from_iterable(slots))
            page_slot_ready_events = tuple(slot.ready_event for slot in page_slots_to_publish)
            for beam_indices in self._base_page_indices:
                for indices in beam_indices:
                    if type(indices) is array.array:
            ...
            if any(
                ...
                pending_ready_events = [
                    event for event in set(page_slot_ready_events) if not event.query_complete()
                ]
                for event in pending_ready_events:
                    event.synchronize()
                for slot in page_slots_to_publish:
                    slot.ready_event = CachedCudaEvent.NULL
            stream_wait_events(self.cuda_stream, page_slot_ready_events)
