...
def sampling_params_from_request(request: generation_pb2.GenerateRequest) -> SamplingParams:
    ...
    if num_sequences > 1:
        # The executor supports greedy multi-return, but SamplingParams intentionally
        # gates direct LLM API usage. Preserve the OpenEngine request without changing
        # its sampling semantics.
        sampling_params._allow_greedy_multiple_returns = True
...
def _prefill_ready_response(
    ...
    disagg_request_id, schedule_style, conversation_id, ctx_usage) into the
