...
def prepare_data(rank, world_size, chunk_dim, x_shape, cache_x_shape=None):
    ...
    if cache_x_shape is not None:
        cache_x = torch.randn(cache_x_shape, dtype=torch.float32, device=local_device, requires_grad=False)
...
def prepare_model_conv3d(rank):
    ...
    wan_conv_3d = WanCausalConv3d(96, 96, kernel_size=3, stride=1, padding=1).to(local_device).to(torch.float32)
...
def prepare_model_conv2d(rank):
    local_device = "cuda:" + str(rank)
    conv_2d = torch.nn.Conv2d(96, 96, kernel_size=3, stride=1, padding=1).to(local_device).to(torch.float32)
...
def prepare_model_conv2d_stride2(rank):
    local_device = "cuda:" + str(rank)
    conv_module = torch.nn.Conv2d(96, 96, kernel_size=3, stride=2).to(local_device).to(torch.float32)
