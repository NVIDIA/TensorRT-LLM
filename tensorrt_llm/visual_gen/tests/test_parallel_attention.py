...
def sample_joint_tensors(batch_size, num_heads, seq_len, head_dim, world_size):
    ...
    joint_q = torch.narrow(
        origin_q, dim=2, start=seq_len - joint_seq_length, length=joint_seq_length
    )
    joint_k = torch.narrow(
        origin_k, dim=2, start=seq_len - joint_seq_length, length=joint_seq_length
    )
    joint_v = torch.narrow(
        origin_v, dim=2, start=seq_len - joint_seq_length, length=joint_seq_length
    )
...
def test_attn_parallel(
    ...
    cos_similarity = cos_sim(
        local_output.reshape(-1).to(torch.float32), local_ref_output.reshape(-1).to(torch.float32)
    )
...
def test_joint_attn_parallel(
    batch_size, num_heads, seq_len, head_dim, world_size, ulysses_size, ring_size, attn_type
):
    """Test basic parallel attention functionality."""
    PipelineConfig.reset()
    (
    ...
    if dist.get_rank() == 0:
        logger.info(
            f"visual_gen attn u{ulysses_size} r{ring_size} time: {start_event.elapsed_time(end_event) / num_iter} ms"
        )
    ...
    for batch_idx in range(batch_size):
        ...
        ref_image_output = ref_output[
            :, :, : (valid_seq_length - valid_joint_seq_length[batch_idx]), :
        ].chunk(world_size, dim=2)[dist.get_rank()]
        ref_joint_output = ref_output[
            :, :, (valid_seq_length - valid_joint_seq_length[batch_idx]) :, :
        ]
...
def test_uneven_attn_parallel(
    ...
    PipelineConfig.set_uneven_cp_config(
        seq_len_padded - uneven_number, seq_len_padded, seq_len_cur_rank, dit_config
    )
    attn = ditAttnProcessor()
    AttentionOpManager.set_attn_config(attn_type=attn_type)
