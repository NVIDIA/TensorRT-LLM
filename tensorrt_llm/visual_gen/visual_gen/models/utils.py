...
class WeightManagedBlocks(torch.nn.ModuleList):
    ...
    def _check_block_weight(self):
        ...
        if not check_transformer_blocks(self):
            raise ValueError(
                "The weights in the ModuleList are not all the same and thus cannot be managed."
            )
    ...
    def _set_next_offloading_layer_weight(self):
        ...
        for i in range(len(self.offloading_block_index_list)):
            ...
            for name in self.managed_weight_layers:
                visual_gen_linear_current_block = current_block.get_submodule(
                    name.replace("-", ".")
                )
                ...
                visual_gen_linear_current_block.next_offloading_layer_weight = (
                    visual_gen_linear_next_block.host_weight
                )
                logger.debug(
    ...
    def setup_offloading(self, offloading_stride: int):
        ...
        offloading_list = [
            True if i % self.offloading_stride == 0 else False for i in range(len(self))
        ]
        ...
        if len(self.offloading_block_index_list) <= 1:
            logger.debug(
                f"Too less offloading blocks, no need to setup offloading: {self.offloading_block_index_list}"
            )
            self.offloading_block_index_list = []
            return
        ...
        def make_wrapped_forward(orig_forward, block, idx, self_ref):
            def wrapped_forward(*args, **kwargs):
                ...
                if is_offloading_block:
                    ...
                    assert self_ref.current_block_idx == idx, (
                ...
                if is_offloading_block:
                    ...
                    self_ref.pingpong_in, self_ref.pingpong_out = (
                        self_ref.pingpong_out,
                        self_ref.pingpong_in,
                    )
                return out
...
class QKVLinearMerger:
    ...
    def find_qkv_linear_layers(
        ...
        def _recursive_search(current_module: nn.Module, current_path: str = ""):
            ...
            q_layer = None
            for name in q_names:
                if hasattr(current_module, name) and isinstance(
                    getattr(current_module, name), nn.Linear
                ):
    ...
    def merge_qkv_layers(
        ...
        assert q_layer.in_features == k_layer.in_features == v_layer.in_features, (
        ...
        merged_layer.weight.data = torch.cat(
            [q_layer.weight.data, k_layer.weight.data, v_layer.weight.data],
            dim=0,  # Concatenate along output dimension
        )
        if q_layer.bias is not None:
            merged_layer.bias.data = torch.cat(
                [q_layer.bias.data, k_layer.bias.data, v_layer.bias.data], dim=0
            )
    ...
    def forward_with_merged_qkv(
        ...
        # Split the result
        q, k, v = QKVLinearMerger.split_qkv_output(
            merged_output, *module.qkv_dims
        )  # Unpack (q_dim, k_dim, v_dim)
        return q, k, v
            ...
            try:
                ...
                for i in range(gelu_index_in_children - 1, -1, -1):
                    if self.is_gemm(child_modules[i]):
                        prev_path = (
                            f"{gelu_path.rsplit('.', 1)[0]}.{child_names[i]}"
                            if "." in gelu_path
                            else child_names[i]
                        )
            ...
            for i, (child_name, child_module) in enumerate(zip(child_names, children)):
                ...
                if self.is_gelu_module(child_module):
                    ...
                    if hasattr(child_module, "proj") and self.is_gemm(child_module.proj):
                        ...
                        self.updated_gemms.append(
                            {"gemm_path": f"{current_path}.proj", "gelu_path": current_path}
                        )
...
def apply_async_cpu_offloading(
    model: nn.Module,
    transformer_blocks_name: str = "transformer_blocks",
    offloading_stride: int = 1,
):
