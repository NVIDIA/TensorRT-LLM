...
class PipelineConfig:
    ...
    seq_len_cur_ring_group = (
        None  # sequence length current ring group, [seq_ring_0, seq_ring_1, ...]
    )
    ulysses_seq_all_ring_ranks = (
        None  # ulysses sequence length all ranks in ring group, [seq_ring_0, seq_ring_1, ...]
    )
    ...
    offloading_stride = (
        0  # offloading stride for block-wise offloading, if stride is 0, not enabled
    )
    enable_torch_compile = True  # enable torch compile
    torch_compile_models = []  # models to compile with torch compile, such as ["transformer", "image_encoder", "text_encoder"]
    torch_compile_mode = "default"  # torch compile mode, such as "default", "max-autotune"
    fuse_qkv = True
    do_classifier_free_guidance = False
    in_refiner_stage = (
        False  # whether in the refiner stage, some models have refiner, such as HunyuanImage
    )
    int8_ulysses = False  # whether to use 8-bit quantization for Ulysses all-to-all communication
    fuse_qkv_in_ulysses = False  # whether to fuse q, k, v communication into single operation for ulysses parallelization
    ...
    def set_uneven_cp_config(cls, seq_len, seq_len_padded, seq_len_cur_rank, ditParallelConfig):
        ...
        gather_list = [
            torch.empty_like(seq_len_cur_rank) for _ in range(torch.distributed.get_world_size())
        ]
        ...
        if ditParallelConfig.ulysses_size() == 1:
            cls.seq_len_cur_ring_group = cls.seq_len_all_ranks[
                torch.tensor(ditParallelConfig.ring_ranks())
            ]
            return
        if ditParallelConfig.ring_size() == 1:
            seq_len_cur_ulysses_group = cls.seq_len_all_ranks[
                torch.tensor(ditParallelConfig.ulysses_ranks())
            ]
            cls.ulysses_seq_all_ring_ranks = [
                torch.sum(seq_len_cur_ulysses_group, dtype=torch.int32)
            ]
            return
        ...
        gather_list = [
            ...
            for _ in range(ditParallelConfig.ring_size())
        ]
        torch.distributed.all_gather(
            gather_list, ulysses_seq_cur_ring_rank, group=ditParallelConfig.ring_group()
        )
