...
def init_dist(device_type: str = "cuda"):
    ...
    if not dist.is_initialized():
        ...
        if WORLD_SIZE > 1:
            ...
            logger.debug(
                f"Distributed process group initialized: rank {dist.get_rank()}/{dist.get_world_size()}"
            )
        else:
...
class BaseParallelConfig:
    ...
    def set_device_mesh(self, device_type: str = "cuda") -> torch.distributed.DeviceMesh:
        ...
        if total_parallel_size == 1:
            logger.debug(
                f"[{self.__class__.__name__}] No parallelism needed, skipping device mesh setup"
            )
            self._device_mesh = None
            return self._device_mesh
        ...
        if self._ulysses_size > 1:
            ...
            logger.debug(
                f"[{self.__class__.__name__}] Added Ulysses dimension: {self._ulysses_size}"
            )
        if self._tp_size > 1:
            mesh_dims.append("tp")
        ...
        if not mesh_dims:
            ...
            logger.info(
                f"[{self.__class__.__name__}] Creating device mesh: dims={mesh_dims}, sizes={mesh_sizes}"
            )
            self._device_mesh = init_device_mesh(
                device_type, tuple(mesh_sizes), mesh_dim_names=tuple(mesh_dims)
            )
    ...
    def set_fsdp_device_mesh(self, device_type: str = "cuda") -> torch.distributed.DeviceMesh:
        ...
        if self._fsdp_size <= 1:
            logger.debug(
                f"[{self.__class__.__name__}] FSDP size is 1, skipping FSDP device mesh setup"
            )
            self._fsdp_device_mesh = None
            return self._fsdp_device_mesh
        world_size = self.get_world_size()
        if world_size % self._fsdp_size != 0:
            raise ValueError(
                f"World size ({world_size}) is not divisible by FSDP size ({self._fsdp_size})"
            )
    ...
    def set_config(
        ...
        if cp_size > 1:
            assert ring_size == 1 and ulysses_size == 1, (
    ...
    def check_parallel_size(self) -> bool:
        ...
        if not is_valid:
            ...
            raise ValueError(
                f"Parallel size {par_size} does not match world size ({self.get_world_size()})"
            )
        return is_valid
    ...
    def sp_size(cls) -> int:
        ...
        """
        return (
            cls.get_instance()._ulysses_size
            * cls.get_instance()._ring_size
            * cls.get_instance()._cp_size
        )
