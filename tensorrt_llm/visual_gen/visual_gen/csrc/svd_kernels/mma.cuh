...
struct f32
{
    static constexpr char const value[] = "f32";
};

struct f16
{
    static constexpr char const value[] = "f16";
};

struct bf16
{
    static constexpr char const value[] = "bf16";
};

struct s32
{
    static constexpr char const value[] = "s32";
};

struct s4
{
    static constexpr char const value[] = "s4";
};

struct u4
{
    static constexpr char const value[] = "u4";
};
...
__device__ __forceinline__ static uint4 mma_m16n8kx_s32common(uint4 a, uint2 b, uint4 c)
{
    ...
    asm volatile(
        ...
        : "=r"(d.x), "=r"(d.y), "=r"(d.z), "=r"(d.w)
        : "r"(a.x), "r"(a.y), "r"(a.z), "r"(a.w), "r"(b.x), "r"(b.y), "r"(c.x), "r"(c.y), "r"(c.z), "r"(c.w),
        "n"(K / 2), "C"(AType::value), "C"(BType::value));
#endif
    return d;
}
