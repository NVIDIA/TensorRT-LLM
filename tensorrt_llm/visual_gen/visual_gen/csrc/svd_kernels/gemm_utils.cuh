...
__device__ __forceinline__ static T load(T const* addr)
{
    if constexpr (shmem)
    {
    ...
    if constexpr (sizeof(T) == 8)
    {
        uint2 data = __ldg(reinterpret_cast<uint2 const*>(addr));
        return *reinterpret_cast<T*>(&data);
    }
    if constexpr (sizeof(T) == 16)
    {
        uint4 data = __ldg(reinterpret_cast<uint4 const*>(addr));
        return *reinterpret_cast<T*>(&data);
    }
...
template <typename T>
__device__ __forceinline__ static T load_pred(T const* addr, bool pred)
{
...
__device__ __forceinline__ static void store(T* addr, T val)
{
    if constexpr (shmem)
    {
        ...
        if constexpr (sizeof(T) == 16)
        {
            ...
            asm volatile("st.shared.v4.b32 [%0], {%1, %2, %3, %4};" ::"l"(__cvta_generic_to_shared(addr)), "r"(data.x),
                "r"(data.y), "r"(data.z), "r"(data.w));
            return;
        }
        *addr = val;
        return;
...
__device__ __forceinline__ static void ldmatrix(void const* ptr, uint4& out)
{
...
#else  // USE_TANH
    float const L2E = 1.442695041f; // log2(exp(1))
    float t, d, e, r;
    t = -L2E * a;
...
template <typename T>
__device__ __forceinline__ static T silu(T const& x)
{
    // x * sigmoid(x)
...
__device__ __forceinline__ static To bit_cast(From const& input)
{
    static_assert(sizeof(To) == sizeof(From));
    // not safe but anyway
    return *reinterpret_cast<To const*>(&input);
}
