    ...
    int attn_tokens)
{
    ...
    auto launch = [&]<typename Epilogue>(Epilogue::Arguments args)
    {
        ...
        {
            dispatchBool(act_unsigned,
                [&]<bool ACT_UNSIGNED>()
                {
                    auto func = invoke_kernel<typename GEMM::gemm_w4a4_kernel<Epilogue, ACT_UNSIGNED>,
                        const packed_act_t*, const packed_wgt_t*, const packed_ascale_t*, const packed_wscale_t*, int,
                    ...
                    func<<<grid, GEMM::WARP_SIZE * GEMM::NUM_WARPS, shmem, getCurrentCUDAStream()>>>(
                        act.data_ptr<packed_act_t>(), wgt.data_ptr<packed_wgt_t>(), ascales.data_ptr<packed_ascale_t>(),
                        wscales.data_ptr<packed_wscale_t>(), M, N, K, args, swapBlockMN, false);
        ...
        if constexpr (USE_FP4)
        {
            dispatchBool(alpha != 1.0f,
                [&]<bool USE_ALPHA>()
                {
                    assert(!act_unsigned);

                    auto func = invoke_kernel<typename GEMM::gemm_w4a4_fp4_kernel<Epilogue, USE_ALPHA>,
                        const packed_act_t*, const packed_wgt_t*, const packed_amscale_t*, const packed_wmscale_t*,
                        float, int, int, int, typename Epilogue::Arguments, bool, bool>;
                    ...
                    func<<<grid, GEMM::WARP_SIZE * GEMM::NUM_WARPS, shmem, getCurrentCUDAStream()>>>(
                        ...
                        ascales.data_ptr<packed_amscale_t>(), wscales.data_ptr<packed_wmscale_t>(), alpha, M, N, K,
                        args, swapBlockMN, false);
                    checkCUDA(cudaGetLastError());
                });
    ...
    auto launch_bias = [&]<typename NextEpilogue>(NextEpilogue::Arguments nextArgs)
    {
        ...
        dispatchBool(bias.valid(),
            [&]<bool USE_BIAS>()
            {
                dispatchBool(wcscales.valid(),
                    [&]<bool USE_SCALE>()
                    {
                        using EpilogueBias = typename GEMM::EpilogueBias<USE_BIAS, USE_SCALE>;
                        // append EpilgoueNop to workaround mismatched memory layout of std::tuple between device and
                           ...
                           NextEpilogue::Arguments nextArgs, MidEpilogue::Arguments midArgs)
    {
        ...
        if (rank_down == 0)
        {
            using Epilogue = typename GEMM::EpilogueCombination<typename LoraUp::EpilogueLoraUp, MidEpilogue,
        ...
        using Epilogue = typename GEMM::EpilogueCombination<typename LoraUp::EpilogueLoraUp, MidEpilogue,
            typename LoraDown::EpilogueLoraDown, NextEpilogue, typename GEMM::EpilogueNop>;
        return launch_bias.template operator()<Epilogue>({typename LoraUp::EpilogueLoraUp::Arguments{
                                                              .lora_act = lora_act_in.data_ptr<float>(),
                                                              .lora_wgt_up = lora_up.data_ptr<packed_fpsum_t>(),
                                                              .rank = rank_up,
...
template <typename Config, bool USE_FP4>
void GEMM_W4A4_Launch<Config, USE_FP4>::quantize_w4a4_act_fuse_lora(Tensor input, Tensor output, Tensor oscales,
    Tensor lora_down, Tensor lora_act_out, Tensor smooth, bool fuse_glu, bool fp4)
{
    int const actualM = input.numel() / input.shape[-1];
    int const actualN = input.shape[-1];

    int const M = ceilDiv(actualM, GEMM::BLOCK_M) * GEMM::BLOCK_M;
    int const N = ceilDiv(actualN / (fuse_glu ? 2 : 1), GEMM::BLOCK_N) * GEMM::BLOCK_N;
    ...
    int const rank = lora_down.shape[1];
