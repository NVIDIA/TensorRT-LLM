...
{
public:
    BufferTorchTensor(at::Tensor tensor)
...
public:
    TorchOpContext();
    TorchOpContext(TorchOpContext const&) = delete;
...
{
public:
    TensorsProviderTorch(std::map<std::string, at::Tensor> dict)
        : storage(std::move(dict))
    {
    }

    virtual bool contains(std::string const& key) const override
    {
        return storage.contains(key);
    }

    virtual Tensor getTensor(std::string const& key) override
    {
