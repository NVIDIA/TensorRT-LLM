...
#pragma unroll
    ...
    struct EpilogueQKVProj
    {
        struct Arguments
        {
            ...
            float const* rotary_emb;        // [M, HEAD_DIM / 2, ROTARY_EMB_NUM_ELEMENTS]
            half_t const* rmsnorm_weight_q; // [HEAD_DIM]
            half_t const* rmsnorm_weight_k; // [HEAD_DIM]
            float epsilon;
        };
        ...
        __device__ __forceinline__ static void apply(fpsum_warp fpsum, half_t* out, int M, int N, int K,
            half_t* pool_out, float const* rotary_emb, half_t const* rmsnorm_weight, float epsilon, int maxRows)
        {
            ...
            {
                rms = load(reinterpret_cast<pack_t const*>(&rmsnorm_weight[laneId * PACK_SIZE]));
            }
            ...
            float const* rotary_emb_base_addr = &rotary_emb[(warpId * WARP_M) * HEAD_DIM / 2 * ROTARY_EMB_NUM_ELEMENTS
                + laneId * PACK_SIZE / 2 * ROTARY_EMB_NUM_ELEMENTS];

            CHECK_NAN(fpsum, "fpsum");

            unpack_fpsum()(fpsum, out + warpId * WARP_M * N, N, maxRows - warpId * WARP_M, INT_MAX, shmem[warpId],
...
#pragma unroll
            ...
            {
                int const row = warpId * NUM_WARPS_PER_POOL;
            ...
            const BlockInfo binfo, fpsum_warp fpsum, int M, int N, int K, Arguments const& args)
        {
            ...
            bool const is_q = bn < binfo.numBlocksN / 3;
            bool const is_k = !is_q && bn < binfo.numBlocksN / 3 * 2;
            ...
            if (is_q || is_k)
            {
                apply(fpsum, args.out + bm * BLOCK_M * args.actualN + bn * BLOCK_N, M, N, K,
                    ...
                    is_q ? args.rmsnorm_weight_q : args.rmsnorm_weight_k, args.epsilon, args.actualM - bm * BLOCK_M);
            }
            else
            {
    ...
    struct EpilogueRMSNormRope
    {
        ...
        struct Arguments
        {
            ...
            packed_rotemb_t const* rotary_emb;
            half_t const* rmsnorm_weight_q; // [HEAD_DIM]
            half_t const* rmsnorm_weight_k; // [HEAD_DIM]
            float epsilon;
        };

        __device__ __forceinline__ static rotemb_warp load_rotemb(packed_rotemb_t const* ptr_rotemb)
        {
            int const laneId = threadIdx.x % WARP_SIZE;
            int const warpId = threadIdx.x / WARP_SIZE;

            rotemb_warp rotemb;
            packed_rotemb_t const* ptrlane
                = &ptr_rotemb[warpId * WARP_M_TILES * WARP_N_ROTEMB_TILES * WARP_SIZE + laneId];

            unrolled_loop<WARP_M_TILES>(
                [&]<int i>()
                {
                    unrolled_loop<WARP_N_ROTEMB_TILES>(
                        [&]<int j>()
                        {
        ...
        __device__ __forceinline__ static void load_rmsnorm(half_t const* ptr_rmsnorm_weight, half_t* shmem)
        {
            int const laneId = threadIdx.x % WARP_SIZE;
            ...
            packed_t pack = load(reinterpret_cast<packed_t const*>(ptr_rmsnorm_weight + laneId * PACK_SIZE));
        ...
        {
            int const laneId = threadIdx.x % WARP_SIZE;
            int const col = n * INSN_N + laneId / 16 * 8; // lane 0-15: n*16+0, lane 16-31: n*16+8
            uint4 tmp;
        ...
        __device__ __forceinline__ static void apply(
            fpsum_warp& fpsum, packed_rotemb_t const* ptr_rotemb, half_t const* ptr_rmsnorm_weight, float epsilon)
        {
            int const laneId = threadIdx.x % WARP_SIZE;
            int const warpId = threadIdx.x / WARP_SIZE;
...
#pragma unroll
            ...
            {
                int const n_offset = head * WARP_N_TILES_PER_HEAD;
...
#pragma unroll
            ...
            const BlockInfo binfo, fpsum_warp& fpsum, int M, int N, int K, Arguments const& args)
        {
            ...
            if (is_q || is_k)
            {
                apply(fpsum, args.rotary_emb + bm * NUM_WARPS * WARP_M_TILES * WARP_N_ROTEMB_TILES * WARP_SIZE,
    ...
    struct EpiloguePackQKV
    {
        ...
        template <typename F>
        __device__ __forceinline__ static void apply(
            fpsum_warp& fpsum, packed_qkv_t* ptr_output, int maxRows, F&& funcPack, attn_half2_t maskVal)
        {
            int const laneId = threadIdx.x % WARP_SIZE;
            int const warpId = threadIdx.x / WARP_SIZE;
            ...
            const BlockInfo binfo, fpsum_warp fpsum, int M, int N, int K, Arguments const& args)
        {
            ...
            assert(binfo.numBlocksN % 3 == 0);
            int const numBlocksQ = binfo.numBlocksN / 3;
            bool const is_q = bn < numBlocksQ;
            bool const is_k = !is_q && bn < numBlocksQ * 2;
    ...
    struct EpilogueLiteLA
    {
        ...
        __device__ __forceinline__ static packed_f32psum_t mma_litela(
            packed_fpsum_t k, packed_fpsum_t v, packed_f32psum_t psum)
        {
            for (int i = 0; i < 4; i++)
            {
        ...
        __device__ __forceinline__ static void apply_litela(
            const BlockInfo binfo, fpsum_warp fpsum, float* out_vk, int num_blocks_per_batch)
        {
            int const laneId = threadIdx.x % WARP_SIZE;
            int const warpId = threadIdx.x / WARP_SIZE;
            ...
            int const num_heads = binfo.numBlocksN / 3 * 2 * (WARP_N / (LITELA_HEAD_DIM * 2));
            int const batch_id = binfo.bm / num_blocks_per_batch;

            for (int head_id = 0; head_id < WARP_N / (LITELA_HEAD_DIM * 2); head_id++)
            {
                int const global_head_id
                ...
                for (int tile_v = 0; tile_v < LITELA_V_TILES; tile_v++)
                {
                    for (int tile_k = 0; tile_k < LITELA_K_TILES; tile_k++)
                    {
                        ...
                        int const row = tile_v * 16 + laneId / 4;
                        int const col = tile_k * 16 + laneId % 4 * 2;
...
#pragma unroll
                    ...
                    int const row = LITELA_HEAD_DIM + laneId / 4;
                    int const col = tile_k * 16 + laneId % 4 * 2;
        ...
        struct vk_mul_q_kernel
        {
            ...
            // FIXME FIXME FIXME
            __device__ void operator()(half_t* q, float const* vk, float eps, int num_tokens)
            {
                int const block_id = blockIdx.x;
                int const head_id = blockIdx.y;
                int const batch_id = blockIdx.z;

                int const num_blocks = gridDim.x;
                int const num_heads = gridDim.y;
                ...
                half_t* localq
                    = &q[(((batch_id * num_blocks + block_id) * block_size + threadIdx.x) * num_heads + head_id)
                        * LITELA_HEAD_DIM];
                float const* localvk = &vk[(batch_id * num_heads + head_id) * (LITELA_HEAD_DIM + 1) * LITELA_HEAD_DIM];
                ...
                for (int i = 0; i < LITELA_HEAD_DIM; i += sizeof(packed_q) / sizeof(half_t))
                {
                    if (pred)
                    {
                        *reinterpret_cast<packed_q*>(&qblock[i]) = load(reinterpret_cast<packed_q const*>(&localq[i]));
                    }
                }
...
#pragma unroll
                    ...
                    {
                        packed_vk vkpack = load(reinterpret_cast<packed_vk const*>(&localvk[j * LITELA_HEAD_DIM + i]));
#pragma unroll
    ...
    struct test_epilogue_kernel
    {
        ...
        __device__ __forceinline__ void operator()(Arguments args)
        {
            ...
            int const n_offset = bn * BLOCK_N;

            extern __shared__ uint8_t shmem[];

            fpsum_warp fpsum;

            Base::template load_act_to_fpsum<false>()(args.input + m_offset * args.actualN + n_offset, args.actualN,
