    ...
    int attn_tokens)
{
    ...
    invoke_launch(dtype, fp4, FasterI2FMode::check(act_unsigned),
        ...
        {
            GEMM_W4A4_Launch<Config, USE_FP4>::gemm_w4a4(act, wgt, out, qout, ascales, wscales, oscales, poolout,
                lora_act_in, lora_up, lora_down, lora_act_out, norm_q, norm_k, rotary_emb, bias, smooth_factor, out_vk,
                out_linearattn, act_unsigned, lora_scales, fuse_silu, fp4, alpha, wcscales, out_q, out_k, out_v,
                attn_tokens);
        });
}
...
{
    invoke_launch(q.dtype(), false, false,
        [&]<typename Config, bool USE_FP4>() { GEMM_W4A4_Launch<Config, false>::linearattn_vk_mul_q(q, vk); });
}
...
{
    invoke_launch(input.dtype(), fp4, false,
        [&]<typename Config, bool USE_FP4>()
        {
...
{
    invoke_launch(input.dtype(), false, false,
        [&]<typename Config, bool USE_FP4>()
