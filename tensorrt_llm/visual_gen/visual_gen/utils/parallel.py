...
def _dit_sp_split(
    parallel_config, x: torch.Tensor | Tuple[torch.Tensor, torch.Tensor], dim: int, allow_uneven: bool = True
) -> torch.Tensor:
    ...
    assert total_splits == parallel_config.sp_size(), "Total splits do not match sequence parallel size"
    ...
    logger.debug(f"Split hidden states: {x.shape}, total_splits={total_splits}, split_dim={dim}, chunk_idx={split_idx}")
    ...
    if seq_len_padded > seq_len:
        ...
        if PipelineConfig.seq_len is None:
            PipelineConfig.set_uneven_cp_config(seq_len, seq_len_padded, seq_len_cur_rank, parallel_config)

    x_splits = torch.chunk(x, total_splits, dim=dim)
    return x_splits[split_idx]


def _all_gather(x: torch.Tensor, group_size: int, group: torch.distributed.ProcessGroup, cat_dim: int) -> torch.Tensor:
...
def _dit_sp_gather(parallel_config, hidden_states: torch.Tensor, dim: int = 1) -> torch.Tensor:
    ...
    if cp_size > 1:
        logger.debug(f"Gather hidden states from cp group: {hidden_states.shape}, cp_size={cp_size}")
        hidden_states = _all_gather(hidden_states, cp_size, parallel_config.cp_group(), dim)

    # Gather from ulysses group
    if ulysses_size > 1:
        logger.debug(f"Gather hidden states from ulysses group: {hidden_states.shape}, ulysses_size={ulysses_size}")
        hidden_states = _all_gather(hidden_states, ulysses_size, parallel_config.ulysses_group(), dim)

    # Then gather from ring group
    if ring_size > 1:
        logger.debug(f"Gather hidden states from ring group: {hidden_states.shape}, ring_size={ring_size}")
...
def _dit_dp_split(parallel_config, x: torch.Tensor, dim: int) -> torch.Tensor:
    ...
    assert x.shape[dim] % dp_size == 0, f"batch size must be divisible by total splits, {x.shape[dim]} % {dp_size} != 0"
...
def enable_tensor_parallel(model: torch.nn.Module) -> torch.nn.Module:
    ...
    if dit_parallel_config.tp_size() > 1:
        model = torch.nn.parallel.DistributedDataParallel(model, device_ids=[dit_parallel_config.tp_rank()])
    return model
