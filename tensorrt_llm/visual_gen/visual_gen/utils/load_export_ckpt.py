...
def save_safetensors(model, export_path: str):
    ...
    index_dict = {
        "metadata": {"total_size": sum(info["size_bytes"] for info in tensor_info.values())}
    }
    weight_map = {}
    ...
    for shard_idx, shard in enumerate(shards):
        if total_shards == 1:
            ...
            filename = (
                f"diffusion_pytorch_model-{shard_idx + 1:05d}-of-{total_shards:05d}.safetensors"
            )
        ...
        logger.info(
            f"Saved shard {shard_idx + 1}/{total_shards}: {filename} ({shard_size_gb:.2f} GB)"
        )
    ...
    logger.info(
        f"Model checkpoint exported to {export_path} ({total_shards} shard{'s' if total_shards > 1 else ''})"
    )
