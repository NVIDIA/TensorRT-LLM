...
class TunableParam:
    ...
    def _validate_value(self, value: Any) -> bool:
        ...
        if isinstance(self.param_range, (list, tuple)):
            if len(self.param_range) == 2 and all(isinstance(x, (int, float)) for x in self.param_range):
    ...
    def from_dict(cls, data: Dict[str, Any]) -> "TunableParam":
        ...
        if isinstance(value, dict) and "type" in value:
            if value["type"] == "torch.Tensor":
                ...
                if len(value["shape"]) > 0:
                    tensor_data = tensor_data.reshape(value["shape"])
                value = torch.nn.Parameter(tensor_data, requires_grad=value.get("requires_grad", True))

        return cls(
            value=value, param_range=data.get("param_range"), name=data.get("name"), description=data.get("description")
        )
...
class OpTuningResult:
    ...
    def to_dict(self) -> Dict[str, Any]:
        ...
        if self.elapsed_time_ms:
            elapsed_time_stats["elapsed_time_ms"] = sum(self.elapsed_time_ms) / len(self.elapsed_time_ms)  # mean
...
class AutoTuner:
    ...
    def __init__(
        ...
        self.tuning_results_per_step_per_layer: Dict[int, Dict[str, Dict[str, Dict[str, OpTuningResult]]]] = {}
    ...
    def benchmark(
        self, op_manager: BaseOpManager, impl_type: str, inputs: Dict[str, torch.Tensor], is_baseline: bool = False
    ) -> OpTuningResult:
        """
        ...
        logger.debug(f"Benchmark: op type {op_type}, impl type {impl_type} took {elapsed_time_ms:.3f} ms")
    ...
    def _get_op_inputs(self, impl_type: str, inputs: Dict[str, torch.Tensor]) -> Dict[str, torch.Tensor]:
        """
    ...
    def tune(
        ...
        if op_manager.op_type() == "linear":
            ...
            if "svd-nvfp4" in tunable_impls and "svd_qweight" not in inputs["special_inputs"]["svd-nvfp4"]:
        ...
        for impl_type in tunable_impls:
            if impl_type not in available_impls:
                logger.error(f"Implementation {impl_type} is not in the available implementations: {available_impls}")
        ...
        assert (
            baseline_impl in tunable_impls
        ...
        if self.save_attention_heatmaps and "query" in inputs and "key" in inputs:
            ...
            if len(query.shape) == 4 and len(key.shape) == 4 and query.shape == key.shape:
                ...
                logger.warning(f"Skipping attention heatmaps for {layer_name}: unexpected tensor shapes")
        ...
        for impl_type in benchmark_impls:
            ...
            current_output = result.op_outputs[0] if isinstance(result.op_outputs, tuple) else result.op_outputs
            for metric in accuracy_metrics:
                if metric == "cosine_similarity":
                    ...
                    logger.debug(f"Cosine similarity between baseline and {impl_type}: {cosine_similarity:.3f}")
                elif metric == "mse":
            ...
            self.record_tuning_result_per_step_per_layer(op_tuning_meta_info, result, step, cfg_type, layer_name)

        outputs = baseline_result.op_outputs.clone()
        del baseline_result.op_outputs
        torch.cuda.empty_cache()
        self.record_tuning_result_per_step_per_layer(op_tuning_meta_info, baseline_result, step, cfg_type, layer_name)
    ...
    def select_best_impl(
        ...
        if cosine_similarity_threshold is not None:
            assert (
                cosine_similarity_threshold >= 0 and cosine_similarity_threshold <= 1
        ...
        best_impl = self._find_best_impl(op_tuning_result, cosine_similarity_threshold, mse_threshold)
    ...
    def _find_best_impl(
        self, op_tuning_result: TuningResult, cosine_similarity_threshold: float, mse_threshold: float
    ) -> str:
        if "best_impl" in op_tuning_result:
        ...
        for impl_type, result in op_tuning_result.items():
            ...
            if cosine_similarity_threshold is not None:
                assert (
                    "cosine_similarity" in result.accuracy_metrics
            ...
            if mse_threshold is not None:
                ...
                if mse_val > mse_threshold:
                    logger.debug(f"MSE {mse_val} is greater than threshold {mse_threshold}, skipping")
                    continue
    ...
    def _create_tuning_results_per_step_per_layer(self, step: int, cfg_type: str, layer_name: str) -> None:
        """
    ...
    def record_tuning_result_per_step_per_layer(
        self, meta_info: OpTuningMetaInfo, result: OpTuningResult, step: int, cfg_type: str, layer_name: str
    ) -> None:
        """
    ...
    def record_io_tensor_per_step_per_layer(
        ...
        for name, tensor in tensor_dict.items():
            if name in io_data:
                logger.warning(f"Tensor {name} already exists for step {step}, layer {layer_name}, skipping")
                continue
        ...
        if self._estimated_memory_usage > self.max_memory_bytes:
            ...
            logger.info(f"IO tensors memory usage ({memory_gb:.2f} GB) exceeds limit ({self.max_memory_gb} GB)")
            self._save_and_clear_io_tensors_chunk()
    ...
    def _save_heatmaps_per_head(
        ...
        for head_idx in range(num_heads):
            ...
            head_filename = f"step_{step:03d}_{cfg_type}_{clean_layer_name}_head_{global_head_idx:02d}.jpg"
        ...
        logger.debug(f"Saved {num_heads} attention heatmaps for {layer_name} ({cfg_type}) at step {step}")
                    ...
                    if isinstance(tuning_result, TuningResult):
                        for meta_info, result_dict in tuning_result.results_of_each_shape.items():
                            ...
                            serializable_results[step][cfg_type][layer_name][meta_key] = {
                                ...
                                "results": {impl_type: result.to_dict() for impl_type, result in result_dict.items()},
                            }
                ...
                for layer_name, layer_data in cfg_data.items():
                    ...
                    for meta_key, meta_result_data in layer_data.items():
                        if (
                            ...
                            best_impl = self._find_best_impl(result_dict, cosine_similarity_threshold, mse_threshold)
                            result_dict["best_impl"] = result_dict[best_impl]

                            # Add to TuningResult
                            tuning_result.results_of_each_shape[meta_info] = result_dict
                        else:
                            logger.warning(f"Unexpected data structure for {meta_key}: {meta_result_data}")

                    self.tuning_results_per_step_per_layer[step][cfg_type][layer_name] = tuning_result
