...
class ditLinear(nn.Linear):
    ...
    def create_nvfp4_quantized_weight(
        ...
        weights_scaling_factor_2 = (
            ...
            * 6.0
            / torch.max(torch.abs(param_value).to(torch.float))
        )
    ...
    def create_torch_ao_quantized_weight(self, param_value: torch.Tensor):
        ...
        weight = quant_api._float8_dynamic_activation_float8_weight_quantize_tensor(
            param_value, config
        )
    ...
    def _register_fp8_weight(self, linear_type: str):
        ...
        if linear_type == "trtllm-fp8-blockwise":
            ...
            if not hasattr(self, weight_name) or not hasattr(self, weight_scale_name):
                weight, weight_scale = self.create_blockwise_quantized_weight(
                    self.weight, use_ue8m0=True
                )
                self.register_parameter(
                    weight_name, torch.nn.Parameter(weight, requires_grad=False)
                )
    ...
    def _register_nvfp4_weight(self, linear_type: str):
        ...
        if (
            ...
            weight, weight_scale, weights_scaling_factor_2 = self.create_nvfp4_quantized_weight(
                self.weight
            )
    ...
    def load_fp4_weight(self, weights, svd_weight_name_table):
        ...
        if name in svd_weight_name_table:
            self.svd_qweight = (
                weights[prefill + "." + svd_weight_name_table[name] + ".qweight"]
                .to("cuda")
                .contiguous()
            )
            self.svd_lora_up = (
                weights[prefill + "." + svd_weight_name_table[name] + ".lora_up"]
                .to("cuda")
                .contiguous()
            )
            self.svd_lora_down = (
                weights[prefill + "." + svd_weight_name_table[name] + ".lora_down"]
                .to("cuda")
                .contiguous()
            )
            self.svd_smooth = (
                weights[prefill + "." + svd_weight_name_table[name] + ".smooth"]
                .to("cuda")
                .contiguous()
            )
            self.svd_wscales = (
                weights[prefill + "." + svd_weight_name_table[name] + ".wscales"]
                .to("cuda")
                .contiguous()
            )
            if prefill + "." + svd_weight_name_table[name] + ".wtscale" in weights:
                self.svd_wtscale = weights[
                    prefill + "." + svd_weight_name_table[name] + ".wtscale"
                ].item()
                self.svd_wcscales = None
            else:
                self.svd_wtscale = torch.ones(1, dtype=torch.float32).item()
                self.svd_wcscales = (
                    weights[prefill + "." + svd_weight_name_table[name] + ".wcscales"]
                    .to("cuda")
                    .contiguous()
                )
            self.svd_bias = (
                weights[prefill + "." + svd_weight_name_table[name] + ".bias"]
                .to("cuda")
                .contiguous()
            )
    ...
    def select_linear_impl(self, input: torch.Tensor = None):
            ...
            if auto_tuner.mode == "inference":
                ...
                for linear_type in LinearOpManager.get_registered_types():
                    ...
                    if linear_type == "trtllm-nvfp4" or linear_type == "comfy-kitchen-nvfp4":
                        ...
                        self._register_fi_nvfp4_weight(
                            linear_type, sf_layout=SfLayout.layout_128x4, do_shuffle=True
                        )
                    elif linear_type in ["flashinfer-nvfp4-cudnn", "flashinfer-nvfp4-cutlass"]:
                        self._register_fi_nvfp4_weight(
                            linear_type, sf_layout=SfLayout.layout_128x4, do_shuffle=False
                        )
                    else:
        ...
        if linear_type == "default":
            ...
            or linear_type == "trtllm-fp8-per-tensor"
            or linear_type == "deepgemm-MXFP8"
        ):
            self._register_fp8_weight(linear_type)
            weight_name = linear_type + "_weight"
            weight_scale_name = linear_type + "_weight_scale"
        ...
        if self.offloading:
            ...
            if disable_offloading:
                ...
                with torch.cuda.stream(stream=self.offloading_stream):
                    ...
                    self.device_weight[self.pingpong_out].copy_(
                        self.next_offloading_layer_weight, non_blocking=True
                    )
