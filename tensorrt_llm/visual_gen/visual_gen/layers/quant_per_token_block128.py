...
def dequant_per_token_block128_kernel(
    ...
    for off_n_base in range(pid_sm * TOKENS_PER_BLOCK, L, num_sm * TOKENS_PER_BLOCK):
        ...
        scale_ptrs = base_ptr_scale + token_ids_1d * stride_sn  # [4]
        scale_1d = tl.load(
            scale_ptrs, mask=mask_token_1d, other=1.0, eviction_policy="evict_last"
        )  # [4]
...
def quantize_per_token_block128(input_tensor, tensor_layout="HND"):
    ...
    if tensor_layout == "HND":
        ...
        output_scale = torch.empty(
            (b, h, seq_len, 1), device=input_tensor.device, dtype=torch.float32
        )
        ...
        output_scale = torch.empty(
            (b, seq_len, h, 1), device=input_tensor.device, dtype=torch.float32
        )
...
def dequantize_per_token_block128(q_int8, q_scale, tensor_layout="HND"):
    ...
    if q_scale.ndim != 4:
        raise ValueError(
            f"q_scale must be 4-dimensional, got shape {q_scale.shape} ({q_scale.ndim}D)"
        )
...
def dequant_per_token_block128_packed_kernel(
    ...
    for off_n_base in range(pid_sm * TOKENS_PER_BLOCK, L, num_sm * TOKENS_PER_BLOCK):
        ...
        scale_1d = tl.load(
            scale_ptrs, mask=mask_token_1d, other=1e-12, eviction_policy="evict_last"
        )
        scale = tl.reshape(scale_1d, [TOKENS_PER_BLOCK, 1])  # [4] -> [4, 1] for broadcasting
...
def dequantize_per_token_block128_packed(packed_tensor, tensor_layout="HND"):
    ...
    if tensor_layout == "HND":
        ...
        stride_bz, stride_h, stride_seq = (
            packed_tensor.stride(0),
            packed_tensor.stride(1),
            packed_tensor.stride(2),
        )
    elif tensor_layout == "NHD":
...
def test_hnd_layout_packed():
    ...
    q_rel_error = (
        (torch.abs(q_dequant_packed - q_expected) / (torch.abs(q_expected) + 1e-6)).mean().item()
    )
...
def test_nhd_layout_packed():
    ...
    k_rel_error = (
        (torch.abs(k_dequant_packed - k_expected) / (torch.abs(k_expected) + 1e-6)).mean().item()
    )
...
def test_nhd_profile(batch, seq_len, heads):
    ...
    for i in range(warm_up_time + run_time):
        q = (
            torch.rand(batch, seq_len, heads, 128, dtype=torch.float32, device=device) - 0.5
        ) * 4000
        q = q.to(dtype)
        q_list.append(q)
    ...
    for i in range(warm_up_time):
        q_dequant = dequantize_per_token_block128(
            q_int8_list[i], q_scale_list[i], tensor_layout="NHD"
        )
...
def test_nhd_profile_packed(batch, seq_len, heads):
    ...
    for i in range(warm_up_time):
        q_dequant_packed = dequantize_per_token_block128_packed(
            q_packed_list[i], tensor_layout="NHD"
        )
    ...
    for i in range(run_time):
        q_dequant_packed = dequantize_per_token_block128_packed(
            q_packed_list[warm_up_time + i], tensor_layout="NHD"
        )
    torch.cuda.synchronize()
    end_time = time.time()
    total_time = end_time - start_time
