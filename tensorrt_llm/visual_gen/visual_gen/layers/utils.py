...
def ring_fwd_out_correction(
    ...
    new_out = out - F.sigmoid(softmax_lse_per_step.unsqueeze(-1) - softmax_lse.unsqueeze(-1)) * (
        out - out_per_step
    )
    out.copy_(new_out)
...
def ring_attn_p2p_communicate(rank, send_tensor, send_dst, recv_tensor, recv_src, ring_group):
    ...
    if rank % 2 == 0:
        send_op = torch.distributed.P2POp(
            torch.distributed.isend, send_tensor, group_peer=send_dst, group=ring_group
        )
        recv_op = torch.distributed.P2POp(
            torch.distributed.irecv, recv_tensor, group_peer=recv_src, group=ring_group
        )
...
def ulysses_wrapper(func):
    ...
    def wrapper(self, query, key, value, tensor_layout, attn_mask=None, **kwargs):
        ...
        if key.shape[head_dim] % ulysses_size != 0:
            raise ValueError(
                f"Head dim {head_dim} of key {key.shape} must be divisible by ulysses size {ulysses_size}"
            )
        ...
        if joint_seq_length > 0:
            if ring_size == 1:
                ...
                assert joint_strategy != "none", (
                    "joint_strategy can not be none when joint_seq_length > 0"
                )
                if joint_strategy == "rear":
                    ...
                    query = torch.narrow(
                        query, seq_dim, 0, query.shape[seq_dim] - joint_seq_length
                    ).contiguous()
                    key = torch.narrow(
                        key, seq_dim, 0, key.shape[seq_dim] - joint_seq_length
                    ).contiguous()
                    value = torch.narrow(
                        value, seq_dim, 0, value.shape[seq_dim] - joint_seq_length
                    ).contiguous()
                else:
        ...
        if joint_seq_length > 0:
            if ring_size == 1 and cp_size == 1:
                ...
                raise NotImplementedError(
                    "joint_seq_length > 0 is not supported when ring size > 1 or cp size > 1"
                )
        ...
        if ring_size == 1:
            if joint_seq_length > 0:
                if joint_strategy == "rear":
                    ...
                    result = torch.narrow(
                        result, seq_dim, 0, result.shape[seq_dim] - joint_seq_length
                    ).contiguous()
                else:
                ...
                joint_result_gathered = torch.empty(
                    ulysses_size,
                    *joint_result.shape,
                    device=joint_result.device,
                    dtype=joint_result.dtype,
                )
                ...
                if tensor_layout == "HND":
                    ...
                    joint_result = joint_result_gathered.permute(1, 0, 2, 3, 4).reshape(
                        B, ulysses_size * H, S, D
                    )
                elif tensor_layout == "NHD":
                    # [ulysses_size, B, S, H, D] -> [B, S, ulysses_size * H, D]
                    _, B, S, H, D = joint_result_gathered.shape
                    joint_result = joint_result_gathered.permute(1, 2, 0, 3, 4).reshape(
                        B, S, ulysses_size * H, D
                    )
                else:
                    raise ValueError(f"Invalid tensor layout: {tensor_layout}")
            ...
            if truncate_and_pad:
                ...
                if block_out.shape[seq_dim] > q_seq:
                    padding_part = torch.narrow(
                        block_out, seq_dim, q_seq, block_out.shape[seq_dim] - q_seq
                    )
                    padding_part.zero_()

                if block_lse.shape[seq_dim] > q_seq:
                    padding_part = torch.narrow(
                        block_lse, seq_dim, q_seq, block_lse.shape[seq_dim] - q_seq
                    )
                    padding_part.zero_()
...
def ring_wrapper(func):
    ...
    def wrapper(self, query, key, value, tensor_layout, attn_mask=None, **kwargs):
        ...
        if self.attn_impl.__class__.__name__ not in [
            "SageAttn",
            "FlashAttn3",
            "FlashAttn4",
            "FlashInferVx",
        ...
        for i in range(ring_size):
            ...
            if PipelineConfig.seq_len_cur_ring_group is not None:
                if ulysses_size == 1:
                    ...
                    if (
                        ...
                        != PipelineConfig.seq_len_cur_ring_group[kv_rank]
                    ):
            ...
            with torch.cuda.device(
                ...
                block_out = func(
                    self, query, kv_inputs[0], kv_inputs[1], tensor_layout, attn_mask, **kwargs
                )
        ...
        if joint_seq_length > 0:
            ...
            block_out = func(
                self, query, joint_key, joint_value, tensor_layout, attn_mask, **kwargs
            )
...
def cp_wrapper(func):
    ...
    def wrapper(self, query, key, value, tensor_layout, attn_mask=None, **kwargs):
        ...
        work = torch.distributed.all_gather_into_tensor(
            kv_gathered, kv, group=cp_group, async_op=True
        )
        work.wait()
        if tensor_layout == "HND":
            _, B, H, S, D = kv.shape
    ...
    def wrapper(self, query, key, value, tensor_layout, attn_mask=None, **kwargs):
        ...
        if joint_seq_length > 0:
            ...
            for batch_idx in range(B):
                ...
                valid_kv_length = (
                    key.shape[seq_dim] - joint_seq_length + valid_joint_seq_length[batch_idx]
                )
                if tensor_layout == "HND":
                    cur_out = func(
                        self,
