...
def main():
    ...
    parser = argparse.ArgumentParser(
        description="Calculate video similarity metrics (PSNR, SSIM, LPIPS)"
    )
    parser.add_argument(
        "--original_path",
        type=str,
    ...
    parser.add_argument(
        ...
        type=str,
        default=None,
        help="Path to save the results (JSON file or directory)",
    )
    parser.add_argument(
        "--lpips_net",
        type=str,
    ...
    parser.add_argument(
        ...
        type=float,
        default=255.0,
        help="Data range for PSNR/SSIM calculation (default: 255.0)",
    )
    parser.add_argument(
        "--max_frames",
        type=int,
    ...
    parser.add_argument(
        ...
        type=int,
        default=0,
        help="Number of frames to skip from the beginning (default: 0)",
    )
    ...
    assert os.path.exists(original_video_path), (
        f"Original video path {original_video_path} does not exist"
    )
    assert os.path.exists(generated_video_path), (
        f"Generated video path {generated_video_path} does not exist"
    )
    ...
    if os.path.isdir(original_video_path) and os.path.isdir(generated_video_path):
        assert len(os.listdir(original_video_path)) == len(os.listdir(generated_video_path)), (
            f"Original video path {original_video_path} and generated video path {generated_video_path} have different number of files"
        )
        psnr_scores = []
        ssim_scores = []
        lpips_scores = []
        for ori_video, gen_video in zip(
            os.listdir(original_video_path), os.listdir(generated_video_path)
        ):
            psnr, ssim, lpips = video_similarity_metrics(
                os.path.join(original_video_path, ori_video),
                os.path.join(generated_video_path, gen_video),
            )
