            ...
            for layer_name, tensors in cfg_data.items():
                ...
                if "query" in tensors and "key" in tensors:
                    ...
                    if len(query.shape) == 4 and len(key.shape) == 4:
                        print(f"Found attention tensors: step={step}, layer={layer_name}, cfg={cfg_type}")
...
def compute_attention_correlation(
    query: torch.Tensor, key: torch.Tensor, temperature: float = 1.0, block_size: int = 128, device: str = "cpu"
) -> torch.Tensor:
    """
...
def process_chunk_on_gpu(
    ...
    print(f"[GPU {gpu_id}] Finished processing {os.path.basename(chunk_file)} - {processed_count} pairs")
...
def main():
    parser = argparse.ArgumentParser(description="Visualize attention correlations from saved io_tensors")
    parser.add_argument(
        "--input_dir", type=str, required=True, help="Directory containing saved io_tensors_per_step files"
    )
    parser.add_argument("--output_dir", type=str, required=True, help="Output directory for heatmap visualizations")
    parser.add_argument(
        "--temperature", type=float, default=1.0, help="Temperature for softmax computation (default: 1.0)"
    )
    parser.add_argument(
        "--steps", type=str, default=None, help="Comma-separated list of specific steps to process (default: all)"
    )
    parser.add_argument(
        "--layers",
        type=str,
    ...
    parser.add_argument(
        "--num_gpus", type=int, default=-1, help="Number of GPUs to use (-1 for all available, 0 for CPU only)"
    )
    ...
    if len(gpu_ids) > 1:
            ...
            for gpu_id, chunk_list in zip(gpu_ids, chunks_per_gpu):
                for chunk_file in chunk_list:
                    task = pool.apply_async(
                        ...
                        (chunk_file, gpu_id, args.output_dir, args.temperature, target_steps, layer_patterns),
                    )
                    tasks.append(task)
        ...
        for chunk_idx, chunk_file in enumerate(tensor_files):
            print(f"\n{'='*60}")
            print(f"Processing chunk {chunk_idx + 1}/{len(tensor_files)}: {os.path.basename(chunk_file)}")
            print(f"{'='*60}")
