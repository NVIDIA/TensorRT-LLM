...
def configure_cpu_offload(pipe, args, local_rank: int, model_wise=None, block_wise=None):
    ...
    if args.enable_sequential_cpu_offload:
        ...
        assert args.visual_gen_block_cpu_offload_stride > 0, (
            "visual_gen_block_cpu_offload_stride must be greater than 0"
        )
        ...
        logger.info(
            f"enabled visual_gen's cpu offload, offloading stride: {args.visual_gen_block_cpu_offload_stride}"
        )
    else:
...
def recompute_shape_for_vae(height, width, vae_scale_factor, patch_size, vae_parallel_dim="height"):
    ...
    if updated_height != height or updated_width != width:
        logger.warning(
            f"Recomputed output shape(hxw) for vae: {height}x{width} -> {updated_height}x{updated_width}"
        )
...
def benchmark_inference(
    ...
    **kwargs,
    ...
    if enable_autotuner:
        assert autotuner_dir is not None, (
            "autotuner_dir must be provided when enable_autotuner is True"
        )
