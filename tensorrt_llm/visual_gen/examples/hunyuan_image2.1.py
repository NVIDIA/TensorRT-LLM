...
from visual_gen.configs.parallel import DiTParallelConfig, RefinerDiTParallelConfig, VAEParallelConfig
...
from visual_gen.utils.parallel import (
    dit_dp_gather,
    dit_dp_split,
    dit_sp_gather,
    dit_sp_split,
...
def _preprocess_text(
    ...
    if "byt5_text_states" in extra_kwargs and extra_kwargs["byt5_text_states"].shape[0] == text_states.shape[0] * 2:
        extra_kwargs["byt5_text_states"] = dit_dp_split(extra_kwargs["byt5_text_states"], dim=0)
    if "byt5_text_mask" in extra_kwargs and extra_kwargs["byt5_text_mask"].shape[0] == text_states.shape[0] * 2:
    ...
    assert valid_text_len.max() == valid_text_len.min(), "valid_text_len must be the same for all samples"
    ...
    if "byt5_text_states" in extra_kwargs:
        ...
        assert (
            valid_byt5_text_len.max() == valid_byt5_text_len.min()
...
def dit_forward(
    ...
    *,
    ...
    with torch.no_grad(), torch.cuda.nvtx.range(
...
def load_and_setup_pipeline(args):
    ...
    def enable_visual_gen(pipe, is_refiner: bool = False):
        ...
        apply_visual_gen_norm(
            pipe.dit,
            rmsnorm=["img_attn_q_norm", "img_attn_k_norm", "txt_attn_q_norm", "txt_attn_k_norm", "q_norm", "k_norm"],
