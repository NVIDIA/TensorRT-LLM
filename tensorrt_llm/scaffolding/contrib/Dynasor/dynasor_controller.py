...
class DynasorGenerationController(Controller):
    ...
    def process(self, tasks: List[GenerationTask], **kwargs):
        ...
        for _ in range(
            initial_prompt_token_num + probe_suffix_token_num, self.max_tokens, self.chunk_size
        ):
            ...
            if (
                equal_group(probe_answers[-self.certainty_threshold :])
                and self.count_not_empty(probe_answers[-self.certainty_threshold :])
                ...
                if "</think>" in current_prompt:
                    tasks[0].output_str = (
                        current_prompt + self.answer_suffix + probe_answers[-1] + "}\n\\]"
                    )
                    return
                else:
