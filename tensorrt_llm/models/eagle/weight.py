...
def load_eagle_hf(eagle_model_dir,
                  eagle_model,
                  base_model,
                  ...
                  rank=0,
                  dtype='float32'):
    logger.info("Loading EAGLE weights ...")

    renamed_weights = {}
    weights = convert_hf_llama(eagle_model, mapping, rank, dtype=dtype)

    eagle_ckpt_file = Path(eagle_model_dir) / "pytorch_model.bin"
    eagle_state_dict = torch.load(eagle_ckpt_file, map_location="cpu")
