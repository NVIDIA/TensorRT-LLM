...
class DecoderModelForCausalLM(PretrainedModel):
    ...
    def forward(self,
        ...
        if self.config.mapping.is_last_pp_rank():
            ...
            if self.config.has_config_group(Gemma2ConfigGroup) or is_gemma_3_cg:
                softcap = self.config.get_config_group(
                    Gemma2ConfigGroup if not is_gemma_3_cg else Gemma3ConfigGroup).final_logit_softcapping
                if softcap:
