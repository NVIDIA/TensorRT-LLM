...
from tensorrt_llm.layers import ColumnLinear, Conv3d, LayerNorm, Linear, RowLinear
from tensorrt_llm.layers.attention import (
    Attention,
    AttentionParams,
    BertAttention,
    KeyValueCacheParams,
...
from tensorrt_llm.models.model_weights_loader import ModelWeightsFormat, ModelWeightsLoader
...
class RotaryEmbedder(Module):
    ...
    def get_freqs(self, t: Tensor, seq_len: Optional[int] = None, offset: int = 0):
        ...
        if should_cache and isinstance(self.cached_freqs, Tensor):
            if (offset + seq_len) <= self.cached_freqs.shape[0]:
                return slice(
                    self.cached_freqs,
                    starts=[offset] + [0] * len(self.cached_freqs.shape[1:]),
    ...
    def get_seq_pos(self, seq_len: int, dtype: trt.DataType, offset: int = 0):
        return (
            arange(start=0, end=seq_len, dtype=trt_dtype_to_str(dtype)) + offset
    ...
    def apply_rotary_emb(
        self, freqs: Tensor, t: Tensor, start_index: int = 0, scale: int = 1.0, seq_dim: int = -2
    ):
        if t.ndim() == 3:
        ...
        t_left = slice(t, starts=[0] * t.ndim(), sizes=[*t.shape[:-1], start_index])
        t_right = slice(
            t,
        ...
        t = (t * cos(freqs) * scale) + (self.rotate_half(t) * sin(freqs) * scale)
    ...
    def rotate_queries_or_keys(
        ...
        assert not self.use_xpos, (
            "you must use `.rotate_queries_and_keys` method "
        ...
        freqs = self.get_freqs(
            self.get_seq_pos(seq_len, dtype=t.dtype, offset=offset), seq_len=seq_len, offset=offset
        )
...
class STDiTRmsNorm(RmsNorm):
    def __init__(
        self, normalized_shape, num_groups=1, eps=1e-06, elementwise_affine=True, dtype=None
    ):
        super().__init__(normalized_shape, num_groups, eps, elementwise_affine, dtype)
...
class STDiTAttention(BertAttention):
    ...
    def forward(
        ...
        query = query.view(
            concat([B, N, self.num_attention_heads, self.attention_head_size])
        ).permute(dims=[0, 2, 1, 3])
        key = key.view(
            concat([B, N, self.num_attention_kv_heads, self.attention_head_size])
        ).permute(dims=[0, 2, 1, 3])
        ...
        # TODO deal with qkv
        query = query.permute(dims=[0, 2, 1, 3]).view(concat([B, N, self.attention_hidden_size]))
        ...
        if default_net().plugin_config.bert_attention_plugin:
            ...
            if default_net().plugin_config.remove_input_padding:
                qkv = qkv.view(concat([-1, self.attention_hidden_size + 2 * kv_size]))
            ...
            def transpose_for_scores(x):
                new_x_shape = concat(
                    [shape(x, 0), shape(x, 1), self.num_attention_heads, self.attention_head_size]
                )
            ...
            attention_scores = attention_scores / (self.q_scaling * self.norm_factor)
            ...
            context = matmul(attention_probs, value, use_fp32_acc=False).permute([0, 2, 1, 3])
            context = context.view(
                concat([shape(context, 0), shape(context, 1), self.attention_hidden_size])
            )
...
class STDiTCrossAttention(Attention):
    ...
    def forward(
        ...
        if default_net().plugin_config.remove_input_padding:
            ...
            encoder_output = encoder_output.view(concat([-1, encoder_hidden_size]))

        context = super().forward(
...
class T2IFinalLayer(Module):
    def __init__(
        ...
        super().__init__()
        self.norm_final = LayerNorm(hidden_size, elementwise_affine=False, eps=1e-6, dtype=dtype)
        self.linear = Linear(hidden_size, num_patch * out_channels, bias=True, dtype=dtype)
        self.scale_shift_table = Parameter(
            torch.randn(2, hidden_size) / hidden_size**0.5, dtype=dtype
        )
    ...
    def t_mask_select(self, x_mask, x, masked_x, T: int, S: int):
        ...
        x = where(expand_dims(x_mask, [x_mask.ndim(), x_mask.ndim() + 1]), x, masked_x)
        x = rearrange(x, "B T S C -> B (T S) C")
        return x

    def forward(self, x, t, x_mask=None, t0=None, T: Optional[int] = None, S: Optional[int] = None):
        if T is None:
            T = self.d_t
        if S is None:
            S = self.d_s
        shift, scale = chunk(
            expand_dims(self.scale_shift_table.value, 0) + expand_dims(t, 1), chunks=2, dim=1
        )
        x = t2i_modulate(self.norm_final(x), shift, scale)
        if x_mask is not None:
            shift_zero, scale_zero = chunk(
                expand_dims(self.scale_shift_table.value, 0) + expand_dims(t0, 1), chunks=2, dim=1
            )
...
class PositionEmbedding2D(Module):
    def __init__(self, dim: int, dtype=None, mapping=Mapping(), quant_mode=QuantMode(0)):
        super().__init__()
        self.dim = dim
...
class TimestepEmbedder(Module):
    def __init__(
        ...
        self.mlp = ModuleSequential(
            [
                Linear(frequency_embedding_size, hidden_size, bias=True, dtype=dtype),
                Activation("silu"),
    ...
    def forward(self, t, dtype):
        t_freq = self.timestep_embedding(t, self.frequency_embedding_size).cast(dtype)
...
class SizeEmbedder(TimestepEmbedder):
    ...
    def forward(self, s, bs: int):
        ...
        s_freq = self.timestep_embedding(s, self.frequency_embedding_size).cast(self.dtype)
...
class CaptionMLP(Module):
    def __init__(
        ...
        if hidden_act not in ACT2FN:
            raise ValueError("unsupported activation function: {}".format(hidden_act))
        fc_output_size = (
            2 * hidden_features if hidden_act in ["swiglu", "gegelu"] else hidden_features
        )
        self.inner_layernorm = (
            LayerNorm(hidden_features, dtype=dtype, eps=eps) if inner_layernorm else None
        )
...
class CaptionEmbedder(Module):
    def __init__(
        ...
        self.y_embedding = Parameter(
            torch.randn(token_num, in_channels) / in_channels**0.5, dtype=dtype
        )
    ...
    def token_drop(self, caption, force_drop_ids=None):
        if not USE_STATIC_SHAPE:
            raise NotImplementedError("Only static shape is supported")
        assert isinstance(force_drop_ids, torch.Tensor) or isinstance(force_drop_ids, np.array)
        if force_drop_ids is None:
            drop_ids = torch.rand(caption.shape[0]).cuda() < self.uncond_prob
        else:
            drop_ids = torch.Tensor(force_drop_ids) == 1
        drop_ids = constant(drop_ids.cpu().numpy())
        caption = where(expand_dims(drop_ids, [1, 2, 3]), self.y_embedding.value, caption)
        return caption
...
class PatchEmbed3D(Module):
    ...
    def forward(self, x):
        ...
        if D % self.patch_size[0] != 0:
            x = pad(x, (0, 0, 0, 0, 0, self.patch_size[0] - D % self.patch_size[0]))
...
class STDiT3Block(Module):
    def __init__(
        ...
        self.norm1 = LayerNorm(hidden_size, eps=1e-6, elementwise_affine=False, dtype=dtype)
        self.attn = attn_cls(
            hidden_size=hidden_size,
        ...
        self.norm2 = LayerNorm(hidden_size, eps=1e-6, elementwise_affine=False, dtype=dtype)
        self.mlp = CaptionMLP(
        ...
        self.scale_shift_table = Parameter(
            torch.randn(6, hidden_size) / hidden_size**0.5, dtype=dtype
        )
        self.dtype = dtype
        self.mapping = mapping
        self.quant_mode = quant_mode
    ...
    def forward(
        ...
        shift_msa, scale_msa, gate_msa, shift_mlp, scale_mlp, gate_mlp = chunk(
            expand_dims(self.scale_shift_table.value, 0) + t.view([B, 6, -1]), chunks=6, dim=1
        )
        if x_mask is not None:
            (
                shift_msa_zero,
                ...
                expand_dims(self.scale_shift_table.value, 0) + t0.view([B, 6, -1]), chunks=6, dim=1
            )
        ...
        if self.temporal:
            ...
            x_m = self.attn(x_m, max_input_length=attention_params.max_context_length)
        ...
        cattn = self.cross_attn(
            ...
            attention_params=AttentionParams(
                ...
                encoder_max_input_length=attention_params.encoder_max_input_length,
...
class STDiT3Model(PretrainedModel):
    def __init__(self, config: STDiTModelConfig):
        ...
        self.rope = RotaryEmbedder(dim=self.hidden_size // self.num_heads, dtype=self.dtype)
        self.x_embedder = PatchEmbed3D(
            self.patch_size, self.in_channels, self.hidden_size, dtype=self.dtype
        )
        ...
        self.t_block = ModuleSequential(
            [
                Activation("silu"),
                Linear(self.hidden_size, 6 * self.hidden_size, bias=True, dtype=self.dtype),
            ]
        )
    ...
    def encode_text(self, y: Tensor, mask: Optional[Tensor] = None):
        ...
        if mask is not None:
            ...
            y = masked_select(
                squeeze(y, 1),
                where(
                    unsqueeze(mask, -1).__eq__(constant_to_tensor_(0, dtype=mask.dtype)),
                    constant_to_tensor_(False),
                    constant_to_tensor_(True),
                ),
            ...
            y_lens = constant(np.array([y.shape[2]] * y.shape[0], dtype=np.int64))
    ...
    def unpatchify(self, x: Tensor, N_t: int, N_h: int, N_w: int, R_t: int, R_h: int, R_w: int):
        ...
        x = slice(
            ...
            starts=[0] * x.ndim(),
            sizes=concat(
                [shape(x, 0), shape(x, 1), constant(np.array([R_t, R_h, R_w]).astype(np.int64))]
            ),
        )
    ...
    def forward(
        ...
        **kwargs,
        ...
        if not USE_STATIC_SHAPE:
            raise NotImplementedError("Only static shape is supported")
        assert tuple(x.shape[2:]) == tuple(self.latent_size), (
            "For now only static shape is supported."
        )
        B = x.shape[0]
        x = x.cast(self.dtype)
        ...
        t0 = t0_mlp = None
        if x_mask is not None:
            t0_timestep = constant(np.zeros(shape=timestep.shape).astype(np.float32)).cast(x.dtype)
    ...
    def prepare_inputs(self, max_batch_size, **kwargs):
        ...
        max_cattn_seq_len = int(
            np.prod([np.ceil(d / p) for d, p in zip(self.latent_size, self.patch_size)])
        )
        ...
        host_max_attention_window_sizes = attn_inputs["host_max_attention_window_sizes"]
        ...
        cross_encoder_input_lengths = Tensor(
    ...
    @classmethod
    def from_pretrained(
        cls, pretrained_model_dir: str, dtype="float16", mapping=Mapping(), **kwargs
    ):
        ...
        model_config = STDiTModelConfig.from_input_config(hf_config, dtype=dtype, mapping=mapping)
...
class STDiT3ModelWeightsLoader(ModelWeightsLoader):
    def translate_to_external_key(self, tllm_key: str, tllm_to_externel_key_dict: dict):
        ...
        trtllm_to_hf_name = {
            r"spatial_blocks.(\d+).attn.q_layernorm.weight": "spatial_blocks.*.attn.q_norm.weight",
            r"spatial_blocks.(\d+).attn.k_layernorm.weight": "spatial_blocks.*.attn.k_norm.weight",
            r"spatial_blocks.(\d+).attn.dense.weight": "spatial_blocks.*.attn.proj.weight",
            r"spatial_blocks.(\d+).attn.dense.bias": "spatial_blocks.*.attn.proj.bias",
            r"temporal_blocks.(\d+).attn.q_layernorm.weight": "temporal_blocks.*.attn.q_norm.weight",
            r"temporal_blocks.(\d+).attn.k_layernorm.weight": "temporal_blocks.*.attn.k_norm.weight",
            r"temporal_blocks.(\d+).attn.dense.weight": "temporal_blocks.*.attn.proj.weight",
            r"temporal_blocks.(\d+).attn.dense.bias": "temporal_blocks.*.attn.proj.bias",
            r"spatial_blocks.(\d+).cross_attn.dense.weight": "spatial_blocks.*.cross_attn.proj.weight",
            r"spatial_blocks.(\d+).cross_attn.dense.bias": "spatial_blocks.*.cross_attn.proj.bias",
            r"temporal_blocks.(\d+).cross_attn.dense.weight": "temporal_blocks.*.cross_attn.proj.weight",
            r"temporal_blocks.(\d+).cross_attn.dense.bias": "temporal_blocks.*.cross_attn.proj.bias",
        }
    ...
    def generate_tllm_weights(self, model, custom_postprocess_kwargs: dict = {}):
        ...
        for tllm_key, _ in tqdm(model.named_parameters()):
            skip_tp = not any(
                [re.match(pattern, tllm_key) is not None for pattern in tp_module_patterns]
            )
            tllm_weights.update(
                self.load(
                    tllm_key, custom_postprocess_kwargs=custom_postprocess_kwargs, skip_tp=skip_tp
                )
            )
