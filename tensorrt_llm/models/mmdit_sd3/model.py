...
from ...functional import Tensor, allgather, chunk, concat, einsum, pad, shape, unsqueeze
from ...layers import LayerNorm, Linear
from ...layers.attention import DiffusersAttention
from ...layers.embedding import CombinedTimestepTextProjEmbeddings, SD3PatchEmbed
from ...layers.mlp import (
    LinearActivation,
    LinearApproximateGELU,
    LinearGEGLU,
    LinearGELU,
    LinearSwiGLU,
)
from ...layers.normalization import AdaLayerNormContinuous, AdaLayerNormZero, SD35AdaLayerNormZeroX
...
class FeedForward(Module):
    def __init__(
        ...
        if activation_fn == "gelu-approximate":
            act_fn = LinearGELU(
                dim, inner_dim, approximate="tanh", bias=bias, mapping=mapping, dtype=dtype
            )
            ...
            act_fn = LinearSwiGLU(dim, inner_dim, bias=bias, mapping=mapping, dtype=dtype)
        elif activation_fn == "linear-silu":
            act_fn = LinearActivation(
                dim, inner_dim, bias=bias, activation="silu", mapping=mapping, dtype=dtype
            )
...
class JointTransformerBlock(Module):
    def __init__(
        ...
        if use_dual_attention:
            self.norm1 = SD35AdaLayerNormZeroX(dim, mapping=mapping, dtype=dtype)
        else:
        ...
        if context_norm_type == "ada_norm_continous":
            ...
            raise ValueError(
                f"Unknown context_norm_type: {context_norm_type}, currently only support `ada_norm_continous`, "
                "`ada_norm_zero`"
            )
        ...
        self.norm2 = LayerNorm(dim, elementwise_affine=False, eps=1e-6, dtype=dtype)
        self.ff = FeedForward(
            dim=dim, dim_out=dim, activation_fn="gelu-approximate", mapping=mapping, dtype=dtype
        )

        if not context_pre_only:
            self.norm2_context = LayerNorm(dim, elementwise_affine=False, eps=1e-6, dtype=dtype)
    ...
    @staticmethod
    def _chunked_feed_forward(ff: Module, hidden_states: Tensor, chunk_dim: int, chunk_size: int):
        # "feed_forward_chunk_size" can be used to save memory
        if hidden_states.shape[chunk_dim] % chunk_size != 0:
            raise ValueError(
                f"`hidden_states` dimension to be chunked: {hidden_states.shape[chunk_dim]} has to be divisible by "
                f"chunk size: {chunk_size}. Make sure to set an appropriate `chunk_size` when calling "
                "`unet.enable_forward_chunking`."
            )

        num_chunks = hidden_states.shape[chunk_dim] // chunk_size
        ff_output = concat(
            [ff(hid_slice) for hid_slice in chunk(hidden_states, num_chunks, dim=chunk_dim)],
            dim=chunk_dim,
        )
        return ff_output
    ...
    def forward(
        ...
        **kwargs,
        ...
        norm_hidden_states = norm_hidden_states * (1 + unsqueeze(scale_mlp, 1)) + unsqueeze(
            shift_mlp, 1
        )

        if self._chunk_size is not None:
            # "feed_forward_chunk_size" can be used to save memory
            ff_output = self._chunked_feed_forward(
        ...
        if self.context_pre_only:
            ...
            norm_encoder_hidden_states = norm_encoder_hidden_states * (
                1 + unsqueeze(c_scale_mlp, 1)
            ) + unsqueeze(c_shift_mlp, 1)
            if self._chunk_size is not None:
                # "feed_forward_chunk_size" can be used to save memory
            ...
            encoder_hidden_states = (
                encoder_hidden_states + unsqueeze(c_gate_mlp, 1) * context_ff_output
            )
...
class SD3Transformer2DModel(PretrainedModel):
    ...
    def __init__(self, config: SD3Transformer2DModelConfig):
        ...
        self.out_channels = (
            config.out_channels if config.out_channels is not None else default_out_channels
        )
        ...
        self.pos_embed = SD3PatchEmbed(
            ...
            pos_embed_max_size=config.pos_embed_max_size,  # hard-code as HF implementation
            dtype=self.dtype,
        )
        ...
        self.transformer_blocks = ModuleList(
            [
                JointTransformerBlock(
                    ...
                    use_dual_attention=True if i in config.dual_attention_layers else False,
                    mapping=self.mapping,
                    dtype=self.dtype,
                )
        ...
        self.norm_out = AdaLayerNormContinuous(
            self.inner_dim, self.inner_dim, elementwise_affine=False, eps=1e-6, dtype=self.dtype
        )
    ...
    def forward(
        ...
        if self.mapping.cp_size > 1:
            hidden_states = chunk(hidden_states, chunks=self.mapping.cp_size, dim=1)[
                self.mapping.cp_rank
            ]
            encoder_redundant = encoder_hidden_states.shape[1] % self.mapping.cp_size
            ...
            if encoder_redundant != 0:
                encoder_hidden_states = pad(encoder_hidden_states, pad=encoder_padding_index)
            encoder_hidden_states = chunk(
                encoder_hidden_states, chunks=self.mapping.cp_size, dim=1
            )[self.mapping.cp_rank]
        for index_block, block in enumerate(self.transformer_blocks):
            # Skip specified layers
            is_skip = (
                True if self.skip_layers is not None and index_block in self.skip_layers else False
            )
        ...
        hidden_states = hidden_states.view(
            concat(
                [shape(hidden_states, 0), height, width, patch_size, patch_size, self.out_channels]
            )
        )
    ...
    def prepare_inputs(self, max_batch_size, **kwargs):
        ...
        hidden_states = Tensor(
            name="hidden_states",
            dtype=self.dtype,
            shape=[-1, self.in_channels, self.config.sample_size, self.config.sample_size],
    ...
    @classmethod
    def from_pretrained(
        cls, pretrained_model_name_or_path: str, dtype="float16", mapping=Mapping(), **kwargs
    ):
        ...
        transformer = StableDiffusion3Pipeline.from_pretrained(
            pretrained_model_name_or_path, torch_dtype=str_dtype_to_torch(dtype)
        ).transformer
        ...
        for name, param in self.named_parameters():
            ...
            if name not in weights:
                ...
                if name.endswith("lm_head.weight") and any(
                    k.endswith("vocab_embedding.weight") for k in weights.keys()
                ):
                    continue
                if name.endswith("lm_head.per_channel_scale") and any(
                    k.endswith("vocab_embedding.per_channel_scale") for k in weights.keys()
                ):
                    continue
            required_names.add(name)
            ...
            if name in provided_names:
                if not from_pruned:
                    try:
                        ...
                        raise RuntimeError(f"Encounter error '{e}' for parameter '{name}'")
                else:
...
class SD3ModelWeightsLoader(ModelWeightsLoader):
    def translate_to_external_key(self, tllm_key: str, tllm_to_externel_key_dict: dict):
        """Convert and load external checkpoint into a TensorRT-LLM model."""
        trtllm_to_hf_name = {
            r"transformer_blocks.(\d+).ff(\w*).net.1.weight": "transformer_blocks.*.ff*.net.2.weight",
            r"transformer_blocks.(\d+).ff(\w*).net.1.bias": "transformer_blocks.*.ff*.net.2.bias",
        }
        import re
