...
def get_scaling_factors(
    ...
    """ Get the scaling factors for qwen model

    Returns a dictionary of scaling factors for the selected layers of the
    qwen model.

    Args:
        model_path (str): Path to the quantized qwen model
...
def parse_bin_config(ini_file):
    ...
    hidden_act = gpt_config.get('qwen', 'hidden_act')
...
def load_from_hf_qwen(tensorrt_llm_qwen: 'Qwen2ForCausalLM',
                       hf_qwen,
                       mapping=Mapping(),
                       dtype='float32',
    ...
    if quant_mode.is_int8_weight_only():
        ...
        plugin_weight_only_quant_type = torch.quint4x2
    use_weight_only = quant_mode.is_weight_only()
    num_kv_heads = tensorrt_llm_qwen.config.num_key_value_heads
    mha_mode = (num_kv_heads == tensorrt_llm_qwen.config.num_attention_heads)
    ...
    for l in range(hf_qwen.config.num_hidden_layers):
        ...
        if not mha_mode:
            head_size = tensorrt_llm_qwen.config.hidden_size // tensorrt_llm_qwen.config.num_attention_heads
    ...
    moe_config = MoeConfig(tensorrt_llm_qwen.config.moe_num_experts,
                           tensorrt_llm_qwen.config.moe_top_k,
                           tensorrt_llm_qwen.config.moe_tp_mode,
                           tensorrt_llm_qwen.config.moe_normalization_mode)
    ...
    for k, v in model_params.items():
        ...
        if 'model.embed_tokens.weight' in k:
            ...
            if hf_qwen.config.tie_word_embeddings:
                # lm_head.weight has the same weights as embedding
                if mapping.is_last_pp_rank():
            ...
            if tensorrt_llm_qwen.config.use_parallel_embedding:
                v = split(v, mapping.tp_size, mapping.tp_rank,
                          tensorrt_llm_qwen.config.embedding_sharding_dim)
            if mapping.is_first_pp_rank():
...
def load_from_fp8_qwen(quant_ckpt_path: str, num_hidden_layers: int,
    ...
    for l in layers_range:
        ...
        weights[f'{tllm_prex}.mlp.fc.activation_scaling_factor'] = torch.tensor(
            [fp8_qwen[f'{prefix}:mlp:fc:activation_scaling_factor'].item()],
            dtype=fake_fp8_sf_dt)
        weights[f'{tllm_prex}.mlp.fc.weights_scaling_factor'] = torch.tensor(
            [fp8_qwen[f'{prefix}:mlp:fc:weights_scaling_factor'].item()],
            dtype=fake_fp8_sf_dt)

        weights[
            f'{tllm_prex}.mlp.gate.activation_scaling_factor'] = torch.tensor(
                [
                    fp8_qwen[f'{prefix}:mlp:gate:activation_scaling_factor'].
                    item()
                ],
                dtype=fake_fp8_sf_dt)
        weights[f'{tllm_prex}.mlp.gate.weights_scaling_factor'] = torch.tensor(
            [fp8_qwen[f'{prefix}:mlp:gate:weights_scaling_factor'].item()],
            dtype=fake_fp8_sf_dt)

        weights[
            f'{tllm_prex}.mlp.proj.activation_scaling_factor'] = torch.tensor(
                [
                    fp8_qwen[f'{prefix}:mlp:proj:activation_scaling_factor'].
                    item()
                ],
                dtype=fake_fp8_sf_dt)
        weights[f'{tllm_prex}.mlp.proj.weights_scaling_factor'] = torch.tensor(
            [fp8_qwen[f'{prefix}:mlp:proj:weights_scaling_factor'].item()],
            dtype=fake_fp8_sf_dt)
...
def load_from_gptq_qwen(quant_ckpt_path,
    ...
    gptq_qwen = safe_open(quant_ckpt_path, framework="pt", device=0)
    gptq_prefix = "model."
    ...
    def load(key, no_prefix=0):
        if no_prefix:
            return gptq_qwen.get_tensor(key)
        else:
            return gptq_qwen.get_tensor(gptq_prefix + key)
...
def load_from_awq_qwen(quant_ckpt_path,
                        num_hidden_layers,
                        vocab_size,
    ...
    if quant_ckpt_path.endswith(".pt"):
        ...
        awq_qwen = np.load(quant_ckpt_path)
        awq_prefix = "_np:"
        awq_suffix_list = [
            ":weight",
        ...
        def load(key):
            v = torch.from_numpy(awq_qwen[awq_prefix + key])
