...
from tensorrt_llm.models.qwen2.weight import (load_from_awq_qwen,
                                              load_from_fp8_qwen)
...
class Qwen2Model(Module):
    ...
    def __init__(self, config: PretrainedConfig) -> None:
        ...
        self.layers = DecoderLayerList(Qwen2DecoderLayer, config)
...
class Qwen2ForCausalLM(DecoderModelForCausalLM, TopModelMixin):

    def __init__(self, config: PretrainedConfig):
        self.check_config(config)
        transformer = Qwen2Model(config)
    ...
    def check_config(self, config):
        ...
        config.set_if_not_exist('dense_bias', False)
    ...
    def from_hugging_face(cls,
        ...
        cfg = qwenConfig.from_pretrained(hf_model_dir)
        ...
        tllm_qwen = Qwen2ForCausalLM(PretrainedConfig.from_dict(config))
        q_weights = {}
        if quant_mode.has_any_quant():
            q_weights = tllm_qwen._quantize(hf_model_dir, dtype, cfg, **kwargs)
        ...
        if not quant_mode.is_int4_weight_only_per_group():
            hf_model = transformers.qwenForCausalLM
            profiler.start("Loading weights from HF")
            hf_qwen = hf_model.from_pretrained(
                hf_model_dir,
                device_map={
                    "model": "cpu",
                    "lm_head": "cpu",
            ...
            tllm_qwen.load(weights)
        else:
            tllm_qwen.load(q_weights)
        return tllm_qwen
    ...
    def _quantize(self, hf_model_dir, dtype, cfg, **kwargs):
        ...
        quantized_temp_dir = tempfile.TemporaryDirectory("qwen-quantized")
        ...
        from .quantize import quantize_qwen_and_export
        quantize_qwen_and_export(hf_model_dir,
        ...
        ckpt = Path(quantized_checkpoint_path) / "qwen_tp1_rank0.npz"
        ...
        if ammo_qformat == 'fp8':
            return load_from_fp8_qwen(
                str(ckpt),
                hf_config.num_hidden_layers,
                cfg.mapping,
                fp8_kv_cache=quant_mode.has_fp8_kv_cache())
        else:
            return load_from_awq_qwen(str(ckpt),
                                       hf_config.num_hidden_layers,
                                       hf_config.vocab_size,
                                       cfg.mapping,
                                       dtype=dtype)

    # qwen specific setters, user shall has the chance to change the module attributes after
