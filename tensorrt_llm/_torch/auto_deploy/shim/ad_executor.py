...
def create_draft_kv_cache_manager_maybe(
    ...
    return _create_kv_cache_manager(
        ...
        kv_cache_config=ad_config.kv_cache_config,
        tokens_per_block=ad_config.kv_cache_config.tokens_per_block,
        max_seq_len=ad_config.max_seq_len,
        max_batch_size=ad_config.max_batch_size,
...
def _generate_dummy_request(
    ...
    # check if we have a free slot, state, and slot available
    ...
    # add to slot manager
    slot_manager.prepare_resources(dummy_scheduled_requests)
...
def create_autodeploy_executor(ad_config: LlmArgs, tokenizer: Optional[TokenizerBase] = None):
    ...
    if ad_config.enable_chunked_prefill:
        chunk_unit_size = ad_config.kv_cache_config.tokens_per_block
