...
class CachedSequenceInterface:
    ...
    def __init__(
        ...
        Args:
            ...
            extra_seq_len_for_kv_cache: Extra sequence length to add to max_seq_len for KV cache
                sizing. Accounts for overlap scheduler and speculative decoding headroom.
    ...
    def _update_kv_cache_manager_for_spec_dec(self) -> None:
        ...
        Sets num_extra_kv_tokens, max_draft_len, max_total_draft_tokens on the KVCacheManager
        after creation. These values cannot be passed via the constructor because that would add
        an extra speculative layer that we have already handled.
        """
        if self._spec_config is None:
            return
        self._kv_cache_manager.num_extra_kv_tokens = get_num_extra_kv_tokens(self._spec_config)
        self._kv_cache_manager.max_draft_len = getattr(self._spec_config, "max_draft_len", 0)
        self._kv_cache_manager.max_total_draft_tokens = getattr(
            self._spec_config, "max_total_draft_tokens", 0
        )
    ...
    def _create_kv_cache_manager(self, max_tokens: Optional[int] = None) -> Dict:
        ...
        # 3b. Update KVCacheManager spec-related attributes for speculative decoding
