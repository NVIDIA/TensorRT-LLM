...
class AuxStreamWrapper(nn.Module):
    """Wraps an _aux compute op to give its outputs stable addresses.

    _aux ops (e.g. trtllm_dist_all_gather_aux) run eagerly on the aux CUDA
    stream and produce fresh output tensors each time.  Downstream static
    CUDA graph partitions need these addresses to be stable across replays.

    During capture: allocate a stable buffer, copy output into it, return it.
    During replay: copy output into the same stable buffer, return it.
    """

    def __init__(self, submodule: nn.Module):
        super().__init__()
        self.submodule = submodule
        self._stable_outputs: Dict[int, torch.Tensor] = {}
    ...
    def forward(self, *args, **kwargs) -> Any:
        ...
        # replay
        stable = self._stable_outputs.get(nt)
        if stable is None:
            return result
        stable.copy_(result)
        return stable
