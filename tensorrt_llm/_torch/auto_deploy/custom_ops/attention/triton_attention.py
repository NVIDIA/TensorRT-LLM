...
def _flash_decode_stage1_kernel(
    ...
    for local_page_idx in range(num_pages_this_split):
        ...
        # [HEAD_RATIO_PADDED, PAGE_BLOCK] @ [PAGE_BLOCK, HEAD_DIM] -> [HEAD_RATIO_PADDED, HEAD_DIM]
...
@triton.autotune(
    ...
    key=["HEAD_DIM", "PAGE_SIZE", "PAGE_BLOCK"],
)
@triton.jit
def _paged_context_kernel(
    ...
    full_page_mask = page_offsets < PAGE_SIZE
    ...
    for page_idx in range(first_window_page, num_full_pages):
        ...
        if SLIDING_WINDOW > 0:
            ...
            full_mask_p1 = q_mask[:, None] & sw_mask & full_page_mask[None, :]
            ...
            qk = tl.where(full_page_mask[None, :], qk, float("-inf"))
...
def triton_context_with_custom_mask(
    q: torch.Tensor,
    kv_cache: torch.Tensor,
    qo_indptr: torch.Tensor,
    kv_indptr: torch.Tensor,
...
@torch.library.custom_op("auto_deploy::triton_prepare_metadata", mutates_args=())
def prepare_triton_metadata(
    position_ids: torch.Tensor,
    batch_info_host: torch.Tensor,
    cu_seqlen: torch.Tensor,
    seq_len_with_cache: torch.Tensor,
) -> List[torch.Tensor]:
    """Prepare metadata for Triton attention."""
...
@prepare_triton_metadata.register_fake
def prepare_triton_metadata_fake(
    position_ids: torch.Tensor,
    batch_info_host: torch.Tensor,
    cu_seqlen: torch.Tensor,
    seq_len_with_cache: torch.Tensor,
...
@torch.library.custom_op("auto_deploy::triton_mha_with_cache", mutates_args=("kv_cache",))
def triton_mha_with_cache(
    # Q, K, V
    q: torch.Tensor,
    k: torch.Tensor,
    v: torch.Tensor,
    ...
    """Triton attention with mixed batch support."""
...
@triton_mha_with_cache.register_fake
def triton_mha_with_cache_fake(
    q: torch.Tensor,
    k: torch.Tensor,
    v: torch.Tensor,
    batch_info_host: torch.Tensor,
...
class TritonAttention(AttentionDescriptor):
    """Descriptor for Triton Attention backend.
    ...
    def get_prepare_extra_metadata_info(
        ...
        return (
            torch.ops.auto_deploy.triton_prepare_metadata.default,
            2,
            [],
        )
