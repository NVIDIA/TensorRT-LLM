...
def _torch_mla_generate_with_absorption(
    q_nope: torch.Tensor,  # [num_decode, N, qk_nope_head_dim]
    q_pe: torch.Tensor,  # [num_decode, N, qk_rope_head_dim]
    compressed_kv: torch.Tensor,  # [num_decode, kv_lora_rank]
    kpe: torch.Tensor,  # [num_decode, qk_rope_head_dim]
    ...
    v_head_dim: int,
    out: torch.Tensor,  # [num_decode, N, v_head_dim]
) -> None:
    """Decode-phase MLA attention with weight absorption.
    ...
    Args:
        q_nope: Query non-positional encoding part [num_decode, N, qk_nope_head_dim]
        q_pe: Query positional encoding part [num_decode, N, qk_rope_head_dim]
        compressed_kv: Compressed KV for current tokens [num_decode, kv_lora_rank]
        kpe: Key positional encoding [num_decode, qk_rope_head_dim]
        kv_b_proj_weight: Weight for expanding compressed KV
        mla_cache: MLA cache [max_batch, max_seq, kv_lora_rank + qk_rope_head_dim]
        slot_idx: Cache slot indices for decode sequences [num_decode]
        input_pos: Position indices for decode sequences [num_decode]
        scale: Attention scale factor
        kv_lora_rank: Rank of compressed KV
        num_heads: Number of attention heads
        qk_nope_head_dim: Dimension of non-positional query/key head
        v_head_dim: Dimension of value head
        out: Output tensor [num_decode, N, v_head_dim]
    """
    num_decode = q_nope.shape[0]
    ...
    for i in range(num_decode):
        ...
        mla_cache[cache_idx, pos, :kv_lora_rank] = compressed_kv[i]
        mla_cache[cache_idx, pos, kv_lora_rank:] = kpe[i]
    ...
    for i in range(num_decode):
        ...
        q_nope_i = q_nope[i]  # [N, qk_nope_head_dim]
        q_pe_i = q_pe[i]  # [N, qk_rope_head_dim]
...
def _torch_mla_context_with_expansion(
    q_nope: torch.Tensor,  # [num_prefill_tokens, N, qk_nope_head_dim]
    q_pe: torch.Tensor,  # [num_prefill_tokens, N, qk_rope_head_dim]
    compressed_kv: torch.Tensor,  # [num_prefill_tokens, kv_lora_rank]
    kpe: torch.Tensor,  # [num_prefill_tokens, 1, qk_rope_head_dim]
    kv_b_proj_weight: torch.Tensor,  # [N * (qk_nope_head_dim + v_head_dim), kv_lora_rank]
    mla_cache: torch.Tensor,  # [max_batch, max_seq, kv_lora_rank + qk_rope_head_dim]
    ...
    qk_nope_head_dim: int,
    v_head_dim: int,
    out: torch.Tensor,  # [num_prefill_tokens, N, v_head_dim]
) -> None:
    """Prefill-phase MLA attention with kv_b_proj expansion.
    ...
    Args:
        q_nope: Query non-positional encoding part [num_prefill_tokens, N, qk_nope_head_dim]
        q_pe: Query positional encoding part [num_prefill_tokens, N, qk_rope_head_dim]
        compressed_kv: Compressed KV for prefill tokens [num_prefill_tokens, kv_lora_rank]
        kpe: Key positional encoding [num_prefill_tokens, 1, qk_rope_head_dim]
        kv_b_proj_weight: Weight for expanding compressed KV
        mla_cache: MLA cache [max_batch, max_seq, kv_lora_rank + qk_rope_head_dim]
        input_pos: Starting position in cache for each prefill sequence [num_prefill]
        slot_idx: Cache slot indices for prefill sequences [num_prefill]
        seq_len: Length of each prefill sequence [num_prefill]
        seq_start: Start index in flattened tokens for each sequence [num_prefill]
        scale: Attention scale factor
        kv_lora_rank: Rank of compressed KV
        num_heads: Number of attention heads
        qk_nope_head_dim: Dimension of non-positional query/key head
        v_head_dim: Dimension of value head
        out: Output tensor [num_prefill_tokens, N, v_head_dim]
    """
...
def torch_backend_mla_with_cache(
    ...
    This implementation explicitly splits processing into prefill and decode phases
    based on batch_info_host metadata, supporting mixed batches where both prefill
    and decode sequences are present.
    """
    # Get dimensions
    b, s = q_nope.shape[:2]
    ...
    # Extract batch info for prefill/decode split
    num_prefill, num_prefill_tokens, num_decode = batch_info_host.tolist()
    num_seq = num_prefill + num_decode
    num_total_tokens = num_prefill_tokens + num_decode

    # Slice metadata for active sequences only
    ...
    kpe_flat = kpe.contiguous().view(bs, 1, qk_rope_head_dim)

    # Pre-allocate output tensor
    y = q_nope.new_empty(bs, num_heads, v_head_dim).contiguous()

    # =========================================================================
    # PREFILL: Process tokens [:num_prefill_tokens]
    # =========================================================================
    if num_prefill > 0:
    ...
    # =========================================================================
    # DECODE: Process tokens [num_prefill_tokens:num_total_tokens]
    # =========================================================================
    if num_decode > 0:
        # Prepare decode inputs - squeeze the extra dimension from kpe
        kpe_decode = kpe_flat[num_prefill_tokens:num_total_tokens].squeeze(1)

        _torch_mla_generate_with_absorption(
            q_nope_flat[num_prefill_tokens:num_total_tokens],
            q_pe_flat[num_prefill_tokens:num_total_tokens],
            compressed_kv_flat[num_prefill_tokens:num_total_tokens],
            kpe_decode,
            kv_b_proj_weight,
            mla_cache,
