...
class PTCacheBackend(CacheBackend):
    ...
    def initialize(
        ...
        self._num_pages = sequence_info.num_blocks

        # Verify config matches sequence_info
        if self._config.tokens_per_block != sequence_info.tokens_per_block:
            ad_logger.warning(
                f"PTCacheConfig tokens_per_block ({self._config.tokens_per_block}) "
                f"!= SequenceInfo tokens_per_block ({sequence_info.tokens_per_block}). "
                f"Using SequenceInfo value."
            )
            self._config.tokens_per_block = sequence_info.tokens_per_block
    ...
    def _allocate_metadata_tensors(self, sequence_info: "SequenceInfo") -> None:
        ...
        # Use PTCacheBackend's actual num_blocks (from config), NOT sequence_info.num_blocks
        # since PTCacheBackend allocates its own pool that may be larger
        max_total_pages = (
            self._config.num_pages if self._config.num_pages > 0 else sequence_info.num_blocks
        )
    ...
    def get_unified_cache(self, layer_idx: int) -> torch.Tensor:
        """Get unified KV cache tensor for a layer from the pool.

        Returns the full KV cache with shape:
        [num_blocks, kv_factor=2, num_kv_heads, page_size, head_dim] (HND layout)

        This matches the unified kv_cache format expected by trtllm_mha_with_cache.
        """
        if not self._initialized:
            raise RuntimeError("PTCacheBackend not initialized")

        # Get the pool data for this layer
        pool_data = self._kv_cache_manager.get_primary_pool_data(layer_idx)
        ...
        # Reshape to [num_blocks, kv_factor=2, page_size, num_kv_heads, head_dim] (NHD)
        pool_reshaped = pool_data.view(num_blocks, kv_factor, page_size, num_kv_heads, head_dim)

        # Transpose to [num_blocks, kv_factor=2, num_kv_heads, page_size, head_dim] (HND)
        # This matches the expected kernel layout
        return pool_reshaped.permute(0, 1, 3, 2, 4).contiguous()
