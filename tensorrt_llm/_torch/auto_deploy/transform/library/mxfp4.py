...
def _slice_expert_dim(gm: GraphModule, tensor_node: Node, lo: int, hi: int) -> Node:
    """Return tensor_node[lo:hi, ...] as a new FX node."""
    # (slice(lo, hi), Ellipsis) keeps remaining dims intact
    idx = (slice(int(lo), int(hi)), Ellipsis)
    with gm.graph.inserting_after(tensor_node):
        return gm.graph.call_function(operator.getitem, args=(tensor_node, idx))
...
def _insert_sharded_mxfp4_mlp_ep(
    ...
    args_ep = tuple(args) + (int(local_lo), int(local_hi))
