    ...
    def apply(self, gm: GraphModule, dc: DistConfig, max_num_tokens: int = 0) -> int:
        ...
        for bn in weight_nodes.biases:
            if split_dim == SplitDimension.COLUMN:
                ...
                # update the bias for dim 1 --> in this case only the first rank gets the bias to avoid
                # double counting it. For all other we will delete the bias.
                args = list(self.node.args)
                args[2] = None
                self.node.args = tuple(args)
                # ``bn`` is a WeightNode wrapper; the actual FX node is bn.node and
                # the param_key / submodule references live on the wrapper.
                gm.graph.erase_node(bn.node)
                bias_param_name = bn.node_key.rpartition(".")[-1]
                setattr(bn.submod, bias_param_name, None)
                gm._register_load_state_dict_pre_hook(
                    partial(_load_hook_remove, param_key=bn.node_key)
                )
