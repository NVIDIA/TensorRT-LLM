...
class FuseNVFP4TRTLLMGenMoeConfig(TransformConfig):
    allow_different_input_scales: bool = Field(
        default=False,
        description=(
            "If False, require w1/w3 per-expert input scales to match. "
            "If True, allow mismatch and derive global scales conservatively."
        ),
    )
    reverse_interleaved_input_scales: bool = Field(
        ...
        description=(
            ...
            "before TRTLLM-Gen shuffle+interleave."
        ),
    )
...
class FuseNVFP4TRTLLMGenMoe(BaseTransform):
    @classmethod
    def get_config_class(cls) -> Type[TransformConfig]:
        return FuseNVFP4TRTLLMGenMoeConfig
