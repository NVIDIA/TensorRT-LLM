...
"""Sharding-aware NemotronH model for AutoDeploy IR sharding.

Derived from modeling_nemotron_h.py with explicit sharding hints on all
custom ops.  ``apply_sharding_hints`` reads these hints to apply
deterministic, node-local TP/EP sharding.
...
class NemotronHMamba2Mixer(nn.Module):
    ...
    def __init__(self, config, layer_idx: int):
        ...
        # Fused output sizes for in_proj sharding:
        # in_proj output = [gate | hidden | B | C | dt]
        ...
        # Fused output sizes for conv1d sharding:
        # conv_dim = [hidden | B | C]
    ...
    def torch_forward(self, input_states):
        ...
        # 1. Gated MLP's linear projection (colwise with fused output sizes)
...
class NemotronHMOE(nn.Module):
    ...
    def forward(self, hidden_states: torch.Tensor):
        ...
        # Shared expert first (dispatch order matches exported graph node order)
