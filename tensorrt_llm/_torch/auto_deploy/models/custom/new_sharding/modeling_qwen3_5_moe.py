...
"""Qwen3.5 MoE model with explicit sharding hint ops.

Based on the original modeling_qwen3_5_moe.py. All shardable operations use
...
Shardable custom ops used:
  - torch.ops.auto_deploy.torch_linear_simple  (tp_mode, output_sizes, tp_min_local_shape)
  ...
  - torch.ops.auto_deploy.torch_causal_conv1d  (shardable, output_sizes)
  - torch.ops.auto_deploy.torch_gated_delta_rule (shardable)
  - torch.ops.auto_deploy.torch_moe            (sharded by apply_sharding_hints)

Vision components are copied unchanged (not exported to FX graph).
"""
...
class Qwen3_5MoeGatedDeltaNet(nn.Module):
    """Prefill-only GatedDeltaNet using autodeploy custom ops with sharding hints.

    Sharding strategy (from _process_delta_sharding, unfused 4-proj layout):
      ...
      A_log, dt_bias -> sharded along num_v_heads by _apply_hint_gdn
      norm.weight  -> replicated (constant head_v_dim)
    ...
    def __init__(self, config: Qwen3_5MoeTextConfig, layer_idx: int):
        ...
        # Gated RMSNorm (per head_v_dim -- replicated, not sharded)
        self.norm = Qwen3_5MoeRMSNormGated(self.head_v_dim, eps=config.rms_norm_eps)

        # Fused output sizes for QKV projection sharding
        self._conv_split_sizes = [self.key_dim, self.key_dim, self.value_dim]
    ...
    def forward(self, hidden_states: torch.Tensor) -> torch.Tensor:
        ...
        # Op handles L2 norm, GQA repeat-interleave, and g/beta computation internally
        ...
        # 7. Output projection (rowwise) + all_reduce
...
class Qwen3_5MoeAttention(nn.Module):
    ...
    def forward(
        ...
        # colwise sharding splits by heads correctly -- no output_sizes needed.
        ...
        # K, V projections with tp_min_local_shape for GQA partial replication
...
class Qwen3_5MoeMLP(nn.Module):
    """SwiGLU MLP used for the shared expert, with sharding hints.

    When used as a shared expert inside MoE, all_reduce is deferred to the MoE
    merge point (add_all_reduce=False). When used standalone, add_all_reduce=True.
    """
...
class Qwen3_5MoeSparseMoeBlock(nn.Module):
    """MoE block with expert list implementation and sharding hints.
    ...
    def forward(self, hidden_states: torch.Tensor) -> torch.Tensor:
        ...
        # Routed experts via torch_moe (sharded by apply_sharding_hints)
