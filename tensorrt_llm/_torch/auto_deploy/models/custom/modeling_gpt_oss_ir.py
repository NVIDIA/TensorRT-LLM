...
"""GPT-OSS model with explicit sharding hint ops (sharding-IR variant).

This is a rewrite of ``modeling_gpt_oss.py`` where every attention Linear is
...
The non-IR ``modeling_gpt_oss.py`` remains the default; this IR variant is
opt-in via ``AD_USE_IR_MODELS=1`` (see ``models/custom/__init__.py``).
