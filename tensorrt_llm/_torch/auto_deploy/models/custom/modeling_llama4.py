...
Sharding note: the routed MoE is expressed as stacked-weight ``torch.bmm`` ops
(matching the HF checkpoint format). The ``match_bmm_moe_pattern`` transform
rewrites those into list-based ``torch_moe`` (with ``layer_type="moe"`` by
default) before the sharding stage runs, so sharding hints inside this file
target only the surrounding modules (router stays TP-replicated, the shared
expert MLP is wired with ``add_all_reduce=False, layer_type="moe"`` to defer
its closing all_reduce to the merge point with the routed sum).
"""
...
class Llama4MLP(nn.Module):
    ...
    Used both as the dense MLP in non-MoE layers and as the shared expert
    inside ``Llama4MoE``. When used as a shared expert, ``add_all_reduce=False``
    and ``layer_type="moe"`` so the closing all_reduce is deferred to the merge
    point with the routed expert output.
...
class Llama4MoE(nn.Module):
    ...
    Sharding strategy:
      Router (TP-replicated ``nn.Linear``) stays unsharded.
      Routed experts are sharded by ``apply_sharding_hints`` after
      ``match_bmm_moe_pattern`` rewrites the BMM pattern into ``torch_moe``
      (which carries ``layer_type="moe"`` by default).
      Shared expert MLP is constructed with ``add_all_reduce=False,
      layer_type="moe"`` so its closing all_reduce is deferred and combined
      with the routed partial sum via a single
    ...
    def __init__(self, config: Llama4TextConfig):
        ...
        self.shared_expert = Llama4MLP(config, add_all_reduce=False, layer_type="moe")
