...
class Qwen3_5MoeGatedDeltaNet(nn.Module):
    ...
    def forward(self, hidden_states: torch.Tensor) -> torch.Tensor:
        ...
        # Reshape to per-head: [B, S, num_heads, head_dim] with -1 at head dim
        ...
        # 4. Gated RMSNorm (norm weight is replicated -- constant head_v_dim)
        ...
        # 5. Output projection (rowwise) + all_reduce
...
class Qwen3_5MoeAttention(nn.Module):
    ...
    def forward(
        ...
        # Q projection with gate (interleaved per-head [q_h0,g_h0,...] -- plain colwise is correct)
        ...
        # K, V projections with tp_min_local_shape for GQA
        ...
        # Per-head Q/K norms (norm on last dim = head_dim, no sharding)
...
class Qwen3_5MoeMLP(nn.Module):
    """SwiGLU MLP used for the shared expert and standalone MLP layers."""
