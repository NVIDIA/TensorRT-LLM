...
from .common import _estimate_mamba_hybrid_cache_cost as _estimate_common_mamba_cache_cost

GB = 1 << 30


def _estimate_mamba_hybrid_cache_cost(model_config, mapping, **kwargs):
    """Keep V1's original model-specific estimate outside the V2 base."""
    from tensorrt_llm._torch.modules.qwen4_exp.cache_manager import (
        _qwen4_exp_ple_state_bytes_per_rank,
    )
    ...
    if not kwargs.get("is_draft", False):
        ...
        extra_bytes = _qwen4_exp_ple_state_bytes_per_rank(
            model_config,
            params,
            mapping,
            spec_config=kwargs.get("spec_config"),
    ...
    return _estimate_common_mamba_cache_cost(
        model_config, mapping, extra_state_bytes_per_rank=extra_bytes, **kwargs
    )
...
class PythonMambaCacheManager(BaseResourceManager):
    ...
    def __init__(
        ...
        if speculative_num_draft_tokens is not None:
            ...
            if self._use_kda_replay_update:
                ...
                spec_kwargs["kda_v_cache"] = torch.zeros(
                    num_local_layers,
                    max_batch_size,
                    M,
                    section_dim,
                ...
                ssm_spec_cache = [
                    ...
                    spec_kwargs["kda_conv_k"],
                    spec_kwargs["kda_conv_v"],
                    spec_kwargs["kda_qkg_cache"],
                    spec_kwargs["kda_v_cache"],
    ...
    def seed_kda_replay_caches_for_disagg_gen(self, request_ids: List[int]) -> None:
        ...
        """
        if not (
            self._use_kda_replay_update and isinstance(self.mamba_cache, self.SpeculativeState)
        ):
            return
        blocks = [
        ...
        idx = torch.tensor(sorted(set(blocks)), dtype=torch.long, device=conv.device)
        d = conv.shape[2] // 3
        ...
        for buf in (
            self.mamba_cache.kda_qkg_cache,
    ...
    def shutdown(self):
        ...
        if isinstance(self.mamba_cache, self.SpeculativeState):
            self.mamba_cache = self.SpeculativeState(
                conv=empty,
                temporal=empty,
                intermediate_conv_window=_drop(self.mamba_cache.intermediate_conv_window),
    ...
    def on_state_transfer_complete(self, request_ids: list[int]) -> None:
        self.seed_kda_replay_caches_for_disagg_gen(request_ids)
...
class MambaCacheManager(BaseResourceManager, BaseMambaCacheManager):
    ...
    def mamba_layer_cache(
        self, layer_idx: int
    ) -> Union[PythonMambaCacheManager.State, PythonMambaCacheManager.SpeculativeState, None]:
...
class _LegacyMambaHybridCacheManager(MambaHybridCacheManager):
    ...
    def _setup_mtp_intermediate_states(self, spec_config, max_batch_size: int) -> None:
        ...
        if not self._use_replay_state_update:
            self.intermediate_ssm_states = torch.zeros(
                size=[self.local_num_mamba_layers, max_batch_size, tokens_per_gen_step]
    ...
    def _reset_context_mamba_slots(self, num_contexts: int) -> None:
        ...
        new_seeds = [
            _compute_deterministic_mamba_seed(counter, slot, rank_offset) for slot in host_slots
        ]
    ...
    def mamba_layer_cache(
        ...
        if self.spec_config is not None:
            ...
            if self._use_replay_state_update:
                ...
                spec_kwargs["prev_num_accepted_tokens"] = self.prev_num_accepted_tokens
            else:
                spec_kwargs["intermediate_ssm"] = self.intermediate_ssm_states[layer_offset]
...
class CppMambaHybridCacheManager(KVCacheManager, _LegacyMambaHybridCacheManager):
...
__all__ = [
    ...
    "warn_legacy_mamba_cache_manager",
]
