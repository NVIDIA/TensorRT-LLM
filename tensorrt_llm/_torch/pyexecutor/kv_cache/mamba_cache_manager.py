...
class MambaHybridCacheManagerV2(KVCacheManagerV2, MambaHybridCacheManager):
    ...
    def get_needed_resource_to_completion(self, request: LlmRequest) -> int:
        # Report state slots, not the base class's KV bytes: the count above is
        # a slot count, and the two must stay in one unit to be comparable. A
        # request occupies exactly one recurrent-state slot for its lifetime.
        return 1
