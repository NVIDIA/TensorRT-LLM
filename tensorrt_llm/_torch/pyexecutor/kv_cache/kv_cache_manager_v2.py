...
def scratch_segment_bounds(
    ...
    """Clamp a scratch range to one request's segment and check its slot budget.

    ``[beg, end)`` may fall partly or entirely outside the segment, and getting
    that wrong silently shifts the wrong blocks rather than raising. Shared by
    the per-layer and the layer-independent page-table builders so the two can
    never disagree about which blocks are scratch.
    """
    lo, hi = max(beg, 0), min(end, seg_len)
    if hi <= lo:
...
def apply_scratch_to_block_segment(
    ...
    """
    lo, hi = scratch_segment_bounds(
        seg.shape[0], beg, end, scratch_pages_per_block, scale, len(slot_ids)
    )
...
def compute_scratch_shared_parts(
    first: int,
    last: int,
    scratch_pages_per_block: int,
    scale: int,
    ...
    """Layer-independent ``(base, rot)`` for scratch positions ``[first, last)``.

    Splits :func:`compute_scratch_flat_page_indices` at the only point where a
    layer enters the arithmetic. ``base`` is ``(slot * scale) // div_factor``
    and ``rot`` is ``total % scale``; neither mentions a layer, and a layer's
    own index is recovered as::

        index_L = base + ((rot + layer_offset_L) % scale) // div_factor

    The identity holds exactly because ``scale`` is a multiple of ``kv_factor``
    (checked by ``_check_per_layer_kv_adjacency``), so ``slot * scale`` divides
    through and the rotation can be added afterwards.
    """
    count = last - first
    if count <= 0:
    ...
    total = (np.arange(first, last, dtype=np.int64)) * scratch_pages_per_block
    slots = np.asarray(slot_ids, dtype=np.int64)[total // scale]
    base = (slots * scale // div_factor).astype(np.int32)
    rot = (total % scale).astype(np.int32)
    return base, rot


def apply_scratch_to_shared_segment(
    base_seg: "np.ndarray",
    rot_seg: "np.ndarray",
    beg: int,
    end: int,
    ...
    """Rewrite one request's layer-independent page-table parts in place.

    Blocks inside ``[beg, end)`` are replaced by their scratch slot's base and
    rotation; blocks outside it keep the base index they already carry and a
    zero rotation, which makes the per-layer step ``+ layer_offset // kv_factor``
    for them -- the same term a non-scratch block gets under PER_LAYER
    addressing.

    Returns the clamped ``[lo, hi)`` the caller needs to mark addressable: a
    scratch block arrives as ``BAD_PAGE_INDEX`` (it owns no page) and only
    acquires a real address here.
    """
    lo, hi = scratch_segment_bounds(
        base_seg.shape[0], beg, end, scratch_pages_per_block, scale, len(slot_ids)
    )
    if hi <= lo:
        return lo, hi
    base_seg[lo:hi], rot_seg[lo:hi] = compute_scratch_shared_parts(
        lo - beg,
        hi - beg,
        scratch_pages_per_block,
        scale,
...
class KVCacheManagerV2(BaseResourceManager):
    ...
    def __init__(
        ...
        **kwargs,
        ...
        # PER_LAYER addressing reaches the backend through two extension points
        # this class owns: get_buffers(index_mode=...) and the scratch guard in
        # get_block_ids_per_seq. Subclasses that predate them -- QSA, MiniMax-M3
        # and DSA all override one or both -- would silently serve SHARED
        # buffers or map a scratch block to page 0. None of them declares a
        # sliding-window lifecycle today, so the narrowing above should already
        # have turned scratch reuse off; say so out loud rather than relying on
        # that argument staying true.
        if self.enable_swa_scratch_reuse and type(self) is not KVCacheManagerV2:
            logger.warning(
                "%s inherits SWA scratch reuse but may override get_buffers() "
                "or get_block_ids_per_seq(); PER_LAYER page indices are only "
                "correct if both honour it. Set "
                "kv_cache_config.enable_swa_scratch_reuse=False if this "
                "manager has not been verified against it.",
                type(self).__name__,
            )
    ...
    def get_layer_page_index_offset(self, layer_idx: int) -> int:
        """This layer's sub-page offset inside its pool group's page.

        Under PER_LAYER addressing a block's page index is
        ``base * scale + layer_offset``, so this is the *only* per-layer term.
        Exposed so a backend can build one page table per ``(pool, scale)``
        group and derive every layer's from it, instead of one table per layer.
        """
        return int(
            self.impl.get_page_index_converter(self.layer_offsets[layer_idx], Role.KEY).layer_offset
        )

    def get_batch_cache_indices_flat_shared(
        self,
        request_ids: List[int],
        num_blocks: List[int],
        layer_idx: int,
    ) -> Tuple[torch.Tensor, Optional[torch.Tensor], torch.Tensor]:
        """Layer-independent decomposition of the PER_LAYER flat page table.

        Every layer sharing ``layer_idx``'s ``(pool, page-index scale)`` group
        has the same table up to its own ``layer_offset``::

            index_L[i] = BAD_PAGE_INDEX                        if valid[i] == 0
                       = base[i] + ((rot[i] + off_L) % scale) // kv_factor

        which reproduces ``get_batch_cache_indices_flat(..., L)`` exactly --
        ``TestGemma4SwaScratchPageIndices.test_derived_indices_match_the_per_layer_reference``
        in ``tests/unittest/_torch/modeling/test_modeling_gemma4.py`` locks that
        equivalence.

        ``rot`` is ``None`` when no request in the batch holds scratch blocks.
        The per-layer step then degenerates to ``base + off_L // kv_factor`` on
        the valid entries, which is a single fused add -- and since a generation
        request can never own scratch blocks (its input range is one block at
        the head of the sequence, never stale), that is the only path a
        decode-only step ever takes.

        Splitting the table this way is the point: the host gather, the scale
        multiply and the scratch slot lookup run once per group instead of once
        per layer, so adding layers stops multiplying host work in ``prepare()``.
        Returns CPU int32 tensors (pinned when supported) ready for async H2D.
        """
        pool_id = self.layer_to_pool_mapping_dict[self.layer_offsets[layer_idx]]
        scale = self.get_layer_page_index_scale(layer_idx)
        div_factor = self.kv_factor
        ...
        base_tensor = torch.empty(total_blocks, dtype=torch.int32, pin_memory=prefer_pinned())
        base = base_tensor.numpy()
        offset = 0
        for req_id, n in zip(request_ids, num_blocks):
            base[offset : offset + n] = np.frombuffer(
                self.kv_cache_map[req_id].get_base_page_indices(pool_id),
                dtype=np.int32,
                count=n,
            )
        ...
        # BAD_PAGE_INDEX entries (e.g. evicted out-of-window SWA blocks) must
        # survive the per-layer step untouched, and the caller applies that step
        # on device with no room for a branch. Hand it the mask instead: the
        # per-layer term is scaled by `valid`, so an invalid entry keeps its
        # sentinel and a valid one gets its layer offset.
        valid_tensor = torch.empty(total_blocks, dtype=torch.int32, pin_memory=prefer_pinned())
        valid = valid_tensor.numpy()
        addressable = base != BAD_PAGE_INDEX
        valid[:] = addressable
        np.copyto(base, base * scale // div_factor, where=addressable)

        rot_tensor = None
        if self.enable_swa_scratch_reuse:
            # Checked lazily, exactly as the per-layer builder does: only a flat
            # page table needs K/V adjacency, so backends that consume
            # copy_batch_block_offsets are never subjected to this restriction.
            self._validate_per_layer_kv_adjacency()
            converter = self.impl.get_page_index_converter(self.layer_offsets[layer_idx], Role.KEY)
            scratch_pages = int(converter.scratch_pages_per_block)
            rot = None
            ...
            for req_id, n in zip(request_ids, num_blocks):
                ...
                if desc is None:
                    ...
                    continue
                if rot is None:
                    # Allocated on first use: a batch with no scratch blocks
                    # (every decode-only step, and any prefill chunk shorter
                    # than the window) hands the caller None and takes the
                    # cheap per-layer path.
                    rot_tensor = torch.zeros(
                        total_blocks, dtype=torch.int32, pin_memory=prefer_pinned()
                    )
                    rot = rot_tensor.numpy()
                self._log_scratch_desc(req_id, pool_id, desc, path="flashinfer_shared")
                try:
                    lo, hi = apply_scratch_to_shared_segment(
                        base[offset : offset + n],
                        rot[offset : offset + n],
                        int(desc.range.beg),
                        int(desc.range.end),
                        scratch_pages,
                    ...
                    # Re-raise with the identity the helper cannot know, so a bad
                    # descriptor names the request and pool rather than surfacing
                    # as a bare arithmetic error from inside the hot path.
                    raise RuntimeError(
                        f"SWA scratch descriptor for request {req_id} pool {pool_id} is "
                        f"inconsistent: {exc}."
                    ) from exc
                # A scratch block has no base page, so it arrived as
                # BAD_PAGE_INDEX and was skipped by the scale multiply above.
                # It has a real address now; mark it addressable.
                valid[offset + lo : offset + hi] = 1
                offset += n

        return base_tensor, rot_tensor, valid_tensor
    ...
    def get_max_resource_count(self) -> int:
        # Capacity is reported in bytes, not blocks. Pool groups can differ in
        # slot size, so their block counts do not add up to a quantity that one
        # tokens_per_block converts back into servable tokens: SWA scratch reuse
        # alone gives the windowed group more, smaller slots for the same bytes.
        # Pairs with get_needed_resource_to_completion, which is also in bytes.
        return int(self.impl.get_quota(GPU_LEVEL))

    def _get_cache_bytes_for_tokens(self, total_tokens: int, *, context: bool) -> int:
        """KV bytes a request of ``total_tokens`` tokens occupies.

        Uses the same two-term model as the startup sizing path
        (``_get_max_tokens_from_quota_impl``): a per-token size that every token
        pays, plus a fixed per-request size for the windows that are retained
        whole rather than grown token by token.
        """
        layer_sizes, attention_windows = self._get_runtime_cache_size_layer_components()
        ...
        swa_size_per_token, swa_size_per_request = _estimate_swa_cache_size(
            ...
            self.tokens_per_block,
            context=context,
            # A generation request holds no stale SWA block for a scratch slot to
            # take over, so only the context term follows the runtime flag.
            scratch=context and self.enable_swa_scratch_reuse,
        )
        return int(
            total_tokens * (full_attn_size_per_token + swa_size_per_token) + swa_size_per_request
        )

    def _get_context_bytes(self, request: LlmRequest) -> int:
        total_tokens = max(0, request.prompt_len) + self.num_extra_kv_tokens
        return self._get_cache_bytes_for_tokens(total_tokens, context=True)

    def _get_generation_bytes(self, request: LlmRequest) -> int:
        total_tokens = (
            max(0, request.prompt_len) + max(0, request.max_new_tokens) + self.num_extra_kv_tokens
        )
    ...
    def get_needed_resource_to_completion(self, request: LlmRequest) -> int:
        ...
        if request.is_context_init_state or request.state == LlmRequestState.CONTEXT_INIT:
