...
class KVCacheManagerV2(BaseResourceManager):
    ...
    def prepare_context(self, req: LlmRequest) -> bool:
        ...
        self.apply_connector_prefix(req)
        return True
    ...
    def apply_connector_prefix(self, req: LlmRequest) -> None:
        """Phases 1 and 2, for a request whose cache is prepared and resident.

        Must run after the caller has set the context cursor from the local
        reuse match ``prepare_context_cache`` returned: phase 2 advances that
        cursor past the match, and the local-reuse rewind would undo it. Phase
        2 also needs the cache ACTIVE, which the resume inside
        ``prepare_context_cache`` has already done.

        Only the first context chunk: later chunks advance the cursor through
        prefill, and re-reserving would rewind it to the offer end.
        """
