...
from tensorrt_llm._utils import (confidential_compute_enabled, is_sm_100f,
...
from ..modules.qwen4_exp.ple_cache import build_qwen4_exp_ple_cache_extension
...
def _create_kv_cache_manager(
    ...
    if is_kimi_linear(config):
        ...
        kimi_extra_kwargs = {}
        state_update_strategy = build_kda_state_update_strategy(
            spec_config=spec_config,
            manager_cls=kv_cache_manager_cls,
        )
        if state_update_strategy is not None:
            if issubclass(kv_cache_manager_cls, MambaHybridCacheManagerV2):
                kimi_extra_kwargs[
                    "state_update_strategy"] = state_update_strategy
            else:
                # Legacy managers keep their established constructor surface;
                # the feature owner still decides whether replay is eligible.
                kimi_extra_kwargs["kda_replay_num_spec"] = (
                    state_update_strategy.num_speculative_tokens)
        ...
        state_update_strategy = build_mamba2_state_update_strategy(
            spec_config=spec_config,
        ...
        if issubclass(kv_cache_manager_cls, MambaHybridCacheManagerV2):
            mamba_manager_extra_kwargs["state_update_strategy"] = (
        ...
        from ..modules.fla.cache_manager import build_gdn_state_update_strategy

        state_update_strategy = build_gdn_state_update_strategy(
            spec_config=spec_config,
            ssm_cache_dtype=mamba_params.mamba_ssm_cache_dtype,
            manager_cls=kv_cache_manager_cls,
        )
        use_replay = state_update_strategy is not None

        mamba_manager_extra_kwargs = dict(manager_extra_kwargs)
        mamba_manager_extra_kwargs.update(
            _mamba_conv_layout_kwargs(kv_cache_manager_cls, "qwen3_next"))
        if (state_update_strategy is not None and issubclass(
                kv_cache_manager_cls, MambaHybridCacheManagerV2)):
            mamba_manager_extra_kwargs[
                "state_update_strategy"] = state_update_strategy
        ...
        if is_qwen4_exp(config) and issubclass(kv_cache_manager_cls,
                                               MambaHybridCacheManagerV2):
            ple_cache_extension = build_qwen4_exp_ple_cache_extension(
                config,
                total_layers=len(mamba_layer_mask),
                is_draft=is_draft,
            )
            if ple_cache_extension is not None:
                mamba_manager_extra_kwargs["aux_cache_extensions"] = (
                    ple_cache_extension, )
        kv_cache_manager = kv_cache_manager_cls(
            # mamba cache parameters
            mamba_params.state_size,
