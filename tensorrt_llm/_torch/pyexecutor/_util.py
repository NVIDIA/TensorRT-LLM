...
from .sampler import (CppSampler, EarlyStopSampler, TorchSampler,
                      TorchStarAttentionSampler)
...
def estimate_max_kv_cache_tokens(py_executor: PyExecutor,
    ...
    if py_executor.dist.mapping.rank == 0:
        # NOTE: CppSampler requires origin_seq_len - 1 for requests.
        #       Spec decoders with overlap require origin_seq_len.
        seq_len = origin_seq_len - 1 if type(
            py_executor.decoder) == CppSampler else origin_seq_len
...
def create_py_executor_instance(dist,
    ...
    sampler = instantiate_sampler(model_engine, executor_config, spec_decoder,
...
def instantiate_sampler(model_engine, executor_config, spec_decoder,
                        pytorch_backend_config, mapping):
    if mapping.cp_config.get('cp_type') == 'star_attention':
        ...
        sampler = CppSampler(executor_config, model_engine.model,
