...
class PyExecutor:
            ...
            while not got_finish_signal or len(self.active_requests) > 0:
                ...
                if self.enable_iter_perf_stats and previous_batch is not None:
                    self._process_iter_stats(finished_requests,
                                             previous_scheduled_batch,
                                             previous_batch.iter_start_time,
                                             previous_batch.iter_stats)
    ...
    def _process_previous_batch(self):
        ...
        if self.enable_iter_perf_stats:
            self._process_iter_stats(finished_requests, scheduled_requests,
                                     self.previous_batch.iter_start_time,
                                     self.previous_batch.iter_stats)
