...
class PyExecutor:
    ...
    def __init__(
        ...
        self._reset_overlap_local_scratch_after_control_action = False
    ...
    def _executor_loop_overlap(self):
        ...
        with self._profiler() as profile_step, self.hang_detector:
            ...
            while True:
                ...
                if self._reset_overlap_local_scratch_after_control_action:
                    target_inputs = None
                    previous_tensors_device = None
                    self._reset_overlap_local_scratch_after_control_action = False
    ...
    def _debug_preempt_request_snapshot(self, req: LlmRequest) -> Dict:
        """Collect request state for preempt debugging without perturbing flow."""
        snapshot = {
            "request_id": getattr(req, "py_request_id", getattr(req, "request_id", None)),
            "state": str(getattr(req, "state", None)),
            "py_decoding_iter": getattr(req, "py_decoding_iter", None),
            "decoding_iter": getattr(req, "decoding_iter", None),
            "prompt_len": getattr(req, "prompt_len", None),
            "py_prompt_len": getattr(req, "py_prompt_len", None),
            "orig_prompt_len": getattr(req, "orig_prompt_len", None),
            "py_orig_prompt_len": getattr(req, "py_orig_prompt_len", None),
            "max_new_tokens": getattr(req, "max_new_tokens", None),
            "py_max_new_tokens": getattr(req, "py_max_new_tokens", None),
            "py_seq_slot": getattr(req, "py_seq_slot", None),
            "py_num_accepted_draft_tokens": getattr(
                req, "py_num_accepted_draft_tokens", None),
            "py_rewind_len": getattr(req, "py_rewind_len", None),
            "py_draft_tokens_len": len(getattr(req, "py_draft_tokens", []) or []),
        }

        try:
            tokens = req.get_tokens(0)
            snapshot["num_tokens"] = len(tokens)
            snapshot["last_tokens"] = list(tokens[-8:])
        except Exception as e:
            snapshot["tokens_error"] = repr(e)

        try:
            snapshot["get_num_tokens"] = req.get_num_tokens(0)
        except Exception as e:
            snapshot["get_num_tokens_error"] = repr(e)

        try:
            log_probs = req.py_result.log_probs
            if log_probs is None:
                snapshot["log_probs_len"] = None
                snapshot["last_log_probs"] = None
            else:
                beam0 = log_probs[0] if len(log_probs) > 0 else []
                snapshot["log_probs_len"] = len(beam0)
                last_log_probs = []
                for token_logprob in beam0[-8:]:
                    try:
                        first_token, first_lp = next(iter(token_logprob.items()))
                        last_log_probs.append({
                            "token": int(first_token),
                            "logprob": float(first_lp.logprob),
                            "rank": getattr(first_lp, "rank", None),
                        })
                    except Exception as e:
                        last_log_probs.append({"error": repr(e)})
                snapshot["last_log_probs"] = last_log_probs
        except Exception as e:
            snapshot["log_probs_error"] = repr(e)

        try:
            num_tokens = snapshot.get("num_tokens")
            orig_prompt_len = snapshot.get("py_orig_prompt_len")
            log_probs_len = snapshot.get("log_probs_len")
            if (num_tokens is not None and orig_prompt_len is not None
                    and log_probs_len is not None):
                snapshot["logprob_token_delta"] = (
                    log_probs_len - (num_tokens - orig_prompt_len))
        except Exception as e:
            snapshot["invariant_error"] = repr(e)

        return snapshot

    def _debug_preempt_state(self, tag: str, requests: Optional[List[LlmRequest]] = None):
        log_path = os.environ.get("NRL_TRTLLM_PREEMPT_DEBUG_LOG")
        if not log_path:
            return
        ...
        try:
            ...
            if self.previous_batch is not None:
                scheduled_requests = self.previous_batch.scheduled_requests
                sample_state_obj = self.previous_batch.sample_state
                scheduled = {
                    "context_ids": [
                        getattr(req, "py_request_id", getattr(req, "request_id", None))
                        for req in scheduled_requests.context_requests
                    ],
                    "context_last_chunk_ids": [
                        getattr(req, "py_request_id", getattr(req, "request_id", None))
                        for req in scheduled_requests.context_requests_last_chunk
                    ],
                    "generation_ids": [
                ...
                sample_state = {
                    "request_ids": [
                        getattr(req, "py_request_id", getattr(req, "request_id", None))
                        for req in getattr(sample_state_obj, "requests", [])
                    ],
                    "host_new_tokens_shape": (
                        list(sample_state_obj.host.new_tokens.shape)
                        if getattr(sample_state_obj, "host", None) is not None
                        and getattr(sample_state_obj.host, "new_tokens", None) is not None
                        else None),
                    "device_new_tokens_shape": (
                        list(sample_state_obj.device.new_tokens.shape)
                        if getattr(sample_state_obj, "device", None) is not None
                        and getattr(sample_state_obj.device, "new_tokens", None) is not None
                        else None),
                    "runtime_draft_len": getattr(sample_state_obj, "runtime_draft_len", None),
                }

            selected_requests = requests if requests is not None else self.active_requests
            payload = {
                "ts": time.time(),
                "tag": tag,
                "pid": os.getpid(),
                "rank": getattr(self.dist, "rank", None),
                "pp_rank": getattr(self.dist, "pp_rank", None),
                "tp_rank": getattr(self.dist, "tp_rank", None),
                "iter_counter": getattr(self, "iter_counter", None),
                ...
                "sample_state": sample_state,
                "requests": [
                    self._debug_preempt_request_snapshot(req)
            ...
            logger.warning(f"Failed to write preempt debug state {tag}: {e}")

    def _drain_overlap_scheduler_previous_batch(self) -> None:
        """Commit overlap scheduler pending state to a non-overlap boundary.

        With the overlap scheduler, the control-action hook can run after an
        iteration's sampler work is enqueued but before that sample state has
        been applied to ``LlmRequest`` objects.  Non-overlap scheduling does
        not expose this intermediate state: by the next scheduler boundary the
        sampled token/logprob has already been written back, responses have
        been handled, and resource-manager accounting has been updated.

        This helper drains ``self.previous_batch`` through the same sequence
        used by the normal overlap loop so callers that mutate request state
        (for example, preempting generation requests back to ``CONTEXT_INIT``)
        start from that stable non-overlap-equivalent boundary.
        """
        ...
        self._debug_preempt_state("previous_batch_after_sampler_sync")

        # First write the sampled token/logprob into the request.  This step is
        # intentionally outside _process_previous_batch() in the normal overlap
        # loop, so it must be performed explicitly before request state is
        # mutated by a caller such as preempt_all_inflight_requests().
        self._update_requests(self.previous_batch.sample_state)
        self._debug_preempt_state("previous_batch_after_update_requests")
        ...
        if self.drafter is not None and self.use_spec_decode:
            # Match the normal overlap loop's cleanup of draft resources
            # associated with the previous target batch.
            self.drafter.cleanup_previous_draft_resources()

        self._process_previous_batch()
        self.perf_manager.compute_batch_gpu_times(
            self.previous_batch.scheduled_requests.all_requests())
        self._debug_preempt_state("previous_batch_after_process_previous_batch")

        # None out so the main overlap loop does not re-apply this batch's
        # sample state after the control action returns.
        self.previous_batch = None

    def _assert_all_active_requests_context_init_after_preempt(self) -> None:
        non_context_init = [
            req for req in self.active_requests
        ...
        if non_context_init:
            examples = ", ".join(
                ...
                for req in non_context_init[:5])
            raise AssertionError(
                "preempt_all_inflight_requests must leave every active "
                "request in CONTEXT_INIT state; found "
                f"{len(non_context_init)} non-context request(s): {examples}")

    def _reset_active_request_overlap_completion_flags_after_preempt(self) -> None:
        for req in self.active_requests:
            req.set_exclude_last_generation_logits(
                self._should_exclude_last_generation_logits())

    def preempt_all_inflight_requests(self) -> int:
        """Preempt every in-flight request and force it back to ``CONTEXT_INIT``.

        Mirrors the existing memory-pressure evict path
        (``_terminate_requests`` + ``_pause_requests``) but applies it to all
        currently-decoding requests at once. Each affected request:

        * releases its KV cache blocks (and any other resource-manager state)
          via ``ResourceManager.free_resources``,
        * has its already-generated tokens promoted to the prompt (capped at
          ``self.max_input_len``) and its state flipped to
          ``LlmRequestState.CONTEXT_INIT`` via ``LlmRequest.pause``,
        * remains in ``self.active_requests`` so the next scheduler iteration
          picks it up as a context request and re-prefills it under the
          current model weights.

        The prefix-reuse pool is also cleared so requests arriving after the
        pause cannot reuse block hashes computed under stale weights.

        Intended for RLHF in-flight weight updates — analogous to vLLM's
        ``reset_prefix_cache(reset_running_requests=True)``.

        Returns the number of requests that were paused.

        Raises:
            RuntimeError: if any in-flight request was sampled with
                ``beam_width > 1``. ``LlmRequest.pause`` is destructive in
                that case (it discards generated tokens), which is not what
                callers expect for in-flight refit.
        """
        self._debug_preempt_state("entry")
        # Drain the overlap scheduler's pending sample state from the previous
        # iter BEFORE mutating request state. With overlap_scheduler enabled,
        # ``_handle_control_request`` (where we get called) fires inside
        # ``_prepare_and_schedule_batch`` — BEFORE ``_update_requests`` writes
        # iter N's sampled token into ``req.mTokens`` (which happens later at
        # line ~2829). If we ``pause()`` here without draining, the previous
        # iter's last token gets appended to mTokens AFTER ``pause()``
        # resized it, leaving the token at a position that's inconsistent
        # with mPromptLen. RL trajectories then see token-position drift and
        # token_mult_prob_error explodes on recompute. The standard
        # memory-pressure evict path doesn't hit this race because it fires
        # at ~line 2844, AFTER ``_update_requests``.
        self._drain_overlap_scheduler_previous_batch()
        if not self.disable_overlap_scheduler:
            self._reset_overlap_local_scratch_after_control_action = True

        # Reset the overlap scheduler's cross-iter scratch state on both the
        # executor and the model engine. Preempt restarts the pipeline from a
        # cold-start posture (all reqs CONTEXT_INIT, no in-flight sample), so
        # any "previous iter" bookkeeping is no longer valid. Without this,
        # the next forward step can pick up stale request IDs / position
        # indices / accepted-token counts captured before the preempt.
        self.has_previous_draft_tokens = False
        me = getattr(self, "model_engine", None)
        if me is not None:
            me.previous_request_ids = []
            me.has_previous_device_draft = False
            if hasattr(me, "previous_accepted_tokens_cuda"):
                me.previous_accepted_tokens_cuda.zero_()
            # Spec-decode-only buffers (only allocated when is_spec_decode).
            for buf_name in (
                "previous_pos_indices_cuda",
                "previous_pos_id_offsets_cuda",
                "previous_kv_lens_offsets_cuda",
            ):
                buf = getattr(me, buf_name, None)
                if buf is not None:
                    buf.zero_()
        ...
        self._debug_preempt_state("to_pause_selected", to_pause)

        if not to_pause:
            self._debug_preempt_state("return_no_to_pause", to_pause)
            self._reset_active_request_overlap_completion_flags_after_preempt()
            self._assert_all_active_requests_context_init_after_preempt()
            return 0

        # ``LlmRequest::pause`` truncates back to the prompt for beam search
        # (see cpp/include/tensorrt_llm/batch_manager/llmRequest.h). That
        # would silently drop already-generated tokens and corrupt RL
        # rollouts, so fail loudly instead.
        bad = [req for req in to_pause if req.sampling_config.beam_width > 1]
        if bad:
            raise RuntimeError(
                f"pause_all_active_requests requires beam_width=1; found "
                f"{len(bad)} in-flight request(s) with beam_width > 1 (e.g. "
                f"request_id={bad[0].request_id}, "
                f"beam_width={bad[0].sampling_config.beam_width})."
            )

        # Free KV cache + any other per-request resource manager state. This
        # mirrors the singular ``_terminate_request`` path used by the
        # memory-pressure evict, except we don't go through the disagg PP
        # termination handler — paused requests aren't actually terminating.
        self._debug_preempt_state("before_free_resources", to_pause)
        for req in to_pause:
            self.resource_manager.free_resources(req)
        self._debug_preempt_state("after_free_resources", to_pause)

        # Flip LlmRequest state to CONTEXT_INIT and promote generated tokens
        # into the prompt (capped by ``max_input_len``).
        self._pause_requests(to_pause)
        self._reset_active_request_overlap_completion_flags_after_preempt()
        self._debug_preempt_state("after_pause_requests", to_pause)
        self._assert_all_active_requests_context_init_after_preempt()
