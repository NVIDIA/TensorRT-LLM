...
class PyExecutor:
    ...
    def _pad_empty_attention_dp_batch(
        ...
        *active* request" -- and it necessarily runs *before* `_schedule()`, so
        ...
        That deadlocks the deployment, silently and permanently:

          1. the empty rank vetoes `can_queue` for the whole fleet, so peers
             never execute the context chunks they *did* schedule
             (`_update_request_states` -- the only caller of
             `move_to_next_context_chunk()` -- lives under `if can_queue:`);
          2. no chunk completes, so no KV cache is released anywhere;
          3. with `can_queue_this_rank` also False the empty rank clears
             `self.previous_batch`, which stops `_send_kv_async` and with it
             `_check_disagg_ctx_cache_transfer_status()` -- the only place a
             completed disaggregated context transfer is reaped -- so even the
             blocks it is waiting on are never freed;
