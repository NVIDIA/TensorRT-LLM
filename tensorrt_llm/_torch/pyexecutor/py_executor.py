...
class PyExecutor:
    ...
    def _broadcast_new_requests(self, new_requests):
        ...
        if self.dist.is_first_pp_rank and len(new_requests) > 0:
            ...
            payloads = new_requests if not has_logit_processors else (new_requests, logit_processors_dict)
            buf = np.array(bytearray(dill.dumps(payloads)))
            buf_size = len(buf)
        else:
            buf, buf_size = None, 0
        metadata_arr = np.array([len(new_requests), buf_size, has_logit_processors])
    ...
    def _fetch_new_requests(self):
        ...
        if self.dist.rank == 0 and not (self.dist.has_pp or self.dist.has_tp):
