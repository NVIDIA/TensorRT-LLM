...
class PyExecutor:
    ...
    def _process_iter_stats(self, finished_requests: list[LlmRequest],
        ...
        self._append_iter_stats(
            self._update_iter_stats(
                ...
                batch_state.sampler_state.scheduled_requests))
            ...
            while not got_finish_signal or len(self.active_requests) > 0:
                ...
                if not can_queue:
                    ...
                    if self.dist.is_last_pp_rank:
                        sampler_state = self._forward_step_last_pp(
                            scheduled_batch, microbatch_id)
                    else:
                        sampler_state = self._forward_step_inter_pp(
                            scheduled_batch)
                ...
                if previous_batch is not None:
                    ...
                    self._update_requests(previous_batch.sampler_state)
                    self._handle_cancelled_requests()
                    finished_requests = self._handle_responses()
                    previous_scheduled_batch = previous_batch.sampler_state.scheduled_requests
                ...
                if not can_queue:
                    ...
                    if not self.dist.is_last_pp_rank:
                        ...
                        with torch.cuda.nvtx.range("_forward_step_last_pp"):
                            ...
                            sampler_state = self._sample_async(
                    ...
                    if num_dummy_request > 0:
                        self._finish_dummy_request(
                            sampler_state.scheduled_requests)
                    self.micro_batches[microbatch_id] = batch_state
                ...
                if previous_batch is not None:
                    if not self.dist.is_last_pp_rank:
                        ...
                        self.dist.recv_tensor_list(
                            previous_batch.sampler_state.host.values(),
                            src=self.dist.prev_pp_rank,
                            tag=prev_microbatch_id)
                    else:
                        torch.cuda.nvtx.range_push("_handle_new_tokens_last_pp")
                        previous_batch.sampler_state.sampler_event.synchronize()
        ...
        with self._profiler() as profile_step:
            ...
            while not got_finish_signal or len(self.active_requests) > 0:
                ...
                if scheduled_batch.batch_size > 0:
                    ...
                    sampler_state = self._sample_async(scheduled_batch,
                                                       batch_outputs)
                    ...
                    self._update_requests(sampler_state)
                ...
                if self.enable_iter_perf_stats:
                    ...
                    self._process_iter_stats(
                        finished_requests,
                        BatchState(sampler_state=SampleState(
        ...
        with self._profiler() as profile_step:
            ...
            while not got_finish_signal or len(self.active_requests) > 0:
                ...
                if scheduled_batch.batch_size > 0:
                    ...
                    previous_tensors_device = self.previous_batch and self.previous_batch.sampler_state.device
    ...
    def _process_previous_batch(self):
        self._update_requests(self.previous_batch.sampler_state)
        ...
        self._handle_cancelled_requests()
        finished_requests = self._handle_responses()
        scheduled_requests = self.previous_batch.sampler_state.scheduled_requests
    ...
    def _forward_step_last_pp(self, scheduled_batch,
                              microbatch_id) -> SampleState:
        batch_outputs = self._forward_step(scheduled_batch)
        sampler_state = self._sample_async(scheduled_batch, batch_outputs)
        self._update_request_states(scheduled_batch)

        if self.send_handles[microbatch_id] is not None:
            self.send_handles[microbatch_id].Wait()
        sampler_state.sampler_event.synchronize()

        self.send_handles[microbatch_id] = self.dist.isend_tensor_list(
            sampler_state.host.values(),
            dest=self.dist.next_pp_rank,
            tag=microbatch_id)
    ...
    def _handle_previous_batch_inter_pp(
        ...
        tensors_host = previous_batch_state.sampler_state.host
    ...
    def _forward_step(self,
                      scheduled_requests,
                      tensors_device: Optional[Dict[str, torch.Tensor]] = None):
        ...
        def forward(scheduled_requests, resource_manager, tensors_device):
            return self.model_engine.forward(scheduled_requests,
                                             resource_manager, tensors_device)
    ...
    def _update_requests(self, sampler_state: SampleState):
        try:
            self.sampler.update_requests(sampler_state)
        except Exception as e:
            traceback.print_exc()
            error_msg = str(e)
            logger.error(f"Encountered an error in sampling: {error_msg}")
    ...
    def _prepare_draft_tokens(self, scheduled_requests: ScheduledRequests):
        try:
            ...
            sampler_state = self._sample_async(draft_batch, outputs)
