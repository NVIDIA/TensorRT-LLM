...
class KVCacheManagerV2(BaseResourceManager):
    def __init__(
        ...
        **kwargs,
        ...
        # _copy_scratch_metadata_to_device. Bounded so a debug-level run does
        # not turn into one log line per context request forever.
    ...
    @property
    def has_swa_layers(self) -> bool:
        """True when at least one layer uses a real sliding window.

        This is the predicate SWA scratch reuse cares about, and it mirrors
        ``compute_scratch_range``'s own guard (a lifecycle with
        ``window_size is None`` never yields a scratch range). It is
        deliberately *not* ``is_vswa``: windows equal to ``max_seq_len`` are
        normalized to ``None``, so a pure-SWA model (every layer windowed) has
        ``is_vswa == False`` while benefiting the most from scratch reuse --
        one lifecycle holds all N windowed layers, so the shared-slot fraction
        1/N is maximal.
        """
        return any(w is not None and w > 0 for w in self.max_attention_window_vec)
    ...
    def _log_swa_scratch_summary(self) -> None:
        ...
        if not swa_lc_ids:
            if enabled:
                logger.warning(
                    f"{type(self).__name__}: kv_cache_config.enable_swa_scratch_reuse is "
                    "enabled but no attention lifecycle has a sliding window "
                    f"(max_attention_window_vec={self.max_attention_window_vec}); "
                    "scratch reuse will do nothing."
                )
            return
        ...
        for pool_id in range(self.num_pools):
            for context_idx, request_id in enumerate(request_ids[:num_contexts]):
                ...
                if self._scratch_debug_budget > 0:
                    ...
                    logger.debug(
                        f"SWA scratch: request={request_id} pool={pool_id} "
                        ...
                        f"num_scratch_slots={len(slot_ids)}"
                    )
    ...
    def get_buffers(
        ...
        ``index_mode`` selects the addressing contract, and must match the mode
        used to build the page indices handed to the kernel:
    ...
    def _apply_scratch_to_flat_indices(
        ...
        offset = 0
        for req_id, n in zip(request_ids, num_blocks):
            desc = self.kv_cache_map[req_id].get_scratch_desc(pool_id)
            if desc is None:
            ...
            for ordinal in range(n):
                pos = offset + ordinal
                if beg <= ordinal < end:
                    total = (ordinal - beg) * scratch_pages
                    slot_id = int(slot_ids[total // scale])
                    sub = (total % scale + layer_offset) % scale
                    out[pos] = (slot_id * scale + sub) // div_factor
                elif out[pos] != BAD_PAGE_INDEX:
                    out[pos] += layer_offset // div_factor
            offset += n
