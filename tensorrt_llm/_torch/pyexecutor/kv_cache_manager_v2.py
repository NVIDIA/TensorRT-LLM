...
class KVCacheManagerV2(BaseResourceManager):
    ...
    def _ensure_draft_mirror(self, req: LlmRequest):
        """Create the request's draft mirror if needed and reactivate it.

        Returns the mirror, or None when it could not be created (saturated
        IndexMapper) or resumed.  Both are back-pressure, not errors, and the
        caller must defer the request rather than schedule one whose mirror is
        missing: its spec-dec forward would then read an absent entry.
        """
        kv_cache = self.kv_cache_map.get(req.py_request_id)
        if kv_cache is None:
            kv_cache = self._create_kv_cache(
    ...
    def _mirror_context_capacity(self, req: LlmRequest, capacity: int) -> bool:
        """Mirror *req* and grow it to *capacity*; False if either step failed."""
        kv_cache = self._ensure_draft_mirror(req)
        if kv_cache is None:
            return False
        return kv_cache.resize(max(kv_cache.capacity, capacity))

    def _draft_gen_growth(self, req: LlmRequest) -> int:
        """Tokens the mirror must gain for one decode step of *req*.

        The same 1 + draft tokens the primary pool grows by, padded up to
        _kv_reserve_draft_tokens (see __init__); the padding is a no-op when
        reserve == draft_token_length.
        """
        return self._required_gen_capacity(req, 0) + max(
    ...
    def reserve_draft_context(self, req: LlmRequest, num_tokens: int) -> bool:
        ...
        The mirror is otherwise only touched in ``_prepare_draft_resources``,
        which runs after scheduling where a shortfall can no longer be deferred.
        Reserving here lets the scheduler treat mirror exhaustion the way it
        already treats primary exhaustion (skip and retry next iteration), as
        ``_try_schedule_cross_context_v2`` does for the enc-dec cross pool.  The
        caller releases the pages held by a refused chunk.

        *num_tokens* is the target-side chunk size (plus draft tokens on the
        last chunk), i.e. the same value passed to ``resize_context``.
        """
        target = req.context_current_position + num_tokens + self.num_extra_kv_tokens
        return self._mirror_context_capacity(req, target)
    ...
    def can_reserve_draft_generation(self, req: LlmRequest) -> bool:
        ...
        *missing* mirror and a *suspended* one: ``resume()`` returning False is a
        ...
        """
        # _ensure_draft_mirror covers both: a mirror missing because creation was
        # deferred while the IndexMapper was saturated, and a suspended one whose
        # resume() is refused.
        kv_cache = self._ensure_draft_mirror(req)
        if kv_cache is None:
            return False
        ...
        pre_cap = kv_cache.capacity
        growth = self._draft_gen_growth(req)
        ...
        target = pre_cap + growth + self._probed_draft_gen_tokens
        if not kv_cache.resize(target):
            return False
        if not kv_cache.resize(pre_cap):
            raise RuntimeError(
                f"Failed to undo draft KV cache generation probe for request "
                f"{req.py_request_id} from {target} to {pre_cap}"
            )
        self._probed_draft_gen_tokens += growth
        return True
    ...
    def _prepare_draft_resources(self, scheduled_batch: ScheduledRequests):
        ...
        # context_chunk_size to a second, draft-side cursor pair that is
        # initialized to prompt_len and only ever written by a two-model drafter.
        # A one-model mirror has no prefill of its own, so reading it inside the
        # block would size the mirror to the whole prompt on the first chunk.
        ctx_capacities = [
        ...
        # each request's capacity at admission, making the resizes below normally
        ...
        # than raising, which would kill the executor loop on every rank.
        with request_context(True, scheduled_batch):
            for req, capacity in zip(scheduled_batch.context_requests, ctx_capacities):
            ...
            for req in scheduled_batch.generation_requests:
                # A missing mirror is created rather than skipped: the request is
                # already scheduled, and copy_batch_block_offsets resolves its ID
                # through the IndexMapper, which aborts on an unknown ID.
                growth = self._draft_gen_growth(req)
                kv_cache = self._ensure_draft_mirror(req)
                if kv_cache is None or not kv_cache.resize(kv_cache.capacity + growth):
                    logger.warning(
                        "Draft KV cache generation mirroring deferred for request %s "
                        "(+%d tokens); retrying next iteration.",
                        req.py_request_id,
                        growth,
                    )
