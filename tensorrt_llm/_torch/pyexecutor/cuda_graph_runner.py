        ...
        if extra_model_inputs is not None:
            for key, tensor in extra_model_inputs.items():
                if isinstance(tensor, torch.Tensor):
                    ...
                    new_list = [t.clone() for t in tensor]
                    inputs[key] = new_list
                    self.extra_model_inputs[key] = new_list
        ...
        if self.extra_model_inputs:
            ...
            for key in self.extra_model_inputs:
                ...
                if isinstance(dst_tensor, torch.Tensor):
                    ...
                    src_tensors = extra_model_inputs[key]
                    for i, t in enumerate(dst_tensor):
                        t.copy_(src_tensors[i])
