...
class PenaltyHandler:
    ...
    def prepare_for_new_request(self, request: LlmRequest, slot: int) -> None:
        ...
        if not _has_occurrence_penalty(request):
        ...
        *,
        ...
        Args:
            request_num_beams / predecessor_beams: required when ``max_beam_width > 1``.
                ``predecessor_beams`` must still hold the *previous* step's parent map,
                i.e. this must run before the step's beam sampling overwrites it.
