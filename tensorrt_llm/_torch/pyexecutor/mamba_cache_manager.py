...
class PythonMambaCacheManager(BaseResourceManager):
    ...
    def __init__(
        ...
        self.conv_states = torch.empty(
            size=(num_local_layers, max_num_sequences) + conv_state_shape,
            dtype=dtype,
            device=device,
        )
        ...
        self.ssm_states = torch.empty(
            size=(num_local_layers, max_num_sequences) + ssm_state_shape,
        ...
        if speculative_num_draft_tokens is not None and spec_state_size is not None:
            ...
            logger.info(
                ...
                f"max_num_sequences: {max_num_sequences}, "
        ...
        self.mamba_cache_free_blocks = [i for i in range(max_num_sequences)]
        ...
        self.intermediate_state_indices = torch.arange(max_num_sequences,
                                                       dtype=torch.int32,
                                                       device=device)
