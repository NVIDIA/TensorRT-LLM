...
class MambaHybridCacheManager(BaseResourceManager, BaseMambaCacheManager):
    ...
    def _setup_mtp_intermediate_states(self, spec_config,
        ...
        use_kda_replay_update = getattr(self, "_use_kda_replay_update", False)
        if not self._use_replay_state_update and not use_kda_replay_update:
    ...
    def _allocate_pool_replay_buffers(
        ...
        if use_kda_replay_update:
            num_spec = self._kda_replay_num_spec
            if num_spec is None:
                raise RuntimeError(
                    "KDA replay is enabled without a replay-cache width")
            ...
            committed_window = self.conv_state_shape[1] - 1
    ...
    def _reset_kda_replay_slots(
        ...
        if (not getattr(self, "_use_kda_replay_update", False)
    ...
    def seed_kda_replay_caches_for_disagg_gen(self,
                                              request_ids: List[int]) -> None:
        """Seed V2 KDA replay caches from transferred convolution states."""
        if (not getattr(self, "_use_kda_replay_update", False)
        ...
        for layer_offset, conv_state in enumerate(self.all_conv_states):
            ...
            for section_idx, replay_buffer in enumerate(replay_conv_buffers):
                ...
                section = selected_conv[:, section_offsets[section_idx]:
                                        section_offsets[section_idx + 1], 1:]
    ...
    def mamba_layer_cache(
        ...
        if self.spec_config is not None:
            ...
            if getattr(self, "_use_kda_replay_update", False):
                ...
                spec_kwargs['kda_conv_q'] = self.kda_conv_q[layer_offset]
                spec_kwargs['kda_conv_k'] = self.kda_conv_k[layer_offset]
                spec_kwargs['kda_conv_v'] = self.kda_conv_v[layer_offset]
                spec_kwargs['kda_qkg_cache'] = self.kda_qkg_cache[layer_offset]
                spec_kwargs['kda_v_cache'] = self.kda_v_cache[layer_offset]
                spec_kwargs['kda_beta_cache'] = self.kda_beta_cache[
                    layer_offset]
            elif self._use_replay_state_update:
            ...
            intermediate_conv_window = (
                self.intermediate_conv_states[layer_offset]
                if self.intermediate_conv_states is not None else None)
...
class MambaHybridCacheManagerV2(KVCacheManagerV2, MambaHybridCacheManager):
    ...
    def _setup_state_indices(self,
                             ...
                             num_contexts: Optional[int] = None) -> None:
        if self.local_num_mamba_layers == 0:
            return
        n = len(requests)
        ...
        self._refresh_dummy_request_mask(is_dummy)
        state_values = self._host_state_indices[:n].tolist()
        if num_contexts is not None and self._use_kda_replay_update:
            # KDA caches carry accepted drafts across verify rounds. V2 can
            # remap a resumed generation request to a different logical SSM
            # slot, so move that replay history before the next forward.
            # Context rows start a new replay lineage and are reset by
            # prepare_resources after this mapping is installed.
            generation_requests = requests[num_contexts:]
            generation_state_values = state_values[num_contexts:]
            old_state_values = [
                self._request_id_to_state_index.get(request.py_request_id, -1)
                for request in generation_requests
            ]
            self._relocate_kda_replay_slots(old_state_values,
                                            generation_state_values)
            new_generation_slots = [
                state_value for old_state_value, state_value in zip(
                    old_state_values, generation_state_values)
                if old_state_value < 0
            ]
            self._reset_kda_replay_slots(new_generation_slots)
        for req, value, dummy in zip(requests, state_values, is_dummy):
            self._request_id_to_state_index[req.py_request_id] = value
            self._request_id_to_is_dummy[req.py_request_id] = dummy
    ...
    def update_resources(self,
        ...
        KVCacheManagerV2.update_resources(self, scheduled_batch, attn_metadata,
                                          kv_cache_dtype_byte_size)
