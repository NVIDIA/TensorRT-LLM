    ...
    def __init__(
        ...
        **kwargs,
        ...
        if mapping.has_cp_helix():
            # Helix stripes only the MLA KV; every other layer (incl. KDA)
            # repurposes CP ranks as plain TP, so each rank holds a
            # 1/(tp*cp) head slice of the mamba state.
    ...
    def __init__(
        ...
        **kwargs,
        ...
        if self.local_num_mamba_layers > 0:
            tp_size = mapping.tp_size if not mapping.enable_attention_dp else 1
            if mapping.has_cp_helix():
                # Helix stripes only the MLA KV; every other layer (incl.
                # KDA) repurposes CP ranks as plain TP — 1/(tp*cp) head
                # slice per rank.
                tp_size = mapping.tp_size * mapping.cp_size
            d_inner = mamba_head_dim * mamba_num_heads
