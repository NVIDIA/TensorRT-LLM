...
def compute_probs(
    ...
    *,
    ...
    """Compute filtered normalized probabilities from logits (scalar params).

    Returns exact one-hot when temperature <= _GREEDY_TEMPERATURE_THRESHOLD.
    """
...
def sample_from_logits(
    ...
    *,
    ...
    """Compute probs from logits then sample; returns (tokens, probs)."""
    probs = compute_probs(logits, temperature=temperature, top_k=top_k, top_p=top_p)
    tokens = sample_from_probs(probs, generator=generator)
    return tokens, probs
