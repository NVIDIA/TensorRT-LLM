...
class PyMicroBatchScheduler(MicroBatchScheduler):
    ...
    def _chunk_forced(self, requests: RequestList, capacity: Optional[int], unit_size: int):
        ...
        Requests use expect_chunking_points when present; otherwise each request
        gets min(context_remaining_length, unit_size). Requests that would exceed
        the capacity budget are rounded down to the nearest lower unit_size multiple.
