...
class TorchDecoder(Decoder):
    ...
    def update_requests(self, decoder_state: DecoderState) -> None:
        ...
        for request in decoder_state.scheduled_requests.context_requests:
            assert not request.py_return_context_logits, "return_context_logits not implemented for TorchDecoder"
            ...
            if request.state != LlmRequestState.GENERATION_COMPLETE:
                new_token = new_tokens_list[logits_idx]
        ...
        if hasattr(decoder_state.scheduled_requests, 'chunked_requests'):
            logits_idx += len(decoder_state.scheduled_requests.chunked_requests)

        # sorted() is stable.
        # TODO: can we ensure extend request (has_draft_tokens = True) are always before generation requests,
        #  therefore this sorted() call can be avoided?
        generation_requests = sorted(
            decoder_state.scheduled_requests.generation_requests,
            key=lambda request: hasattr(request, 'py_draft_tokens') and request.
            py_draft_tokens is not None,
            reverse=True)
        ...
        for request in generation_requests:
            ...
            if request.py_draft_tokens is not None:
                ...
                for i in range(len(request.py_draft_tokens)):
                    ...
                    new_token = new_tokens_list[logits_idx + accepted_tokens]
                    num_tokens = request.add_new_token(new_token, beam_idx)
                    accepted_tokens += 1
...
class TorchStarAttentionDecoder(TorchDecoder):
    ...
    def update_requests(self, decoder_state: DecoderState):
        ...
        for request in decoder_state.scheduled_requests.context_requests:
            assert not request.py_return_context_logits, "return_context_logits not implemented for TorchStarAttentionDecoder"
...
class TRTLLMDecoder(Decoder):
    ...
    def setup_decoder(self, scheduled_requests: ScheduledRequests,
        ...
        decoding_input, self.decoding_output = self.algs.make_decoding_batch_input_output(
            ...
            self.model_config, self.max_num_sequences, self.beam_width,
            self.store["buffer_manager"], self.store["cuda_stream"])
