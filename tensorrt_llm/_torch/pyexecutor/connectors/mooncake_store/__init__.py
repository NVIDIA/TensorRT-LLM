...
This package currently provides the store itself rather than the connector that
reads and writes it. The pool is described in `KvCacheConnectorConfig.
mooncake_store`, which lets `trtllm-serve` provision it during bringup so no
external script has to; see `master.py`. Capacity comes only from processes that
open a store handle, which in a disaggregated deployment is the context servers
alone, so `donor.py` lends a node's memory to the pool without giving it a
connector. Both need the Mooncake Python bindings (`pip install
mooncake-transfer-engine`).

The connector that moves KV pages in and out of the pool needs `KVCacheManagerV2`
to describe its pools through `register_kv_cache_layout`, and lands separately.
`keys.py` and `staging.py` hold the pieces it shares with the store side: how a
block of tokens becomes a store key, and how pages reach the fabric on hosts
