...
def _request_strategy(request: LlmRequest, *, vocab_size: int) -> Strategy:
    ...
    if hasattr(request, "py_sampling_strategy"):
        return cast(Strategy, request.py_sampling_strategy)  # pyright: ignore
    ...
    if _request_sampling_params_cachable(params):
        request.py_sampling_strategy = sampling_strategy  # type: ignore[attr-defined]
...
class TorchSampler(Sampler[SampleStateTorch], AsyncWorkerMixin):
    ...
    @cached_property
    def _pad_steps_mask(self) -> torch.Tensor:
...
class TRTLLMSampler(Sampler[SampleStateTRTLLM], AsyncWorkerMixin):
    ...
    def sample_async(
        ...
        gathered_ids = None
        if beam_width > 1:
            finished_sum_device = self.store["decoder_state"].finished_sum  # type: ignore
            ...
            gathered_ids = self._copy_to_host(self.store["decoder_state"].gathered_ids)  # type: ignore
        new_output_tokens = self._copy_to_host(self.store["decoder_state"].all_new_tokens)  # type: ignore
        finished_sum = self._copy_to_host(self.store["decoder_state"].finished_sum)  # type: ignore
        finish_reasons = self._copy_to_host(self.store["decoder_state"].finish_reasons)  # type: ignore
        sequence_lengths = self._copy_to_host(self.store["decoder_state"].sequence_lengths)  # type: ignore

        log_probs = None
        cum_log_probs = None
        if any(request.py_return_log_probs for request in scheduled_requests.all_requests()):
            log_probs = self._copy_to_host(self.store["decoder_state"].log_probs)  # type: ignore
            cum_log_probs = self._copy_to_host(self.store["decoder_state"].cum_log_probs)  # type: ignore

        device = SampleStateTensors(new_tokens=self.store["decoder_state"].all_new_tokens)  # type: ignore
