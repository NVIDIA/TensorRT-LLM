...
class TorchSampler(Sampler):
    ...
    def _process_draft_tokens_tree(self, request: LlmRequest,
        ...
        if get_draft_token_length(request) == 0:
            ...
            if spec_tree_manager.use_dynamic_tree and cur_draft_layer_idx == spec_tree_manager.max_draft_len - 1:
                # TODO: For the last layer of the dynamic tree, we need to resampling all the draft tokens.
                raise NotImplementedError("Dynamic tree is not supported yet")
            else:
                # Add the draft tokens of the previous layer.
                for i in range(
                        spec_tree_manager.get_current_layer_draft_len(
            ...
            all_draft_tokens = torch.tensor(
                request.py_draft_tokens, dtype=torch.int,
                device='cpu')  # [max_total_draft_tokens]
    ...
    def _tree_sampling_batch(self, new_tokens_cuda: torch.Tensor,
                             requests: list[LlmRequest], max_num_sequences: int,
                             seq_slots: torch.Tensor,
        ...
        new_tokens_cuda = new_tokens_cuda.zero_()
        new_tokens_cuda = new_tokens_cuda.reshape(
        ...
        for i, top_k_list_i in enumerate(top_k_list):
            ...
            new_tokens_cuda[
                ...
                top_k_list_i:top_k_list_cumsum[i]] = indices[request_index].to(
                    torch.int)
    ...
    def process_draft_tokens(
        ...
        if request.py_draft_logits is None:
            ...
            if spec_tree_manager is not None:
                if get_draft_token_length(request) == 0:
                    # draft request
                    num_draft_tokens_this_layer = spec_tree_manager.get_current_layer_draft_len(
                        spec_tree_manager.cur_draft_layer_idx - 1)
                    for i in range(num_draft_tokens_this_layer):
                        new_token = add_token(request,
                                              new_tokens,
                                              beam=self.BEAM,
                                              step=i)
    ...
    def update_requests(
        ...
        for req in state.scheduled_requests.generation_requests:
            if req.state == LlmRequestState.GENERATION_COMPLETE:
                continue
            processed = 1
            num_accepted = self.process_draft_tokens(req, state.host.new_tokens,
                                                     resource_manager)
