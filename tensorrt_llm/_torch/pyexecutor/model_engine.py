...
class PyTorchModelEngine(ModelEngine):
    ...
    def _run_attention_warmup(self,
        ...
        if (not self.is_draft_model and self.guided_decoder is None
            ...
            # The cute_dsl FMHA lib now only support the generation-only batch, we need to warmup the TRTLLM-Gen FMHA lib for the mixed context+generation batch.
            # One MIXED context+generation batch (1 ctx token + 1 gen request).
            warmup_requests_configs.append(
                (1 + self.max_total_draft_tokens + 1, 1))
        else:
            logger.debug(
                "Skipped TRTLLM-Gen flashinfer_trtllm_gen FMHA lib JIT warmup When enable cute_dsl FMHA lib"
            )
