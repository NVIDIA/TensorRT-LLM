...
class SpecTreeManager:
    ...
    max_draft_len: int  # The number of all nodes in the tree (except the root)
    dynamic_tree_max_topK: int  # If using dynamic tree, the number of nodes to expand each time.
    num_eagle_layers: int  # The number of drafter layer
    cur_eagle_layer_idx: int  # The current index of the drafter layer

    # Auxiliary buffers
    eagle_choices: Optional[List[List[
        int]]] = None  # The user input eagle choices, only available when using static tree.
    num_trees: Optional[
        int] = None  # If dynamice tree, each request has their own tree. If static tree, all requests share the same tree.
    index_mapping_list: Optional[List[Dict[
        str,
        int]]] = None  # Mapping each choice/path to an index, which can also be treated as the node index.
    # Each tree will have an associated mapping list. Root node is '0'.
    eagle_paths: Optional[
        torch.
        Tensor] = None  # Convert the choice to a path. Each path is an array of indices from the root to other nodes in the tree.
    # shape: [num_trees, max_draft_len + 1, num_eagle_layers + 1]
    nodes_list_per_layer: Optional[
        torch.Tensor] = None  # The nodes list of each layer. Padding with -1.
    # shape: [num_trees, num_eagle_layers + 1, max_draft_len + 1]
    num_nodes_per_layer: Optional[
        torch.
        Tensor] = None  # The number of nodes in each layer. Actually a count of nodes_list_per_layer values that are not -1.
    # shape: [num_trees, num_eagle_layers + 1]
    parent_node_index: Optional[
        torch.
        Tensor] = None  # The parent node of each node. For the root node, it is -1.
    # shape: [num_trees, max_draft_len + 1]
    child_nodes_list: Optional[
        torch.Tensor] = None  # The child nodes of each node. Padding with -1.
    # shape: [num_trees, max_draft_len + 1, max_draft_len]
    num_child_nodes: Optional[
        torch.
        Tensor] = None  # The number of child nodes of each node. Actually a count of child_nodes_list values that are not -1.

    # shape: [num_trees, max_draft_len + 1]

    def __init__(self, max_num_requests: int, use_dynamic_tree: bool,
                 max_draft_len: int, num_eagle_layers: int,
        ...
        self.cur_eagle_layer_idx = -1

        # Initialize the buffers
        self.index_mapping_list = [{} for _ in range(self.num_trees)]
        self.eagle_paths = torch.ones(
            (self.num_trees, self.max_draft_len + 1, self.num_eagle_layers + 1),
            dtype=torch.int32,
            device='cpu',
            pin_memory=True,
        ) * -1
        self.nodes_list_per_layer = torch.ones(
            (self.num_trees, self.num_eagle_layers + 1, self.max_draft_len + 1),
            dtype=torch.int32,
            device='cpu',
            pin_memory=True,
        ) * -1
        self.num_nodes_per_layer = torch.zeros(
            (self.num_trees, self.num_eagle_layers + 1),
            dtype=torch.int32,
            device='cpu',
            pin_memory=True,
        )
        ...
        self.child_nodes_list = torch.ones(
            (self.num_trees, self.max_draft_len + 1, self.max_draft_len),
            dtype=torch.int32,
            device='cpu',
            pin_memory=True,
        ) * -1
    ...
    def init_tree_from_input_choices(self):
        ...
        tmp_child_nodes_list = [[] for _ in range(self.max_draft_len + 1)]
        for choice in self.eagle_choices:
            if len(choice) == 1:
