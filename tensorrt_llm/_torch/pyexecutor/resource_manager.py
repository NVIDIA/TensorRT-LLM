...
class KVCacheManagerV2(BaseResourceManager):
    ...
    def _extra_buffers_per_layer(
            self, *,
            tokens_per_block: int) -> Optional[Dict[int, List[BufferConfig]]]:
    ...
    def get_index_k_buffer(
        ...
        *,
        ...
        layer_offset = self.layer_offsets[layer_idx]
        try:
            addr = self.impl.get_mem_pool_base_address(layer_offset,
                                                       Role.INDEX_KEY)
            page_stride = self.impl.get_page_stride(layer_offset,
                                                    Role.INDEX_KEY)
            page_upper = self.impl.get_page_index_upper_bound(
                layer_offset, Role.INDEX_KEY)
        ...
        expected_stride = (num_heads * head_dim * elem_bytes *
                           self.tokens_per_block)
        ...
        if scale == 1:
            ...
            return convert_to_torch_tensor(
                TensorWrapper(addr, torch_dtype, shape))
        ...
        # ``s`` in [0, num_slots).
        full_slot_shape = [
            num_slots, scale, self.tokens_per_block, num_heads, head_dim
        ]
        full_view = convert_to_torch_tensor(
            TensorWrapper(addr, torch_dtype, full_slot_shape))
        return full_view[:, 0]
