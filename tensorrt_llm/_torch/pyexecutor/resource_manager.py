...
class KVCacheManager(BaseResourceManager):
        ...
        if is_estimating_kv_cache:
            ...
            if self.is_linear_attention:
                ...
                if kv_cache_config.enable_block_reuse:
                    ...
                    ) if linear_attention_metadata.states_snapshot_interval > 0 else 1
                    max_snapshots = max(regular_snapshots + last_snapshot_slots,
                                        max_snapshots)
    ...
    def _calculate_max_num_blocks_for_linear_attention(
        ...
        if (kv_cache_config.enable_block_reuse and interval is not None
                and interval > 0):
            max_snapshots = max(max_tokens // interval + last_snapshot_slots,
                                max_snapshots)
