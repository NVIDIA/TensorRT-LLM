...
class _ExecutorCreationStage(enum.Enum):
    SAMPLER = "Sampler"
    KV_CACHE_INITIAL = "Initial KV cache (temporary for KV cache size estimation)"
    EXTRA_INITIAL = "Additional executor resources (temporary for KV cache size estimation)"
    EXTRA = "Additional executor resources"
    KV_CACHE = "KV cache"
...
class _ExecutorMemoryMonitor():
    ...
    @staticmethod
    def _maybe_explain_if_oom(e: Exception) -> Optional[str]:
        if isinstance(e, torch.OutOfMemoryError):
            return "Executor creation failed due to insufficient GPU memory."
    ...
    def observe_creation_stage(self, current_stage: _ExecutorCreationStage):
        ...
        try:
            ...
            explanation = self._maybe_explain_if_oom(e)
            ...
            tuning_knobs = {
                ...
                _ExecutorCreationStage.KV_CACHE_INITIAL:
                ...
                ("reduce max_num_tokens and/or shard the model weights across GPUs by enabling "
                 "pipeline and/or tensor parallelism"),
                _ExecutorCreationStage.EXTRA_INITIAL:
                "reduce max_num_tokens",
                _ExecutorCreationStage.EXTRA:
                "reduce max_num_tokens",
            }
            ...
            msg = "\n".join([
                ...
                *(f"  {stage.value}: {tuning_knobs[stage]}"
                  for sample in self._samples
                  if (stage := sample.creation_stage) in tuning_knobs),
            ])
...
def create_py_executor(executor_config: ExecutorConfig,
    ...
    if executor_config.pytorch_backend_config.use_kv_cache:
        kv_cache_creator = KvCacheCreator(executor_config=executor_config,
                                          ...
                                          mapping=mapping)
        estimating_kv_cache = kv_cache_creator.maybe_prepare_estimation()
        with mem_monitor.observe_creation_stage(
                _ExecutorCreationStage.KV_CACHE_INITIAL
    ...
    with mem_monitor.observe_creation_stage(
            _ExecutorCreationStage.EXTRA_INITIAL
            if estimating_kv_cache else _ExecutorCreationStage.EXTRA):
    ...
    if estimating_kv_cache:
        ...
        kv_cache_creator.estimate_max_tokens(py_executor, origin_seq_len)
        ...
        with mem_monitor.observe_creation_stage(_ExecutorCreationStage.EXTRA):
            py_executor = create_py_executor_instance(
