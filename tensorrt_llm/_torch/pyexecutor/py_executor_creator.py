...
def create_py_executor(
    ...
    if is_mla(config):
        if model_engine.model.model_config.enable_flash_mla:
            tokens_per_block = 64
            # Propagate the override back to kv_cache_config so downstream
            # consumers that read kv_cache_config.tokens_per_block later see
            # the effective value. In particular, KvCacheConnectorScheduler
            # subclasses (LMCache, Dynamo KVBM) are instantiated further down
            # via `scheduler_cls(llm_args)` and the connectors size their
            # block pools from llm_args.kv_cache_config.tokens_per_block.
            # Without this assignment the connector's block size desynced
            # from the KVCacheManager's actual tokens_per_block (user-set or
            # default 32 vs. FlashMLA's forced 64), producing a frozen
            # cache_block_ids view to the connector and silently-corrupted
            # decode KV.
