...
if IS_CUDA_TILE_AVAILABLE:
    ...
    def rms_norm_fuse_residual_kernel(
        ...
        for j in range(0, num_tiles):
            ...
            ct.store(
                residual, index=(row, j), tile=xj_stored,
                allow_tma=False,
                latency=1,
            )
    ...
    def rms_norm_fuse_residual_kernel_static_persistent(
        ...
        for current_block in range(pid, upper_bound, num_tiles_x):
            ...
            # Load residual tile
            residual = ct.load(
                Residual, index=(current_block, 0), shape=(TILE_SIZE_M, TILE_SIZE_N),
                latency=10,
            )
            ...
            ct.store(
                Residual, index=(current_block, 0), tile=x_stored,
                allow_tma=False,
                latency=3,
            )
            ...
            # Store result to X
            ct.store(
                X, index=(current_block, 0), tile=y,
