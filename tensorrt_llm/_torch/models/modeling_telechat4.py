...
"""TeleChat4 TRT-LLM PyTorch model with mHC residual mixing."""
...
def _patch_rope_scaling(config: PretrainedConfig) -> None:
    """Normalize TeleChat4 rope scaling for TRT-LLM."""
...
class Telechat4DecoderLayer(DecoderLayer):
    ...
    def __init__(
        ...
        self.n_streams = getattr(config, "num_residual_streams", 1)
        ...
        sinkhorn_iters = int(getattr(config, "mhc_sinkhorn_iterations", 20))
        mhc_norm_eps = float(getattr(config, "mhc_norm_eps", 1e-6))
        mhc_pre_eps = float(getattr(config, "mhc_pre_eps", 1e-6))
        mhc_sinkhorn_eps = float(getattr(config, "mhc_sinkhorn_eps", 1e-6))
...
class Telechat4Model(DecoderModel):
    def __init__(self, model_config: ModelConfig[PretrainedConfig]):
        ...
        self.layers = nn.ModuleList(
            [
                Telechat4DecoderLayer(model_config, layer_idx, self.aux_stream_dict)
...
class Telechat4WeightLoader(DeepseekV3WeightLoader):
    """Load TeleChat4 weights and reuse the DeepSeekV3 loader."""
    ...
    def _load_hc(self, weights):
        ...
        for name, module in hc_modules.items():
            ...
            if mapping_key not in weights or bias_key not in weights:
                continue

            mapping_w = weights[mapping_key][:]
            bias = weights[bias_key][:]
            alpha_pre = weights[f"{name}.alpha_pre"][:]
            alpha_post = weights[f"{name}.alpha_post"][:]
            alpha_res = weights[f"{name}.alpha_res"][:]

            module.fn.data.copy_(mapping_w.to(torch.float32).contiguous())
            module.base.data.copy_(bias.to(torch.float32).contiguous())
...
@register_auto_model("TeleChat4ForCausalLM")
class TeleChat4ForCausalLM(SpecDecOneEngineForCausalLM[Telechat4Model, PretrainedConfig]):
    def __init__(self, model_config: ModelConfig[PretrainedConfig]):
        _patch_rope_scaling(model_config.pretrained_config)
        super().__init__(
            model=Telechat4Model(model_config),
            model_config=model_config,
        )
    ...
    def load_weights(self, weights):
        loader = Telechat4WeightLoader(self)
