...
class DynamicResolutionImageTiler:
    ...
    def compute_params(
        ...
        num_tokens_available = num_tokens_available * (self._downsample_ratio**2)
...
class NanoV2VLVisionEncoder(transformers.PreTrainedModel):
    ...
    def extract_feature_dynamic(
        self, pixel_values_flat: torch.Tensor, imgs_sizes: List[Tuple[int, int]]
    ) -> torch.Tensor:
        """Dynamic resolution feature extraction for variable-size images."""
        vit_embeds = self.vision_model(pixel_values_flat, imgs_sizes=imgs_sizes)
    ...
    def forward(
        ...
        has_dynamic = any(
            "imgs_sizes" in multimodal_data.get(modality_type, {})
            for modality_type, multimodal_data in zip(modality_types, multimodal_data_lst)
        )
        if has_dynamic:
            # Dynamic resolution path: process each request's images in one encoder pass.
            for modality_type, multimodal_data in zip(modality_types, multimodal_data_lst):
                data = multimodal_data[modality_type]
                pixel_values_flat = data["pixel_values"]
                imgs_sizes = data["imgs_sizes"]
                embeds = self.extract_feature_dynamic(pixel_values_flat, imgs_sizes)
                # embeds: [1, total_tokens, llm_hidden_size]
...
class NanoV2VLInputProcessor(BaseMultimodalInputProcessor, BaseMultimodalDummyInputsBuilder):
        ...
        **kwargs,
        ...
        if isinstance(vision_args, dict) and "min_num_patches" in vision_args:
            self.dynamic_tiler = DynamicResolutionImageTiler(
                max_model_len=getattr(config, "max_sequence_length", 131072),
    ...
    def _process_images_dynamic(
        ...
        for params in params_list:
            ...
            imgs_sizes.append((tensor.shape[-2], tensor.shape[-1]))
    ...
    def __call__(
        ...
        if images is not None:
            ...
            if self.dynamic_tiler is not None:
                ...
                modality_data["imgs_sizes"] = processed_data["imgs_sizes"]
