...
class KimiLinearForCausalLM(SpecDecOneEngineForCausalLM[KimiLinearModel, Any]):
    ...
    def load_weights(self, weights: Dict):
        ...
        # definitions near the top of this file). The KDA conversion supplies
        # the fused qkvg projection through the mixer's FP8 module, so the BF16
        # qkvg weight is not built when it is enabled. The FP8 decode glue
        # (enabled unless KIMI_K3_KDA_GLUE_FP8=0) still builds the shared BF16
        # [f_a | b] projection and kernel-layout constants.
