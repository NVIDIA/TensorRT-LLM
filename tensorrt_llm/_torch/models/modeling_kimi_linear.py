...
#
# * NoPE. ``mla_use_nope=True`` in K3 config disables the rotary embedding;
#   both the query and key rope slots pass through the backend unchanged.
# * Output gate before ``o_proj``. When ``mla_use_output_gate=True`` an extra
#   ``g_proj`` computes ``sigmoid(g_proj(hidden_states)) * attn_output`` before
#   the final projection (installed as a base-MLA output hook).
# * Softmax scale. ``(qk_nope + qk_rope) ** -0.5 = 192 ** -0.5`` for real K3
#   dims — matches ``TrtllmAttention`` default MLA q_scaling.
#
# The base ``MLA`` class owns context attention, cached/chunked prefill,
# absorbed generation, and paged-cache handling. The code below only supplies
# the K3 projection topology, NoPE identity table, KV-B checkpoint layout, and
# gated output projection. It was inlined here (from the former
# ``modules/kimi_k3_mla`` package) to match the per-model ``modeling_xxx.py``
# convention; ``kimi_kda`` stays a standalone module.
# ---------------------------------------------------------------------------
