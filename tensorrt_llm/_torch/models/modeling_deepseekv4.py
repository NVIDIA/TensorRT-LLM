...
def sparse_attn(q, kv, attn_sink, topk_idxs, softmax_scale):
    ...
    for ib in range(b):
        for iseq in range(s):
            ...
            scores = torch.matmul(q[ib, iseq], kv_selected.transpose(0, 1)) * softmax_scale # [h, num_valid]
...
def hc_split_sinkhorn(mixes, hc_scale, hc_base, hc_mult=4, sinkhorn_iters=20, eps=1e-6):
    ...
    comb = mixes[..., 2*hc_mult:].view(b, s, hc_mult, hc_mult) * hc_scale[2] + hc_base[2*hc_mult:].view(hc_mult, hc_mult)
...
class Attention(nn.Module):
    ...
    def forward(self, x: torch.Tensor, start_pos: int):
        ...
        def get_compress_topk_idxs_local(ratio: int, bsz: int, seqlen: int, start_pos: int, offset: int):
            if start_pos > 0:
        ...
        if self.compress_ratio:
            ...
            if self.indexer is not None:
                ...
                compress_topk_idxs = get_compress_topk_idxs_local(ratio, bsz, seqlen, start_pos, offset)
...
class DeepseekV4Model(DecoderModel):
    def __init__(self, model_config: ModelConfig[DeepseekV4Config]):
        ...
        self.args = ModelArgs(
            ...
            window_size=self.config.sliding_window if hasattr(self.config, 'sliding_window') else 128,
...
class DeepseekV4ForCausalLM(SpecDecOneEngineForCausalLM[DeepseekV4Model, PretrainedConfig]):
    def __init__(self, model_config: ModelConfig[PretrainedConfig]):
        ...
        self.head = ParallelHead(model_config.pretrained_config.vocab_size, model_config.pretrained_config.hidden_size)
    ...
    def forward(self, input_ids: torch.Tensor, start_pos: int = 0):
        ...
        logits = self.head(x, self.hc_head_fn, self.hc_head_scale, self.hc_head_base, self.model.norm)
        return logits
