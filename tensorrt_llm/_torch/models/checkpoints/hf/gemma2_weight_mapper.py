...
class Gemma2HfWeightMapper(HfWeightMapper):
    ...
    def should_skip_module(self, module_name: str) -> bool:
        if self.model.config.tie_word_embeddings and module_name.startswith("lm_head"):
            return True

        if (
            hasattr(self.model, "model")
            and hasattr(self.model.model, "has_custom_embed_tokens")
            and self.model.model.has_custom_embed_tokens
            and module_name == "model.embed_tokens"
        ):
            return True
        if (
            hasattr(self.model, "has_custom_lm_head")
            and self.model.has_custom_lm_head
        ...
        if "norm" in module_name:
            if n not in module_weights:
                if allow_partial_loading:
                    return
                raise KeyError(f"Missing weight '{n}' while loading module '{module_name}'.")
