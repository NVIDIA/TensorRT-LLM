...
class Qwen3NextMTP(Qwen3NextFullAttentionDecoderLayer):
    ...
    def forward(
        ...
        **kwargs,
        ...
        # Draft iterations can have a different per-rank token distribution
        # from the target forward stored in attn_metadata. In particular, MTP
        # Eagle processes one token per sequence after step 0. MoE chunking
        # consumes this metadata, so keep it aligned with the draft input and
        # restore the target metadata before returning to the caller.
