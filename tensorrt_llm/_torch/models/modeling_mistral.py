...
class Mistral3InputProcessor(InputProcessor):
    ...
    def __call__(
        ...
        if pixel_values is not None:
            ...
            processed["image_sizes"].to(self._device)
            extra_processed_inputs = {
                # This strictly has to be a tensor, as the `tensorrt_llm.bindings.executor.Request`
                # object rejects anything other than a `torch.Tensor`.
                "mm_embedding": pixel_values,
                # These can't be passed this way because they are supposed to be a `MultimodalInput`
                # instance...
                # "multimodal_input": image_sizes,
            }
...
class Mistral3VLM(PreTrainedModel):
    ...
    def forward(
        ...
        **kwargs,
        ...
        if len(multi_modal_data) > 0:
            image_features = [
                self._get_image_features(pixel_values=x)
                for x in multi_modal_data
            ]
    ...
    def _get_image_features(
        ...
        # This will be a `[B, 2]` tensor.
        image_sizes = torch.tensor(pixel_values.shape[-2:]).expand(
