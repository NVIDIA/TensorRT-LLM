...
class PixtralTransformer(torch.nn.Module):
    ...
    def forward(
        ...
        for encoder_layer in self.layers:
            # This is a massive hack. The alternative is to pipe the original
            # `position_ids` and `inputs_embeds` to every `PixtralAttentionLayer.attention` layer,
            # and have each layer recompute the `position_embeddings`.
...
# @reviewer: Should I be calling `register_auto_model` for this?
class PixtralVisionModel(torch.nn.Module):
    def __init__(
        ...
        # @reviewer: Are we supposed to use `tensorrt_llm.layers.conv.Conv2d` here? That one seems
        # to require a TRT harness for building the network, the runtime, etc.
    ...
    def forward(
        ...
        # The flashinfer implementation seems to have wildly different outputs than the HF ref.
        patch_embeds = self.ln_pre(patch_embeds, use_flash_infer=False)
