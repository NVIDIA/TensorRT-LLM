...
"""Fused Gemma4 decoder-layer residual-chain kernels.

One Triton kernel body (specialized via constexpr flags) serves two per-layer
fusions on the residual stream:

1. Layer tail (``gemma4_fused_norm_add_scale``): post_ffn RMSNorm + residual
   add + fp32 layer_scalar mul + bf16 cast in one kernel, replacing

       flashinfer_rmsnorm(mlp_out)          (read + write [M, H] bf16)
       residual + normed                    (2 reads + write, bf16)
       hidden * layer_scalar                (fp32[1] buffer promotes the mul:
                                             read bf16 + WRITE FP32 [M, H])
       .to(bf16) at the next layer's entry  (read fp32 + write bf16)

   Optionally (``next_norm_weight``) the kernel also emits the NEXT layer's
   input RMSNorm of the tail output as a second output, so the next layer
   skips its standalone input-norm pass entirely (one extra [M, H] write here
   replaces a read + write there).

2. Post-attention norm + residual add (``gemma4_fused_norm_add``): the same
   body without the scalar stage, replacing

       flashinfer_rmsnorm(attn_out)         (read + write [M, H] bf16)
       residual + normed                    (2 reads + write, bf16)
...
`.to(bfloat16)`, and the secondary norm reads the bf16-rounded tail output
exactly as the next layer's standalone norm would. The only residual
difference is the fp32 reduction order inside the sums of squares (same
...
@triton.jit
def _gemma4_tail_kernel(
    x_ptr,  # [M, H] bf16 kernel input (mlp/down_proj or attention output)
    r_ptr,  # [M, H] bf16 residual
    w_ptr,  # [H] bf16 RMSNorm weight for x
    sc_ptr,  # [1] fp32 layer_scalar (unused when HAS_SCALE == False)
    o_ptr,  # [M, H] bf16 out
    ...
    if HAS_SCALE:
        # aten mul(bf16, fp32[1]) promotes to fp32; the next layer's dtype
    ...
    if HAS_NORM2:
        # The next layer's input RMSNorm, computed on the bf16-rounded tail
        # output exactly as its standalone flashinfer_rmsnorm would read it.
...
def gemma4_fused_norm_add_scale(
    x: torch.Tensor,
    residual: torch.Tensor,
    norm_weight: torch.Tensor,
    ...
    """Fused ``bf16(fp32(residual + rmsnorm(x)) * layer_scalar)``.

    Args:
        x: [M, H] bf16 - the MLP (down_proj) output.
        residual: [M, H] bf16 - the pre-MLP residual.
        norm_weight: [H] bf16 - post_feedforward_layernorm weight (plain
            multiplier convention, ``use_gemma=False``).
        layer_scalar: [1] fp32 buffer (checkpoint-loaded; any value).
        eps: RMSNorm epsilon.
        next_norm_weight: optional [H] bf16 - the NEXT layer's
            input_layernorm weight. When given, the kernel also returns
            ``rmsnorm(out, next_norm_weight)`` computed on the bf16-rounded
            tail output, replacing the next layer's standalone input norm.
        next_norm_eps: epsilon for the secondary norm.

    Returns:
        [M, H] bf16 tail output - or ``(out, normed_next)`` when
        ``next_norm_weight`` is given. The intermediate fp32 tensor of the
        unfused chain is never materialized.
    """
    return _launch_tail(
        x, residual, norm_weight, layer_scalar, eps, next_norm_weight, next_norm_eps
    )


def gemma4_fused_norm_add(
    x: torch.Tensor,
    residual: torch.Tensor,
    norm_weight: torch.Tensor,
    eps: float,
    ...
    The post-attention half of the Gemma4 residual chain: RMSNorm on the
    attention output followed by the aten bf16 residual add, in one kernel.

    Args:
        x: [M, H] bf16 - the attention (o_proj) output.
        residual: [M, H] bf16 - the pre-attention residual.
        norm_weight: [H] bf16 - post_attention_layernorm weight (plain
            multiplier convention, ``use_gemma=False``).
        eps: RMSNorm epsilon.
    ...
    """
    out = _launch_tail(x, residual, norm_weight, None, eps, None, 0.0)
