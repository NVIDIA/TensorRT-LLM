...
class ViTPatchGenerator(nn.Module):
    ...
    def forward(
            self,
            x: torch.Tensor,
            imgs_sizes: Optional[List[Tuple[int, int]]] = None) -> torch.Tensor:
        if imgs_sizes is not None:
            # Dynamic resolution: x is pre-rearranged patches [1, total_patches, C*P*P]
            patches = self.embedder(x)
            patches, _ = self.apply_pos_enc_dynamic(patches, imgs_sizes)
            patches = self.cls_token_dynamic(patches, imgs_sizes)
        else:
...
class VisionTransformer(nn.Module):
    ...
    def forward_features(
        ...
        x = self.patch_generator(x, imgs_sizes=imgs_sizes)
...
class RADIOVisionModelBase(nn.Module):
    ...
    def forward(
        ...
        y = self.model.forward_features(x, imgs_sizes=imgs_sizes)
...
class RADIOVisionModel(PreTrainedModel):
    ...
    def forward(self,
                x: torch.Tensor,
                imgs_sizes: Optional[List[Tuple[int, int]]] = None):
        return self.radio_model.forward(x, imgs_sizes=imgs_sizes)
