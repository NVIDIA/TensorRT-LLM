...
class MultiStreamNode:
    ...
    def __init__(self, node, in_edges):
...
class MultiStreamDAG:
    ...
    def create_dag_from_gm(self, gm: GraphModule):
        ...
        self.entry_node = MultiStreamNode(None, [])

        latest_inplace_stat = {}
        inplace_map = inplace_info()

        def get_latest_version(node):
            return node if node not in latest_inplace_stat else latest_inplace_stat[
                node]
                ...
                for arg in args:
                    if isinstance(arg, dict):
                        ...
                        changed = True
                                    ...
                                    torch.fx.Node) and arg.op != "placeholder":
                        args_new.append(arg)
                args = args_new

            in_edges = [get_latest_version(self.nodes[arg]) for arg in args]

            # For node without in edge, connect it to the entry
            if len(in_edges) == 0:
                in_edges.append(self.entry_node)
            ...
            if node.op == "call_function":
                ...
                if func in inplace_map:
                    for inplace_arg in inplace_map[func].values():
                        ...
                        latest_inplace_stat[self.nodes[
                            node.kwargs[inplace_arg]]] = vertex
    ...
    def assign_streams(self, max_num_streams):
        ...
        def calc_priority(node_id, distance):
        ...
        def pick_stream(start_time, node):
            if node.weight == 0:
        ...
        while not worklist.empty():
            ...
            start_time = max([in_edge.end_time for in_edge in node.in_edges])
    ...
    def optimize(self, max_num_streams):
...
def print_dag_to_dot(dag: MultiStreamDAG):
    global call_cnt
    COLORS = [
