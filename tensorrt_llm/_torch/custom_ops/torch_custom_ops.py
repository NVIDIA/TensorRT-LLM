...
class QuantizeE4M3PerTensorRunner(TunableRunner):
    ...
    AutoTuner will automatically select the fastest backend for each input shape.
    """
    ...
    def unique_id(self):
        return ("quantize_e4m3_per_tensor", )

    # Tactic mapping: index -> backend name
    _tactics_map = ["trtllm", "te"]

    def get_valid_tactics(self, inputs: List[torch.Tensor],
                          profile: OptimizationProfile, **kwargs) -> List[int]:
        """Return list of available backend indices."""
        tactics = [0]  # 0 = TRTLLM (always available)

        if self._check_te_available():
            tactics.append(1)  # 1 = TE

        return tactics

    def forward(self,
                inputs: List[torch.Tensor],
                tactic: int = 0) -> Tuple[torch.Tensor, torch.Tensor]:
        """
        Forward pass with backend selection.

        Args:
            inputs: [input_tensor]
            tactic: 0=TRTLLM, 1=TE
        ...
        # Map tactic index to backend name
        backend = self._tactics_map[tactic] if 0 <= tactic < len(
            self._tactics_map) else "trtllm"
...
def quantize_e4m3_per_tensor(
    ...
    Args:
        ...
        tune_max_num_tokens: Maximum number of tokens for tuning
    ...
    QuantizeE4M3PerTensorRunner.tuning_config.tune_max_num_tokens = tune_max_num_tokens
...
def _(input: torch.Tensor,
      tune_max_num_tokens: int = 8192) -> Tuple[torch.Tensor, torch.Tensor]:
