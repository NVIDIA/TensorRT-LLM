...
class W4A16GemmRunner(TunableRunner):
    ...
    def forward(self,
                ...
                do_preparation: bool = False,
                **kwargs) -> torch.Tensor:
        activation, weights_packed, scales, bias = inputs
...
def w4a16_gemm(input: torch.Tensor,
    ...
    kwargs = {"group_size": group_size, "zeros": zeros}
    _, best_tactic = tuner.choose_one("trtllm::w4a16_gemm::gemm",
                                      [w4a16_gemm_runner], tuning_config,
                                      [input, weight, scales, bias], **kwargs)

    return w4a16_gemm_runner(inputs=[input, weight, scales, bias],
                             tactic=best_tactic,
                             **kwargs)
