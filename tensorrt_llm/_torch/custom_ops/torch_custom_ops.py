...
def fused_moe(
    ...
    # Slot-indexed CUDA-graph LoRA inputs (mutually exclusive with the per-request schema).
    # Each slot_*_ranks   : CPU pinned int32 [max_lora_size]
    # Each slot_*_weight_ptrs : CPU pinned int64 [max_lora_size, 3]  (A_ptr, B_ptr, dora_ptr)
    ...
    try:
        if min_latency_mode:
            output = run_moe(input, token_selected_experts, token_final_scales,
                             ...
                             input_sf, swizzled_input_sf, swiglu_alpha,
                             swiglu_beta, swiglu_limit, tp_size, tp_rank,
