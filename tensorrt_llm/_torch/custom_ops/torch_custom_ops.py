...
def _init_nccl_init_workaround() -> bool:
    ...
    NCCL_RUNTIME_CONNECT=0 in opUtils.cpp::getComm and ncclCommunicator.cpp)
    deadlocks during ncclCommInitRank on GB10 (DGX Spark) and on x86_64 hosts
    that have no MNNVL/IMEX fabric (B300 SM103 here).  Mirrors the PP-side
    workaround landed for nvbugs/6322073.  No-op on NCCL >= 2.30.4 and on
    other platforms.  Honors user-set values via os.environ.setdefault.
    """
