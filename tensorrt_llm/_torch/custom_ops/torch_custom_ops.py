...
class Fp8PrequantizedGemmRunner(TunableRunner):
    """Runs DeepGEMM FP8 GEMM with a pre-quantized activation."""
    ...
    def forward(
        ...
        act_fp8, act_sf, weight, weight_scale = inputs
        output = torch.empty(
            (act_fp8.size(0), weight.size(0)),
...
@torch.library.custom_op("trtllm::fp8_prequantized_gemm", mutates_args=())
def fp8_prequantized_gemm(
    act_fp8: torch.Tensor,
    act_sf: torch.Tensor,
    weight: torch.Tensor,
    weight_scale: torch.Tensor,
    ...
    tuner = AutoTuner.get()
    gemm_runner = Fp8PrequantizedGemmRunner(output_dtype, disable_ue8m0_cast)
    _, best_tactic = tuner.choose_one(
        "trtllm::fp8_prequantized_gemm",
        [gemm_runner],
        Fp8PrequantizedGemmRunner.tuning_config,
        [act_fp8, act_sf, weight, weight_scale],
    )
    return gemm_runner(
        inputs=[act_fp8, act_sf, weight, weight_scale],
        tactic=best_tactic,
    )


@fp8_prequantized_gemm.register_fake
def _(
    act_fp8: torch.Tensor,
    act_sf: torch.Tensor,
    weight: torch.Tensor,
