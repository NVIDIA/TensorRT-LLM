...
class FusedMoE(nn.Module):
    ...
    MoE Op:
        ...
        trtllm_gen backend:
            Only support min latency mode now.
            Quant: fp8 block scales quant(1 * 128) and nvfp4 quant
    ...
    FusedMoE module:
        cutlass Backend
            min latency mode:
                ...
                equals to: dynamic quant + routing(topK, etc.) + gemm1 + swiglu + gemm2

            maxthroughput mode:
                routing(topK, etc.) [+ dynamic quant for fp8 qdq and nvfp4 ] [+ allgather when dp enabled and nvfp4 allgather enabled]  + FusedMoe Op [+ allReduce is self.reduce_results] + reducescatter
                equals to: dynamic quant + routing(topK, etc.) [+ allgather when dp enabled and nvfp4 allgather disabled] + scatter + gemm1 + swiglu + gemm2 + finalizeMoeRoute [+ allReduce is self.reduce_results] + reducescatter
    ...
    In min latency mode, there is no reduce in FusedMoE module, they shall be added in model definition.
    Min Latency mode doesn’t work with dp.
    """
    ...
    def __init__(
        ...
        *,
        ...
        self._config_compitable_checking()

    def _config_compitable_checking(self):
        if self.enable_alltoall:
            assert self.use_dp and self.parallel_size > 1,\
                "alltoall should only enabled with attention dp and parallel_size > 1"
