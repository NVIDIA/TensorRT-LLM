...
from tensorrt_llm.quantization.utils.fp4_utils import (
    ...
    get_shuffle_matrix_sf_a_row_indices, shuffle_matrix_a, shuffle_matrix_sf_a)
...
class FusedMoE(nn.Module):
    ...
    def create_weights(self):
        raise NotImplementedError(
            f"create_weights is not implemented in {self.__class__}.")

    def load_weights(self, weights):
        raise NotImplementedError(
            f"load_weights is not implemented in {self.__class__}.")
    ...
    def forward(
        ...
        raise NotImplementedError(
            f"forward is not implemented in {self.__class__}.")


def get_fusedMoe_cls(model_config):
    quant_config = model_config.quant_config
    moe_backend = model_config.moe_backend
    if moe_backend == "TRTLLM" and quant_config and quant_config.quant_mode.has_any_quant(
            exclude_kv_cache=True):
        return FusedMoETrtllm
    else:
        return FusedMoECutlass


class BaseFusedMoE(FusedMoE):
    """
    ...
    def _create_qdq_scale_weights(self):
        ...
        fc31_dequant = nn.Parameter(torch.empty(self.expert_size_per_partition,
                                                dtype=torch.float32),
                                    requires_grad=False)
        self.register_parameter("fc31_dequant", fc31_dequant)

        fc2_dequant = nn.Parameter(torch.empty(self.expert_size_per_partition,
                                               dtype=torch.float32),
                                   requires_grad=False)
        ...
        fc31_input_dequant = nn.Parameter(torch.tensor(1., dtype=torch.float32),
                                          requires_grad=False)
        self.register_parameter("fc31_input_dequant", fc31_input_dequant)
    ...
    def _create_block_scale_weights(self):
        ...
        w2_weight_scaling_factor = nn.Parameter(torch.empty(
            (self.expert_size_per_partition, cell_div(
    ...
    def _create_int4_weight_only_per_group_weights(self):
        ...
        fc2_act_scale = nn.Parameter(torch.empty(self.expert_size_per_partition,
                                                 1,
                                                 dtype=self.dtype),
                                     requires_grad=False)
        ...
        fc31_weight_scale = nn.Parameter(torch.empty(
            ...
            self.intermediate_size_per_partition * 2 * self.interleave[0],
            dtype=self.dtype),
                                         requires_grad=False)
        ...
        fc2_weight_scale = nn.Parameter(torch.empty(
            self.expert_size_per_partition,
            self.intermediate_size_per_partition // (128 * self.interleave[1]),
            self.hidden_size * self.interleave[1],
            dtype=self.dtype),
                                        requires_grad=False)
        self.register_parameter("fc2_weight_scale", fc2_weight_scale)

        fc31_alpha = nn.Parameter(torch.empty(self.expert_size_per_partition,
                                              1,
                                              dtype=torch.float32),
                                  requires_grad=False)
        self.register_parameter("fc31_alpha", fc31_alpha)

        fc2_alpha = nn.Parameter(torch.empty(self.expert_size_per_partition,
                                             1,
                                             dtype=torch.float32),
                                 requires_grad=False)
        self.register_parameter("fc2_alpha", fc2_alpha)

    def _create_nvfp4_scale_weights(self, block_scales_dtype,
        ...
        w3_w1_weight_scale = nn.Parameter(
            ...
            requires_grad=False)
        self.register_parameter("w3_w1_weight_scale", w3_w1_weight_scale)
        ...
        fc31_alpha = nn.Parameter(torch.ones(self.expert_size_per_partition,
                                             dtype=torch.float32),
                                  requires_grad=False)
        self.register_parameter("fc31_alpha", fc31_alpha)

        fc2_alpha = nn.Parameter(torch.ones(self.expert_size_per_partition,
                                            dtype=torch.float32),
                                 requires_grad=False)
        self.register_parameter("fc2_alpha", fc2_alpha)
    ...
    def _create_quant_scale_weights(self):
        raise NotImplementedError(
            f"_create_quant_scale_weights is not implemented in {self.__class__}."
        )
    ...
    def forward(
        ...
        return self._do_forward(x, router_logits, cutlass_min_latency_mode,
    ...
    def _do_forward(
        ...
        raise NotImplementedError("_do_forward is not implemented.")
    ...
    def _load_moe_weights(self, weights):
    ...
    def _load_quant_scales(self, weights):
        if self.quant_config and self.quant_config.quant_mode.has_any_quant(
                exclude_kv_cache=True):
    ...
    def load_weights(self, weights: List[Dict]):
        ...
        self._load_moe_weights(weights, self.weight_loading_mode,
                               self.initial_local_expert_ids)
    ...
    def _load_weight_scale(self, weights, experts, has_second_scale):
        load_weigths_scales = {}
        for expert_id in experts:
            expert_scales = {}
            if self.weight_loading_mode == MoEWeightLoadingMode.VANILLA:
            ...
            expert_scales["w1"] = w1_weight_scale
            expert_scales["w2"] = w2_weight_scale
            expert_scales["w3"] = w3_weight_scale
            if has_second_scale:
                expert_scales["w1_2"] = w1_weight_scale_2
                expert_scales["w2_2"] = w2_weight_scale_2
                expert_scales["w3_2"] = w3_weight_scale_2

            load_weigths_scales[expert_id] = expert_scales

        return load_weigths_scales

    def _load_fp8_block_scales_scales(self, weights: Dict):
        all_w1_scales, all_w2_scales, all_w3_scales = [[
    ...
    def _load_fp8_qdq_scales(self, weights: Dict):
        ...
        def load_expert_fc31_scale_fp8_qdq(w1_input_scale, w3_input_scale,
                                           dst_fc31_input_scale: torch.Tensor):
            dst_fc31_input_scale.copy_(
                max(w1_input_scale[...].reshape([]),
                    w3_input_scale[...].reshape([])))

        def load_expert_fc2_scale_fp8_qdq(w2_input_scale,
        ...
        loaded_scales = self._load_input_scales(weights,
                                                range(self.num_experts))
        for expert_id in range(self.num_experts):
            load_expert_fc31_scale_fp8_qdq(loaded_scales[expert_id]["w1"],
                                           loaded_scales[expert_id]["w3"],
                                           tmp_fc31_input_scale[expert_id])

            load_expert_fc2_scale_fp8_qdq(loaded_scales[expert_id]["w2"],
        ...
        loaded_scales = self._load_weight_scale(weights,
                                                self.initial_local_expert_ids,
                                                False)
        for local_slot_id, expert_id in enumerate(
                self.initial_local_expert_ids):
    ...
    def _load_nvfp4_scales(self, weights: Dict):
        ...
        loaded_scales = self._load_input_scales(weights,
                                                range(self.num_experts))
        for expert_id in range(self.num_experts):
            load_expert_fc31_input_scale_nvfp4(loaded_scales[expert_id]["w1"],
                                               loaded_scales[expert_id]["w3"],
                                               tmp_fc31_input_scale[expert_id])
            load_expert_fc2_input_scale_nvfp4(loaded_scales[expert_id]["w2"],
                                              tmp_fc2_input_scale[expert_id])
        ...
        for local_slot_id, expert_id in enumerate(
            ...
            load_expert_w2_weight_scale_nvfp4(
                loaded_scales[expert_id]["w2"],
                self.w2_weight_scale.data[expert_idx], self.is_trtllm())

            load_expert_fc31_alpha_nvfp4(loaded_scales[expert_id]["w1_2"],
                                         loaded_scales[expert_id]["w3_2"],
    ...
    def _load_int4_groupwise_scales(self, weights: Dict):
        ...
        all_w1_input_scales, all_w2_input_scales, all_w3_input_scales = [[
    ...
    def _post_process_weights(self):
        raise NotImplementedError("_post_process_weights is not implemented.")
...
class FusedMoETrtllm(BaseFusedMoE):
    ...
    def _post_process_weights(self):
        if self.quant_config.quant_mode.has_nvfp4():
    ...
    def _create_quant_scale_weights(self):
        ...
        if self.quant_config and self.quant_config.quant_mode.has_any_quant(
            ...
            if qc.quant_mode.has_fp8_qdq():
                ...
                self._create_int4_weight_only_per_group_weights()
            elif qc.quant_mode.has_nvfp4():
                block_scales_dtype = torch.float8_e4m3fn
                block_scales_vec_size = 1
                self._create_nvfp4_scale_weights(block_scales_dtype,
                                                 block_scales_vec_size)
...
class FusedMoECutlass(BaseFusedMoE):
