...
class FusedMoE(nn.Module):
    ...
    def load_weights(self, weights: List[Dict]):
        ...
        for expert_id in range(self.expert_start, self.expert_end):
            ...
            thread = threading.Thread(target=load_expert_w3_w1_weight, args=(w1_weight, w3_weight,
                                     self.w3_w1_weight.data[expert_idx],
                                     is_trtllm_nvfp4))
            thread.start()
            threads.append(thread)

            thread = threading.Thread(target=load_expert_w2_weight, args=(w2_weight, self.w2_weight.data[expert_idx],
                                  is_trtllm_nvfp4))
            thread.start()
            threads.append(thread)
