...
_FUSED_EPILOGUE_ENV = "TRTLLM_LOW_M_GEMM_FUSED_EPILOGUE"

# M-dimension profiles generated by the TRT-LLM AutoTuner.
_M_TUNING_BUCKETS: tuple[int, ...] = (1, 2, 3, 4, 5, 6, 7, 8, 16, 32)
# low_m_bf16_direct uses the exact runtime M as its row tile through this bound.
_MAX_EXACT_DIRECT_M = 8


def _map_m_to_tuning_bucket(m: int) -> int:
    """Keep row-specialized direct shapes exact and bucket larger M values."""
    if m <= _MAX_EXACT_DIRECT_M:
        return m
    return next((b for b in _M_TUNING_BUCKETS if b >= m), _M_TUNING_BUCKETS[-1])
...
_M_DIM_SPEC = DynamicTensorSpec(
    ...
    map_to_tuning_buckets=_map_m_to_tuning_bucket,
)
...
_FUSED_EPILOGUE_ENABLED_VALUES = frozenset({"1", "auto", "on", "true"})
_FUSED_EPILOGUE_DISABLED_VALUES = frozenset({"", "0", "none", "off", "false"})
...
def low_m_gemm_fused_epilogue_enabled() -> bool:
    """Return whether low-M fused epilogues are explicitly enabled."""
    raw = os.environ.get(_FUSED_EPILOGUE_ENV, "off").strip().lower()
    if raw in _FUSED_EPILOGUE_ENABLED_VALUES:
        return True
    if raw in _FUSED_EPILOGUE_DISABLED_VALUES:
        return False
    raise ValueError(
        f"Invalid {_FUSED_EPILOGUE_ENV}={os.environ.get(_FUSED_EPILOGUE_ENV)!r}; "
        "expected one of: off, auto, on, true."
    )
...
class _DirectGemmRunner(TunableRunner):
    ...
    **Bias is not supported** — ``get_valid_tactics`` returns ``[]`` when
    ...
    def is_tactic_compatible(
        ...
        """Return whether a cached direct tactic can serve the runtime shape."""
        ...
        try:
            validate_tactic(DirectTactic(*tactic), m, n, k)
...
class LowMGemmDispatcher:
    ...
    def apply(
        ...
        # Larger M values still share coarse buckets. Revalidate direct tactics
        # before launching the tail-less kernel and use the guaranteed baseline
        # when a cached row tile is incompatible with the runtime M.
        if best_runner is self._direct and not self._direct.is_tactic_compatible(
            [input_2d, weight_t], best_tactic
        ):
            best_runner = cublas_runner
            best_tactic = -1
