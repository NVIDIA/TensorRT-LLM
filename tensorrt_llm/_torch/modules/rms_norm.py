    ...
    def forward(
        ...
        if self.is_nvfp4 and residual is not None and not self.use_gemma:
            ...
            if nvfp4_scale is None:
                ...
                # res_fp32 = residual.to(torch.float32)
                # ref_residual_out = (hs_fp32 + res_fp32).to(hidden_states.dtype)
                # variance = (hs_fp32 + res_fp32).pow(2).mean(-1, keepdim=True)
                # normed_fp32 = (hs_fp32 + res_fp32) * torch.rsqrt(variance + self.variance_epsilon)
                # normed_fp32 = normed_fp32 * self.weight.to(torch.float32)
                # normed = normed_fp32.to(hidden_states.dtype)

                # ref_fp4, ref_sf = torch.ops.trtllm.fp4_quantize(
                #     normed.contiguous(),
                #     nvfp4_scale,
                #     16,  # scaling_vector_size for NVFP4
                #     False,  # sfUseUE8M0 (NVFP4 uses UE4M3)
                # )

                # hidden_states_fused = Fp4QuantizedTensor(
                #     ref_fp4, ref_sf)
                # if return_residual:
                #     return hidden_states_fused, ref_residual_out
                # else:
