...
class KDAKernelDispatch:
    ...
    def prefill_chunk_kda(
        ...
        *,
        ...
        this replays the remaining preprocessing that FLA's
        ``ChunkKDAFunction.forward`` performs when the caller enables
        ``use_beta_sigmoid_in_kernel``, ``use_gate_in_kernel``, and
        ``state_v_first``; then dispatches to the in-tree
        ``trtllm::kda_prefill`` CuTe DSL op.
