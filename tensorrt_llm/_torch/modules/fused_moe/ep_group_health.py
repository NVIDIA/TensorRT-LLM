...
class EPGroupHealth:
    ...
    Args:
        ep_size: Total number of ranks in the EP group. Must be ``> 0``.

    Raises:
        ValueError: If ``ep_size <= 0``.
    """

    def __init__(self, ep_size: int) -> None:
        if ep_size <= 0:
            raise ValueError(f"ep_size must be > 0, got {ep_size}")
        # _ep_size and _all_active_mask are set once here and never mutated;
        # they are therefore safe to read without holding _lock (e.g. in
        # _validate_rank, ep_size property).
        self._ep_size: int = ep_size
        self._all_active_mask: int = (1 << ep_size) - 1
        self._active_mask: int = self._all_active_mask
        self._active_count: int = ep_size
    ...
    def ep_size(self) -> int:
        """Total number of ranks in the EP group (immutable)."""
    ...
    def mark_failed(self, rank: int) -> bool:
        ...
        Args:
            rank: Index of the rank to mark, in ``[0, ep_size)``.
        ...
        Raises:
            ValueError: If ``rank`` is outside ``[0, ep_size)``.
        """
    ...
    def get_mask_words(self, num_words: int = EP_MASK_NUM_WORDS) -> tuple[int, ...]:
        ...
        Args:
            num_words: Number of 64-bit words to produce. Must be large enough
                to hold ``ep_size`` bits (``num_words * 64 >= ep_size``).
        ...
        if num_words * 64 < self._ep_size:
            raise ValueError(f"num_words={num_words} cannot represent ep_size={self._ep_size}")
        word_mask = (1 << 64) - 1
        with self._lock:
    ...
    def __len__(self) -> int:
        """Return the EP group size (``ep_size``), not the active count.
    ...
    def __repr__(self) -> str:
        with self._lock:
            return (
                f"EPGroupHealth(ep_size={self._ep_size}, "
