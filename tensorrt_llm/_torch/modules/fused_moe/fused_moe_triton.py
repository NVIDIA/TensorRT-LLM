...
class TritonMXFP4FusedMoEMethod(TritonUnquantizedFusedMoEMethod):
    ...
    def _swizzle_and_replace(module, weight_name, scale_name, weight_data,
                             scale_data):
        """Swizzle weight/scale pair, free old parameters, assign new buffers."""
        new_weight, new_scale = swizzle_weight_and_scale(
            weight_data, scale_data)
        for name in (weight_name, scale_name):
            old = module._parameters.pop(name, None)
            old.data.storage().resize_(0)
        torch.cuda.empty_cache()
    ...
    def post_load_weights(self, module: torch.nn.Module):
        # When using dummy/random weights (load_format="dummy"), load_quant_scales
        # is never called, so MXFP4 weights remain as raw uint8 Parameters without
        # the layout transformation required by triton_kernels matmul_ogs.
        # Detect this case and swizzle weights into the expected format.
