...
class GatedMLP(nn.Module):
    ...
    def _fused_gate_up_swiglu(self, x):
        """Fused FC1 GEMM + SwiGLU using CuteDSL dense kernel (BF16 output).
        ...
        Returns:
            Output tensor [m, intermediate_size/tp] in bfloat16.
        """
        ...
        # Fused GEMM + SwiGLU: weight has N = 2*intermediate_size/tp columns,
        # kernel produces output with N/2 columns after SwiGLU
    ...
    def _fused_gate_up_swiglu_fp4out(self, x):
        """Fused FC1 GEMM + SwiGLU with FP4 output using CuteDSL.

        Produces FP4 output with scale factors, eliminating the bf16→fp4
        requantization between FC1 and FC2 (down_proj).

        Args:
            x: Input tensor or Fp4QuantizedTensor.

        Returns:
            Fp4QuantizedTensor with fused SwiGLU output in FP4 format.
        """
        module = self.gate_up_proj
        ...
        # Fused GEMM + SwiGLU with FP4 output + SFC quantization
