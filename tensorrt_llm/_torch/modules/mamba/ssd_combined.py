...
def _get_flashinfer_ssd_kernel(chunk_size, nheads, headdim, dstate, ngroups,
    ...
    kernels per mode.
    """
    # Imported lazily: flashinfer.mamba exports SSDCombined only when its own
    # ``import cutlass.cute`` succeeds, so importing it at module scope would
    # turn an optional symbol into a hard requirement for importing this file.
    # Callers only reach here on SM100+ with TRTLLM_USE_MAMBA_FI_SSD=1, i.e.
    # exactly where the symbol is exported.
