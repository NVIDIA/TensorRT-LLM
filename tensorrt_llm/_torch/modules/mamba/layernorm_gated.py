...
def _pdl_device_properties(device_index: int) -> tuple[int, int]:
    """SM count and SM version of the device the kernel will run on."""
    props = torch.cuda.get_device_properties(device_index)
    return props.multi_processor_count, props.major * 10 + props.minor


def _multirow_pdl(grid_size: int, device_index: int) -> bool:
    """Use PDL only when the launch grid cannot cover all SMs by itself."""
    num_sms, sm_version = _pdl_device_properties(device_index)
    return (grid_size < num_sms and sm_version >= 90
