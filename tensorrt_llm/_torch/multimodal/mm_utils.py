...
import torch
from torch.multiprocessing.reductions import rebuild_cuda_tensor, reduce_tensor, reduce_storage, rebuild_tensor, rebuild_meta_tensor, rebuild_storage_filename, rebuild_typed_storage
...
class SharedTensorContainer:
    ...
    @staticmethod
    def cpu_handle_to_dict(meta_data: Dict[str, Any], storage_metadata: Dict[str, Any]) -> Dict[str, Any]:
    ...
    def dict_to_cpu_handle(tensor_info: Dict[str, Any]) -> Tuple:
        ...
        try:
            ...
            storage_handle = base64.b64decode(tensor_info['storage_handle'])
            storage_metadata = (torch.storage.TypedStorage, 
                                manager_handle, 
                                storage_handle, 
            ...
            if not isinstance(storage, torch.storage.TypedStorage):
                storage = rebuild_typed_storage(storage, eval(tensor_info['storage_dtype']))

            meta_data = (tensor_info['tensor_storage_offset'],
                         tuple(tensor_info['tensor_size']),
                         tuple(tensor_info['tensor_stride']),
                         False) # requires_grad is always False for cpu tensor
            tensor_handle = (torch.Tensor,
                             storage,
                             meta_data)
            return tensor_handle
