...
def get_compiled__reg(tpl, dump_dir=None, pdl=False):
    ...
    if compiled is None:
        ...
        kernel = GvrTopkRegKernel(blk, vpt, minb, kpt, cur, deg, img, nbh, pdl=pdl)
...
class GvrClusKernel:
    ...
    def __init__(
        self, blk: int, u: int, minb: int, nbs: int, cs: int, scap: int = 8192, cmp_: int = 2048
    ):
    ...
    def __call__(
        ...
        b = logits.shape[0]
        self.kern(logits, pre_idx, out, n, npad, k, SCAP, CMP, SMP, TGT, Q, SS2, TGT2).launch(
            grid=(self.cs, b, 1),
            block=(self.blk, 1, 1),
            cluster=(self.cs, 1, 1),
            stream=stream,
...
def get_compiled__clus(tpl, scap: int = 8192, cmp_: int = 2048, options_extra: str = ""):
