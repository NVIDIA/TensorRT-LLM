...
class PhasedFmha(Fmha):
    ...
    def _get_total_num_blocks(
        ...
        if isinstance(kv_cache_manager, KVCacheManagerV2):
            try:
                return int(
                    kv_cache_manager.impl.get_page_index_upper_bound(local_layer_idx, Role.KEY)
                )
            except IndexError:
                # Not every V2 manager registers a buffer under the generic `Role.KEY`;
                # DSv4 keys its buffers by (layer, attention type) and allocates none.
                # `blocks_in_primary_pool` is the manager's own answer for its layout.
