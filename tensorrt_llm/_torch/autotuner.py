...
class AutoTuner:
    ...
    def _optimization_profiles(
        ...
        for spec in tuning_config.dynamic_tensor_specs:
            ...
            opt_shapes.add(spec.map_to_tuning_buckets(base_profile.shapes[spec.input_idx][spec.dim_idx].val))
    ...
    def _generate_all_configs(
        ...
        if not tuning_config.configs:
            return [{}]
        print(f"not tuning_config.configs: {not tuning_config.configs}")
        print(f"tuning_config.configs: {tuning_config.configs}")
        print(
            f"tuning_config.configs.values(): {tuning_config.configs.values()}")
