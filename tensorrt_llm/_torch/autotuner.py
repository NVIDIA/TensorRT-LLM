    ...
    Args:
        ...
        constraint_specs (Tuple[ConstraintSpec]): Specifications for constraints on tensor dimensions.
            ...
            Example:
                ...
                ...     )
                ... )
        configs (Dict[str, Any]): A dictionary of configuration values for each key.
            The keys are the names of the configuration parameters, and the values are the
            values to try for each parameter.

            Example:
                >>> config = TuningConfig(
                ...     configs={"batch_size": [1, 2, 4], "sequence_length": [128, 256, 512]}
                ... )
...
class AutoTuner:
        ...
        **kwargs,
        ...
        for p in profiles:
            ...
            if not is_cache_hit:
                ...
                best_runner_id, best_tactic, best_config, has_tuning_failure_occured = self._profile_runners(
    ...
    def _profile_runners(self, custom_op: str, runners: List[TunableRunner],
        ...
        return best_runner_id, best_tactic, best_config, has_tuning_failure_occured
    ...
    def _profile_single_kernel(self, runner: TunableRunner,
        ...
        logger.debug(
            f"[Autotuner] Profiled runner={runner}, tactic={tactic}, shapes={shapes}{f', {config}' if config else ''}: {avg_time:.6f}ms."
        )
    ...
    def print_profiling_cache(self):
        ...
        for key, value in self.profiling_cache.items():
            runner_id, tactic, config, profile = value
            logger.debug(
                f"[Autotuner] {key}: (runner_id={runner_id}, tactic={tactic}{f', {config}' if config else ''})"
            )
