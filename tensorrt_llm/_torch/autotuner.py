...
class TunableRunner(ABC):
    ...
    # Declarative opt-in for multiprocess tactic profiling.
    #
    # Set this to True on runners whose per-tactic cost is dominated by a
    # (Python) JIT compilation step -- e.g. CuTe DSL / Triton style kernels.
    # For such runners the autotuner can compile candidate tactics concurrently
    # in worker subprocesses while keeping the actual timed measurement
    # serialized on the GPU, which overlaps the expensive compiles and cuts
    # tuning wall time substantially.
    #
    # It is left False by default because the speedup only exists when compile
    # dominates, and because subprocess profiling additionally requires that the
    # runner be picklable (it is sent to workers via the "spawn" start method)
    # and that its kernel timing not depend on the *values* of the input tensors
    # (workers reconstruct inputs from shape/stride/dtype metadata and fill them
    # with synthetic data). Runners that need finer, per-tactic control can
    # override should_profile_tactic_in_subprocess() instead.
    tactic_compile_dominates: bool = False
    ...
    def should_profile_tactic_in_subprocess(
        ...
        **kwargs,
        ...
        The default delegates to the declarative ``tactic_compile_dominates``
        flag, so most runners only need to set that attribute to opt in. Runners
        that want per-tactic granularity (e.g. only delegate split-K tactics)
        can override this method.
        """
        return self.tactic_compile_dominates
...
def autotune(tune_mode: bool = True,
    ...
    try:
        ...
        if autotune_enabled:
            ...
            # Tuning is over, so release any multiprocess profiling workers.
            # Each worker holds its own CUDA context (hundreds of MB of device
            # memory), which must not outlive the tuning phase -- otherwise a
            # long-lived server would keep them alive until process exit.
            autotuner.shutdown_subprocess_pools()
...
class AutoTuner:
    ...
    def __init__(self, warmup=2, repeat=10, stream_delay_micro_secs=1000):
        ...
        # Persistent subprocess worker pools for multiprocess tactic profiling,
        # keyed by CUDA device index (None for CPU-only). The pools are created
        # lazily on first use and reused across choose_one() / _profile_runners
        # calls so that the per-worker CUDA context initialization cost is paid
        # only once for the whole tuning session.
        # device_index -> list of single-worker pools ("shards"). One shard per
        # worker so tactics can be pinned to a fixed process (sticky affinity),
        # which preserves each worker's process-local compiled-kernel cache
        # across shape buckets.
        self._subprocess_pools: Dict[Optional[int],
                                     List[ProcessPoolExecutor]] = {}
        # atexit cleanup is registered lazily on first pool creation so that
        # transient AutoTuner instances (e.g. those created inside workers) do
    ...
    def _profile_runners(
        ...
        **kwargs,
        ...
        # Keep the pre-hook tensors around: subprocess profiling serializes
        # these and applies inputs_pre_hook once inside the worker, exactly
        # mirroring the local path below (applying the hook here *and* in the
        # worker would run it twice, which breaks shape-changing or
        # non-idempotent hooks).
        pre_hook_input_tensors = input_tensors
        ...
        if total_pairs == 1 and tuning_config.distributed_tuning_strategy not in (
            ...
            for runner_id, runner, runner_arg_names, all_valid_tactics in candidates:
                ...
                # Tactics that opt into multiprocess profiling are compiled and
                # timed in worker subprocesses; the rest stay on the local path.
                subprocess_tactics = self._get_subprocess_tactics(
                    custom_op, runner, input_tensors, valid_tactics,
                    tuning_config, **kwargs)

                for tac in valid_tactics:
                    # Tactic objects may be unhashable (e.g. dict configs), so
                    # use list membership (==) rather than a set.
                ...
                # Profile the opted-in tactics concurrently in worker
                # subprocesses. Pass the pre-hook tensors: the worker applies
                # inputs_pre_hook itself, mirroring the local path.
                for tac, time_measured, error in self._profile_tactics_in_subprocesses(
                        custom_op,
                        runner,
                        runner_id,
                        pre_hook_input_tensors,
                        subprocess_tactics,
                        tuning_config,
                        **kwargs,
                ):
    ...
    def _is_subprocess_profiling_enabled(self,
        ...
        """Whether multiprocess tactic profiling is allowed in this context.

        Opt-in: disabled unless TLLM_AUTOTUNER_MULTIPROCESS=1 is set explicitly.
        With it unset the autotuner keeps its previous single-process behaviour,
        so enabling this feature is an explicit, reversible decision.
        """
        if os.getenv("TLLM_AUTOTUNER_MULTIPROCESS", "0") != "1":
            return False

        # Multi-rank (MPI) tuning already coordinates work across ranks and
        # relies on collective barriers; spawning extra processes under it is
        # unsafe, so subprocess profiling is restricted to single-process tuning.
        #
        # Note this is deliberately stricter than _is_distributed(), which only
        # looks at tp_size: a PP-only or CP-only job (tp_size == 1) is still a
        # multi-rank MPI job that must not spawn extra worker processes.
    ...
    def _get_subprocess_tactics(
        ...
        **kwargs,
        ...
        """Select the subset of tactics to profile in worker subprocesses."""
        ...
        # Below the threshold the spawn + per-worker CUDA-context cost is not
        # amortized, so fall back to local profiling for those few tactics.
    ...
    def _get_subprocess_worker_count(self) -> int:
        """Resolve the persistent pool size (independent of tactic count)."""
    ...
    def _get_subprocess_profile_lock_path(
        ...
        """Path of the cross-process file lock that serializes measurements."""
    ...
    def _get_persistent_pool(
            self, device_index: Optional[int]) -> List[ProcessPoolExecutor]:
        """Return the persistent worker shards for the given device.

        Each shard is a single-worker ProcessPoolExecutor, so a tactic can be
        pinned to one specific worker process for the whole tuning session
        (see _tactic_shard_index). That matters because JIT-compiled runners
        keep their compiled kernels in a *process-local* cache: an op is tuned
        once per shape bucket, and if a tactic's repeated profiles landed on
        different workers each time, every worker would recompile it and the
        parallel win would be cancelled out by redundant compiles.

        Shards are created lazily and reused across the whole tuning session so
        per-worker CUDA-context init is paid only once.
        """
        shards = self._subprocess_pools.get(device_index)
        if shards is not None:
            return shards

        worker_count = self._get_subprocess_worker_count()
        ctx = multiprocessing.get_context("spawn")
        shards = [
            ProcessPoolExecutor(max_workers=1, mp_context=ctx)
            for _ in range(worker_count)
        ]
        self._subprocess_pools[device_index] = shards
        if not self._atexit_registered:
            atexit.register(self.shutdown_subprocess_pools)
            self._atexit_registered = True
        self._debug_logger(
            f"[Autotuner] Created persistent subprocess pool with "
            f"{worker_count} single-worker shards for device_index={device_index}."
        )
        return shards

    @staticmethod
    def _tactic_shard_index(tactic: Any, num_shards: int) -> int:
        """Map a tactic to a fixed worker shard.

        Deterministic within a process (uses repr, not hash(), because str/tuple
        hashing is salted per process for some types), so the same tactic always
        lands on the same worker and hits that worker's compiled-kernel cache on
        subsequent shape buckets.
        """
        if num_shards <= 1:
            return 0
        digest = 0
        for ch in repr(tactic):
            digest = (digest * 131 + ord(ch)) & 0xFFFFFFFF
        return digest % num_shards

    def shutdown_subprocess_pools(self) -> None:
        """Shut down all persistent worker pools. Safe to call repeatedly."""
        pools, self._subprocess_pools = self._subprocess_pools, {}
        for shards in pools.values():
            for pool in shards:
                try:
                    pool.shutdown(wait=True)
                except (OSError, RuntimeError) as e:
                    self._debug_logger(
                        f"[Autotuner] Ignoring error while shutting down a "
                        f"subprocess profiling shard: {e!r}")
    ...
    def _profile_tactics_in_subprocesses(
        ...
        runner: TunableRunner,
        runner_id: int,
        pre_hook_input_tensors: List[torch.Tensor],
        tactics: List[Any],
        tuning_config: TuningConfig,
        **kwargs,
    ) -> List[Tuple[Any, float, Optional[str]]]:
        """Profile the given tactics across the persistent worker pool.

        ``pre_hook_input_tensors`` are the inputs *before* ``inputs_pre_hook``
        has been applied: each worker reconstructs them from metadata and
        applies the hook itself exactly once, mirroring the local path. The
        worker then compiles the tactic under a shared lock (so compiles
        overlap) and measures it under an exclusive lock (so timed windows never
        overlap on the GPU). Returns one (tactic, time_ms, error) per tactic.
        """
        if not tactics:
            return []
        ...
        input_specs = [
            _serialize_subprocess_tensor_spec(t) for t in pre_hook_input_tensors
        ]
        worker_config = {
            "custom_op": custom_op,
            "runner": runner,
            ...
            # Propagate inputs_pre_hook so subprocess profiling sees the same
            # (massaged) input distribution as the local path.
        ...
        self._debug_logger(
            ...
            f"in persistent subprocess pool; "
            f"profile_lock_path={profile_lock_path}.")

        results: List[Optional[Tuple[Any, float,
                                     Optional[str]]]] = [None] * len(tactics)
        fallback_local = os.getenv(
            "TLLM_AUTOTUNER_MP_FALLBACK_LOCAL_ON_PROCESS_FAILURE", "1") != "0"

        try:
            shards = self._get_persistent_pool(device_index)
            process_failure_error = None
            futures = {}
            for idx, tactic in enumerate(tactics):
                # Pin each tactic to a fixed worker so its compiled kernel is
                # reused when the same tactic is profiled again for another
                # shape bucket (the compiled-kernel cache is process-local).
                shard = shards[self._tactic_shard_index(tactic, len(shards))]
                future = shard.submit(_profile_tactic_in_subprocess,
            ...
            for future in as_completed(futures):
                ...
                try:
                    ...
                    # A raised (rather than returned) error means the worker
                    # process itself died -- the pool may be broken now.
                    process_failure_error = repr(e)
                    results[idx] = (tactic, float('inf'), process_failure_error)

            if process_failure_error is not None:
                # A broken pool cannot be reused; drop it so the next call spawns
                # a fresh one.
                self._discard_pool(device_index)
                if fallback_local:
                    logger.warning_once(
                    ...
                    return self._profile_tactics_locally_after_subprocess_failure(
                        runner, pre_hook_input_tensors, tactics, tuning_config,
                        device_index, **kwargs)
        except Exception as e:
            error = repr(e)
            self._discard_pool(device_index)
    ...
    def _discard_pool(self, device_index: Optional[int]) -> None:
        """Drop the worker shards for a device, waiting for them to finish.

        The wait is deliberate: shutdown(wait=False) returns while surviving
        workers may still be inside their *timed* measurement window holding the
        exclusive profile lock. The caller typically continues with local
        profiling right after, and if that timing overlapped a worker's timing
        on the same GPU both measurements would be inflated -- silently
        poisoning the tactic that gets cached. Draining here restores the
        "only one timed window per device" invariant.
        """
        shards = self._subprocess_pools.pop(device_index, None)
        for pool in shards or ():
            try:
                pool.shutdown(wait=True)
            except (OSError, RuntimeError) as e:
                self._debug_logger(
    ...
    def _profile_tactics_locally_after_subprocess_failure(
        ...
        **kwargs,
        ...
        """Profile tactics locally after the subprocess path could not be used.

        Mirrors the worker: applies ``inputs_pre_hook`` exactly once and measures
        under the same exclusive device lock, so a fallback triggered while
        workers are still winding down cannot overlap their timed windows.
        Returns one (tactic, time_ms, error) triple per tactic.
        """
        # Apply inputs_pre_hook here (once), matching what the workers would
        # have done, since the caller hands us the pre-hook tensors.
        input_tensors = pre_hook_input_tensors
        if tuning_config.inputs_pre_hook is not None:
            input_tensors = tuning_config.inputs_pre_hook(input_tensors)

        # Take the same exclusive profile lock the workers use. Belt-and-braces
        # on top of the drain in _discard_pool: if any worker (or a concurrent
        # tuning process on this device) is still timing, we must not measure at
        # the same time or both timings are wrong.
        lock_path = self._get_subprocess_profile_lock_path(device_index)
        ...
        for tactic in tactics:
            try:
                ...
                # Clear any pending CUDA error left by the failed tactic so it
                # does not poison the measurements of the remaining tactics.
                try:
                    torch.cuda.synchronize()
                except Exception:
                    pass
    ...
    def _is_multi_rank(self) -> bool:
        """Whether this process is part of a multi-rank (MPI) job.

        Unlike _is_distributed(), which is TP-centric, this also catches
        PP-only / CP-only jobs where tp_size == 1 but several ranks still
        share the job. Used to gate multiprocess tactic profiling, which must
        never spawn worker processes underneath MPI ranks.
        """
        if self.mapping is None:
            return False
        world_size = getattr(self.mapping, "world_size", 1) or 1
        return world_size > 1 or self._is_distributed()
...
# ---------------------------------------------------------------------------
# Multiprocess tactic profiling helpers
#
# These module-level functions are used both in the parent process (to
# describe input tensors) and inside spawned worker processes (to rebuild the
# inputs and run the profiling). They are kept at module scope so they are
# importable by the "spawn" start method.
# ---------------------------------------------------------------------------


def _serialize_subprocess_tensor_spec(input_obj: Any) -> Dict[str, Any]:
    """Describe a tensor input by metadata so a worker can rebuild it.

    Only shape/stride/dtype/device are captured -- not the values -- which is
    why subprocess profiling is restricted to runners whose timing does not
...
def _dtype_from_subprocess_name(dtype_name: str) -> torch.dtype:
    """Resolve a torch dtype from the short name stored in a tensor spec."""
    return getattr(torch, dtype_name)


def _make_subprocess_tensor(spec: Dict[str, Any],
                            default_device_index: Optional[int]) -> Any:
    """Rebuild a tensor from a spec produced by
    _serialize_subprocess_tensor_spec, filled with synthetic data."""
...
def _subprocess_profile_lock(lock_path: Optional[str], exclusive: bool):
    """Cross-process file lock. A shared lock is held during compile/warmup so
    workers compile concurrently; an exclusive lock is held during the timed
    measurement so timed windows never overlap on the same device."""
...
def _profile_tactic_in_subprocess(
    ...
    """Worker entry point: rebuild inputs, compile under a shared lock, then
    measure under an exclusive lock. Returns (tactic, time_ms, error)."""
    # Guard against recursive subprocess spawning inside the worker.
    os.environ["TLLM_AUTOTUNER_MULTIPROCESS"] = "0"

    # Optional observability hook: when TLLM_AUTOTUNER_MP_DEBUG_PID_DIR is set,
    # each worker drops a marker file named by its PID. This lets tests/users
    # confirm that profiling really ran across multiple worker processes.
    debug_pid_dir = os.getenv("TLLM_AUTOTUNER_MP_DEBUG_PID_DIR")
    if debug_pid_dir:
        try:
            os.makedirs(debug_pid_dir, exist_ok=True)
            open(os.path.join(
                debug_pid_dir,
                f"pid_{os.getpid()}",
            ), "a").close()
    ...
    inputs_pre_hook = worker_config.get("inputs_pre_hook")
    ...
    try:
        ...
        # Mirror the local path: apply inputs_pre_hook once before profiling so
        # the worker sees the same massaged input distribution.
        if inputs_pre_hook is not None:
            inputs = inputs_pre_hook(inputs)

        # Compile and run the untimed warmup launches while holding a shared
        # lock. Multiple workers can compile concurrently, but measured
        # profiling windows take the lock exclusively and therefore do not
        # overlap with these untimed launches.
        #
        # Honor the configured warmup count so subprocess measurements start
        # from the same cache/clock state as local ones (at least one launch is
        # always needed to trigger the JIT compile).
        warmup_iters = max(1, int(worker_config.get("warmup", 1) or 0))
