...
def _parse_arch_str(arch_str: str) -> Tuple[int, int]:
    ...
    if not match:
        raise ValueError(f"Invalid SOL_ATTN_ARCH format: {arch_str!r} (expected e.g. '90', 'sm_90')")
...
def _get_device_capacity_cached(device: torch.device = None) -> Tuple[int, int]:
    ...
    Override with SOL_ATTN_ARCH (e.g. 'sm_90' or '90') for CPU-only compilation
    without a GPU present.
    """
    arch_override = os.environ.get("SOL_ATTN_ARCH")
