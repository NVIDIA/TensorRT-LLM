...
from ..common.selector import sol_attn_popc_b32, sol_attn_route_is_exact
from . import math as mma_utils
from .softmax import _load_m64_n128_score as _load_pair_score
from .softmax import _online_update_one_half as _online_update_pair
from .softmax import _rescale_m64_partial_o as _rescale_pair_o
from .tmem import (
...
def _store_pair_probability_chunked_tmemp(
    ...
    for chunk_idx in cutlass.range_constexpr(PAIR_P_CHUNKS):
        p_store_registers = cute.make_rmem_tensor(p_store_coordinates.shape, Float32)
        assert cute.size(p_store_registers) == PAIR_P_PACKED_REGISTERS_PER_THREAD_PER_CHUNK
        assert cute.size(probabilities) == 2 * cute.size(p_store_registers) * PAIR_P_CHUNKS
        ...
        for i in cutlass.range(cute.size(p_store_registers), unroll_full=True):
            ...
            if (lane & Int32(2)) == Int32(0):
                ...
                p_store_words[i] = _prmt_b32(own, peer, Int32(0x3276))

        destination_chunk = cute.make_tensor(
            destination.iterator + chunk_idx * PAIR_P_CHUNK_PACKED_COLUMNS,
            destination.layout,
        )
...
class SharedStorage:
    ...
    pack_k_mbar_ptr: cute.struct.MemRange[cutlass.Int64, PAIR_STAGES * 2]
    pack_v_mbar_ptr: cute.struct.MemRange[cutlass.Int64, PAIR_STAGES * 2]
    pair_score_mbar_ptr: cute.struct.MemRange[cutlass.Int64, 2]
    pair_o_mbar_ptr: cute.struct.MemRange[cutlass.Int64, 2]
    final_stats: cute.struct.Align[cute.struct.MemRange[Float32, M * 2], 128]
    route_partial: cute.struct.Align[cute.struct.MemRange[Float32, 4 * ROUTE_TILE_SIZE], 16]
    column_masks: cute.struct.Align[cute.struct.MemRange[Float32, ROUTE_TILE_SIZE], 16]
    route_packet: cute.struct.Align[cute.struct.MemRange[Int32, PACKET_WORDS], 16]
    ...
    route_indices: cute.struct.Align[cute.struct.MemRange[Int32, ROUTE_INDEX_CAPACITY], 16]
...
def _sol_attn_sm100_bf16_kernel(
    ...
    sFinalStats = storage.final_stats.get_tensor(cute.make_layout((M, 2)))
    route_partial = storage.route_partial.get_tensor(cute.make_layout((4, ROUTE_TILE_SIZE)))
    column_masks = storage.column_masks.get_tensor(cute.make_layout((ROUTE_TILE_SIZE,)))
    route_packet = storage.route_packet.get_tensor(cute.make_layout((PACKET_WORDS,)))
    route_indices = storage.route_indices.get_tensor(cute.make_layout((ROUTE_INDEX_CAPACITY,)))
    ...
    sPackKGather = cute.make_tensor(
        cute.recast_ptr(sPackK.iterator, pack_k_gather_layout.inner, BFloat16),
        pack_k_gather_layout.outer,
    )
    sPackVGather = cute.make_tensor(
        cute.recast_ptr(sPackV.iterator, pack_v_gather_layout.inner, BFloat16),
    ...
    score_loaded_barrier = pipeline.NamedBarrier(barrier_id=2, num_threads=4 * 32)
    final_stats_ready_barrier = pipeline.NamedBarrier(barrier_id=3, num_threads=4 * 32)
    pack_score_loaded_barrier = pipeline.NamedBarrier(barrier_id=4, num_threads=4 * 32)
    route_packet_ready_barrier = pipeline.NamedBarrier(barrier_id=5, num_threads=5 * 32)
    exact_pair_p_ready_barrier = pipeline.NamedBarrier(barrier_id=6, num_threads=5 * 32)
    ...
    pack_owner_threads = pipeline.CooperativeGroup(pipeline.Agent.Thread, 4 * 32)
    q_bytes = cute.size_in_bytes(BFloat16, cute.select(q_layout, mode=[0, 1, 2]))
    route_k_bytes = cute.size_in_bytes(BFloat16, cute.select(route_k_layout, mode=[0, 1, 2]))
    route_v_bytes = cute.size_in_bytes(BFloat16, cute.select(route_v_layout, mode=[0, 1, 2]))
    pack_k_bytes = cute.size_in_bytes(BFloat16, cute.select(pack_k_layout, mode=[0, 1, 2]))
    pack_v_bytes = cute.size_in_bytes(BFloat16, cute.select(pack_v_layout, mode=[0, 1, 2]))
    ...
    gPackK = cute.local_tile(mPackK_cur, (N_MEMBER, 64), (None, None))
    gPackV = cute.local_tile(mPackV_cur, (64, N_MEMBER), (None, None))
    ...
    pack_score_shape = tiled_pack_qk.partition_shape_C(PACK_QK_TILE[:2])
    ...
    pair_tP_storage = cute.make_tensor(pair_tScore.iterator, pack_p_layout.outer)
    pair_tP_base = tiled_pack_pv.make_fragment_A(pair_tP_storage)[None, None, None, 0]
    pair_tP = cute.make_tensor(
        pair_tP_base.iterator + tmem_base + tmem_base + Int32(PAIR_P_OFFSET * 2),
        pair_tP_base.layout,
    )
    q_producer = fa_pipeline.make_pipeline_state(pipeline.PipelineUserType.Producer, 1)
    q_consumer = fa_pipeline.make_pipeline_state(pipeline.PipelineUserType.Consumer, 1)
    pack_k_producer = pipeline.make_pipeline_state(pipeline.PipelineUserType.Producer, PAIR_STAGES)
    pack_k_consumer = pipeline.make_pipeline_state(pipeline.PipelineUserType.Consumer, PAIR_STAGES)
    pack_v_producer = pipeline.make_pipeline_state(pipeline.PipelineUserType.Producer, PAIR_STAGES)
    pack_v_consumer = pipeline.make_pipeline_state(pipeline.PipelineUserType.Consumer, PAIR_STAGES)
    pair_score_producer = fa_pipeline.make_pipeline_state(pipeline.PipelineUserType.Producer, 1)
    pair_score_consumer = fa_pipeline.make_pipeline_state(pipeline.PipelineUserType.Consumer, 1)
    pair_o_producer = fa_pipeline.make_pipeline_state(pipeline.PipelineUserType.Producer, 1)
    pair_o_consumer = fa_pipeline.make_pipeline_state(pipeline.PipelineUserType.Consumer, 1)
    route_start_base = Int32(0)
    q_len = token_count - q_block_idx * Int32(M)
    if q_len > Int32(M):
        q_len = Int32(M)
    threshold = Float32(mThreshold_bnh[batch_idx, q_block_idx, head_idx])
    ...
    num_logical_groups = (num_route_tiles + Int32(ROUTE_HALVES_PER_GROUP - 1)) // Int32(
    ...
    while logical_group_idx < num_logical_groups:
        is_final_logical_group = logical_group_idx + Int32(1) == num_logical_groups
        group_route_tile_base = logical_group_idx * Int32(ROUTE_HALVES_PER_GROUP)
        ...
        for half_idx in cutlass.range(physical_halves_this_group, unroll=1):
            route_tile_idx = cute.arch.make_warp_uniform(group_route_tile_base + half_idx)
            is_final_route_tile = route_tile_idx + Int32(1) == num_route_tiles
            is_logical_terminal_half = half_idx + Int32(1) == physical_halves_this_group
            ...
            if warp_idx == Int32(5):
                ...
                route_k_barrier = pack_k_pipe.producer_get_barrier(pack_k_producer)
                cute.copy(
                    tma_atom_kc,
            ...
            if is_owner:
                ...
                lane = owner_tidx % Int32(32)
                semantic_row = (score_coords[0][0] + Int32(SEMANTIC_ROW_OFFSET)) & Int32(M - 1)
                ...
                # 2*(col%2).
                for pair_idx in cutlass.range_constexpr(0, ROUTE_TILE_SIZE // 2, 2):
                    ...
                    peer_scaled0 = cute.arch.shuffle_sync_bfly(scaled0, offset=1)
                    peer_scaled1 = cute.arch.shuffle_sync_bfly(scaled1, offset=1)
                    partial0, partial1 = cute.arch.fma_packed_f32x2(
                        (raw_partial0, raw_partial1),
                        (softmax_scale_log2, softmax_scale_log2),
                        (peer_scaled0, peer_scaled1),
                    )
                    peer0 = cute.arch.shuffle_sync_bfly(partial0, offset=16)
                    peer1 = cute.arch.shuffle_sync_bfly(partial1, offset=16)
                    partial0, partial1 = cute.arch.add_packed_f32x2(
                        (partial0, partial1), (peer0, peer1)
                    )
                    peer0 = cute.arch.shuffle_sync_bfly(partial0, offset=8)
                    peer1 = cute.arch.shuffle_sync_bfly(partial1, offset=8)
                    partial0, partial1 = cute.arch.add_packed_f32x2(
                        (partial0, partial1), (peer0, peer1)
                    )
                    peer0 = cute.arch.shuffle_sync_bfly(partial0, offset=4)
                    peer1 = cute.arch.shuffle_sync_bfly(partial1, offset=4)
                    partial0, partial1 = cute.arch.add_packed_f32x2(
                        (partial0, partial1), (peer0, peer1)
                    )
                    if lane == Int32(0):
                        route_partial[owner_warp, 2 * pair_idx] = partial0
                        route_partial[owner_warp, 2 * (pair_idx + 1)] = partial1
                    if lane == Int32(2):
                        route_partial[owner_warp, 2 * pair_idx + 1] = partial0
                        route_partial[owner_warp, 2 * (pair_idx + 1) + 1] = partial1
                ...
                if owner_warp == Int32(0):
                    ...
                    for word in cutlass.range_constexpr(ROUTE_MASK_WORDS):
                        ...
                        word_mask = Int32(cute.arch.vote_ballot_sync(exact_pred))
                        ...
                        if cutlass.const_expr(word == 0):
                            ...
                            mask3 = word_mask
                        preceding_word_count = preceding_word_count + sol_attn_popc_b32(word_mask)
                ...
                if has_route_approx:
                    ...
                    for i in cutlass.range_constexpr(0, cute.size(score_raw), 2):
                    ...
                    local_max = fa_utils.fmax_reduce(route_scores.load(), arch=100)
                    ...
                    route_probabilities = cute.make_rmem_tensor(route_scores.shape, Float32)
                    if new_max == -Float32.inf:
                        for i in cutlass.range(cute.size(route_scores), unroll_full=True):
                    ...
                    for i in cutlass.range_constexpr(0, cute.size(route_probabilities), 2):
                        block_idx0 = route_start + score_coords[i][1]
                        raw_length0 = token_count - block_idx0 * Int32(N_MEMBER)
                        block_length0 = max(Int32(0), min(raw_length0, Int32(N_MEMBER)))
                        block_idx1 = route_start + score_coords[i + 1][1]
                        raw_length1 = token_count - block_idx1 * Int32(N_MEMBER)
                        block_length1 = max(Int32(0), min(raw_length1, Int32(N_MEMBER)))
                    ...
                    current_sum = fa_utils.fadd_reduce(route_scores.load(), arch=100)
                    current_sum += cute.arch.shuffle_sync_bfly(current_sum, offset=2)
            ...
            if warp_idx == Int32(0):
                ...
                if route_has_approx:
                    ...
                    if is_final_route_tile and Int32(route_packet[6]) == Int32(0):
            ...
            if is_owner:
                ...
                if is_final_route_tile and cumulative_exact_count == Int32(0):
        ...
        if warp_idx == Int32(5) and has_pair_exact:
            for pair_idx in cutlass.range(pair_count, unroll=1):
                ...
                pack_k_pipe.producer_acquire(pack_k_producer)
                pair_k_barrier = pack_k_pipe.producer_get_barrier(pack_k_producer)
                _load_pack_k_half(
                    tma_atom_pack_k,
                    tPackKgK,
                    tPackKsK,
        ...
        if warp_idx == Int32(0) and has_pair_exact:
            ...
            for pair_idx in cutlass.range(pair_count, unroll=1):
                ...
                if is_final_logical_group and pair_idx + Int32(1) == pair_count:
                    pair_o_pipe.producer_commit(pair_o_producer)
                mma_o_initialized = Int32(1)
                pack_v_pipe.consumer_release(pack_v_consumer)
                pack_v_consumer.advance()
        ...
        if is_owner and has_pair_exact:
            ...
            for pair_idx in cutlass.range(pair_count, unroll=1):
                ...
                semantic_row = (pair_coords[0][0] + Int32(SEMANTIC_ROW_OFFSET)) & Int32(M - 1)
                row_valid = semantic_row < q_len
                row_mask = -Float32.inf
                if row_valid:
                    row_mask = Float32(0.0)
                assert cute.size(pair_scores) % 2 == 0
                for i in cutlass.range_constexpr(0, cute.size(pair_scores), 2):
                    column0 = pair_coords[i][1]
                    column1 = pair_coords[i + 1][1]
                    mask0 = Float32(column_masks[column0])
                    mask1 = Float32(column_masks[column1])
                    mask0, mask1 = cute.arch.add_packed_f32x2((mask0, mask1), (row_mask, row_mask))
                    mask0, mask1 = cute.arch.add_packed_f32x2(
                        (
                ...
                probabilities, next_max, next_sum, row_alpha = _online_update_pair(
                    pair_scores,
                    running_max,
                    running_sum,
                    softmax_scale,
        ...
        logical_group_idx = cute.arch.make_warp_uniform(logical_group_idx + Int32(1))
    ...
    if is_owner:
        ...
        semantic_row0 = (
            owner_warp * Int32(16) + lane // Int32(4) + Int32(SEMANTIC_ROW_OFFSET)
        ) & Int32(M - 1)
        semantic_row1 = (semantic_row0 + Int32(8)) & Int32(M - 1)
        even_col_base = (lane % Int32(4)) * Int32(2)

        if semantic_row0 < q_len:
            inv_sum0 = cute.arch.rcp_approx(Float32(sFinalStats[semantic_row0, 0]))
            ...
            for word_i in cutlass.range(O_PACKED_WORDS_PER_ROW_PER_THREAD, unroll_full=True):
                even_i = word_i * 4
                odd_i = even_i + 1
                even_value = Float32(o_regs[even_i]) * inv_sum0
                odd_value = Float32(o_regs[odd_i]) * inv_sum0
                packed_word = _cvt_bf16x2_f32(Float32(odd_value), Float32(even_value))
                even_col = even_col_base + word_i * O_PACKED_COLUMN_STRIDE
                _store_global_u32_inline(destination_row0.iterator + even_col, packed_word)

        if semantic_row1 < q_len:
            inv_sum1 = cute.arch.rcp_approx(Float32(sFinalStats[semantic_row1, 0]))
            ...
            for word_i in cutlass.range(O_PACKED_WORDS_PER_ROW_PER_THREAD, unroll_full=True):
                ...
                packed_word = _cvt_bf16x2_f32(Float32(odd_value), Float32(even_value))
                even_col = even_col_base + word_i * O_PACKED_COLUMN_STRIDE
                _store_global_u32_inline(destination_row1.iterator + even_col, packed_word)

        if (lane & Int32(2)) == Int32(0) and owner_row < q_len:
            query_idx = q_block_idx * Int32(M) + owner_row
            mLSE_bth[batch_idx, query_idx, head_idx] = running_max + cute.math.log2(
...
def _sol_attn_sm100_bf16_host(
    ...
    q, k, v, o, kc, vc = tuple(assume_tensor_aligned(t) for t in (q, k, v, o, kc, vc))
    q_mkl, k_nkl, kc_nkl = [layout_utils.select(t, [1, 3, 2, 0]) for t in (q, k, kc)]
    v_nkl, vc_nkl = [layout_utils.select(t, [3, 1, 2, 0]) for t in (v, vc)]
    ...
    q_layout = sm100_utils.make_smem_layout_a(tiled_pack_qk, PACK_QK_TILE, BFloat16, 1)
    ...
    pack_p_layout = sm100_utils.make_smem_layout_a(tiled_pack_pv, PACK_PV_TILE, BFloat16, 1)
