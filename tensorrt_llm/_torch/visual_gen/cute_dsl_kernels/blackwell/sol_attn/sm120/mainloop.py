...
#
# The warp-MMA/TMA skeleton is adapted from NVIDIA cuDNN Frontend
# (https://github.com/NVIDIA/cudnn-frontend), Apache-2.0; its license text
# is vendored at sol_attn/sm120/LICENSE.cudnn-frontend.
...
from flash_attn.cute import utils as kernel_utils
...
class SolAttnForwardSm120:
    ...
    def kernel(
        ...
        threshold = cutlass.Float32(mThreshold[batch_idx, q_tile_idx, head_idx])
        ...
        Q_producer = pipeline.make_pipeline_state(pipeline.PipelineUserType.Producer, self.q_stage)
        Q_consumer = pipeline.make_pipeline_state(pipeline.PipelineUserType.Consumer, self.q_stage)
        K_producer = pipeline.make_pipeline_state(pipeline.PipelineUserType.Producer, self.kv_stage)
        K_consumer = pipeline.make_pipeline_state(pipeline.PipelineUserType.Consumer, self.kv_stage)
        V_producer = pipeline.make_pipeline_state(pipeline.PipelineUserType.Producer, self.kv_stage)
        V_consumer = pipeline.make_pipeline_state(pipeline.PipelineUserType.Consumer, self.kv_stage)

        sQ = storage.Q_smem.get_tensor(Q_smem_layout.outer, swizzle=Q_smem_layout.inner)
        sK = storage.K_smem.get_tensor(K_smem_layout.outer, swizzle=K_smem_layout.inner)
        sV = storage.V_smem.get_tensor(V_smem_layout.outer, swizzle=V_smem_layout.inner)
        ...
        route_f32_ptr = cute.recast_ptr(storage.Q_smem.data_ptr(), dtype=cutlass.Float32)
        route_i32_ptr = cute.recast_ptr(storage.Q_smem.data_ptr(), dtype=cutlass.Int32)
        ...
        gQ = cute.local_tile(mQ_slice, (M, D), coord=(q_tile_idx, 0))
        ...
        gO = cute.local_tile(mO_slice, (M, DV), coord=(q_tile_idx, 0))
        ...
        tSrS = cute.make_rmem_tensor(thr_mma_qk.partition_shape_C((M, N)), self.acc_dtype)
        tScS = thr_mma_qk.partition_C(cS)

        thr_mma_pv = tiled_mma_pv.get_slice(tidx)
        tOsV = thr_mma_pv.partition_B(sV)
        tOrV = tiled_mma_pv.make_fragment_B(tOsV[None, None, None, 0])
        tOrO = cute.make_rmem_tensor(thr_mma_pv.partition_shape_C((M, DV)), self.acc_dtype)
        ...
        for route_group in cutlass.range(0, num_route_groups, 1, unroll=1):
            ...
            if warp == 0:
                if cutlass.const_expr(self.prefetch_next_route_k):
                    ...
                    if route_group == 0:
                        ...
                        previous_group_exact_count = cutlass.Int32(route_meta[0])
            ...
            if warp == 0:
                ...
                lane_mask_lt = cutlass.Int32(0x7FFFFFFF) >> (cutlass.Int32(31) - lane)
                for word in cutlass.range_constexpr(2):
                    ...
                    if valid:
                        ...
                        col_mean = col_sum * scale_softmax_log2e / cutlass.Float32(q_len)
                    ...
                    column_masks[off] = (
                        -cutlass.Float32.inf if (exact or not valid) else cutlass.Float32(0.0)
                    )
        ...
        sO = storage.Q_smem.get_tensor(O_smem_layout.outer, swizzle=O_smem_layout.inner)
    ...
    def __call__(
        ...
        tma_atom_KC, tma_tensor_KC = cute.nvgpu.cpasync.make_tiled_tma_atom(
            g2s_op,
            kc_nkl,
            self.K_smem_layout,
            (N, D),
        ...
        tma_atom_VC, tma_tensor_VC = cute.nvgpu.cpasync.make_tiled_tma_atom(
            g2s_op,
            vc_nkl,
            self.V_smem_layout,
            (DV, N),
...
def apply_route_mask(
    ...
    for m in cutlass.range_constexpr(cute.size(scores_mn, mode=[0])):
        ...
        for n in cutlass.range_constexpr(cute.size(scores_mn, mode=[1])):
            ...
            scores_mn[m, n] = (
                cutlass.Float32(scores_mn[m, n]) + cutlass.Float32(column_masks[column])
                if valid_row
...
def online_softmax(
    ...
    for m in cutlass.range_constexpr(cute.size(row_max)):
        ...
        current_max = kernel_utils.fmax_reduce(score_row, init_val=row_max[m], arch=80)
        current_max = cute.arch.warp_reduction_max(current_max, threads_in_group=4)
        previous_max = row_max[m]
        row_max[m] = current_max
        safe_max = cutlass.Float32(0.0) if current_max == -cutlass.Float32.inf else current_max
        scaled_max = safe_max * scale_log2e
        probabilities = cute.math.exp2(score_row * scale_log2e - scaled_max, fastmath=True)
        row_scale[m] = cute.math.exp2((previous_max - safe_max) * scale_log2e, fastmath=True)
...
def online_softmax_route(
    ...
    for m in cutlass.range_constexpr(cute.size(row_max)):
        ...
        row_scale[m] = cute.math.exp2((previous_max - safe_max) * scale_log2e, fastmath=True)
        masses = cute.make_rmem_tensor_like(scores_mn[m, None], cutlass.Float32)
        for n in cutlass.range_constexpr(cute.size(masses)):
            ...
            masses[n] = cutlass.Float32(probabilities[n]) * cutlass.Float32(length)
        row_sum[m] = kernel_utils.fadd_reduce(
            masses.load(),
            init_val=row_sum[m] * row_scale[m],
            arch=80,
...
def finalize_softmax(
    ...
    for m in cutlass.range_constexpr(cute.size(row_sum)):
        ...
        lse[m] = (
            -cutlass.Float32.inf
            if invalid
            else (row_max[m] * scale_log2e + cute.math.log2(total, fastmath=True))
            * 0.6931471805599453
        )
    return ratio, lse
...
def rescale_o_for_next_acc(
    ...
    for m in cutlass.range_constexpr(cute.size(row_scale)):
        output_mn[m, None].store(output_mn[m, None].load() * row_scale[m])
