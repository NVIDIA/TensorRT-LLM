...
class WanBlock(nn.Module):
    ...
    def forward(
        ...
        # I2V: compute text and image attention
        if encoder_hidden_states_img is not None:
            batch_size, seq_len = norm_x.shape[:2]

            q, k_text, v_text = self.attn2.get_qkv(norm_x, encoder_hidden_states_text)
            q, k_text = self.attn2.apply_qk_norm(q, k_text)

            text_attn = self.attn2._attn_impl(
                q,
                k_text,
                v_text,
            ...
            img_attn = self.attn2._attn_impl(
                q,
                key_img,
                value_img,
                batch_size=batch_size,
            ...
            attn2_output = self.attn2.to_out[0](text_attn + img_attn)
        else:
            # T2V: standard cross-attention
