...
other audio autoencoders) and the registered Kaiser-window resampling `filter` buffers of the
...
class MiniMaxH3AudioDiagonalGaussianDistribution:
    ...
    def __init__(self, mean: torch.Tensor, logs: torch.Tensor) -> None:
        self.mean = mean
        self.logs = logs
...
def _wn_conv1d(*args: int, **kwargs: int | bool) -> nn.Conv1d:
...
class MiniMaxH3AudioSnake1d(nn.Module):
    r"""`x + (alpha + 1e-9)^-1 * sin(alpha * x)^2` over `[batch_size, channels, length]`."""
...
class MiniMaxH3AudioLowPassFilter1d(nn.Module):
    r"""Depthwise Kaiser-sinc low-pass filter with a stride."""
...
class MiniMaxH3AudioResidualUnit(nn.Module):
    r"""DAC residual unit: `Snake -> dilated Conv1d(k=7) -> Snake -> Conv1d(k=1)`."""
    ...
    def __init__(self, dim: int, dilation: int) -> None:
        ...
        self.block = nn.Sequential(
            ...
            _wn_conv1d(
                dim, dim, kernel_size=7, dilation=dilation, padding=((7 - 1) * dilation) // 2
            ),
            MiniMaxH3AudioSnake1d(dim),
...
class AutoencoderKLMiniMaxH3Audio(MiniMaxH3ModelMixin):
    r"""
    ...
    def encode(
        ...
        right_pad = (
            math.ceil(sample.shape[-1] / self.hop_length) * self.hop_length - sample.shape[-1]
        )
        if right_pad > 0:
            sample = F.pad(sample, (0, right_pad))

        encoder_dtype = get_module_parameter_dtype(self.encoder)
    ...
    def decode(
        ...
        decoder_dtype = get_module_parameter_dtype(self.decoder)
