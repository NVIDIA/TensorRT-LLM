...
# Enough to cover the ISO-BMFF ftyp box that `sniff_media_kind` bounds its scan to.
MEDIA_SNIFF_HEADER_BYTES = 4096
...
def read_video_tensor(path: Path, max_frames: int | None = None) -> torch.Tensor:
    """Decode a video file to a uint8 ``[T, H, W, C]`` CPU tensor."""
    cv2 = _import_cv2("control")
    capture = cv2.VideoCapture(str(path))
    if not capture.isOpened():
        raise ValueError(f"Cosmos3 transfer could not open control media: {path}")
    frames = []
    try:
        ...
        capture.release()
    if not frames:
        raise ValueError(f"Cosmos3 transfer decoded no frames from control media: {path}")
...
def _path_media_to_uint8_frames(path: str | Path, max_frames: int | None) -> torch.Tensor:
    ...
    # Classify by content, not suffix — the same container sniff the V2V path uses.
    with media_path.open("rb") as handle:
        media_kind = sniff_media_kind(handle.read(MEDIA_SNIFF_HEADER_BYTES))
    if media_kind == "image":
