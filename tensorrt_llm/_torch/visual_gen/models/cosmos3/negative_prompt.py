...
# ruff: noqa: E501
"""Cosmos3's default negative prompt for video modes.

Verbatim copy of the reference's ``inference/defaults/neg_prompts.json``, which every
video mode (``text2video``, ``image2video``, ``video2video``, ``audio_image2video``)
points at via ``negative_prompt_file``. Image modes declare none.

Kept as a Python literal rather than a data file so it ships with the package in every
install mode without a ``package_data`` entry. Key order is significant: the serialized
form must match the reference byte for byte, and ``json.dumps`` preserves insertion
order.
"""
