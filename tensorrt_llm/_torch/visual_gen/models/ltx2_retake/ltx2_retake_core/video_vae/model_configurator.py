...
from .video_vae import RetakeVideoDecoder, RetakeVideoEncoder


class RetakeVideoDecoderConfigurator:
    """Create a RetakeVideoDecoder from the LTX-2 native config dict."""
...
class RetakeVideoEncoderConfigurator:
    """Create a RetakeVideoEncoder from the LTX-2 native config dict."""
    ...
    def from_config(cls, config: dict) -> RetakeVideoEncoder:
        ...
        return RetakeVideoEncoder(
            convolution_dimensions=config.get("dims", 3),
            in_channels=config.get("in_channels", config.get("out_channels", 3)),
