...
class AudioEncoderConfigurator:
    """Create an AudioEncoder from the LTX-2.3 native config dict."""
    ...
    def from_config(cls, config: dict) -> AudioEncoder:
        ...
        if mel_bins is None:
            raise ValueError("LTX-2.3 audio VAE config does not define the mel-bin count.")

        # The native encoder implements only vanilla attention.
        attn_type = ddconfig.get("attn_type", "vanilla")
        if attn_type != "vanilla":
            raise ValueError(
                f"LTX-2.3 native audio encoder supports attn_type='vanilla'; got {attn_type!r}."
            )
