...
class LTX2Attention(Attention):
    ...
    def __init__(
        ...
        stage2_ulysses_group: Optional["torch_dist.ProcessGroup"] = None,
    ):
        from tensorrt_llm._torch.visual_gen.config import DiffusionModelConfig
        ...
        # by set_active_attn(); with no stage2 group they alias one instance. The
        # stage2 stack gets a FRESH inner backend for heads/stage2-group-size and a
        # fresh cp wrapper over the SAME cp group (a shared wrapper would carry the
        # wrong head-count metadata and reallocate its shape-keyed buffers across
        # CUDA-graphed topologies).
        ...
        if stage2_ulysses_group is not None and enable_sp:
            ...
            self._attn_stage2 = wrap_parallel_attention(
                ...
                use_ulysses=True,
                async_ulysses=self._use_async_ulysses,
                ulysses_group=stage2_ulysses_group,
            )
    ...
    def set_active_attn(self, is_stage2: bool = False) -> None:
        """Select the topology stack ``self.attn`` points at.

        Called only via ``LTXModel.set_ulysses_topology``. A no-op when the two
        stacks alias one instance (no stage2 group, or cfg_size == 1).
        """
...
class Stage2Groups:
    """LTX-2 two-stage dual-topology groups, built by the two-stage pipeline at
    load. ``ulysses_group`` feeds the {default, stage2} attention stacks; the
    seq fields configure the stage-2 ``SequenceSharder`` (shard index and gather
...
class BasicAVTransformerBlock(nn.Module):
    ...
    def __init__(
        ...
        config: Optional["DiffusionModelConfig"] = None,
        stage2_ulysses_group: Optional["torch_dist.ProcessGroup"] = None,
        stage2_sharder: Optional[SequenceSharder] = None,
    ):
        super().__init__()
        self.idx = idx
    ...
    def _audio_shard_mode(self) -> AudioShardMode:
        ...
        if not self._active_sharder.is_active:
    ...
    def _init_video_modules(self, cfg, rope_type, eps, model_config, idx):
        ...
        self.attn1 = LTX2Attention(
            ...
            stage2_ulysses_group=self._stage2_ulysses_group,
        )
        ...
        num_ada_slots = 9 if cfg.cross_attention_adaln else 6
        self.scale_shift_table = nn.Parameter(torch.empty(num_ada_slots, cfg.dim))
    ...
    def _init_audio_modules(self, cfg, rope_type, eps, model_config, idx):
        ...
        num_ada_slots = 9 if cfg.cross_attention_adaln else 6
        self.audio_scale_shift_table = nn.Parameter(torch.empty(num_ada_slots, cfg.dim))
        if cfg.cross_attention_adaln:
    ...
    def _apply_text_cross_attention(
        ...
        if prompt_scale_shift_table is None:
            raise RuntimeError("cross_attention_adaln requires prompt scale/shift weights")

        (
            (q_shift_table, q_shift_ts),
            (q_scale_table, q_scale_ts),
            (q_gate_table, q_gate_ts),
        ) = self._get_ada_table_ts_pairs(scale_shift_table, q_input.shape[0], timestep, slice(6, 9))
        q_shift = q_shift_table[None, None].to(device=q_input.device, dtype=q_input.dtype) + q_shift_ts
        q_scale = q_scale_table[None, None].to(device=q_input.device, dtype=q_input.dtype) + q_scale_ts
        q_gate = q_gate_table[None, None].to(device=q_input.device, dtype=q_input.dtype) + q_gate_ts
        attn_input = apply_shift_scale(q_input, q_scale, q_shift)

        kv_modulation = prompt_scale_shift_table[None, None].to(
            device=context.device, dtype=context.dtype
        )
        if prompt_timestep is not None:
            kv_modulation = kv_modulation + prompt_timestep.reshape(
                prompt_timestep.shape[0], prompt_timestep.shape[1], 2, -1
            ).to(device=context.device, dtype=context.dtype)
        kv_shift, kv_scale = kv_modulation.unbind(dim=2)
        context = apply_shift_scale(context, kv_scale, kv_shift)
        return attn(
            attn_input,
            pre_projected_kv=attn.project_kv(context),
            timestep=timestep,
        ) * q_gate
    ...
    def forward(
        ...
        if run_vx:
            ...
            text_v_attn_raw = self._apply_text_cross_attention(
                self.attn2,
                attn2_q_input,
                video.context,
                text_kv_video,
        ...
        if run_ax:
            ...
            text_a_attn_raw = self._apply_text_cross_attention(
                self.audio_attn2,
                audio_attn2_q_input,
                audio.context,
                text_kv_audio,
        ...
        if run_a2v or run_v2a:
            ...
            if run_v2a and not skip_v2a:
                if self._audio_shard_mode == AudioShardMode.CONDITIONAL:
                    ...
                    ax_v2a_local = self._active_sharder.shard(ax_scaled_v2a, dim=1)
                    ...
                    if a_cross_pe is not None:
                        ...
                        a_cross_pe = (
                            self._active_sharder.shard(a_cross_pe[0], dim=pe_dim),
                            self._active_sharder.shard(a_cross_pe[1], dim=pe_dim),
                        )
                    if self._async_ulysses and self.video_to_audio_attn.is_ulysses:
                        ...
                        if (
                            not self.video_to_audio_attn.is_ulysses
                            and self._active_sharder.is_active
                        ):
                    ...
                    if not self.video_to_audio_attn.is_ulysses and self._active_sharder.is_active:
...
class LTXModel(BaseDiffusionModel):
    ...
    def __init__(
        ...
        *,
        ...
        stage2_groups: Optional[Stage2Groups] = None,
    ):
        from tensorrt_llm._torch.visual_gen.config import DiffusionModelConfig
        ...
        self._stage2_groups = stage2_groups
        self._has_stage2 = stage2_groups is not None
        if self._has_stage2:
            self._sharder_s2 = SequenceSharder(
                size=stage2_groups.seq_size,
                rank=stage2_groups.seq_rank,
                group=stage2_groups.seq_group,
                gather_index=stage2_groups.gather_index,
            )
        else:
        ...
        if (
            self._active_sharder.is_active
            and vgm is not None
    ...
    def _init_video(self, in_channels, out_channels, caption_channels, norm_eps):
        ...
        self.prompt_adaln_single = (
            AdaLayerNormSingle(self.inner_dim, embedding_coefficient=2, make_linear=self._make_linear)
    ...
    def _init_audio(self, in_channels, out_channels, caption_channels, norm_eps):
        ...
        if self.caption_proj_before_connector:
            self.audio_caption_projection = torch.nn.Identity()
        else:
    ...
    def _gather_sequence(self, x: torch.Tensor) -> torch.Tensor:
        ...
        return self._active_sharder.gather(x, dim=1)
    ...
    def set_ulysses_topology(self, is_stage2: bool = False) -> None:
        ...
        for block in self.transformer_blocks:
            ...
            for name in ("attn1", "video_to_audio_attn", "audio_attn1"):
                mod = getattr(target, name, None)
                if mod is not None:
                    # No-op for modules whose two stacks alias one instance.
                    mod.set_active_attn(is_stage2=is_stage2)
    ...
    def configure_audio_ulysses(self, audio_seq_len: int) -> None:
        ...
        """
        if not self._active_sharder.is_active and not self._has_stage2:
            self._audio_pad = 0
            return

        # Single pad: the stage-2 seq size is a multiple of every group size the
        # audio touches in either topology, so one pad serves both (no repad at
        # the stage boundary; audio shapes stay constant across both graphs).
        U = self._sharder_s2.size if self._has_stage2 else self._active_sharder.size
    ...
    def forward(
        ...
        *,
        ...
        torch._check(self._active_seq_size >= 1)
        ...
        if self._active_sharder.is_active:
    ...
    def _adapt_cross_attention_adaln_checkpoint_weights(self, weights: dict) -> dict:
        """Adapt LTX-2.3 cross-attention AdaLN checkpoint tensors.

        Official LTX-2.3 checkpoints can store ``cross_attention_adaln=True``
        transformer tensors: six base AdaLN slots plus three extra text
        cross-attention slots. Native LTX-2.3 models consume the full tensors;
        the slicing fallback only handles older native configs whose target
        shape is intentionally shorter.
        """

        if not weights:
            return weights

        param_shapes = {name: tuple(param.shape) for name, param in self.named_parameters()}
        adapted = {}
        sliced: list[tuple[str, tuple[int, ...], tuple[int, ...]]] = []

        for key, value in weights.items():
            target_shape = param_shapes.get(key)
            if (
                target_shape is not None
                and isinstance(value, torch.Tensor)
                and tuple(value.shape) != target_shape
            ):
                first_dim_slice_ok = (
                    value.ndim == len(target_shape)
                    and value.ndim >= 1
                    and tuple(value.shape[1:]) == tuple(target_shape[1:])
                    and value.shape[0] > target_shape[0]
                )
                cross_adaln_name = key.endswith(
                    (
                ...
                if first_dim_slice_ok and cross_adaln_name:
                    adapted[key] = value.narrow(0, 0, target_shape[0]).contiguous()
                    sliced.append((key, tuple(value.shape), target_shape))
                    continue
        ...
        if sliced:
            logger.warning(
                "Adapted %d LTX-2.3 cross-attention AdaLN checkpoint tensors "
                "to native base-slot shapes: %s%s"
                % (
                    len(sliced),
                    sliced[:8],
                    "..." if len(sliced) > 8 else "",
                )
            )
        return adapted
    ...
    def load_weights(self, weights: dict) -> None:
        ...
        weights = self._adapt_cross_attention_adaln_checkpoint_weights(weights)
