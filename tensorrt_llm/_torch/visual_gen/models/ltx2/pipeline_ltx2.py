...
class _LTX2CUDAGraphRunner(CUDAGraphRunner):
    ...
    The base runner derives graph keys from ``torch.Tensor`` shapes and
    clones / copies tensors for static buffers.  LTX-2's transformer takes
    ``Modality`` frozen dataclasses (bundles of tensors) and optional
    ``BatchedPerturbationConfig`` objects that affect control flow.
    ...
    ``BatchedPerturbationConfig`` is fingerprinted into the key so different
    STG block patterns produce distinct graph captures.
    """
    ...
    def _make_output_ref(x):
        ...
        The upstream ``make_weak_ref`` raises on ``None``.
        """
        if x is None:
            return None
        ...
        if isinstance(x, dict):
            return {k: _LTX2CUDAGraphRunner._make_output_ref(v) for k, v in x.items()}
        return make_weak_ref(x)

    # -- capture / replay ----------------------------------------------------

    def capture(self, key, fn, args, kwargs):
        logger.info(f"Capturing CUDA graph for LTX2 transformer, key: {key}")
...
class LTX2Pipeline(BasePipeline):
    ...
    def _setup_cuda_graphs(self):
        ...
        logger.info("CUDA graph runner: wrapping transformer.forward (Modality-aware)")
