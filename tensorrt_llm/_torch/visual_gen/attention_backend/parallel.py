...
class RingAttention(AttentionBackend):
    ...
    def __init__(
        ...
        if (
            ring_transport in ("ub", "auto")
            and os.environ.get("TRTLLM_FORCE_NCCL_ALLREDUCE", "0") == "0"
        ):
            try:
    ...
    def _ring_send_recv_ub(self, send: torch.Tensor, recv: torch.Tensor) -> None:
        ...
        self._ub_send_fn(
            self._ub_send_buf.handle,
            0,
            self._ub_recv_buf.handle,
            0,
            nbytes,
            self._send_rank,
            stream_ptr,
    ...
    def _init_nixl_ring(self) -> None:
        ...
        try:
            ...
            all_descs = [
                torch.zeros(max_len, dtype=torch.uint8).cuda() for _ in range(self.world_size)
            ]
