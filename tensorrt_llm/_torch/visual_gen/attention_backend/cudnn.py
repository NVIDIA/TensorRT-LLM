...
Three quantization modes, selected by ``quant_attention_config`` (see
``tensorrt_llm.visual_gen.args.AttentionConfig``):

=============  =========================================  ==================
Quant dtype    ``quant_attention_config``                 cuDNN node
=============  =========================================  ==================
``None``       ``None``                                   ``sdpa``
...
def _quantize_fp8(x: torch.Tensor) -> Tuple[torch.Tensor, torch.Tensor]:
    ...
    returns the descale on device, so quantization stays free of host synchronization.
...
class _CuDNNProblemShape:
    """Problem geometry shared by all three quantization modes.

    Every tensor's strides are part of the geometry, and of the graph cache key:
    the FP8 and unquantized modes hand cuDNN BSHD-strided views of NHD buffers,
    and the MXFP8 mode passes Q/K as views into sequence-padded buffers.
    """
...
class CuDNNAttention(AttentionBackend):
    ...
    through cuDNN's fused ``sdpa`` / ``sdpa_fp8`` / ``sdpa_mxfp8`` nodes. The quantization
    dtype comes from ``quant_attention_config``; ``None`` means unquantized.
    ...
    def __init__(
        ...
        **kwargs,
        ...
        self.quant_dtype = self.check_quant_dtype(quant_attention_config)
        self.check_library_feature(self.quant_dtype)
    ...
    @staticmethod
    def check_quant_dtype(
        quant_attention_config: Optional[QuantAttentionConfig],
    ) -> str | None:
        """Map the validated public config onto a cuDNN SDPA node."""
        ...
        if qk_dtype in ("fp8", "mxfp8"):
            return qk_dtype
        raise ValueError(
            f"cuDNN backend does not support qk_dtype={qk_dtype!r}; supported quantization "
            "dtypes are unquantized (quant_attention_config=None), 'fp8' and 'mxfp8'."
        )
    ...
    def check_library_feature(cls, quant_dtype: str | None = None) -> None:
        ...
        if cls._get_lib_version() < 90100:
            raise RuntimeError("cuDNN attention backend requires cuDNN library v9.1.0 or later.")
        if cls._get_lib_version() < 92100 and quant_dtype == "mxfp8":
            raise RuntimeError("cuDNN MXFP8 attention requires cuDNN library v9.21.0 or later.")
    ...
    def _build_graph(
        ...
        io_dtype = fp8 if quant_dtype in ("fp8", "mxfp8") else out_cudnn_dtype
        graph = cudnn.pygraph(
            io_data_type=io_dtype,
            intermediate_data_type=f32,
            compute_data_type=f32,
            name=f"visual_gen_sdpa_{quant_dtype or ''}",
        )
        ...
        if quant_dtype is None:
            ...
            raise ValueError(f"Unknown cuDNN SDPA quant_dtype {quant_dtype!r}.")
    ...
    def _get_or_build_graph(
        ...
        device_index = device.index if device.index is not None else torch.cuda.current_device()
        key = (device_index, quant_dtype, shape, is_causal, sm_scale, out_dtype, with_lse)
        with cls._cache_lock:
            bundle = cls._graph_cache.get(key)
            if bundle is None:
                cls.check_hardware_compatibility(device, quant_dtype)
                logger.debug(
                    f"[CuDNNAttention] building graph on cuda:{device_index}: "
                    f"quant_dtype={quant_dtype or ''} {shape} causal={is_causal}"
                )
                with torch.cuda.device(device_index):
                    bundle = cls._build_graph(
                        quant_dtype, shape, is_causal, sm_scale, out_dtype, with_lse
                    )
    ...
    @classmethod
    @torch.compiler.disable
    def _softmax_scales(cls, quant_dtype: str | None, device: torch.device) -> Any:
        """Get the per-device cache for quantization-dtype-specific constants.

        - quant_dtype="fp8": (scale_s, descale_s, scale_o) for FP8 attention.
        ...
        with cls._cache_lock:
            ...
            if scales is None:
                if quant_dtype == "fp8":
                    ...
                    # Unquantized and MXFP8 attention don't need device-side constants.
                    pass
            return scales
    ...
    def _validate_inputs(self, q: torch.Tensor, k: torch.Tensor, v: torch.Tensor) -> None:
        ...
        if self.quant_dtype is not None and max(q.shape[3], v.shape[3]) > 128:
            raise ValueError(
        ...
        if self.quant_dtype == "mxfp8" and q.shape[3] % 32 != 0:
            raise ValueError(
    ...
    def _run(
        ...
        if self.quant_dtype is None:
            ...
            scale_s, descale_s, scale_o = self._softmax_scales(self.quant_dtype, device)
            buffers.update(
                q=q_q.transpose(1, 2),
                k=k_q.transpose(1, 2),
                v=v_q.transpose(1, 2),
