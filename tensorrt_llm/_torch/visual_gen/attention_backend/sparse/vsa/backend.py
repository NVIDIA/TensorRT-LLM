...
class VSATrtllmAttention(TrtllmAttention):
    ...
    def __init__(
        ...
        self.predictor = VSAPredictor(num_heads=num_heads, num_kv_heads=num_kv_heads)
    ...
    def forward(
        ...
        **kwargs,
        ...
        """Run the VSA coarse stage, then the fine stage through the core forward.

        The coarse stage predicts the complete block-sparse payload, which is
        handed to the core prediction hook via ``sparse_backend_args``. The fine
        output is blended with the coarse output afterward.
        """
        ...
        fine_output = super().forward(
            ...
            inputs.seq_len,
            attention_mask=attention_mask,
            seq_len_kv=inputs.seq_len,
            sparse_backend_args=sparse_backend_args,
            **kwargs,
        )
        combined = vsa_post_process(fine_output, inputs)
