...
class HaloExchangeConv(nn.Module):
    ...
    def __init__(
        ...
        if isinstance(kernel_size, int):
            kernel_size = (kernel_size,) * (chunk_dim - 1)
    ...
    def _exchange_halos(self, x: torch.Tensor) -> torch.Tensor:
        ...
        send_right = torch.narrow(
            x, dim, x.shape[dim] - self.halo_right, self.halo_right
        ).contiguous()

        recv_from_left = torch.zeros_like(send_right)
        recv_from_right = torch.zeros_like(send_left)
        ...
        if self.rank % 2 == 0:
            if self.rank > 0:
                ...
                dist.all_gather(
                    gather_buf, send_left, group=self.adj_groups[self.rank - 1]
                )
            if self.rank < self.world_size - 1:
                gather_buf = [send_right, recv_from_right]
                dist.all_gather(
                    gather_buf, send_right, group=self.adj_groups[self.rank]
                )
        else:
            if self.rank < self.world_size - 1:
                gather_buf = [send_right, recv_from_right]
...
class HaloExchangeConvStride2(nn.Module):
