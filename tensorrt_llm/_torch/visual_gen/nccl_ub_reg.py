...
_NCCL_MIN_CUMEM = (2, 21, 0)  # Conservative floor for VisualGen's CUMEM path
...
def _load_nccl() -> ctypes.CDLL | None:
...
def _extract_comm_ptr(group: dist.ProcessGroup) -> int | None:
    ...
    try:
        ...
        # Recent ProcessGroupNCCL builds expose _comm_ptr() returning the
        # ncclComm_t as an integer.
        if hasattr(backend, "_comm_ptr"):
            return int(backend._comm_ptr())
...
class NCCLBufferRegistrar:
    ...
    def __init__(self, process_group: dist.ProcessGroup):
        ...
        self._comm_ptr: int | None = None
        self._handles: dict[int, tuple[torch.Tensor, ctypes.c_void_p]] = {}
    ...
    def register(self, tensor: torch.Tensor) -> bool:
        ...
        if key in self._handles:
            cached_tensor, cached_handle = self._handles[key]
            if cached_tensor.nbytes == tensor.nbytes:
                return True  # already registered
            logger.warning(
                f"Registered tensor address {key:#x} changed size from "
                f"{cached_tensor.nbytes} B to {tensor.nbytes} B; re-registering"
            )
        ...
        if ret == 0:  # ncclSuccess
            self._handles[key] = (tensor, handle)
    ...
    def register_all(self, tensors: Iterable[torch.Tensor]) -> int:
