...
class DynamicLinearWeightLoader:
    ...
    def _check_static_quant_scales(
        ...
        if expected_scales is None or self.dynamic_weight_quant:
            return
        ...
        if missing:
            raise ValueError(
                ...
                f"without the expected scale tensor(s) {missing}. The checkpoint "
                "appears to be unquantized; loading it would silently corrupt the "
                "weights. Use a checkpoint that carries quantized weights and "
                "scales (e.g. exported by ModelOpt), or enable load-time "
                "quantization by setting 'dynamic': true in quant_config."
            )
