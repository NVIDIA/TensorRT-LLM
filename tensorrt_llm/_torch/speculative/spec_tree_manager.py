...
class SpecTreeManager:
    ...
    def init_tree_info_for_static_tree(self):
        ...
        for i in range(self.max_draft_len):
            ...
            assert sum(tmp_topK_list) == len(nodes_list_per_layer[i + 1])
            self.topK_list.append(
                torch.tensor(tmp_topK_list,
                             dtype=torch.int32,
                             device='cpu',
                             pin_memory=True))
    ...
    def get_topK_list(self, draft_layer_id):
        assert draft_layer_id >= 0
        return self.topK_list[draft_layer_id]
    ...
    def dump_tree_info(self):
        ...
        print(f"TopK list: {self.topK_list}")
