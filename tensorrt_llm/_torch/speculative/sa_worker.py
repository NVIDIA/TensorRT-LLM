...
"""
SA (Suffix Automaton) speculative decoding worker that runs inside model forward.

This module provides CUDA graph compatible SA speculative decoding by
...
@dataclass
class SASpecMetadata(SpecMetadata):
    """
    Metadata for SA (Suffix Automaton) speculative decoding.
    ...
    # SA/ngram matching configuration
...
class SAWorker(SpecWorkerBase):
    """
    SA (Suffix Automaton) speculative decoding worker that runs inside model forward.

    Uses suffix automaton for pattern matching - CUDA graph compatible.
    Unlike MTP which uses neural network layers for drafting, SA uses
    pattern matching on previously generated tokens.
    """

    def __init__(self, spec_config: "SADecodingConfig", model_config=None):
        super().__init__()
    ...
    def forward(
        ...
        Args:
            ...
            position_ids: Position IDs (not used for SA)
            hidden_states: Hidden states from target model (not used for SA)
            logits: Target model logits for acceptance
            attn_metadata: Attention metadata
            spec_metadata: SASpecMetadata with SA manager
            draft_model: Not used for SA (pattern matching, not neural)
    ...
    def _generate_draft_tokens(
        ...
        Args:
            ...
            spec_metadata: SA metadata with SA manager
