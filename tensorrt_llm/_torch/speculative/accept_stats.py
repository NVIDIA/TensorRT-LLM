...
def maybe_create_recorder(max_draft_len: int,
                          rank: int) -> Optional["DFlashAcceptStatsRecorder"]:
...
class DFlashAcceptStatsRecorder:
    ...
    # with active ranks. Job 2679294: without this filter, idle DEP ranks
    ...
    def __init__(self,
        ...
        self.exclude_request_ids = frozenset(
            int(r) for r in exclude_request_ids)
        ...
        self.calib_attempts = [[0] * num_conf_bins
    ...
    def on_accept(self, request_ids: Sequence[int],
                  num_accepted_tokens: Sequence[int]) -> None:
    ...
    def on_draft_confidence(self, request_ids: Sequence[int],
                            confidence_rows: Sequence[Sequence[float]]) -> None:
    ...
    def record_draft_confidence(self, request_ids: Sequence[int],
                                draft_model, gen_hidden, first_prev_tokens,
                                gen_draft_tokens) -> None:
        ...
        if self.confidence_provider is None:
            return
        rows = self.confidence_provider(draft_model, gen_hidden,
                                        first_prev_tokens, gen_draft_tokens)
        if rows is None:
            return
...
def merge_snapshots(snapshots: Sequence[Dict[str, Any]]) -> Dict[str, Any]:
    ...
    for s in snapshots:
        ...
        for rid, v in s["per_request"].items():
            entry = merged["per_request"].setdefault(
                rid, {
                    "steps": 0,
                    "accepted_draft": 0
                })
        ...
        for k in range(K):
            for b in range(nbins):
                merged["confidence_calibration"]["attempts"][k][b] += \
                    cc["attempts"][k][b]
                merged["confidence_calibration"]["accepted"][k][b] += \
                    cc["accepted"][k][b]
    return merged


def calibration_table(attempts: Sequence[Sequence[int]],
                      accepted: Sequence[Sequence[int]]) -> Dict[str, Any]:
    ...
    shifted/scaled curve is what !75's Sequential Temperature Scaling
    corrects.
    """
    K = len(attempts)
