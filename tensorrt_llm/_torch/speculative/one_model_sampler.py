...
def compute_probs_from_logits(
    ...
    # For greedy tokens (temperature <= DISABLE_TEMP_VAL), replace softmax probs
    # with exact one-hot distributions.  With temperature=1e-5 the softmax is
    # nearly but not exactly one-hot: floating-point rounding can give
    # draft_prob[argmax] > target_prob[argmax], causing rejection_sampling to
    # spuriously reject the correct greedy token and diverge from the reference.
    # One-hot probs guarantee acceptance when both models agree on the argmax.
    GREEDY_TEMP_THRESHOLD = 1e-4
    is_greedy = temperatures <= GREEDY_TEMP_THRESHOLD  # [num_tokens]
    argmax_ids = logits.argmax(dim=-1, keepdim=True)  # [num_tokens, 1]
    one_hot = torch.zeros_like(probs).scatter_(1, argmax_ids, 1.0)
    probs = torch.where(is_greedy.unsqueeze(1), one_hot, probs)
    return probs
