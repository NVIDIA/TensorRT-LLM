...
"""DSpark draft I/O: capture projection + block-draft proposal stage.
...
they can be unit-tested in isolation:
  ...
  - ``DSparkCapture``: ``main_proj`` + ``main_norm`` — projects the concatenation
    of the captured target-layer hidden states ([58,59,60] for V4-Pro) down to
    one hidden, forming the draft backbone's cross-attention context (``main_x``).
...
from .dspark_heads import confident_prefix_length


class DSparkCapture(nn.Module):
    """Project concat(captured target-layer hidden states) -> draft context.

    Mirrors the reference ``main_proj`` (Linear, no bias) + ``main_norm`` (RMSNorm)
    on stage 0 of the DSpark draft.
    """
    ...
    def __init__(
        ...
        *,
        ...
        self.num_capture_layers = int(num_capture_layers)
        self.main_proj = nn.Linear(
            self.hidden_size * self.num_capture_layers, self.hidden_size, bias=False, dtype=dtype
        )
        self.main_norm = nn.RMSNorm(self.hidden_size, eps=eps, dtype=dtype)

    def forward(self, captured_hidden: torch.Tensor) -> torch.Tensor:
        """captured_hidden: ``[..., num_capture_layers * hidden]`` -> ``[..., hidden]``."""
        return self.main_norm(self.main_proj(captured_hidden))
...
def dspark_propose(
    ...
    *,
    ...
    if markov_head is not None:
        ...
        from .dspark_heads import greedy_or_sample
