...
class SpecMetadata:
    ...
    # [0, num_seq_slots); overlap can use 2 * max_batch_size, larger than
    ...
    def prepare_rejection_sampling_buffers(self):
        ...
        # can range over [0, num_seq_slots), which under overlap exceeds
        # max_num_requests. Fall back to max_num_requests when the pool size is
        # unknown (0). One extra scratch row at index ``slot_capacity`` absorbs
        # CUDA-graph dummy/padding requests (``py_seq_slot is None``).
