...
class Eagle3OneModelWorker(SpecWorkerBase):
    ...
    def _prepare_flash_mla_generation_layout(self, attn_metadata, num_contexts,
                                             batch_size):
        """Stage the batch-ordered block table for the draft step.

        The caller has already set ``num_contexts = 0``, so every row is a
        generation row and ``_compute_flash_mla_metadata`` slices
        ``kv_lens_cuda`` from row 0 in batch order. Rotating the table so gen
        rows precede context rows would pair each row's kv_len with another
        row's block pointers, corrupting the drafted tokens.
        """
