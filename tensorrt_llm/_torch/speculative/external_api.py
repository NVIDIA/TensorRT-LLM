...
class APIDrafter(Drafter):
    ...
    async def async_prepare_draft_tokens(
        ...
        for request in sorted_requests:
            ...
            if request.py_parallel_spec_dec_params:
                if not request.py_parallel_spec_dec_params["pre_verify"]:
                    prefix += list(
                        request.py_parallel_spec_dec_params["old_draft_tokens"])
        ...
        for request, draft_tokens in zip(sorted_requests, all_draft_tokens):
            if isinstance(draft_tokens, Exception):
                ...
                logger.error(
                    f"Returning empty draft tokens for request {request.request_id}. Set TLLM_LOG_LEVEL for more details."
                )
            else:
            ...
            draft_tokens_result[request.py_request_id] = draft_tokens
        return draft_tokens_result
    ...
    def prepare_draft_tokens(
        ...
        for request in scheduled_requests.generation_requests:
            request.py_draft_tokens = draft_tokens[request.py_request_id]
