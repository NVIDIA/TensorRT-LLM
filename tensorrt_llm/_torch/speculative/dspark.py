...
class DSparkWorker(SpecWorkerBase):
    ...
    def _assign_slot(self, req_id: int, reset: bool) -> int:
        ...
        if req_id not in self._req_to_slot:
            if not self._free_slots:
                logger.warning("DSpark: no free window slots; reusing slot 0")
                self._free_slots.append(0)
    ...
    def _draft_gen_block(
        ...
        Kp1 = K + 1
        conf_thr = float(getattr(self.spec_config, "confidence_threshold", 0.0))
    ...
    def forward(
        ...
        if num_contexts > 0:
            # Captured (hc-mean) target hiddens; context tokens occupy
            # [0, num_ctx_tokens) in request order.
            ctx_captured = spec_metadata.get_hidden_states(total_target_tokens)
            ctx_off = 0
            for i in range(num_contexts):
                req_id = spec_metadata.request_ids[i]
                slot = self._assign_slot(req_id, reset=True)
                seq_len = int(attn_metadata._seq_lens[i])
                self._ctx_len[slot] = seq_len
                # Only the most recent `window` positions survive in the rolling
                # window; window index = committed_position + 1 (the generation
                # path stores the bonus at start_pos = ctx_len + nacc).
                if ctx_captured is not None and seq_len > 0:
                    keep = min(self._win, seq_len)
                    hid = ctx_captured[ctx_off + seq_len - keep : ctx_off + seq_len]
                    # Seed prompt token at absolute position p into window frame p+1
                    # (the generation path stores the bonus at start_pos = ctx_len).
                    idx = torch.arange(seq_len - keep, seq_len, device=hid.device) + 1
