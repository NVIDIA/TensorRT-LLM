...
class NVLinkTwoSidedFlashinfer(Communication):
    ...
    def __init__(
        ...
        assert alltoall_result_do_sum, "flashinfer do_reduce must be True"
        assert use_low_precision_combine is False, (
            "flashinfer use_low_precision_combine must be False"
        )
    ...
    def dispatch(
        ...
        **kwargs,
        ...
        if alltoall_info is None:
            raise ValueError(
                "NVLinkTwoSidedFlashinfer dispatch requires prepare_dispatch() to be called first"
            )
