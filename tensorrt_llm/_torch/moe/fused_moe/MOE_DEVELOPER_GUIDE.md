...
| `fused_moe_cute_dsl_fc12.py` | `CuteDslFc12FusedMoE` | Rubin/SM107 | NVFP4 with a fused FC1+FC2 CuteDSL kernel; select via the `CUTEDSL_FC12` backend string (own BACKEND_FAMILY). Gated epilogue is SwiGLU or SiTU (trace-time specialization). | `EXTERNAL_COMM` |
