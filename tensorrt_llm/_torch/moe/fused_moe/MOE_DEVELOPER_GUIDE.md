...
| `mega_moe/mega_moe_cute_dsl.py` | `MegaMoECuteDsl` | SM100/SM103/SM107 | NVFP4 via an architecture-dispatched fused dispatch+FC1+activation+FC2+combine kernel; requires CUDA 13 Cutlass DSL and, for multi-rank EP, the PyTorch cuMem symmetric-memory provider; supports per-expert scales, SwiGLU clamp, and kernel-side top-k reduction | `FUSED_COMM` |
...
| NVFP4 | Y (SM100/103/120/121) | Y (SM100/103) | N | Y (SM100/103) | Y (SM100/103/120/121) | N | Y (SM100/103/107, CUDA 13 Cutlass DSL + PyTorch cuMem symmetric memory for multi-rank EP) | N | Y (SM89-SM99) | Y |
...
- **Do NOT allocate symmetric memory from `run_moe` in `FUSED_COMM` backends** — Symmetric-memory rendezvous is a build-time collective and is unsafe under PP / layer-skip or CUDA graph capture; allocate from `create_weights()` after `ConfigurableMoE` has synchronized EPLB-derived attributes.
...
- **Do NOT use a dataclass for an autotuner tactic** — `AutoTuner` requires JSON serialization and an `eval(repr(tactic))` round-trip. Use tuples or lists of primitives; `_unpack_tactic` defines the canonical and legacy MegaMoE layouts.
- **Use `DistributedTuningStrategy.MERGE` for multi-rank `FUSED_COMM` tuning** — EP ranks must profile the same tactic in lockstep and converge on one result; `PARALLEL` can deadlock the fused collective.
