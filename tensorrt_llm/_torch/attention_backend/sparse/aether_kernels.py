...
def aether_sparse_attention_kernel(
    Q_ptr, K_ptr, V_ptr,
    Active_Indices_Ptr,  # [B, H, Max_Active]
    Active_Counts_Ptr,   # [B, H]
    Out_Ptr,
    stride_q_b, stride_q_h, stride_q_d,
    stride_k_b, stride_k_h, stride_k_s, stride_k_d,
    stride_v_b, stride_v_h, stride_v_s, stride_v_d,
    stride_out_b, stride_out_h, stride_out_d,
    stride_idx_b, stride_idx_h, stride_idx_max,
    SCALE: tl.constexpr,
    BLOCK_SIZE: tl.constexpr,
    HEAD_DIM: tl.constexpr,
):
    """
    SS-Level Block-Sparse Attention Execution Kernel.
    
    Iterates only through 'active' blocks identified by the Aether Radar.
    Uses online softmax for numerical stability across non-contiguous blocks.
    """
    bh_idx = tl.program_id(0)
    b_idx = bh_idx // 32  # Simplified for demonstration
    h_idx = bh_idx % 32
    
    # Load number of active blocks for this head
    n_active = tl.load(Active_Counts_Ptr + bh_idx)
    ...
    # Iterate through ONLY active blocks
    for i in range(n_active):
        # Indirect lookup of block index
        block_idx = tl.load(Active_Indices_Ptr + bh_idx * stride_idx_h + i)
        
        # Load K, V for this specific block
        # (Assuming contiguous blocks for simplicity in this MVP spec)
        # In a real SS-level kernel, we would shard this into CTAs
        for j in range(BLOCK_SIZE):
            token_idx = block_idx * BLOCK_SIZE + j
            k = tl.load(K_ptr + bh_idx * stride_k_h + token_idx * stride_k_s + dim_offsets * stride_k_d)
            v = tl.load(V_ptr + bh_idx * stride_v_h + token_idx * stride_v_s + dim_offsets * stride_v_d)
    ...
    # Final normalization
    out = acc / l_i
    tl.store(Out_Ptr + bh_idx * stride_out_h + dim_offsets * stride_out_d, out)
...
def run_aether_sparse_attention(
    ...
    """
    Run the SS-Level Block-Sparse Attention Execution Kernel.
    
    Args:
        query: [B, H, D]
        keys: [B, H, S, D]
        values: [B, H, S, D]
        active_indices: [B, H, Max_Active]
        active_counts: [B, H]
    ...
    # Each head gets one program for now (Simplified SS-level)
    grid = (B * H,)
    
    aether_sparse_attention_kernel[grid](
        query, keys, values,
        active_indices, active_counts,
        output,
        query.stride(0), query.stride(1), query.stride(2),
