...
# Triton JIT functions may only capture module globals materialized as
# constexpr values. Scores use base-2 exponentials for the online softmax.
_LOG2_E = tl.constexpr(1.4426950408889634)
# Triton ranges are power-of-two sized; four lanes carry three position axes.
_POSITION_AXIS_TILE = tl.constexpr(4)


def _require_same_cuda_device(name: str, *tensors: torch.Tensor) -> None:
    if not tensors or not all(tensor.is_cuda for tensor in tensors):
        raise ValueError(f"{name} requires CUDA tensors")
    device = tensors[0].device
    if any(tensor.device != device for tensor in tensors[1:]):
        raise ValueError(f"{name} tensors must be on the same CUDA device")
...
def _qsa_unscale_block_table_kernel(
    ...
    # V2 scales valid pool offsets, but keeps its negative invalid-page
    # sentinel unchanged.  Dividing that sentinel would turn -1 into another
    # value and make an invalid entry look like a lifecycle slot.
    slot = tl.where(scaled < 0, scaled, scaled // PAGE_INDEX_SCALE)
    tl.store(
...
def triton_qsa_unscale_block_table(
    *,
    ...
    if scaled_block_table.device != block_table.device:
        raise ValueError("QSA block tables must be on the same CUDA device")
...
def triton_qsa_decode_token_mapping(
    *,
    ...
    if kv_lens.ndim != 1 or seq_lens.ndim != 1:
        raise ValueError("QSA decode lengths must be one-dimensional")
    rows = kv_lens.numel()
    ...
    if kv_lens.dtype != torch.int32 or seq_lens.dtype != torch.int32:
        raise ValueError("QSA decode lengths must use int32 storage")
    expected_dtypes = (torch.int32, torch.int64, torch.int32, torch.int32)
    if any(tensor.dtype != dtype for tensor, dtype in zip(outputs, expected_dtypes)):
        raise ValueError("QSA decode token-mapping outputs have incompatible dtypes")
    _require_same_cuda_device("QSA decode token mapping", kv_lens, seq_lens, *outputs)
    if compress_ratio <= 0:
...
def triton_qsa_paged_kv_store(
    *,
    ...
    if num_kv_heads <= 0 or head_dim <= 0:
        raise ValueError("QSA K/V store requires non-empty head geometry")
    ...
    if tokens_per_block <= 0:
        raise ValueError("QSA tokens_per_block must be positive")
    if k.dtype != v.dtype or k_cache.dtype != v_cache.dtype:
        raise ValueError("QSA K/V pairs must use matching dtypes")
    if k.dtype not in (torch.bfloat16, torch.float16):
        raise ValueError("QSA K/V inputs must use BF16 or FP16")
    if k_cache.dtype not in (torch.bfloat16, torch.float16, torch.float8_e4m3fn):
        raise ValueError("QSA K/V cache must use BF16, FP16, or FP8 E4M3")
    if request_indices.shape != (rows,) or logical_positions.shape != (rows,):
        raise ValueError("QSA K/V store metadata must have one entry per input row")
    ...
    if (
        request_indices.dtype not in _INTEGER_DTYPES
        or logical_positions.dtype not in _INTEGER_DTYPES
    ):
        raise ValueError("QSA K/V store request metadata must use integer storage")
    tensors = (
        k,
        v,
        k_cache,
    ...
    _require_same_cuda_device("QSA paged K/V store", *tensors)
    if rows == 0:
        return
    if k_cache.shape[0] == 0 or block_table.shape[0] == 0 or block_table.shape[1] == 0:
        raise ValueError("QSA K/V store requires non-empty cache and block-table geometry")
...
def _qsa_decode_pre_indexer_kernel(
    ...
    if work == NUM_QUERY_TILES:
        ...
        axes = tl.arange(0, _POSITION_AXIS_TILE)
        tl.store(
            position_cache
...
def triton_qsa_decode_pre_indexer(
    *,
    ...
    if q.ndim != 3:
        raise ValueError("QSA decode index Q must have shape [rows, heads, dim]")
    ...
    if position_coordinates.dtype not in _INTEGER_DTYPES:
        raise ValueError("QSA decode positions must use integer storage")
    if request_indices.shape != (rows,) or logical_positions.shape != (rows,):
        raise ValueError("QSA decode index metadata must have one entry per row")
    if (
        request_indices.dtype not in _INTEGER_DTYPES
        or logical_positions.dtype not in _INTEGER_DTYPES
    ):
        raise ValueError("QSA decode index metadata must use integer storage")
    if block_table.ndim != 2 or block_table.dtype != torch.int32:
        raise ValueError("QSA decode pre-indexer requires a two-dimensional int32 block table")
    if index_cache.ndim != 4 or index_cache.shape[2:] != (1, head_dim):
        raise ValueError("QSA decode index cache has incompatible geometry")
    if position_cache.ndim != 3 or position_cache.shape[2] < 3:
        raise ValueError("QSA decode position cache must contain three axes")
    if position_cache.dtype != torch.int32:
        raise ValueError("QSA decode position cache must use int32 storage")
    if index_cache.shape[1] != tokens_per_block or position_cache.shape[1] != tokens_per_block:
        raise ValueError("QSA decode side-cache pages do not match tokens_per_block")
    if q_norm_weight.numel() != head_dim or k_norm_weight.numel() != head_dim:
        raise ValueError("QSA decode norm weights do not match the index head dimension")
    if tokens_per_block <= 0:
        raise ValueError("QSA decode tokens_per_block must be positive")
    tensors = (
        q,
        token_k,
        position_coordinates,
    ...
    _require_same_cuda_device("QSA decode pre-indexer", *tensors)
    if q.dtype != torch.bfloat16 or token_k.dtype != q.dtype or index_cache.dtype != q.dtype:
        raise ValueError("QSA fused decode index Q/K and cache must use BF16")
    ...
    ratio_is_power_of_two = compress_ratio >= 2 and (compress_ratio & (compress_ratio - 1)) == 0
    ...
    if mrope_section is not None and (
        len(mrope_section) != 3 or sum(mrope_section) != rotary_pairs
    ):
        raise ValueError("QSA decode MRoPE sections do not cover the rotary width")
    if rows == 0:
        return q
...
def triton_qsa_prefill_compress(
    *,
    ...
    if logical_positions.ndim != 1:
        raise ValueError("QSA prefill request metadata must be one-dimensional")
    if (
        logical_positions.dtype not in _INTEGER_DTYPES
        or request_indices.dtype not in _INTEGER_DTYPES
    ):
        raise ValueError("QSA prefill request metadata must use integer storage")
    if block_table.ndim != 2 or block_table.dtype != torch.int32:
        raise ValueError("QSA prefill compression requires a two-dimensional int32 block table")
    ...
    if position_cache.dtype != torch.int32:
        raise ValueError("QSA prefill position cache must use int32 storage")
    if index_cache.dtype != torch.bfloat16:
        raise ValueError("QSA fused prefill index cache must use BF16")
    if index_cache.shape[1] != tokens_per_block or position_cache.shape[1] != tokens_per_block:
        raise ValueError("QSA prefill side-cache pages do not match tokens_per_block")
    if cos_sin.ndim != 2 or cos_sin.shape[1] % 2 != 0:
        raise ValueError("QSA RoPE cache must be a packed 2D cos/sin tensor")
    ...
    if k_norm_weight.numel() != head_dim:
        raise ValueError("QSA prefill norm weight does not match the index head dimension")
    if tokens_per_block <= 0:
        raise ValueError("QSA prefill tokens_per_block must be positive")
    _require_same_cuda_device(
        "QSA prefill compression",
        logical_positions,
        request_indices,
        block_table,
        index_cache,
    ...
    if mrope_section is not None and (
        ...
        raise ValueError("QSA prefill MRoPE sections do not cover the rotary width")
    if rows == 0:
        return
    section = mrope_section if mrope_section is not None else (0, 0, 0)
...
def triton_expand_qsa_block_indices(
    ...
    *,
    ...
    """Expand prefix-packed Top-K block IDs without Torch temporaries.

    Valid block IDs must precede the ``-1`` padding, as guaranteed by the
    CUDA Top-K producer. This lets the kernel place the incomplete causal tail
    directly after the expanded valid groups.
    """
    if block_indices.ndim != 2:
        raise ValueError("QSA block indices must have shape [rows, block_topk]")
    if compress_ratio < 2 or token_topk <= 0 or token_topk % compress_ratio:
        raise ValueError("QSA expansion requires token_topk divisible by compress_ratio >= 2")
    rows, block_topk = block_indices.shape
    if block_topk != token_topk // compress_ratio:
        raise ValueError("QSA block-index width does not match the selection geometry")
    if query_positions.shape != (rows,) or sequence_lengths.shape != (rows,):
        raise ValueError("QSA expansion metadata must have one entry per row")
    if block_indices.dtype != torch.int32:
        raise ValueError("QSA block indices must use int32 storage")
    if (
        query_positions.dtype not in _INTEGER_DTYPES
...
def _qsa_paged_index_scores_kernel(
    ...
    query_values = tl.load(
        ...
        mask=(dim_offsets[:, None] < INDEX_HEAD_DIM) & (head_offsets[None, :] < NUM_INDEX_HEADS),
        other=0.0,
    )
    ...
    for tile in tl.range(first_tile, last_tile, num_stages=2):
        ...
        keys = tl.load(
            ...
            mask=valid_blocks[:, None] & (dim_offsets[None, :] < INDEX_HEAD_DIM),
            other=0.0,
...
def _qsa_paged_index_scores_qtile_kernel(
    ...
    query_values = tl.load(
        ...
        mask=(dim_offsets[:, None] < INDEX_HEAD_DIM) & present[None, :],
        other=0.0,
    )
    column_offsets = tl.arange(0, BLOCK_N)
...
def triton_qsa_paged_index_scores(
    *,
    ...
    """
    if q.ndim != 3:
        raise ValueError("QSA index queries must have shape [rows, heads, dim]")
    if index_cache.ndim != 4 or index_cache.shape[2] != 1:
        raise ValueError("QSA index cache must have shape [pages, tokens, 1, dim]")
    if block_table.ndim != 2 or block_table.dtype != torch.int32:
        raise ValueError("QSA index scoring requires a two-dimensional int32 block table")
    rows, num_index_heads, index_head_dim = q.shape
    if num_index_heads <= 0 or index_head_dim <= 0:
        raise ValueError("QSA index scoring requires non-empty head geometry")
    if index_cache.shape[3] != index_head_dim:
        raise ValueError("QSA index query and cache dimensions do not match")
    if q.dtype != index_cache.dtype:
        raise ValueError("QSA index query and cache dtypes must match")
    if query_positions.shape != (rows,) or request_indices.shape != (rows,):
        raise ValueError("QSA index-scoring metadata must have one entry per row")
    if query_positions.dtype not in _INTEGER_DTYPES or request_indices.dtype not in _INTEGER_DTYPES:
        raise ValueError("QSA index-scoring metadata must use integer storage")
    if tokens_per_block <= 0 or compress_ratio < 2:
        raise ValueError("QSA index scoring requires positive pages and compression ratio >= 2")
    _require_same_cuda_device(
        "QSA paged index scoring", q, index_cache, block_table, query_positions, request_indices
    )
    max_compressed_blocks = block_table.shape[1] * tokens_per_block // compress_ratio
    if max_compressed_blocks <= 0:
        raise ValueError("QSA index scoring requires a non-empty block table")
    output = torch.empty(
    ...
    if rows == 0:
        return output
    # Decode favors many narrow score tiles; large prefill batches reuse each
    # query across a deeper column range to reduce launch and gather overhead.
    is_prefill_sized = rows > 256
    block_n = 64 if is_prefill_sized else 8
    tiles_per_program = 8 if is_prefill_sized else 1
    query_tile = 1
    if context_rows and is_prefill_sized:
        query_tile = _qsa_index_scores_query_tile(num_index_heads)
    if query_tile > 1:
        # Row packing reduces the grid, so each program walks a deeper column
        # range to retain enough work per launch.
        ...
        _qsa_paged_index_scores_qtile_kernel[grid](
            ...
            BLOCK_D=max(16, triton.next_power_of_2(index_head_dim)),
            TOKENS_PER_BLOCK=tokens_per_block,
            COMPRESS_RATIO=compress_ratio,
...
def _qsa_paged_sparse_gqa_kernel(
    ...
    for tile in tl.range(0, valid_tiles, num_stages=2):
        ...
        scores = tl.dot(query_values, keys) * softmax_scale * _LOG2_E
...
def triton_qsa_paged_sparse_gqa(
    *,
    ...
    if q.ndim != 3:
        raise ValueError("QSA queries must have shape [rows, query_heads, head_dim]")
    if k_cache.ndim != 4 or v_cache.shape != k_cache.shape:
        raise ValueError("QSA K/V caches must have matching HND paged shapes")
    if block_table.ndim != 2 or block_table.dtype != torch.int32:
        raise ValueError("QSA sparse GQA requires a two-dimensional int32 block table")
    if selected_tokens.ndim != 2 or selected_tokens.dtype != torch.int32:
        raise ValueError("QSA selected tokens must be a two-dimensional int32 tensor")
    rows, num_q_heads, head_dim = q.shape
    if selected_tokens.shape[0] != rows or selected_tokens.shape[1] <= 0:
        raise ValueError("QSA selected-token rows must match non-empty query rows")
    if request_indices.shape != (rows,) or request_indices.dtype not in _INTEGER_DTYPES:
        raise ValueError("QSA request indices must have one integer entry per row")
    if not request_indices.is_contiguous():
        raise ValueError("QSA request indices must be contiguous")
    if tokens_per_block <= 0 or k_cache.shape[2] != tokens_per_block:
        raise ValueError("QSA cache token dimension must match positive tokens_per_block")
    if k_cache.shape[3] != head_dim or k_cache.dtype != v_cache.dtype:
        raise ValueError("QSA query and K/V cache head geometry is incompatible")
    if q.dtype not in (torch.bfloat16, torch.float16):
        raise ValueError("QSA queries must use BF16 or FP16")
    if k_cache.dtype not in (torch.bfloat16, torch.float16, torch.float8_e4m3fn):
        raise ValueError("QSA K/V cache must use BF16, FP16, or FP8 E4M3")
    if head_dim <= 0 or head_dim & (head_dim - 1):
        raise ValueError("QSA sparse GQA requires a power-of-two head dimension")
    num_kv_heads = k_cache.shape[1]
    if num_kv_heads <= 0 or num_q_heads <= 0 or num_q_heads % num_kv_heads:
        raise ValueError("QSA query heads must be divisible by non-zero KV heads")
    if k_cache.shape[0] <= 0 or block_table.shape[0] <= 0 or block_table.shape[1] <= 0:
        raise ValueError("QSA sparse GQA requires non-empty cache and block-table geometry")
    if not math.isfinite(softmax_scale) or softmax_scale <= 0:
        raise ValueError("QSA sparse GQA softmax scale must be finite and positive")
    _require_same_cuda_device(
        "QSA paged sparse GQA",
        q,
        k_cache,
        v_cache,
        block_table,
    ...
    if query_positions is not None:
        ...
        if query_positions.dtype not in _INTEGER_DTYPES:
            raise ValueError("QSA query positions must use integer storage")
        _require_same_cuda_device("QSA causal-prefix bounds", q, query_positions)
        if compress_ratio <= 0:
