...
from ..dsa.indexer import HAS_FAST_HADAMARD
...
class DeepseekV4VanillaIndexer:
    """FP32 golden for DeepSeek-V4 indexer scoring."""

    def __init__(self, indexer: DeepseekV4Indexer) -> None:
    ...
    def _hadamard(x: torch.Tensor) -> torch.Tensor:
        ...
        stride = 1
        while stride < width:
            indices = torch.arange(width, device=x.device)
            lower = (indices & stride) == 0
            upper = indices ^ stride
            a = y[..., lower].clone()
            b = y[..., upper[lower]].clone()
            y[..., lower] = a + b
            y[..., upper[lower]] = a - b
            stride <<= 1
    ...
    def _mxfp4_quant_dequant(
        ...
        *,
        ...
        scaled = torch.clamp(blocks.abs().amax(dim=-1, keepdim=True), min=min_amax) / 6.0
        bits = scaled.contiguous().view(torch.int32)
        log2_ceil = ((bits >> 23) & 0xFF) - 127 + (bits & 0x7FFFFF).ne(0)
        scale = ((log2_ceil + 127) << 23).view(torch.float32)

        normalized = (blocks / scale).abs().clamp(max=6.0)
        thresholds = torch.tensor([0.0, 0.25, 0.75, 1.25, 1.75, 2.5, 3.5, 5.0], device=x.device)
        value_indices = torch.zeros_like(normalized, dtype=torch.uint8)
        for level in range(1, len(thresholds)):
            take_upper = normalized > thresholds[level]
            if round_ties_to_even:
                take_upper |= (normalized == thresholds[level]) & (level % 2 == 0)
            value_indices = torch.where(
                take_upper, torch.full_like(value_indices, level), value_indices
            )
        values = fp4_values[value_indices.to(torch.long)]
        values = torch.where(torch.signbit(blocks), -values, values)
        return (values * scale).reshape_as(x)

    def _prepare_fp4(self, x: torch.Tensor, *, is_query: bool) -> torch.Tensor:
        if HAS_FAST_HADAMARD:
            x = self._hadamard(x)
        return self._mxfp4_quant_dequant(
            x,
            round_ties_to_even=not is_query,
            min_amax=1.0e-12 if is_query else 6.0 * torch.finfo(torch.float32).tiny,
        )
    ...
    def project_query(
        ...
        self._apply_rope(q[..., -self.indexer.rope_dim :], freqs_cis[position_ids.long()])
        q = q.squeeze(0)
        return self._prepare_fp4(q, is_query=True) if self.uses_fp4 else q

    @staticmethod
    def _apply_rope(x: torch.Tensor, freqs_cis: torch.Tensor) -> None:
        complex_x = torch.view_as_complex(x.float().unflatten(-1, (-1, 2)))
        freqs_cis = freqs_cis.view(1, complex_x.size(1), 1, complex_x.size(-1))
        x.copy_(torch.view_as_real(complex_x * freqs_cis).flatten(-2))

    def prepare_key(self, k: torch.Tensor) -> torch.Tensor:
        return self._prepare_fp4(k, is_query=False) if self.uses_fp4 else k
