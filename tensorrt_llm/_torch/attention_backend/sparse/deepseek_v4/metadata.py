...
class DeepseekV4TrtllmAttentionMetadata(DSAtrtllmAttentionMetadata):
    ...
    def prepare_for_draft_replay(self) -> dict:
        ...
        dsv4_saved = self.prepare_for_draft_forward()
        return {"dsa": dsa_saved, "dsv4": dsv4_saved}

    def restore_after_draft_replay(self, saved_state: dict | None) -> None:
        if saved_state is None:
            return
        super().restore_after_draft_replay(saved_state["dsa"])
        self.restore_after_draft_forward(saved_state["dsv4"])
    ...
    def prepare(self):
        ...
        # Set the draft manager's table count before the generic prepare copies
        # its block offsets; switching to draft state is then copy-only.
        draft_mgr = self.draft_kv_cache_manager
    ...
    def _compute_token_positions(
        ...
        # DSA metadata rebuilt this map for the current seq_lens.
