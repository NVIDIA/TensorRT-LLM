...
class Fp4QuantizedTensor:
    ...
    # When the FP4 tensor is produced by a fused (add+)RMSNorm+NVFP4-quant that
    # also returns the un-quantized (post-RMSNorm) value, this carries that
    # BF16 tensor so downstream consumers needing the un-quantized form (e.g.
    # DSv3.2's DSA indexer at sparse/dsa.py:pre_indexer_proj) can use it
    # without dequantizing FP4.
