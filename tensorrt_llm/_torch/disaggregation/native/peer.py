...
class PeerRegistrar:
    ...
    def frags_per_block_per_group(self, peer_ri: RankInfo) -> Dict[int, int]:
        ...
        for self_key, peer_key in self.get_pool_mapping(peer_ri).items():
            if self._self_ext_cache.page_table.layer_groups[self_key[0]].kind != CacheKind.PAGED:
                continue
    ...
    def fan_in_replicated_owners(self, peer_ri: RankInfo, sender_ranks: List[int]) -> List[bool]:
        ...
        Mirrors each sender's own ``should_send_pool`` using the CP-minor rank layout.
        """
        return [
            self.elects_fan_in_owner(
                sender_tp_rank=(rank // peer_ri.cp_size) % peer_ri.tp_size,
