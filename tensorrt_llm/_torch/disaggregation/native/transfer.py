...
class RecvReqInfo:
    ...
    instance_name: str
    instance_rank: int
    # Positional pool slots per layer group (see KVSlice): entry i is the
    ...
    # Beam search (V1 only): divergent per-beam final blocks, paired index-wise.
        ...
        return msgpack.packb(
            {
                ...
                "beam_tails_per_layer_groups": (
                    None
                    if self.beam_tails_per_layer_groups is None
                    else [arr.tobytes() for arr in self.beam_tails_per_layer_groups]
                ),
            }
        )
    ...
    def from_bytes(cls, data: bytes) -> "RecvReqInfo":
        ...
        tails = d.get("beam_tails_per_layer_groups")
        if tails is not None:
            d["beam_tails_per_layer_groups"] = [
                np.frombuffer(b, dtype=np.int64).copy() for b in tails
            ]
        return cls(**d)
...
class Sender(SenderBase):
    ...
    def _pair_ordinals(
        ...
        Both tables are indexed by block ordinal (see KVSlice); an entry is -1
    ...
    @staticmethod
    def _beam_tails(tails_per_group: Optional[list[np.ndarray]], group: int) -> np.ndarray:
        if tails_per_group is None or group >= len(tails_per_group):
            return np.array([], dtype=np.int64)
        return np.asarray(tails_per_group[group], dtype=np.int64)

    @staticmethod
    def _pair_beam_tails(
        src_tails: np.ndarray, dst_tails: np.ndarray
    ) -> tuple[np.ndarray, np.ndarray]:
        """Pair divergent beam tails index-wise; either side may opt out with an empty list."""
        if src_tails.size == 0 or dst_tails.size == 0:
            empty = np.array([], dtype=np.int64)
            return empty, empty
        if src_tails.size != dst_tails.size:
            raise ValueError(f"beam tail count mismatch: src={src_tails.size} dst={dst_tails.size}")
        return src_tails, dst_tails
    ...
    def _build_kv_write_meta(self, task: KVSendTask, req_info: RecvReqInfo) -> WriteMeta:
        ...
        for (self_lg, self_pi), (peer_lg, peer_pi) in pool_mapping.items():
            ...
            if lg_info.kind == CacheKind.STATE:
                ...
                src_tails, dst_tails = Sender._pair_beam_tails(
                    Sender._beam_tails(task._slice.beam_tails_per_layer_groups, self_lg),
                    Sender._beam_tails(req_info.beam_tails_per_layer_groups, peer_lg),
                )
                if src_tails.size:
                    src_block_ids = np.concatenate([src_block_ids, src_tails])
                    dst_block_ids = np.concatenate([dst_block_ids, dst_tails])
...
class Receiver(ReceiverBase):
    ...
    def _build_recv_req_info(self, task: KVRecvTask) -> RecvReqInfo:
        ...
        return RecvReqInfo(
            ...
            beam_tails_per_layer_groups=task._kv_slice.beam_tails_per_layer_groups,
