...
class _AgentOperationGate:
    ...
    def acquire_transfer(self) -> Callable[[], None]:
        with self._condition:
            self._condition.wait_for(lambda: self._closing or not self._metadata_pending)
            if self._closing:
    ...
    def metadata(self) -> Iterator[None]:
        with self._condition:
            ...
            if self._metadata_pending:
                raise RuntimeError("concurrent NIXL metadata mutation")
            self._metadata_pending = True
            self._condition.wait_for(lambda: self._active_transfers == 0)
        try:
            yield
        finally:
            with self._condition:
    ...
    def close(self) -> None:
        with self._condition:
            ...
            self._condition.wait_for(
                lambda: not self._metadata_pending and self._active_transfers == 0
            )
...
class _ReceiveOperationOwner:
    ...
    def mark_publication_ambiguous(self) -> None:
        with self._lock:
...
class Sender(SenderBase):
    ...
    def __init__(
        ...
        self._agent_operation_gate = _AgentOperationGate()
    ...
    def _submit_transfer(
        ...
        with self._ownership_poison_lock:
            if self._ownership_poisoned is not None:
                raise RuntimeError("NIXL sender is quarantined") from self._ownership_poisoned
        ...
        try:
            ...
            if not status.wait():
                ...
                with self._ownership_poison_lock:
                    self._ownership_poisoned = RuntimeError(
                        f"NIXL transfer outcome is ambiguous: {detail}"
                    )
                return False, detail
            return True, None
    ...
    def _deliver_aux_to_agent(self, write_meta: WriteMeta):
        ...
        if write_meta.src_ptrs.size > 0:
            ...
            if timer:
                timer.record_transfer_start(write_meta.peer_rank)
            transfer_finished, _ = self._submit_transfer(aux_task, write_meta.peer_rank, request)
            if transfer_finished:
                del request
                aux_task.finish_physical_operation(write_meta.peer_rank)
            if not transfer_finished:
                ...
                if owned:
                    raise RuntimeError("aux transfer outcome is ambiguous; retaining source")
            if timer:
                timer.record_transfer_end(write_meta.peer_rank)
        elif owned:
            aux_task.finish_physical_operation(write_meta.peer_rank)
    ...
    def _register_peer_rank(self, _send_id: bytes, message: list[bytes]):
        ...
        with self._agent_operation_gate.metadata():
    ...
    def _send_failed_task_result_to_receiver(self, task: SendTaskBase, info: RecvReqInfo) -> None:
        ...
        try:
            ...
            if isinstance(task, KVSendTask):
                ...
                message = [MessageType.AUX_AGENT_RESULT] + [
                    str(value).encode("ascii")
                    for value in (self._instance_rank, info.unique_rid, AgentResult.FAILED.value)
                ]
...
class KVRecvTask:
    ...
    def mark_publication_ambiguous(self) -> None:
        self._get_physical_owner().mark_publication_ambiguous()
...
class Receiver(ReceiverBase):
    ...
    def dispatch_task(self, task: KVRecvTask) -> None:
        ...
        candidate_cohorts: Optional[list[set[int]]] = None
        try:
        ...
        if sender_dp_rank is not None:
            ...
            candidate_cohorts = [set(dp0_overlap.ranks)]
            all_ranks_set: set[int] = set(candidate_cohorts[0])
            for dp in range(1, dp_size):
                cohort = set(self._registrar.get_peer_overlap(peer_infos, dp).ranks)
                candidate_cohorts.append(cohort)
                all_ranks_set.update(cohort)
        ...
        if not session.try_begin_transfer(
            task.slice_id,
            sender_endpoints,
            set(peer_ranks) if candidate_cohorts is None else None,
...
class RxSession(RxSessionBase):
    def __init__(
        ...
        self._candidate_by_writer: dict[int, frozenset[int]] = {}
        self._selected_writer_cohort: Optional[frozenset[int]] = None
        self._cohort_ambiguous = False
        ...
        *,
        ...
        with self.lock:
            ...
            if candidate_cohorts is None:
                ...
                if not self._candidate_by_writer:
                    cohorts = tuple(frozenset(cohort) for cohort in candidate_cohorts)
                    if not cohorts or any(
                        len(cohort) != task.expected_transfers for cohort in cohorts
                    ):
                        raise ValueError("invalid candidate writer cohorts")
                    self._candidate_by_writer = {
                        writer: cohort for cohort in cohorts for writer in cohort
                    }
                if self._selected_writer_cohort is not None:
                    task.seal_writer_cohort(set(self._selected_writer_cohort))
            task.status = TaskStatus.TRANSFERRING
            ...
            if publish is not None:
                ...
                try:
                    ...
                    # A send may have escaped before a later peer failed.
                    self._cohort_ambiguous = True
                    task.mark_publication_ambiguous()
                    if self._aux_physical_owner is not None:
                        self._aux_physical_owner.mark_publication_ambiguous()
                    raise
            return True
    ...
    def _accept_candidate_writer(self, peer_rank: int, succeeded: bool) -> bool:
        if not self._candidate_by_writer:
            return True
        cohort = self._candidate_by_writer.get(peer_rank)
        selected = self._selected_writer_cohort
        ambiguous = cohort is None or selected is None and not succeeded
        ambiguous |= selected is not None and peer_rank not in selected and succeeded
        if self._cohort_ambiguous or ambiguous:
            self._cohort_ambiguous = True
            raise RuntimeError(f"ambiguous ADP writer-cohort evidence from rank {peer_rank}")
        if selected is None:
            assert cohort is not None
            self._selected_writer_cohort = selected = cohort
            for task in self._kv_tasks:
                if task.status == TaskStatus.TRANSFERRING:
                    task.seal_writer_cohort(set(selected))
            if self._aux_physical_owner is not None:
                self._aux_physical_owner.seal_writer_cohort(len(selected), set(selected))
        return peer_rank in selected
        ...
        with self.lock:
            ...
            if self._enforce_physical_ownership:
                if status == AgentResult.FAILED or is_last_slice:
                    try:
                        if not self._accept_candidate_writer(
    ...
    def process_aux_agent_result(self, peer_rank: int, status: AgentResult):
        ...
        with self.lock:
            ...
            if self._aux_physical_owner is not None:
                try:
                    if not self._accept_candidate_writer(peer_rank, status == AgentResult.SUCCESS):
                        return
    ...
    def resources_drained(self) -> bool:
        ...
        return kv_drained and aux_drained and not self._cohort_ambiguous
