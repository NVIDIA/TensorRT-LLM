...
def build_page_table(kv_cache_manager: KVCacheManager) -> KVCachePageTable:
    ...
    for group_id, window_size in enumerate(sorted_window_sizes):
        ...
        # every TP rank (single index head), so its view is REPLICATED: head
        # matching is skipped and the layer set is read from buffer_entries.
        # With a per-layer indexer mask (cross-layer indexer sharing, e.g.
        # GLM 5.2) only the "full" indexer-owning layers get a pool row, so the
        # view covers that subset: one buffer entry per owning layer, each
        # mapped to its packed row in the (possibly masked) pool. When the mask
        # is absent every layer owns a row (dense/legacy layout) and this
        # reduces to the equal-sized packing in local-layer order.
