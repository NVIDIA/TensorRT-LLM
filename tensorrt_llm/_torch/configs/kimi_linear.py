...
class KimiLinearConfig(PretrainedConfig):
    ...
    @property
    def is_mla(self):
        return (self.q_lora_rank is not None or self.kv_lora_rank is not None
                or self.qk_nope_head_dim is not None
                or self.qk_rope_head_dim is not None
                or self.v_head_dim is not None or self.mla_use_nope is True)
    ...
    def is_linear_attn(self) -> bool:
        return not (self.linear_attn_config is None
                    or (isinstance(self.linear_attn_config, dict)
    ...
    def is_full_attn_layer(self, layer_idx: int) -> bool:
        ...
        return (self.linear_attn_config is not None and
                (layer_idx + 1) in self.linear_attn_config["full_attn_layers"])
