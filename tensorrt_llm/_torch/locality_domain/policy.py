...
class LocalityDomainExecutionPlanner:
    ...
    def plan_linear(
        ...
        *,
        ...
        # The locality-domain path requires the Rubin CuTe DSL kernel set.
        ...
        if not IS_CUTLASS_DSL_RUBIN_AVAILABLE:
            return LinearPartitionPlan(
                enabled=False, reason_if_disabled="CuTe DSL Rubin kernels not available"
            )
    ...
    def plan_bf16_bmm(
        ...
        *,
        ...
        if not IS_CUTLASS_DSL_RUBIN_AVAILABLE:
            return PartitionPlan(
                enabled=False,
                reason_if_disabled="CuTe DSL Rubin kernels not available",
            )
        return PartitionPlan(
            enabled=True,
            num_partitions=self.policy.num_partitions,
